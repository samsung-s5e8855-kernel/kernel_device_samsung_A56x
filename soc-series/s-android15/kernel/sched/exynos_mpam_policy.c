...
#include "../../drivers/soc/samsung/mpam/exynos-mpam.h"

MODULE_LICENSE("GPL");
MODULE_AUTHOR("Yeonghwan Son <yhwan.son@samsung.com>");
MODULE_SOFTDEP("mpam_policy pre: exynos-mpam");
...
static int mpam_fs_create_files(struct super_block *sb)
{
	...
	mpam_fs.partitions_count = mpam_get_partid_count(NULL);
	...
	for (i = 0; i < mpam_get_partid_count(NULL); i++) {
