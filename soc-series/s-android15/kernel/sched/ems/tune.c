...
static void parse_cpufreq_gov(struct device_node *dn, struct emstune_cpufreq_gov *cpufreq_gov)
{
	...
	parse_coregroup_field(dn, "pelt-margin", cpufreq_gov->split_pelt_margin, 25);
	parse_coregroup_field(dn, "pelt-margin-freq", cpufreq_gov->split_pelt_margin_freq, INT_MAX);
	...
	parse_coregroup_field(dn, "up-rate-limit-ms", cpufreq_gov->split_up_rate_limit, 4);
	parse_coregroup_field(dn, "up-rate-limit-freq", cpufreq_gov->split_up_rate_limit_freq, INT_MAX);
}
...
enum {
	...
	cpufreq_gov_up_rate_limit,
	cpufreq_gov_up_rate_limit_freq,
	ontime_en,
	ontime_boundary,
...
struct aio_tuner_item aio_tuner_items[] = {
	...
	{ I(cpufreq_pelt_margin),	CPU_ONLY, SIGNED_TYPE, MAX_RATIO,},
	{ I(cpufreq_pelt_margin_freq),	CPU_ONLY, UNSIGNED_TYPE, INT_MAX},
	...
	{ I(cpufreq_gov_up_rate_limit),	CPU_ONLY, SIGNED_TYPE, MAX_RATIO,},
	{ I(cpufreq_gov_up_rate_limit_freq), CPU_ONLY, UNSIGNED_TYPE, INT_MAX},
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	case cpufreq_pelt_boost:
		...
		break;
	case cpufreq_pelt_margin:
		set_value_coregroup(set->cpufreq_gov.split_pelt_margin, cpu, value);
		break;
	case cpufreq_pelt_margin_freq:
		set_value_coregroup(set->cpufreq_gov.split_pelt_margin_freq, cpu, value);
		break;
	case cpufreq_gov_up_rate_limit:
		set_value_coregroup(set->cpufreq_gov.split_up_rate_limit, cpu, value);
		break;
	case cpufreq_gov_up_rate_limit_freq:
		set_value_coregroup(set->cpufreq_gov.split_up_rate_limit_freq, cpu, value);
		break;
		...
		struct bin_attribute *attr, char *buf, loff_t offset, size_t size)
{
	...
	count += sprintf(msg + count, "      %12s  %10s  %12s %12s  %20s  %20s  %25s\n",
	"pelt_boost", "ptb_min_freq", "ptb_margin", "window_count", "htask_ratio_threshold", "split_pelt_margin", "split_pelt_margin_freq");
	for_each_possible_cpu(cpu) {
		count += sprintf(msg + count, "cpu%d: %12d    %10d  %12d %12d   %20d  %20d", cpu,
		...
		if (cur_set->cpufreq_gov.split_pelt_margin_freq[cpu] == INT_MAX)
			count += sprintf(msg + count, "  %25s\n", "INF");
		else
			count += sprintf(msg + count, "  %25d\n", cur_set->cpufreq_gov.split_pelt_margin_freq[cpu]);
	}
	count += sprintf(msg + count, "\n");
	...
	for_each_possible_cpu(cpu) {
		if (cur_set->cpufreq_gov.split_up_rate_limit_freq[cpu] == INT_MAX)
			count += sprintf(msg + count, "cpu%d:%4d %12s\n",
				cpu, cur_set->cpufreq_gov.split_up_rate_limit[cpu],
				"INF");
		else
			count += sprintf(msg + count, "cpu%d:%4d %12d\n",
				cpu, cur_set->cpufreq_gov.split_up_rate_limit[cpu],
