...
void taskgroup_enqueue_task(struct rq *rq, struct task_struct *p, int *flags)
{
	unsigned long uclamp_min_util;

	uclamp_min_util = uclamp_eff_value(p, UCLAMP_MIN);

	if (uclamp_min_util > 0)
		*flags |= UCLAMP_TASK;
}
