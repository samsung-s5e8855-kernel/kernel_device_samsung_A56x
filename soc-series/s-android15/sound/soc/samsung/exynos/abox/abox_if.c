...
struct abox_if_data * uaif0_data;

enum abox_if_quirk {
	ABOX_IF_QUIRK_DELAYED_STOP,
	ABOX_IF_QUIRK_CHUB_GPIO,
	ABOX_IF_QUIRK_COUNT,
};
...
static void abox_if_bclk_sanity_check(struct abox_if_data *data, unsigned long target_bclk)
{
	struct device *dev = data->cmpnt->dev;
	unsigned long current_bclk;

	abox_dbg(dev, "%s\n", __func__);

	current_bclk = clk_get_rate(data->clk_bclk);
	if (abs(target_bclk - current_bclk) > BCLK_MARGIN) {
		abox_warn(dev, "current uaif%d bclk: %lu(target: %lu)\n",
					data->id, current_bclk, target_bclk);
		abox_oem_noise_detect(data->abox_data, ABOX_OEM_CLK);
	}
}
...
static void abox_if_cfg_gpio(struct device *dev, struct abox_if_data *data,
		const char *name)
{
...
void abox_uaif0_cfg_gpio(bool on)
{
	int ret = 0;
	abox_info(uaif0_data->cmpnt->dev, "%s(%d)\n", __func__, on);

	if(on) {
		if (pm_runtime_enabled(uaif0_data->cmpnt->dev))
			pm_runtime_disable(uaif0_data->cmpnt->dev);
		ret = clk_set_rate(uaif0_data->clk_mclk, 24576000);
		if (ret < 0)
			abox_err(uaif0_data->cmpnt->dev, "mclk set error: %d\n", ret);
		dev_info(uaif0_data->cmpnt->dev, "mclk rate %lu\n", clk_get_rate(uaif0_data->clk_mclk));

		ret = clk_enable(uaif0_data->clk_mclk);
		if (ret < 0)
			dev_err(uaif0_data->cmpnt->dev, "Failed to enable mclk: %d\n", ret);
		abox_if_cfg_gpio(uaif0_data->cmpnt->dev, uaif0_data, "active");
	} else {
		abox_if_cfg_gpio(uaif0_data->cmpnt->dev, uaif0_data, "idle");
		clk_disable(uaif0_data->clk_mclk);
		if (!pm_runtime_enabled(uaif0_data->cmpnt->dev))
			pm_runtime_enable(uaif0_data->cmpnt->dev);
	}
}
		...
		struct snd_soc_dai *dai)
{
	...
	abox_if_cfg_gpio(dev, data, "active");
err:
	return ret;
}
		...
		struct snd_soc_dai *dai)
{
	...
	ret = snd_soc_component_update_bits(cmpnt, UAIF_REG_CTRL0,
				mask, 0 << shift);
	if (ret < 0)
		abox_err(dev, "%s, Update register error: %d\n", __func__, ret);
		...
		struct snd_soc_dai *dai)
{
	...
	if (abox_if_test_quirk(data->quirks, ABOX_IF_QUIRK_DELAYED_STOP)) {
		...
		udelay(time);
		abox_info(dev, "delayed stop: %lu us\n", time);
	}
		...
		struct snd_pcm_hw_params *hw_params, struct snd_soc_dai *dai)
{
	...
	abox_if_bclk_sanity_check(data, bclk_rate);
...
static int abox_uaif_mute_stream(struct snd_soc_dai *dai, int mute, int stream)
{
	...
	struct snd_soc_component *cmpnt = data->cmpnt;
	unsigned long mask, shift;
	int vol_factor = UAIF_VOL_FACTOR; 	/*default value*/
	int ret = 0;
	...
	if (stream == SNDRV_PCM_STREAM_CAPTURE) {
		...
		mask = ABOX_SPK_ENABLE_MASK;
		shift = ABOX_SPK_ENABLE_L;
		ret = snd_soc_component_update_bits(cmpnt, UAIF_REG_SPK_VOL_FACTOR,
						ABOX_VOL_FACTOR_SPK_MASK, vol_factor << ABOX_VOL_FACTOR_SPK_L);
		usleep_range(50, 100); /*Processing time to complete mute*/
	}
		...
		int clk_id, unsigned int freq, int dir)
{
	...
	unsigned int rate, width, channels, bclk, mux_rate;
	int ret;
	...
	abox_info(dev, "%s: mux=%lu\n", __func__, clk_get_rate(data->clk_mux));
	...
	abox_if_bclk_sanity_check(data, bclk);
...
static SOC_ENUM_SINGLE_DECL(abox_uaif_mic_func_enum, UAIF_REG_CTRL0,
		ABOX_MIC_FUNC_L, uaif_func_enum_texts);
static SOC_ENUM_SINGLE_DECL(abox_uaif_spk_func_enum, UAIF_REG_CTRL0,
		ABOX_SPK_FUNC_L, uaif_func_enum_texts);
static const char * const uaif_func_status_enum_texts[] = {
	"Idle", "Normal", "Fade out", "Mute", "Fade in",
};
static const unsigned int uaif_func_status_enum_values[] = {
	0, 1, 2, 3, 5,
};
static SOC_VALUE_ENUM_SINGLE_DECL(abox_uaif_mic_func_status_enum, UAIF_REG_STATUS,
		UAIF_VOL_STATUS_OF_MIC_SHIFT, UAIF_VOL_STATUS_OF_MIC_MASK,
		uaif_func_status_enum_texts, uaif_func_status_enum_values);
static SOC_VALUE_ENUM_SINGLE_DECL(abox_uaif_spk_func_status_enum, UAIF_REG_STATUS,
		UAIF_VOL_STATUS_OF_SPK_SHIFT, UAIF_VOL_STATUS_OF_SPK_MASK,
		uaif_func_status_enum_texts, uaif_func_status_enum_values);
static const struct snd_kcontrol_new abox_uaif_vol_controls[] = {
	...
	SOC_ENUM("Mic Func Status", abox_uaif_mic_func_status_enum),
	...
	SOC_ENUM("Spk Func Status", abox_uaif_spk_func_status_enum),
...
int abox_chub_pinctrl_event(struct snd_soc_dapm_widget *w,
...
#endif
	ret = dapm_pinctrl_event(w, kcontrol, event);
#if IS_ENABLED(CONFIG_NANOHUB)
...
#define ABOX_CHUB_DAPM_PINCTRL(wname, active, sleep) \
{	.id = snd_soc_dapm_pinctrl, .name = wname, \
	.priv = (&(struct snd_soc_dapm_pinctrl_priv) \
		{ .active_state = active, .sleep_state = sleep,}), \
	.reg = SND_SOC_NOPM, .event = abox_chub_pinctrl_event, \
	.event_flags = SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD }

static const struct snd_soc_dapm_widget abox_if_chub_pinctrl_widgets[] = {
	ABOX_CHUB_DAPM_PINCTRL("PINCTRL", "active", "idle"),
};
...
static int abox_if_cmpnt_probe(struct snd_soc_component *cmpnt)
{
	...
	if (!IS_ERR(p)) {
		 ...
		 */
		if (abox_if_test_quirk(data->quirks, ABOX_IF_QUIRK_CHUB_GPIO))
			ret = snd_soc_dapm_new_controls(dapm, abox_if_chub_pinctrl_widgets,
					ARRAY_SIZE(abox_if_chub_pinctrl_widgets));
		if (ret < 0)
...
static int abox_if_runtime_suspend(struct device *dev)
{
	...
	abox_if_disable_qchannel(data, 0);
	abox_if_cfg_gpio(dev, data, "idle");
