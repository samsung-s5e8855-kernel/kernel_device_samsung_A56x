...
bool abox_dma_func_status_check(struct snd_soc_component *cmpnt, unsigned int mute){
	struct device *dev = cmpnt->dev;
	...
	/* only normal set */
	if(!mute) {
		func_status = snd_soc_component_read(cmpnt, DMA_REG_STATUS_FUNC);
		func_status = func_status & ABOX_DMA_VOL_STATUS_MASK;
		abox_info(dev, "%s: func_status(%d)\n", __func__, func_status);
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	if (abox_dma_enable_check(cmpnt)) {
		...
		while (!abox_dma_func_status_check(cmpnt, item[0])) {
			usleep_range(100, 200);
			snd_soc_component_update_bits(cmpnt, DMA_REG_CTRL,
					ABOX_DMA_FUNC_MASK, 2 << ABOX_DMA_FUNC_L);
...
static const char * const abox_dma_func_status_texts[] = {
	"Normal", "Pend", "Mute", "Fade out", "Fade in",
};
...
static int abox_dma_dump_set_format(struct abox_dma_data *data)
{
	...
	case ABOX_WIDGET_SPUS_SSRC0 ... ABOX_WIDGET_SPUS_SSRC3:
		...
		format = abox_cmpnt_asrc_get_dst_format(abox_data,
				SNDRV_PCM_STREAM_PLAYBACK, idx);
		break;
	case ABOX_WIDGET_NSRC0 ... ABOX_WIDGET_NSRC15:
...
static enum abox_widget abox_ddma_get_src_widget(struct abox_dma_data *data)
{
	...
	case 0x0 ... 0xf:
		...
		break;
	case 0x20 ... 0x27:
		w = ABOX_WIDGET_SPUS_ASRC0 + val - 0x20;
		break;
	case 0x28 ... 0x2b:
		w = ABOX_WIDGET_SPUS_SSRC0 + val - 0x28;
		break;
	case 0x40:
		w = ABOX_WIDGET_SIFS0 + val - 0x40;
		break;
	case 0x80 ... 0x8f:
		w = ABOX_WIDGET_NSRC0 + val - 0x80;
		break;
	case 0xA0 ... 0xA3:
		w = ABOX_WIDGET_SPUM_ASRC0 + val - 0xA0;
		break;
#endif
	default:
		w = -EINVAL;
...
static const char * const abox_ddma_src_enum_texts[] = {
	...
	"RDMA6", "RDMA7", "RDMA8", "RDMA9", "RDMA10", "RDMA11",
	"RDMA12", "RDMA13", "RDMA14", "RDMA15",
	"SPUS_ASRC0", "SPUS_ASRC1", "SPUS_ASRC2", "SPUS_ASRC3",
	"SPUS_ASRC4", "SPUS_ASRC5", "SPUS_ASRC6", "SPUS_ASRC7",
	"SPUS_SSRC0", "SPUS_SSRC1", "SPUS_SSRC2", "SPUS_SSRC3",
	"SPUS_MIXER",
	"SPUM_SIFM0", "SPUM_SIFM1", "SPUM_SIFM2", "SPUM_SIFM3",
	"SPUM_SIFM4", "SPUM_SIFM5", "SPUM_SIFM6", "SPUM_SIFM7",
	"SPUM_SIFM8", "SPUM_SIFM9", "SPUM_SIFM10", "SPUM_SIFM11",
	"SPUM_SIFM12", "SPUM_SIFM13", "SPUM_SIFM14", "SPUM_SIFM15",
	"SPUM_ASRC0", "SPUM_ASRC1", "SPUM_ASRC2", "SPUM_ASRC3",
};
