...
static struct snd_kcontrol *abox_tplg_get_kcontrol(struct abox_tplg_kcontrol_data *kdata)
{
	...
	case SND_SOC_TPLG_DAPM_CTL_PIN:
		...
		break;
	default:
		abox_warn(dev, "unsupported pipeline item type: %s\n",
				kdata->hdr->name);
		break;
	}
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	for (i = 0; i < kdata->count; i++) {
		if (value[i] < mc->min || value[i] > mc->max) {
			abox_err(dev, "%s: value[%d]=%ld, min=%d, max=%d\n",
...
static int abox_tplg_control_load_pre(struct snd_soc_component *cmpnt,
		...
		struct snd_soc_tplg_ctl_hdr *hdr)
{
	...
	ret = abox_tplg_control_load_pre(cmpnt, kctl, hdr, kdata, se);
	if (ret < 0) {
		abox_err(cmpnt->dev, "failure control load : %d\n", ret);
		return ret;
	}
		...
		struct snd_soc_tplg_ctl_hdr *hdr)
{
	...
	ret = abox_tplg_control_load_pre(cmpnt, kctl, hdr, kdata, se);
	if (ret < 0) {
		abox_err(dev, "failure control load : %d\n", ret);
		return ret;
	}

	ret = abox_tplg_control_load_pipeline_items(kdata->cmpnt, kdata->tplg_ec, se, kdata);
	if (ret < 0) {
...
static int abox_tplg_complete(struct snd_soc_component *cmpnt)
{
