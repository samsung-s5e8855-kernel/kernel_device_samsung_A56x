...
struct abox_vdma_info *abox_vdma_get_info(int id)
{
	int idx = abox_vdma_get_idx(id);
		...
		unsigned long pos, struct iov_iter *iter, unsigned long bytes)
{
	...
	abox_vdma_copy_user_dma_area(substream, channel, pos, iter, bytes);
