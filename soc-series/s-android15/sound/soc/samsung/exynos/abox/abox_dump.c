...
static int abox_dump_file_open(struct inode *i, struct file *f)
{
	...
	if (abox_is_on() || pm_runtime_active(abox_dump_dev_abox))
...
void abox_dump_restore(struct abox_data *data)
{
	struct abox_dump_info *info;
	info = abox_dump_get_info(SLOG_DUMP_ID);

	if (info && info->file_started) {
