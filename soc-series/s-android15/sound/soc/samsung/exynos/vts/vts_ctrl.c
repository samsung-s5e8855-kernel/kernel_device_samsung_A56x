...
static void vts_ctrl_set_sol_status(struct device *dev)
{
	struct vts_data *data = dev_get_drvdata(dev);
	u32 values[3];
	int result = 0;
	...
	for (i = 0; i < 10; i++)
		data->shared_info->stored_num_data[i] = data->stored_data.num_data[i];

	values[0] = VTS_SOL_STATUS;
...
static int vts_ctrl_get_dmic_gain(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component =
		snd_soc_kcontrol_component(kcontrol);
	...
	if (VTS_DMIC_GAIN_OFFSET == mc->shift) {
		val = data->dmic_gain[mic_id];
	}
	else if (VTS_DMIC_1DB_GAIN_OFFSET == mc->shift) {
		val = data->dmic_1db_gain[mic_id] ;
	}

	ucontrol->value.integer.value[0] = val;

	vts_dev_info(dev, "%s Dmic[%d] offset:%#x value : %d\n", __func__, mic_id, mc->shift, val);

	return 0;
}

static int vts_ctrl_set_dmic_gain(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	...
	if(val > mc->max) {
		vts_dev_err(dev, "%s error, value[%d] > max[%d]\n", __func__, val, mc->max);
		return -EINVAL;
	}
	...
	if (VTS_DMIC_GAIN_OFFSET == mc->shift) {
		data->dmic_gain[mic_id] = val;
	}
	else if (VTS_DMIC_1DB_GAIN_OFFSET == mc->shift) {
		data->dmic_1db_gain[mic_id] = val;
	}
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	/* After all configuraion from HAL is done, set solution status saved from firwmare. */
	vts_ctrl_set_sol_status(component->dev);
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	struct device *dev = component->dev;
	struct vts_data *data = vts_get_vts_data();
	struct soc_mixer_control *e = (struct soc_mixer_control *)kcontrol->private_value;
	unsigned int *item = ucontrol->value.enumerated.item;
	u32 values[3];
	int result = 0;
	int val = ucontrol->value.integer.value[0];
...
static int vts_ctrl_get_bargein(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct vts_data *data = vts_get_vts_data();

	ucontrol->value.integer.value[0] = data->is_bargein;

	return 0;
}

static int vts_ctrl_set_bargein(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_soc_kcontrol_component(kcontrol);
	struct device *dev = component->dev;
	...
	data->is_bargein = (unsigned int)ucontrol->value.integer.value[0];

	vts_dev_info(dev, "%s: is_bargein(%d)\n", __func__, data->is_bargein);

	return 0;
}

static int vts_ctrl_get_bargein_gain(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct vts_data *data = vts_get_vts_data();

	ucontrol->value.integer.value[0] = data->bargein_gain;

	return 0;
}

static int vts_ctrl_set_bargein_gain(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_soc_kcontrol_component(kcontrol);
	struct device *dev = component->dev;
	...
	data->bargein_gain = (unsigned int)ucontrol->value.integer.value[0];

	vts_dev_info(dev, "%s: bargein_gain(%d)\n", __func__, data->bargein_gain);

	return 0;
}

static int vts_ctrl_get_sramlog_offset(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct vts_data *data = vts_get_vts_data();

	ucontrol->value.integer.value[0] = data->sramlog_offset;

	return 0;
}

static int vts_ctrl_set_sramlog_offset(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_soc_kcontrol_component(kcontrol);
	struct device *dev = component->dev;
	...
	if (item[0] >= e->max) {
		vts_dev_err(dev,
			"%s try to set 0x%x but should be under 0x%x\n",
			__func__, item[0], e->max);
		return -EINVAL;
	}

	data->sramlog_offset = (unsigned int)ucontrol->value.integer.value[0];

	data->sramlog_baseaddr = (char *)(data->sram_base + data->sramlog_offset);
	data->shared_info = (struct vts_shared_info *)(data->sramlog_baseaddr +	VTS_SRAMLOG_SIZE_MAX);

	vts_dev_info(dev, "%s: sramlog offset(0x%x)\n", __func__, data->sramlog_offset);

	return 0;
}

static int vts_ctrl_get_wfi_addr(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct vts_data *data = vts_get_vts_data();

	ucontrol->value.integer.value[0] = data->wfi_addr;

	return 0;
}

static int vts_ctrl_set_wfi_addr(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_component *component = snd_soc_kcontrol_component(kcontrol);
	struct device *dev = component->dev;
	...
	data->wfi_addr = (unsigned int)ucontrol->value.integer.value[0];

	vts_dev_info(dev, "%s: wfi address(0x%x)\n", __func__, data->wfi_addr);
...
static const struct snd_kcontrol_new vts_ctrl_controls[] = {
...
#if defined(CONFIG_SOC_S5E9945) || defined(CONFIG_SOC_S5E8845) ||defined(CONFIG_SOC_S5E9955) || defined(CONFIG_SOC_S5E8855)
	SOC_SINGLE("LPFEXT EN", VTS_DMIC_CONTROL_DMIC_IF,
		VTS_DMIC_LPFEXT_EN_OFFSET, 1, 0),
	SOC_SINGLE("CPSEXT EN", VTS_DMIC_CONTROL_DMIC_IF,
		VTS_DMIC_CPSEXT_EN_OFFSET, 1, 0),
#endif
	SOC_ENUM("HPF SEL", vts_hpf_sel),
	SOC_ENUM("CPS SEL", vts_cps_sel),
	SOC_SINGLE_EXT("Dmic_IF0 GAIN",
	0, VTS_DMIC_GAIN_OFFSET, 4, 0,
	vts_ctrl_get_dmic_gain,
	vts_ctrl_set_dmic_gain),
	SOC_SINGLE_EXT("Dmic_IF0 1DB GAIN",
	0, VTS_DMIC_1DB_GAIN_OFFSET, 5, 0,
	vts_ctrl_get_dmic_gain,
	vts_ctrl_set_dmic_gain),
	SOC_SINGLE_EXT("Dmic_IF1 GAIN",
	1, VTS_DMIC_GAIN_OFFSET, 4, 0,
	vts_ctrl_get_dmic_gain,
	vts_ctrl_set_dmic_gain),
	SOC_SINGLE_EXT("Dmic_IF1 1DB GAIN",
	1, VTS_DMIC_1DB_GAIN_OFFSET, 5, 0,
	vts_ctrl_get_dmic_gain,
	vts_ctrl_set_dmic_gain),
	...
	SOC_SINGLE_EXT("Bargein State",
		SND_SOC_NOPM,
		0, 10, 0,
		vts_ctrl_get_bargein,
		vts_ctrl_set_bargein),
	SOC_SINGLE_EXT("Sram Log Offset",
		SND_SOC_NOPM,
		0, VTS_SRAM_SZ, 0,
		vts_ctrl_get_sramlog_offset,
		vts_ctrl_set_sramlog_offset),
	SOC_SINGLE_EXT("Wfi Addr",
		SND_SOC_NOPM,
		0, VTS_SRAM_SZ, 0,
		vts_ctrl_get_wfi_addr,
		vts_ctrl_set_wfi_addr),
	SOC_SINGLE_EXT("Bargein Gain",
		SND_SOC_NOPM,
		0, 240, 0,
		vts_ctrl_get_bargein_gain,
		vts_ctrl_set_bargein_gain),
};
