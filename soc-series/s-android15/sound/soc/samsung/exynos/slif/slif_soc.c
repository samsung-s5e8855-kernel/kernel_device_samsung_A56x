...
#if IS_ENABLED(CONFIG_SOC_S5E9925) || IS_ENABLED(CONFIG_SOC_S5E9935) || IS_ENABLED(CONFIG_SOC_S5E8835) || IS_ENABLED(CONFIG_SOC_S5E9945) || IS_ENABLED(CONFIG_SOC_S5E8845) || IS_ENABLED(CONFIG_SOC_S5E9955) || IS_ENABLED(CONFIG_SOC_S5E8855)
...
static int slif_clk_set_rate_enable(struct slif_data *data, struct clk_path* path)
{
	...
	for (i = 0; (path[i].index >= 0) && (i < CLK_MAX_CNT); i++) {
		...
		slif_info(dev,"%s: index:%d, %s: %d -> %lu, enabled: %d -> %d\n",
...
void slif_ctrl_set(int enable, struct slif_data *data)
{
	struct device *dev = data->dev;
	struct snd_soc_component *cmpnt = data->cmpnt;
	int ret = 0;
