...
#include "../../include/dsp-common-type.h"
#endif
#include "../../npu-hw-device.h"
#ifdef CONFIG_EXYNOS_UNPU_DRIVER
#include "../../unpu.h"

#define UNPU_NCP_TEST_ID 7
#endif
...
#ifdef CONFIG_EXYNOS_UNPU_DRIVER
struct npu_interface *gnpu_if = &interface;
#endif
...
#ifdef CONFIG_EXYNOS_UNPU_DRIVER
		if (type == NPU_MBOX_REQUEST_UNPU) {
			npu_dbg("test id is %d\n", cmd->c.fw_test.param);
			if (cmd->c.fw_test.param != UNPU_NCP_TEST_ID)
				unpu_nw_request_test(cmd->c.fw_test.param);
			else
				/* Internally DD does call the real use case */
				unpu_ncp_test();
			/* call from host to uNPU interrupt generate func in unpu.c */
			break;
		} /* else fallthrough */
#endif
...
int nw_req_manager(int msgid, struct npu_nw *nw)
{
	...
	case NPU_NW_CMD_POWER_CTL:
		...
		break;
#ifdef CONFIG_EXYNOS_UNPU_DRIVER
	case NPU_NW_CMD_UNPU_TC_EXECUTE:
		...
		msg.mid = msgid;
		__send_interrupt(msg.command, &cmd, NPU_MBOX_REQUEST_UNPU);
		return TRUE;
#endif
...
int nw_rslt_manager(int *ret_msgid, struct npu_nw *nw)
{
	...
	if (nw->cmd == NPU_NW_CMD_UNPU_TC_EXECUTE) {
		msg.command = COMMAND_DONE;
		goto bypass_gnpu_mbx;
	}
