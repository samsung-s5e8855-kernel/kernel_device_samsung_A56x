				...
				phys_addr_t *fw_phys_base, size_t *fw_bin_size, size_t *fw_mem_size)
{
	...
	for (i = 0; i < iter; i++) {
		...
		ret = exynos_smc(SMC_CMD_REG, SMC_REG_ID_SFR_W((int)(mem->paddr + UNPU_FW_OFFSET + (i * ACCESS_4BYTE))), v, 0x0);
	}
#else
	memcpy(mem->vaddr + UNPU_FW_OFFSET, metadata, size);
#endif
	npu_dbg("vaddr: 0x%p paddr: 0x%x size: 0x%x\n", ((mem)->vaddr + UNPU_FW_OFFSET),
			((mem)->paddr + UNPU_FW_OFFSET), (mem)->size);

	*fw_phys_base = mem->paddr + UNPU_FW_OFFSET;
	*fw_bin_size = size;
