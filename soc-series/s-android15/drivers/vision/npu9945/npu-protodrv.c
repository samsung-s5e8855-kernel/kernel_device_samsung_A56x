...
static struct proto_req_nw *unpu_entry;
#endif
...
#ifdef CONFIG_EXYNOS_UNPU_DRIVER
			if (entry->nw.cmd == NPU_NW_CMD_UNPU_TC_EXECUTE)
				unpu_entry = entry;
#endif
...
static bool is_nw_processing_unpu(struct proto_req_nw *entry)
{
#ifdef CONFIG_EXYNOS_UNPU_DRIVER
	if ((entry == NULL) & (unpu_entry != NULL)) {
		if (unpu_entry->nw.msgid != (-1)) {
			entry = msgid_claim_get_ref(&npu_proto_drv.msgid_pool,
					unpu_entry->nw.msgid, PROTO_DRV_REQ_TYPE_NW);
			entry->nw.msgid = -1;
			unpu_entry = NULL;
			proto_nw_lsm.lsm_move_entry(COMPLETED, entry);
			return true;
...
static int  npu_protodrv_handler_nw_processing(void)
{
	...
	if (is_nw_processing_unpu(entry))
		handle_cnt++;
