 ...
 */
#if IS_ENABLED(EXYNOS_NPU_EXPOSE_KERNEL_ADDR)
...
#define npu_health_target(fmt, ...)	npu_health_on_lv_target(MEMLOG_LEVEL_ERR, fmt, ##__VA_ARGS__)  // dump
#else
...
#define npu_dump(fmt, args...) \
	npu_dump_target("NPU:[" NPU_LOG_TAG "]%s(%d):" fmt, __func__, __LINE__, ##args)

#define npu_health(fmt, args...) \
	npu_health_target("NPU:[" NPU_LOG_TAG "]%s(%d):" fmt, __func__, __LINE__, ##args)
