...
static inline void set_int_mif_freq(struct npu_sessionmgr *sess_mgr, int freq_idx, u32 requester)
{
	u32 request_freq_idx = max(0, min(MAX_MIF_FREQ_LEVELS - 1, freq_idx));

	if ((request_freq_idx > 0) && abs(request_freq_idx - atomic_read(&sess_mgr->freq_mif_cur)) < 2)
		return ;
	else if (((requester & 0xffff) == 0x3) && (request_freq_idx < atomic_read(&sess_mgr->freq_mif_cur))) {
		return ;
	}
	...
	atomic_set(&sess_mgr->freq_mif_cur, request_freq_idx);
}
...
static void set_new_freq(struct npu_sessionmgr *sess_mgr, u32 freq_index, u32 requester)
{
	s64 start, end;
...
static u32 get_next_set_freq(struct npu_sessionmgr *sess_mgr, u32 *next_mif_level)
{
	...
	list_for_each_entry(cur, &sess_mgr->active_session_list, active_session) {
		...
		if (is_happening_soon && (max_freq < cur->prev_npu_level)) {
			...
			if (next_mif_level)
				*next_mif_level = cur->next_mif_level;
		}
	}
	...
	while (!kthread_should_stop()) {
		...
		if (!atomic_read(&sess_mgr->queue_cnt)) {
			if (prev_npu_level > 0) {
				...
				set_int_mif_freq(sess_mgr, next_mif_level, 0);
				set_new_freq(sess_mgr, prev_npu_level, 1);
			} else {
				sess_mgr->cmdq_list_changed_from = 2;
				set_int_mif_freq(sess_mgr, 0, 0);
...
#elif IS_ENABLED(CONFIG_SOC_S5E8855)
		cur_pc = npu_read_hw_reg(gnpu0, 0x2300, 0xFFFFFFFF, 1);
#endif
		break;
	case CMDQ_PROCESSING_INSTN0:
		cur_pc = npu_read_hw_reg(gnpu0, 0x2300, 0xFFFFFFFF, 0);
		break;
#if IS_ENABLED(CONFIG_SOC_S5E9955)
	case CMDQ_PROCESSING_INSTN1:
		cur_pc = npu_read_hw_reg(gnpu1, 0x2300, 0xFFFFFFFF, 0);
		break;
#endif
	case CMDQ_PENDING:
...
static u32 npu_get_predict_frequency(struct npu_sessionmgr *sess_mgr, struct npu_session *session,
		s32 tune_npu_level, int from)
{
	...
	list_for_each_entry_safe(base, base_next, &sess_mgr->cmdq_head, list) {
		...
		for (i = freq_index; i < HIGEST_FREQ_IDX; i++) {
			...
			list_for_each_entry_safe(cur, cur_next, &sess_mgr->cmdq_head, list) {
				cancel_request = proto_check_already_been_requested(cur->session->uid);
...
static void update_earliest_arrival_time(struct npu_session *session, struct npu_cmdq_progress *cmdq_ptr)
{
...
static int npu_cmdq_update_alpha_beta(struct npu_session *session,
...
void npu_update_frequency_from_queue(struct npu_session *session, u32 buff_cnt)
{
	...
	if (freq_index == HIGEST_FREQ_IDX)
		set_int_mif_freq(sess_mgr, session->next_mif_level, requester_id);
...
void npu_governor_make_ncp_valid(struct ncp_header *ncp)
{
	u32 npu_max_freq = freq_level[HIGEST_FREQ_IDX].npu;
	u32 dnc_max_freq = freq_level[HIGEST_FREQ_IDX].dnc;

	ncp->model_name[NCP_MODEL_NAME_LEN - 1] = '\0';
	if (!ncp->computational_workload)
		ncp->computational_workload = ncp->io_workload;

	ncp->computational_workload = (ncp->computational_workload < npu_max_freq)?
		npu_max_freq : ncp->computational_workload;
	ncp->io_workload = (ncp->io_workload < dnc_max_freq)? dnc_max_freq : ncp->io_workload;
}
...
void init_workload_params(struct npu_session *session)
{
	...
	struct npu_model_info_hash *model = session->model_hash_node;
	u32 shift = 0;
	u32 npu_min_freq = freq_level[LOWEST_FREQ_IDX].npu;
	u32 dnc_min_freq = freq_level[LOWEST_FREQ_IDX].dnc;

	bool has_same_value = true;
	...
	while(has_same_value) {
		model->exec[LOWEST_FREQ_IDX].x = model->computational_workload / (npu_min_freq >> shift);
		model->exec[LOWEST_FREQ_IDX].z = model->io_workload / (dnc_min_freq >> shift);

		has_same_value = false;
		for(u32 i = LOWEST_FREQ_IDX+1; i <= HIGEST_FREQ_IDX; i++) {
			...
			if(model->exec[i].x == model->exec[i-1].x &&
					model->exec[i].z == model->exec[i-1].z) {
				has_same_value = true;
			}
		}

		shift++;
		if (((dnc_min_freq >> shift) == 0) || ((npu_min_freq >> shift) == 0)) {
			break;
		}
	}

	for(u32 i = LOWEST_FREQ_IDX; i <= HIGEST_FREQ_IDX; i++) {
		npu_dbg("x %lld z %lld shift %d compute_workload %d io_workload %d\n",
...
int npu_cmdq_table_read_probe(struct cmdq_table_info *cmdq_info)
{
	...
	dhcp_table->DVFS_TABLE_ADDR = cmdq_table_buf->daddr;
