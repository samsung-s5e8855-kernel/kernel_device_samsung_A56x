...
/* Singleton reference to afm object */
static struct npu_afm *afm_ref;
...
{
	int ret = 0;
	struct npu_afm *afm = afm_ref;
	if (!afm)
		return -EFAULT;

	ret = scnprintf(buf, PAGE_SIZE, "afm_onoff : %s\n", afm->afm_local_onoff ? "on" : "off");
...
static ssize_t afm_tdt_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	ret = scnprintf(buf, PAGE_SIZE, "afm_tdt_debug : %s, afm_tdt : %u\n"
					...
					afm->npu_afm_tdt_debug ? "on" : "off", afm->npu_afm_tdt);
...
void npu_afm_debug_set_irp(struct npu_system *system)
{
	...
	npu_write_hw_reg(npu_get_io_area(system, "htugnpu0"), 0x48, (afm_irp.m << 0), 0x000000FF, 0);
	npu_write_hw_reg(npu_get_io_area(system, "htugnpu0"), 0x48, (afm_irp.x << 20), 0x00F00000, 0);
	npu_write_hw_reg(npu_get_io_area(system, "htugnpu0"), 0x48, (afm_irp.r << 24), 0x3F000000, 0);
	npu_write_hw_reg(npu_get_io_area(system, "htugnpu0"), 0x20, (afm_irp.v << 11), 0x00003800, 0);
	npu_write_hw_reg(npu_get_io_area(system, "htugnpu0"), 0x6c, (afm_irp.trw << 3), 0x000000F8, 0);
	...
	npu_write_hw_reg(npu_get_io_area(system, "htugnpu1"), 0x48, (afm_irp.r << 24), 0x3F000000, 0);
	...
	npu_write_hw_reg(npu_get_io_area(system, "htusnpu0"), 0x48, (afm_irp.m << 0), 0x000000FF, 0);
	npu_write_hw_reg(npu_get_io_area(system, "htusnpu0"), 0x48, (afm_irp.x << 20), 0x00F00000, 0);
	npu_write_hw_reg(npu_get_io_area(system, "htusnpu0"), 0x48, (afm_irp.r << 24), 0x3F000000, 0);
	npu_write_hw_reg(npu_get_io_area(system, "htusnpu0"), 0x20, (afm_irp.v << 11), 0x00003800, 0);
	npu_write_hw_reg(npu_get_io_area(system, "htusnpu0"), 0x6c, (afm_irp.trw << 3), 0x000000F8, 0);

	npu_write_hw_reg(npu_get_io_area(system, "htusnpu1"), 0x48, (afm_irp.m << 0), 0x000000FF, 0);
	npu_write_hw_reg(npu_get_io_area(system, "htusnpu1"), 0x48, (afm_irp.x << 20), 0x00F00000, 0);
	npu_write_hw_reg(npu_get_io_area(system, "htusnpu1"), 0x48, (afm_irp.r << 24), 0x3F000000, 0);
	npu_write_hw_reg(npu_get_io_area(system, "htusnpu1"), 0x20, (afm_irp.v << 11), 0x00003800, 0);
	npu_write_hw_reg(npu_get_io_area(system, "htusnpu1"), 0x6c, (afm_irp.trw << 3), 0x000000F8, 0);
#endif
}
...
void npu_afm_debug_set_tdt(struct npu_system *system)
{
	...
	npu_write_hw_reg(npu_get_io_area(system, "htugnpu0"), 0x80, (afm->npu_afm_tdt << 20), 0xFFF00000, 0);
#if !IS_ENABLED(CONFIG_SOC_S5E8855)
	npu_write_hw_reg(npu_get_io_area(system, "htugnpu1"), 0x80, (afm->npu_afm_tdt << 20), 0xFFF00000, 0);
	npu_write_hw_reg(npu_get_io_area(system, "htusnpu0"), 0x80, (afm->npu_afm_tdt << 20), 0xFFF00000, 0);
	npu_write_hw_reg(npu_get_io_area(system, "htusnpu1"), 0x80, (afm->npu_afm_tdt << 20), 0xFFF00000, 0);
#endif
}
