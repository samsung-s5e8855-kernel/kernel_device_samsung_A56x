...
static struct npu_profile* npu_profile_alloc(int uid) {
	if (npu_profiles[uid])
		return npu_profiles[uid];
	npu_profiles[uid] = kzalloc(sizeof(struct npu_profile), GFP_KERNEL);
	return npu_profiles[uid];
}

static void npu_profile_free(int uid) {
	if (npu_profiles[uid]) {
		kfree(npu_profiles[uid]);
	}
	npu_profiles[uid] = NULL;
}
...
int npu_profile_init(struct npu_memory_buffer *profile_mem_buf, struct npu_memory_buffer *profile_mem_sub_buf,
		struct vs4l_profile_ready *pread, unsigned int mode, int uid)
{
	int i = 0, j = 0;
	struct npu_profile *npu_profile = npu_profile_alloc(uid);

	if (!npu_profile) {
		npu_err("npu_profile_alloc failed\n");
		return -ENOMEM;
	}
