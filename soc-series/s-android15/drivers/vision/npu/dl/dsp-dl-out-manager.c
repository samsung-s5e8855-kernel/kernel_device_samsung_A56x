...
void dsp_dl_hash_push(struct dsp_dl_out *dl_out)
{
	...
	unsigned int key = dsp_hash_get_key(data);
...
int dsp_dl_out_create(struct dsp_lib *lib)
{
	...
	if (!lib->dl_out) {
		dsp_err("Alloc is failed (lib dl_out)\n");
		return -1;
	}
...
{
	unsigned int idx;
	unsigned int *data = (unsigned int *)dl_out->data;
	unsigned int sec_end = sec.size / sizeof(unsigned int);
	data = (unsigned int *)(data + sec.offset);
...
int dsp_dl_out_manager_init(unsigned long start_addr, size_t size)
{
	...
	if (!out_manager) {
		dsp_err("Alloc is failed (Out manager)\n");
		return -1;
	}
	...
	if (dsp_tlsf_init(out_manager, mem_str, mem_size, DL_DL_OUT_ALIGN)) {
		dsp_err("dsp_tlsf_init is failed\n");
		return -1;
	}
