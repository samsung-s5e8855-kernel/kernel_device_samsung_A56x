...
struct dsp_link_info *dsp_link_info_create(struct dsp_elf32 *elf)
{
	...
	if (!l_info) {
		dsp_err("Alloc is failed (lib link_info)\n");
		return NULL;
	}
	...
	l_info->sec = (unsigned long *)dsp_dl_malloc(
			...
			"info sec");
	if (!l_info->sec) {
		dsp_err("Alloc is failed (info sec)\n");
		dsp_dl_free(l_info);
		return NULL;
	}
...
int dsp_linker_alloc_bss(struct dsp_elf32 *elf)
{
	struct dsp_list_node *node;
	...
	if (!alloc_tmp) {
		dsp_err("Alloc is failed (alloc bss alloc_tmp)\n");
		return -ENOMEM;
	}
...
static unsigned int __rule_get_value(struct dsp_reloc_rule *rule,
	struct dsp_link_info *sym_info, struct dsp_link_info *rela_info,
	struct dsp_elf32_shdr *rela_shdr, struct dsp_elf32_sym *sym,
	struct dsp_elf32_rela *rela, int *ret)
{
	...
	struct dsp_lib **common_libs, int common_size)
{
	...
	if (ret == -1) {
		...
		*value = __rule_get_value(rule, sym_info, rela_info, rela_shdr,
		...
		if (ret == -1) {
			dsp_err("fail __rule_get_value. ret = %d\n", ret);
			return -1;
		}
	}
	...
	if (strncmp(sym_str, "LE_F", 4) != 0) {
		...
		if (!reloc_sym) {
			dsp_err("Alloc is failed (Information of symbol relocated\n");
			return -1;
		}
...
int dsp_linker_init(struct dsp_dl_lib_file *file)
{
	...
	rules = (struct dsp_reloc_rule_list *)dsp_dl_malloc(
			...
			"Reloc rules");
	if (!rules) {
		dsp_err("Alloc is failed (Reloc rules)\n");
		return -1;
	}
	...
	gp_str = (struct dsp_string_tree_node *)dsp_dl_malloc(
			...
			"Linker string tree");
	if (!gp_str) {
		dsp_err("Alloc is failed (Linker string tree)\n");
		dsp_dl_free(rules);
		return -1;
	}
	...
	for (idx = 0; idx < RELOC_GP_NUM; idx++) {
		...
		ret = dsp_string_tree_push(gp_str, reloc_gp_str[idx], idx);
		if (ret < 0) {
