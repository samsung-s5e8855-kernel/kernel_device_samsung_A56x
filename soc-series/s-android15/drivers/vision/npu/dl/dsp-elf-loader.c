...
static int __dsp_elf32_check_magic(struct dsp_dl_lib_file *file)
{
	...
	if (file->size < 4) {
		dsp_err("Elf file is invalid(%zu)\n", file->size);
		return -1;
	}
...
static int __dsp_elf32_strncmp(struct dsp_elf32 *elf, const char *str, const char *elf_str, size_t size)
{
	ssize_t remaining = elf->data + elf->size - (char *)elf_str;
	if (remaining < 0 || (size_t)remaining < size)
		return -1;

	return strncmp(str, elf_str, size);
}
...
static int  __dsp_elf32_get_text(int idx, struct dsp_elf32 *elf)
{
	...
	if (!sec) {
		dsp_err("Alloc is failed (Text idx node)\n");
		return -ENOMEM;
	}
...
static int __dsp_elf32_idx_node_push_back(int idx, struct dsp_list_head *head)
{
	struct dsp_elf32_idx_node *sec;
...
static int __dsp_elf32_get_mem(int idx, const char *mem_name,
	...
	struct dsp_list_head *TCMb_local, struct dsp_list_head *SFRw)
{
	...
	if (strncmp("DMb", mem_name, 3) == 0) {
		...
		if (strlen(local_name) == 13 &&
			...
			ret = __dsp_elf32_idx_node_push_back(idx, DMb_local);
			if (ret) {
				dsp_err("Alloc is failed (DMb_local)\n");
				return -ENOMEM;
			}
		} else {
			dsp_dbg("sec : %d to DMb\n", idx);
			ret = __dsp_elf32_idx_node_push_back(idx, DMb);
			if (ret) {
				dsp_err("Alloc is failed (DMb)\n");
				return -ENOMEM;
			}
		}
		...
		if (strlen(local_name) == 13 &&
			...
			ret = __dsp_elf32_idx_node_push_back(idx, TCMb_local);
			if (ret) {
				dsp_err("Alloc is failed (TCMb_local)\n");
				return -ENOMEM;
			}
		} else {
			dsp_dbg("sec : %d to TCMb\n", idx);
			ret = __dsp_elf32_idx_node_push_back(idx, TCMb);
			if (ret) {
				dsp_err("Alloc is failed (TCMb)\n");
				return -ENOMEM;
			}
		}
	} else if (strncmp("DRAMb", mem_name, 5) == 0) {
		ret = __dsp_elf32_idx_node_push_back(idx, DRAMb);
		if (ret) {
			dsp_err("Alloc is failed (TCMb)\n");
			return -ENOMEM;
		}
	} else if (strncmp("SFRw", mem_name, 4) == 0) {
		ret = __dsp_elf32_idx_node_push_back(idx, SFRw);
		if (ret) {
			dsp_err("Alloc is failed (TCMb)\n");
			return -ENOMEM;
		}
...
static int __dsp_elf32_get_bss_symhash(struct dsp_elf32 *elf)
{
	...
	for (idx = 0; idx < elf->symtab_num; idx++) {
		...
		if (ndx > 0 && ndx < elf->hdr->e_shnum) {
			...
			if (dsp_hash_push(&elf->symhash, sym_str, sym)) {
				dsp_err("failed dsp_hash_push\n");
				return -1;
			}
			...
			if (ret == -1) {
				symnode =
					...
					dsp_dl_malloc(
						...
						"extern sym node");
				if (!symnode) {
					dsp_err("extern symnode is NULL\n");
					return -1;
				}
...
int dsp_elf32_load(struct dsp_elf32 *elf, struct dsp_dl_lib_file *file)
{
	...
	for (idx = 0; idx < elf->hdr->e_shnum; idx++) {
		...
		} else if (__dsp_elf32_strncmp(elf, ".text", shdr_name, 5) == 0) {
			ret = __dsp_elf32_get_text(idx, elf);
			if (ret) {
				dsp_err(".tex is NULL\n");
				return -1;
			}
		} else if (__dsp_elf32_strncmp(elf, ".robss.", shdr_name, 7) == 0) {
			const char *mem_name = shdr_name + 7;

			ret = __dsp_elf32_get_mem(idx, mem_name, &elf->DMb.robss,
				&elf->DMb_local.robss,
				&elf->DRAMb.robss, &elf->TCMb.robss,
				&elf->TCMb_local.robss,
				&elf->SFRw.robss);
			if (ret) {
				dsp_err("Failed to get mem robss\n");
				return -1;
			}
		} else if (__dsp_elf32_strncmp(elf, ".bss.", shdr_name, 5) == 0) {
			const char *mem_name = shdr_name + 5;

			ret = __dsp_elf32_get_mem(idx, mem_name, &elf->DMb.bss,
				&elf->DMb_local.bss, &elf->DRAMb.bss,
				&elf->TCMb.bss, &elf->TCMb_local.bss,
				&elf->SFRw.bss);
			if (ret) {
				dsp_err("Failed to get mem bss\n");
				return -1;
			}
		} else if (__dsp_elf32_strncmp(elf, ".data.", shdr_name, 6) == 0) {
			const char *mem_name = shdr_name + 6;

			ret = __dsp_elf32_get_mem(idx, mem_name, &elf->DMb.data,
				&elf->DMb_local.data, &elf->DRAMb.data,
				&elf->TCMb.data, &elf->TCMb_local.data,
				&elf->SFRw.data);
			if (ret) {
				dsp_err("Failed to get mem data\n");
				return -1;
			}
		} else if (__dsp_elf32_strncmp(elf, ".rodata.", shdr_name, 8) == 0) {
			const char *mem_name = shdr_name + 8;

			ret = __dsp_elf32_get_mem(idx, mem_name, &elf->DMb.rodata,
				&elf->DMb_local.rodata,
				&elf->DRAMb.rodata, &elf->TCMb.rodata,
				&elf->TCMb_local.rodata,
				&elf->SFRw.rodata);
			if (ret) {
				dsp_err("Failed to get mem rodata\n");
				return -1;
			}
		} else if (__dsp_elf32_strncmp(elf, ".rela.", shdr_name, 6) == 0) {
