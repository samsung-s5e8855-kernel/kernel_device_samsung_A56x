...
static struct proto_req_frame *req_frames[NPU_MAX_SESSION] = { 0, };
static struct proto_req_frame *req_frames_cancel[NPU_MAX_SESSION] = { 0, };
...
/*******************************************************************************
 * proto_req_frame structure API
 *
 */
static struct proto_req_frame* __proto_req_frames_alloc(int uid) {
	req_frames[uid] = kzalloc(NPU_MAX_QUEUE * sizeof(*req_frames[uid]), GFP_KERNEL);
	return req_frames[uid];
}

static struct proto_req_frame* __proto_req_frames_cancel_alloc(int uid) {
	req_frames_cancel[uid] = kzalloc(NPU_MAX_QUEUE * sizeof(*req_frames_cancel[uid]), GFP_KERNEL);
	return req_frames_cancel[uid];
}

static void __proto_req_frames_free(int uid) {
	if (req_frames[uid])
		kfree(req_frames[uid]);
	req_frames[uid] = NULL;
}

static void __proto_req_frames_cancel_free(int uid) {
	if (req_frames_cancel[uid])
		kfree(req_frames_cancel[uid]);
	req_frames_cancel[uid] = NULL;
}

int proto_req_frames_alloc(int uid) {
	int ret = 0;
	if (!__proto_req_frames_alloc(uid)) {
		ret = -ENOMEM;
		goto err_exit;
	}
	if (!__proto_req_frames_cancel_alloc(uid)) {
		ret = -ENOMEM;
		goto err_exit;
	}
	return ret;
err_exit:
	__proto_req_frames_free(uid);
	__proto_req_frames_cancel_free(uid);
	return ret;
}

void proto_req_frames_free(int uid) {
	__proto_req_frames_free(uid);
	__proto_req_frames_cancel_free(uid);
}
...
int register_session_ref(struct npu_session *sess)
{
	...
	if (unlikely(sess_entry == NULL)) {
		npu_err("fail in alloc_session_ref_entry\n");
		return -ENOMEM;
	}
	...
	for (add_val = 0; add_val < SESS_REF_HASH_TABLE_SIZE; add_val += SESS_REF_HASH_MAGIC) {
		...
		if (unlikely(sess_ref->hash_table[hash_id] == NULL)) {
			/* Find room */
			npu_dbg("Register UID at hashtable[%u]\n", hash_id);
	...
	if (unlikely(add_val >= SESS_REF_HASH_TABLE_SIZE)) {
		...
		npu_warn("UID cannot be stored on hash table.\n");
	}

	list_add_tail(&sess_entry->list, &sess_ref->entry_list);
	npu_dbg("session ref @%pK registered.\n", sess_entry);
...
int link_session_nw(struct proto_req_nw *req_nw)
{
	...
	if (unlikely(!is_belong_session(nw))) {
		/* No link necessary */
		npu_info("NW no need to linked to session reference.\n");
		return 0;
	}

	sess_entry = find_session_ref_nw(req_nw);
	if (unlikely(sess_entry == NULL)) {
		npu_err("cannot found session ref.\n");
		return -EINVAL;
	}
	if (unlikely(is_list_used(&req_nw->sess_ref_list))) {
		npu_err("network mgmt. seemed to belong other session. next pointer is %pK\n"
			, req_nw->sess_ref_list.next);
		return -EFAULT;
	}
	INIT_LIST_HEAD(&req_nw->sess_ref_list);
	list_add_tail(&req_nw->sess_ref_list, &sess_entry->nw_list);
	npu_dbg("NW linked to ref@%pK.\n", sess_entry);
	return 0;
}
...
int unlink_session_nw(struct proto_req_nw *req_nw)
{
	...
	if (!is_belong_session(nw)) {
		/* No unlink necessary */
		npu_info("NW no need to be unlinked from session reference.\n");
		return 0;
	}
	...
	npu_err("network mgmt. does not belong to session ref@%pK.\n", sess_entry);
	return -EINVAL;

found_match:
#endif /* UNLINK_CHECK_OWNERSHIP */
	list_del_init(&req_nw->sess_ref_list);
	npu_info("NW unlinked from ref@%pK.\n", sess_entry);
	return 0;
}
...
static int force_clear_cb(struct proto_req_nw *req_nw)
{
	...
	list_for_each_entry(iter_frame, &sess_entry->frame_list, sess_ref_list) {
		npu_ufdbg("Reset src_queue for frame in state [%d]\n",
			&iter_frame->frame, iter_frame->frame.session->hids, iter_frame->state);
		iter_frame->frame.src_queue = NULL;
		cnt++;
	}
	npu_info("Clear src_queue for frame: %d entries\n", cnt);

	cnt = 0;
	list_for_each_entry(iter_nw, &sess_entry->nw_list, sess_ref_list) {
		/* Do not clear CB for req_nw itself */
		if (iter_nw != req_nw)	{
			npu_info("Reset notify_func for nw in state [%d]\n",
				 iter_nw->state);
			iter_nw->nw.notify_func = NULL;
			cnt++;
		}
	}
	npu_info("Clear notify_func for nw: %d entries\n", cnt);
...
{
	struct npu_nw *nw = &target->nw;

	if (unlikely(!nw)) {
		npu_err("Failed to get npu_nw\n");
		return 0;
	}
...
static int nw_mgmt_op_put_result(const struct proto_req_nw *src)
{
	...
	if (unlikely(ret)) {
		npu_err("error(%d) in npu_ncp_mgmt_save_result\n", ret);
		return ret;
	}
...
static int npu_queue_op_get_request(struct proto_req_frame *target, struct npu_frame *frame)
{
	if (frame == NULL) {
...
static int npu_protodrv_handler_nw_free(void)
{
	...
	while ((entry = proto_nw_lsm.lsm_get_entry(FREE)) != NULL) {
		...
		if (likely(nw_mgmt_op_get_request(entry) != 0)) {
			...
			npu_info("(REQ)NW: cmd:%u(%s) / req_id:%u\n",
				entry->nw.cmd, __cmd_name(entry->nw.cmd),
				entry->nw.npu_req_id);
			if (entry->nw.cmd == NPU_NW_CMD_LOAD) {
				/* Load command -> Register the session */
				ret = register_session_ref(entry->nw.session);
				if (unlikely(ret)) {
					npu_err("cannot register session: ret = %d\n",
						ret);
			...
			if (unlikely(ret)) {
				npu_err("cannot link session: ret = %d\n",
					ret);
			...
			case NPU_NW_CMD_STREAMOFF:
				...
				if (unlikely(session_ref_entry == NULL)) {
					npu_err("requested STREAM_OFF, but Session entry find error.\n");
					entry->nw.result_code = NPU_ERR_DRIVER(NPU_ERR_INVALID_UID);
					goto error_req;
				}
				npu_dbg("requested STREAM_OFF : Change s_state [%d] -> [%d]\n",
				...
				case SESSION_REF_STATE_STOPPING:
					...
					npu_warn("stream off on SESSION_REF_STATE_STOPPING state. It seems retry of STREAM_OFF.\n");
					break;
				default:
					/* Invalid state */
					npu_err("requested STREAM_OFF, but Session state is not Active (%d).\n",
			...
			case NPU_NW_CMD_CLEAR_CB:
				...
				if (unlikely(ret))
					npu_warn("force_clear_cb error (%d).\n", ret);
				break;
...
int proto_drv_handler_frame_requested(struct npu_frame *frame)
{
	...
	case NPU_FRAME_CMD_Q:
		entry = &req_frames[uid][incl->index];
		if (entry->state != FREE) {
	...
	case NPU_FRAME_CMD_Q_CANCEL:
		entry = &req_frames_cancel[uid][incl->index];
		if (entry->state != FREE) {
			npu_uferr("entry->state[%d] is not FREE",
				&entry->frame, entry->frame.session->hids, entry->state);
			entry->frame.result_code = NPU_ERR_DRIVER(NPU_ERR_INVALID_STATE);
		...
		opposite_entry = &req_frames[uid][incl->index];
	...
	/* Is request available ? */
	if (npu_queue_op_get_request(entry, frame) == 0) {
...
static int __mbox_nw_ops_put(struct proto_req_nw *entry)
{
	...
	if (unlikely(npu_device_is_emergency_err(npu_proto_drv.npu_device))) {
		...
		npu_warn("EMERGENCY - do not send request [%d:%s] to hardware.\n",
			entry->nw.cmd, __cmd_name(entry->nw.cmd));
		return 0;
	}
	...
	if (unlikely(ret <= 0))
		npu_warn("REQUESTED %s cannot be queued to mbox [npu_req_id=%u]\n",
				TYPE_NAME_NW, entry->nw.npu_req_id);
	return ret;
}
...
static int  npu_protodrv_handler_nw_requested(void)
{
	...
	LSM_FOR_EACH_ENTRY_IN(proto_nw_lsm, REQUESTED, entry,
...
#else
			npu_info("BYPASS_HW_STREAMOFF enabled. Bypassing Streamoff.\n");
		...
		case NPU_NW_CMD_CLEAR_CB:
			...
			break;
		default:
			npu_notice("Conventional command cmd:(%u)(%s) nw->uid(%d)\n",
				entry->nw.cmd, __cmd_name(entry->nw.cmd), entry->nw.uid);
...
{
	int handle_cnt = 0;
	struct npu_queue_list *incl; // input container list
	struct proto_req_frame *entry;
	struct proto_req_frame *received_entry;
	npu_uid_t uid;

	while (frame_mbox_ops_get(&received_entry) > 0) {
		/* Check entry's state */
		uid = received_entry->frame.uid;
		incl = received_entry->frame.input;

		if (received_entry->frame.cmd == NPU_FRAME_CMD_Q_CANCEL)
			entry = &req_frames_cancel[uid][incl->index];
		else
			entry = &req_frames[uid][incl->index];
		...
		entry->state = FREE;
		entry->frame.result_code = (received_entry->frame).result_code;
		entry->frame.duration = (received_entry->frame).duration;
		...
		if (received_entry->frame.cmd == NPU_FRAME_CMD_Q) {
			npu_queue_op_put_result(entry);
		} else {
			entry->frame.session->nw_result.result_code = (received_entry->frame).result_code;
...
static int npu_protodrv_handler_nw_completed(void)
{
	...
	LSM_FOR_EACH_ENTRY_IN(proto_nw_lsm, COMPLETED, entry,
		...
		if (unlikely(!is_belong_session(&entry->nw))) {
			...
			npu_dbg("complete (%s), with result code (0x%08x)\n",
				__cmd_name(entry->nw.cmd), entry->nw.result_code);
			...
			session_ref_entry = find_session_ref_nw(entry);
			if (unlikely(session_ref_entry == NULL)) {
				npu_err("Session entry find error.\n");
				transition = 1;
			} else {
				switch (entry->nw.cmd) {
				case NPU_NW_CMD_LOAD:
					...
					if (likely(entry->nw.result_code == NPU_ERR_NO_ERROR)) {
						npu_dbg("complete load : Change s_state (%d) -> (%d)\n",
				...
				case NPU_NW_CMD_STREAMOFF:
					...
					if (likely((entry->nw.result_code == NPU_ERR_NO_ERROR) && is_last_session_ref(entry))) {
						npu_dbg("complete STREAM_OFF : change s_state (%d) -> (%d)\n",
							session_ref_entry->s_state, SESSION_REF_STATE_ACTIVE);
						...
						npu_dbg("there are pending request(s). STREAM_OFF blocked at s_state [%d]\n",
							session_ref_entry->s_state);
						transition = 0;
					}
					break;
				case NPU_NW_CMD_UNLOAD:
					/* Claim the packet if the current is last one and result is DONE / or result is NDONE */
					if (likely((entry->nw.result_code == NPU_ERR_NO_ERROR) && is_last_session_ref(entry))) {
						npu_dbg("complete UNLOAD : change s_state (%d) -> (%d)\n",
						...
						/* No error but there are outstanding requests */
						npu_dbg("there are pending request(s). UNLOAD blocked at s_state (%d)\n",
							session_ref_entry->s_state);
						transition = 0;
					}
					break;
				...
				case NPU_NW_CMD_MODE:
					if (entry->nw.result_code == NPU_ERR_NO_ERROR) {
						npu_dbg("complete MODE\n");
						transition = 1;
					} else if (entry->nw.result_code != NPU_ERR_NO_ERROR) {
						npu_err("complete MODE, with error(%u/0x%08x) : s_state [%d]\n",
							entry->nw.result_code, entry->nw.result_code,
							session_ref_entry->s_state);
						transition = 1;
						fw_will_note(FW_LOGSIZE);
					} else {
						/* No error but there are outstanding requests */
						npu_dbg("there are pending request(s). MODE blocked at s_state (%d)\n",
							session_ref_entry->s_state);
						transition = 0;
					}
					break;
				...
				case NPU_NW_CMD_CLEAR_CB:
					/* Always success */
					entry->nw.result_code = NPU_ERR_NO_ERROR;
					npu_dbg("complete CLEAR_CB\n");

					/* Leave assertion if this message is request other than emergency mode */
					if (unlikely(!npu_device_is_emergency_err(npu_proto_drv.npu_device))) {
						npu_warn("NPU_NW_CMD_CLEAR_CB posted on non-emergency situation.\n");
					}
					transition = 1;
					break;
				case NPU_NW_CMD_POWER_CTL:
				...
				default:
					npu_err("invalid command(%u)\n", entry->nw.cmd);
				}
			}
		}
		/* Post result if processing can be completed */
		if (likely(transition)) {
			npu_notice("(COMPLETED)NW: cmd(%u)(%s) / req_id(%u) / result(%u/0x%08x) NPU-TIMESTAT:%s\n",
				entry->nw.cmd, __cmd_name(entry->nw.cmd),
			...
			if (likely(!nw_mgmt_op_put_result(entry))) {
				npu_notice("(COMPLETED)NW: notification sent result(0x%08x)\n",
					entry->nw.result_code);
			} else {
				/* Shall be retried on next iteration */
				npu_warn("COMPLETED %s cannot be queued to result npu_req_id(%u)\n",
					   TYPE_NAME_NW, entry->nw.npu_req_id);
				goto retry_on_next;
			}
			...
			if (unlikely(ret)) {
				npu_err("unlink_session_nw for CMD[%u] failed : %d\n",
					entry->nw.cmd, ret);
			}
			...
			if (unlikely(entry->nw.cmd == NPU_NW_CMD_LOAD &&
				...
				if (unlikely(ret))
					npu_err("drop_session_ref error : %d\n", ret);
			}

			if (unlikely(entry->nw.cmd == NPU_NW_CMD_UNLOAD)) {
				/* Failed UNLOAD - left warning but session ref shall be destroyed anyway */
				if (unlikely(entry->nw.result_code != NPU_ERR_NO_ERROR))
					npu_warn("Unload failed but Session ref will be destroyed.\n");

				/* Destroy session ref */
				ret = drop_session_ref(entry->nw.uid);
				if (unlikely(ret))
					npu_err("drop_session_ref error : %d", ret);
			}
...
static int proto_drv_timedout_handling(const lsm_list_type_e state)
{
	...
	LSM_FOR_EACH_ENTRY_IN(proto_nw_lsm, state, entry,
		if (unlikely(is_timedout(entry, NPU_PROTODRV_TIMEOUT_MAP[state][PROTO_DRV_REQ_TYPE_NW].timeout_ns, now))) {
			...
			proto_nw_lsm.lsm_move_entry(COMPLETED, entry);
			npu_warn("timeout entry (%s) on state %s - req_id(%u), result_code(%u/0x%08x)\n",
...
bool proto_check_already_been_requested(int uid)
{
	if (!req_frames[uid])
		return false;

	for (int i = 0; i < NPU_MAX_QUEUE; i++)
		if (req_frames[uid][i].state != FREE
			&& req_frames[uid][i].frame.request_cancel)
			return true;
