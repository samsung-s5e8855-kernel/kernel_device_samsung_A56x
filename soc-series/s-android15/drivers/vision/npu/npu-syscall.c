				...
				bool chk_compat)
{
	...
	if (unlikely(chk_compat))
		MASK_COMPAT_UPTR(kp->containers);
...
static int get_vs4l_buf_votf(struct vs4l_buf_votf *kp, struct vs4l_buf_votf __user *up,
				bool chk_compat)
{
	int ret = 0;

	if (!__npu_access_ok(up, sizeof(struct vs4l_buf_votf)))
		npu_err("Cannot access to user ptr(%pK)\n", up);

	ret = copy_from_user((void *)kp, (void __user *)up,
				sizeof(struct vs4l_buf_votf));
	if (ret) {
		npu_err("copy_from_user failed(%d) from %pK\n", ret, up);
		ret = -EFAULT;
	}
...
static int put_vs4l_buf_votf(struct vs4l_buf_votf *kp, struct vs4l_buf_votf __user *up,
				bool chk_compat)
{
	int ret = 0;
	...
	ret = copy_to_user(up, kp, sizeof(struct vs4l_buf_votf));
	if (ret) {
		npu_err("copy_from_user failed(%d) from %pK\n", ret, up);
		ret = -EFAULT;
	}
			    ...
			    struct vs4l_version __user *up, bool chk_compat)
{
	...
	if (unlikely(chk_compat))
		MASK_COMPAT_UPTR(kp->fw_version);
			    ...
			    struct vs4l_version __user *up, bool chk_compat)
{
	...
	if (unlikely(chk_compat))
		MASK_COMPAT_UPTR(temp.build_info);
	...
	if (unlikely(chk_compat))
		MASK_COMPAT_UPTR(temp.signature);
	...
	if (unlikely(chk_compat))
		MASK_COMPAT_UPTR(temp.driver_version);
	...
	if (unlikely(chk_compat))
		MASK_COMPAT_UPTR(temp.fw_version);
			   ...
			   unsigned long arg, bool chk_compat)
{
	...
	case VS4L_VERTEXIOC_QBUF_BUNDLE_CANCEL:
		...
		put_vs4l_container_bundle(&vs4l_kvar.vscb,
				(struct vs4l_container_bundle __user *)arg, chk_compat);
		break;
	case VS4L_VERTEXIOC_BUF_VOTF:
		ret = get_vs4l_buf_votf(&vs4l_kvar.vsbe,
				(struct vs4l_buf_votf __user *)arg, chk_compat);
		if (ret)
			break;

		ret = npu_vertex_buf_votf(file, &vs4l_kvar.vsbe);
		if (ret)
			npu_err("npu_vertex_buf_export failed(%d)\n", ret);

		put_vs4l_buf_votf(&vs4l_kvar.vsbe,
				(struct vs4l_buf_votf __user *)arg, chk_compat);
		break;
	default:
...
int vision_register_device(struct vision_device *vdev, int minor, struct module *owner)
{
	...
	if (unlikely(!vdev->parent)) {
		probe_err("Failed to get vdev->parent\n");
		return -EINVAL;
	}
