...
int npu_clk_get(struct npu_clocks *clocks, struct device *dev)
{
...
#if !IS_ENABLED(CONFIG_NPU_CLOCK)
	return 0;
#endif

	if (unlikely(!clocks)) {
		probe_err("Failed to get clock\n");
		goto err;
	}
	...
	for (i = 0; i < clocks->clk_count; i++) {
		ret = of_property_read_string_index(dev->of_node, "clock-names",
								i, &clk_ids[i]);
		if (ret) {
			probe_err("failed(%d) to read clocks name %d from %s node\n",
					ret, i, dev->of_node->name);
...
int npu_clk_prepare_enable(struct npu_clocks *clocks)
{
	...
	for (i = 0; i < clocks->clk_count; i++) {
		...
		if (ret) {
			npu_err("couldn't prepare and enable clock[%d](%d)\n", i, ret);
			goto err;
		}
	}
	return 0;
