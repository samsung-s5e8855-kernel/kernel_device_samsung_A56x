...
static ssize_t stm_qactive_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	int ret = 0;
	u32 cssys_cpucl0_qactive, cssys_npu_qactive;

	cssys_cpucl0_qactive =
		npu_read_hw_reg(npu_get_io_area(npu_stm_data.system, "sfrcoresight"), 0x2C, 0xFFFFFFFF, 0);
#ifdef NPU_USE_CORESIGHT_NEW_CSLOCK
	cssys_npu_qactive =
		npu_smc_read_hw_reg(npu_get_io_area(npu_stm_data.system, "sfrcssys_npu"), 0x2C, 0xFFFFFFFF, 0);
#else
	cssys_npu_qactive =
		npu_read_hw_reg(npu_get_io_area(npu_stm_data.system, "sfrcssys_npu"), 0x2C, 0xFFFFFFFF, 0);
#endif
	ret = scnprintf(buf, PAGE_SIZE, "QACTIVE - cssys_cpucl0(0x%x), cssys_npu(0x%x)\n",
						cssys_cpucl0_qactive, cssys_npu_qactive);

	return ret;
}

static ssize_t stm_qactive_store(
	struct device *dev, struct device_attribute *attr,
	const char *buf, size_t len)
{
	int ret = 0;
	...
	if (sscanf(buf, "%u", &x) > 0) {
		if (x > 2) {
			npu_err("Invalid qactive setting : 0x%x, please input 0(off) or 1(on)\n", x);
			ret = -EINVAL;
			goto err_exit;
		}
	}
	...
	if (x) {
		npu_write_hw_reg(npu_get_io_area(npu_stm_data.system, "sfrcoresight"), 0x2C, 0x1, 0x00000001, 0);
#ifdef NPU_USE_CORESIGHT_NEW_CSLOCK
		npu_smc_write_hw_reg(npu_get_io_area(npu_stm_data.system, "sfrcssys_npu"), 0x2C, 0x1, 0x00000001, 0);
#else
		npu_write_hw_reg(npu_get_io_area(npu_stm_data.system, "sfrcssys_npu"), 0x2C, 0x1, 0x00000001, 0);
#endif
	} else {
		npu_write_hw_reg(npu_get_io_area(npu_stm_data.system, "sfrcoresight"), 0x2C, 0x0, 0x00000001, 0);
#ifdef NPU_USE_CORESIGHT_NEW_CSLOCK
		npu_smc_write_hw_reg(npu_get_io_area(npu_stm_data.system, "sfrcssys_npu"), 0x2C, 0x0, 0x00000001, 0);
#else
		npu_write_hw_reg(npu_get_io_area(npu_stm_data.system, "sfrcssys_npu"), 0x2C, 0x0, 0x00000001, 0);
#endif
	}
...
static DEVICE_ATTR_RW(stm_qactive);

static ssize_t stm_sdma_vc_set_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	int ret = 0;

	ret = scnprintf(buf, PAGE_SIZE, "sdma_vc_set : 0x%x\n",npu_stm_data.sdma_vc_set);

	return ret;
}

static ssize_t stm_sdma_vc_set_store(
	struct device *dev, struct device_attribute *attr,
	const char *buf, size_t len)
{
	int ret = 0;
	...
	if (sscanf(buf, "%u", &x) > 0) {
		if (x > 2) {
			npu_err("Invalid sdma_vc_set setting : 0x%x, please input 0(0-15) or 1(16-31)\n", x);
			ret = -EINVAL;
			goto err_exit;
		}
	}

	npu_stm_data.sdma_vc_set = x;
...
static DEVICE_ATTR_RW(stm_sdma_vc_set);
...
static int register_stm_ts_sysfs(struct npu_system *system)
{
	...
	ret = sysfs_create_file(&device->dev->kobj, &dev_attr_stm_qactive.attr);
	if (ret) {
		probe_err("sysfs_create_file error : ret = %d\n", ret);
		goto err_exit;
	}

	ret = sysfs_create_file(&device->dev->kobj, &dev_attr_stm_sdma_vc_set.attr);
	if (ret) {
		probe_err("sysfs_create_file error : ret = %d\n", ret);
		goto err_exit;
	}
...
//		npu_stm_data.enable_cnt++;
	...
	if (!npu_stm_data.enable_cnt) {
		if (!npu_stm_data.sdma_vc_set) {
			ret = npu_cmd_map(npu_stm_data.system, "sdma_vc0");
		} else {
			ret = npu_cmd_map(npu_stm_data.system, "sdma_vc1");
		}
	}
	...
	npu_info("STM enabled(%d), sdma_vc_set(0x%x)\n", hid, npu_stm_data.sdma_vc_set);
err_exit:
	return ret;
}
...
static int npu_stm_allow_64_trace(struct npu_system *system)
{
	...
	u32	old_hemcr_1;	/* Backup of current STMHEMCR value */

	/* Unlock STM register for writing */
	CS_UNLOCK(npu_get_io_area(system, "sfrstm")->vaddr, npu_get_io_area(system, "sfrstm")->paddr);

	old_hemcr = npu_smc_read_hw_reg(npu_get_io_area(system, "sfrstm"), STMHEMCR, 0xFFFFFFFF, 0);
	...
	/* Restore STMHEMCR */
	npu_smc_write_hw_reg(npu_get_io_area(system, "sfrstm"), STMHEMCR, old_hemcr, 0xFFFFFFFF, 0);

	/* Lock the STM SFR again */
	CS_LOCK(npu_get_io_area(system, "sfrstm")->vaddr, npu_get_io_area(system, "sfrstm")->paddr);

	CS_UNLOCK(npu_get_io_area(system, "sfrstm1")->vaddr, npu_get_io_area(system, "sfrstm1")->paddr);

	old_hemcr_1 = npu_smc_read_hw_reg(npu_get_io_area(system, "sfrstm1"), STMHEMCR, 0xFFFFFFFF, 0);

	/* Stop STM before set trace configuration : Settings are in effect after restarting */
	/* Enable events both bank 0 and 1 */
	ret = npu_cmd_map(system, "allow64stm1");
	if (ret) {
		npu_err("Failed to write registers on probe_npu_stm_regs array (%d)\n", ret);
		goto err_exit_stm1;
	}
	...
	/* Restore STMHEMCR */
	npu_smc_write_hw_reg(npu_get_io_area(system, "sfrstm1"), STMHEMCR, old_hemcr_1, 0xFFFFFFFF, 0);

	/* Lock the STM SFR again */
	CS_LOCK(npu_get_io_area(system, "sfrstm1")->vaddr, npu_get_io_area(system, "sfrstm1")->paddr);

	npu_dbg("128 STM events are enabled\n");
	return ret;
...
static int npu_stm_allow_64_trace(struct npu_system *system)
{
	...
	/* Stop STM before set trace configuration : Settings are in effect after restarting */
	/* Enable events both bank 0 and 1 */
	ret = npu_cmd_map(system, "allow64ns");
	if (ret) {
		npu_err("Failed to write registers on probe_npu_stm_regs array (%d)\n", ret);
		goto err_exit;
	}
	...
	CS_UNLOCK(npu_get_io_area(system, "sfrstm1")->vaddr);

	old_hemcr_1 = npu_read_hw_reg(npu_get_io_area(system, "sfrstm1"), STMHEMCR, 0xFFFFFFFF, 0);

	/* Stop STM before set trace configuration : Settings are in effect after restarting */
	/* Enable events both bank 0 and 1 */
	ret = npu_cmd_map(system, "allow64ns1");
	if (ret) {
		npu_err("Failed to write registers on probe_npu_stm_regs array (%d)\n", ret);
		goto err_exit_stm1;
	}
	...
	/* Restore STMHEMCR */
	npu_write_hw_reg(npu_get_io_area(system, "sfrstm1"), STMHEMCR, old_hemcr_1, 0xFFFFFFFF, 0);

	/* Lock the STM SFR again */
	CS_LOCK(npu_get_io_area(system, "sfrstm1")->vaddr);
 ...
 * Exported functions
 */
void npu_stm_set_dhcp(struct npu_system *system, int hid, bool on)
{
	...
	if (hid & NPU_HWDEV_ID_DNC) {
		if (on) {
			dhcp_table->STM_ENABLE = STM_ENABLE_TO_DHCP;
			npu_info("DNC - stm enabled, set ENABLE to DHCP\n");
		} else {
			dhcp_table->STM_ENABLE = STM_DISABLE_TO_DHCP;
			npu_info("DNC - stm disabled, set DISABLE to DHCP\n");
		}
	}
}
