...
#endif
#else
	NPU_ERR_RET(validate_ncp_offset_range(ncp_header->vector_list[NCP_VECTOR_TYPE_ADDRESS].offset,
		sizeof(struct address_vector), ncp_header->vector_list[NCP_VECTOR_TYPE_ADDRESS].cnt,
		ncp_size),
		"address vector out of bounds. start=0x%x, cnt=%u, ncp_size=0x%lx\n",
		ncp_header->vector_list[NCP_VECTOR_TYPE_ADDRESS].offset,
		ncp_header->vector_list[NCP_VECTOR_TYPE_ADDRESS].cnt, ncp_size);

	NPU_ERR_RET(validate_ncp_offset_range(ncp_header->vector_list[NCP_VECTOR_TYPE_MEMORY].offset,
		sizeof(struct memory_vector), ncp_header->vector_list[NCP_VECTOR_TYPE_MEMORY].cnt,
		ncp_size),
		"memory vector out of bounds. start=0x%x, cnt=%u, ncp_size=0x%lx\n",
		ncp_header->vector_list[NCP_VECTOR_TYPE_MEMORY].offset,
		ncp_header->vector_list[NCP_VECTOR_TYPE_MEMORY].cnt, ncp_size);

	NPU_ERR_RET(validate_ncp_offset_range(ncp_header->vector_list[NCP_VECTOR_TYPE_PWREST].offset,
		sizeof(struct pwr_est_vector), ncp_header->vector_list[NCP_VECTOR_TYPE_PWREST].cnt,
		ncp_size),
		"power estimation vector out of bounds. start=0x%x, cnt=%u, ncp_size=0x%lx\n",
		ncp_header->vector_list[NCP_VECTOR_TYPE_PWREST].offset,
		ncp_header->vector_list[NCP_VECTOR_TYPE_PWREST].cnt, ncp_size);

	NPU_ERR_RET(validate_ncp_offset_range(ncp_header->vector_list[NCP_VECTOR_TYPE_INTERRUPTION].offset,
		sizeof(struct interruption_vector), ncp_header->vector_list[NCP_VECTOR_TYPE_INTERRUPTION].cnt, ncp_size),
		"interruption vector out of bounds. start=0x%x, cnt=%u, ncp_size=0x%lx\n",
		ncp_header->vector_list[NCP_VECTOR_TYPE_INTERRUPTION].offset,
		ncp_header->vector_list[NCP_VECTOR_TYPE_INTERRUPTION].cnt,
		ncp_size);

	NPU_ERR_RET(validate_ncp_offset_range(ncp_header->vector_list[NCP_VECTOR_TYPE_GROUP].offset,
		sizeof(struct group_vector), ncp_header->vector_list[NCP_VECTOR_TYPE_GROUP].cnt,
		ncp_size),
		"group vector out of bounds. start=0x%x, cnt=%u, ncp_size=0x%lx\n",
		ncp_header->vector_list[NCP_VECTOR_TYPE_GROUP].offset,
		ncp_header->vector_list[NCP_VECTOR_TYPE_GROUP].cnt, ncp_size);

	NPU_ERR_RET(validate_ncp_offset_range(ncp_header->vector_list[NCP_VECTOR_TYPE_THREAD].offset,
		sizeof(struct thread_vector), ncp_header->vector_list[NCP_VECTOR_TYPE_THREAD].cnt,
		ncp_size),
		"thread vector out of bounds. start=0x%x, cnt=%u, ncp_size=0x%lx\n",
		ncp_header->vector_list[NCP_VECTOR_TYPE_THREAD].offset,
		ncp_header->vector_list[NCP_VECTOR_TYPE_THREAD].cnt, ncp_size);

	NPU_ERR_RET(validate_ncp_offset_range(ncp_header->vector_list[NCP_VECTOR_TYPE_LLC].offset,
		sizeof(struct llc_vector), ncp_header->vector_list[NCP_VECTOR_TYPE_LLC].cnt,
		ncp_size),
		"llc vector out of bounds. start=0x%x, cnt=%u, ncp_size=0x%lx\n",
		ncp_header->vector_list[NCP_VECTOR_TYPE_LLC].offset,
		ncp_header->vector_list[NCP_VECTOR_TYPE_LLC].cnt, ncp_size);
#endif
...
#else
	mv_cnt = ncp_header->vector_list[NCP_VECTOR_TYPE_MEMORY].cnt;
	mv_offset = ncp_header->vector_list[NCP_VECTOR_TYPE_MEMORY].offset;
	av_offset = ncp_header->vector_list[NCP_VECTOR_TYPE_ADDRESS].offset;
#endif
...
#else
		if (av_index >= ncp_header->vector_list[NCP_VECTOR_TYPE_ADDRESS].cnt) {
#endif
...
#else
	int_v_cnt = ncp_header->vector_list[NCP_VECTOR_TYPE_INTERRUPTION].cnt;
	int_v_offset = ncp_header->vector_list[NCP_VECTOR_TYPE_INTERRUPTION].offset;
#endif
...
#else
	gv_cnt = ncp_header->vector_list[NCP_VECTOR_TYPE_GROUP].cnt;
	gv_offset = ncp_header->vector_list[NCP_VECTOR_TYPE_GROUP].offset;
#endif
...
#else
		NPU_ERR_RET(validate_ncp_offset_range(
			ncp_header->body_offset + gv[i].isa_offset[0], gv[i].isa_size[0], 1, ncp_size),
			"group vector isa out of bounds. start=0x%x, size=0x%x, ncp_size=0x%lx\n",
			ncp_header->body_offset + gv[i].isa_offset[0], gv[i].isa_size[0], ncp_size);

		NPU_ERR_RET(validate_ncp_offset_range(
			ncp_header->body_offset + gv[i].isa_offset[1], gv[i].isa_size[1], 1, ncp_size),
			"group vector isa out of bounds. start=0x%x, size=0x%x, ncp_size=0x%lx\n",
			ncp_header->body_offset + gv[i].isa_offset[1], gv[i].isa_size[1], ncp_size);
...
static int validate_ncp_thread_vector(struct ncp_header *ncp_header, size_t ncp_size)
{
...
#else
	tv_cnt = ncp_header->vector_list[NCP_VECTOR_TYPE_THREAD].cnt;
	tv_offset = ncp_header->vector_list[NCP_VECTOR_TYPE_THREAD].offset;
#endif
	...
	for (i = 0; i < tv_cnt; i++) {
...
#else
		/* Validate group vector index out of bounds. */
		if (ncp_header->vector_list[NCP_VECTOR_TYPE_GROUP].cnt) {
			if (tv[i].group_str_idx >= ncp_header->vector_list[NCP_VECTOR_TYPE_GROUP].cnt ||
				tv[i].group_end_idx >= ncp_header->vector_list[NCP_VECTOR_TYPE_GROUP].cnt) {
				npu_err("thread vector group index out of bounds\n");
				return -EFAULT;
			}
		}
		...
		/* Validate interruption vector index out of bounds. */
		if (ncp_header->vector_list[NCP_VECTOR_TYPE_INTERRUPTION].cnt) {
			if (tv[i].interruption_str_idx >= ncp_header->vector_list[NCP_VECTOR_TYPE_INTERRUPTION].cnt ||
				tv[i].interruption_end_idx >= ncp_header->vector_list[NCP_VECTOR_TYPE_INTERRUPTION].cnt) {
				npu_err("thread vector interruption index out of bounds\n");
				return -EFAULT;
			}
		}
