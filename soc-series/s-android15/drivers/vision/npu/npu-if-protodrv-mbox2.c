	...
	if (unlikely(ret <= 0)) {
		...
		*target = msgid_claim_get_ref(pool, msgid, PROTO_DRV_REQ_TYPE_NW);
		...
		if (likely(*target != NULL)) {
			npu_info("mbox->protodrv : NW msgid(%d)\n", msgid);
			(*target)->nw.msgid = -1;
...
int npu_nw_mbox_ops_put(struct msgid_pool *pool, struct proto_req_nw *src)
{
	...
	src->nw.msgid = msgid;
	npu_info("protodrv-> : NW msgid(%d)\n", msgid);
	if (unlikely(msgid < 0)) {
		npu_warn("no message ID available. Posting message is not temporally available.\n");
		return 0;
	}
	...
	if (unlikely(ret <= 0)) {
		...
		npu_trace("nw_post_request failed. Reclaiming msgid(%d)\n", msgid);
		...
		*target = msgid_claim_get_ref(pool, msgid, PROTO_DRV_REQ_TYPE_FRAME);
		...
		if (likely(*target != NULL)) {
			...
			if ((*target)->frame.frame_id == 1) {
				...
				npu_info("target frame_id : %u, frame.ncp_type : %u\n", (*target)->frame.frame_id, frame.ncp_type);
...
int npu_kpi_frame_mbox_put(struct msgid_pool *pool, struct npu_frame *frame)
{
	...
	if (unlikely(ret <= 0)) {
		/* Push failed -> return msgid */
		npu_uftrace("frame_post_request failed. Reclaiming msgid(%d)\n", frame, frame->session->hids, msgid);
		msgid_claim(pool, msgid);
	}
	return ret;
}
