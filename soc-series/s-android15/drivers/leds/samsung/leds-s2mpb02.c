...
static struct s2mpb02_led_data *global_led_datas[S2MPB02_LED_MAX];
...
static u32 flash1_gpio;
static u32 torch1_gpio;
#ifdef CONFIG_LEDS_S2MPB02_MULTI_TORCH_REAR2
static u32 flash2_gpio;
static u32 torch2_gpio;
#endif
static enum s2mpb02_torch_current torch_recording;
static bool flash_config_factory;
static u32 original_brightness[S2MPB02_LED_MAX];

const char *get_torch_str(enum s2mpb02_torch_mode torch_mode)
{
	switch (torch_mode) {
...
static void s2mpb02_gpio_set(unsigned int gpio, int onoff)
{
	if (!gpio_is_valid(gpio)) {
		pr_info("%s: not valid gpio", __func__);
		return;
	}
...
int s2mpb02_set_torch_current(enum s2mpb02_torch_mode torch_mode, unsigned int intensity)
{
	...
	pr_info("%s: torch_mode[%s] intensity[%d] cur_brightness[%d]\n",
		__func__,
		get_torch_str(torch_mode), intensity, cur_brightness);
	mutex_lock(&led_data->lock);
...
int s2mpb02_set_flash_current(unsigned int intensity)
{
	...
	if (cur_brightness >= S2MPB02_FLASH_OUT_I_MAX)
