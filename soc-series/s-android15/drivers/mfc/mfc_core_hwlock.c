...
int mfc_core_get_hwlock_dev(struct mfc_core *core)
{
	...
	/* Reset HW, if it's in HWAPG state */
	mfc_core_run_hwapg_reset(core);
			...
			int drm_switch)
{
	...
	case NAL_Q_STATE_CREATED:
		if (mfc_core_nal_q_check_enable(core, ctx) == 0) {
			if (IS_BATCH(core)) {
				spin_lock_irqsave(&core->batch_lock, flags);
				if (ctx->batch_queue_cnt) {
					mfc_core_debug(2, "[BATCH] there are %d bufs\n", ctx->batch_queue_cnt);
					/* NAL QUEUE - BATCH*/
					...
					for (i = 0; i < ctx->batch_queue_cnt; i++) {
						if (mfc_core_nal_q_enqueue_in_buf(core, core_ctx,
							nal_q_handle->nal_q_in_handle) && i == 0) {
							mfc_debug(2, "[BATCH] first enqueue must succeed\n");
					...
					if (ctx->batch_queue_cnt) {
						mfc_debug(2, "[BATCH] start NAL QUEUE with batch mode\n");
					...
					ctx->batch_timeout = 0;
					mfc_debug(2, "[BATCH] clear batch queue cnt:%d, timeout:%d\n",
							ctx->batch_queue_cnt, ctx->batch_timeout);

					spin_unlock_irqrestore(&core->batch_lock, flags);

					mfc_batch_update(ctx);
				} else {
					spin_unlock_irqrestore(&core->batch_lock, flags);
					mfc_core_debug(2, "[BATCH] there is no batch cnt\n");
	...
	case NAL_Q_STATE_STARTED:
		if (mfc_core_nal_q_check_enable(core, ctx) == 0 ||
			...
			if (nal_q_handle->nal_q_exception == 2 || IS_BATCH(core)) {
