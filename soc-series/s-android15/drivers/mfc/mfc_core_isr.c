		...
		if (ctx->select_view_irq == MFC_VIEW_ID_MAIN) {
			...
			if (is_hdr10_plus_full) {
				if (dec->hdr10_plus_full_sub) {
					mfc_core_get_dec_metadata_sei_nal(core, ctx, index);
					mfc_set_mb_flag(dst_mb, MFC_FLAG_HDR_PLUS);
					mfc_ctx_debug(2, "[HDR+] HDR10 plus full SEI metadata of sub-view parsed\n");
				} else {
					mfc_ctx_err("[HDR+] HDR10 plus full of sub-view cannot be copied\n");
				}
			}
		}
...
static void __mfc_handle_released_buf(struct mfc_core *core, struct mfc_ctx *ctx)
{
	 ...
	 *
	 * 1) The driver splits the original released_flag into released_flag_sub, released_flag_main.
	...
	if (ctx->multi_view_enable) {
		...
		new_released_flag = ((unsigned long)released_flag_sub << (MFC_MAX_DPBS / 2)) | released_flag_sub;
	...
	if (dec->display_index >= 0) {
		...
		if (!ctx->multi_view_enable) {
			...
			if (!(dec->dynamic_used & (1UL << main_i)) && dec->dpb[main_i].mapcnt
					&& !dec->dpb[main_i].queued
					&& !(dec->dynamic_used & (1UL << sub_i)) && dec->dpb[sub_i].mapcnt
					&& !dec->dpb[sub_i].queued) {
			...
			unsigned int reason, unsigned int err)
{
	...
	if (qos_update) {
		...
		if (IS_TWO_MODE1(ctx)) {
			...
			if (subcore) {
				subcore->core_ctx[ctx->num]->dynamic_weight_level = core_ctx->dynamic_weight_level;
...
static int __mfc_handle_seq_dec(struct mfc_core *core, struct mfc_ctx *ctx)
{
	...
	if (dev->pdata->nal_q_batch_dec) {
		if (dev->debugfs.batch_buf_num > MFC_BATCH_FRAME)
			ctx->dpb_count += dev->debugfs.batch_buf_num - MFC_BATCH_FRAME;
		if ((IS_VP9_DEC(ctx) || IS_AV1_DEC(ctx)) && UNDER_FHD_RES(ctx))
			ctx->dpb_count += 1;
		mfc_ctx_debug(2, "[BATCH] dpb_count: %d\n", ctx->dpb_count);
	}
		...
		unsigned int reason, unsigned int err)
{
	...
	case MFC_REG_R2H_CMD_QUEUE_DONE_RET:
		...
		if (!nal_q_handle->nal_q_exception && !IS_BATCH(core))
	...
	case MFC_REG_R2H_CMD_COMPLETE_QUEUE_RET:
		if (IS_BATCH(core) && (nal_q_handle->nal_q_state == NAL_Q_STATE_BATCH_STARTED)) {
			if (core->hwapg_status == MFC_HWAPG_ENABLE) {
				mfc_core_set_hwapg_ctrl();
				core->hwapg_status = MFC_HWAPG_HOST_SET;
				mfc_core_enable_hwapg(core);
				mfc_core_debug(2, "[BATCH][HWAPG] set hwapg host ctrl and enable hwapg\n");
			}
			...
			while (pOutStr) {
				if (mfc_core_nal_q_handle_out_buf(core, pOutStr))
					mfc_core_err("[NALQ] Failed to handle out buf\n");
				mfc_core_debug(2, "[BATCH] dequeue bufs..\n");
				pOutStr = mfc_core_nal_q_dequeue_out_buf(core,
						nal_q_handle->nal_q_out_handle, &errcode);
			}
			...
			if (core->hwapg_status == MFC_HWAPG_HOST_SET) {
				mfc_core_wait_bus(core);
				mfc_core_clear_hwapg_ctrl();
				core->hwapg_status = MFC_HWAPG_HOST_CLEAR;
				if (core->dev->debugfs.batch_qos_enable)
					mfc_qos_off(core, core->dev->ctx[core->batch_index]);
				mfc_core_debug(2, "[BATCH][HWAPG] clear host ctrl\n");
			}
			core->sched->clear_work(core, core->core_ctx[core->batch_index]);
			core->sched->enqueue_work(core, core->core_ctx[core->batch_index]);
			if (nal_q_handle->nal_q_exception)
				core->sched->set_work(core, core->core_ctx[core->batch_index]);

			mfc_core_release_hwlock_ctx(core->core_ctx[core->batch_index]);
		} else {
			mfc_core_meerkat_stop_tick(core);
			nal_q_handle->nal_q_state = NAL_Q_STATE_CREATED;
			MFC_TRACE_CORE("** NAL Q state : %d\n", nal_q_handle->nal_q_state);
