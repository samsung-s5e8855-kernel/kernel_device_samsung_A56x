...
static int __mfc_enc_check_adaptive_temporal_svc(int id, struct mfc_enc_params *p,
		...
		struct mfc_ctx *ctx, struct mfc_buf_ctrl *buf_ctrl)
{
	...
	if (buf_ctrl->id
		...
		if (__mfc_enc_check_adaptive_temporal_svc(buf_ctrl->id, p, &temporal_LC)) {
			...
			struct list_head *head, EncoderInputStr *pInStr)
{
	...
	list_for_each_entry(buf_ctrl, head, list) {
		...
		case V4L2_CID_MPEG_VIDEO_HEVC_HIERARCHICAL_CODING_LAYER_CH:
			...
			if (__mfc_enc_check_adaptive_temporal_svc(buf_ctrl->id, p, &temporal_LC)) {
				if (temporal_LC.temporal_layer_count > 1)
					pInStr->ParamChange |= (0x1 << 21);
				else
					pInStr->ParamChange |= (0x2 << 21);
			}
