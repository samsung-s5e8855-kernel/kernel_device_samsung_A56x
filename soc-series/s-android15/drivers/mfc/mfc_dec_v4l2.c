				      ...
				      struct v4l2_pix_format_mplane *pix_fmt_mp)
{
	...
	mfc_set_view_buf_info(ctx, ctx->dst_fmt->mem_planes, num_fd_depth_map, num_fd_sub_view_meta);
...
static int mfc_dec_dqbuf(struct file *file, void *priv, struct v4l2_buffer *buf)
{
	...
	exynos_video_sub_view_meta *dst_sei_meta_sub;
	...
	if (buf->type == V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE) {
		...
		if (dec->hdr10_plus_full) {
			...
			if (ctx->multi_view_enable) {
				src_sei_full = HDR10_PLUS_ADDR(dec->hdr10_plus_full_sub, buf->index);
				sub_view_vaddr = dec->sub_view_vaddr[buf->index];
				if (sub_view_vaddr != NULL) {
					dst_sei_meta_sub = (exynos_video_sub_view_meta *) sub_view_vaddr;
					memcpy(&dst_sei_meta_sub->sHdrDynamicBlob, src_sei_full,
						HDR10_PLUS_DATA_SIZE);
					dst_sei_meta_sub->eType =
						VIDEO_SUBVIEW_INFO_TYPE_HDR_DYNAMIC_BLOB;
					if (ctx->dev->debugfs.hdr_dump == 1) {
						mfc_ctx_err("[HDR+][DUMP] SH_HANDLE data for sub-view (idx %d)....\n",
								buf->index);
						print_hex_dump(KERN_ERR, "", DUMP_PREFIX_OFFSET, 32,
								4, sub_view_vaddr, 68, false);
					}
				}
			}
			 ...
			 struct v4l2_control *ctrl)
{
	...
	case V4L2_CID_MPEG_MFC_BATCH_DISABLE:
		mfc_ctx_debug(2, "[BATCH] disable batch mode: %d\n", ctrl->value);
		ctx->batch_disable = ctrl->value;
		break;
