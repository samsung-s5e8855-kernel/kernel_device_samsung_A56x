...
int __mfc_core_nal_q_check_enable_dec(struct mfc_core *core, struct mfc_ctx *ctx)
{
	...
	/* NAL-Q DPB options can support multi-frame, interlaced */
	if (!IS_DPB(core) && !IS_BATCH(core)) {
		if (dec->is_multiframe && ctx->plugin_type) {
			core->nal_q_stop_cause |= (1 << NALQ_STOP_MULTI_FRAME);
			mfc_core_debug(2, "[MULTIFRAME] There is a multi frame to the end\n");
			return 0;
		}
...
int mfc_core_nal_q_check_enable(struct mfc_core *core, struct mfc_ctx *curr_ctx)
{
	struct mfc_dev *dev = core->dev;
	struct mfc_ctx *ctx;
	...
	for (i = 0; i < MFC_NUM_CONTEXTS; i++) {
		...
		if (core_ctx && ctx) {
			...
			if (mfc_check_buf_mb_flag_all(ctx, core_ctx, MFC_FLAG_LAST_FRAME) == 1) {
			...
			if (ctx->type == MFCINST_DECODER) {
				ret = __mfc_core_nal_q_check_enable_dec(core, ctx);
				if (!ret)
					goto nal_start;
			...
			if (ctx->multi_view_enable) {
				core->nal_q_stop_cause |= (1 << NALQ_STOP_MULTI_VIEW);
				mfc_debug(2, "There is an instance of Multi-View\n");
				goto nal_start;
			}
	...
	/* Check NAl-Q batch mode */
	if (mfc_batch_mode_check(core, curr_ctx)) {
...
void mfc_core_nal_q_clock_on(struct mfc_core *core, nal_queue_handle *nal_q_handle)
{
	...
	if (clk_on && (mfc_core_get_clk_ref_cnt(core) == 1))
...
void mfc_core_nal_q_clock_off(struct mfc_core *core, nal_queue_handle *nal_q_handle)
{
	...
	if (clk_off && (mfc_core_get_clk_ref_cnt(core) == 0))
		...
		nal_queue_handle *nal_q_handle)
{
	...
	if (core->dev->pdata->nal_q_dpb || core->dev->pdata->nal_q_batch_dec) {
		nal_q_in_handle->dpb_offset =
			(core->dev->pdata->nal_q_entry_size * NAL_Q_QUEUE_SIZE);
		mfc_core_debug(2, "[NALQ][OPT] DPBInputStr offset: %d\n", nal_q_in_handle->dpb_offset);
		/* DPB size (16ea * 16 bytes) * 32 instance = 8,192 bytes */
		nal_q_in_handle->in_buf.size += core->dev->pdata->nal_q_dpb_size * (MFC_NUM_CONTEXTS);
	}
...
void mfc_core_nal_q_init(struct mfc_core *core, nal_queue_handle *nal_q_handle)
{
	...
	if (IS_DPB(core) || IS_BATCH(core)) {
		mfc_core_debug(2, "[NALQ][OPT] clear DPBInputStruct region\n");
		memset((DPBInputStr *)(nal_q_handle->nal_q_in_handle->nal_q_in_addr +
				nal_q_handle->nal_q_in_handle->dpb_offset),
				0,
				core->dev->pdata->nal_q_dpb_size * MFC_NUM_CONTEXTS);
		for (i = 0; i < MFC_NUM_CONTEXTS; i++)
			nal_q_handle->nal_q_dpb_idx[i] = 0;
	}
...
void mfc_core_nal_q_start(struct mfc_core *core, nal_queue_handle *nal_q_handle)
{
	...
	if (IS_BATCH(core))
		nal_q_handle->nal_q_state = NAL_Q_STATE_BATCH_STARTED;
	else
...
static unsigned int __mfc_core_nal_q_handle_frame_field(struct mfc_core *core,
		struct mfc_ctx *ctx, DecoderOutputStr *pOutStr)
{
	...
	if (is_interlace || dec->is_mbaff)
		mfc_ctx_debug(2, "[NALQ][INTERLACE] is_interlace: %d (type : %d), is_mbaff: %d, field: %#x\n",
...
static int __mfc_core_nal_q_run_in_buf_dec_dpb(struct mfc_core *core, struct mfc_core_ctx *core_ctx)
{
	nal_queue_in_handle *nal_q_in_handle;
	struct mfc_ctx *ctx = core_ctx->ctx;
	struct mfc_dec *dec = ctx->dec_priv;
	...
	unsigned int dpb_slot_index = 0, dpb_slot_offset = 0, inst_offset = 0;
	int dst_index, i;
	DPBInputStr *dpb_slot_ptr = NULL;

	if (!dec->is_dynamic_dpb) {
		mfc_err("[NALQ] only support dynamic_dpb for NAL_Q DPB opt\n");
		return -EINVAL;
	}

	nal_q_in_handle = core->nal_q_handle->nal_q_in_handle;

	inst_offset = core->dev->pdata->nal_q_dpb_size * core_ctx->inst_no;
	dpb_slot_index = core->nal_q_handle->nal_q_dpb_idx[core_ctx->inst_no] % NAL_Q_DPB_INPUT_NUM;
	dpb_slot_offset = dpb_slot_index * NAL_Q_DPB_INPUT_SIZE;
	dpb_slot_ptr = (DPBInputStr *)(nal_q_in_handle->nal_q_in_addr +
			nal_q_in_handle->dpb_offset + inst_offset + dpb_slot_offset);

	mfc_debug(2, "[NALQ][OPT] DPBInStr idx: %d, offset: %d, inst_offset: %d\n",
			dpb_slot_index, dpb_slot_offset, inst_offset);
	MFC_TRACE_CTX("NALQ opt: DPBInStr idx: %d, offset: %d, inst_offset: %d\n",
			dpb_slot_index, dpb_slot_offset, inst_offset);

	if (dpb_slot_ptr->DPBIdx) {
		if (!mfc_get_queue_count(&ctx->buf_queue_lock, &ctx->src_buf_nal_queue)) {
			mfc_debug(2, "[NALQ][OPT] DPBIdx slot is full (no src)\n");
		} else {
			mfc_err("[NALQ][OPT] DPBIdx slot is full\n");
			print_hex_dump(KERN_ERR, "", DUMP_PREFIX_OFFSET, 32, 4,
					(int *)(nal_q_in_handle->nal_q_in_addr +
						nal_q_in_handle->dpb_offset + inst_offset),
					core->dev->pdata->nal_q_dpb_size, false);
		}
		return -EBUSY;
	}
	...
	raw = &core_ctx->ctx->raw_buf;
	for (i = 0; i < raw->num_planes; i++) {
		dpb_slot_ptr->DPBAddr[i] = MFC_NALQ_DMA_WRITEL(dst_mb->addr[0][i]);
	...
	/*
	 * FW is polling the dpb_idx value until non-zero.
	 * Must update the DPB index after writing the DPB address
	 */
	dpb_slot_ptr->DPBIdx = dst_index + 1;

	if (core->dev->debugfs.nal_q_dump == 1) {
		mfc_err("[NAL-Q][DUMP][DPB][c: %d]\n", core_ctx->inst_no);
		print_hex_dump(KERN_ERR, "", DUMP_PREFIX_OFFSET, 32, 4,
				(int *)(nal_q_in_handle->nal_q_in_addr +
					nal_q_in_handle->dpb_offset + inst_offset),
				core->dev->pdata->nal_q_dpb_size, false);
	...
	core->nal_q_handle->nal_q_dpb_idx[core_ctx->inst_no]++;
...
static int __mfc_core_nal_q_run_in_buf_dec_src(struct mfc_core *core, struct mfc_core_ctx *core_ctx,
			DecoderInputStr *pInStr)
{
	...
	/*
	 * In NAL_Q mode, firmware handles consumed bytes
	 * so do not use mfc_dec_get_strm_size/offset() function
	 */
	if (pInStr->CommandId == 0) {
		strm_size = mfc_dec_get_strm_size(ctx, src_mb);
		offset = mfc_dec_get_strm_offset(ctx, src_mb);
	} else {
		if (vb->planes[0].bytesused > vb->planes[0].data_offset) {
			strm_size = vb->planes[0].bytesused - vb->planes[0].data_offset;
...
static int __mfc_core_nal_q_run_in_buf_dec_dst(struct mfc_core *core, struct mfc_core_ctx *core_ctx,
			DecoderInputStr *pInStr)
{
	struct mfc_ctx *ctx = core_ctx->ctx;
	struct mfc_dec *dec = ctx->dec_priv;
			...
			DecoderInputStr *pInStr)
{
	...
	/* buffer count check & dst setting */
	if (IS_DPB(core) || IS_BATCH(core)) {
		if (!mfc_is_queue_count_same(&ctx->buf_queue_lock, &ctx->dst_buf_queue, 0))
			__mfc_core_nal_q_run_in_buf_dec_dpb(core, core_ctx);
		...
		if (__mfc_core_nal_q_run_in_buf_dec_dst(core, core_ctx, pInStr))
			return -EAGAIN;
	}
	...
	if (__mfc_core_nal_q_run_in_buf_dec_src(core, core_ctx, pInStr))
		return -EAGAIN;
	...
	raw = (dec->is_dynamic_dpb) ? &ctx->raw_buf : &ctx->internal_raw_buf;
	for (i = 0; i < raw->num_planes; i++) {
		...
		struct mfc_ctx *ctx, DecoderOutputStr *pOutStr)
{
	...
	if (dst_mb) {
		...
		dst_mb->vb.sequence = ctx->sequence;
		dst_mb->vb.field = __mfc_core_nal_q_handle_frame_field(core, ctx, pOutStr);
		...
		if (is_av1_film_grain_sei) {
			...
			} else if (dec->av1_film_grain_info) {
		...
		if (dec->immediate_display == 1) {
			mfc_ctx_debug(6, "update frame tag for imediate display\n");
			...
			unsigned int err, DecoderOutputStr *pOutStr)
{
	 ...
	 */
	if (dec->consumed && !IS_DPB(core)
			&& core->nal_q_handle->nal_q_state != NAL_Q_STATE_BATCH_STARTED) {
	...
	if (IS_BATCH(core) && ctx->batch_debug) {
		ctx->batch_debug = 0;
		call_dop(core, dump_and_broadcast, core);
	}
	if (!src_mb) {
	...
	if (!deleted) {
		...
		if (!IS_DPB(core) && core->nal_q_handle->nal_q_state != NAL_Q_STATE_BATCH_STARTED) {
			core->nal_q_stop_cause |= (1 << NALQ_EXCEPTION_MULTI_FRAME);
			core->nal_q_handle->nal_q_exception = 1;
		}

		mfc_ctx_debug(2, "[NALQ][MULTIFRAME] consumed:%d, remained:%d\n",
			dec->consumed, mfc_dec_get_strm_size(ctx, src_mb));
		MFC_TRACE_CTX("** consumed:%d, remained:%d\n",
			dec->consumed, mfc_dec_get_strm_size(ctx, src_mb));
	...
	if (((IS_VP8_DEC(ctx) || IS_VP9_DEC(ctx)) &&
		(dst_frame_status == MFC_REG_DEC_STATUS_DECODING_ONLY)) ||
		((pOutStr->DisplayStatus >> MFC_REG_DISP_STATUS_FIELD_SHIFT)
					& MFC_REG_DISP_STATUS_FIELD_MASK)) {
		mfc_set_mb_flag(src_mb, MFC_FLAG_CONSUMED_ONLY);
		mfc_ctx_debug(2, "[NALQ][STREAM] %s decoding only stream has no buffer to DQ\n",
				ctx->src_fmt->name);
	}
	...
	vb2_buffer_done(&src_mb->vb.vb2_buf, VB2_BUF_STATE_DONE);
	mfc_ctx_debug(2, "[NALQ][STREAM] src index: %d(%d), addr: 0x%08llx is buffer done\n",
			...
			DecoderOutputStr *pOutStr)
{
	...
	if (dst_frame_status != MFC_REG_DEC_STATUS_DISPLAY_ONLY)
		...
		if (!IS_DPB(core) && core->nal_q_handle->nal_q_state != NAL_Q_STATE_BATCH_STARTED)
			mfc_debug(2, "[NALQ] can't support display only in NAL-Q, is_dpb_full: %d\n",
				dec->is_dpb_full);
...
static void __mfc_core_nal_q_update_avail_slot(struct mfc_core *core)
{
	...
	/*
	 * output_cmd_count is the number of execution for the same input.
	 * When nal_q_exception, interlace NPF(Non-Paired Field) and multiframe, it is not zero.
	 */
	if (output_cmd_count) {
		mfc_core_debug(2, "[NALQ][LL] same index %d is freed (output_cmd_cnt: %d)\n",
				index, output_cmd_count);
		return;
	}
	...
	nal_queue_in_handle *nal_q_in_handle)
{
	...
	state = nal_q_in_handle->nal_q_handle->nal_q_state;
	if ((state != NAL_Q_STATE_STARTED)
		&& !(IS_BATCH(core)	&& (state == NAL_Q_STATE_CREATED))) {
