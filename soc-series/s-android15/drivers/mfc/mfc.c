...
static int __mfc_init_dec_ctx(struct mfc_ctx *ctx)
{
	...
	timer_setup(&ctx->batch_timer, mfc_batch_timeout, 0);
...
static int __mfc_parse_dt(struct device_node *np, struct mfc_dev *mfc)
{
	...
	of_property_read_u32(np, "nal_q_dpb_size", &pdata->nal_q_dpb_size);

	/* NAL-Q mode & Batch */
	of_property_read_u32(np, "nal_q_ll", &pdata->nal_q_ll);
	of_property_read_u32(np, "nal_q_dpb", &pdata->nal_q_dpb);
	of_property_read_u32(np, "nal_q_batch_dec", &pdata->nal_q_batch_dec);
	of_property_read_u32(np, "nal_q_batch_enc", &pdata->nal_q_batch_enc);
	of_property_read_u32(np, "batch_drm", &pdata->batch_drm);
	of_property_read_u32(np, "nal_q_mode1", &pdata->nal_q_mode1);
