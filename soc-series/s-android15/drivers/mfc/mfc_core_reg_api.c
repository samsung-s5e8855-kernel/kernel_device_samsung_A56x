...
int mfc_core_set_dec_codec_buffers(struct mfc_core_ctx *core_ctx)
{
	...
	if (ctx->multi_view_enable) {
		reg &= ~(0x1 << MFC_REG_D_INIT_BUF_OPT_ENABLE_DECODE_ONLY_VIEW0);
	}
		...
		struct mfc_buf *mfc_buf)
{
	...
	if (IS_MULTI_MODE(ctx) && !ctx->is_drm &&
		...
		struct mfc_buf *dst_mb)
{
	...
	for (i = 0; i < raw->num_planes; i++) {
		...
		if (IS_2BIT_NEED(ctx))
			MFC_CORE_WRITEL(raw->plane_size_2bits[i],
					MFC_REG_D_FIRST_PLANE_2BIT_DPB_SIZE + (i * 4));
		mfc_debug(2, "[BUFINFO][DPB] set dst index: [%d][%d], addr[%d]: 0x%08llx, fd: %d, size: %d, index_view: %d, offset: %d\n",
				dst_mb->vb.vb2_buf.index, dst_mb->dpb_index,
				i, dst_mb->addr[index_view][i], dst_mb->vb.vb2_buf.planes[offset + i].m.fd,
				raw->plane_size[i], index_view, offset);
	}
...
void memcpy_for_sub_view_meta(struct hdr10_plus_meta *dst_sei_meta,
		exynos_video_sub_view_meta *src_sub_meta)
{
	int i;

	dst_sei_meta->valid = src_sub_meta->sHdrDynamicInfo.valid;

	dst_sei_meta->t35_country_code = src_sub_meta->sHdrDynamicInfo.data.country_code;
	dst_sei_meta->t35_terminal_provider_code = src_sub_meta->sHdrDynamicInfo.data.provider_code;
	dst_sei_meta->t35_terminal_provider_oriented_code =
		src_sub_meta->sHdrDynamicInfo.data.provider_oriented_code;
	dst_sei_meta->application_identifier =
		src_sub_meta->sHdrDynamicInfo.data.application_identifier;
	dst_sei_meta->application_version = src_sub_meta->sHdrDynamicInfo.data.application_version;
	dst_sei_meta->num_windows = src_sub_meta->sHdrDynamicInfo.data.num_windows;

	dst_sei_meta->target_maximum_luminance =
		src_sub_meta->sHdrDynamicInfo.data.targeted_system_display_maximum_luminance;
	dst_sei_meta->target_actual_peak_luminance_flag =
		src_sub_meta->sHdrDynamicInfo.data.targeted_system_display_actual_peak_luminance_flag;

	dst_sei_meta->mastering_actual_peak_luminance_flag =
		src_sub_meta->sHdrDynamicInfo.data.mastering_display_actual_peak_luminance_flag;

	for (i = 0; i < HDR_MAX_WINDOWS; i++) {
		memcpy(dst_sei_meta->win_info[i].maxscl, src_sub_meta->sHdrDynamicInfo.data.maxscl[i],
			HDR_MAX_WINDOWS * sizeof(int));
		dst_sei_meta->win_info[i].average_maxrgb =
			src_sub_meta->sHdrDynamicInfo.data.average_maxrgb[i];
		dst_sei_meta->win_info[i].num_distribution_maxrgb_percentiles =
			src_sub_meta->sHdrDynamicInfo.data.num_maxrgb_percentiles[i];
		memcpy(dst_sei_meta->win_info[i].distribution_maxrgb_percentages,
			src_sub_meta->sHdrDynamicInfo.data.maxrgb_percentages[i],
			HDR_MAX_DISTRIBUTION * sizeof(char));
		memcpy(dst_sei_meta->win_info[i].distribution_maxrgb_percentiles,
			src_sub_meta->sHdrDynamicInfo.data.maxrgb_percentiles[i],
			HDR_MAX_DISTRIBUTION * sizeof(int));
		dst_sei_meta->win_info[i].fraction_bright_pixels =
			src_sub_meta->sHdrDynamicInfo.data.fraction_bright_pixels[i];
		dst_sei_meta->win_info[i].tone_mapping_flag =
			src_sub_meta->sHdrDynamicInfo.data.tone_mapping.tone_mapping_flag[i];
		dst_sei_meta->win_info[i].knee_point_x =
			src_sub_meta->sHdrDynamicInfo.data.tone_mapping.knee_point_x[i];
		dst_sei_meta->win_info[i].knee_point_y =
			src_sub_meta->sHdrDynamicInfo.data.tone_mapping.knee_point_y[i];
		dst_sei_meta->win_info[i].num_bezier_curve_anchors =
			src_sub_meta->sHdrDynamicInfo.data.tone_mapping.num_bezier_curve_anchors[i];
		memcpy(dst_sei_meta->win_info[i].bezier_curve_anchors,
			src_sub_meta->sHdrDynamicInfo.data.tone_mapping.bezier_curve_anchors[i],
			HDR_MAX_BEZIER_CURVES * sizeof(short));

		dst_sei_meta->win_info[i].color_saturation_mapping_flag =
			src_sub_meta->sHdrDynamicInfo.data.color_saturation_mapping_flag[i];
		dst_sei_meta->win_info[i].color_saturation_weight =
			src_sub_meta->sHdrDynamicInfo.data.color_saturation_weight[i];
	}
...
void mfc_core_get_dec_metadata_sei_nal(struct mfc_core *core, struct mfc_ctx *ctx, int index)
{
	...
	if (ctx->select_view == MFC_VIEW_ID_MAIN)
		...
		addr = HDR10_PLUS_ADDR(dec->hdr10_plus_full_sub, index);
