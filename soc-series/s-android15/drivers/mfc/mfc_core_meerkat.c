...
static int (*mfc_secdbg_cb)(char *errorInfo) = NULL;

int mfc_register_secdbg_cb(
        int (*secdbg_cb)(char *errorinfo))
{
    mfc_secdbg_cb = secdbg_cb;
    return 0;
}
EXPORT_SYMBOL(mfc_register_secdbg_cb);
...
static void __mfc_save_logging_sfr(struct mfc_core *core)
{
	...
	/* last information */
	core->logging_data->curr_ctx =
		(curr_ctx < 0) ? 0xFF /* max of u8 */ : curr_ctx;
...
void mfc_dump_state(struct mfc_dev *dev)
{
	...
	for (i = 0; i < MFC_NUM_CONTEXTS; i++) {
		if (dev->ctx[i]) {
			...
			mfc_dev_err("	main core: %d, op_mode: %d(stream: %d), idle_mode: %d, wait_state %d, prio %d, rt %d, queue_cnt(src:%d, dst:%d, ref:%d, qsrc:%d, qdst:%d)\n",
...
void __mfc_core_dump_state(struct mfc_core *core, int curr_ctx)
{
	...
	dev_err(core->device, "batch enable: %d, hwapg status: %d, batch_index: %d\n",
			core->batch_enable, core->hwapg_status, core->batch_index);
	if (nal_q_handle)
		dev_err(core->device, "NAL-Q state:%d, mode: %d, exception:%d, in_exe_cnt: %d, out_exe_cnt: %d, stop cause: %#x\n",
...
void __mfc_dump_nal_q_buffer_info(struct mfc_core *core, int curr_ctx)
{
	...
	int i, offset, Inindex, index, cnt = 0;
	...
	/* Input queue */
	if (IS_LL(core)) {
		pEncIn = (EncoderInputStr *)(nal_q_in_handle->nal_q_in_addr);
		dev_err(core->device, "-----------dumping NAL Linked List status\n");
		do {
			index = pEncIn->NextListIndex;
			offset = dev->pdata->nal_q_entry_size * index;
			pEncIn = (EncoderInputStr *)(nal_q_in_handle->nal_q_in_addr + offset);
			dev_err(core->device, "[NALQ][LL][c:%d] index %d, prio %d, NextListIndex %d\n",
					pEncIn->InstanceId, index,
					pEncIn->Priority, pEncIn->NextListIndex);
			cnt++;
			if (cnt > 31)
				break;
		} while (pEncIn->NextListIndex);
	}

	if (IS_DPB(core) || IS_BATCH(core)) {
		dev_err(core->device, "-----------dumping NAL DPB status[ctx:%d]\n", ctx->num);
		print_hex_dump(KERN_ERR, "", DUMP_PREFIX_OFFSET, 32, 4,
				(int *)(nal_q_in_handle->nal_q_in_addr +
				nal_q_in_handle->dpb_offset +
				(core->dev->pdata->nal_q_dpb_size * ctx->num)),
