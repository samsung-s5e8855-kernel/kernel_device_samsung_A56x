...
static int __mfc_core_parse_llcaid_sfr(struct mfc_core *core, unsigned int llcaid_idx)
{
	struct device_node *node = core->device->of_node;
	const __be32 *prop;
	size_t num_of_member =
		sizeof(struct mfc_llcaid_sfrs)/sizeof(unsigned int);
	int i, sfr_idx, cnt;
	char *node_name;

	node_name = kasprintf(GFP_KERNEL, "sfr_llcaid_%d", llcaid_idx);
	prop = of_get_property(node, node_name, &cnt);
	if (!prop) {
		mfc_core_err("No data for llcaid_0\n");
		kfree(node_name);
		return -ENOENT;
	}

	cnt /= sizeof(unsigned int);
	if (cnt % num_of_member != 0) {
		mfc_core_err("Invalid number(%d) of values\n", cnt);
		kfree(node_name);
		return -EINVAL;
	}

	core->llcaid_info[llcaid_idx].sfrs = devm_kzalloc(core->device,
			sizeof(unsigned int) * cnt, GFP_KERNEL);
	if (!core->llcaid_info[llcaid_idx].sfrs) {
		mfc_core_err("no memory for llcaid_%d sfr\n", llcaid_idx);
		return -ENOMEM;
	}

	for (i = 0; i < cnt; i += num_of_member) {
		sfr_idx = i / num_of_member;
		core->llcaid_info[llcaid_idx].sfrs[sfr_idx].offset =
			of_read_number(prop + i, 1);
		core->llcaid_info[llcaid_idx].sfrs[sfr_idx].data =
			of_read_number(prop + i + 1, 1);
		core->llcaid_info[llcaid_idx].num_sfrs++;
	}
...
static int __mfc_core_register_llcaid_base(struct mfc_core *core)
{
	struct device_node *node = core->device->of_node;
	const __be32 *prop;
	...
	int i, cnt, num_sfr_bases, llcaid_idx;
	phys_addr_t sfr_base;
	size_t sfr_size;

	prop = of_get_property(node, "llcaid", &cnt);
	if (!prop) {
		mfc_core_info("No data for llcaid SFR\n");
		return 0;
	}
	...
	num_sfr_bases = cnt / n_all_cells;
	core->llcaid_info = devm_kzalloc(core->device,
		sizeof(struct mfc_llcaid_info) * num_sfr_bases, GFP_KERNEL);
	if (!core->llcaid_info) {
		mfc_core_err("no memory for llcaid_info\n");
		return -ENOMEM;
	}

	for (i = 0; i < cnt; i += n_all_cells) {
		llcaid_idx = i / n_all_cells;
		sfr_base = of_read_number(prop + i, n_addr_cells);
		sfr_size = of_read_number(prop + i + n_addr_cells, n_size_cells);
		core->llcaid_info[llcaid_idx].base =
			devm_ioremap(core->device, sfr_base, sfr_size);
		if (!core->llcaid_info[llcaid_idx].base)
			return -ENOMEM;
		core->llcaid_cnt++;
	}
...
static int __mfc_core_update_llcaid(struct mfc_core *core)
{
	int i, ret;

	ret = __mfc_core_register_llcaid_base(core);
	if (ret) {
		mfc_core_err("failed to update llcaid SFR base\n");
		return ret;
	}

	for (i = 0; i < core->llcaid_cnt; i++) {
		ret = __mfc_core_parse_llcaid_sfr(core, i);
		if (ret) {
			mfc_core_err("failed to parse llcaid_%d dt \n", i);
			return ret;
		}
	}
		...
		struct mfc_core *core)
{
	...
	if (pmu) {
		...
		if (core->pmu_base == NULL) {
			core->has_pmu = 0;
			dev_err(&pdev->dev, "failed to iomap pmu address region\n");
...
static int mfc_core_probe(struct platform_device *pdev)
{
	...
	ret = __mfc_core_update_llcaid(core);
	if (ret)
		goto err_update_llcaid;
