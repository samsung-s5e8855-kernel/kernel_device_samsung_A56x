		...
		unsigned int processing_cycle, unsigned int frame_type)
{
	...
	unsigned long frame_time, hw_mb, need_mb, avg_mb, margin_mb, total_mb = 0;
	...
	frame_time = processing_cycle / (core->last_mfc_freq / 1000) + MFC_DRV_TIME;
	...
	if (frame_time) {
		new_mb->mb_per_sec = (1000000 * hw_mb) / frame_time;
	...
	if (need_mb < MFC_MB_PER_TABLE)
		margin_mb = need_mb / 2;
			...
			(IS_MULTI_MODE(ctx) && IS_MFC_HEAVY_PERF(ctx, need_fps)))
	...
	if (atomic_read(&core->qos_req_cur)) {
		...
		if (ctx->type == MFCINST_DECODER && (new_mb->mb_per_sec < need_mb)
			&& (cur_qos < num_qos_steps)) {
			level_num = ((need_mb - new_mb->mb_per_sec) + MFC_MB_PER_TABLE - 1) / MFC_MB_PER_TABLE;
			if (cur_qos + level_num <= num_qos_steps)
				core_ctx->dynamic_weight_level = core_ctx->dynamic_weight_level + level_num;
			else
...
#ifdef CONFIG_MFC_USE_BTS
		__mfc_qos_set(core, ctx, &mfc_bw, table_type, qos_level);
#else
