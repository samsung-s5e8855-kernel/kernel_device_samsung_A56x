	 ...
	 * this can be set after the header was parsed */
		...
		*plane_count = ctx->num_fd_frame;
		...
		if (ctx->multi_view_enable) {
			...
			/* View1 Meta */
			offset = ctx->view_buf_info[MFC_MV_BUF_IDX_VIEW1_META].offset;
			psize[offset] = HDR10_PLUS_DATA_SIZE;
			alloc_devs[offset] = dev->device;
		}
...
static void mfc_dec_buf_queue(struct vb2_buffer *vb)
{
	...
	if (vq->type == V4L2_BUF_TYPE_VIDEO_OUTPUT_MPLANE) {
		...
		dec->sub_view_vaddr[buf->vb.vb2_buf.index] =
			vb2_plane_vaddr(&buf->vb.vb2_buf,
