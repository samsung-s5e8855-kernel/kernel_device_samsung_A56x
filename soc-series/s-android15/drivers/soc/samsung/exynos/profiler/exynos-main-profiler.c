...
static inline bool is_enabled(s32 id)
{
	...
	return profiler.main.domains[id].enabled;
}
...
static inline struct domain_data *next_domain(s32 *id)
{
	...
	for (++idx; idx < NUM_OF_DOMAIN; idx++)
		if (profiler.main.domains[idx].enabled)
			break;
	*id = idx;

	return idx == NUM_OF_DOMAIN ? NULL : &profiler.main.domains[idx];
}
...
{
	int ret = atomic_xchg(&profiler.main.profiler_running, 1);

	profiler.main.profiler_stay_counter = PROFILER_STAY_TIMES;
	if (ret == 0)
		schedule_work_on(0, &profiler.main.profiler_wq);
}

static void profiler_sleep(void)
{
	if (profiler.main.profiler_stay_counter > 0) {
		wakeup_gfx(0);
		wakeup_job_submit(0);
		profiler.main.profiler_stay_counter = 0;
	}
}

static int profiler_check_gfx(void)
{
	int cnt = profiler.main.gfx_wakeup_counter;
	int heavy_thr1 = profiler.main.gfx_wakeup_thr1 << 3;
	int heavy_thr2 = profiler.main.gfx_wakeup_thr2 << 3;
	int heavy_thr3 = profiler.main.gfx_wakeup_thr3 << 3;

	profiler.main.gfx_wakeup_counter = 0;

	/* Workload detection */
	if (psd.gfxinfo[1] > heavy_thr1 && psd.gfxinfo[2] > heavy_thr2 && psd.gfxinfo[3] > heavy_thr3)
		return 1;

	if (psd.gfxinfo[1] < profiler.main.gfx_wakeup_thr1)
		return 0;

	if (psd.gfxinfo[2] < profiler.main.gfx_wakeup_thr2)
		return 0;

	if (psd.gfxinfo[3] < profiler.main.gfx_wakeup_thr3)
		return 0;

	profiler.main.gfx_wakeup_counter = ++cnt;
	if (profiler.main.gfx_wakeup_counter > DEF_GFX_COUNTER)
		return 1;

	return 0;
}

static int profiler_check_user(void)
{
	if (psd.user_target_fps) {
		return 1;
	} else {
		return 0;
	}
...
{
	int ret = (profiler.main.profiler_state & PROFILER_STATE_JOB_SUBMIT)? 1:0;
	u64 mainbw = psd.stats1_sum + psd.stats2_sum;
	int ratio = (mainbw > 0)? (int)(psd.stats2_sum *100 /mainbw) : 0;

	if (ret) {
		if (mainbw < profiler.main.perf_mainbw_thr || ratio < profiler.main.perf_gpubw_dn_ratio)
			ret = 0;
	} else {
		if (mainbw > profiler.main.perf_mainbw_thr && ratio > profiler.main.perf_gpubw_up_ratio)
			ret = 1;
	}
...
static void print_profile_sharing_data(struct profile_sharing_data *data, char *buf, int size) {
	int len = 0;
	len += snprintf(buf + len, size - len, "profiler:");
	len += snprintf(buf + len, size - len, "%d,%d,%d,%d,",
		data->monitor, data->enable_gfx, data->disable_job_submit, data->disable_dyn_mo);

	len += snprintf(buf + len, size - len, "%lld,%lld,%lld,",
		data->profile_time_ms, data->frame_done_time_us, data->frame_vsync_time_us);

	len += snprintf(buf + len, size - len, "%llu,%llu,%llu,",
		data->profile_frame_cnt, data->profile_frame_vsync_cnt, data->profile_fence_cnt);

	len += snprintf(buf + len, size - len, "%lld,%d,%d,%llu,",
		data->profile_fence_time_us, data->user_fps, data->frame_cnt_by_swap, data->delta_ms_by_swap);
#if IS_ENABLED(CONFIG_EXYNOS_PROFILER_CPU)
	int max = -1;
	int i, j = 0;

	for (i = 0; i < NUM_OF_CPU_DOMAIN; i++) {
		len += snprintf(buf + len, size - len, "%d,%d,%d,",
			data->max_freq[i], data->min_freq[i], data->freq[i]);
		len += snprintf(buf + len, size - len, "%llu,%llu,%d,%d,",
			data->dyn_power[i], data->st_power[i],data->temp[i], data->active_pct[i]);

		for (j = 0; j < MAXNUM_OF_CPU; j++) {
			if (max < data->cpu_active_pct[i][j]) {
				max = data->cpu_active_pct[i][j];
			}
		}
		len += snprintf(buf + len, size - len, "%d,", max);
		max = -1;
	}
#endif
#if IS_ENABLED(CONFIG_EXYNOS_PROFILER_MIF)
	i = PROFILER_MIF;
	...
	len += snprintf(buf + len, size - len, "%llu,%llu,%llu,",
		data->stats0_sum, data->stats0_ratio, data->stats0_avg);

	len += snprintf(buf + len, size - len, "%llu,%llu,%llu,%llu,",
		data->stats1_sum, data->stats1_ratio, data->stats2_sum, data->stats2_ratio);

	len += snprintf(buf + len, size - len, "%llu,%d,%d,",
		data->llc_status, data->mif_pm_qos_cur_freq, data->llc_config);
#endif
#if IS_ENABLED(CONFIG_EXYNOS_PROFILER_DSU)
	i = PROFILER_DSU;
	...
	len += snprintf(buf + len, size - len, "%d,", data->gpu_mo);

	for (i = 0; i < NUM_OF_RTIMEINFO; i++) {
		len += snprintf(buf + len, size - len, "%llu,", data->rtimes[i]);
	}

	for (i = 0; i < NUM_OF_DOMAIN; i++) {
		len += snprintf(buf + len, size - len, "%u,%d,",
			data->pb_mgn_no_boost[i], data->pb_mgn[i]);
	}
	for (i = 0; i < NUM_OF_PID; i++) {
		len += snprintf(buf + len, size - len, "%u,",data->pid_list[i]);
		len += snprintf(buf + len, size - len, "%u,",data->rendercore_list[i]);
	}
#endif
}
...
static void profiler_work(struct work_struct *work)
{
	...
	while (profiler.main.profiler_stay_counter > 0) {
		ktime_t ts = ktime_get();
		int remained_ms = PROFILER_INTERVAL_MS - (int)(ktime_to_ms(ts) % PROFILER_INTERVAL_MS);

		profiler.main.profiler_stay_counter--;
		profiler_update_profile();

		if (profiler.main.profiler_stay_counter > 0 && (profiler_check_gfx() || profiler_check_user())) {
			wakeup_gfx(1);
		} else if ((profiler.main.profiler_state & PROFILER_STATE_GFX) && !(profiler_check_gfx() && profiler_check_user())) {
			wakeup_gfx(0);
		}

		if (profiler.main.profiler_stay_counter > 0 && profiler_check_job_submit())
			wakeup_job_submit(1);
		else if (profiler.main.profiler_state & PROFILER_STATE_JOB_SUBMIT)
			wakeup_job_submit(0);

		if (remained_ms < (PROFILER_INTERVAL_MS / 10))
			remained_ms += PROFILER_INTERVAL_MS;
		msleep_interruptible(remained_ms);
		if(profiler.main.forced_running == PROFILER_FORCED_RUNNING_DBG) {
			profiler.main.profiler_stay_counter = PROFILER_STAY_TIMES;
			char log_buf[1024];
			print_profile_sharing_data(&psd, log_buf, (int)sizeof(log_buf));
			printk("%s\n", log_buf);
		}
	}
	profiler_finish_profile();
	atomic_set(&profiler.main.profiler_running, 0);
}
		...
		size_t count, loff_t *f_pos)
{
	...
	if (!priv || priv->id < 0 || priv->id >= SVC_RESERVED)
		return -EINVAL;

	if (!access_ok(buf, sizeof(s32)))
		return -EFAULT;

	if (copy_to_user(buf, &profiler.main.profiler_state, sizeof(s32)))
...
static __poll_t profiler_fops_poll(struct file *filp, struct poll_table_struct *wait)
{
	...
	poll_wait(filp, &profiler.main.wq, wait);
	...
	case IOCTL_WR_DSD:
		{
			...
			if (!copy_from_user(&dsd, user_dsd, sizeof(struct delta_sharing_data))) {
				...
				dom = &profiler.main.domains[dsd.id];
...
{
	struct profiler_fops_priv *priv = (struct profiler_fops_priv *)filp->private_data;
	struct domain_data *dom = &profiler.main.domains[PROFILER_GPU];
	struct profiler_data_gpu *private = dom->private;

	if (!priv || priv->id < 0 || priv->id >= SVC_RESERVED)
		return -EINVAL;

	if (priv->id == SVC_GFX) {
		if (psd.enable_gfx && (profiler.main.profiler_state & PROFILER_STATE_GFX)) {
			pr_info("PROFILER : release service id (%d)", priv->id);
			psd.enable_gfx = 0;
			profiler.main.profiler_state ^= PROFILER_STATE_GFX;
			private->fn->set_profiler_governor(0);
		}
	}
#if IS_ENABLED(CONFIG_EXYNOS_PROFILER_GFLOW)
	else if (priv->id == SVC_JOB_SUBMIT) {
		if (psd.enable_gfx && (profiler.main.profiler_state & PROFILER_STATE_JOB_SUBMIT)) {
			pr_info("PROFILER : release service id (%d)", priv->id);
			profiler.main.profiler_state ^= PROFILER_STATE_JOB_SUBMIT;
		}
	}
#endif
...
{
	if (profiler.main.max_fps != new_fps * 10) {
		profiler.main.max_fps = new_fps * 10;
		schedule_work(&profiler.main.fps_work);
	}
}
...
{
	tsd.max_fps = profiler.main.max_fps;
}
...
static void profiler_get_profile_data(struct domain_data *dom)
{
	...
	if (id < NUM_OF_CPU_DOMAIN) {
		struct profiler_data_cpu *private = dom->private;

		private->fn->cpu_active_pct(id, psd.cpu_active_pct[id]);
	} else if (id == PROFILER_GPU) {
		struct profiler_data_gpu *private = dom->private;
		int i;
		private->fn->get_frameinfo(&psd.frame_cnt_by_swap,
			&psd.profile_frame_vsync_cnt,
			&psd.delta_ms_by_swap);
		private->fn->get_gfxinfo(psd.rtimes, psd.gfxinfo);
		private->fn->get_pidinfo(psd.pid_list, psd.rendercore_list);
		for (i = 0; i < NUM_OF_DOMAIN; i++)
			psd.pb_mgn[i] = private->fn->pb_get_mgninfo(i, &psd.pb_mgn_no_boost[i]);
	} else if (id == PROFILER_MIF) {
		struct profiler_data_mif *private = dom->private;
...
void profiler_set_tunable_data(void)
{
	...
	for_each_mdomain(dom, id) {
		...
		if (id < NUM_OF_CPU_DOMAIN) {
			...
			tsd.minlock_low_limit[id] = private->pm_qos_min_freq;
		} else if (id == PROFILER_DSU) {
			struct profiler_data_dsu *private = dom->private;
...
static void profiler_enable_profiler_govs(int enable)
{
	...
	if (enable) {
		/* Enable governors */
		if (profiler.main.dm_dynamic == 1) {
			profiler.main.dm_dynamic = 0;
			exynos_dm_dynamic_disable(profiler.main.dm_dynamic);
		}
		if (profiler.main.gpu_profiler_gov == 0) {
			profiler.main.gpu_profiler_gov = 1;
			private->fn->set_profiler_governor(profiler.main.gpu_profiler_gov);
		}
	} else {
		/* Disable governors */
		if (profiler.main.dm_dynamic == 0 && psd.enable_gfx && (profiler.main.profiler_state & PROFILER_STATE_GFX)) {
			profiler.main.dm_dynamic = 1;
			exynos_dm_dynamic_disable(profiler.main.dm_dynamic);
		}
		if (profiler.main.gpu_profiler_gov == 1) {
			profiler.main.gpu_profiler_gov = 0;
			private->fn->set_profiler_governor(profiler.main.gpu_profiler_gov);
		}
	}
}
...
static void profiler_update_profile(void)
{
	...
	profiler.main.end_time = ktime_get();
	psd.profile_time_ms = ktime_to_ms(ktime_sub(profiler.main.end_time, profiler.main.start_time));

	if (profiler_drmdev_fn) {
		profiler_drmdev_fn->get_frame_cnt(&profiler.main.end_frame_cnt, &time);
		psd.profile_frame_cnt = profiler.main.end_frame_cnt - profiler.main.start_frame_cnt;
		psd.frame_done_time_us = ktime_to_us(time);
		profiler_drmdev_fn->get_vsync_cnt(&profiler.main.end_frame_vsync_cnt, &time);
		psd.frame_vsync_time_us = ktime_to_us(time);
		profiler_drmdev_fn->get_fence_cnt(&profiler.main.end_fence_cnt, &time);
		psd.profile_fence_cnt = profiler.main.end_fence_cnt - profiler.main.start_fence_cnt;
	...
	profiler.main.start_time = profiler.main.end_time;
	profiler.main.start_frame_cnt = profiler.main.end_frame_cnt;
	profiler.main.start_frame_vsync_cnt = profiler.main.end_frame_vsync_cnt;
	profiler.main.start_fence_cnt = profiler.main.end_fence_cnt;
}
...
static void profiler_start_profile(void)
{
	...
	profiler.main.start_time = ktime_get();
	if (profiler_drmdev_fn) {
		profiler_drmdev_fn->get_frame_cnt(&profiler.main.start_frame_cnt, &time);
		profiler_drmdev_fn->get_vsync_cnt(&profiler.main.start_frame_vsync_cnt, &time);
	}
...
static void profiler_finish_profile(void)
{
	...
	for_each_mdomain(dom, id) {
		...
		if (id < NUM_OF_CPU_DOMAIN) {
			...
			if (profiler.main.disable_llc_way) {
				private->fn->disable_llc_way(0);
				profiler.main.disable_llc_way = 0;
			}

			if (profiler.main.llc_config)
				control_llc(0);
		} else if (id == PROFILER_MIF) {
			struct profiler_data_mif *private = dom->private;
			...
			/* Skip this routine for DSU Profiler */
			if (id != PROFILER_DSU) {
				pr_err("profiler: invalid domain id(%d)\n", id);
			}
		}
	}

	if (profiler.main.bts_added) {
		bts_del_scenario(profiler.main.bts_idx);
		profiler.main.bts_added = 0;
		psd.gpu_mo = 0;
	}
...
{
	int flag = 0;
	if (enable && !(profiler.main.profiler_state & PROFILER_STATE_GFX)) {
		profiler.main.profiler_state |= PROFILER_STATE_GFX;
		if (!profiler.main.disable_gfx)
			psd.enable_gfx = 1;
		flag = 1;
	} else if (!enable && (profiler.main.profiler_state & PROFILER_STATE_GFX) && !profiler.main.forced_running) {
		profiler.main.profiler_state ^= PROFILER_STATE_GFX;
		psd.enable_gfx = 0;
		flag = 1;
	}
	if (flag) {
		...
		wake_up_interruptible(&profiler.main.wq);
	}
...
static void wakeup_job_submit(int enable)
{
	...
	if (enable && !(profiler.main.profiler_state & PROFILER_STATE_JOB_SUBMIT)) {
		profiler.main.profiler_state |= PROFILER_STATE_JOB_SUBMIT;
		flag = 1;
	} else if (!enable && (profiler.main.profiler_state & PROFILER_STATE_JOB_SUBMIT)) {
		profiler.main.profiler_state ^= PROFILER_STATE_JOB_SUBMIT;
		flag = 1;
	}
...
{
	int ret = (profiler.main.profiler_state & PROFILER_STATE_GFX)? 1:0;
...
{
	if (!profiler.main.profiler_gov) {
		u32 v;

		if (sscanf(buf, "%u", &v) != 1)
			return -EINVAL;

		if (v <= PROFILER_FORCED_RUNNING_DBG)
			profiler.main.forced_running = v;

		wakeup_gfx(profiler.main.forced_running);
	}
	return count;
}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	/* start */
	if (mode) {
		profiler_drmdev_fn->get_frame_cnt(&fps_profile.fps, &fps_profile.profile_time);
	} else {
		u64 cur_cnt;
		ktime_t cur_time;

		profiler_drmdev_fn->get_frame_cnt(&cur_cnt, &cur_time);
...
{										\
	...
	for_each_mdomain(dom, id) {						\
		ret += snprintf(buf + ret, PAGE_SIZE - ret, "[%d]%s: %d\n",	\
				id, profiler.main.domain_name[id], tsd._name[id]);		\
	}									\
										\
	return ret;								\
}										\
...
{
	struct domain_data *dom = &profiler.main.domains[domid];
	int i;
	int freq = 0;
	...
	if (domid < NUM_OF_CPU_DOMAIN) {
		...
		if (dev_pm_qos_request_active(&private->pm_qos_min_req)) {
			dev_pm_qos_update_request(&private->pm_qos_min_req, freq);
		}
	}
}
...
{
	struct domain_data *dom = &profiler.main.domains[id];

	/* CPU uses pelt margin via ems service */
	if (id < NUM_OF_CPU_DOMAIN) {
		if (is_enabled(PROFILER_GPU)) {
			struct domain_data *gpudom = &profiler.main.domains[PROFILER_GPU];
...
static void control_mo(int id, int value)
{
	...
	if (!profiler.main.bts_idx || !profiler.main.len_mo_id)
		return;

	if (value > 0) {
		if (!profiler.main.bts_added) {
			bts_add_scenario(profiler.main.bts_idx);
			profiler.main.bts_added = 1;
		}

		for (idx = 0; idx < profiler.main.len_mo_id; idx++)
			bts_change_mo(profiler.main.bts_idx, profiler.main.mo_id[idx], value, value);
	} else if (profiler.main.bts_added) {
		bts_del_scenario(profiler.main.bts_idx);
		profiler.main.bts_added = 0;
	}
...
static void control_llc(int value)
{
	...
	/* GFX LLC */
	if ((value & PROFILER_LLC_GFX_BIT) && !(profiler.main.llc_config & PROFILER_LLC_GFX_BIT)) {
		if (llc_region_alloc(LLC_REGION_PROFILER, 1, profiler.main.perf_max_llc_way) == 0)
			profiler.main.llc_config |= PROFILER_LLC_GFX_BIT;
	} else if (!(value & PROFILER_LLC_GFX_BIT) && (profiler.main.llc_config & PROFILER_LLC_GFX_BIT)) {
		if (llc_region_alloc(LLC_REGION_PROFILER, 0, 0) == 0)
			profiler.main.llc_config &= PROFILER_LLC_GPU_BIT;
	}

	/* Job-submit LLC */
	if (profiler.main.llc_config & PROFILER_LLC_GPU_BIT)
		prev_gpu_llc = (profiler.main.llc_config & PROFILER_LLC_HEAVY_BIT)? profiler.main.perf_max_llc_way : profiler.main.perf_light_llc_way;

	if (value & PROFILER_LLC_GPU_BIT)
		next_gpu_llc = (value & PROFILER_LLC_HEAVY_BIT)? profiler.main.perf_max_llc_way : profiler.main.perf_light_llc_way;
	...
	if (prev_gpu_llc != next_gpu_llc) {
		if (prev_gpu_llc > 0) {
			...
			if (llc_region_alloc(LLC_REGION_GPU, 1, next_gpu_llc) == 0)
				profiler.main.llc_config |= PROFILER_LLC_GPU_BIT | (value & PROFILER_LLC_HEAVY_BIT);
		}
	}

	psd.llc_config = profiler.main.llc_config;
}
...
static void control_set_pb_params(int idx, int value)
{
	if (is_enabled(PROFILER_GPU)) {
		struct domain_data *dom = &profiler.main.domains[PROFILER_GPU];
		struct profiler_data_gpu *private = dom->private;

		private->fn->set_pb_params(idx, value);
	}
}

static int control_get_pb_params(int idx)
{
	if (is_enabled(PROFILER_GPU)) {
		struct domain_data *dom = &profiler.main.domains[PROFILER_GPU];
		struct profiler_data_gpu *private = dom->private;

		return private->fn->get_pb_params(idx);
	}
	return -1;
}

static void control_mifgov(int id, int value)
{
	if (is_enabled(PROFILER_GPU) && (value >= 0)) {
		int run = 0;
		if (value > 0) {
			run = 1;
		}
#if IS_ENABLED(CONFIG_EXYNOS_MIFGOV)
		exynos_mifgov_run(run, "PROFILER");
#endif
	}
}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (op == OP_PB_PARAMS) {
		/* PB params's id is not IP-ID and is parameter index, so it is required to avoid exception */
		control_set_pb_params(id, value);
		return count;
	}
		...
		struct device_attribute *attr, char *buf)
{
	...
	ret += snprintf(buf + ret, PAGE_SIZE - ret, "user_fps=%d\n", psd.user_fps);
	ret += snprintf(buf + ret, PAGE_SIZE - ret, "user_skin=%d\n", psd.user_skin);
	ret += snprintf(buf + ret, PAGE_SIZE - ret, "user_mode=%d\n", psd.user_mode);
	...
	ret += snprintf(buf + ret, PAGE_SIZE - ret, "perf_mainbw_thr=%llu\n", profiler.main.perf_mainbw_thr);
	ret += snprintf(buf + ret, PAGE_SIZE - ret, "perf_gpubw_up_ratio=%llu\n", profiler.main.perf_gpubw_up_ratio);
	ret += snprintf(buf + ret, PAGE_SIZE - ret, "perf_gpubw_dn_ratio=%llu\n", profiler.main.perf_gpubw_dn_ratio);
	ret += snprintf(buf + ret, PAGE_SIZE - ret, "perf_max_llc_way=%d\n", profiler.main.perf_max_llc_way);
	ret += snprintf(buf + ret, PAGE_SIZE - ret, "perf_light_llc_way=%d\n", profiler.main.perf_light_llc_way);
	ret += snprintf(buf + ret, PAGE_SIZE - ret, "llc_config=%d\n", profiler.main.llc_config);
	ret += snprintf(buf + ret, PAGE_SIZE - ret, "main_bw=%llu, gpubw%%=%d\n", mainbw, ratio);
	ret += snprintf(buf + ret, PAGE_SIZE - ret, "catid=%d\n", psd.catid);
	ret += snprintf(buf + ret, PAGE_SIZE - ret, "gfxthr=%llu, %llu, %llu\n",
			profiler.main.gfx_wakeup_thr1, profiler.main.gfx_wakeup_thr2, profiler.main.gfx_wakeup_thr3);

	for (int i = 0; i < 30; i++) {
		int value = control_get_pb_params(i);

		if (value < 0)
			break;
		ret += snprintf(buf + ret, PAGE_SIZE - ret, "PB %2d: %d\n", i, value);
	}
	...
	if (mode == REQ_PROFILE)
		...
		switch (cmd) {
			case CONTROL_CMD_PERF_MAINBW_THR:
				profiler.main.perf_mainbw_thr = value;
				break;
			case CONTROL_CMD_PERF_UP_RATIO:
				profiler.main.perf_gpubw_up_ratio = value;
				break;
			case CONTROL_CMD_PERF_DN_RATIO:
				profiler.main.perf_gpubw_dn_ratio = value;
				break;
			case CONTROL_CMD_DISABLE_LLC_WAY:
				private->fn->disable_llc_way(value);
				profiler.main.disable_llc_way = (bool)value;
				break;
			case CONTROL_CMD_ENABLE_PROFILER_GOVERNOR:
				profiler_enable_profiler_govs(value);
				break;
			case CONTROL_CMD_MAX_LLC_WAY:
				profiler.main.perf_max_llc_way = value;
				break;
			case CONTROL_CMD_LIGHT_LLC_WAY:
				profiler.main.perf_light_llc_way = value;
				break;
			case CONTROL_CMD_TERMINATE_GFX:
				if (value)
					wakeup_gfx(0);
				break;
			case CONTROL_CMD_GFX_THR1:
				profiler.main.gfx_wakeup_thr1 = value;
				break;
			case CONTROL_CMD_GFX_THR2:
				profiler.main.gfx_wakeup_thr2 = value;
				break;
			case CONTROL_CMD_GFX_THR3:
				profiler.main.gfx_wakeup_thr3 = value;
				break;
			case CONTROL_CMD_SET_CATID:
				psd.catid = value;
				break;
			default:
				break;
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	psd.user_target_fps = val;

	return count;
}
static DEVICE_ATTR_RW(user_target_fps);

static ssize_t user_fps_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%d\n", psd.user_fps / 10);
}
static ssize_t user_fps_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	s32 val;
...
static DEVICE_ATTR_RW(user_fps);

static ssize_t user_skin_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%d\n", psd.user_skin);
}
static ssize_t user_skin_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	s32 val;
...
static DEVICE_ATTR_RW(user_skin);
...
static DEVICE_ATTR_RW(user_mode);

static ssize_t user_stats0_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%llu\n", psd.user_stats0);
}
static ssize_t user_stats0_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	u64 val;
...
static DEVICE_ATTR_RW(user_stats0);

static ssize_t user_stats1_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%llu\n", psd.user_stats1);
}
static ssize_t user_stats1_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	u64 val;
...
static DEVICE_ATTR_RW(user_stats1);

static struct attribute *control_attrs[] = {
	&dev_attr_set_margin.attr,
	&dev_attr_control_profile.attr,
	&dev_attr_user_fps.attr,
	&dev_attr_user_skin.attr,
	&dev_attr_user_mode.attr,
	&dev_attr_user_stats0.attr,
	&dev_attr_user_stats1.attr,
				...
				unsigned long val, void *v)
{
	...
	if (profiler.main.forced_running)
		return NOTIFY_OK;

	if (!profiler.main.disable_gfx && (set->mode == EMSTUNE_GAME_MODE)) {
		profiler.main.profiler_gov = 1;
		wakeup_gfx(1);
	} else {
		profiler.main.profiler_gov = 0;
		wakeup_gfx(0);
	}
	return NOTIFY_OK;
}
			...
			struct domain_data *dom, void *private_fn)
{
	...
	if (!root) {
		pr_err("profiler: invalid root device node\n");
		return -EINVAL;
	}

	dn = of_get_child_by_name(root, profiler.main.domain_name[id]);
	if (!dn) {
	...
	if (id < NUM_OF_CPU_DOMAIN) {
		...
		private = kzalloc(sizeof(struct profiler_data_cpu), GFP_KERNEL);
		if (!private)
			return -ENOMEM;
		...
		struct profiler_data_gpu *private;

		private = kzalloc(sizeof(struct profiler_data_gpu), GFP_KERNEL);
		if (!private)
			return -ENOMEM;
		...
		struct profiler_data_mif *private;

		private = kzalloc(sizeof(struct profiler_data_mif), GFP_KERNEL);
		if (!private)
			return -ENOMEM;
		...
		struct profiler_data_dsu *private;

		private = kzalloc(sizeof(struct profiler_data_dsu), GFP_KERNEL);
		if (!private)
			return -ENOMEM;
...
s32 exynos_profiler_register_misc_device(void)
{
	...
	profiler.main.gov_fops.fops.owner		= THIS_MODULE;
	profiler.main.gov_fops.fops.llseek		= no_llseek;
	profiler.main.gov_fops.fops.write	= profiler_fops_write;
	profiler.main.gov_fops.fops.read		= profiler_fops_read;
	profiler.main.gov_fops.fops.poll		= profiler_fops_poll;
	profiler.main.gov_fops.fops.unlocked_ioctl	= profiler_fops_ioctl;
	profiler.main.gov_fops.fops.compat_ioctl	= profiler_fops_ioctl;
	profiler.main.gov_fops.fops.open			= profiler_fops_open;
	profiler.main.gov_fops.fops.release		= profiler_fops_release;

	profiler.main.gov_fops.miscdev.minor		= MISC_DYNAMIC_MINOR;
	profiler.main.gov_fops.miscdev.name		= "exynos-profiler";
	profiler.main.gov_fops.miscdev.fops		= &profiler.main.gov_fops.fops;

	ret = misc_register(&profiler.main.gov_fops.miscdev);
	if (ret) {
...
struct domain_data* exynos_profiler_get_domain(s32 id) {
	return &profiler.main.domains[id];
}
EXPORT_SYMBOL_GPL(exynos_profiler_get_domain);
...
void exynos_profiler_register_drmdev_fn(struct exynos_profiler_drmdev_fn *fn)
{
	profiler_drmdev_fn = fn;
}
EXPORT_SYMBOL_GPL(exynos_profiler_register_drmdev_fn);
...
static s32 parse_profiler_dt(struct device_node *dn)
{
	...
	profiler.main.bts_idx = bts_get_scenindex("g3d_heavy");
	profiler.main.len_mo_id = of_property_count_u32_elems(dn, "mo-id");
	if (profiler.main.len_mo_id > 0) {
		profiler.main.mo_id = kzalloc(sizeof(int) * profiler.main.len_mo_id, GFP_KERNEL);
		if (!profiler.main.mo_id)
			return ret;

		ret |= of_property_read_u32_array(dn, "mo-id", profiler.main.mo_id, profiler.main.len_mo_id);
	}

	num_of_domains = of_property_count_strings(dn, "domain-names");
	if (num_of_domains == NUM_OF_DOMAIN) {
		int rc = 0;
		rc = of_property_read_string_array(dn, "domain-names", profiler.main.domain_name, NUM_OF_DOMAIN);
...
static s32 exynos_profiler_probe(struct platform_device *pdev)
{
	...
	INIT_WORK(&profiler.main.fps_work, profiler_fps_change_work);

	psd.enable_gfx = 0;
	psd.disable_job_submit = 0;
	psd.disable_dyn_mo = 0;
	profiler.main.forced_running = 0;
	profiler.main.disable_gfx = 0;
	profiler.main.profiler_gov = 0;
	profiler.main.gpu_profiler_gov = 0;
	profiler.main.disable_llc_way = 0;
	profiler.main.bts_added = 0;
	profiler.main.gfx_wakeup_counter = 0;
	profiler.main.gfx_wakeup_thr1 = DEF_GFX_THR1;
	profiler.main.gfx_wakeup_thr2 = DEF_GFX_THR2;
	profiler.main.gfx_wakeup_thr3 = DEF_GFX_THR3;
	profiler.main.perf_mainbw_thr = DEF_PERF_MAINBW_THR;
	profiler.main.perf_gpubw_up_ratio = DEF_PERF_GPUBW_UP_RATIO;
	profiler.main.perf_gpubw_dn_ratio = DEF_PERF_GPUBW_DN_RATIO;
	profiler.main.perf_max_llc_way = MAX_LLC_WAY;
	profiler.main.perf_light_llc_way = MAX_LLC_WAY >> 1;
	profiler.main.profiler_state = PROFILER_STATE_SLEEP;
	profiler.main.profiler_stay_counter = 0;

	atomic_set(&profiler.main.profiler_running, 0);
	init_waitqueue_head(&profiler.main.wq);
	...
	if(profiler_gpudev_fn) {
		profiler_gpudev_fn->profiler_sched = profiler_sched;
		profiler_gpudev_fn->profiler_sleep = profiler_sleep;
	}
#endif
	INIT_WORK(&profiler.main.profiler_wq, profiler_work);
...
MODULE_DESCRIPTION("Exynos Main Profiler v2");
MODULE_SOFTDEP("pre: exynos_thermal_v2 exynos-cpufreq sgpu exynos_devfreq sgpu-profiler");
MODULE_LICENSE("GPL");
