#include <exynos-cpu-profiler.h>
#include <sgpu-profiler.h>

struct exynos_profiler_gpudev_fn *profiler_gpudev_fn;
...
struct profiler_fn_cpu cpu_fn_profiler = {
	.cpu_active_pct		= &cpupro_cpu_active_pct,
	.cpu_asv_ids		= &cpupro_cpu_asv_ids,
};

struct domain_fn cpu_fn_domain = {
...
static void cpupro_hook_freq_change(void *data, struct cpufreq_policy *policy)
{
	...
	if(profiler_gpudev_fn) {
		if (PROFILER_CL0 <= dompro->profiler_id && dompro->profiler_id < NUM_OF_CPU_DOMAIN) {
			profiler_gpudev_fn->pb_set_cur_freqlv(dompro->profiler_id, new_freq_idx);
		}
	}
		...
		struct cpufreq_policy *policy)
{
	...
	if (dompro->table_cnt == 0) {
		pr_err("cpupro: table count is zero value\n");
		return NULL;
	}
...
static int exynos_cpu_profiler_probe(struct platform_device *pdev)
{
	...
	/* init domains */
	for_each_child_of_node(profiler.dn, dn) {
		...
		if (init_domain(dompro, dn)) {
			pr_err("cpupro: failed to alloc domain\n");
			exynos_profiler_register_domain(dompro->profiler_id, NULL, NULL);
			return -ENOMEM;
		}
		...
		if (dompro->profiler_id != -1) {
			exynos_profiler_register_domain(dompro->profiler_id, &cpu_fn_domain, &cpu_fn_profiler);
		}
	}
