...
#ifndef __CUSTOS_LOG_H__
#define __CUSTOS_LOG_H__

#include <linux/device.h>

#define LOG_TAG "[custos]"
#define LOG_FMT " %s:%s:%d: "
#define LOG_CUSTOS "log_custos"
#define LOG_CUST_PM "log_cust_pm"
#define LOG_CUST_SACPM "log_cust_sacpm"
#define CUSTOS_TAG "[CUSTOS]"
...
#define CUSTOS_PRINT(fmt, ...)                                               \
	pr_info(CUSTOS_TAG fmt "\n", \
		##__VA_ARGS__)

#ifdef DEBUG
#define LOG_DUMP_BUF(buf, len)                                               \
	print_hex_dump(KERN_DEBUG, "[custos] ", DUMP_PREFIX_OFFSET, 16, 1, \
		       buf, len, true)
#else
#define LOG_DUMP_BUF(buf, len) \
	do {                   \
