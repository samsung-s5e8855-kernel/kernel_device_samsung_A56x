...
static unsigned long *exynos_vm_bitmap_512KB;
					 ...
					 bool has_2nd_crash)
{
	...
	do {
		...
		pr_info("\tspsr_el1= 0x%llx\n", read_sysreg(spsr_el1));
		pr_info("\tttbr0_el1 = 0x%llx\n", read_sysreg(ttbr0_el1));
		pr_info("\tttbr1_el1 = 0x%llx\n", read_sysreg(ttbr1_el1));
		pr_info("\ttcr_el1 = 0x%llx\n", read_sysreg(tcr_el1));
		pr_info("\tmair_el1 = 0x%llx\n", read_sysreg(mair_el1));
		pr_info("\telr_el1 = 0x%llx\n", read_sysreg(elr_el1));
		pr_info("\tesr_el1 = 0x%llx\n", read_sysreg(esr_el1));
		pr_info("\tfar_el1 = 0x%llx\n", read_sysreg(far_el1));
...
static irqreturn_t exynos_el2_dynamic_lv3_irq_handler_thread(int irq, void *dev_id)
{
	...
	do {
		...
		if (buf != NULL) {
			...
			pr_info("%s: Allocate exynos lv3 256K table buf VA[%#lx]/IPA[%#lx]\n",
	...
	ret = exynos_hvc(HVC_FID_SET_EL2_LV3_TABLE_BUF,
			 ...
			 0, 0);
	if (ret) {
		pr_err("%s: deliver lv3 table failed ret:%lx\n",
				__func__, ret);
	}
					 ...
					 size_t count)
{
	...
	for(i = 0; i < num_possible_cpus(); i++) {
		...
		if (!ret) {
			stage2_success = 0;
			dev_err(dev,
				"%lx core Stage2 mapping is not enable\n",
				i);
			return -EINVAL;
		}
	}
				    ...
				    char *buf)
{
	...
	if (ret_64) {
		dev_err(dev,
			"Fail to get EL2 log buf, ret[%lx]\n",
			ret_64);
		ret = -EINVAL;
		goto out;
	}
				    ...
				    size_t count)
{
	...
	if (ret) {
		dev_err(dev,
			"Fail to reset EL2 log cur, ret[%lx]\n",
			ret);
		return -EINVAL;
	}
...
static int exynos_el2_irq_enable(struct platform_device *pdev)
{
	...
	if (!irq_num) {
		dev_err(dev, "Fail to get irq from dt\n");
		return -EINVAL;
	}
	...
	if (ret) {
		dev_err(dev, "Fail to request IRQ handler. ret(%d) irq(%d)\n",
			ret, irq_num);
		return ret;
	}
	...
	if (!el2_d_irqd) {
		dev_err(dev, "Fail to get irq data\n");
		return -EINVAL;
	}
	...
	ret_64 = exynos_hvc(HVC_FID_DELIVER_LV3_ALLOC_IRQ_NUM,
			    hwirq, 0, 0, 0);
	if (ret_64) {
		dev_err(dev, "Fail to deliver irq num, ret[%lx]\n", ret_64);
		return -EINVAL;
	}

	dev_info(dev, "IRQ register success\n");

	return 0;
}

static int exynos_el2_log_dump_enable(struct device *dev)
{
	int ret = 0;
#if IS_ENABLED(CONFIG_DEBUG_SNAPSHOT)
	...
	if (info_ver != HARX_INFO_VERSION) {
		dev_err(dev, "HARX_INFO_VERSION mismatch [%lx]\n", info_ver);
		return -EINVAL;
	}
	...
	if (log_base == ERROR_INVALID_INFO_TYPE) {
		dev_err(dev, "Fail to get EL2 log base\n");
		return -EINVAL;
	}
	...
	if (log_size == ERROR_INVALID_INFO_TYPE) {
		dev_err(dev, "Fail to get EL2 log size\n");
		return -EINVAL;
	}

	dev_info(dev, "EL2 log buffer base[%#lx]/size[%#lx]\n",
		 log_base, log_size);

	ret = dbg_snapshot_add_bl_item_info(EL2_LOG_DSS_NAME,
					    log_base, log_size);
	if (ret)
		dev_err(dev, "Fail to add EL2 log dump - ret[%d]\n", ret);
#endif
	return ret;
}
...
static int exynos_el2_probe(struct platform_device *pdev)
{
	...
	if (!pkvm_enabled) {
		ret = exynos_el2_irq_enable(pdev);
		if (ret) {
			dev_err(dev, "Dynamic level 3 IRQ disabled ret(%d)\n", ret);
			return ret;
		}
	}

	ret = device_create_file(dev, &dev_attr_exynos_stage2_enable);
	if (ret)
		dev_err(dev, "stage2_enable sysfs create file fail\n");
	else
		dev_info(dev, "stage2_enable sysfs create file success\n");

	ret = device_create_file(dev, &dev_attr_exynos_el2_log);
	if (ret)
		dev_err(dev, "exynos_el2_log sysfs create file fail\n");
	else
		dev_info(dev, "exynos_el2_log sysfs create file success\n");

	ret = exynos_el2_log_dump_enable(dev);
...
static int __init exynos_el2_dynamic_lv3_table_init(void)
{
...
#endif /* CONFIG_DEBUG_KMEMLEAK */
			buf_pa = virt_to_phys(buf);
			pr_info("%s: Allocate 4MB buffer VA[%#lx]/IPA[%#lx]\n",
					__func__, (unsigned long)buf, buf_pa);
			break;
		}
	} while ((buf == NULL) && (retry_cnt-- > 0));
	...
	if (ret) {
		pr_err("%s: Fail to allocate lv3 table buf ret = %lx\n",
				__func__, ret);
		return -EINVAL;
	}
...
static int __init exynos_el2_crash_info_init(void)
{
	...
	do {
		...
		if (buf != NULL) {
			el2_crash_buf = buf;
			buf_pa = virt_to_phys(buf);
			pr_info("%s: Allocate EL2 crash buffer VA[%#lx]/IPA[%#lx] len[%#lx]\n",
...
static int __init exynos_vm_bitmap_init(void)
{
	unsigned long *buf;
	unsigned long buf_pa;
	int retry_cnt = MAX_RETRY_COUNT;
	...
	do {
		buf = kzalloc(EL2_VM_BITMAP_BUF_SIZE_512K, GFP_KERNEL);
		if (buf != NULL) {
			exynos_vm_bitmap_512KB = buf;
#ifdef CONFIG_DEBUG_KMEMLEAK
			kmemleak_no_scan(exynos_vm_bitmap_512KB);
#endif /* CONFIG_DEBUG_KMEMLEAK */
			buf_pa = virt_to_phys(buf);
			pr_info("%s: Allocate 512KB buffer VA[%#lx]/IPA[%#lx]\n",
					__func__, (unsigned long)buf, buf_pa);
			break;
		}
	} while ((buf == NULL) && (retry_cnt-- > 0));
	...
	if (buf == NULL) {
		pr_err("%s: Fail to allocate bitmap buffer of VM Stage2 Map table\n",
				__func__);
		return -ENOMEM;
	}

	ret = exynos_hvc(HVC_FID_SET_VM_BITMAP_BUF,
			 buf_pa, EL2_VM_BITMAP_BUF_SIZE_512K, 0, 0);
	if (ret) {
		pr_err("%s: Fail to allocate vm bitmap buf ret = %lx, size: %x\n",
				__func__, ret, EL2_VM_BITMAP_BUF_SIZE_512K);
		return -EINVAL;
	}

	pr_info("%s: bitmap buffer of VM Stage2 map table deliver successfully\n",
			__func__);
...
static int __init exynos_el2_module_init(void)
{
	...
	if (pkvm_enabled) {
		ret = exynos_hvc(HVC_FID_PKVM_RECORD_EL2_INFO, 0, 0, 0, 0);
		if (ret) {
			pr_err("%s: Fail to record EL2 info (%d)\n",
					__func__, ret);
			return ret;
		}

		ret = exynos_vm_bitmap_init();
		if (ret) {
			pr_err("%s: Fail to set VM bitmap buf (%d)\n",
					__func__, ret);
			return ret;
		}
	} else {
