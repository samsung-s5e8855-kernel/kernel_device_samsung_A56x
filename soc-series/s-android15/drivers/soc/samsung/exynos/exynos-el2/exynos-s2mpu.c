...
unsigned long exynos_s2mpu_notifier_call_register(struct s2mpu_notifier_block *snb)
{
	...
	if (subsys_idx >= subsystem_num) {
		pr_info("%s: %s not managed by this SoC\n", __func__,
				snb->subsystem);
...
static unsigned long exynos_s2mpu_notifier_call(struct s2mpu_info_data *data)
{
	...
	if (p_ret & S2MPU_NOTIFIER_PANIC) {
		pr_err("%s: Notifier Call Request Panic p_ret[%lx]\n",
					__func__, p_ret);
		return p_ret;
	}
...
static void s2mpu_check_blocklist_owner(unsigned int blocklist_owner)
{
	if (blocklist_owner == subsystem_num)
		return;

	if (blocklist_owner < subsystem_num) {
		pr_info("  (Overlapped with [%s])\n",
				subsystem_list[blocklist_owner]);
		return;
	}

	switch (blocklist_owner) {
	case OWNER_IS_KERNEL_RO:
		pr_info("  (Overlapped with Kernel Code Region)\n");
		break;
	case OWNER_IS_TEST:
		pr_info("  (Overlapped with Test Region)\n");
		break;
	case OWNER_IS_TZMP2:
		pr_info("  (Overlapped with TZMP2 Buffers)\n");
		break;
	default:
		pr_info("  (Overlapped with unknown owner [%d])\n",
				blocklist_owner);
		break;
	}
}
...
static void s2mpu_v9_0_1_get_fault_info(struct __s2mpu_fault_info_v9_0_1 *fault_data,
					struct s2mpu_fault_info_common *fault_info,
					unsigned int s2mpu_type)
{
	fault_info->s2mpu_type = s2mpu_type;
	...
	fault_info->subsystem_index = fault_data->subsystem;
	fault_info->blocklist_owner = fault_data->blocklist_owner;
	fault_info->by_s1ptw = fault_data->by_s1ptw;
	fault_info->ap_fault_src = fault_data->ap_fault_src;
	fault_info->ptw_fault_src = fault_data->ptw_fault_src;
...
static irqreturn_t exynos_s2mpu_irq_handler(int irq, void *dev_id)
{
	...
	ret = exynos_hvc(HVC_CMD_GET_S2MPUFD_FAULT_INFO,
			 irq_idx, 0, 0, 0);
	if (ret)
		pr_err("Can't handle %s S2MPU fault [%#lx]\n",
...
static irqreturn_t exynos_s2mpu_irq_handler_thread(int irq, void *dev_id)
{
	...
	struct s2mpu_fault_info_v9_0_1 *finfo_v9_0_1;
	...
	if (ver >= S2MPU_V9_0_1) {
		finfo_v9_0_1 = (struct s2mpu_fault_info_v9_0_1 *)data->fault_info;
		cnt = finfo_v9_0_1[irq_idx].fault_cnt;
	} else if (ver >= S2MPU_V9_0) {
	...
	for (i = 0; i < cnt; i++) {
		if (ver >= S2MPU_V9_0_1) {
			s2mpu_v9_0_1_get_fault_info(&finfo_v9_0_1[irq_idx].info[i], &fi,
						    finfo_v9_0_1[irq_idx].s2mpu_type);
		} else if (ver >= S2MPU_V9_0) {
		...
		s2mpu_check_blocklist_owner(fi.blocklist_owner);
		...
		if (ver >= S2MPU_V9_0_1) {
			pr_info("  - Fault BY_S1PTW : %d\n", fi.by_s1ptw);
			if (fi.by_s1ptw)
				pr_info("  - Fault occurred by stage-1 PTW\n");

			if (fi.ap_fault_src == S2MPU_REGION_ACCESS_PERMISSION_FAULT) {
				pr_info("  - Fault Source\t\t: Region access control fault\n");
			} else if (fi.ap_fault_src == S2MPU_S1PTW_RANGE_CHK_FAULT) {
				pr_info("  - Fault Source\t\t: S1PTW range check fault\n");
			} else if (fi.ap_fault_src == S2MPU_PROTECTION_TABLE_ENTRY_CHK_FAULT) {
				pr_info("  - Fault Source\t\t: protection table entry check fault\n");
			} else {
				pr_info("  - Fault Source\t\t: %s, Source[%#x]\n",
						"Anonymous source", fi.ap_fault_src);
			}

			if (fi.type == FAULT_TYPE_MPTW_ACCESS_FAULT) {
				if (fi.ptw_fault_src == S2MPU_PTW_OUT_OF_PA_RANGE_ACCESS) {
					pr_info("  - Fault PTW Source\t: Out of PA_RANGE access\n");
				} else {
					pr_info("  - Fault PTW Source\t: PTW Transaction has error response\n");
				}
			}
		}
		...
		if (ver >= S2MPU_V9_0) {
			...
			pr_info("    -> From SFR : base[%#lx]/offset[%#x] - AP[%#x]\n",
					fi.sfr_l2table_addr, fi.l2table_offset, fi.sfr_ap_val);
			pr_info("    -> From map_desc : addr[%#lx] - AP[%#x]\n",
					     ...
					     unsigned long size)
{
	...
	ret = exynos_hvc(HVC_FID_SET_ALL_S2MPU_BLOCKLIST,
			 base, size, 0, 0);
	if (ret) {
		pr_err("%s: [ERROR] base[%lx], size[%lx]\n",
				__func__, base, size);
		return ret;
	}
					       ...
					       size_t fw_mem_size)
{
	...
	if ((fw_base == 0) || (fw_bin_size == 0) || (fw_mem_size == 0)) {
		pr_err("%s: Invalid FW[%d] binary information",
				__func__, fw_id);
		pr_err(" - fw_base[%#lx]\n", fw_base);
		pr_err(" - fw_bin_size[%#lx]\n", fw_bin_size);
		pr_err(" - fw_mem_size[%#lx]\n", fw_mem_size);
	...
	ret = exynos_hvc(HVC_FID_VERIFY_SUBSYSTEM_FW,
			 ...
			 fw_mem_size);
	if (ret) {
		pr_err("%s: %s FW[%d] verification is failed (%#lx)\n",
...
unsigned long exynos_s2mpu_request_fw_stage2_ap(const char *subsystem)
{
	...
	ret = exynos_hvc(HVC_FID_REQUEST_FW_STAGE2_AP,
			 idx, 0, 0, 0);
	if (ret) {
		pr_err("%s: Enabling %s FW Stage 2 AP is failed (%#lx)\n",
						...
						unsigned int fw_id)
{
	...
	ret = exynos_hvc(HVC_FID_RELEASE_FW_STAGE2_AP,
			 idx, fw_id, 0, 0);
	if (ret) {
		pr_err("%s: %s FW[%d] release is failed (%#lx)\n",
				__func__, subsystem, fw_id, ret);
		return ret;
	}
...
unsigned long exynos_s2mpu_subsystem_set_allowlist(const char *subsystem,
						   unsigned long base,
						   unsigned long size)
{
	unsigned int idx;
	...
	if ((base == 0) || (size == 0)) {
		pr_err("%s: Invalid allowlist area base[%#lx] / size[%#lx]\n",
				__func__, base, size);
		return ERROR_INVALID_ALLOWLIST_INFO;
	}
	...
	ret = exynos_hvc(HVC_FID_SET_S2MPU_WHITELIST, idx, base, size, 0);
	if (ret) {
		pr_err("%s: %s allowlist registration is failed (%#lx)\n",
				__func__, subsystem, ret);
		pr_err("%s: - allowlist base[%#lx] / size[%#lx]\n",
				__func__, base, size);
	} else {
		pr_info("%s: %s allowlist [%#lx / %#lx] is set!\n",
				__func__, subsystem, base, size);
	}

	return ret;
}
EXPORT_SYMBOL(exynos_s2mpu_subsystem_set_allowlist);
...
unsigned long exynos_s2mpu_backup_subsystem(const char *subsystem)
{
	unsigned int idx, pidx;
	unsigned long ret, pd;
	...
	pd = exynos_hvc(HVC_FID_GET_PD_FROM_SUBSYSTEM,
			 idx, 0, 0, 0);

	if (pd == 0) {
		pr_err("%s: Fail to get PD Subsystem PD bitmap\n", __func__);
		return ERROR_INVALID_SUBSYSTEM_PD;
	}

	for (pidx = 0; pidx < BITS_PER_LONG; pidx++) {
		if ((pd & BIT_ULL_MASK(pidx)) == 0)
			continue;

		ret = exynos_pd_backup_s2mpu(pidx);
		if (ret) {
			pr_err("%s: Fail to backup PD[%d] - %s\n",
				__func__, pidx,
				(has_s2mpu(pidx) ? "PD has s2mpu" :
						   "PD has no S2MPU"));
			return ret;
		}
	}

	pr_info("%s: Backup %s Subsystem power domain done\n",
			__func__, subsystem);

	return 0;
}
EXPORT_SYMBOL(exynos_s2mpu_backup_subsystem);

#ifdef CONFIG_OF_RESERVED_MEM
static int __init exynos_s2mpu_reserved_mem_setup(struct reserved_mem *remem)
{
	pr_err("%s: Reserved memory for S2MPU table: addr=%llx, size=%llx\n",
					...
					size_t count)
{
	...
	for (idx = 0; idx < s2mpu_num; idx++) {
		...
		if (skip_ret & ((unsigned long)0x1 << idx)) {
			s2mpu_success[idx] = 2;
			dev_info(dev,
				 "s2mpu(%lu) is not initialized yet\n",
				 idx);
		} else if (!(ret & ((unsigned long)0x1 << idx))) {
			s2mpu_success[idx] = 0;
			dev_err(dev,
				"s2mpu(%lu) is not enable\n",
				idx);
		} else {
			s2mpu_success[idx] = 1;
		}
...
static void get_s2mpu_pd_bitmap(void)
{
...
static int exynos_s2mpu_probe(struct platform_device *pdev)
{
	...
	unsigned int s2mpu_rev = 0;

	pkvm_enabled = exynos_hvc(HVC_FID_GET_PROTECTED_KVM_ENABLED,
				  0, 0, 0, 0);
	if (!pkvm_enabled)
		pr_notice("%s: pKVM is not enabled\n", __func__);

	rmem_np = of_parse_phandle(np, "memory_region", 0);
	if (!rmem_np) {
		pr_err("%s: failed to acquire memory region phandle\n", __func__);
		return -ENOENT;
	}
	...
	ret = of_property_read_u32(np, "s2mpu-revision", &s2mpu_rev);
	if (ret) {
		s2mpu_rev = 0;
		pr_info("%s: s2mpu-revision not defined, set s2mpu-rev[%d]\n",
			  __func__, s2mpu_rev);
	}

	s2mpu_version = (s2mpu_version << S2MPU_VERSION_MAJOR_MINOR_SHIFT) |
			s2mpu_rev;
	...
	if (!pkvm_enabled) {
		...
		if (hvc_ret != 0)
			pr_info("%s: S2MPU blocklist set fail, hvc_ret[%lx]\n",
					__func__, hvc_ret);
		else
	...
	if (data->hw_version >= S2MPU_V9_0_1)
		data->fault_info_size = sizeof(struct s2mpu_fault_info_v9_0_1);
	else if (data->hw_version >= S2MPU_V9_0)
		data->fault_info_size = sizeof(struct s2mpu_fault_info_v9);
	else
		data->fault_info_size = sizeof(struct s2mpu_fault_info);

	data->fault_info = kzalloc(data->fault_info_size *
				   data->instance_num,
				   GFP_KERNEL);
	if (!data->fault_info) {
		dev_err(data->dev,
			...
			(data->hw_version >= S2MPU_V9_0_1) ?
			"s2mpu_fault_info_v9_0_1" :
	...
	hvc_ret = exynos_hvc(HVC_CMD_INIT_S2MPUFD,
			data->fault_info_pa,
			data->instance_num,
	...
	if (hvc_ret) {
		dev_err(data->dev,
			"Fail to initialize S2MPUFD - hvc_ret(%#lx)\n",
			hvc_ret);
		dev_err(data->dev,
	...
	/* S2MPU Notifier */
	data->noti_info = kzalloc(sizeof(struct s2mpu_notifier_info) *
				  subsystem_num,
				  GFP_KERNEL);
	if (!data->noti_info) {
		dev_err(data->dev,
	...
	data->notifier_flag = kzalloc(sizeof(unsigned int) *
				      subsystem_num,
				      GFP_KERNEL);
...
static int exynos_s2mpu_remove(struct platform_device *pdev)
{
	...
	kfree(data->noti_info);
	kfree(data->notifier_flag);
	kfree(data->fault_info);
	data->instance_num = 0;
	data->irqcnt = 0;
