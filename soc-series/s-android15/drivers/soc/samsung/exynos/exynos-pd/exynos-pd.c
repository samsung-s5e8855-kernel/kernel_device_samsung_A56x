...
static DEFINE_MUTEX(exynos_pd_glock);
#endif
...
#if IS_ENABLED(CONFIG_SOC_S5E9955)
	if (of_property_read_bool(np, "need-global-lock")) {
		pd->need_glock = true;
		dev_info(dev, EXYNOS_PD_PREFIX "%s: %s need global lock\n",
				__func__, pd->name);
	}
#endif
