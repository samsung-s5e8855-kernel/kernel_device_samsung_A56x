...
static int set_ap2cp_cfg(struct modem_ctl *mc)
{
	...
	if (!mc->ap2cp_cfg_ioaddr) {
		mif_info("Try ap2cp_cfg by smc\n");
		return cpif_exynos_smc(CP_AP2CP_CFG_DONE, 1, 0);
	}
...
static int init_control_messages(struct modem_ctl *mc)
{
	...
	cp_cma = get_cp_cma_region(0);
	if (!cp_cma) {
		mif_err("cp_cma 0 is null\n");
		return -EINVAL;
	}
	cp_cma->paddr ?
		set_ctrl_msg(&mld->ap2cp_l2b_size_0, cp_cma->req_size) :
		set_ctrl_msg(&mld->ap2cp_l2b_size_0, 0);

	cp_cma = get_cp_cma_region(1);
	if (!cp_cma) {
		mif_err("cp_cma 1 is null\n");
		return -EINVAL;
	}
	cp_cma->paddr ?
		set_ctrl_msg(&mld->ap2cp_l2b_size_1, cp_cma->req_size) :
...
static bool _is_first_dump = true;
static int power_on_cp(struct modem_ctl *mc)
{
	...
	if (cal_cp_status(mc->pmu_cp_status) == 0) {
		if (_is_first_boot) {
			...
			ret = cal_cp_init();
			if (ret) {
				mif_err("cal_cp_init error:%d\n", ret);
				return ret;
			}
			_is_first_boot = false;

#if IS_ENABLED(CONFIG_SOC_S5E9955)
			exynos_pmu_read(0x3920, &val);
			mif_info("OUTISO_EN_PWROFF:%d\n", val & 0x1);
			if (val & 0x1) {
				mif_err("Retry CP boot\n");
				return -EINVAL;
			}
#endif
		} else {
...
static int power_off_cp(struct modem_ctl *mc)
{
	...
	if (_is_first_boot) {
		mif_info("cp does not boot up. cp power off sequence is skipped\n");
		return 0;
	}
...
static int power_shutdown_cp(struct modem_ctl *mc)
{
	...
	if (_is_first_boot) {
		mif_info("cp does not boot up. cp shutdown sequence is skipped\n");
		return 0;
	}
