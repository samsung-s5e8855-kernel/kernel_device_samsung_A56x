...
static bool is_desc_valid(struct pktproc_queue *q, struct pktproc_desc *desc)
{
...
static u8 *get_packet_vaddr(struct pktproc_queue *q, struct pktproc_desc *desc, u32 idx)
{
	u8 *ret;
	struct pktproc_adaptor *ppa = q->ppa;
...
static struct sk_buff *cpif_build_skb(struct pktproc_queue *q, u8 *src, u16 len, u16 front_pad_size)
{
	struct sk_buff *skb;

	if (q->manager) {
		skb = napi_build_skb(src - front_pad_size, SKB_DATA_ALIGN(front_pad_size + len) 
...
static int pktproc_get_pkt(struct pktproc_queue *q, struct sk_buff **new_skb)
{
	int ret = 0;
	u16 len = 0;
	u8 ch_id;
	...
	struct pktproc_desc desc_done_ptr = q->desc[q->done_ptr];
	struct link_device *ld = &q->mld->link_dev;
	bool csum = false;
	...
	pr_buffer("desc", (char *)&desc_done_ptr, (size_t)sizeof(struct pktproc_desc),
			(size_t)sizeof(struct pktproc_desc));
...
flush:
			...
			if (iph->version == 4) /* ipv4 checksum will be done in kernel */ {
				iph->check = 0;
				if (likely(iph->ihl <= sizeof(struct iphdr) + MAX_IPOPTLEN))
...
static ssize_t status_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i < ppa->num_queue; i++) {
		...
		if (q->manager && !q->manager->use_one_vmap) {
			count += scnprintf(&buf[count], PAGE_SIZE - count, "  temp map: in_idx%d out_idx%d\n",
				q->manager->temp_map->in_idx, q->manager->temp_map->out_idx);
		}
	}
...
int pktproc_init(struct pktproc_adaptor *ppa)
{
	...
	mif_info("cp_base:0x%08llx num_queue:%d\n",
		ppa->cp_base, ppa->num_queue);
...
static int pktproc_create_buffer_manager(struct pktproc_queue *q, u64 ap_desc_pbase)
{
	...
	desc_total_size = q->num_desc * sizeof(struct pktproc_desc);
	ppa = q->ppa;
	if (!ppa) {
		mif_err("ppa is null\n");
		return -EINVAL;
	...
	desc_addr_pair.is_tmp_alloc = ppa->use_one_vmap ? 1 : 0;
	...
	mif_info("about to init netrx mng: cp_addr: 0x%llX ap_addr: %pK frag_size: %llu\n",
			q->cp_desc_pbase, q->desc, frag_size);
...
static int pktproc_get_info(struct pktproc_adaptor *ppa, struct device_node *np)
{
	...
	mif_dt_read_u32(np, "pktproc_dl_use_one_vmap", ppa->use_one_vmap);
	mif_info("iocc:%d max_packet_size:%d baaw:%d one_vmap:%d\n",
		ppa->use_hw_iocc, ppa->max_packet_size, ppa->use_dedicated_baaw, ppa->use_one_vmap);
					...
					unsigned long memaddr, u32 memsize)
{
	...
	for (i = 0; i < ppa->num_queue; i++) {
		...
		/* Info region */
		q->info = (struct pktproc_info *)ppa->info_vbase;
		q->info->num_queues = ppa->num_queue;
		q->info->irq_mode = ppa->use_exclusive_irq;
		q->info->max_packet_size = ppa->max_packet_size;
		q->q_info_ptr = &q->info->q_info[i];
		...
		q->desc = ppa->desc_vbase +
				(i * sizeof(struct pktproc_desc) *
				 q->num_desc);
		...
		q->desc_size = sizeof(struct pktproc_desc) * q->num_desc;
