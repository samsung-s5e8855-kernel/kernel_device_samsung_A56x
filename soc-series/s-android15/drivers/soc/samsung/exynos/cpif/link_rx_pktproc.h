...
#if IS_ENABLED(CONFIG_CP_PKTPROC_LRO)
#define MAX_NUM_LRO_STREAM	4
struct pktproc_lro_stream {
	struct sk_buff *skb;
	struct sk_buff *last;
	bool empty; /* true : no pkt, false : pkt exists */
	bool bypass_lro;
};
#endif
...
struct pktproc_queue {
	...
	/* Pointer to fore_ptr of q_info. */
	u32 *fore_ptr;
	/* Pointer to rear_ptr of q_info. */
	u32 *rear_ptr;
	/* Follow rear_ptr */
	u32 done_ptr;
	...
	/* Pointer to info region */
	struct pktproc_info *info;
	struct pktproc_q_info *q_info_ptr;	/* Pointer to q_info of info_v */

	/* Pointer to desc region */
	struct pktproc_desc *desc;
	u32 desc_size;
...
#if IS_ENABLED(CONFIG_CP_PKTPROC_LRO)
	struct pktproc_lro_stream lro_stream[MAX_NUM_LRO_STREAM];
#endif
...
struct pktproc_adaptor {
	...
	bool use_one_vmap;	/* Decide use two or single vmap */
...
extern ssize_t perftest_show(struct device *dev, struct device_attribute *attr, char *buf);
extern ssize_t perftest_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count);
#else
...
static inline ssize_t perftest_show(struct device *dev, struct device_attribute *attr, char *buf) { return 0; };
static inline ssize_t perftest_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count) { return 0; };
