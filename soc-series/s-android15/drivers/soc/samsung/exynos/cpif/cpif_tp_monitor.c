...
{
	mif_info_limited("DL:%ldMbps(%ld/%ld/%ld) Q:%d/%d/%d tcp_rmem:%d/%d/%d\n",
...
static int rps_cpumask_housekeeping(struct cpumask *mask)
{
...
#if IS_ENABLED(CONFIG_FREQ_QOS_TRACER)
...
static void tpmon_set_dynamic_pool(struct tpmon_data *data)
{
	struct mem_link_device *mld = ld_to_mem_link_device(data->tpmon->ld);
	struct pktproc_adaptor *ppa = &mld->pktproc;
	struct cpif_page_pool *pool;
	struct hrtimer *timer = &data->tpmon->dynamic_retry_timer;
	int ret;
	u32 val;
	u32 prev_val;
	...
	pool = ppa->q[0]->manager->data_pool;

	val = tpmon_get_curr_level(data);
	prev_val = data->level[data->prev_level_pos];

	if (prev_val < val) {
		if (pool->page_alloc_complete)
			ret = cpif_page_pool_expand(pool, (u64) val);
		else
			ret = -ENOMEM;
	} else
		ret = cpif_page_pool_lazy_deletion(pool, (u64) val);

	if (ret < 0) {
		mif_info("dynamic pool resize failed %d -> %d\n", prev_val, val);
		if (prev_val < val && !hrtimer_is_queued(timer)) {
			ktime_t ktime = ktime_set(0, 1000 * NSEC_PER_MSEC);
			hrtimer_start(timer, ktime, HRTIMER_MODE_REL);
		}
	} else {
		mif_info("dynamic pool resize %d -> %d\n", prev_val, val);
		if (data->curr_level_pos == 0) {
...
static enum hrtimer_restart dynamic_pool_retry_timer(struct hrtimer *timer)
{
	struct cpif_tpmon *tpmon = &_tpmon;
	struct tpmon_data *data;

	list_for_each_entry(data, &tpmon->all_data_list, data_node) {
		if (data->set_data == tpmon_set_dynamic_pool)
			break;
	}

	// If level is 0, no need to boost tpmon
	if (data->curr_level_pos)
		data->set_data(data);
	...
	struct device_attribute *attr, char *buf)
{
	...
	len += scnprintf(buf + len, PAGE_SIZE - len,
			"queue status pktproc:%d netdev:%d legacy:%d\n",
			tpmon->q_status_pktproc_dl,
			tpmon->q_status_netdev_backlog,
			tpmon->legacy_packet_count);
	len += scnprintf(buf + len, PAGE_SIZE - len,
...
static int tpmon_set_target(struct tpmon_data *data)
{
	...
	case TPMON_TARGET_CPU_CL2_MAX:
		...
		break;
#endif
	case TPMON_TARGET_DYNAMIC_POOL:
		data->set_data = tpmon_set_dynamic_pool;
		break;
...
int tpmon_create(struct platform_device *pdev, struct link_device *ld)
{
	...
	hrtimer_init(&tpmon->dynamic_retry_timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
	tpmon->dynamic_retry_timer.function = dynamic_pool_retry_timer; 
