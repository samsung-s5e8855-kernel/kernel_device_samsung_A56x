...
data_error:
	mif_err("CPIF shmem offset data is damaged\n");
	return -EFAULT;
}
...
void change_modem_state(struct modem_ctl *mc, enum modem_state state)
{
	...
	/* recording monothonic time of becoming CP ONLINE state */
	if (state == STATE_ONLINE)
		mc->reset_time = ktime_get();
...
int send_panic_to_cp_notifier(struct notifier_block *nb, unsigned long action,
			      void *nb_data)
{
	struct modem_ctl *mc = container_of(nb, struct modem_ctl, send_panic_nb);
	struct modem_data *modem = mc->mdm_data;
	struct mem_link_device *mld = modem->mld;

	mif_info("Send CMD_KERNEL_PANIC message to CP\n");
	send_ipc_irq(mld, cmd2int(CMD_KERNEL_PANIC));
...
int modem_force_crash_exit(struct modem_ctl *mc)
{
	...
	ld->link_trigger_cp_crash(mld, CRASH_REASON_MIF_FORCED, "Forced crash is called");

	return 0;
}
EXPORT_SYMBOL(modem_force_crash_exit);
