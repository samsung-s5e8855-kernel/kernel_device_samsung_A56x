						...
						u64 frag_size, u64 num_packet)
{
	...
	cm->use_one_vmap = desc_addr_pair->is_tmp_alloc;
	 ...
	 * reserve twice as large of the least required
	 */
	num_packet_per_page = NETRX_POOL_PAGE_SIZE / frag_size;
	total_page_count = (num_packet / num_packet_per_page) * CPIF_PAGE_POOL_MULT + 1;
	/**
	 * total buffer size is calculated based on worst case. buffer
	 * composed of 4KB pages only
	 */
	cm->total_buf_size = (total_page_count  + 1) * NETRX_POOL_PAGE_SIZE;
	...
	cm->data_map = cpif_vmap_create(databuf_cp_pbase, cm->total_buf_size, frag_size,
			cm->use_one_vmap ? num_packet : total_page_count);
	if (!cm->data_map) {
	...
	if (!cm->use_one_vmap) {
		tmp_size = calc_vmap_size(frag_size, PAGE_SIZE, num_packet);
		temp = calc_vmap_size(frag_size, NETRX_POOL_PAGE_SIZE, num_packet);
		tmp_size = tmp_size > temp ? tmp_size : temp;
		cm->temp_map = cpif_vmap_create(databuf_cp_pbase + cm->total_buf_size, tmp_size, frag_size,
	        		num_packet);
		cm->total_buf_size += tmp_size;
	} else
		cm->temp_map = cm->data_map;

	if (!cm->use_one_vmap && !cm->temp_map) {
		cpif_vmap_free(cm->desc_map);
	...
	cm->data_pool = cpif_page_pool_create((num_packet / num_packet_per_page), NETRX_POOL_PAGE_SIZE);
	...
	mif_info("temp vmap: va_start: 0x%llX va_end: 0x%llX va_size: %llu\n",
		 cm->temp_map->va_start, cm->temp_map->va_end, cm->temp_map->va_size);
...
fail:
	cpif_vmap_free(cm->desc_map);
	cpif_vmap_free(cm->data_map);
	if (!cm->use_one_vmap) cpif_vmap_free(cm->temp_map);
fail_vmap:
	kvfree(cm->apair_arr);
fail_apair_arr:
	kvfree(cm);
...
void cpif_exit_netrx_mng(struct cpif_netrx_mng *cm)
{
	if (cm) {
		if (cm->data_pool)
			cm->data_pool = cpif_page_pool_delete(cm->data_pool);
...
void cpif_init_netrx_mng(struct cpif_netrx_mng *cm)
{
	if (cm->data_map && cm->data_pool) {
		...
		if (!cm->use_one_vmap) cpif_vmap_init(cm->temp_map);
...
struct cpif_addr_pair *cpif_map_rx_buf(struct cpif_netrx_mng *cm, u32 idx)
{
	...
	if (unlikely(!cm->temp_map)) {
		mif_err_limited("temp map is not created yet\n");
		goto done;
	}
	...
	if (used_tmp_alloc || cm->use_one_vmap) {
		cp_addr = cpif_vmap_map_area(cm->temp_map, page_to_phys(page),
						page_size, virt_to_phys(data));
	} else {
		in_item = &cm->data_map->item_arr[cm->data_pool->rpage_arr_idx];
		if (!in_item->vaddr_base)
			cp_addr = cpif_vmap_map_area(cm->data_map, page_to_phys(page),
						page_size, virt_to_phys(data));
		else
			cp_addr = in_item->vaddr_base + virt_to_phys(data) - page_to_phys(page);
	}
...
void *cpif_unmap_rx_buf(struct cpif_netrx_mng *cm, u32 idx)
{
	...
	if (unlikely(!cm->temp_map)) {
		mif_err_limited("temp map does not exist\n");
		goto done;
	}
	...
	if (apair->is_tmp_alloc || cm->use_one_vmap) {
		ap_paddr = cpif_vmap_unmap_area(cm->temp_map, apair->cp_addr);
