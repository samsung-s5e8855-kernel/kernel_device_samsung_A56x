...
static int btl_set_size(struct cp_btl *btl, u32 btl_size)
{
	...
	btl->mem.v_base = cp_shmem_get_region_with_size(SHMEM_BTL, btl_size);
	if (!btl->mem.v_base) {
	...
	ret = (int)exynos_smc(SMC_ID, CP_CPMEM_ON, btl_size, 0);
	if (ret) {
	...
	if (release_size > 0) {
		cp_shmem_release_rmem(SHMEM_BTL, btl_size);
		cp_shmem_set_size(SHMEM_BTL, btl_size);
		btl->mem.size = cp_shmem_get_size(SHMEM_BTL);
	}

	return 0;

set_err:
	cp_shmem_release_region(SHMEM_BTL);
	btl->mem.v_base = NULL;
	cp_shmem_release_rmem(SHMEM_BTL, 0);
...
int cp_btl_create(struct cp_btl *btl, struct device *dev)
{
	...
	switch (btl->link_type) {
		case LINKDEV_SHMEM:
			btl->mem.v_base = NULL;
			btl->mem.p_base = cp_shmem_get_base(SHMEM_BTL);
			btl->mem.size = cp_shmem_get_size(SHMEM_BTL);
			break;
	...
	mif_info("name:%s link:%d\n", btl->name, btl->link_type);
