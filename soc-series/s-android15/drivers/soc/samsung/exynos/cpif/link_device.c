...
#if IS_ENABLED(CONFIG_MCU_IPC) && IS_ENABLED(CONFIG_CPIF_DYNAMIC_L2B)
static int init_l2b_region(struct mem_link_device *mld);
#endif
...
static void write_clk_table_to_shmem(struct mem_link_device *mld)
{
	...
	memcpy(clk_tb->table_info[3].table_name, "CP_M", 4);
	clk_tb->table_info[3].table_count = mld->cp_mcw_table.num_of_table;

	memcpy(clk_tb->table_info[4].table_name, "INT", 4);
	clk_tb->table_info[4].table_count = mld->int_table.num_of_table;
...
static void cmd_init_start_handler(struct mem_link_device *mld)
{
	...
	/* TODO: In case of 5G proejct, error case should be considered. */
	direct_dm_init(ld);
#endif
...
exit:
#if IS_ENABLED(CONFIG_CP_PKTPROC)
	start_tx_timer(mld, &mld->pktproc_print_timer);
#endif
#if IS_ENABLED(CONFIG_LINK_DEVICE_WITH_SBD_ARCH)
	if (ld->sbd_ipc)
...
static int legacy_ipc_rx_func(struct mem_link_device *mld, struct legacy_ipc_device *dev)
{
	...
	while (rcvd < size) {
		...
		if (is_boot_iod(iod) || is_dump_iod(iod)) {
			if (skb->len == SIT_HEADER_LEN) {
				const char *data = skb->data;
				/* cd ab 03 00 00 00 10 00 f1 03 00 00 0d c0 00 00 */
				if (!strncmp(&data[skb->len - SIT_MSG_LEN], SIT_MSG, SIT_MSG_LEN))
					pr_skb("DUMP", skb, skbpriv(skb)->ld);
			}
		}
...
static int shm_get_security_param(struct link_device *ld,
	unsigned long *mode, unsigned long *param2, unsigned long *param3)
{
	int ret = 0;

	switch (*mode) {
	case CP_BOOT:
		if (cp_shmem_get_mem_map_on_cp_flag())
			*mode |= cp_shmem_get_base(SHMEM_CP);
		break;

	case CP_DUMP:
		*param3 = cp_shmem_get_base(SHMEM_IPC);
		break;
	...
	case CP_CHECK_TOC_SIGN:
		*param2 += cp_shmem_get_base(SHMEM_CP);
		break;

#if IS_ENABLED(CONFIG_VERIFY_VSS)
	case CP_VERIFY_VSS:
		*param2 += cp_shmem_get_base(SHMEM_CP);
		*param3 |= ((unsigned long)cp_shmem_get_size(SHMEM_VSS) << 32);
		break;
#endif
...
int cpif_exynos_smc(unsigned long mode, unsigned long param2, unsigned long param3)
{
	int err = 0;
...
#if IS_ENABLED(CONFIG_CP_PKTPROC)
static int request_pktproc_baaw(struct mem_link_device *mld)
{
	int err = 0;
...
#else
	unsigned long shm_pktproc_size = (unsigned long)cp_shmem_get_size(SHMEM_PKTPROC);
#if IS_ENABLED(CONFIG_CP_PKTPROC_UL)
	shm_pktproc_size += (unsigned long)cp_shmem_get_size(SHMEM_PKTPROC_UL);
#endif
#endif

#if IS_ENABLED(CONFIG_EXYNOS_CPIF_IOMMU) || IS_ENABLED(CONFIG_EXYNOS_CPIF_IOMMU_V9)
	err = cpif_exynos_smc(CP_SET_BAAW, ppa->cp_base, SYSMMU_BAAW_SIZE);
#else
	err = cpif_exynos_smc(CP_SET_BAAW,
		(unsigned long)cp_shmem_get_base(SHMEM_PKTPROC), shm_pktproc_size);
#endif

	mif_info("return_value=%d\n", err);
...
static void check_vss_stop(struct link_device *ld)
{
	...
	if (first_boot == true) {
		mif_info("first boot - skip waiting vss stop\n");
		first_boot = false;
		return;
	}
...
static int request_smc_call(struct link_device *ld, unsigned long mode, unsigned long param2,
	unsigned long param3)
{
	...
	err = cpif_exynos_smc(mode, param2, param3);
	while (err == CP_CHECK_SIGN_NOT_FINISH && try_cnt < MAX_TRY_CNT) {
		try_cnt++;
		err = cpif_exynos_smc(CP_CHECK_SIGN_CONTINUE, 0x0, 0x0);
	}
				...
				unsigned long arg)
{
	...
	param2 = (unsigned long)msr.param2;
	param3 = (unsigned long)msr.param3;

	err = shm_get_security_param(ld, &mode, &param2, &param3);
	if (err) {
		mif_err("%s: ERR! parameter is invalid\n", ld->name);
		goto exit;
	}
...
#if IS_ENABLED(CONFIG_CP_SECURE_BOOT)
	err = request_smc_call(ld, mode, param2, param3);
#endif

	if (msr.mode == CP_REINIT) {
#if IS_ENABLED(CONFIG_REINIT_VSS)
		check_vss_stop(ld);
#endif

#if IS_ENABLED(CONFIG_CP_PKTPROC)
		if (mld->pktproc.use_dedicated_baaw)
			err = request_pktproc_baaw(mld);
#endif

#if IS_ENABLED(CONFIG_CPIF_DYNAMIC_L2B)
		init_l2b_region(mld);
#endif
	}
...
static int init_l2b_region(struct mem_link_device *mld)
{
	...
	mif_info("init l2b region\n");
	for (i = 0; i < L2B_IDX_MAX; i++) {
		cp_cma = get_cp_cma_region(i);
		if (!cp_cma) {
		...
		if (cp_cma->paddr) {
			mif_info("paddr for dealloc:0x%llx\n", cp_cma->paddr);
			err = cpif_exynos_smc(CP_DYNAMIC_L2B_DEALLOC, cp_cma->paddr, 0);
			if (err)
				mif_info("cpif_exynos_smc() dealloc:%d\n", err);
		}

		err = dealloc_cp_cma_region(cp_cma);
		if (err) {
			mif_err("dealloc_cp_cma_region() err:%d\n", err);
			continue;
		}
	}

	set_ctrl_msg(&mld->ap2cp_l2b_msg, 0);
	set_ctrl_msg(&mld->cp2ap_l2b_msg, 0);
	set_ctrl_msg(&mld->cp2ap_l2b_address, 0);
	set_ctrl_msg(&mld->cp2ap_l2b_size, 0);
	set_ctrl_msg(&mld->ap2cp_l2b_size_0, 0);
	set_ctrl_msg(&mld->ap2cp_l2b_size_1, 0);

	return 0;
}

static void cpif_l2b_work(struct work_struct *ws)
{
	struct mem_link_device *mld = container_of(ws,
		struct mem_link_device, l2b_dwork.work);
	enum cp_l2b_req req;
	enum cp_l2b_idx idx;
	int err;
	u32 cp_paddr;
	...
	req = extract_ctrl_msg(&mld->cp2ap_l2b_msg,
		mld->sbi_cp_l2b_request_mask, mld->sbi_cp_l2b_request_pos);
	idx = extract_ctrl_msg(&mld->cp2ap_l2b_msg,
		mld->sbi_cp_l2b_index_mask, mld->sbi_cp_l2b_index_pos);
	cp_paddr = get_ctrl_msg(&mld->cp2ap_l2b_address);
	size = get_ctrl_msg(&mld->cp2ap_l2b_size);

	mif_info("cp2ap req:%d idx:%d cp_paddr:0x%08x size:0x%08x\n",
		req, idx, cp_paddr, size);
	...
	update_ctrl_msg(&mld->ap2cp_l2b_msg, req,
		mld->sbi_ap_l2b_request_mask,
		mld->sbi_ap_l2b_request_pos);
	update_ctrl_msg(&mld->ap2cp_l2b_msg, idx,
		mld->sbi_ap_l2b_index_mask,
		mld->sbi_ap_l2b_index_pos);

	switch (req) {
	case L2B_REQ_ALLOC:
		if (!cp_paddr) {
			mif_err("cp_paddr is null\n");
			goto error;
		}

		err = alloc_cp_cma_region(cp_cma, size);
		if (err) {
			mif_err("alloc_cp_cma_region() err:%d\n", err);
			goto error;
		}
		mif_info("paddr for alloc:0x%llx\n", cp_cma->paddr);

		err = cpif_exynos_smc(CP_DYNAMIC_L2B_ALLOC | cp_paddr,
			cp_cma->paddr, size);
		if (err) {
			mif_err("cpif_exynos_smc() alloc err:%d\n", err);
			goto error;
		}
		break;
	case L2B_REQ_DEALLOC:
		if (!cp_cma->paddr) {
			mif_err("cp_cma->paddr is null\n");
			goto error;
		}
		mif_info("paddr for dealloc:0x%llx\n", cp_cma->paddr);
		err = cpif_exynos_smc(CP_DYNAMIC_L2B_DEALLOC, cp_cma->paddr, 0);
		if (err) {
			mif_err("cpif_exynos_smc() dealloc err:%d\n", err);
			goto error;
		}
	...
	update_ctrl_msg(&mld->ap2cp_l2b_msg, L2B_RETURN_SUCCESS,
		mld->sbi_ap_l2b_return_mask,
		mld->sbi_ap_l2b_return_pos);
	idx == 1 ? set_ctrl_msg(&mld->ap2cp_l2b_size_1, size) :
		set_ctrl_msg(&mld->ap2cp_l2b_size_0, size);

	cp_mbox_set_interrupt(CP_MBOX_IRQ_IDX_0, mld->int_ap2cp_l2b_status);

	mif_info("success idx:%d size:%d\n", idx, size);

	return;

error:
	update_ctrl_msg(&mld->ap2cp_l2b_msg, L2B_RETURN_FAIL,
		mld->sbi_ap_l2b_return_mask,
		mld->sbi_ap_l2b_return_pos);
	idx == 1 ? set_ctrl_msg(&mld->ap2cp_l2b_size_1, 0) :
		set_ctrl_msg(&mld->ap2cp_l2b_size_0, 0);

	cp_mbox_set_interrupt(CP_MBOX_IRQ_IDX_0, mld->int_ap2cp_l2b_status);

	mif_info("fail idx:%d size:%d\n", idx, size);
}

static irqreturn_t shmem_cp2ap_l2b_handler(int irq, void *data)
{
	struct mem_link_device *mld = (struct mem_link_device *)data;

	if (!mld) {
		mif_err_limited("mld is null\n");
		return IRQ_HANDLED;
	}

	queue_delayed_work(mld->l2b_wq, &mld->l2b_dwork, 0);
...
static int parse_ect(struct mem_link_device *mld, char *dvfs_domain_name)
{
	...
	if (!strcmp(dvfs_domain_name, "MIF")) {
		...
		mld->total_freq_table_count++;
		for (i = dvfs_domain->num_of_level - 1; i >= 0; i--) {
			mld->int_table.freq[i] =
				dvfs_domain->list_level[counter++].level;
			mif_info("INT_LEV[%d] : %u\n", i + 1,
					mld->int_table.freq[i]);
		}
	}
...
#if IS_ENABLED(CONFIG_CPIF_DYNAMIC_L2B)
static ssize_t cp_l2b_status_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	ret = sscanf(buf, "%i %i %i %i", &req, &idx, &cp_paddr, &size);
	if (ret < 1) {
		mif_err("invalid input values. ret:%d\n", ret);
		return -EINVAL;
	}
	...
	update_ctrl_msg(&mld->cp2ap_l2b_msg, req,
		mld->sbi_cp_l2b_request_mask,
		mld->sbi_cp_l2b_request_pos);
	update_ctrl_msg(&mld->cp2ap_l2b_msg, idx,
		mld->sbi_cp_l2b_index_mask,
		mld->sbi_cp_l2b_index_pos);
	set_ctrl_msg(&mld->cp2ap_l2b_address, cp_paddr);
	set_ctrl_msg(&mld->cp2ap_l2b_size, size);

	cp_mbox_set_interrupt(CP_MBOX_IRQ_IDX_0, modem->mbx->int_ap2cp_wakeup);
	shmem_cp2ap_l2b_handler(0, mld);

	return count;
}

static ssize_t cp_l2b_status_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct modem_data *modem;
	struct mem_link_device *mld;
	...
	count += scnprintf(&buf[count], PAGE_SIZE - count,
		"cp2ap req:%d idx:%d cp_paddr:0x%08x size:0x%08x\n",
		req, idx, cp_paddr, size);
	...
	req = extract_ctrl_msg(&mld->ap2cp_l2b_msg,
		mld->sbi_ap_l2b_request_mask, mld->sbi_ap_l2b_request_pos);
	idx = extract_ctrl_msg(&mld->ap2cp_l2b_msg,
		mld->sbi_ap_l2b_index_mask, mld->sbi_ap_l2b_index_pos);
	ap_ret = extract_ctrl_msg(&mld->ap2cp_l2b_msg,
		mld->sbi_ap_l2b_return_mask, mld->sbi_ap_l2b_return_pos);

	count += scnprintf(&buf[count], PAGE_SIZE - count,
		"ap2cp req:%d idx:%d paddr:0x%llx ap_ret:0x%08x\n",
		req, idx, cp_cma->paddr, ap_ret);

	return count;
}
#endif /* end of CONFIG_CPIF_DYNAMIC_L2B */
...
#endif
#if IS_ENABLED(CONFIG_CPIF_DYNAMIC_L2B)
static DEVICE_ATTR_RW(cp_l2b_status);
#endif
...
#if IS_ENABLED(CONFIG_CP_PKTPROC)
static u32 p_pktproc_dl[PKTPROC_MAX_QUEUE][3];
static u32 c_pktproc_dl[PKTPROC_MAX_QUEUE][3];
#if IS_ENABLED(CONFIG_CP_PKTPROC_UL)
static u32 p_pktproc_ul[PKTPROC_UL_QUEUE_MAX][3];
static u32 c_pktproc_ul[PKTPROC_UL_QUEUE_MAX][3];
#endif

static enum hrtimer_restart pktproc_print(struct hrtimer *timer)
{
	struct mem_link_device *mld = container_of(timer, struct mem_link_device,
							pktproc_print_timer);
	struct pktproc_adaptor *ppa = &mld->pktproc;
#if IS_ENABLED(CONFIG_CP_PKTPROC_UL)
	struct pktproc_adaptor_ul *ppa_ul = &mld->pktproc_ul;
	...
	for (i = 0; i < ppa->num_queue; i++) {
		...
		c_pktproc_dl[i][0] = *q->fore_ptr;
		c_pktproc_dl[i][1] = *q->rear_ptr;
		c_pktproc_dl[i][2] = q->done_ptr;

		if (memcmp(p_pktproc_dl[i], c_pktproc_dl[i], sizeof(u32)*3)) {
			mif_info("[DL][%d] f:%06d r:%06d d:%06d (lro:%lld csum:%lld itg:%lld)\n",
				i, c_pktproc_dl[i][0], c_pktproc_dl[i][1], c_pktproc_dl[i][2],
				q->stat.lro_cnt, q->stat.err_csum, q->stat.err_itg);
			memcpy(p_pktproc_dl[i], c_pktproc_dl[i], sizeof(u32)*3);
		}
	}
	...
	for (i = 0; i < ppa_ul->num_queue; i++) {
		...
		c_pktproc_ul[i][0] = *q->fore_ptr;
		c_pktproc_ul[i][1] = *q->rear_ptr;
		c_pktproc_ul[i][2] = q->done_ptr;

		if (memcmp(p_pktproc_ul[i], c_pktproc_ul[i], sizeof(u32)*3)) {
			mif_info("[UL][%d] f:%06d r:%06d d:%06d (buff_full:%lld inactive:%lld)\n",
				i, c_pktproc_ul[i][0], c_pktproc_ul[i][1], c_pktproc_ul[i][2],
				q->stat.buff_full_cnt, q->stat.inactive_cnt);
			memcpy(p_pktproc_ul[i], c_pktproc_ul[i], sizeof(u32)*3);
		}
	}
#endif /* CONFIG_CP_PKTPROC_UL */
...
#endif
#if IS_ENABLED(CONFIG_CP_PKTPROC)
	hrtimer_init(&mld->pktproc_print_timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
	mld->pktproc_print_timer.function = pktproc_print;
#endif
...
#endif
#if IS_ENABLED(CONFIG_CPIF_DYNAMIC_L2B)
	construct_ctrl_msg(&mld->ap2cp_l2b_msg, modem->ap2cp_l2b_msg, cmsg_base);
	construct_ctrl_msg(&mld->cp2ap_l2b_msg, modem->cp2ap_l2b_msg, cmsg_base);
	construct_ctrl_msg(&mld->cp2ap_l2b_address, modem->cp2ap_l2b_address, cmsg_base);
	construct_ctrl_msg(&mld->cp2ap_l2b_size, modem->cp2ap_l2b_size, cmsg_base);
	construct_ctrl_msg(&mld->ap2cp_l2b_size_0, modem->ap2cp_l2b_size_0, cmsg_base);
	construct_ctrl_msg(&mld->ap2cp_l2b_size_1, modem->ap2cp_l2b_size_1, cmsg_base);
#endif
#if IS_ENABLED(CONFIG_CP_BTL)
...
#if IS_ENABLED(CONFIG_CPIF_DYNAMIC_L2B)
	irq_num = mld->irq_cp2ap_l2b_status;
	err = cp_mbox_register_handler(CP_MBOX_IRQ_IDX_0, irq_num,
				       shmem_cp2ap_l2b_handler, mld);
	if (err)
		goto irq_error;
#endif
	...
	struct mem_link_device *mld)
{
	...
	mif_info("Parsing INT frequency table...\n");
	err = parse_ect(mld, "INT");
	if (err < 0)
		mif_err("Can't get INT frequency table!!!!!\n");
...
#if IS_ENABLED(CONFIG_CPIF_DYNAMIC_L2B)
	mld->l2b_wq = alloc_workqueue("cpif_l2b_wq",
					__WQ_LEGACY | WQ_MEM_RECLAIM | WQ_UNBOUND, 1);
	if (!mld->l2b_wq) {
		mif_err("alloc_workqueue() l2b_wq error\n");
		goto error;
	}
	INIT_DELAYED_WORK(&mld->l2b_dwork, cpif_l2b_work);
#endif
...
#if IS_ENABLED(CONFIG_CPIF_DYNAMIC_L2B)
	mld->int_ap2cp_l2b_status = modem->mbx->int_ap2cp_l2b_status;
	mld->irq_cp2ap_l2b_status = modem->mbx->irq_cp2ap_l2b_status;

	mld->sbi_ap_l2b_request_mask = modem->sbi_ap_l2b_request_mask;
	mld->sbi_ap_l2b_request_pos = modem->sbi_ap_l2b_request_pos;
	mld->sbi_ap_l2b_index_mask = modem->sbi_ap_l2b_index_mask;
	mld->sbi_ap_l2b_index_pos = modem->sbi_ap_l2b_index_pos;
	mld->sbi_ap_l2b_return_mask = modem->sbi_ap_l2b_return_mask;
	mld->sbi_ap_l2b_return_pos = modem->sbi_ap_l2b_return_pos;
	mld->sbi_cp_l2b_request_mask = modem->sbi_cp_l2b_request_mask;
	mld->sbi_cp_l2b_request_pos = modem->sbi_cp_l2b_request_pos;
	mld->sbi_cp_l2b_index_mask = modem->sbi_cp_l2b_index_mask;
	mld->sbi_cp_l2b_index_pos = modem->sbi_cp_l2b_index_pos;
	mld->sbi_cp_l2b_return_mask = modem->sbi_cp_l2b_return_mask;
	mld->sbi_cp_l2b_return_pos = modem->sbi_cp_l2b_return_pos;
#endif
