...
static int cp_get_log_dump(struct io_device *iod, struct link_device *ld, unsigned long arg)
{
	...
	case LOG_IDX_SHMEM:
		if (modem->legacy_raw_rx_buffer_cached) {
			base = phys_to_virt(cp_shmem_get_base(SHMEM_IPC));
			size = cp_shmem_get_size(SHMEM_IPC);
		} else {
			base = mld->base;
			size = mld->size;
		}
	...
	case LOG_IDX_VSS:
		base = mld->vss_base;
		size = cp_shmem_get_size(SHMEM_VSS);
		break;
	...
	case LOG_IDX_ACPM:
		if (mld->acpm_base) {
			...
			for (i = 0; i < strlen(log_dump.name); i++) {
				if (isdigit(log_dump.name[i])) {
					ret = kstrtoint(&log_dump.name[i], 10, &dump_idx);
					if (ret < 0) {
						mif_err("%s kstrtoint error:%d\n", log_dump.name, ret);
						return ret;
					}
					mif_info("%s dump_idx:%d\n", log_dump.name, dump_idx);
					break;
				}
			}
			if ((dump_idx >= mld->esca_num_dump) || (dump_idx < 0)) {
				mif_err("%s dump_idx is invalid:%d\n", log_dump.name, dump_idx);
				return -EINVAL;
			}
			base = mld->esca_dump_addrs[dump_idx];
			size = mld->esca_dump_sizes[dump_idx];
		}
		break;
	...
	case LOG_IDX_DATABUF:
		base = phys_to_virt(cp_shmem_get_base(SHMEM_PKTPROC));
#if IS_ENABLED(CONFIG_CP_PKTPROC_UL)
		size = cp_shmem_get_size(SHMEM_PKTPROC) +
	...
	case LOG_IDX_DATABUF_UL:
		base = phys_to_virt(cp_shmem_get_base(SHMEM_PKTPROC_UL));
		size = cp_shmem_get_size(SHMEM_PKTPROC_UL);
		break;
#endif

#if !IS_ENABLED(CONFIG_CPIF_DYNAMIC_L2B)
	case LOG_IDX_L2B:
		base = phys_to_virt(cp_shmem_get_base(SHMEM_L2B));
		size = cp_shmem_get_size(SHMEM_L2B);
		break;
#endif

	case LOG_IDX_DDM:
		base = phys_to_virt(cp_shmem_get_base(SHMEM_DDM));
		size = cp_shmem_get_size(SHMEM_DDM);
		break;
...
static int cp_get_num_of_log_dump(struct io_device *iod, struct link_device *ld, unsigned long arg)
{
	struct mem_link_device *mld = to_mem_link_device(ld);
	void __user *uarg = (void __user *)arg;
	struct cp_log_num_dump num_log_dump;
	int ret = 0;

	ret = copy_from_user(&num_log_dump, uarg, sizeof(struct cp_log_num_dump));
	if (ret) {
		mif_err("copy_from_user() error:%d\n", ret);
		return ret;
	}
	mif_info("%s index:%d\n", iod->name, num_log_dump.idx);

	switch (num_log_dump.idx) {
	case LOG_IDX_ACPM:
		if (mld->acpm_base) {
			num_log_dump.num_dump = 1;
		} else {
			mif_info("esca num dump:%d\n", mld->esca_num_dump);
			num_log_dump.num_dump = mld->esca_num_dump;
		}
		break;
	...
	mif_info("%s num dump:%d\n", iod->name, num_log_dump.num_dump);

	ret = copy_to_user(uarg, &num_log_dump, sizeof(num_log_dump));
	if (ret) {
		mif_err("copy_to_user() error:%d\n", ret);
		return ret;
	}
...
static long bootdump_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case IOCTL_REQ_SECURITY:
		...
		if (ret) {
			mif_err("security_req() error:0x%x\n", ret);
			return ret;
		}
		return ret;
	...
	case IOCTL_TRIGGER_AP_WDTRESET:
		mif_info("%s: IOCTL_TRIGGER_AP_WDTRESET\n", iod->name);
	...
	case IOCTL_GET_NUM_OF_LOG_DUMP:
		mif_info("%s: IOCTL_GET_NUM_OF_LOG_DUMP\n", iod->name);

		return cp_get_num_of_log_dump(iod, ld, arg);
	...
	case IOCTL_GET_RFBOARD_DISCONNECT: {
		if (!mc->ops.check_rfboard_disconnect) {
			mif_err("%s: check_rfborad_disconnect is null\n", iod->name);
			return -EINVAL;
		}

		mif_info("%s: IOCTL_GET_RFBOARD_DISCONNECT\n", iod->name);

		return mc->ops.check_rfboard_disconnect(mc);
	}
