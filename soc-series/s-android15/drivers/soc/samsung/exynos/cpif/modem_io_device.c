...
enqueue:
	if (is_boot_iod(iod) || is_dump_iod(iod)) {
		if (len == SIT_MSG_LEN) {
			/* 0D C0 00 00 */
			if (!strncmp((skb)->data, SIT_MSG, SIT_MSG_LEN))
...
static int rx_raw_misc(struct sk_buff *skb)
{
	...
	if (is_boot_iod(iod) || is_dump_iod(iod)) {
		int len = skb->len;
		if (len == SIT_HEADER_LEN) {
			const char *data = skb->data;
			/* cd ab 03 00 00 00 10 00 f1 03 00 00 0d c0 00 00 */
			if (!strncmp(&data[len - SIT_MSG_LEN], SIT_MSG, SIT_MSG_LEN))
				pr_skb("DUMP", skb, skbpriv(skb)->ld);
		}
	}
...
int sipc5_init_io_device(struct io_device *iod, struct modem_data *pdata)
{
	...
	case IODEV_NET:
		...
		rxqs = pdata->mld->pktproc.num_queue ? pdata->mld->pktproc.num_queue : 1;
#endif
		skb_queue_head_init(&iod->sk_rx_q);
