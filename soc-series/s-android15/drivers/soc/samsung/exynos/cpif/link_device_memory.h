...
struct mem_link_device {
	...
	unsigned int int_ap2cp_l2b_status;
	unsigned int irq_cp2ap_l2b_status;

	unsigned int sbi_ap_l2b_request_mask;
	unsigned int sbi_ap_l2b_request_pos;
	unsigned int sbi_ap_l2b_index_mask;
	unsigned int sbi_ap_l2b_index_pos;
	unsigned int sbi_ap_l2b_return_mask;
	unsigned int sbi_ap_l2b_return_pos;
	unsigned int sbi_cp_l2b_request_mask;
	unsigned int sbi_cp_l2b_request_pos;
	unsigned int sbi_cp_l2b_index_mask;
	unsigned int sbi_cp_l2b_index_pos;
	unsigned int sbi_cp_l2b_return_mask;
	unsigned int sbi_cp_l2b_return_pos;
#endif
	...
	struct hrtimer pktproc_print_timer;
#endif
...
#if IS_ENABLED(CONFIG_CPIF_DYNAMIC_L2B)
	struct workqueue_struct *l2b_wq;
	struct delayed_work l2b_dwork;
#endif
...
#endif
#if IS_ENABLED(CONFIG_CPIF_DYNAMIC_L2B)
	struct ctrl_msg ap2cp_l2b_msg;
	struct ctrl_msg cp2ap_l2b_msg;
	struct ctrl_msg cp2ap_l2b_address;
	struct ctrl_msg cp2ap_l2b_size;
	struct ctrl_msg ap2cp_l2b_size_0;
	struct ctrl_msg ap2cp_l2b_size_1;
#endif
	struct ctrl_msg ap2cp_btl_size;
...
#define MEM_MINIDUMP_MAGIC	0x4D494E49
