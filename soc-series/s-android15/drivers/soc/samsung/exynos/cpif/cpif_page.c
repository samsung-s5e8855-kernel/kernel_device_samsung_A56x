...
struct cpif_page_pool *cpif_page_pool_delete(struct cpif_page_pool *pool)
{
	...
	if (rpage_arr) {
		for (i = 0; i < pool->rpage_arr_maxlen; i++) {
...
int cpif_page_pool_continue(u64 page_size, struct cpif_page_pool *pool)
{
	...
	u64 max_num_page = (pool->rpage_arr_baselen * CPIF_PAGE_POOL_MULT) + 1;
	u64 min_num_page = (pool->rpage_arr_baselen * MIN_PAGE_POOL_MULT) + 1;

	mif_info("Continue allocate page pool %u -> %llu\n", pool->rpage_arr_len, min_num_page);

	for (; i < min_num_page; i++) {
	...
	/* Initialize dummpy cpif_page */
	for (; i < max_num_page; i++) {
		struct cpif_page *cur = kvzalloc(sizeof(struct cpif_page), GFP_KERNEL);
...
struct cpif_page_pool *cpif_page_pool_create(u64 num_page, u64 page_size)
{
	...
	u64 max_num_page = (num_page * CPIF_PAGE_POOL_MULT) + 1;
	u64 min_num_page = (num_page * MIN_PAGE_POOL_MULT) + 1;
	...
	rpage_arr = kvzalloc(sizeof(struct cpif_page *) * max_num_page, GFP_KERNEL);
	...
	pool->rpage_arr_len = min_num_page;
	pool->rpage_arr_maxlen = max_num_page;
...
int cpif_page_pool_expand(struct cpif_page_pool *pool, u64 pool_mult)
{
	int i;
	u64 num_page = pool->rpage_arr_baselen * pool_mult;
	struct cpif_page *cur;

	if (num_page < pool->rpage_arr_baselen * MIN_PAGE_POOL_MULT
		|| num_page > pool->rpage_arr_baselen * CPIF_PAGE_POOL_MULT) {
		mif_err("page pool size is out of bound\n");
		return -EINVAL;
	}

	if (++num_page <= pool->rpage_arr_len) {
		mif_info("page pool already expanded\n");
		return 0;
	}

	for (i = pool->rpage_arr_len; i < num_page; i++) {
		cur = pool->recycling_page_arr[i];
		cur->page = __dev_alloc_pages(GFP_KERNEL | CPIF_GFP_MASK, pool->page_order);
		if (unlikely(!cur->page)) {
			mif_err("failed to get page\n");
			cur->usable = false;
...
int cpif_page_pool_lazy_deletion(struct cpif_page_pool *pool, u64 pool_mult)
{
	...
	if (++num_page >= prev_num_page) {
		mif_info("page pool already reduced\n");
		return 0;
	}

	pool->rpage_arr_len = num_page;

	for (i = num_page; i < prev_num_page; i++) {
		cur = rpage_arr[i];
		if (unlikely(!cur)) {
			mif_err("cannot delete invalid page idx: %d\n", i);
			continue;
		}
		put_page(cur->page);
	}
...
EXPORT_SYMBOL(cpif_page_pool_lazy_deletion);
