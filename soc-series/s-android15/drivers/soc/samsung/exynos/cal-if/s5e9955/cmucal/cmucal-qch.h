#ifndef __CMUCAL_QCH_H__
#define __CMUCAL_QCH_H__

#include "../../cmucal.h"
enum qch_id {
    APBIF_GPIO_ALIVE_QCH = QCH_TYPE,
    APBIF_INTCOMB_VGPIO2AP_QCH,
    APBIF_INTCOMB_VGPIO2APM_QCH,
    APBIF_INTCOMB_VGPIO2APM_1_QCH,
    APBIF_INTCOMB_VGPIO2AP_1_QCH,
    APBIF_INTCOMB_VGPIO2PMU_QCH,
    APBIF_INTCOMB_VGPIO2PMU_1_QCH,
    APBIF_PMU_ALIVE_QCH,
    APM_DTA_QCH_APB,
    CLKMON_QCH,
    CMU_ALIVE_QCH,
    D_TZPC_ALIVE_QCH,
    ECU_ALIVE_QCH,
    GREBEINTEGRATION0_QCH_S_DBG,
    GREBEINTEGRATION0_QCH_S_GREBE,
    HW_SCANDUMP_CLKSTOP_CTRL_QCH,
    LH_AXI_SI_IP_ASYNCPMU_ALIVE_QCH,
    LH_INT_COMB_ALIVE_QCH,
    MCT_ALIVE_QCH,
    PMU_QCH_PMU,
    QCH_ADAPTER_AHB_BUSMATRIX_ALIVE_QCH,
    QCH_ADAPTER_DTA_XIU_DP_APM_QCH,
    QCH_ADAPTER_SCA_XIU_DP_APM_QCH,
    QCH_ADAPTER_XIU_DP_APM_QCH,
    RSTnSYNC_CLK_ALIVE_APM0_QCH,
    S2PC_ALIVE_QCH,
    SPC_ALIVE_QCH,
    SYSREG_ALIVE_QCH,
    TIMER0_ALIVE_QCH,
    VGEN_LITE_ALIVE_QCH,
    WDT_APM0_QCH,
    LH_AXI_MI_IP_ASYNCPMU_ALIVE_QCH,
    LH_AXI_SI_IPMU_ALIVE_QCH,
    SLH_AXI_SI_PMU_ALIVE_AUD_QCH,
    SLH_AXI_SI_PMU_ALIVE_CHUBVTS_QCH,
    SLH_AXI_SI_PMU_ALIVE_CMGP_QCH,
    SLH_AXI_SI_PMU_ALIVE_CSIS_QCH,
    SLH_AXI_SI_PMU_ALIVE_HSI0_QCH,
    SLH_AXI_SI_PMU_ALIVE_NPUMEM_QCH,
    SLH_AXI_SI_PMU_ALIVE_PSP_QCH,
    SLH_AXI_SI_PMU_ALIVE_UFD_QCH,
    SLH_AXI_SI_PMU_ALIVE_UNPU_QCH,
    ASYNCAHB_MI_APM0_QCH,
    RTC_0_QCH,
    RTC_S_QCH,
    SPMI_MASTER_PMIC_QCH_P,
    SPMI_MASTER_PMIC_QCH_P_2,
    SPMI_MASTER_PMIC_QCH_P_3,
    SPMI_MASTER_PMIC_QCH_P_4,
    SPMI_MASTER_PMIC_QCH_P_5,
    SPMI_MASTER_PMIC_QCH_S,
    SPMI_MASTER_PMIC_1_QCH_P,
    SPMI_MASTER_PMIC_1_QCH_P_2,
    SPMI_MASTER_PMIC_1_QCH_P_3,
    SPMI_MASTER_PMIC_1_QCH_P_4,
    SPMI_MASTER_PMIC_1_QCH_P_5,
    SPMI_MASTER_PMIC_1_QCH_S,
    RSTnSYNC_SR_CLK_SCA_DBGCORE_UART_QCH,
    APM1_ISRAMC_QCH,
    ASM_ISRAMC_QCH,
    ASYNCAHB_MI_APM1_QCH,
    ASYNCAHB_MI_ASM_QCH,
    AsyncInterrupt_APM1_QCH_AsyncInterrupt_APM1,
    AsyncInterrupt_ASM_QCH_AsyncInterrupt_ASM,
    BARAC_APM1_QCH,
    BARAC_ASM_QCH,
    BARAC_UNPU_REMAPPER_QCH,
    CMU_SCA_QCH,
    DBGCORE_UART_QCH,
    INTMEM_QCH,
    LH_AXI_MI_ID_DBGCORE_ALIVE_QCH,
    LH_AXI_MI_LD_UNPU_ALIVE_QCH,
    LH_AXI_SI_IP_ALIVE_QCH,
    LLCAID_AXI_D_NOCL2B_QCH,
    MAILBOX_APM0_APM1_QCH,
    MAILBOX_APM1_AP_QCH,
    MAILBOX_APM1_ASM_QCH,
    MAILBOX_APM1_DNC_QCH,
    MAILBOX_APM_AP_QCH,
    MAILBOX_APM_ASM_QCH,
    MAILBOX_APM_AUD_QCH,
    MAILBOX_APM_CHUB_QCH,
    MAILBOX_APM_CP_QCH,
    MAILBOX_APM_CP_1_QCH,
    MAILBOX_APM_GNSS_QCH,
    MAILBOX_APM_VTS_QCH,
    MAILBOX_AP_ASM_QCH,
    MAILBOX_AP_CHUB_QCH,
    MAILBOX_AP_CP_QCH,
    MAILBOX_AP_CP_S_QCH,
    MAILBOX_AP_DBGCORE_QCH,
    MAILBOX_AP_GNSS_QCH,
    MAILBOX_ASM_CP_QCH,
    MAILBOX_CP_CHUB_QCH,
    MAILBOX_CP_GNSS_QCH,
    MAILBOX_GNSS_CHUB_QCH,
    MAILBOX_SHARED_SRAM_QCH,
    QCH_ADAPTER_SCA_XIU_D_ALIVE_QCH,
    QCH_ADAPTER_SCA_XIU_INTMEM_ALIVE_QCH,
    RSTnSYNC_CLK_SCA_AON_QCH,
    RSTnSYNC_CLK_SCA_APM1_QCH,
    RSTnSYNC_CLK_SCA_ASM_QCH,
    RSTnSYNC_CLK_SCA_NOC_QCH,
    RSTnSYNC_SR_CLK_SCA_NOC_QCH,
    S2MPU_S0_ALIVE_QCH_S0,
    S2MPU_S0_PMMU0_ALIVE_QCH_S0,
    S2MPU_S0_PMMU1_ALIVE_QCH_S0,
    SLH_AXI_MI_LD_CHUBVTS_ALIVE_QCH,
    SLH_AXI_MI_LD_GNSS_ALIVE_QCH,
    SLH_AXI_MI_LP_MODEM_ALIVE_QCH,
    SLH_AXI_SI_LP_ALIVE_CHUBVTS_QCH,
    SLH_AXI_SI_LP_ALIVE_CMGP_QCH,
    SLH_AXI_SI_LP_ALIVE_UNPU_QCH,
    TIMER_APM1_QCH,
    TIMER_ASM_QCH,
    WDT_APM1_QCH,
    WDT_ASM_QCH,
    YAMIN_MCU_APM1_QCH_CLKIN,
    YAMIN_MCU_APM1_QCH_DBGCLK,
    YAMIN_MCU_ASM_QCH_CLKIN,
    YAMIN_MCU_ASM_QCH_DBGCLK,
    LH_AXI_SI_D_ALIVE_QCH,
    RSTnSYNC_CLK_SCA_NOC_LH_QCH,
    RSTnSYNC_SR_CLK_SCA_NOC_LH_QCH,
    SLH_AXI_MI_P_ALIVE_QCH,
    SLH_AXI_SI_LP_PPU_ALIVE_CPUCL0_QCH,
    QCH_ADAPTER_SCA_XIU_PMU_SCA_QCH,
    RSTnSYNC_CLK_SCA_NOC_PMULH_QCH,
    RSTnSYNC_CLK_SCA_PMULH_AON_QCH,
    RSTnSYNC_SR_CLK_SCA_NOC_PMULH_QCH,
    SLH_AXI_SI_PMU_ALIVE_NOCL0_QCH,
    SLH_AXI_SI_PMU_ALIVE_NOCL1_QCH,
    SLH_AXI_SI_PMU_ALIVE_NOCL2A_QCH,
    SLH_AXI_SI_PMU_ALIVE_NOCL2B_QCH,
    RSTnSYNC_CLK_SCA_OSCCLK_RCO_QCH,
    RTC_1_QCH,
    RSTnSYNC_SR_CLK_SCA_TIMER_APM1_QCH,
    RSTnSYNC_SR_CLK_SCA_TIMER_ASM_QCH,
    APBIF_S2D_DBGCORE_QCH,
    CMU_DBGCORE_QCH,
    D_TZPC_DBGCORE_QCH,
    GREBEINTEGRATION_DBGCORE_QCH_S_DBG,
    GREBEINTEGRATION_DBGCORE_QCH_S_GREBE,
    LH_AXI_MI_IG_CSSYS_ALIVE_QCH,
    LH_AXI_MI_IP_ALIVE_QCH,
    LH_AXI_SI_ID_DBGCORE_ALIVE_QCH,
    MDIS_DBGCORE_QCH,
    RSTnSYNC_CLK_DBGCORE_GREBE_QCH,
    SYSREG_DBGCORE_ALO_QCH,
    SYSREG_DBGCORE_CORE_PG_QCH,
    SYSREG_DBGCORE_PG_QCH,
    SYSREG_DBGCORE_RET_QCH,
    WDT_DBGCORE_QCH,
    SLH_AXI_SI_G_DBGCORE_ALIVE_CPUCL0_QCH,
    SLH_AXI_SI_G_SCAN2DRAM_ALIVE_MIF0_QCH,
    LH_AXI_SI_IG_CSSYS_ALIVE_QCH,
    ABOX_QCH_CNT,
    ABOX_QCH_ARAM,
    ABOX_QCH_C2A0,
    ABOX_QCH_C2A1,
    ABOX_QCH_CSTAT,
    ABOX_QCH_XCLK,
    BARAC_D_AUDCHUBVTS_QCH,
    CMU_AUD_QCH,
    DMAILBOX_AUD_QCH_ACLK,
    DMAILBOX_AUD_QCH_PCLK,
    DMIC_AUD0_QCH_PCLK,
    DMIC_AUD1_QCH_PCLK,
    DMIC_AUD2_QCH_PCLK,
    LH_ACEL_SI_D_AUD_QCH,
    LH_AXI_MI_ID_UDMA_AUD_QCH,
    LH_AXI_MI_IP_PERI_AUD_QCH,
    LLCAID_D_AUD_QCH,
    MAILBOX_AUD0_QCH,
    MAILBOX_AUD1_QCH,
    MAILBOX_AUD2_QCH,
    MAILBOX_AUD3_QCH,
    PPMU_AUD_QCH,
    SERIAL_LIF_QCH_ACLK,
    SERIAL_LIF_QCH_PCLK,
    SLH_AST_SI_G_PPMU_AUD_QCH,
    SLH_AXI_SI_LP_AUD_CHUBVTS_QCH,
    SYSMMU_S0_AUD_QCH_S0,
    SYSMMU_S0_PMMU0_AUD_QCH_S0,
    SYSREG_AUD_QCH,
    VGEN_LITE_AUD_QCH,
    WDT_AUD_QCH,
    LH_AXI_MI_ID_ACP_AUD_QCH,
    ABOX_QCH_CPU0,
    ABOX_QCH_CPU1,
    ABOX_QCH_CPU2,
    ABOX_QCH_L2,
    ABOX_QCH_NEON0,
    ABOX_QCH_NEON1,
    ABOX_QCH_NEON2,
    RSTnSYNC_CLK_AUD_CPU0_SW_RESET_QCH,
    RSTnSYNC_CLK_AUD_CPU1_SW_RESET_QCH,
    RSTnSYNC_CLK_AUD_CPU2_SW_RESET_QCH,
    RSTnSYNC_CLK_AUD_CPU_DBG_QCH,
    RSTnSYNC_CLK_AUD_CPU_PCLKDBG_QCH,
    ABOX_QCH_BCLK_DSIF,
    ABOX_QCH_ASTAT,
    ABOX_QCH_SPUM,
    ABOX_QCH_SPUS,
    LH_AXI_SI_ID_ACP_AUD_QCH,
    LH_AXI_SI_ID_UDMA_AUD_QCH,
    D_TZPC_AUD_QCH,
    ECU_AUD_QCH,
    LH_AXI_SI_IP_PERI_AUD_QCH,
    LH_INT_COMB_AUD_QCH,
    S2PC_AUD_QCH,
    SLH_AXI_MI_P_AUD_QCH,
    SPC_AUD_QCH,
    DMAILBOX_AUD_QCH_CCLK,
    ABOX_QCH_PCMC_CLK,
    SERIAL_LIF_QCH_BCLK,
    SERIAL_LIF_QCH_CCLK,
    ABOX_QCH_BCLK0,
    ABOX_QCH_BCLK1,
    ABOX_QCH_BCLK2,
    ABOX_QCH_BCLK3,
    ABOX_QCH_BCLK4,
    ABOX_QCH_BCLK5,
    ABOX_QCH_BCLK6,
    LH_ACEL_SI_D_BYRP_QCH,
    LH_AST_MI_OTF0_CSIS_BYRP_QCH,
    LH_AST_MI_OTF1_CSIS_BYRP_QCH,
    LH_AST_MI_OTF2_CSIS_BYRP_QCH,
    LH_AST_MI_OTF3_CSIS_BYRP_QCH,
    LH_AST_SI_OTF0_BYRP_RGBP_QCH,
    LH_AST_SI_OTF1_BYRP_RGBP_QCH,
    LH_AST_SI_OTF2_BYRP_RGBP_QCH,
    LH_AST_SI_OTF3_BYRP_RGBP_QCH,
    LLCAID_D_BYRP_QCH,
    SIPU_BYRP_QCH,
    SIPU_BYRP_QCH_VOTF_R0,
    SIPU_BYRP_QCH_VOTF_R1,
    SIPU_BYRP_QCH_VOTF_R2,
    SIPU_BYRP_QCH_VOTF_R3,
    SIPU_BYRP_QCH_VOTF_R4,
    SIPU_BYRP_QCH_VOTF_W0,
    SIPU_BYRP_QCH_VOTF_W1,
    SIPU_BYRP_QCH_VOTF_W2,
    SIPU_BYRP_QCH_VOTF_W3,
    SIPU_BYRP_QCH_VOTF_W4,
    SLH_AST_SI_G_PPMU_BYRP_QCH,
    SYSMMU_S0_BYRP_QCH_S0,
    SYSMMU_S0_PMMU0_BYRP_QCH_S0,
    VGEN_D0_BYRP_QCH,
    VGEN_D1_BYRP_QCH,
    VGEN_D2_BYRP_QCH,
    VGEN_D3_BYRP_QCH,
    VGEN_D4_BYRP_QCH,
    VGEN_D5_BYRP_QCH,
    VGEN_LITE_BYRP0_QCH,
    VGEN_LITE_BYRP1_QCH,
    CMU_BYRP_QCH,
    D_TZPC_BYRP_QCH,
    LH_INT_COMB_BYRP_QCH,
    PPMU_BYRP_QCH,
    S2PC_BYRP_QCH,
    SLH_AXI_MI_P_BYRP_QCH,
    SPC_BYRP_QCH,
    SYSREG_BYRP_QCH,
    I3C_CHUB0_QCH_P,
    I3C_CHUB0_QCH_S,
    RSTnSYNC_CLK_CHUB_I2C_QCH,
    RSTnSYNC_SR_CLK_CHUB_I2C_QCH,
    APBIF_CHUB_COMBINE_WAKEUP_SRC_QCH,
    BARAC_CHUB_QCH,
    CMU_CHUB_QCH,
    I2C_CHUB0_QCH,
    I2C_CHUB1_QCH,
    I2C_CHUB2_QCH,
    I2C_CHUB3_QCH,
    I2C_CHUB4_QCH,
    I2C_CHUB5_QCH,
    LH_AXI_MI_IP_VC2CHUB_CHUBVTS_QCH,
    LH_AXI_SI_ID_CHUB2VC_CHUBVTS_QCH,
    MAILBOX_CHUB_ABOX_QCH,
    MAILBOX_CHUB_UNPU_QCH,
    PWM_CHUB_QCH,
    RSTnSYNC_CLK_CHUB_NOC_QCH,
    RSTnSYNC_CLK_CHUB_SYSRESETn_QCH,
    RSTnSYNC_SR_CLK_CHUB_NOC_QCH,
    SERIAL_LIF_US_PROX_CHUB_QCH_ACLK,
    SERIAL_LIF_US_PROX_CHUB_QCH_PCLK,
    SPI_I2C_CHUB0_QCH,
    SPI_I2C_CHUB1_QCH,
    SYSREG_CHUB_QCH,
    SYSREG_COMBINE_CHUB2AP_QCH,
    SYSREG_COMBINE_CHUB2APM_QCH,
    TIMER_CHUB_QCH,
    USI_CHUB0_QCH,
    USI_CHUB1_QCH,
    USI_CHUB2_QCH,
    USI_CHUB3_QCH,
    WDT_CHUB_QCH,
    RSTnSYNC_CLK_CHUB_OSCCLK_QCH,
    RSTnSYNC_CLK_CHUB_RESETn_CPU_QCH,
    RSTnSYNC_SR_CLK_CHUB_OSCCLK_QCH,
    RSTnSYNC_SR_CLK_CHUB_SERIAL_LIF_US_PROX_QCH,
    SERIAL_LIF_US_PROX_CHUB_QCH_TX_BCLK,
    RSTnSYNC_SR_CLK_CHUB_SERIAL_LIF_US_PROX_CORE_QCH,
    SERIAL_LIF_US_PROX_CHUB_QCH_CCLK,
    RSTnSYNC_SR_CLK_CHUB_SPI_I2C0_QCH,
    RSTnSYNC_SR_CLK_CHUB_SPI_I2C1_QCH,
    RSTnSYNC_SR_CLK_CHUB_SPI_MS_CTRL_QCH,
    SPI_MULTI_SLV_Q_CTRL_CHUB_QCH,
    RSTnSYNC_SR_CLK_CHUB_TIMER_QCH,
    RSTnSYNC_SR_CLK_CHUB_USI0_QCH,
    RSTnSYNC_SR_CLK_CHUB_USI1_QCH,
    RSTnSYNC_SR_CLK_CHUB_USI2_QCH,
    RSTnSYNC_SR_CLK_CHUB_USI3_QCH,
    AsyncInterrupt_QCH_AsyncInterrupt_VT,
    RSTnSYNC_CLK_CHUB_YAMIN_QCH,
    RSTnSYNC_SR_CLK_CHUB_YAMIN_QCH,
    YAMIN_MCU_CHUB_QCH_CLKIN,
    YAMIN_MCU_CHUB_QCH_DBGCLK,
    BARAC_VTS_QCH,
    CMU_VTS_QCH,
    DMIC_IF0_QCH_PCLK,
    DMIC_IF1_QCH_PCLK,
    DMIC_IF2_QCH_PCLK,
    GPIO_VTS_QCH,
    INTMEM_CODE_QCH,
    INTMEM_DATA0_QCH,
    INTMEM_DATA1_QCH,
    INTMEM_PCM_QCH,
    LH_AXI_MI_IP_VC2VTS_CHUBVTS_QCH,
    LH_AXI_SI_ID_VTS2VC_CHUBVTS_QCH,
    MAILBOX_ABOX_VTS_QCH,
    MAILBOX_AP_VTS_QCH,
    MAILBOX_UNPU_VTS_QCH,
    SERIAL_LIF_US_PROX_VTS_QCH_ACLK,
    SERIAL_LIF_US_PROX_VTS_QCH_PCLK,
    SERIAL_LIF_VT_QCH_ACLK,
    SERIAL_LIF_VT_QCH_PCLK,
    SYSREG_VTS_QCH,
    TIMER_VTS_QCH,
    WDT_VTS_QCH,
    SERIAL_LIF_VT_QCH_BCLK,
    SERIAL_LIF_VT_QCH_CCLK,
    SERIAL_LIF_US_PROX_VTS_QCH_RX_BCLK,
    SERIAL_LIF_US_PROX_VTS_QCH_CCLK,
    AsyncInterrupt_VTS_QCH_AsyncInterrupt_VT,
    YAMIN_MCU_VTS_QCH_CLKIN,
    YAMIN_MCU_VTS_QCH_DBGCLK,
    DMAILBOX_CHUBVTS_QCH_CCLK,
    I3C_CHUB1_QCH_P,
    I3C_CHUB1_QCH_S,
    I3C_CHUB2_QCH_P,
    I3C_CHUB2_QCH_S,
    APBIF_GPIO_CHUBVTS_QCH,
    APB_SEMA_DMAILBOX_QCH,
    APB_SEMA_I3C1_QCH,
    APB_SEMA_I3C2_QCH,
    APB_SEMA_PDMA_QCH,
    BARAC_LD_CHUBVTS_QCH,
    CMU_CHUBVTS_QCH,
    DMAILBOX_CHUBVTS_QCH_ACLK,
    DMAILBOX_CHUBVTS_QCH_PCLK,
    D_TZPC_CHUBVTS_QCH,
    ECU_CHUBVTS_QCH,
    LH_AXI_MI_ID_CHUB2VC_CHUBVTS_QCH,
    LH_AXI_MI_ID_VTS2VC_CHUBVTS_QCH,
    LH_AXI_SI_IP_VC2CHUB_CHUBVTS_QCH,
    LH_AXI_SI_IP_VC2VTS_CHUBVTS_QCH,
    LH_INT_COMB_CHUBVTS_QCH,
    MAILBOX_CHUB_VTS_QCH,
    PDMA_CHUBVTS_QCH,
    QCH_ADAPTER_CHUBVTS_QCH,
    S2PC_CHUBVTS_QCH,
    SLH_AXI_MI_LP_ALIVE_CHUBVTS_QCH,
    SLH_AXI_MI_LP_AUD_CHUBVTS_QCH,
    SLH_AXI_SI_LD_CHUBVTS_ALIVE_QCH,
    SPC_CHUBVTS_QCH,
    SYSREG_CHUBVTS_QCH,
    VGEN_LITE_CHUBVTS_QCH,
    CHUB_RTC_QCH,
    APBIF_GPIO_CMGP_QCH,
    CMU_CMGP_QCH,
    D_TZPC_CMGP_QCH,
    I2C_CMGP2_QCH,
    I2C_CMGP3_QCH,
    I2C_CMGP4_QCH,
    I2C_CMGP5_QCH,
    I2C_CMGP6_QCH,
    LH_INT_COMB_CMGP_QCH,
    SLH_AXI_MI_LP_ALIVE_CMGP_QCH,
    SLH_AXI_SI_LP_CMGP_UFD_QCH,
    SPC_CMGP_QCH,
    SPI_I2C_CMGP0_QCH,
    SPI_I2C_CMGP1_QCH,
    SYSREG_CMGP_QCH,
    SYSREG_CMGP2APM_QCH,
    SYSREG_CMGP2CHUB_QCH,
    SYSREG_CMGP2CP_QCH,
    SYSREG_CMGP2GNSS_QCH,
    SYSREG_CMGP2PMU_AP_QCH,
    USI_CMGP0_QCH,
    USI_CMGP1_QCH,
    USI_CMGP2_QCH,
    USI_CMGP3_QCH,
    USI_CMGP4_QCH,
    USI_CMGP5_QCH,
    USI_CMGP6_QCH,
    SPI_MULTI_SLV_Q_CTRL_CMGP_QCH,
    CLUSTER0_QCH_COMPLEX0,
    CLUSTER0_QCH_CORE0,
    CLUSTER0_QCH_CORE1,
    CMU_CPUCL0_QCH,
    LH_ATB_MI_IT_CLUSTER0_CPUCL0_QCH,
    LH_ATB_MI_IT_DDCBIG_CPUCL0_QCH,
    LH_ATB_MI_IT_DDCMID0_CPUCL0_QCH,
    LH_ATB_MI_IT_DDCMID1_CPUCL0_QCH,
    LH_ATB_MI_IT_DDCMID2_CPUCL0_QCH,
    LH_ATB_MI_IT_DDCMID3_CPUCL0_QCH,
    LH_ATB_MI_IT_DDCMID4_CPUCL0_QCH,
    LH_ATB_MI_IT_DDCMID5_CPUCL0_QCH,
    LH_ATB_MI_IT_DDCMID6_CPUCL0_QCH,
    LH_ATB_MI_LD_DNC_CPUCL0_QCH,
    LH_ATB_MI_T_BDU_CPUCL0_QCH,
    LH_ATB_MI_T_DDCG3D_G3D_CPUCL0_QCH,
    LH_ATB_MI_T_PPMU_CPUCL0_QCH,
    LH_AXI_MI_IG_STM_CPUCL0_QCH,
    LH_AXI_SI_G_ETR_CPUCL0_QCH,
    LLCAID_G_ETR_QCH,
    CSSYS_QCH,
    LH_AXI_SI_G_CSSYS_CPUCL0_QCH,
    SECJTAG_QCH,
    HTU_CPUCL0_QCH_CLK,
    HTU_CPUCL1_0_QCH_CLK,
    HTU_CPUCL1_1_QCH_CLK,
    HTU_CPUCL1_2_QCH_CLK,
    HTU_CPUCL1_3_QCH_CLK,
    HTU_CPUCL1_4_QCH_CLK,
    HTU_CPUCL2_0_QCH_CLK,
    HTU_CPUCL2_1_QCH_CLK,
    HTU_CPUCL3_QCH_CLK,
    BPS_CPUCL0_QCH,
    BUSIF_DDC_CPUCL1_QCH,
    BUSIF_DDC_CPUCL2_QCH,
    BUSIF_DDC_CPUCL3_QCH,
    CFM_CPUCL0_QCH,
    CMU_CPUCL0_GLB_QCH,
    DLDO_SM_MID0_QCH,
    DLDO_SM_MID1_QCH,
    DLDO_SM_MID2_QCH,
    DLDO_SM_MID3_QCH,
    DLDO_SM_MID4_QCH,
    DLDO_SM_MID5_QCH,
    DLDO_SM_MID6_QCH,
    D_TZPC_CPUCL0_QCH,
    ECU_CPUCL0_0_QCH,
    ECU_CPUCL0_1_QCH,
    HTU_CPUCL0_QCH_PCLK,
    HTU_CPUCL1_0_QCH_PCLK,
    HTU_CPUCL1_1_QCH_PCLK,
    HTU_CPUCL1_2_QCH_PCLK,
    HTU_CPUCL1_3_QCH_PCLK,
    HTU_CPUCL1_4_QCH_PCLK,
    HTU_CPUCL2_0_QCH_PCLK,
    HTU_CPUCL2_1_QCH_PCLK,
    HTU_CPUCL3_QCH_PCLK,
    LH_AXI_SI_IG_STM_CPUCL0_QCH,
    LH_AXI_SI_IP_PAGECONF0_CPUCL0_QCH,
    LH_AXI_SI_IP_UTILITY_CPUCL0_QCH,
    LH_INT_COMB_CPUCL0_QCH,
    PMU_PCSM_QCH,
    PPC_INSTRRET_CLUSTER0_0_QCH,
    PPC_INSTRRET_CLUSTER0_1_QCH,
    PPC_INSTRRET_CLUSTER0_2_QCH,
    PPC_INSTRRET_CLUSTER0_3_QCH,
    PPC_INSTRRUN_CLUSTER0_0_QCH,
    PPC_INSTRRUN_CLUSTER0_1_QCH,
    PPC_INSTRRUN_CLUSTER0_2_QCH,
    PPC_INSTRRUN_CLUSTER0_3_QCH,
    PPMU_D_DECOMP_QCH,
    QOS_D0_CPUCL0_QCH,
    S2PC_CPUCL0_QCH,
    SLH_AXI_MI_G_DBGCORE_ALIVE_CPUCL0_QCH,
    SLH_AXI_MI_LP_PPU_ALIVE_CPUCL0_QCH,
    SLH_AXI_MI_P_CPUCL0_QCH,
    SPC_CPUCL0_QCH,
    SYSREG_CPUCL0_QCH,
    APB_ASYNC_P_VGEN_LITE_QCH,
    LH_ACEL_SI_ID_DECOMP_CPUCL0_QCH,
    LH_AXI_MI_IP_PAGECONF1_CPUCL0_QCH,
    PageDecomp_CPUCL0_QCH,
    S2MPU_S0_CPUCL0_QCH_S0,
    S2MPU_S0_PMMU0_CPUCL0_QCH_S0,
    VGEN_LITE_D_DECOMP_QCH,
    CLUSTER_QCH_CORE2,
    CLUSTER_QCH_CORE3,
    CLUSTER_QCH_CORE4,
    CLUSTER_QCH_CORE5,
    CLUSTER_QCH_CORE6,
    DDC_CPUCL1_ATB_0_QCH,
    DDC_CPUCL1_ATB_1_QCH,
    DDC_CPUCL1_ATB_2_QCH,
    DDC_CPUCL1_ATB_3_QCH,
    DDC_CPUCL1_ATB_4_QCH,
    DDC_QCH_GCPM_CORE2,
    DDC_QCH_GCPM_CORE3,
    DDC_QCH_GCPM_CORE4,
    DDC_QCH_GCPM_CORE5,
    DDC_QCH_GCPM_CORE6,
    LH_ATB_SI_IT_DDCMID0_CPUCL0_QCH,
    LH_ATB_SI_IT_DDCMID1_CPUCL0_QCH,
    LH_ATB_SI_IT_DDCMID2_CPUCL0_QCH,
    LH_ATB_SI_IT_DDCMID3_CPUCL0_QCH,
    LH_ATB_SI_IT_DDCMID4_CPUCL0_QCH,
    CMU_CPUCL1_QCH,
    BUSIF_ADD_CPUCL1_QCH,
    RSTnSYNC_SR_CLK_CPUCL1_POWERIP_QCH,
    CLUSTER_QCH_CORE7,
    CLUSTER_QCH_CORE8,
    DDC_CPUCL2_ATB_0_QCH,
    DDC_CPUCL2_ATB_1_QCH,
    DDC_QCH_GCPM_CORE7,
    DDC_QCH_GCPM_CORE8,
    LH_ATB_SI_IT_DDCMID7_CPUCL0_QCH,
    LH_ATB_SI_IT_DDCMID8_CPUCL0_QCH,
    CMU_CPUCL2_QCH,
    BUSIF_ADD_CPUCL2_QCH,
    RSTnSYNC_SR_CLK_CPUCL2_POWERIP_QCH,
    CLUSTER_QCH_CORE10,
    DDC_CPUCL3_ATB_QCH,
    DDC_QCH_GCPM_CORE10,
    LH_ATB_SI_IT_DDCBIG_CPUCL0_QCH,
    CMU_CPUCL3_QCH,
    BUSIF_ADD_CPUCL3_QCH,
    RSTnSYNC_SR_CLK_CPUCL3_POWERIP_QCH,
    CLUSTER0_QCH_ATCLK,
    CLUSTER0_QCH_GIC,
    CLUSTER0_QCH_PCLK,
    CLUSTER0_QCH_PDBGCLK,
    CLUSTER0_QCH_PERIPHCLK,
    CLUSTER0_QCH_PPUCLK,
    CLUSTER0_QCH_SCLK,
    LH_AXI_MI_IP_PAGECONF0_CPUCL0_QCH,
    LH_AXI_SI_IP_PAGECONF1_CPUCL0_QCH,
    SLH_AXI_SI_P_PERIM_CPUCL0_QCH,
    LH_ATB_SI_IT_CLUSTER0_CPUCL0_QCH,
    LH_AST_MI_LD_IRI_PERIS_CPUCL0_QCH,
    LH_AST_SI_LD_ICC_CPUCL0_PERIS_QCH,
    CMU_DSU_QCH,
    LH_AXI_MI_IP_UTILITY_CPUCL0_QCH,
    LH_ACEL_MI_ID_DECOMP_CPUCL0_QCH,
    LH_CHI_SI_D0_CPUCL0_QCH,
    LH_CHI_SI_D1_CPUCL0_QCH,
    LH_CHI_SI_D2_CPUCL0_QCH,
    LH_CHI_SI_D3_CPUCL0_QCH,
    CSIS_PDP_QCH_CSIS_TOP,
    CSIS_PDP_QCH_DMA,
    CSIS_PDP_QCH_PDP,
    CSIS_PDP_QCH_PDP_VOTF,
    CSIS_PDP_QCH_VOTF0,
    CSIS_PDP_QCH_VOTF1,
    LH_ACEL_SI_D2_CSIS_QCH,
    LH_AST_SI_OTF0_CSIS_BYRP_QCH,
    LH_AST_SI_OTF1_CSIS_BYRP_QCH,
    LH_AST_SI_OTF2_CSIS_BYRP_QCH,
    LH_AST_SI_OTF3_CSIS_BYRP_QCH,
    LH_AXI_MI_ID_D21D2_CSIS_QCH,
    LH_AXI_SI_D0_CSIS_QCH,
    LH_AXI_SI_D1_CSIS_QCH,
    LLCAID_D0_CSIS_QCH,
    LLCAID_D1_CSIS_QCH,
    LLCAID_D2_CSIS_QCH,
    SIU_G_PPMU_CSIS_QCH,
    SLH_AST_SI_G_PPMU_CSIS_QCH,
    SYSMMU_S0_CSIS_QCH_S0,
    SYSMMU_S0_PMMU0_CSIS_QCH_S0,
    SYSMMU_S0_PMMU1_CSIS_QCH_S0,
    SYSMMU_S0_PMMU2_CSIS_QCH_S0,
    VGEN_D0_CSIS_QCH,
    VGEN_D1_CSIS_QCH,
    VGEN_D2_CSIS_QCH,
    VGEN_D3_CSIS_QCH,
    VGEN_D4_CSIS_QCH,
    VGEN_D5_CSIS_QCH,
    VGEN_D6_CSIS_QCH,
    VGEN_D7_CSIS_QCH,
    VGEN_LITE_D1_CSIS_QCH,
    CMU_CSIS_QCH,
    D_TZPC_CSIS_QCH,
    LH_AXI_SI_IP_P0OIS_CSIS_QCH,
    LH_AXI_SI_IP_P0P1_CSIS_QCH,
    LH_INT_COMB_CSIS_QCH,
    PPMU_D0_CSIS_QCH,
    PPMU_D1_CSIS_QCH,
    PPMU_D2_CSIS_QCH,
    QE_CSIS_WDMA0_QCH,
    QE_CSIS_WDMA1_QCH,
    QE_CSIS_WDMA2_QCH,
    QE_CSIS_WDMA3_QCH,
    QE_CSIS_WDMA4_QCH,
    QE_PDP_D0_QCH,
    QE_PDP_D1_QCH,
    QE_PDP_D2_QCH,
    S2PC_CSIS_QCH,
    SLH_AXI_MI_P_CSIS_QCH,
    SPC_CSIS_QCH,
    SYSREG_CSIS_QCH,
    LH_AXI_MI_IP_P0OIS_CSIS_QCH,
    OIS_MCU_TOP_QCH,
    RSTnSYNC_CLK_CSIS_OIS_MCU_CPU_SW_RESET_QCH,
    SLH_AXI_SI_LP_CSIS_PERIC2_QCH,
    LH_AST_MI_OTF0_RGBP_CSIS_QCH,
    LH_AST_MI_OTF1_RGBP_CSIS_QCH,
    LH_AST_MI_OTF2_RGBP_CSIS_QCH,
    LH_AST_MI_OTF3_RGBP_CSIS_QCH,
    LH_AST_SI_OTF0_CSIS_MLSC_QCH,
    LH_AST_SI_OTF1_CSIS_MLSC_QCH,
    LH_AST_SI_OTF2_CSIS_MLSC_QCH,
    LH_AST_SI_OTF3_CSIS_MLSC_QCH,
    LH_AXI_SI_ID_D21D2_CSIS_QCH,
    YUVSC_QCH,
    MIPI_PHY_LINK_WRAP_QCH_UFD,
    SLH_AST_SI_OTF_CSIS_UFD_QCH,
    CMU_AOCCSIS_QCH,
    D_TZPC_AOCCSIS_QCH,
    LH_AXI_MI_IP_P0P1_CSIS_QCH,
    LH_INT_COMB_CSIS_ALIVE_QCH,
    MIPI_PHY_LINK_WRAP_QCH_CSIS0,
    MIPI_PHY_LINK_WRAP_QCH_CSIS1,
    MIPI_PHY_LINK_WRAP_QCH_CSIS2,
    MIPI_PHY_LINK_WRAP_QCH_CSIS3,
    MIPI_PHY_LINK_WRAP_QCH_CSIS4,
    MIPI_PHY_LINK_WRAP_QCH_CSIS5,
    MIPI_PHY_LINK_WRAP_QCH_CSIS6,
    SLH_AXI_MI_LP_UFD_CSIS_QCH,
    SPC_AOCCSIS_QCH,
    SYSREG_AOCCSIS_QCH,
    DLFE_QCH,
    LH_AST_MI_OTF0_MTNR_DLFE_QCH,
    LH_AST_MI_OTF1_MTNR_DLFE_QCH,
    LH_AST_SI_OTF_DLFE_MTNR_QCH,
    LH_AXI_SI_LD_DLFE_MTNR_QCH,
    RSTnSYNC_CLK_DLFE_NOCD_QCH,
    RSTnSYNC_SR_CLK_DLFE_NOCD_QCH,
    VGEN_LITE_DLFE_QCH,
    CMU_DLFE_QCH,
    D_TZPC_DLFE_QCH,
    ECU_DLFE_QCH,
    LH_INT_COMB_DLFE_QCH,
    RSTnSYNC_CLK_DLFE_NOCP_QCH,
    RSTnSYNC_SR_CLK_DLFE_NOCP_QCH,
    S2PC_DLFE_QCH,
    SLH_AXI_MI_P_DLFE_QCH,
    SPC_DLFE_QCH,
    SYSREG_DLFE_QCH,
    IP_DNC_QCH,
    LH_AST_MI_LD_SRAMRDRSP0_NPUMEM_DNC_QCH,
    LH_AST_MI_LD_SRAMWRRSP0_NPUMEM_DNC_QCH,
    LH_AST_SI_LD_SRAMRDREQ0_DNC_NPUMEM_QCH,
    LH_AST_SI_LD_SRAMWRREQ0_DNC_NPUMEM_QCH,
    LH_ATB_SI_LD_DNC_CPUCL0_QCH,
    LH_AXI_MI_ID_IPDNC_DNC_QCH,
    LH_AXI_MI_LD_CTRL_DSP_DNC_QCH,
    LH_AXI_MI_LD_CTRL_GNPU0_DNC_QCH,
    LH_AXI_MI_LD_CTRL_GNPU1_DNC_QCH,
    LH_AXI_MI_LD_CTRL_SNPU0_DNC_QCH,
    LH_AXI_MI_LD_CTRL_SNPU1_DNC_QCH,
    LH_AXI_MI_LD_DRAM_DSP_DNC_QCH,
    LH_AXI_MI_LD_DRAM_GNPU0_DNC_QCH,
    LH_AXI_MI_LD_DRAM_GNPU1_DNC_QCH,
    LH_AXI_MI_LD_DRAM_SNPU0_DNC_QCH,
    LH_AXI_MI_LD_DRAM_SNPU1_DNC_QCH,
    LH_AXI_SI_LD_CTRL_DNC_DSP_QCH,
    LH_AXI_SI_LD_CTRL_DNC_GNPU0_QCH,
    LH_AXI_SI_LD_CTRL_DNC_GNPU1_QCH,
    LH_AXI_SI_LD_CTRL_DNC_SDMA_QCH,
    LH_AXI_SI_LD_CTRL_DNC_SNPU0_QCH,
    LH_AXI_SI_LD_CTRL_DNC_SNPU1_QCH,
    LH_AXI_SI_LD_MMU_DNC_SDMA_QCH,
    SLH_AXI_SI_LD_CTRL_DNC_NPUMEM_QCH,
    CMU_DNC_QCH,
    D_TZPC_DNC_QCH,
    LH_AXI_SI_ID_IPDNC_DNC_QCH,
    LH_INT_COMB_DNC_QCH,
    SLH_AXI_MI_P_DNC_QCH,
    SLH_AXI_SI_LP_DNC_DSP_QCH,
    SLH_AXI_SI_LP_DNC_GNPU0_QCH,
    SLH_AXI_SI_LP_DNC_GNPU1_QCH,
    SLH_AXI_SI_LP_DNC_NPUMEM_QCH,
    SLH_AXI_SI_LP_DNC_SDMA_QCH,
    SLH_AXI_SI_LP_DNC_SNPU0_QCH,
    SLH_AXI_SI_LP_DNC_SNPU1_QCH,
    SPC_DNC_QCH,
    SYSREG_DNC_QCH,
    DPUB_QCH_AIQE_0,
    DPUB_QCH_AIQE_1,
    DPUB_QCH_DECON,
    LH_AXI_SI_LD0_DPUB_DPUF0_QCH,
    LH_AXI_SI_LD1_DPUB_DPUF0_QCH,
    CMU_DPUB_QCH,
    D_TZPC_DPUB_QCH,
    ECU_DPUB_QCH,
    LH_INT_COMB_DPUB_QCH,
    S2PC_DPUB_QCH,
    SLH_AXI_MI_P_DPUB_QCH,
    SPC_DPUB_QCH,
    SYSREG_DPUB_QCH,
    DPUB_QCH_ALV_DSIM0,
    DPUB_QCH_ALV_DSIM1,
    DPUB_QCH_OSC_DSIM0,
    DPUB_QCH_OSC_DSIM1,
    DPUF0_QCH_DPUF,
    DPUF0_QCH_SRAMC,
    DPUF0_QCH_VOTF,
    LH_AXI_MI_LD0_DPUB_DPUF0_QCH,
    LH_AXI_MI_LD0_DPUF1_DPUF0_QCH,
    LH_AXI_MI_LD1_DPUB_DPUF0_QCH,
    LH_AXI_MI_LD1_DPUF1_DPUF0_QCH,
    LH_AXI_SI_D0_DPUF0_QCH,
    LH_AXI_SI_D1_DPUF0_QCH,
    LLCAID_D0_DPUF0_QCH,
    LLCAID_D1_DPUF0_QCH,
    RSTnSYNC_CLK_DPUF0_NOCD_QCH,
    RSTnSYNC_SR_CLK_DPUF0_NOCD_QCH,
    SIU_DPUF0_QCH,
    SLH_AST_SI_G_PPMU_DPUF0_QCH,
    SYSMMU_S0_DPUF_QCH_S0,
    SYSMMU_S0_PMMU0_DPUF_QCH_S0,
    SYSMMU_S0_PMMU1_DPUF_QCH_S0,
    SYSMMU_S0_PMMU2_DPUF_QCH_S0,
    SYSMMU_S0_PMMU3_DPUF_QCH_S0,
    SYSMMU_S0_PMMU4_DPUF_QCH_S0,
    SYSMMU_S0_PMMU5_DPUF_QCH_S0,
    CMU_DPUF0_QCH,
    D_TZPC_DPUF0_QCH,
    ECU_DPUF0_QCH,
    LH_INT_COMB_DPUF0_QCH,
    PPMU_D0_DPUB_QCH,
    PPMU_D0_DPUF0_QCH,
    PPMU_D0_DPUF1_QCH,
    PPMU_D1_DPUB_QCH,
    PPMU_D1_DPUF0_QCH,
    PPMU_D1_DPUF1_QCH,
    RSTnSYNC_CLK_DPUF0_NOCP_QCH,
    RSTnSYNC_SR_CLK_DPUF0_NOCP_QCH,
    S2PC_DPUF0_QCH,
    SLH_AXI_MI_P_DPUF0_QCH,
    SLH_AXI_SI_LP_DPUF0_DPUF1_QCH,
    SPC_DPUF0_QCH,
    SYSREG_DPUF0_QCH,
    DPUF1_QCH_DPUF,
    DPUF1_QCH_SRAMC,
    DPUF1_QCH_VOTF,
    LH_AXI_SI_LD0_DPUF1_DPUF0_QCH,
    LH_AXI_SI_LD1_DPUF1_DPUF0_QCH,
    RSTnSYNC_CLK_DPUF1_NOCD_QCH,
    RSTnSYNC_SR_CLK_DPUF1_NOCD_QCH,
    CMU_DPUF1_QCH,
    D_TZPC_DPUF1_QCH,
    ECU_DPUF1_QCH,
    LH_INT_COMB_DPUF1_QCH,
    RSTnSYNC_CLK_DPUF1_NOCP_QCH,
    RSTnSYNC_SR_CLK_DPUF1_NOCP_QCH,
    S2PC_DPUF1_QCH,
    SLH_AXI_MI_LP_DPUF0_DPUF1_QCH,
    SPC_DPUF1_QCH,
    SYSREG_DPUF1_QCH,
    IP_DSP_QCH,
    LH_AST_MI_LD_SRAMRDREQ0_NPUMEM_DSP_QCH,
    LH_AST_MI_LD_SRAMWRREQ0_NPUMEM_DSP_QCH,
    LH_AST_SI_LD_SRAMRDRESP0_DSP_NPUMEM_QCH,
    LH_AST_SI_LD_SRAMWRRESP0_DSP_NPUMEM_QCH,
    LH_AXI_MI_LD_CTRL_DNC_DSP_QCH,
    LH_AXI_SI_LD_CTRL_DSP_DNC_QCH,
    LH_AXI_SI_LD_DRAM_DSP_DNC_QCH,
    RSTnSYNC_CLK_DSP_NOCD_QCH,
    RSTnSYNC_SR_CLK_DSP_NOCD_QCH,
    CMU_DSP_QCH,
    D_TZPC_DSP_QCH,
    ECU_DSP_QCH,
    LH_INT_COMB_DSP_QCH,
    RSTnSYNC_CLK_DSP_NOCP_QCH,
    RSTnSYNC_SR_CLK_DSP_NOCP_QCH,
    SLH_AXI_MI_LP_DNC_DSP_QCH,
    SPC_DSP_QCH,
    SYSREG_DSP_QCH,
    HTU_G3D_QCH_CLK,
    RSTnSYNC_CLK_G3D_HTU_QCH,
    RSTnSYNC_SR_CLK_G3D_HTU_QCH,
    LH_ACEL_MI_ID0_G3D_QCH,
    LH_ACEL_MI_ID1_G3D_QCH,
    LH_ACEL_MI_ID2_G3D_QCH,
    LH_ACEL_MI_ID3_G3D_QCH,
    LH_ACEL_SI_D0_G3D_QCH,
    LH_ACEL_SI_D1_G3D_QCH,
    LH_ACEL_SI_D2_G3D_QCH,
    LH_ACEL_SI_D3_G3D_QCH,
    LH_AST_MI_ID0_PMMU4_G3D_QCH,
    LH_AST_SI_ID1_PMMU4_G3D_QCH,
    LLCAID_D_SMMU_G3D_QCH,
    RSTnSYNC_CLK_G3D_NOCD_QCH,
    RSTnSYNC_SR_CLK_G3D_NOCD_QCH,
    S2MPU_S0_G3D_QCH_S0,
    S2MPU_S0_PMMU0_G3D_QCH_S0,
    S2MPU_S0_PMMU1_G3D_QCH_S0,
    S2MPU_S0_PMMU2_G3D_QCH_S0,
    S2MPU_S0_PMMU3_G3D_QCH_S0,
    SIU_G_PPMU_G3D_QCH,
    SLH_AST_SI_G_PPMU_G3D_QCH,
    VGEN_D0_G3D_QCH,
    VGEN_D1_G3D_QCH,
    VGEN_D2_G3D_QCH,
    VGEN_D3_G3D_QCH,
    BUSIF_DDC_G3D_QCH,
	BG3D_PWRCTL_QCH,
    CFM_G3D_QCH,
    CMU_G3D_QCH,
    D_TZPC_G3D_QCH,
    ECU_G3D_QCH,
    HTU_G3D_QCH_PCLK,
    LH_AST_MI_ID1_PMMU4_G3D_QCH,
    LH_AST_SI_ID0_PMMU4_G3D_QCH,
    LH_AXI_SI_IP_G3D_QCH,
    LH_INT_COMB_G3D_QCH,
    PPMU_D0_G3D_QCH,
    PPMU_D1_G3D_QCH,
    PPMU_D2_G3D_QCH,
    PPMU_D3_G3D_QCH,
    RSTnSYNC_CLK_G3D_NOCP_QCH,
    RSTnSYNC_SR_CLK_G3D_NOCP_QCH,
    S2MPU_S0_PMMU4_G3D_QCH_S0,
    S2PC_G3D_QCH,
    SLH_AXI_MI_P0_G3D_QCH,
    SLH_AXI_MI_P1_G3D_QCH,
    SLH_AXI_SI_P_PERIM_G3D_QCH,
    SPC_G3D_QCH,
    SYSREG_G3D_QCH,
    TREX_D_G3D_QCH,
    VGEN_P_G3D_QCH,
    RSTnSYNC_CLK_G3D_OSCCLK_QCH,
    RSTnSYNC_SR_CLK_G3D_OSCCLK_QCH,
    ASB_G3D_QCH_LH0,
    ASB_G3D_QCH_LH1,
    ASB_G3D_QCH_LH2,
    ASB_G3D_QCH_LH3,
    ASB_G3D_QCH_LH_P,
    DDC_G3D_ATB_QCH,
    DDC_QCH_GCPM_G3D,
    LH_ATB_SI_T_DDCG3D_QCH,
    CMU_G3DCORE_QCH,
    RSTnSYNC_CLK_G3DCORE_NOCP_QCH,
    RSTnSYNC_SR_CLK_G3DCORE_NOCP_QCH,
    RSTnSYNC_CLK_G3DCORE_OSCCLK_QCH,
    RSTnSYNC_SR_CLK_G3DCORE_OSCCLK_QCH,
    DDC_QCH_GCPM_GNPU,
    IP_NPUCORE_QCH_CORE,
    LH_AST_MI_LD_SRAMRDRSP0_NPUMEM_GNPU_QCH,
    LH_AST_MI_LD_SRAMRDRSP1_NPUMEM_GNPU_QCH,
    LH_AST_MI_LD_SRAMRDRSP2_NPUMEM_GNPU_QCH,
    LH_AST_MI_LD_SRAMRDRSP3_NPUMEM_GNPU_QCH,
    LH_AST_MI_LD_SRAMRDRSP4_NPUMEM_GNPU_QCH,
    LH_AST_MI_LD_SRAMRDRSP5_NPUMEM_GNPU_QCH,
    LH_AST_MI_LD_SRAMRDRSP6_NPUMEM_GNPU_QCH,
    LH_AST_MI_LD_SRAMRDRSP7_NPUMEM_GNPU_QCH,
    LH_AST_MI_LD_SRAMWRRSP0_NPUMEM_GNPU_QCH,
    LH_AST_MI_LD_SRAMWRRSP1_NPUMEM_GNPU_QCH,
    LH_AST_MI_LD_SRAMWRRSP2_NPUMEM_GNPU_QCH,
    LH_AST_MI_LD_SRAMWRRSP3_NPUMEM_GNPU_QCH,
    LH_AST_MI_LD_SRAMWRRSP4_NPUMEM_GNPU_QCH,
    LH_AST_MI_LD_SRAMWRRSP5_NPUMEM_GNPU_QCH,
    LH_AST_MI_LD_SRAMWRRSP6_NPUMEM_GNPU_QCH,
    LH_AST_MI_LD_SRAMWRRSP7_NPUMEM_GNPU_QCH,
    LH_AST_SI_LD_SRAMRDREQ0_GNPU_NPUMEM_QCH,
    LH_AST_SI_LD_SRAMRDREQ1_GNPU_NPUMEM_QCH,
    LH_AST_SI_LD_SRAMRDREQ2_GNPU_NPUMEM_QCH,
    LH_AST_SI_LD_SRAMRDREQ3_GNPU_NPUMEM_QCH,
    LH_AST_SI_LD_SRAMRDREQ4_GNPU_NPUMEM_QCH,
    LH_AST_SI_LD_SRAMRDREQ5_GNPU_NPUMEM_QCH,
    LH_AST_SI_LD_SRAMRDREQ6_GNPU_NPUMEM_QCH,
    LH_AST_SI_LD_SRAMRDREQ7_GNPU_NPUMEM_QCH,
    LH_AST_SI_LD_SRAMWRREQ0_GNPU_NPUMEM_QCH,
    LH_AST_SI_LD_SRAMWRREQ1_GNPU_NPUMEM_QCH,
    LH_AST_SI_LD_SRAMWRREQ2_GNPU_NPUMEM_QCH,
    LH_AST_SI_LD_SRAMWRREQ3_GNPU_NPUMEM_QCH,
    LH_AST_SI_LD_SRAMWRREQ4_GNPU_NPUMEM_QCH,
    LH_AST_SI_LD_SRAMWRREQ5_GNPU_NPUMEM_QCH,
    LH_AST_SI_LD_SRAMWRREQ6_GNPU_NPUMEM_QCH,
    LH_AST_SI_LD_SRAMWRREQ7_GNPU_NPUMEM_QCH,
    LH_AXI_MI_LD_CTRL_DNC_GNPU_QCH,
    LH_AXI_SI_LD_CTRL_GNPU_DNC_QCH,
    LH_AXI_SI_LD_DRAM_GNPU_DNC_QCH,
    RSTnSYNC_SR_CLK_GNPU_NOCD_QCH,
    BUSIF_DDC_GNPU_QCH,
    CMU_GNPU_QCH,
    D_TZPC_GNPU_QCH,
    ECU_GNPU_QCH,
    HTU_GNPU_QCH_PCLK,
    LH_INT_COMB_GNPU_QCH,
    RSTnSYNC_CLK_GNPU_NOCP_QCH,
    RSTnSYNC_SR_CLK_GNPU_NOCP_QCH,
    SLH_AXI_MI_LP_DNC_GNPU_QCH,
    SPC_GNPU_QCH,
    SYSREG_GNPU_QCH,
    HTU_GNPU_QCH_CLK,
    RSTnSYNC_SR_CLK_GNPU_XMAA0_QCH,
    RSTnSYNC_SR_CLK_GNPU_XMAA1_QCH,
    DP_LINK_QCH_GTC_CLK,
    DP_LINK_QCH_OSC_CLK,
    DP_LINK_QCH_PCLK,
    USB32DRD_QCH_S_EUSBPHY,
    CMU_HSI0_QCH,
    D_TZPC_HSI0_QCH,
    ECU_HSI0_QCH,
    LH_INT_COMB_HSI0_QCH,
    LLCAID_D_HSI0_QCH,
    PPMU_HSI0_BUS1_QCH,
    S2MPU_HSI0_S0_QCH_S0,
    S2MPU_HSI0_S0_PMMU0_QCH_S0,
    S2PC_HSI0_QCH,
    SLH_ACEL_SI_D_HSI0_QCH,
    SLH_AST_SI_G_PPMU_HSI0_QCH,
    SLH_AXI_MI_P_HSI0_QCH,
    SPC_HSI0_QCH,
    SYSREG_HSI0_QCH,
    USB32DRD_QCH_S_CTRL,
    USB32DRD_QCH_S_EUSBCTL,
    USB32DRD_QCH_S_LINK,
    USB32DRD_QCH_S_SUBCTRL,
    USB32DRD_QCH_S_TCA,
    VGEN_LITE_HSI0_QCH,
    CMU_HSI1_QCH,
    D_TZPC_HSI1_QCH,
    ECU_HSI1_QCH,
    GPIO_HSI1_QCH,
    LH_ACEL_SI_D_HSI1_QCH,
    LH_AXI_MI_ID_PCIEGEN40MSTR_HSI1_QCH,
    LH_AXI_SI_IP_PCIEGEN40DBISLV_HSI1_QCH,
    LH_INT_COMB_HSI1_QCH,
    LLCAID_D_HSI1_QCH,
    PPMU_HSI1_QCH,
    S2PC_HSI1_QCH,
    SLH_AST_SI_G_PPMU_HSI1_QCH,
    SLH_AXI_MI_P_HSI1_QCH,
    SPC_HSI1_QCH,
    SYSMMU_S0_HSI1_QCH_S0,
    SYSMMU_S0_PMMU0_HSI1_QCH_S0,
    SYSREG_HSI1_QCH,
    LH_AXI_MI_IP_PCIEGEN40DBISLV_HSI1_QCH,
    LH_AXI_SI_ID_PCIEGEN40MSTR_HSI1_QCH,
    PCIE_GEN4_2L_0_QCH_APB,
    PCIE_GEN4_2L_0_QCH_AXI,
    PCIE_GEN4_2L_0_QCH_DBI,
    PCIE_GEN4_2L_0_QCH_FPHYCON,
    PCIE_GEN4_2L_0_QCH_PCS_APB,
    PCIE_GEN4_2L_0_QCH_PMA_IF,
    PCIE_GEN4_2L_0_QCH_SRAM_APB,
    PCIE_GEN4_2L_0_QCH_UDBG_APB,
    PCIE_IA__DEBUG_0_QCH,
    PCIE_IA__DEBUG_1_QCH,
    PCIE_IA__DEBUG_2_QCH,
    VGEN_LITE_HSI1_QCH,
    ICPU_QCH_CPU0,
    ICPU_QCH_CPU1,
    ICPU_QCH_CPU_SI,
    ICPU_QCH_L2,
    ICPU_QCH_NEON0,
    ICPU_QCH_NEON1,
    RSTnSYNC_CLK_ICPU_CORE0_QCH,
    RSTnSYNC_CLK_ICPU_CORE1_QCH,
    RSTnSYNC_CLK_ICPU_CPUP1_QCH,
    RSTnSYNC_CLK_ICPU_CPUPO_QCH,
    RSTnSYNC_CLK_ICPU_CPU_DBG_QCH,
    ICPU_QCH_PERI,
    ICPU_QCH_PERI_MI,
    LH_AXI_MI_IP_ICPU_QCH,
    LH_AXI_SI_ID_ICPU_QCH,
    ICTRL_QCH,
    LH_ACEL_SI_D_ICPU_QCH,
    LH_AXI_MI_ID_ICPU_QCH,
    LLCAID_D_ICPU_QCH,
    SLH_AST_SI_G_PPMU_ICPU_QCH,
    SYSMMU_S0_ICPU_QCH_S0,
    SYSMMU_S0_PMMU0_ICPU_QCH_S0,
    VGEN_LITE_ICPU_QCH,
    CMU_ICPU_QCH,
    D_TZPC_ICPU_QCH,
    LH_AXI_SI_IP_ICPU_QCH,
    LH_INT_COMB_ICPU_QCH,
    PPMU_D_ICPU_QCH,
    S2PC_ICPU_QCH,
    SLH_AXI_MI_P_ICPU_QCH,
    SPC_ICPU_QCH,
    SYSREG_ICPU_QCH,
    DOF_QCH_0,
    LH_ACEL_SI_D_DOF_QCH,
    LLCAID_D_DOF_QCH,
    SLH_AST_SI_G_PPMU_DOF_QCH,
    SYSMMU_S0_DOF_QCH_S0,
    SYSMMU_S0_PMMU0_DOF_QCH_S0,
    VGEN_LITE_D_DOF_QCH,
    CMU_DOF_QCH,
    D_TZPC_DOF_QCH,
    LH_INT_COMB_DOF_QCH,
    PPMU_D_DOF_QCH,
    S2PC_DOF_QCH,
    SLH_AXI_MI_P_DOF_QCH,
    SPC_DOF_QCH,
    SYSREG_DOF_QCH,
    GDC_M_QCH,
    GDC_M_QCH_C2_M,
    GDC_M_QCH_C2_S,
    GDC_O_QCH,
    GDC_O_QCH_C2_M,
    GDC_O_QCH_C2_S,
    LH_ACEL_SI_D0_LME_QCH,
    LH_ACEL_SI_D1_LME_QCH,
    LH_ACEL_SI_D2_LME_QCH,
    LH_AST_SI_OTF0_LME_MFC_QCH,
    LH_AST_SI_OTF1_LME_MFC_QCH,
    LLCAID_D0_LME_QCH,
    LLCAID_D1_LME_QCH,
    LLCAID_D2_LME_QCH,
    LME_QCH_0,
    SIU_G_PPMU_LME_QCH,
    SLH_AST_SI_G_PPMU_LME_QCH,
    SYSMMU_S0_LME_QCH_S0,
    SYSMMU_S0_PMMU0_LME_QCH_S0,
    SYSMMU_S0_PMMU1_LME_QCH_S0,
    SYSMMU_S0_PMMU2_LME_QCH_S0,
    VGEN_LITE_LME0_QCH,
    VGEN_LITE_LME1_QCH,
    VGEN_LITE_LME2_QCH,
    CMU_LME_QCH,
    D_TZPC_LME_QCH,
    LH_INT_COMB_LME_QCH,
    PPMU_D0_LME_QCH,
    PPMU_D1_LME_QCH,
    PPMU_D2_LME_QCH,
    S2PC_LME_QCH,
    SLH_AXI_MI_P_LME_QCH,
    SPC_LME_QCH,
    SYSREG_LME_QCH,
    JPEG0_QCH,
    JPEG1_QCH,
    LH_AXI_SI_ID_JPEG0_M2M_QCH,
    LH_AXI_SI_ID_JPEG1_M2M_QCH,
    FRC_MC_QCH,
    JSQZ_QCH,
    LH_AXI_SI_ID_FRCMC_M2M_QCH,
    LH_AXI_SI_ID_JSQZ_M2M_QCH,
    FG_QCH,
    LH_ACEL_SI_D0_M2M_QCH,
    LH_AXI_MI_ID_FRCMC_M2M_QCH,
    LH_AXI_MI_ID_JPEG0_M2M_QCH,
    LH_AXI_MI_ID_JPEG1_M2M_QCH,
    LH_AXI_MI_ID_JSQZ_M2M_QCH,
    LH_AXI_SI_D1_M2M_QCH,
    LLCAID_D0_M2M_QCH,
    LLCAID_D1_M2M_QCH,
    M2M_QCH,
    M2M_QCH_VOTF,
    RSTnSYNC_SR_CLK_M2M_NOCD_SW_RESET_QCH,
    SIU_G_PPMU_M2M_QCH,
    SLH_AST_SI_G_PPMU_M2M_QCH,
    SYSMMU_S0_M2M_QCH_S0,
    SYSMMU_S0_PMMU0_M2M_QCH_S0,
    SYSMMU_S1_M2M_QCH_S0,
    SYSMMU_S1_PMMU0_M2M_QCH_S0,
    VGEN_LITE_M2M_QCH,
    CMU_M2M_QCH,
    D_TZPC_M2M_QCH,
    LH_INT_COMB_M2M_QCH,
    PPMU_D0_M2M_QCH,
    PPMU_D1_M2M_QCH,
    PPMU_D2_M2M_QCH,
    QE_FRC_MC_QCH,
    QE_JPEG0_QCH,
    QE_JPEG1_QCH,
    QE_JSQZ_QCH,
    QE_M2M_QCH,
    QE_VOTF_QCH,
    S2PC_M2M_QCH,
    SLH_AXI_MI_P_M2M_QCH,
    SPC_M2M_QCH,
    SYSREG_M2M_QCH,
    LH_ACEL_SI_D0_MCSC_QCH,
    LH_ACEL_SI_D1_MCSC_QCH,
    LH_ACEL_SI_D2_MCSC_QCH,
    LH_AST_MI_OTF_MSNR_MCSC_QCH,
    LH_AST_MI_OTF_MTNR_MCSC_QCH,
    LH_AST_MI_OTF_YUVP_MCSC_QCH,
    LH_AST_SI_OTF_MCSC_MSNR_QCH,
    LLCAID_D0_MCSC_QCH,
    LLCAID_D1_MCSC_QCH,
    LLCAID_D2_MCSC_QCH,
    MCSC_QCH,
    MCSC_QCH_C2W,
    MTNR0_QCH,
    RSTnSYNC_CLK_MCSC_NOCD_QCH,
    RSTnSYNC_SR_CLK_MCSC_NOCD_QCH,
    SIU_G_PPMU_MCSC_QCH,
    SLH_AST_SI_G_PPMU_MCSC_QCH,
    SYSMMU_S0_MCSC_QCH_S0,
    SYSMMU_S0_PMMU0_MCSC_QCH_S0,
    SYSMMU_S0_PMMU1_MCSC_QCH_S0,
    SYSMMU_S0_PMMU2_MCSC_QCH_S0,
    VGEN_LITE_MCSC0_QCH,
    VGEN_LITE_MCSC1_QCH,
    VGEN_LITE_MCSC2_QCH,
    CMU_MCSC_QCH,
    D_TZPC_MCSC_QCH,
    ECU_MCSC_QCH,
    LH_INT_COMB_MCSC_QCH,
    PPMU_D0_MCSC_QCH,
    PPMU_D1_MCSC_QCH,
    PPMU_D2_MCSC_QCH,
    RSTnSYNC_CLK_MCSC_NOCP_QCH,
    RSTnSYNC_SR_CLK_MCSC_NOCP_QCH,
    S2PC_MCSC_QCH,
    SLH_AXI_MI_P_MCSC_QCH,
    SPC_MCSC_QCH,
    SYSREG_MCSC_QCH,
    LH_AST_MI_OTF0_LME_MFC_QCH,
    LH_AST_MI_OTF0_MFD_MFC_QCH,
    LH_AST_MI_OTF1_LME_MFC_QCH,
    LH_AST_MI_OTF1_MFD_MFC_QCH,
    LH_AST_MI_OTF2_MFD_MFC_QCH,
    LH_AST_MI_OTF3_MFD_MFC_QCH,
    LH_AST_SI_OTF0_MFC_MFD_QCH,
    LH_AST_SI_OTF1_MFC_MFD_QCH,
    LH_AST_SI_OTF2_MFC_MFD_QCH,
    LH_AST_SI_OTF3_MFC_MFD_QCH,
    LH_ATB_SI_IT_MFC_QCH,
    LH_AXI_MI_ID_MFC_QCH,
    LH_AXI_SI_D0_MFC_QCH,
    LH_AXI_SI_D1_MFC_QCH,
    LLCAID_D0_MFC_QCH,
    LLCAID_D1_MFC_QCH,
    MFC_QCH,
    MFC_QCH_VOTF,
    RSTnSYNC_CLK_MFC_NOCD_MFC_QCH,
    RSTnSYNC_CLK_MFC_NOCD_MFC_SW_RESET_QCH,
    RSTnSYNC_SR_CLK_MFC_NOCD_LH_AST_MI_OTF0_MFC_SW_RESET_QCH,
    RSTnSYNC_SR_CLK_MFC_NOCD_LH_AST_MI_OTF1_MFC_SW_RESET_QCH,
    RSTnSYNC_SR_CLK_MFC_NOCD_LH_AST_MI_OTF2_MFC_SW_RESET_QCH,
    RSTnSYNC_SR_CLK_MFC_NOCD_LH_AST_MI_OTF3_MFC_SW_RESET_QCH,
    RSTnSYNC_SR_CLK_MFC_NOCD_LH_AST_SI_OTF0_MFC_SW_RESET_QCH,
    RSTnSYNC_SR_CLK_MFC_NOCD_LH_AST_SI_OTF1_MFC_SW_RESET_QCH,
    RSTnSYNC_SR_CLK_MFC_NOCD_LH_AST_SI_OTF2_MFC_SW_RESET_QCH,
    RSTnSYNC_SR_CLK_MFC_NOCD_LH_AST_SI_OTF3_MFC_SW_RESET_QCH,
    RSTnSYNC_SR_CLK_MFC_NOCD_LH_ATB_MFC_SI_SW_RESET_QCH,
    RSTnSYNC_SR_CLK_MFC_NOCD_MFC_QCH,
    RSTnSYNC_SR_CLK_MFC_NOCD_MFC_SW_RESET_QCH,
    SIU_G_PPMU_MFC_QCH,
    SLH_AST_SI_G_PPMU_MFC_QCH,
    SLH_AXI_SI_LP_MFC_MFD_QCH,
    SYSMMU_S0_MFC_QCH_S0,
    SYSMMU_S0_PMMU0_MFC_QCH_S0,
    SYSMMU_S0_PMMU1_MFC_QCH_S0,
    VGEN_LITE_MFC_QCH,
    LH_ATB_MI_IT_MFC_QCH,
    LH_AXI_SI_ID_MFC_QCH,
    RSTnSYNC_CLK_MFC_NOCD_WFD_QCH,
    RSTnSYNC_SR_CLK_MFC_NOCD_LH_ATB_MFC_MI_SW_RESET_QCH,
    RSTnSYNC_SR_CLK_MFC_NOCD_WFD_QCH,
    RSTnSYNC_SR_CLK_MFC_NOCD_WFD_SW_RESET_QCH,
    WFD_QCH,
    CMU_MFC_QCH,
    D_TZPC_MFC_QCH,
    ECU_MFC_QCH,
    LH_INT_COMB_MFC_QCH,
    PPMU_D0_MFC_QCH,
    PPMU_D1_MFC_QCH,
    PPMU_D2_MFC_QCH,
    RSTnSYNC_CLK_MFC_NOCP_QCH,
    RSTnSYNC_SR_CLK_MFC_NOCP_QCH,
    S2PC_MFC_QCH,
    SLH_AXI_MI_P_MFC_QCH,
    SPC_MFC_QCH,
    SYSREG_MFC_QCH,
    LH_AST_MI_OTF0_MFC_MFD_QCH,
    LH_AST_MI_OTF1_MFC_MFD_QCH,
    LH_AST_MI_OTF2_MFC_MFD_QCH,
    LH_AST_MI_OTF3_MFC_MFD_QCH,
    LH_AST_SI_OTF0_MFD_MFC_QCH,
    LH_AST_SI_OTF1_MFD_MFC_QCH,
    LH_AST_SI_OTF2_MFD_MFC_QCH,
    LH_AST_SI_OTF3_MFD_MFC_QCH,
    LH_AXI_SI_D0_MFD_QCH,
    LH_AXI_SI_D1_MFD_QCH,
    LLCAID_D0_MFD_QCH,
    LLCAID_D1_MFD_QCH,
    MFD_QCH,
    MFD_QCH_VOTF,
    RSTnSYNC_CLK_MFD_NOCD_MFD_QCH,
    RSTnSYNC_CLK_MFD_NOCD_MFD_SW_RESET_QCH,
    RSTnSYNC_SR_CLK_MFD_NOCD_LH_AST_MI_OTF0_MFD_SW_RESET_QCH,
    RSTnSYNC_SR_CLK_MFD_NOCD_LH_AST_MI_OTF1_MFD_SW_RESET_QCH,
    RSTnSYNC_SR_CLK_MFD_NOCD_LH_AST_MI_OTF2_MFD_SW_RESET_QCH,
    RSTnSYNC_SR_CLK_MFD_NOCD_LH_AST_MI_OTF3_MFD_SW_RESET_QCH,
    RSTnSYNC_SR_CLK_MFD_NOCD_LH_AST_SI_OTF0_MFD_SW_RESET_QCH,
    RSTnSYNC_SR_CLK_MFD_NOCD_LH_AST_SI_OTF1_MFD_SW_RESET_QCH,
    RSTnSYNC_SR_CLK_MFD_NOCD_LH_AST_SI_OTF2_MFD_SW_RESET_QCH,
    RSTnSYNC_SR_CLK_MFD_NOCD_LH_AST_SI_OTF3_MFD_SW_RESET_QCH,
    RSTnSYNC_SR_CLK_MFD_NOCD_MFD_QCH,
    RSTnSYNC_SR_CLK_MFD_NOCD_MFD_SW_RESET_QCH,
    SIU_G0_PPMU_MFD_QCH,
    SLH_AST_SI_G_PPMU_MFD_QCH,
    SLH_AXI_MI_LP_MFC_MFD_QCH,
    SYSMMU_S0_MFD_QCH_S0,
    SYSMMU_S0_PMMU0_MFD_QCH_S0,
    SYSMMU_S0_PMMU1_MFD_QCH_S0,
    VGEN_LITE_MFD_QCH,
    CMU_MFD_QCH,
    D_TZPC_MFD_QCH,
    ECU_MFD_QCH,
    LH_INT_COMB_MFD_QCH,
    PPMU_D0_MFD_QCH,
    PPMU_D1_MFD_QCH,
    RSTnSYNC_CLK_MFD_NOCP_QCH,
    RSTnSYNC_SR_CLK_MFD_NOCP_QCH,
    S2PC_MFD_QCH,
    SLH_AXI_MI_P_MFD_QCH,
    SPC_MFD_QCH,
    SYSREG_MFD_QCH,
    DMC_QCH,
    SLH_AST_SI_G_PPMU_MIF_QCH,
    CMU_MIF_QCH,
    D_TZPC_MIF_QCH,
    ECU_MIF_QCH,
    LH_INT_COMB_MIF_QCH,
    QCH_ADAPTER_DDRPHY_QCH,
    QCH_ADAPTER_DMC_QCH,
    QCH_ADAPTER_PPC_DEBUG_QCH,
    SLH_AXI_MI_P_MIF_QCH,
    SPC_MIF_QCH,
    SYSREG_MIF_QCH,
    SYSREG_PRIVATE_MIF_QCH,
    CMU_S2D_QCH,
    SLH_AXI_MI_G_SCAN2DRAM_ALIVE_MIF_QCH,
    LH_ACEL_SI_D0_MLSC_QCH,
    LH_ACEL_SI_D1_MLSC_QCH,
    LH_AST_MI_OTF0_CSIS_MLSC_QCH,
    LH_AST_MI_OTF1_CSIS_MLSC_QCH,
    LH_AST_MI_OTF2_CSIS_MLSC_QCH,
    LH_AST_MI_OTF3_CSIS_MLSC_QCH,
    LLCAID_D0_MLSC_QCH,
    LLCAID_D1_MLSC_QCH,
    MLSC_QCH,
    MLSC_QCH_VOTF_R0,
    MLSC_QCH_VOTF_W0,
    MLSC_QCH_VOTF_W1,
    MLSC_QCH_VOTF_W2,
    MLSC_QCH_VOTF_W3,
    SIU_G_PPMU_MLSC_QCH,
    SLH_AST_SI_G_PPMU_MLSC_QCH,
    SYSMMU_S0_MLSC_QCH_S0,
    SYSMMU_S0_PMMU0_MLSC_QCH_S0,
    SYSMMU_S0_PMMU1_MLSC_QCH_S0,
    VGEN_D0_MLSC_QCH,
    VGEN_D10_MLSC_QCH,
    VGEN_D11_MLSC_QCH,
    VGEN_D12_MLSC_QCH,
    VGEN_D13_MLSC_QCH,
    VGEN_D14_MLSC_QCH,
    VGEN_D15_MLSC_QCH,
    VGEN_D16_MLSC_QCH,
    VGEN_D17_MLSC_QCH,
    VGEN_D18_MLSC_QCH,
    VGEN_D19_MLSC_QCH,
    VGEN_D1_MLSC_QCH,
    VGEN_D2_MLSC_QCH,
    VGEN_D3_MLSC_QCH,
    VGEN_D4_MLSC_QCH,
    VGEN_D5_MLSC_QCH,
    VGEN_D6_MLSC_QCH,
    VGEN_D7_MLSC_QCH,
    VGEN_D8_MLSC_QCH,
    VGEN_D9_MLSC_QCH,
    VGEN_LITE_G_MLSC_QCH,
    CMU_MLSC_QCH,
    D_TZPC_MLSC_QCH,
    LH_INT_COMB_MLSC_QCH,
    PPMU_D0_MLSC_QCH,
    PPMU_D1_MLSC_QCH,
    S2PC_MLSC_QCH,
    SLH_AXI_MI_P_MLSC_QCH,
    SPC_MLSC_QCH,
    SYSREG_MLSC_QCH,
    LH_AST_MI_OTF0_MTNR_MSNR_QCH,
    LH_AST_MI_OTF1_MTNR_MSNR_QCH,
    LH_AST_MI_OTF2_MTNR_MSNR_QCH,
    LH_AST_MI_OTF3_MTNR_MSNR_QCH,
    LH_AST_MI_OTF_MCSC_MSNR_QCH,
    LH_AST_SI_OTF_MSNR_MCSC_QCH,
    LH_AST_SI_OTF_MSNR_YUVP_QCH,
    LH_AXI_SI_LD0_MSNR_YUVP_QCH,
    LH_AXI_SI_LD1_MSNR_YUVP_QCH,
    MSNR_QCH,
    RSTnSYNC_CLK_MSNR_NOCD_QCH,
    RSTnSYNC_SR_CLK_MSNR_NOCD_QCH,
    VGEN_LITE_MSNR_QCH,
    CMU_MSNR_QCH,
    D_TZPC_MSNR_QCH,
    ECU_MSNR_QCH,
    LH_INT_COMB_MSNR_QCH,
    RSTnSYNC_CLK_MSNR_NOCP_QCH,
    RSTnSYNC_SR_CLK_MSNR_NOCP_QCH,
    S2PC_MSNR_QCH,
    SLH_AXI_MI_P_MSNR_QCH,
    SPC_MSNR_QCH,
    SYSREG_MSNR_QCH,
    LH_ACEL_SI_D0_MTNR_QCH,
    LH_ACEL_SI_D1_MTNR_QCH,
    LH_AST_MI_OTF_DLFE_MTNR_QCH,
    LH_AST_SI_OTF0_MTNR_DLFE_QCH,
    LH_AST_SI_OTF0_MTNR_MSNR_QCH,
    LH_AST_SI_OTF1_MTNR_DLFE_QCH,
    LH_AST_SI_OTF1_MTNR_MSNR_QCH,
    LH_AST_SI_OTF2_MTNR_MSNR_QCH,
    LH_AST_SI_OTF3_MTNR_MSNR_QCH,
    LH_AST_SI_OTF_MTNR_MCSC_QCH,
    LH_AXI_MI_LD_DLFE_MTNR_QCH,
    LH_AXI_SI_D2_MTNR_QCH,
    LH_AXI_SI_D3_MTNR_QCH,
    LLCAID_D0_MTNR_QCH,
    LLCAID_D1_MTNR_QCH,
    LLCAID_D2_MTNR_QCH,
    LLCAID_D3_MTNR_QCH,
    MTNR_QCH,
    RSTnSYNC_CLK_MTNR_NOCD_QCH,
    RSTnSYNC_SR_CLK_MTNR_NOCD_QCH,
    SIU_G_PPMU_MTNR_QCH,
    SLH_AST_SI_G_PPMU_MTNR_QCH,
    SYSMMU_S0_MTNR_QCH_S0,
    SYSMMU_S0_PMMU0_MTNR_QCH_S0,
    SYSMMU_S0_PMMU1_MTNR_QCH_S0,
    SYSMMU_S0_PMMU2_MTNR_QCH_S0,
    SYSMMU_S0_PMMU3_MTNR_QCH_S0,
    VGEN_LITE_D0_MTNR_QCH,
    VGEN_LITE_D1_MTNR_QCH,
    VGEN_LITE_D2_MTNR_QCH,
    VGEN_LITE_D3_MTNR_QCH,
    VGEN_LITE_D4_MTNR_QCH,
    CMU_MTNR_QCH,
    D_TZPC_MTNR_QCH,
    ECU_MTNR_QCH,
    LH_INT_COMB_MTNR_QCH,
    PPMU_D0_MTNR_QCH,
    PPMU_D1_MTNR_QCH,
    PPMU_D2_MTNR_QCH,
    PPMU_D3_MTNR_QCH,
    RSTnSYNC_CLK_MTNR_NOCP_QCH,
    RSTnSYNC_SR_CLK_MTNR_NOCP_QCH,
    S2PC_MTNR_QCH,
    SLH_AXI_MI_P_MTNR_QCH,
    SPC_MTNR_QCH,
    SYSREG_MTNR_QCH,
    LH_ACEL_MI_D0_G3D_QCH,
    LH_ACEL_MI_D1_G3D_QCH,
    LH_ACEL_MI_D2_G3D_QCH,
    LH_ACEL_MI_D3_G3D_QCH,
    LH_AST_MI_G_NOCL1_NOCL0_QCH,
    LH_AST_MI_G_NOCL2A_NOCL0_QCH,
    LH_AST_MI_G_NOCL2B_NOCL0_QCH,
    LH_ATB_SI_T_BDU_CPUCL0_QCH,
    LH_CHI_MI_D0_CPUCL0_QCH,
    LH_CHI_MI_D1_CPUCL0_QCH,
    LH_CHI_MI_D2_CPUCL0_QCH,
    LH_CHI_MI_D3_CPUCL0_QCH,
    LH_TAXI_MI_D0_NOCL1_NOCL0_QCH,
    LH_TAXI_MI_D1_NOCL1_NOCL0_QCH,
    LH_TAXI_MI_D2_NOCL1_NOCL0_QCH,
    LH_TAXI_MI_D3_NOCL1_NOCL0_QCH,
    RSTnSYNC_CLK_NOCL0_NOCD_QCH,
    RSTnSYNC_SR_CLK_NOCL0_NOCD_QCH,
    SIU_G0_PPMU_NOCL0_QCH,
    SIU_G1_PPMU_NOCL0_QCH,
    SIU_G2_PPMU_NOCL0_QCH,
    SIU_G3_PPMU_NOCL0_QCH,
    SLH_AST_MI_G_PPMU_G3D_QCH,
    SLH_AST_MI_G_PPMU_MIF0_QCH,
    SLH_AST_MI_G_PPMU_MIF1_QCH,
    SLH_AST_MI_G_PPMU_MIF2_QCH,
    SLH_AST_MI_G_PPMU_MIF3_QCH,
    SLH_AST_SI_G_PPMU_NOCL0_NOCL1_QCH,
    BDU_QCH,
    CMU_NOCL0_QCH,
    D_TZPC_NOCL0_QCH,
    ECU_NOCL0_QCH,
    LH_INT_COMB_NOCL0_QCH,
    NOCIF_CMUTOPC_QCH,
    PPC_SCI_QCH,
    PPMU_D0_CPUCL0_QCH,
    PPMU_D0_IRPS_QCH,
    PPMU_D1_CPUCL0_QCH,
    PPMU_D1_IRPS_QCH,
    PPMU_D2_CPUCL0_QCH,
    PPMU_D2_IRPS_QCH,
    PPMU_D3_CPUCL0_QCH,
    PPMU_D3_IRPS_QCH,
    RSTnSYNC_CLK_NOCL0_NOCP_QCH,
    RSTnSYNC_SR_CLK_NOCL0_NOCP_QCH,
    SLH_AXI_MI_P_NOCL1_NOCL0_QCH,
    SPC_NOCL0_QCH,
    SYSREG_NOCL0_QCH,
    TREX_D_NOCL0_QCH,
    WOW_DVFS_D0_CPUCL0_QCH,
    WOW_DVFS_D0_G3D_QCH,
    WOW_DVFS_D0_IRPS_QCH,
    WOW_DVFS_D1_CPUCL0_QCH,
    WOW_DVFS_D1_IRPS_QCH,
    WOW_DVFS_D2_CPUCL0_QCH,
    WOW_DVFS_D2_IRPS_QCH,
    WOW_DVFS_D3_CPUCL0_QCH,
    WOW_DVFS_D3_IRPS_QCH,
    WOW_DVFS_D_MIF0_QCH,
    WOW_DVFS_D_MIF1_QCH,
    WOW_DVFS_D_MIF2_QCH,
    WOW_DVFS_D_MIF3_QCH,
    WOW_DVFS_NOCL0_QCH,
    CCI_QCH,
    RSTnSYNC_CLK_NOCL0_OSCCLK_QCH,
    RSTnSYNC_SR_CLK_NOCL0_OSCCLK_QCH,
    LH_ACEL_MI_D0_M2M_QCH,
    LH_ACEL_MI_D_AUD_QCH,
    LH_ACEL_MI_D_DOF_QCH,
    LH_ACEL_MI_D_ICPU_QCH,
    LH_ACEL_MI_D_PSP_QCH,
    LH_ACEL_MI_D_UFS_QCH,
    LH_AST_MI_IG_CPUCL0PPMU_NOCL1_QCH,
    LH_AST_MI_IG_DEBUGMUX_NOCL1_QCH,
    LH_AST_MI_IG_DP_NOCL1_QCH,
    LH_AST_MI_IG_G3DPPMU_NOCL1_QCH,
    LH_AST_SI_G_NOCL1_NOCL0_QCH,
    LH_ATB_SI_T_PPMU_CPUCL0_QCH,
    LH_AXI_MI_D0_DPUF0_QCH,
    LH_AXI_MI_D1_DPUF0_QCH,
    LH_AXI_MI_D1_M2M_QCH,
    LH_AXI_MI_D1_MODEM_QCH,
    LH_AXI_MI_D_PERIS_QCH,
    LH_AXI_MI_G_CSSYS_CPUCL0_QCH,
    LH_AXI_MI_G_ETR_CPUCL0_QCH,
    LH_AXI_SI_IDP_NOCL1_QCH,
    LH_TAXI_MI_D0_NOCL2A_NOCL1_QCH,
    LH_TAXI_MI_D0_NOCL2B_NOCL1_QCH,
    LH_TAXI_MI_D1_NOCL2A_NOCL1_QCH,
    LH_TAXI_MI_D1_NOCL2B_NOCL1_QCH,
    LH_TAXI_MI_D2_NOCL2A_NOCL1_QCH,
    LH_TAXI_MI_D2_NOCL2B_NOCL1_QCH,
    LH_TAXI_MI_D3_NOCL2A_NOCL1_QCH,
    LH_TAXI_MI_D3_NOCL2B_NOCL1_QCH,
    LH_TAXI_SI_D0_NOCL1_NOCL0_QCH,
    LH_TAXI_SI_D1_NOCL1_NOCL0_QCH,
    LH_TAXI_SI_D2_NOCL1_NOCL0_QCH,
    LH_TAXI_SI_D3_NOCL1_NOCL0_QCH,
    LLCAID_D0_MODEM_QCH,
    LLCAID_D1_MODEM_QCH,
    LLCAID_D2_MODEM_QCH,
    RSTnSYNC_CLK_NOCL1_NOCD_QCH,
    RSTnSYNC_SR_CLK_NOCL1_NOCD_QCH,
    SIU_G0_PPMU_NOCL1_QCH,
    SIU_G1_PPMU_NOCL1_QCH,
    SIU_G2_PPMU_NOCL1_QCH,
    SIU_G3_PPMU_NOCL1_QCH,
    SLH_ACEL_MI_D_HSI0_QCH,
    SLH_AST_MI_G_PPMU_AUD_QCH,
    SLH_AST_MI_G_PPMU_DOF_QCH,
    SLH_AST_MI_G_PPMU_DPUF0_QCH,
    SLH_AST_MI_G_PPMU_HSI0_QCH,
    SLH_AST_MI_G_PPMU_ICPU_QCH,
    SLH_AST_MI_G_PPMU_M2M_QCH,
    SLH_AST_MI_G_PPMU_MODEM_QCH,
    SLH_AST_MI_G_PPMU_NOCL0_NOCL1_QCH,
    SLH_AST_MI_G_PPMU_NOCL2A_NOCL1_QCH,
    SLH_AST_MI_G_PPMU_NOCL2B_NOCL1_QCH,
    SLH_AST_MI_G_PPMU_PERIS_QCH,
    SLH_AST_MI_G_PPMU_PSP_QCH,
    SLH_AST_MI_G_PPMU_UFS_QCH,
    SLH_AXI_MI_D0_MODEM_QCH,
    SLH_AXI_MI_D2_MODEM_QCH,
    SYSMMU_S0_MODEM_QCH_S0,
    SYSMMU_S0_PMMU0_MODEM_QCH_S0,
    BARAC_P_MODEM_QCH,
    CMU_NOCL1_QCH,
    D_TZPC_NOCL1_QCH,
    ECU_NOCL1_QCH,
    LH_AST_SI_IG_CPUCL0PPMU_NOCL1_QCH,
    LH_AST_SI_IG_DEBUGMUX_NOCL1_QCH,
    LH_AST_SI_IG_DP_NOCL1_QCH,
    LH_AST_SI_IG_G3DPPMU_NOCL1_QCH,
    LH_AXI_MI_IDP_NOCL1_QCH,
    LH_AXI_SI_P_MODEM_QCH,
    LH_INT_COMB_NOCL1_0_QCH,
    LH_INT_COMB_NOCL1_1_QCH,
    LH_TAXI_SI_P_NOCL1_NOCL2A_QCH,
    LH_TAXI_SI_P_NOCL1_NOCL2B_QCH,
    PBHA_GEN_D0_MODEM_QCH,
    PBHA_GEN_D1_MODEM_QCH,
    PPMU_DP_NOCL1_QCH,
    PPMU_P_PERIM_CPUCL0_QCH,
    PPMU_P_PERIM_G3D_QCH,
    PPMU_S0_NOCL1_QCH,
    PPMU_S1_NOCL1_QCH,
    PPMU_S2_NOCL1_QCH,
    PPMU_S3_NOCL1_QCH,
    PPMU_TREX_D0_MODEM_QCH,
    PPMU_TREX_D1_MODEM_QCH,
    PPMU_TREX_D2_MODEM_QCH,
    RSTnSYNC_CLK_NOCL1_NOCP_QCH,
    RSTnSYNC_SR_CLK_NOCL1_NOCP_QCH,
    S2PC_NOCL1_QCH,
    SLH_AXI_MI_P_PERIM_CPUCL0_QCH,
    SLH_AXI_MI_P_PERIM_G3D_QCH,
    SLH_AXI_SI_P0_G3D_QCH,
    SLH_AXI_SI_P1_G3D_QCH,
    SLH_AXI_SI_P_AUD_QCH,
    SLH_AXI_SI_P_CPUCL0_QCH,
    SLH_AXI_SI_P_DOF_QCH,
    SLH_AXI_SI_P_DPUB_QCH,
    SLH_AXI_SI_P_DPUF0_QCH,
    SLH_AXI_SI_P_GIC_PERIS_QCH,
    SLH_AXI_SI_P_HSI0_QCH,
    SLH_AXI_SI_P_ICPU_QCH,
    SLH_AXI_SI_P_M2M_QCH,
    SLH_AXI_SI_P_MIF0_QCH,
    SLH_AXI_SI_P_MIF1_QCH,
    SLH_AXI_SI_P_MIF2_QCH,
    SLH_AXI_SI_P_MIF3_QCH,
    SLH_AXI_SI_P_NOCL1_NOCL0_QCH,
    SLH_AXI_SI_P_PERIC0_QCH,
    SLH_AXI_SI_P_PERIC1_QCH,
    SLH_AXI_SI_P_PERIC2_QCH,
    SLH_AXI_SI_P_PERIS_QCH,
    SLH_AXI_SI_P_PSP_QCH,
    SLH_AXI_SI_P_UFS_QCH,
    SPC_NOCL1_QCH,
    SYSREG_NOCL1_QCH,
    TREX_D_NOCL1_QCH,
    TREX_P_NOCL1_QCH,
    VGEN_LITE_MODEM_QCH,
    RSTnSYNC_CLK_NOCL1_OSCCLK_QCH,
    LH_ACEL_MI_D0_MCSC_QCH,
    LH_ACEL_MI_D0_MLSC_QCH,
    LH_ACEL_MI_D0_MTNR_QCH,
    LH_ACEL_MI_D0_RGBP_QCH,
    LH_ACEL_MI_D0_YUVP_QCH,
    LH_ACEL_MI_D1_MCSC_QCH,
    LH_ACEL_MI_D1_MLSC_QCH,
    LH_ACEL_MI_D1_MTNR_QCH,
    LH_ACEL_MI_D1_RGBP_QCH,
    LH_ACEL_MI_D1_YUVP_QCH,
    LH_ACEL_MI_D2_CSIS_QCH,
    LH_ACEL_MI_D2_MCSC_QCH,
    LH_ACEL_MI_D_BYRP_QCH,
    LH_ACEL_MI_D_HSI1_QCH,
    LH_AST_MI_IG_DEBUGMUX_NOCL2A_QCH,
    LH_AST_SI_G_NOCL2A_NOCL0_QCH,
    LH_AXI_MI_D0_CSIS_QCH,
    LH_AXI_MI_D1_CSIS_QCH,
    LH_AXI_MI_D2_MTNR_QCH,
    LH_AXI_MI_D3_MTNR_QCH,
    LH_TAXI_MI_D0_SDMA_NOCL2A_QCH,
    LH_TAXI_MI_D1_SDMA_NOCL2A_QCH,
    LH_TAXI_MI_D2_SDMA_NOCL2A_QCH,
    LH_TAXI_MI_D3_SDMA_NOCL2A_QCH,
    LH_TAXI_SI_D0_NOCL2A_NOCL1_QCH,
    LH_TAXI_SI_D1_NOCL2A_NOCL1_QCH,
    LH_TAXI_SI_D2_NOCL2A_NOCL1_QCH,
    LH_TAXI_SI_D3_NOCL2A_NOCL1_QCH,
    RSTnSYNC_SR_CLK_NOCL2A_NOCD_QCH,
    SIU_G0_NOCL2A_QCH,
    SIU_G1_NOCL2A_QCH,
    SIU_G2_NOCL2A_QCH,
    SLH_AST_MI_G_PPMU_BYRP_QCH,
    SLH_AST_MI_G_PPMU_CSIS_QCH,
    SLH_AST_MI_G_PPMU_HSI1_QCH,
    SLH_AST_MI_G_PPMU_MCSC_QCH,
    SLH_AST_MI_G_PPMU_MLSC_QCH,
    SLH_AST_MI_G_PPMU_MTNR_QCH,
    SLH_AST_MI_G_PPMU_RGBP_QCH,
    SLH_AST_MI_G_PPMU_SDMA_QCH,
    SLH_AST_MI_G_PPMU_UFD_QCH,
    SLH_AST_MI_G_PPMU_YUVP_QCH,
    SLH_AST_SI_G_PPMU_NOCL2A_NOCL1_QCH,
    SLH_AXI_MI_D_UFD_QCH,
    CMU_NOCL2A_QCH,
    D_TZPC_NOCL2A_QCH,
    ECU_NOCL2A_QCH,
    LH_AST_SI_IG_DEBUGMUX_NOCL2A_QCH,
    LH_INT_COMB_NOCL2A_0_QCH,
    LH_INT_COMB_NOCL2A_1_QCH,
    LH_TAXI_MI_P_NOCL1_NOCL2A_QCH,
    PPMU_TREX_S0_NOCL2A_QCH,
    PPMU_TREX_S1_NOCL2A_QCH,
    PPMU_TREX_S2_NOCL2A_QCH,
    PPMU_TREX_S3_NOCL2A_QCH,
    RSTnSYNC_CLK_NOCL2A_NOCP_QCH,
    RSTnSYNC_SR_CLK_NOCL2A_NOCP_QCH,
    SLH_AXI_SI_P_BYRP_QCH,
    SLH_AXI_SI_P_CSIS_QCH,
    SLH_AXI_SI_P_DLFE_QCH,
    SLH_AXI_SI_P_DNC_QCH,
    SLH_AXI_SI_P_HSI1_QCH,
    SLH_AXI_SI_P_MCSC_QCH,
    SLH_AXI_SI_P_MLSC_QCH,
    SLH_AXI_SI_P_MSNR_QCH,
    SLH_AXI_SI_P_MTNR_QCH,
    SLH_AXI_SI_P_RGBP_QCH,
    SLH_AXI_SI_P_YUVP_QCH,
    SPC_NOCL2A_QCH,
    SYSREG_NOCL2A_QCH,
    TREX_D_NOCL2A_QCH,
    TREX_P_NOCL2A_QCH,
    LH_ACEL_MI_D0_LME_QCH,
    LH_ACEL_MI_D1_LME_QCH,
    LH_ACEL_MI_D2_LME_QCH,
    LH_AST_MI_IG_BDU_NOCL2B_QCH,
    LH_AST_SI_G_NOCL2B_NOCL0_QCH,
    LH_AXI_MI_D0_MFC_QCH,
    LH_AXI_MI_D0_MFD_QCH,
    LH_AXI_MI_D1_MFC_QCH,
    LH_AXI_MI_D1_MFD_QCH,
    LH_AXI_MI_D_ALIVE_QCH,
    LH_TAXI_SI_D0_NOCL2B_NOCL1_QCH,
    LH_TAXI_SI_D1_NOCL2B_NOCL1_QCH,
    LH_TAXI_SI_D2_NOCL2B_NOCL1_QCH,
    LH_TAXI_SI_D3_NOCL2B_NOCL1_QCH,
    RSTnSYNC_SR_CLK_NOCL2B_NOCD_QCH,
    SIU_G0_PPMU_NOCL2B_QCH,
    SLH_AST_MI_G_PPMU_GNSS_QCH,
    SLH_AST_MI_G_PPMU_LME_QCH,
    SLH_AST_MI_G_PPMU_MFC_QCH,
    SLH_AST_MI_G_PPMU_MFD_QCH,
    SLH_AST_SI_G_PPMU_NOCL2B_NOCL1_QCH,
    BARAC_P_GNSS_QCH,
    CMU_NOCL2B_QCH,
    D_TZPC_NOCL2B_QCH,
    ECU_NOCL2B_QCH,
    LH_AST_SI_IG_BDU_NOCL2B_QCH,
    LH_INT_COMB_NOCL2B_QCH,
    LH_TAXI_MI_P_NOCL1_NOCL2B_QCH,
    PPMU_S0_NOCL2B_QCH,
    PPMU_S1_NOCL2B_QCH,
    PPMU_S2_NOCL2B_QCH,
    PPMU_S3_NOCL2B_QCH,
    RSTnSYNC_CLK_NOCL2B_NOCP_QCH,
    RSTnSYNC_SR_CLK_NOCL2B_NOCP_QCH,
    SLH_AXI_SI_P_ALIVE_QCH,
    SLH_AXI_SI_P_GNSS_QCH,
    SLH_AXI_SI_P_LME_QCH,
    SLH_AXI_SI_P_MFC_QCH,
    SLH_AXI_SI_P_MFD_QCH,
    SPC_NOCL2B_QCH,
    SYSREG_NOCL2B_QCH,
    TREX_D_NOCL2B_QCH,
    TREX_P_NOCL2B_QCH,
    IP_NPUMEM_QCH,
    IP_NPUMEM_QCH_2,
    LH_AST_MI_LD_SRAMRDREQ0_DNC_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMRDREQ0_GNPU0_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMRDREQ0_GNPU1_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMRDREQ0_SDMA_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMRDREQ0_SNPU0_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMRDREQ0_SNPU1_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMRDREQ0_UNPU_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMRDREQ1_GNPU0_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMRDREQ1_GNPU1_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMRDREQ1_SDMA_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMRDREQ1_SNPU0_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMRDREQ1_SNPU1_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMRDREQ2_GNPU0_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMRDREQ2_GNPU1_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMRDREQ2_SDMA_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMRDREQ2_SNPU0_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMRDREQ2_SNPU1_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMRDREQ3_GNPU0_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMRDREQ3_GNPU1_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMRDREQ3_SDMA_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMRDREQ3_SNPU0_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMRDREQ3_SNPU1_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMRDREQ4_GNPU0_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMRDREQ4_GNPU1_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMRDREQ4_SDMA_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMRDREQ4_SNPU0_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMRDREQ4_SNPU1_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMRDREQ5_GNPU0_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMRDREQ5_GNPU1_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMRDREQ5_SDMA_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMRDREQ5_SNPU0_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMRDREQ5_SNPU1_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMRDREQ6_GNPU0_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMRDREQ6_GNPU1_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMRDREQ6_SDMA_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMRDREQ6_SNPU0_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMRDREQ6_SNPU1_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMRDREQ7_GNPU0_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMRDREQ7_GNPU1_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMRDREQ7_SDMA_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMRDREQ7_SNPU0_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMRDREQ7_SNPU1_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMRDRESP0_DSP_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMWRREQ0_DNC_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMWRREQ0_GNPU0_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMWRREQ0_GNPU1_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMWRREQ0_SDMA_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMWRREQ0_SNPU0_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMWRREQ0_SNPU1_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMWRREQ0_UNPU_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMWRREQ1_GNPU0_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMWRREQ1_GNPU1_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMWRREQ1_SDMA_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMWRREQ1_SNPU0_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMWRREQ1_SNPU1_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMWRREQ2_GNPU0_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMWRREQ2_GNPU1_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMWRREQ2_SDMA_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMWRREQ2_SNPU0_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMWRREQ2_SNPU1_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMWRREQ3_GNPU0_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMWRREQ3_GNPU1_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMWRREQ3_SDMA_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMWRREQ3_SNPU0_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMWRREQ3_SNPU1_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMWRREQ4_GNPU0_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMWRREQ4_GNPU1_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMWRREQ4_SDMA_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMWRREQ4_SNPU0_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMWRREQ4_SNPU1_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMWRREQ5_GNPU0_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMWRREQ5_GNPU1_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMWRREQ5_SDMA_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMWRREQ5_SNPU0_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMWRREQ5_SNPU1_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMWRREQ6_GNPU0_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMWRREQ6_GNPU1_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMWRREQ6_SDMA_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMWRREQ6_SNPU0_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMWRREQ6_SNPU1_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMWRREQ7_GNPU0_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMWRREQ7_GNPU1_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMWRREQ7_SDMA_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMWRREQ7_SNPU0_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMWRREQ7_SNPU1_NPUMEM_QCH,
    LH_AST_MI_LD_SRAMWRRESP0_DSP_NPUMEM_QCH,
    LH_AST_SI_LD_SRAMRDREQ0_NPUMEM_DSP_QCH,
    LH_AST_SI_LD_SRAMRDRSP0_NPUMEM_DNC_QCH,
    LH_AST_SI_LD_SRAMRDRSP0_NPUMEM_GNPU0_QCH,
    LH_AST_SI_LD_SRAMRDRSP0_NPUMEM_GNPU1_QCH,
    LH_AST_SI_LD_SRAMRDRSP0_NPUMEM_SDMA_QCH,
    LH_AST_SI_LD_SRAMRDRSP0_NPUMEM_SNPU0_QCH,
    LH_AST_SI_LD_SRAMRDRSP0_NPUMEM_SNPU1_QCH,
    LH_AST_SI_LD_SRAMRDRSP0_NPUMEM_UNPU_QCH,
    LH_AST_SI_LD_SRAMRDRSP1_NPUMEM_GNPU0_QCH,
    LH_AST_SI_LD_SRAMRDRSP1_NPUMEM_GNPU1_QCH,
    LH_AST_SI_LD_SRAMRDRSP1_NPUMEM_SDMA_QCH,
    LH_AST_SI_LD_SRAMRDRSP1_NPUMEM_SNPU0_QCH,
    LH_AST_SI_LD_SRAMRDRSP1_NPUMEM_SNPU1_QCH,
    LH_AST_SI_LD_SRAMRDRSP2_NPUMEM_GNPU0_QCH,
    LH_AST_SI_LD_SRAMRDRSP2_NPUMEM_GNPU1_QCH,
    LH_AST_SI_LD_SRAMRDRSP2_NPUMEM_SDMA_QCH,
    LH_AST_SI_LD_SRAMRDRSP2_NPUMEM_SNPU0_QCH,
    LH_AST_SI_LD_SRAMRDRSP2_NPUMEM_SNPU1_QCH,
    LH_AST_SI_LD_SRAMRDRSP3_NPUMEM_GNPU0_QCH,
    LH_AST_SI_LD_SRAMRDRSP3_NPUMEM_GNPU1_QCH,
    LH_AST_SI_LD_SRAMRDRSP3_NPUMEM_SDMA_QCH,
    LH_AST_SI_LD_SRAMRDRSP3_NPUMEM_SNPU0_QCH,
    LH_AST_SI_LD_SRAMRDRSP3_NPUMEM_SNPU1_QCH,
    LH_AST_SI_LD_SRAMRDRSP4_NPUMEM_GNPU0_QCH,
    LH_AST_SI_LD_SRAMRDRSP4_NPUMEM_GNPU1_QCH,
    LH_AST_SI_LD_SRAMRDRSP4_NPUMEM_SDMA_QCH,
    LH_AST_SI_LD_SRAMRDRSP4_NPUMEM_SNPU0_QCH,
    LH_AST_SI_LD_SRAMRDRSP4_NPUMEM_SNPU1_QCH,
    LH_AST_SI_LD_SRAMRDRSP5_NPUMEM_GNPU0_QCH,
    LH_AST_SI_LD_SRAMRDRSP5_NPUMEM_GNPU1_QCH,
    LH_AST_SI_LD_SRAMRDRSP5_NPUMEM_SDMA_QCH,
    LH_AST_SI_LD_SRAMRDRSP5_NPUMEM_SNPU0_QCH,
    LH_AST_SI_LD_SRAMRDRSP5_NPUMEM_SNPU1_QCH,
    LH_AST_SI_LD_SRAMRDRSP6_NPUMEM_GNPU0_QCH,
    LH_AST_SI_LD_SRAMRDRSP6_NPUMEM_GNPU1_QCH,
    LH_AST_SI_LD_SRAMRDRSP6_NPUMEM_SDMA_QCH,
    LH_AST_SI_LD_SRAMRDRSP6_NPUMEM_SNPU0_QCH,
    LH_AST_SI_LD_SRAMRDRSP6_NPUMEM_SNPU1_QCH,
    LH_AST_SI_LD_SRAMRDRSP7_NPUMEM_GNPU0_QCH,
    LH_AST_SI_LD_SRAMRDRSP7_NPUMEM_GNPU1_QCH,
    LH_AST_SI_LD_SRAMRDRSP7_NPUMEM_SDMA_QCH,
    LH_AST_SI_LD_SRAMRDRSP7_NPUMEM_SNPU0_QCH,
    LH_AST_SI_LD_SRAMRDRSP7_NPUMEM_SNPU1_QCH,
    LH_AST_SI_LD_SRAMWRREQ0_NPUMEM_DSP_QCH,
    LH_AST_SI_LD_SRAMWRRSP0_NPUMEM_DNC_QCH,
    LH_AST_SI_LD_SRAMWRRSP0_NPUMEM_GNPU0_QCH,
    LH_AST_SI_LD_SRAMWRRSP0_NPUMEM_GNPU1_QCH,
    LH_AST_SI_LD_SRAMWRRSP0_NPUMEM_SDMA_QCH,
    LH_AST_SI_LD_SRAMWRRSP0_NPUMEM_SNPU0_QCH,
    LH_AST_SI_LD_SRAMWRRSP0_NPUMEM_SNPU1_QCH,
    LH_AST_SI_LD_SRAMWRRSP0_NPUMEM_UNPU_QCH,
    LH_AST_SI_LD_SRAMWRRSP1_NPUMEM_GNPU0_QCH,
    LH_AST_SI_LD_SRAMWRRSP1_NPUMEM_GNPU1_QCH,
    LH_AST_SI_LD_SRAMWRRSP1_NPUMEM_SDMA_QCH,
    LH_AST_SI_LD_SRAMWRRSP1_NPUMEM_SNPU0_QCH,
    LH_AST_SI_LD_SRAMWRRSP1_NPUMEM_SNPU1_QCH,
    LH_AST_SI_LD_SRAMWRRSP2_NPUMEM_GNPU0_QCH,
    LH_AST_SI_LD_SRAMWRRSP2_NPUMEM_GNPU1_QCH,
    LH_AST_SI_LD_SRAMWRRSP2_NPUMEM_SDMA_QCH,
    LH_AST_SI_LD_SRAMWRRSP2_NPUMEM_SNPU0_QCH,
    LH_AST_SI_LD_SRAMWRRSP2_NPUMEM_SNPU1_QCH,
    LH_AST_SI_LD_SRAMWRRSP3_NPUMEM_GNPU0_QCH,
    LH_AST_SI_LD_SRAMWRRSP3_NPUMEM_GNPU1_QCH,
    LH_AST_SI_LD_SRAMWRRSP3_NPUMEM_SDMA_QCH,
    LH_AST_SI_LD_SRAMWRRSP3_NPUMEM_SNPU0_QCH,
    LH_AST_SI_LD_SRAMWRRSP3_NPUMEM_SNPU1_QCH,
    LH_AST_SI_LD_SRAMWRRSP4_NPUMEM_GNPU0_QCH,
    LH_AST_SI_LD_SRAMWRRSP4_NPUMEM_GNPU1_QCH,
    LH_AST_SI_LD_SRAMWRRSP4_NPUMEM_SDMA_QCH,
    LH_AST_SI_LD_SRAMWRRSP4_NPUMEM_SNPU0_QCH,
    LH_AST_SI_LD_SRAMWRRSP4_NPUMEM_SNPU1_QCH,
    LH_AST_SI_LD_SRAMWRRSP5_NPUMEM_GNPU0_QCH,
    LH_AST_SI_LD_SRAMWRRSP5_NPUMEM_GNPU1_QCH,
    LH_AST_SI_LD_SRAMWRRSP5_NPUMEM_SDMA_QCH,
    LH_AST_SI_LD_SRAMWRRSP5_NPUMEM_SNPU0_QCH,
    LH_AST_SI_LD_SRAMWRRSP5_NPUMEM_SNPU1_QCH,
    LH_AST_SI_LD_SRAMWRRSP6_NPUMEM_GNPU0_QCH,
    LH_AST_SI_LD_SRAMWRRSP6_NPUMEM_GNPU1_QCH,
    LH_AST_SI_LD_SRAMWRRSP6_NPUMEM_SDMA_QCH,
    LH_AST_SI_LD_SRAMWRRSP6_NPUMEM_SNPU0_QCH,
    LH_AST_SI_LD_SRAMWRRSP6_NPUMEM_SNPU1_QCH,
    LH_AST_SI_LD_SRAMWRRSP7_NPUMEM_GNPU0_QCH,
    LH_AST_SI_LD_SRAMWRRSP7_NPUMEM_GNPU1_QCH,
    LH_AST_SI_LD_SRAMWRRSP7_NPUMEM_SDMA_QCH,
    LH_AST_SI_LD_SRAMWRRSP7_NPUMEM_SNPU0_QCH,
    LH_AST_SI_LD_SRAMWRRSP7_NPUMEM_SNPU1_QCH,
    RSTnSYNC_CLK_NPUMEM_NOCD_QCH,
    RSTnSYNC_SR_CLK_NPUMEM_NOCD_QCH,
    SLH_AXI_MI_LD_CTRL_DNC_NPUMEM_QCH,
    CMU_NPUMEM_QCH,
    D_TZPC_NPUMEM_QCH,
    ECU_NPUMEM_QCH,
    LH_INT_COMB_NPUMEM_0_QCH,
    LH_INT_COMB_NPUMEM_1_QCH,
    RSTnSYNC_CLK_NPUMEM_NOCP_QCH,
    RSTnSYNC_SR_CLK_NPUMEM_NOCP_QCH,
    SLH_AXI_MI_LP_DNC_NPUMEM_QCH,
    SPC_NPUMEM_QCH,
    SYSREG_NPUMEM_QCH,
    I3C00_QCH_P,
    I3C00_QCH_S,
    I3C01_QCH_P,
    I3C01_QCH_S,
    I3C12_QCH_P,
    I3C12_QCH_S,
    CMU_PERIC0_QCH,
    DBG_UART_QCH,
    D_TZPC_PERIC0_QCH,
    GPIO_DSE_QCH,
    GPIO_PERIC0_QCH,
    LH_INT_COMB_PERIC0_QCH,
    PWM_QCH,
    SLH_AXI_MI_P_PERIC0_QCH,
    SPC_PERIC0_QCH,
    SYSREG_PERIC0_QCH,
    USI04_I2C_QCH,
    USI04_USI_QCH,
    USI12_I2C_QCH,
    USI13_I2C_QCH,
    USI22_SPI_I2C_QCH,
    I3C02_QCH_P,
    I3C02_QCH_S,
    I3C04_QCH_P,
    I3C04_QCH_S,
    I3C05_QCH_P,
    I3C05_QCH_S,
    BT_UART_QCH,
    CMU_PERIC1_QCH,
    D_TZPC_PERIC1_QCH,
    GPIO_PERIC1_QCH,
    LH_INT_COMB_PERIC1_QCH,
    PERIC1_I2C_QCH,
    SLH_AXI_MI_P_PERIC1_QCH,
    SPC_PERIC1_QCH,
    SYSREG_PERIC1_QCH,
    USI07_SPI_I2C_QCH,
    USI07_USI_QCH,
    USI08_SPI_I2C_QCH,
    USI08_USI_QCH,
    USI09_I2C_QCH,
    USI09_USI_QCH,
    USI10_I2C_QCH,
    USI10_USI_QCH,
    USI14_I2C_QCH,
    USI15_I2C_QCH,
    USI16_I2C_QCH,
    SPI_MULTI_SLV_Q_CTRL_PERIC1_QCH,
    I3C03_OIS_QCH_P,
    I3C03_OIS_QCH_S,
    I3C06_QCH_P,
    I3C06_QCH_S,
    I3C07_QCH_P,
    I3C07_QCH_S,
    I3C08_QCH_P,
    I3C08_QCH_S,
    I3C09_QCH_P,
    I3C09_QCH_S,
    I3C10_QCH_P,
    I3C10_QCH_S,
    I3C11_QCH_P,
    I3C11_QCH_S,
    CMU_PERIC2_QCH,
    D_TZPC_PERIC2_QCH,
    GPIO_PERIC2_QCH,
    LH_INT_COMB_PERIC2_QCH,
    SLH_AXI_MI_LP_CSIS_PERIC2_QCH,
    SLH_AXI_MI_P_PERIC2_QCH,
    SPC_PERIC2_QCH,
    SYSREG_PERIC2_QCH,
    USI00_SPI_I2C_QCH,
    USI00_USI_QCH,
    USI01_SPI_I2C_QCH,
    USI01_USI_QCH,
    USI02_I2C_QCH,
    USI02_USI_QCH,
    USI03_I2C_QCH,
    USI03_USI_QCH,
    USI05_I2C_QCH,
    USI05_USI_OIS_QCH,
    USI06_I2C_QCH,
    USI06_USI_OIS_QCH,
    USI11_I2C_QCH,
    USI11_USI_QCH,
    USI17_I2C_QCH,
    USI18_I2C_QCH,
    USI19_I2C_QCH,
    USI20_I2C_QCH,
    USI21_I2C_QCH,
    SPI_MULTI_SLV_Q_CTRL_PERIC2_QCH,
    OTP_CON_SUB_QCH,
    OTP_CON_TOP_QCH,
    GIC_QCH,
    LH_AST_MI_LD_ICC_CPUCL0_PERIS_QCH,
    LH_AST_SI_LD_IRI_PERIS_CPUCL0_QCH,
    SLH_AXI_MI_P_GIC_PERIS_QCH,
    LH_AXI_SI_D_PERIS_QCH,
    LLCAID_D_PERIS_QCH,
    PDMA_QCH,
    S2MPU_S0_PERIS_QCH_S0,
    S2MPU_S0_PMMU0_PERIS_QCH_S0,
    SLH_AST_SI_G_PPMU_PERIS_QCH,
    SPDMA_QCH,
    VGEN_D_PDMA_QCH,
    VGEN_D_SPDMA_QCH,
    CMU_PERIS_QCH,
    D_TZPC_PERIS_QCH,
    LH_INT_COMB_PERIS_QCH,
    PARITY_INT_COMB_QCH,
    PPMU_D_PERIS_QCH,
    QE_D_PDMA_QCH,
    QE_D_SPDMA_QCH,
    S2PC_PERIS_QCH,
    SLH_AXI_MI_P_PERIS_QCH,
    SPC_PERIS_QCH,
    SYSREG_PERIS_QCH,
    TMU_0_QCH,
    TMU_1_QCH,
    TMU_2_QCH,
    TMU_3_QCH,
    TMU_4_QCH,
    WDT0_QCH,
    WDT1_QCH,
    LH_ACEL_SI_D0_RGBP_QCH,
    LH_ACEL_SI_D1_RGBP_QCH,
    LH_AST_MI_OTF0_BYRP_RGBP_QCH,
    LH_AST_MI_OTF1_BYRP_RGBP_QCH,
    LH_AST_MI_OTF2_BYRP_RGBP_QCH,
    LH_AST_MI_OTF3_BYRP_RGBP_QCH,
    LH_AST_SI_OTF0_RGBP_CSIS_QCH,
    LH_AST_SI_OTF1_RGBP_CSIS_QCH,
    LH_AST_SI_OTF2_RGBP_CSIS_QCH,
    LH_AST_SI_OTF3_RGBP_CSIS_QCH,
    LLCAID_D0_RGBP_QCH,
    LLCAID_D1_RGBP_QCH,
    L_SIU_RGBP_QCH,
    RGBP_QCH,
    RGBP_QCH_VOTF0,
    SLH_AST_SI_G_PPMU_RGBP_QCH,
    SYSMMU_S0_PMMU0_RGBP_QCH_S0,
    SYSMMU_S0_PMMU1_RGBP_QCH_S0,
    SYSMMU_S0_RGBP_QCH_S0,
    VGEN_D10_RGBP_QCH,
    VGEN_D11_RGBP_QCH,
    VGEN_D12_RGBP_QCH,
    VGEN_D13_RGBP_QCH,
    VGEN_LITE_RGBP_QCH,
    CMU_RGBP_QCH,
    D_TZPC_RGBP_QCH,
    LH_INT_COMB_RGBP_QCH,
    PPMU_D0_RGBP_QCH,
    PPMU_D1_RGBP_QCH,
    S2PC_RGBP_QCH,
    SLH_AXI_MI_P_RGBP_QCH,
    SPC_RGBP_QCH,
    SYSREG_RGBP_QCH,
    IP_SDMA_QCH,
    LH_AST_MI_LD_SRAMRDRSP0_NPUMEM_SDMA_QCH,
    LH_AST_MI_LD_SRAMRDRSP1_NPUMEM_SDMA_QCH,
    LH_AST_MI_LD_SRAMRDRSP2_NPUMEM_SDMA_QCH,
    LH_AST_MI_LD_SRAMRDRSP3_NPUMEM_SDMA_QCH,
    LH_AST_MI_LD_SRAMRDRSP4_NPUMEM_SDMA_QCH,
    LH_AST_MI_LD_SRAMRDRSP5_NPUMEM_SDMA_QCH,
    LH_AST_MI_LD_SRAMRDRSP6_NPUMEM_SDMA_QCH,
    LH_AST_MI_LD_SRAMRDRSP7_NPUMEM_SDMA_QCH,
    LH_AST_MI_LD_SRAMWRRSP0_NPUMEM_SDMA_QCH,
    LH_AST_MI_LD_SRAMWRRSP1_NPUMEM_SDMA_QCH,
    LH_AST_MI_LD_SRAMWRRSP2_NPUMEM_SDMA_QCH,
    LH_AST_MI_LD_SRAMWRRSP3_NPUMEM_SDMA_QCH,
    LH_AST_MI_LD_SRAMWRRSP4_NPUMEM_SDMA_QCH,
    LH_AST_MI_LD_SRAMWRRSP5_NPUMEM_SDMA_QCH,
    LH_AST_MI_LD_SRAMWRRSP6_NPUMEM_SDMA_QCH,
    LH_AST_MI_LD_SRAMWRRSP7_NPUMEM_SDMA_QCH,
    LH_AST_SI_LD_SRAMRDREQ0_SDMA_NPUMEM_QCH,
    LH_AST_SI_LD_SRAMRDREQ1_SDMA_NPUMEM_QCH,
    LH_AST_SI_LD_SRAMRDREQ2_SDMA_NPUMEM_QCH,
    LH_AST_SI_LD_SRAMRDREQ3_SDMA_NPUMEM_QCH,
    LH_AST_SI_LD_SRAMRDREQ4_SDMA_NPUMEM_QCH,
    LH_AST_SI_LD_SRAMRDREQ5_SDMA_NPUMEM_QCH,
    LH_AST_SI_LD_SRAMRDREQ6_SDMA_NPUMEM_QCH,
    LH_AST_SI_LD_SRAMRDREQ7_SDMA_NPUMEM_QCH,
    LH_AST_SI_LD_SRAMWRREQ0_SDMA_NPUMEM_QCH,
    LH_AST_SI_LD_SRAMWRREQ1_SDMA_NPUMEM_QCH,
    LH_AST_SI_LD_SRAMWRREQ2_SDMA_NPUMEM_QCH,
    LH_AST_SI_LD_SRAMWRREQ3_SDMA_NPUMEM_QCH,
    LH_AST_SI_LD_SRAMWRREQ4_SDMA_NPUMEM_QCH,
    LH_AST_SI_LD_SRAMWRREQ5_SDMA_NPUMEM_QCH,
    LH_AST_SI_LD_SRAMWRREQ6_SDMA_NPUMEM_QCH,
    LH_AST_SI_LD_SRAMWRREQ7_SDMA_NPUMEM_QCH,
    LH_AXI_MI_LD_CTRL_DNC_SDMA_QCH,
    LH_AXI_MI_LD_MMU_DNC_SDMA_QCH,
    LH_TAXI_SI_D0_SDMA_NOCL2A_QCH,
    LH_TAXI_SI_D1_SDMA_NOCL2A_QCH,
    LH_TAXI_SI_D2_SDMA_NOCL2A_QCH,
    LH_TAXI_SI_D3_SDMA_NOCL2A_QCH,
    LLCAID_D0_SDMA_QCH,
    LLCAID_D1_SDMA_QCH,
    LLCAID_D2_SDMA_QCH,
    LLCAID_D3_SDMA_QCH,
    LLCAID_D_IPDNC_QCH,
    RSTnSYNC_CLK_SDMA_NOCD_QCH,
    RSTnSYNC_SR_CLK_SDMA_NOCD_QCH,
    SIU_G_PPMU_SDMA_QCH,
    SLH_AST_SI_G_PPMU_SDMA_QCH,
    SYSMMU_S0_PMMU0_SDMA_QCH_S0,
    SYSMMU_S0_PMMU1_SDMA_QCH_S0,
    SYSMMU_S0_PMMU2_SDMA_QCH_S0,
    SYSMMU_S0_PMMU3_SDMA_QCH_S0,
    SYSMMU_S0_SDMA_QCH_S0,
    SYSMMU_S1_PMMU0_SDMA_QCH_S0,
    SYSMMU_S1_SDMA_QCH_S0,
    VGEN_D0_SDMA_QCH,
    VGEN_D1_SDMA_QCH,
    VGEN_D2_SDMA_QCH,
    VGEN_D3_SDMA_QCH,
    VGEN_D_IPDNC_QCH,
    CMU_SDMA_QCH,
    D_TZPC_SDMA_QCH,
    ECU_SDMA_QCH,
    LH_INT_COMB_SDMA_QCH,
    PPMU_IPDNC_QCH,
    PPMU_SDMA0_QCH,
    PPMU_SDMA1_QCH,
    PPMU_SDMA2_QCH,
    PPMU_SDMA3_QCH,
    RSTnSYNC_CLK_SDMA_NOCP_QCH,
    RSTnSYNC_SR_CLK_SDMA_NOCP_QCH,
    S2PC_SDMA_QCH,
    SLH_AXI_MI_LP_DNC_SDMA_QCH,
    SPC_SDMA_QCH,
    SYSREG_SDMA_QCH,
    TREX_D_SDMA_QCH,
    IP_SNPU_QCH,
    RSTnSYNC_SR_CLK_SNPU_CU_QCH,
    LH_AST_MI_LD_SRAMRDRSP0_NPUMEM_SNPU_QCH,
    LH_AST_MI_LD_SRAMRDRSP1_NPUMEM_SNPU_QCH,
    LH_AST_MI_LD_SRAMRDRSP2_NPUMEM_SNPU_QCH,
    LH_AST_MI_LD_SRAMRDRSP3_NPUMEM_SNPU_QCH,
    LH_AST_MI_LD_SRAMRDRSP4_NPUMEM_SNPU_QCH,
    LH_AST_MI_LD_SRAMRDRSP5_NPUMEM_SNPU_QCH,
    LH_AST_MI_LD_SRAMRDRSP6_NPUMEM_SNPU_QCH,
    LH_AST_MI_LD_SRAMRDRSP7_NPUMEM_SNPU_QCH,
    LH_AST_MI_LD_SRAMWRRSP0_NPUMEM_SNPU_QCH,
    LH_AST_MI_LD_SRAMWRRSP1_NPUMEM_SNPU_QCH,
    LH_AST_MI_LD_SRAMWRRSP2_NPUMEM_SNPU_QCH,
    LH_AST_MI_LD_SRAMWRRSP3_NPUMEM_SNPU_QCH,
    LH_AST_MI_LD_SRAMWRRSP4_NPUMEM_SNPU_QCH,
    LH_AST_MI_LD_SRAMWRRSP5_NPUMEM_SNPU_QCH,
    LH_AST_MI_LD_SRAMWRRSP6_NPUMEM_SNPU_QCH,
    LH_AST_MI_LD_SRAMWRRSP7_NPUMEM_SNPU_QCH,
    LH_AST_SI_LD_SRAMRDREQ0_SNPU_NPUMEM_QCH,
    LH_AST_SI_LD_SRAMRDREQ1_SNPU_NPUMEM_QCH,
    LH_AST_SI_LD_SRAMRDREQ2_SNPU_NPUMEM_QCH,
    LH_AST_SI_LD_SRAMRDREQ3_SNPU_NPUMEM_QCH,
    LH_AST_SI_LD_SRAMRDREQ4_SNPU_NPUMEM_QCH,
    LH_AST_SI_LD_SRAMRDREQ5_SNPU_NPUMEM_QCH,
    LH_AST_SI_LD_SRAMRDREQ6_SNPU_NPUMEM_QCH,
    LH_AST_SI_LD_SRAMRDREQ7_SNPU_NPUMEM_QCH,
    LH_AST_SI_LD_SRAMWRREQ0_SNPU_NPUMEM_QCH,
    LH_AST_SI_LD_SRAMWRREQ1_SNPU_NPUMEM_QCH,
    LH_AST_SI_LD_SRAMWRREQ2_SNPU_NPUMEM_QCH,
    LH_AST_SI_LD_SRAMWRREQ3_SNPU_NPUMEM_QCH,
    LH_AST_SI_LD_SRAMWRREQ4_SNPU_NPUMEM_QCH,
    LH_AST_SI_LD_SRAMWRREQ5_SNPU_NPUMEM_QCH,
    LH_AST_SI_LD_SRAMWRREQ6_SNPU_NPUMEM_QCH,
    LH_AST_SI_LD_SRAMWRREQ7_SNPU_NPUMEM_QCH,
    LH_AXI_MI_LD_CTRL_DNC_SNPU_QCH,
    LH_AXI_SI_LD_CTRL_SNPU_DNC_QCH,
    LH_AXI_SI_LD_DRAM_SNPU_DNC_QCH,
    RSTnSYNC_CLK_SNPU_NOCD_QCH,
    RSTnSYNC_SR_CLK_SNPU_NOCD_QCH,
    CMU_SNPU_QCH,
    D_TZPC_SNPU_QCH,
    ECU_SNPU_QCH,
    HTU_SNPU_QCH_PCLK,
    LH_INT_COMB_SNPU_QCH,
    RSTnSYNC_CLK_SNPU_NOCP_QCH,
    RSTnSYNC_SR_CLK_SNPU_NOCP_QCH,
    SLH_AXI_MI_LP_DNC_SNPU_QCH,
    SPC_SNPU_QCH,
    SYSREG_SNPU_QCH,
    RSTnSYNC_CLK_SNPU_OSCCLK_QCH,
    HTU_SNPU_QCH_CLK,
    RSTnSYNC_SR_CLK_SNPU_XMAA0_QCH,
    IP_SNPU_QCH_CORE,
    RSTnSYNC_SR_CLK_SNPU_XMAA1_QCH,
    LH_AXI_MI_IP_PSP_QCH,
    LLCAID_D_PSP_QCH,
    RSTnSYNC_CLK_PSP_CM35P_QCH,
    RSTnSYNC_SR_CLK_PSP_PUF_QCH,
    S2MPU_S0_PMMU0_PSP_QCH,
    S2MPU_S0_PSP_QCH,
    SLH_AST_SI_G_PPMU_PSP_QCH,
    SS_PSP_QCH_BARAC_D,
    SS_PSP_QCH_BARAC_P,
    SS_PSP_QCH_CM35P,
    SS_PSP_QCH_DBG,
    SS_PSP_QCH_FPU,
    SS_PSP_QCH_PUF,
    SS_PSP_QCH_SC,
    SS_PSP_QCH_SPEX,
    VGEN_LITE_PSP_QCH,
    LH_ACEL_SI_D_PSP_QCH,
    CMU_PSP_QCH,
    D_TZPC_PSP_QCH,
    LH_AXI_SI_IP_PSP_QCH,
    LH_INT_COMB_PSP_QCH,
    PPMU_PSP_QCH,
    RTIC_PSP_QCH,
    S2PC_PSP_QCH,
    SLH_AXI_MI_P_PSP_QCH,
    SPC_PSP_QCH,
    SYSREG_PSP_QCH,
    I3C_UFD0_QCH_PCLK,
    I3C_UFD0_QCH_SCLK,
    I3C_UFD1_QCH_PCLK,
    I3C_UFD1_QCH_SCLK,
    CMU_UFD_QCH,
    D_TZPC_UFD_QCH,
    I2C_UFD0_QCH,
    I2C_UFD1_QCH,
    LH_INT_COMB_UFD_QCH,
    LLCAID_D_UFD_QCH,
    PDMA_UFD_QCH,
    PPMU_D_UFD_QCH,
    RSTnSYNC_SR_CLK_UFD_NOC_SLH_AST_MI_OTF_CSIS_UFD_SW_RESET_QCH,
    RSTnSYNC_SR_CLK_UFD_NOC_UFD_SW_RESET_QCH,
    S2PC_UFD_QCH,
    SLH_AST_MI_OTF_CSIS_UFD_QCH,
    SLH_AST_SI_G_PPMU_UFD_QCH,
    SLH_AXI_MI_LP_CMGP_UFD_QCH,
    SLH_AXI_SI_LP_UFD_CSIS_QCH,
    SPC_UFD_QCH,
    SRAM_MIU_UFD_QCH,
    SYSMMU_S0_PMMU0_UFD_QCH_S0,
    SYSMMU_S0_UFD_QCH_S0,
    SYSREG_UFD_QCH,
    SYSREG_UFD_SECURE_QCH,
    VGEN_LITE_D_UFD_QCH,
    uFD_QCH,
    SLH_AXI_SI_D_UFD_QCH,
    MMC_CARD_QCH,
    CMU_UFS_QCH,
    D_TZPC_UFS_QCH,
    ECU_UFS_QCH,
    GPIO_HSI1UFS_QCH,
    GPIO_UFS_QCH,
    LH_ACEL_SI_D_UFS_QCH,
    LH_INT_COMB_UFS_QCH,
    LLCAID_D_UFS_QCH,
    PPMU_UFS_QCH,
    QE_UFS_EMBD_QCH,
    S2MPU_S0_PMMU_UFS_QCH_S0,
    S2MPU_S0_UFS_QCH_S0,
    S2PC_UFS_QCH,
    SLH_AST_SI_G_PPMU_UFS_QCH,
    SLH_AXI_MI_P_UFS_QCH,
    SPC_UFS_QCH,
    SYSREG_UFS_QCH,
    UFS_EMBD_QCH,
    UFS_EMBD_QCH_FMP,
    VGEN_LITE_UFS_QCH,
    UFS_EMBD_QCH_FPC,
    UFS_EMBD_QCH_PCS,
    BARAC_UNPUALIVE_QCH,
    IP_UNPU_QCH,
    IP_UNPU_QCH_CLKIN,
    IP_UNPU_QCH_DBGCLK,
    IP_UNPU_QCH_SNPU,
    LH_AST_MI_LD_SRAMRDRSP0_NPUMEM_UNPU_QCH,
    LH_AST_MI_LD_SRAMWRRSP0_NPUMEM_UNPU_QCH,
    LH_AST_SI_LD_SRAMRDREQ0_UNPU_NPUMEM_QCH,
    LH_AST_SI_LD_SRAMWRREQ0_UNPU_NPUMEM_QCH,
    LH_AXI_MI_ID_IPUNPU1_UNPU_QCH,
    VGEN_LITE_UNPU_QCH,
    LH_AXI_SI_LD_UNPU_ALIVE_QCH,
    CMU_UNPU_QCH,
    D_TZPC_UNPU_QCH,
    LH_AXI_SI_ID_IPUNPU1_UNPU_QCH,
    LH_INT_COMB_UNPU_QCH,
    S2PC_UNPU_QCH,
    SLH_AXI_MI_LP_ALIVE_UNPU_QCH,
    SPC_UNPU_QCH,
    SYSREG_UNPU_QCH,
    LH_ACEL_SI_D0_YUVP_QCH,
    LH_ACEL_SI_D1_YUVP_QCH,
    LH_AST_MI_OTF_MSNR_YUVP_QCH,
    LH_AST_SI_OTF_YUVP_MCSC_QCH,
    LH_AXI_MI_LD0_MSNR_YUVP_QCH,
    LH_AXI_MI_LD1_MSNR_YUVP_QCH,
    LLCAID_D0_YUVP_QCH,
    LLCAID_D1_YUVP_QCH,
    RSTnSYNC_CLK_YUVP_NOCD_QCH,
    RSTnSYNC_SR_CLK_YUVP_NOCD_QCH,
    SIU_G_PPMU_YUVP_QCH,
    SLH_AST_SI_G_PPMU_YUVP_QCH,
    SYSMMU_S0_PMMU0_YUVP_QCH_S0,
    SYSMMU_S0_PMMU1_YUVP_QCH_S0,
    SYSMMU_S0_YUVP_QCH_S0,
    VGEN_LITE_YUVP0_QCH,
    VGEN_LITE_YUVP1_QCH,
    YUVP_QCH,
    YUVP_QCH_VOTF0,
    CMU_YUVP_QCH,
    D_TZPC_YUVP_QCH,
    ECU_YUVP_QCH,
    LH_INT_COMB_YUVP_QCH,
    PPMU_D0_YUVP_QCH,
    PPMU_D1_YUVP_QCH,
    RSTnSYNC_CLK_YUVP_NOCP_QCH,
    RSTnSYNC_SR_CLK_YUVP_NOCP_QCH,
    S2PC_YUVP_QCH,
    SLH_AXI_MI_P_YUVP_QCH,
    SPC_YUVP_QCH,
    SYSREG_YUVP_QCH,
	DFTMUX_CMU_QCH_CIS_CLK0,
	DFTMUX_CMU_QCH_CIS_CLK1,
	DFTMUX_CMU_QCH_CIS_CLK2,
	DFTMUX_CMU_QCH_CIS_CLK3,
	DFTMUX_CMU_QCH_CIS_CLK4,
	DFTMUX_CMU_QCH_CIS_CLK5,
	DFTMUX_CMU_QCH_CIS_CLK6,
    end_of_qch,
    num_of_qch = (end_of_qch - QCH_TYPE) & MASK_OF_ID,
};
enum option_id {
    CTRL_OPTION_S5E9955 = OPTION_TYPE,
    CTRL_OPTION_ALIVE,
    CTRL_OPTION_PMU_treeOnly,
    CTRL_OPTION_SCA,
    CTRL_OPTION_DBGCORE,
    CTRL_OPTION_AUD,
    CTRL_OPTION_BYRP,
    CTRL_OPTION_CHUB,
    CTRL_OPTION_VTS,
    CTRL_OPTION_CHUBVTS,
    CTRL_OPTION_CMGP,
    CTRL_OPTION_TOP,
    CTRL_OPTION_CPUCL0,
    CTRL_OPTION_CPUCL0_GLB,
    CTRL_OPTION_CPUCL1,
    CTRL_OPTION_CPUCL2,
    CTRL_OPTION_CPUCL3,
    CTRL_OPTION_DSU,
    CTRL_OPTION_CSIS,
    CTRL_OPTION_AOCCSIS,
    CTRL_OPTION_DLFE,
    CTRL_OPTION_DNC,
    CTRL_OPTION_DPUB,
    CTRL_OPTION_DPUF0,
    CTRL_OPTION_DPUF1,
    CTRL_OPTION_DSP,
    CTRL_OPTION_G3D,
    CTRL_OPTION_G3DCORE,
    CTRL_OPTION_GNPU,
    CTRL_OPTION_HSI0,
    CTRL_OPTION_HSI1,
    CTRL_OPTION_ICPU,
    CTRL_OPTION_DOF,
    CTRL_OPTION_LME,
    CTRL_OPTION_M2M,
    CTRL_OPTION_MCSC,
    CTRL_OPTION_MFC,
    CTRL_OPTION_MFD,
    CTRL_OPTION_MIF,
    CTRL_OPTION_S2D,
    CTRL_OPTION_MLSC,
    CTRL_OPTION_MSNR,
    CTRL_OPTION_MTNR,
    CTRL_OPTION_NOCL0,
    CTRL_OPTION_NOCL1,
    CTRL_OPTION_NOCL2A,
    CTRL_OPTION_NOCL2B,
    CTRL_OPTION_NPUMEM,
    CTRL_OPTION_PERIC0,
    CTRL_OPTION_PERIC1,
    CTRL_OPTION_PERIC2,
    CTRL_OPTION_PERIS,
    CTRL_OPTION_RGBP,
    CTRL_OPTION_SDMA,
    CTRL_OPTION_SNPU,
    CTRL_OPTION_PSP,
    CTRL_OPTION_UFD,
    CTRL_OPTION_UFS,
    CTRL_OPTION_UNPU,
    CTRL_OPTION_YUVP,
    CTRL_OPTION_MODEM_treeOnly,
	end_of_option,
	num_of_option = (end_of_option - OPTION_TYPE) & MASK_OF_ID,
};
#endif
