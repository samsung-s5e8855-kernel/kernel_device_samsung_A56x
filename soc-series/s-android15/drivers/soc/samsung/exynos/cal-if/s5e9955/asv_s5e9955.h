#ifndef __ASV_EXYNOS9955_H__
#define __ASV_EXYNOS9955_H__
...
#define T6_OFFSET	(16)
#define T6_MASK		(0xff)
#define T7_ADDR		(0x10009020)
#define T7_OFFSET	(0)
#define T7_MASK		(0xffff)
...
struct id_tbl_info {
	...
	unsigned char asb_test_version_sub1:4;
	unsigned char asb_test_version_sub2:4;
...
int asv_get_grp(unsigned int id)
{
	...
	case CAM:
		...
		break;
	case DISP:
		grp = asv_tbl.disp_asv_group;
		break;
	case NPU:
		grp = asv_tbl.npu0_asv_group;
		break;
	case AUD:
		grp = asv_tbl.aud_asv_group;
		break;
	case CP:
...
{
	return id_tbl.asb_test_version + id_tbl.asb_test_version_sub1 + id_tbl.asb_test_version_sub2;
...
int asv_table_init(void)
{
	...
	power_info.t2 = ((tmp >> T2_OFFSET) & T2_MASK) * 100 / 34;
	regs = ioremap(T3_ADDR, sizeof(int));
	tmp = __raw_readl(regs);
	power_info.t3 = ((tmp >> T3_OFFSET) & T3_MASK) * 100 / 34;
	...
	exynos_smc_readsfr((unsigned long)T7_ADDR, &tmp);
