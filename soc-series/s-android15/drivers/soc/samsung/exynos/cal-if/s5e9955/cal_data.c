...
#include "pmucal/flexpmu_cal_cpu_s5e9955.h"
#include "pmucal/flexpmu_cal_local_s5e9955.h"
#include "pmucal/flexpmu_cal_p2vmap_s5e9955.h"
#include "pmucal/flexpmu_cal_system_s5e9955.h"
#include "pmucal/flexpmu_cal_define_s5e9955.h"

#if IS_ENABLED(CONFIG_CP_PMUCAL)
#include <soc/samsung/pmucal/pmucal_cp.h>
#include "pmucal/pmucal_cp_s5e9955.h"
#endif

#if IS_ENABLED(CONFIG_GNSS_PMUCAL)
#include <soc/samsung/pmucal/pmucal_gnss.h>
#include "pmucal/pmucal_gnss_s5e9955.h"
#endif

#if IS_ENABLED(CONFIG_CHUB_PMUCAL)
#include <soc/samsung/pmucal/pmucal_chub.h>
#include "pmucal/pmucal_chub_s5e9955.h"
#endif
...
#include "cmucal/clkout_s5e9955.c"
#include "acpm_dvfs_s5e9955.h"
#include "asv_s5e9955.h"
#include "../ra.h"
#include <linux/smc.h>
...
#define S5E9955_CMU_TOP_BASE		(0x2a0a0000)
...
unsigned int frac_rpll_list[15];
unsigned int frac_rpll_size;

unsigned int s5e9955_frac_rpll_list[] = {
	AUD_PLL_AUD,
	TOP_PLL_MMC,
	TOP_PLL_SHARED0,
	TOP_PLL_SHARED1,
...
void s5e9955_cal_data_init(void)
{
	int i;
	...
	cmu_top = ioremap(S5E9955_CMU_TOP_BASE, SZ_4K);
	if (!cmu_top)
		pr_err("%s: cmu_top ioremap failed\n", __func__);

	frac_rpll_size = ARRAY_SIZE(s5e9955_frac_rpll_list);
	for (i = 0; i < frac_rpll_size; i++)
		frac_rpll_list[i] = s5e9955_frac_rpll_list[i];

	if (exynos_soc_info.main_rev >= 1) {
		pr_info("Overriding EVT1.x PMUCAL\n");
		pmucal_pd_list[PD_GNPU0].save = gnpu0_save_evt1;
		pmucal_pd_list[PD_GNPU1].save = gnpu1_save_evt1;
		pmucal_pd_list[PD_GNPU0].num_save = ARRAY_SIZE(gnpu0_save_evt1);
		pmucal_pd_list[PD_GNPU1].num_save = ARRAY_SIZE(gnpu1_save_evt1);
...
void (*cal_data_init)(void) = s5e9955_cal_data_init;
