...
#else
void set_power_rail_data(struct req_volt *rail_info)
{
	struct regulator *reg = dbg_info->reg_ptr;
	unsigned int i, j;

	for (i = 0; i < power_rail_name_size; i++) {
		struct req_volt rail = { 0 };

		/* req_tail is count of requesting dvfs domain id in each power rail */
		/* get domain id that has max voltage of power rail */
		for (j = 0; j < reg[i].req_tail; j++) {
			/* power off domain's voltage is skip */
			if ((reg[i].voltages[j].type == ID_IN_POWER_OFF) ||
					reg[i].voltages[j].type == ID_IN_MIN_LOCK_OF_POWER_OFF)
				continue;

			if (rail.volt < reg[i].voltages[j].volt)
				rail = reg[i].voltages[j];
		}
		...
		size_t count, loff_t *ppos)
{
	...
	set_power_rail_data(dbg_info->rail_info);
	...
	for(i = 0; i < power_rail_name_size; i++) {
		domain_id = dbg_info->rail_info[i].req_id;
		volt = dbg_info->rail_info[i].volt;
...
#else
	prop = of_get_property(node, "dvfs-layer-offset", &len);
	if(prop) {
		offset = be32_to_cpup(prop);
	}
	else {
		pr_err("%s %s: could not parsing dvfs-layer-offset\n",
				EXYNOS_POWER_RAIL_DBG_PREFIX, __func__);
		goto err_parse;
	}

	info->apm_sram_base = get_esca_dump_addr(offset);
	if (IS_ERR(info->apm_sram_base)) {
		pr_err("%s %s: get_esca_dump_addr return error\n",
				EXYNOS_POWER_RAIL_DBG_PREFIX, __func__);
		goto err_parse;
	}

	prop = of_get_property(node, "regulator-offset", &len);
	if(prop) {
		offset = be32_to_cpup(prop);
	}
	else {
		pr_err("%s %s: could not parsing regulator-offset\n",
				EXYNOS_POWER_RAIL_DBG_PREFIX, __func__);
		goto err_parse;
	}
	offset = (*(unsigned int*)(get_fvmap_base() + offset));
#if IS_ENABLED(CONFIG_SOC_S5E8855)
#define SECURE_ACPM_SIZE (0x6000)
	info->reg_ptr = (struct regulator *)(info->apm_sram_base - SECURE_ACPM_SIZE + offset);
#else
	info->reg_ptr = (struct regulator *)(info->apm_sram_base + offset);
#endif
	pr_info("%s %d %d\n", __func__, __LINE__, offset);

	prop = of_get_property(node, "num-power-rails", &len);
	if (prop) {
		size = be32_to_cpup(prop);
		if (size <= 0) {
			pr_err("%s %s: invalid num-power-rails value\n",
					EXYNOS_POWER_RAIL_DBG_PREFIX, __func__);
			goto err_parse;
		}
	} else {
		pr_err("%s %s: missing num-power-rails value\n",
				EXYNOS_POWER_RAIL_DBG_PREFIX, __func__);
		goto err_parse;
	}

	dbg_info->rail_info = kzalloc(sizeof(struct req_volt) * size, GFP_KERNEL);
	if (!dbg_info->rail_info) {
		pr_err("%s %s: could not allocate mem for rail_info array\n",
				EXYNOS_POWER_RAIL_DBG_PREFIX, __func__);
		goto err_parse;
	}
#endif
