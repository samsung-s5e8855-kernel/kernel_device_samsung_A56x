...
static void memlog_clean_prvobj_work(struct work_struct *work)
{
	...
	do {
		...
		if (prvobj) {
			...
			ida_simple_remove(&main_desc.memlog_minor_id, prvobj->obj.file->minor);
				...
				void *src, size_t size)
{
	...
	if (prvobj->support_logfwd &&
		...
		prvobj->to_string(src, size, prvobj->string_buf, MEMLOG_STRING_BUF_SIZE - 1, &pos);

		if (pos < MEMLOG_STRING_BUF_SIZE) {
			dev_err(prvdesc->dev, "%s", (char *)prvobj->string_buf);
	...
	if ((u64)(prvobj->curr_ptr + size) > (u64)(obj->vaddr + obj->size)) {
		void *curr_ptr;

		file_prvobj = obj_to_data(prvobj->file_obj);

		if (prvobj->support_file && atomic_read(&file_prvobj->open_cnt)) {
			if (!prvobj->is_full) {
				prvobj->buffered_data_size =
						(u64)prvobj->curr_ptr - (u64)prvobj->read_ptr;
				memcpy(prvobj->file_buffer,
					prvobj->read_ptr, prvobj->buffered_data_size);
				prvobj->is_full = true;
				memlog_queue_work(&prvobj->file_work);
			} else {
				memlog_update_lost_data(file_prvobj, (u64)prvobj->curr_ptr -
									(u64)prvobj->read_ptr);
			}
		...
		if ((prvobj->support_file && atomic_read(&file_prvobj->open_cnt)) &&
			!prvobj->is_full && ((u64)prvobj->curr_ptr - (u64)prvobj->read_ptr >
			...
			if (atomic_read(&file_prvobj->open_cnt)) {
				if (!prvobj->is_full) {
					prvobj->buffered_data_size = obj->size -
						(prvobj->array_unit_size * prvobj->read_log_idx);
	...
	if ((u64)(prvobj->curr_ptr + log_len) >
		...
		if (prvobj->support_file &&
			...
			if (!prvobj->is_full) {
				...
				prvobj->buffered_data_size = min(prvobj->buffered_data_size,
								prvobj->file_buffer_size);
...
int memlog_do_dump(struct memlog_obj *obj, int log_level)
{
	...
	if (prvobj->support_file) {
		if (!prvobj->is_full) {
			prvobj->buffered_data_size = obj->size;
			memcpy(prvobj->file_buffer, obj->vaddr, prvobj->buffered_data_size);
			prvobj->is_full = true;
			memlog_queue_work(&prvobj->file_work);
		} else {
					...
					const char *name, u32 uflag)
{
	...
	if (file_obj) {
		if ((file_obj->log_type != MEMLOG_TYPE_FILE) || (file_obj->size < size)) {
	...
	if (obj) {
		...
		if (file_obj) {
			struct memlog_obj_prv *file_prvobj = obj_to_data(file_obj);
					...
					u32 uflag)
{
	...
	if (obj) {
		...
		if (file_obj) {
			...
			if (main_desc.mem_constraint && !!prvobj->file_buffer) {
				void *vaddr;
				size_t reset_size;
					...
					const char *name)
{
	...
	flag = MEMLOG_TYPE_DUMP | MEMLOG_PROPERTY_NONCACHABLE;
...
static int memlog_devnode_init(struct device *dev)
{
	...
	main_desc.memlog_class = class_create(MEMLOG_NAME);
					...
					char *buf, loff_t off, size_t count)
{
	...
	if (data == NULL) {
		...
		memlog_ba_file_add(
			&main_desc.total_info_file_list, &main_desc.total_info_file_lock, data);
	}
	...
	if (data->size > off) {
		...
		memlog_ba_file_delete(
				&main_desc.total_info_file_list,
