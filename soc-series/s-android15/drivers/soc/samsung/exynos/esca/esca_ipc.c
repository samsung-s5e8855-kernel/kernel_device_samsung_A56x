...
static LIST_HEAD(deferred_slave_list);
...
static irqreturn_t esca_ipc_irq_handler(int irq, void *data)
{
	...
	for (i = 0; i < ipc->num_channels; i++) {
		if (status & (0x1 << ipc->channel[i].id)) {
			if (!list_empty(&ipc->channel[i].slave_node)) {
				esca_interrupt_clr(ipc, ipc->channel[i].id);
			} else if (ipc->channel[i].interrupt) {
				esca_interrupt_clr(ipc, ipc->channel[i].id);
			...
			if (ipc->channel[i].is_master)
				defer_masters |= (0x1 << ipc->channel[i].id);
		}
	}
	...
	if (defer_masters) {
		...
		/* Update deferred IPC channels */
		list_for_each_entry(channel, &deferred_slave_list, slave_node) {
...
bool is_esca_ipc_busy(unsigned ch_id)
{
	...
	/* Return true if ESCA IPC is busy */
	return !ret;
}
...
static int channel_init(struct esca_ipc_info *ipc, u32 *ch_buf, int len)
{
	...
	for (i = 0; i < ipc->num_channels; i++) {
		...
		INIT_LIST_HEAD(&ipc->channel[i].slave_node);
...
static int esca_ipc_dt_parse(struct platform_device *pdev)
{
	...
	if (!prop) {
		dev_err(&pdev->dev, "esca-layer property is missing\n");
		return -ENODEV;
	}
	...
	if(of_property_read_bool(np, "deferred_ipc_masters")) {
		int size, i;

		size = of_property_count_u32_elems(np, "deferred_ipc_masters");

		for (i = 0; i < size; i++) {
			u32 ch;

			ret = of_property_read_u32_index(np,
					"deferred_ipc_masters", i, &ch);
			if (ret) {
				dev_err(&pdev->dev, "failed to find deferred master\n");
				return ret;
			}

			ch &= ESCA_IPC_CHANNEL_MASK;

			if (ch >= esca->ipc.num_channels) {
				dev_err(&pdev->dev, "master id isn't valid\n");
				return -EINVAL;
			}

			esca->ipc.channel[ch].is_master = true;
		}
	}

	if(of_property_read_bool(np, "deferred_ipc_slaves")) {
		int size, i;

		size = of_property_count_u32_elems(np, "deferred_ipc_slaves");
		for (i = 0; i < size; i++) {
			u32 ch;

			ret = of_property_read_u32_index(np,
					"deferred_ipc_slaves", i, &ch);
			if (ret) {
				dev_err(&pdev->dev, "failed to find deferred slave\n");
				return ret;
			}
			...
			if (ch >= esca->ipc.num_channels) {
				dev_err(&pdev->dev, "slave id isn't valid\n");
				return -EINVAL;
			}
			list_add_tail(&esca->ipc.channel[ch].slave_node,
						&deferred_slave_list);
		}
	}
