...
void esca_init_eint_uwb_clk_req(u32 eint_num)
{
	u32 pend;

	exynos_pmu_write(EXYNOS_PMU_SPARE7, 0);
	exynos_pmu_write(EXYNOS_PMU_SPARE7, (1 << 26) | (eint_num & 0xff));		//PMU_SPARE7
	exynos_pmu_write(EXYNOS_SPARE_CTRL__DATA7 | 0xc000, 7);		//SPARE_CTRL__DATA7
	do {
		exynos_pmu_read(EXYNOS_SPARE_CTRL__DATA7, &pend);
	} while (pend & (1 << 7));
}
EXPORT_SYMBOL_GPL(esca_init_eint_uwb_clk_req);
