...
{
	u32 domain, freq, noti_flag;
	u16 start_time, end_time;
	ktime_t time;
	unsigned long flags;
	struct exynos_dm_data *data;
...
#if IS_ENABLED(CONFIG_EXYNOS_ESCA)
	time = esca_time_calc_u16(start_time, end_time);
#else
	time = acpm_time_calc(start_time, end_time);
#endif
	data = &exynos_dm->dm_data[domain];

//	pr_info("%s: domain %d start %d end %d time %llu freq %u noti_flag %u\n",
//			__func__, domain, start_time, end_time,
//			time, freq, noti_flag);
...
int policy_update_call_to_DM(int dm_type, u32 min_freq, u32 max_freq)
{
	...
	if (data->fast_switch && !data->num_constraints)
		return 0;

	dbg_snapshot_dm(POLICY_IN, dm_type, min_freq, max_freq);
	...
	dbg_snapshot_dm(POLICY_OUT, dm_type, min_freq, max_freq);
...
int DM_CALL(int dm_type, unsigned long *target_freq)
{
	...
	if (data->fast_switch) {
		dbg_snapshot_dm(FREQ_ASYNC, dm_type, *target_freq, 0);
		...
		if (data->num_constraints)
			exynos_acpm_dm_set_freq_async(dm_type, *target_freq);

		spin_unlock_irqrestore(&fast_switch_glb_lock, flags);
	} else {
		dbg_snapshot_dm(FREQ_SYNC_IN, dm_type, *target_freq, 0);
		...
		dbg_snapshot_dm(FREQ_SYNC_OUT, dm_type, *target_freq, 0);
	}
