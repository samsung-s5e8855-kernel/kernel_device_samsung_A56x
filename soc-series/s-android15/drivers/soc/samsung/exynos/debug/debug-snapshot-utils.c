...
#define EL1_UNDEF_OTP_UNPROGRAMMED (-1)

static struct cpumask cpu_dss_context_saved_mask;

static dss_el1_undef_hook_fn dss_el1_undef_callback;
atomic_t dss_el1_undef_otp = ATOMIC_INIT(EL1_UNDEF_OTP_UNPROGRAMMED);
					   ...
					   unsigned long l, void *buf)
{
	...
	if (dss_desc.panic_to_wdt ||
		(num_possible_cpus() != cpumask_weight(&cpu_dss_context_saved_mask))) {
...
static void __maybe_unused dbg_snapshot_do_el1_fpac(void *data, struct pt_regs *regs,
		unsigned long esr)
{
...
static DEFINE_PER_CPU(unsigned long, dbg_snapshot_bad_stack_chk);

static void __maybe_unused dbg_snapshot_do_handle_bad_stack(void *data, struct pt_regs *regs,
               unsigned long esr, unsigned long far)
{
	if (raw_cpu_read(dbg_snapshot_bad_stack_chk)) {
		dbg_snapshot_expire_watchdog();
		cpu_park_loop();
	}

	raw_cpu_write(dbg_snapshot_bad_stack_chk, 0xBAD);
}

int register_dss_el1_undef_hook(dss_el1_undef_hook_fn fn)
{
	int cpu;
	int otp_status;

	cpu = raw_smp_processor_id();
	otp_status = atomic_cmpxchg(&dss_el1_undef_otp, EL1_UNDEF_OTP_UNPROGRAMMED, cpu);

	if (otp_status != EL1_UNDEF_OTP_UNPROGRAMMED) {
		pr_info("%s: dss el1 undef hooker already registered\n", __func__);
		return -EINVAL;
	}

	dss_el1_undef_callback = fn;
	pr_info("%s: dss el1 undef hooker registered\n", __func__);

	return 0;
}
EXPORT_SYMBOL_GPL(register_dss_el1_undef_hook);

static void __maybe_unused dbg_snapshot_do_el1_undef(void *data, struct pt_regs *regs,
               unsigned long esr)
{
	if (!!dss_el1_undef_callback)
		dss_el1_undef_callback(regs, esr);
...
void dbg_snapshot_init_utils(struct device *dev)
{
	...
	register_trace_android_rvh_do_el1_fpac(dbg_snapshot_do_el1_fpac, NULL);
	register_trace_android_rvh_handle_bad_stack(dbg_snapshot_do_handle_bad_stack, NULL);
	register_trace_android_rvh_do_el1_undef(dbg_snapshot_do_el1_undef, NULL);
#endif
