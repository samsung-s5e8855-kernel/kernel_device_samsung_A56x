...
static BLOCKING_NOTIFIER_HEAD(dss_pmsg_notifier_list);
void dbg_snapshot_pmsg_chain_register(struct notifier_block *nb)
{
	blocking_notifier_chain_register(&dss_pmsg_notifier_list, nb);
}
EXPORT_SYMBOL_GPL(dbg_snapshot_pmsg_chain_register);
...
static void dbg_snapshot_combine_pmsg(char *buffer, size_t count, unsigned int level)
{
	...
	unsigned char prio;
	struct dss_pmsg_notifier_data nb_data;
	...
	case LOGGER_LEVEL_TEXT:
		...
		nb_data.buffer = buffer;
		nb_data.count = count;
		blocking_notifier_call_chain(&dss_pmsg_notifier_list, 0,(void *)&nb_data);
		break;
	default:
		break;
	}
...
static char *dbg_snapshot_pmsg_devnode(const struct device *dev, umode_t *mode)
{
	if (mode)
		*mode = 0220;
	return NULL;
...
void dbg_snapshot_init_pmsg(void)
{
	...
	dss_pmsg_class = class_create(PMSG_NAME);
