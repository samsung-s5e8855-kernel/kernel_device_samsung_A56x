...
{
	exynos_esca_force_apm_wdt_reset();
	asm volatile("b .");
}
#else
static void simulate_APM_WDT(char *arg)
...
static void simulate_FPAC(char *arg)
{
	dev_crit(exynos_debug_desc.dev, "%s()\n", __func__);

	// Code from tools/testing/selftests/arm64/pauth/pac_corruptor.S
	asm("paciasp\n\t"
	    "eor lr, lr, #1 << 53\n\t"
	    "autiasp\n\t"
	    "ret");
...
static void simulate_S2RLOCKUP(char *arg)
{
	...
	memcpy(ptr, arg, index);
	ptr[index] = '\0';
	if (enabled && !strcmp(ptr, "unlock")) {
		enabled = 0;
	...
	for (i = eS2R_PM_SUSPEND_PREPARE; i < eS2R_STAGE_CNT; i++) {
		if (!strcmp(ptr, exynos_debug_desc.s2r_stage_name[i])) {
			if (!enabled)
				mutex_lock(&exynos_debug_desc.s2r_lock);
			...
			if (index != 0) {
				...
				memcpy(ptr, tmparg, index);
				ptr[index] = '\0';
				if (!strcmp(ptr, "async")) {
					device_enable_async_suspend(exynos_debug_desc.dev);
					pr_emerg("async Suspend/Resume test start");
				}
			}
			enabled = 1;
