...
int adv_tracer_ehld_set_enable(int en, bool log)
{
	...
	if (ret < 0) {
		...
		ret = adv_tracer_recovery();
		if (!ret)
			dbg_snapshot_expire_watchdog();
		else
			panic("EAT hang & recovery failed\n");
		return ret;
	}
...
int adv_tracer_ehld_clr_data(void)
{
	struct adv_tracer_ipc_cmd cmd;
	int ret = 0;

	cmd.cmd_raw.cmd = eEHLD_IPC_CMD_CLR_DATA;
	ret = adv_tracer_ipc_send_data_polling(plugin_ehld.ehld_dev->id, &cmd);
	if (ret < 0) {
		pr_err("ehld-ipc: failed to clear data\n");
		return ret;
	}

	pr_info("ehld-ipc: clear EHLD data\n");
	return 0;
}
