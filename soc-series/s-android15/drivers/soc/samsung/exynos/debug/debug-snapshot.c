...
{
	if (dss_desc.scratch_disable || !dss_desc.scratch_offset || !dss_desc.scratch_bit)
		return;
...
static void dbg_snapshot_init_desc(struct device *dev)
{
	...
	if (!of_property_read_u32(dev->of_node, "scratch-disable", &val))
		dss_desc.scratch_disable = val;

	if (dss_desc.scratch_disable) {
		dev_info(dss_desc.dev, "Ramdump scratch disabled\n");
		return;
	}
...
static void *_dbg_snapshot_rmem_set_vmap(struct reserved_mem *rmem)
{
	...
	size = rmem->size;
	base = ALIGN_DOWN(rmem->base, PAGE_SIZE);
	if (base != rmem->base)
		size += (rmem->base - base);
...
static int dbg_snapshot_rmem_setup(struct device *dev)
{
	...
	for (i = 0; i < mem_count; i++) {
		...
		if (!vaddr) {
			dev_err(dev, "%s: paddr:%llx size:%llx failed to vmap\n",
					rmem->name, rmem->base, rmem->size);
			continue;
		}
