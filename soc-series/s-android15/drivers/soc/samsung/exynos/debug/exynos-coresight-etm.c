...
#if IS_ENABLED(CONFIG_EXYNOS_CORESIGHT_ETF)
struct etf_info {
	void __iomem	*base;
	u32		f_port[ARR_SZ];
};
#endif
#if IS_ENABLED(CONFIG_EXYNOS_CORESIGHT_ETR)
struct etr_info {
	void __iomem	*base;
...
static void exynos_etm_etf_enable(void)
{
	...
	for (i = 0; i < ee_info->etf_num; i++) {
		...
		etm_writel(etf->base, 0x100, TMCBUFWM);
...
static void exynos_etm_etr_enable(void)
{
	...
	etm_writel(etr->base, etr_buf_size, TMCRSZ);
...
int coresight_etm_funnel_on(unsigned int funnel_num, unsigned int port_num, bool on)
{
	struct funnel_info *funnel;

	if (funnel_num > ee_info->funnel_num) {
		pr_warn("Out of range of funnel num");
		return -EINVAL;
	}

	if (port_num > ee_info->funnel_port_num) {
		pr_warn("Out of range of port num");
		return -EINVAL;
	}

	funnel = &ee_info->funnel[funnel_num];
	spin_lock(&ee_info->trace_lock);
	exynos_etm_set_funnel_port(funnel, port_num, on);
	spin_unlock(&ee_info->trace_lock);

	return 0;
}

EXPORT_SYMBOL_GPL(coresight_etm_funnel_on);
...
static ssize_t exynos_etm_print_info(char *buf)
{
	...
	struct etf_info *etf;
#endif
	unsigned long ctrl, port_status;
#if IS_ENABLED(CONFIG_EXYNOS_CORESIGHT_ETF) || IS_ENABLED(CONFIG_EXYNOS_CORESIGHT_ETR)
	unsigned long sts, read_p;
#endif
...
static int exynos_etm_cs_etm_init_dt(struct device *dev)
{
	...
	if (of_property_read_u32(etm_np, "funnel-port-num", &ee_info->funnel_port_num))
		return -EINVAL;
