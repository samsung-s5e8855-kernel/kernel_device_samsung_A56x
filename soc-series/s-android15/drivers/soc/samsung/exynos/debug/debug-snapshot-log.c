...
struct dbg_snapshot_log_item dss_log_items[] = {
	...
	[DSS_LOG_PMIC_ID]	= {DSS_LOG_PMIC,	{0, 0, 0, false}, 0, false},
	[DSS_LOG_PRINTK_ID]	= {DSS_LOG_PRINTK,	{0, 0, 0, false}, 0, false},
};

struct dbg_snapshot_log_misc dss_log_misc;
char dss_freq_name[SZ_32][SZ_8];
EXPORT_SYMBOL_GPL(dss_freq_name);
unsigned int dss_freq_size = 0;
EXPORT_SYMBOL_GPL(dss_freq_size);

struct cpuidle_dbg {
	void __iomem	*reg;
	struct dbg_cpuidle *cpuidle;
	bool enabled;
};

static struct cpuidle_dbg g_cpuidle_dbg;
...
long dss_get_first_##item##_log_idx(void) {				\
	...
	return v < dss_get_len_##item##_log() ? 0 : v &			\
			(dss_get_len_##item##_log() - 1);		\
}									\
...
long dss_get_first_##item##_log_idx(int cpu) {				\
	...
	if (cpu < 0 || cpu >= dss_get_len_##item##_log())		\
		return -EINVAL;						\
	return v < dss_get_len_##item##_log_by_cpu(cpu) ? 0 : v &		\
			(dss_get_len_##item##_log_by_cpu(cpu) - 1);		\
}									\
EXPORT_SYMBOL_GPL(dss_get_first_##item##_log_idx);			\
...
static void dbg_snapshot_task(int cpu, struct task_struct *task)
{
	...
	idx = (atomic_fetch_inc(&dss_log_misc.task_log_idx[cpu]) &
			(log_item->log_num - 1)) + (cpu * log_item->log_num);
...
static void dbg_snapshot_work(work_func_t fn, int en)
{
	...
	idx = (atomic_fetch_inc(&dss_log_misc.work_log_idx[cpu]) &
			(log_item->log_num - 1)) + (cpu * log_item->log_num);
	entry[idx].time = local_clock();
	entry[idx].fn = fn;
	entry[idx].en = en;
...
static inline void write_cpuidle_val(int cpu, u32 val)
{
	__raw_writel(val, &g_cpuidle_dbg.cpuidle->cpu[cpu].val);
}

static void dbg_snapshot_cpuidle_dbg(int cpu, char *mode, int en)
{
	/* Error occurs when 64 bit is accessed */
	//g_cpuidle_dbg.cpuidle->cpu[cpu].time = cpu_clock(cpu);

	if (en != DSS_FLAG_IN) {
		write_cpuidle_val(cpu,(0xDB90 << 16) | DSS_CPUIDLE_ON);
		return;
	}

	write_cpuidle_val(cpu, (0xDB90 << 16) | DSS_CPUIDLE_POWER_MODE);
}
...
void dbg_snapshot_cpuidle(char *modes, unsigned int state, s64 diff, int en)
{
	...
	if (g_cpuidle_dbg.enabled)
		dbg_snapshot_cpuidle_dbg(cpu, modes, en);

	if (!dbg_snapshot_is_log_item_enabled(DSS_LOG_CPUIDLE_ID))
		return;

	idx = (atomic_fetch_inc(&dss_log_misc.cpuidle_log_idx[cpu]) &
			(log_item->log_num - 1)) + (cpu * log_item->log_num);
...
void dbg_snapshot_irq(int irq, void *fn, int en)
{
	...
	idx = (atomic_fetch_inc(&dss_log_misc.irq_log_idx[cpu]) &
			(log_item->log_num - 1)) + (cpu * log_item->log_num);
	entry[idx].time = local_clock();
	entry[idx].irq = irq;
	entry[idx].fn = fn;
...
EXPORT_SYMBOL_GPL(dbg_snapshot_irq);
				 ...
				 void *fn, int en)
{
	...
	idx = (atomic_fetch_inc(&dss_log_misc.hrtimer_log_idx[cpu]) &
			(log_item->log_num - 1)) + (cpu * log_item->log_num);
	entry[idx].time = local_clock();
	entry[idx].now = now;
					...
					unsigned long fn)
{
	...
	idx = (atomic_fetch_inc(&dss_log_misc.reg_log_idx[cpu]) &
			(log_item->log_num - 1)) + (cpu * log_item->log_num);
	entry[idx].time = local_clock();
...
{
	dbg_snapshot_reg('w', width, val, (void *)addr, fn0);
}
...
{
	dbg_snapshot_reg('r', width, 0, (void *)addr, fn0);
}
...
__maybe_unused static void dbg_snapshot_reg_post_read(void *ignore,
						...
						const volatile void __iomem *addr)
{
	dbg_snapshot_reg('R', width, val, (void *)addr, fn0);
}
				...
				int event, int en)
{
	...
	idx = (atomic_fetch_inc(&dss_log_misc.suspend_log_idx) &
			(log_item->log_num - 1));
	entry[idx].time = local_clock();
			...
			unsigned int rvolt, int en)
{
	...
	idx = (atomic_fetch_inc(&dss_log_misc.regulator_log_idx) &
			(log_item->log_num - 1));
			...
			unsigned long long max_cooling)
{
	...
	idx = (atomic_fetch_inc(&dss_log_misc.thermal_log_idx) &
		(log_item->log_num - 1));
...
void dbg_snapshot_clk(void *clock, const char *func_name, unsigned long arg, int mode)
{
	...
	idx = (atomic_fetch_inc(&dss_log_misc.clk_log_idx) &
			(log_item->log_num - 1));
...
void dbg_snapshot_pmu(int id, const char *func_name, int mode)
{
	...
	idx = (atomic_fetch_inc(&dss_log_misc.pmu_log_idx) &
		(log_item->log_num - 1));
...
void dbg_snapshot_freq(int type, u64 time, int seq_no, unsigned int old_freq,
			unsigned int target_freq, int en)
{
	...
	idx = (atomic_fetch_inc(&dss_log_misc.freq_log_idx[type]) &
			(log_item->log_num - 1)) + (type * log_item->log_num);

	entry[idx].time = time;
	entry[idx].cpu = seq_no;
...
void dbg_snapshot_dm(int req_type, int dm_type, unsigned int min, unsigned int max)
{
	unsigned long idx;
	...
	idx = (atomic_fetch_inc(&dss_log_misc.dm_log_idx) &
			(log_item->log_num - 1));

	entry[idx].time = local_clock();
	entry[idx].cpu = raw_smp_processor_id();
	entry[idx].req_type = req_type;
	entry[idx].dm_num = dm_type;
...
void dbg_snapshot_esca(unsigned long long timestamp, const char *log,
			unsigned int data)
{
	unsigned long idx;
	...
	idx = (atomic_fetch_inc(&dss_log_misc.acpm_log_idx) &
		(log_item->log_num - 1));
	entry[idx].time = local_clock();
	entry[idx].acpm_time = timestamp;
...
EXPORT_SYMBOL_GPL(dbg_snapshot_esca);
#else
...
void dbg_snapshot_pmic(int irq, const char *str, int data1, int data2)
{
	unsigned long idx;
	struct dbg_snapshot_log_item *log_item = &dss_log_items[DSS_LOG_PMIC_ID];
	struct pmic_log *entry = (struct pmic_log *)log_item->entry.vaddr;
	int cpu = raw_smp_processor_id();

	if (!dbg_snapshot_is_log_item_enabled(DSS_LOG_PMIC_ID))
		return;

	idx = atomic_fetch_inc(&dss_log_misc.pmic_log_idx) % log_item->log_num;

	entry[idx].time = cpu_clock(cpu);
	entry[idx].cpu = cpu;
	entry[idx].irq = irq;
	entry[idx].data1 = data1;
	entry[idx].data2 = data2;
	strncpy(entry[idx].str, str, sizeof(entry[idx].str) - 1);
}
EXPORT_SYMBOL_GPL(dbg_snapshot_pmic);
...
void dbg_snapshot_printk(const char *fmt, ...)
{
	...
	idx = (atomic_fetch_inc(&dss_log_misc.print_log_idx) &
		(log_item->log_num - 1));
	va_start(args, fmt);
...
static void dbg_snapshot_print_last_irq(int cpu)
{
	...
	idx = ((atomic_read(&dss_log_misc.irq_log_idx[cpu]) - 1) &
			(log_item->log_num - 1)) + (cpu * log_item->log_num);
...
static void dbg_snapshot_print_last_task(int cpu)
{
	...
	idx = ((atomic_read(&dss_log_misc.task_log_idx[cpu]) - 1) &
			(log_item->log_num - 1)) + (cpu * log_item->log_num);
	dbg_snapshot_get_sec(entry[idx].time, &sec, &msec);
	task = entry[idx].task;
...
static void dbg_snapshot_print_last_work(int cpu)
{
	...
	idx = ((atomic_read(&dss_log_misc.work_log_idx[cpu]) - 1) &
			(log_item->log_num - 1)) + (cpu * log_item->log_num);
	dbg_snapshot_get_sec(entry[idx].time, &sec, &msec);
...
static void dbg_snapshot_print_last_cpuidle(int cpu)
{
	...
	idx = ((atomic_read(&dss_log_misc.cpuidle_log_idx[cpu]) - 1) &
			(log_item->log_num - 1)) + (cpu * log_item->log_num);
	dbg_snapshot_get_sec(entry[idx].time, &sec, &msec);
...
static void dbg_snapshot_print_freqinfo(void)
{
	...
	for (i = 0; i < dss_freq_size; i++) {
		...
		idx = (atomic_read(&dss_log_misc.freq_log_idx[i]) &
				(log_item->log_num - 1)) + (i * log_item->log_num);
...
static void dbg_snapshot_set_log_item_field(void)
{
	...
	case eDSS_LOG_TYPE_DEFAULT:
		...
		log_item_set_field(struct dbg_snapshot_log, PMIC, pmic);
	...
	case eDSS_LOG_TYPE_MINIMIZED:
		...
		log_item_set_field(struct dbg_snapshot_log_minimized, PMIC, pmic);
		break;
	default:
		break;
	}
...
static void init_cpuidle_dbg(struct device_node *np)
{
	int cpu;

	if (of_get_property(np, "enable_cpuidle_dbg", NULL) == NULL)
		return;

	g_cpuidle_dbg.reg = of_iomap(np, 0);
	if (!g_cpuidle_dbg.reg) {
		dev_err(dss_desc.dev, "DSS cpuidle dbg remap failed.\n");
		return;
	}

	g_cpuidle_dbg.cpuidle = (struct dbg_cpuidle *)g_cpuidle_dbg.reg;

	for (cpu = 0; cpu < NR_CPUS; cpu++)
		write_cpuidle_val(cpu, (0xDB90 << 16) | DSS_CPUIDLE_INVAL);

	__raw_writel(0xDB90, &g_cpuidle_dbg.cpuidle->cpuidle_magic);

	g_cpuidle_dbg.enabled = 1;

	dev_info(dss_desc.dev, "DSS cpuidle dbg is enabled.\n");
}
