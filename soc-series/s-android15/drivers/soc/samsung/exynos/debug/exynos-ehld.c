...
static void ehld_event_update(int cpu)
{
	...
	data->instret[count] = readl(ctrl->reg_instret +
				(PPC_PMCNT0_LOW + (ctrl->subset_id * SZ_4)));
	if (ehld->ver == EHLD_V3)
	...
	if (!ctrl->cpupm_enabled) {
		...
		DBG_UNLOCK(ctrl->dbg_base + ehld->pmu_offset);
		val = __raw_readq(ctrl->dbg_base + ehld->pmu_offset + PMUPCSR);
		val = __raw_readq(ctrl->dbg_base + ehld->pmu_offset + PMUPCSR);
		if (MSB_MASKING == (MSB_MASKING & val))
			val |= MSB_PADDING;
		data->pmpcsr[count] = val;
		DBG_LOCK(ctrl->dbg_base + ehld->pmu_offset);
	}
...
static int ehld_init_dt_parse(struct device_node *np)
{
	...
	if (!of_property_read_u32(np, "pmu-offset", &offset)) {
		ehld->pmu_offset = offset;
		ehld_info(1, "ehld: set pmu offset 0x%x\n",ehld->pmu_offset);
	} else {
		ehld->pmu_offset = PMU_OFFSET;
		ehld_info(1, "ehld: set pmu offset defult 0x%x\n",ehld->pmu_offset);
	}

	nr_subset = of_property_count_u32_elems(np, "cpu-end");
	if (nr_subset <= 0) {
		ehld_err(1, "ehld: cpu-end invalid\n");
		return -EINVAL;
	}

	ehld->reg_instret = kmalloc_array(nr_subset, sizeof(void *), GFP_KERNEL);
	if (!ehld->reg_instret) {
		ehld_err(1, "ehld: reg_instret alloc failed\n");
		return -ENOMEM;
	}

	ehld->reg_instrun = kmalloc_array(nr_subset, sizeof(void *), GFP_KERNEL);
	if (!ehld->reg_instrun) {
		ehld_err(1, "ehld: reg_instrun  alloc failed\n");
		ret = -ENOMEM;
		goto err_alloc_instrun;
	}

	for (i = 0; i < nr_subset; i++) {
		ehld->reg_instret[i] = of_iomap(np, i);
		if (ehld->reg_instret[i])
			ehld_info(1, "ehld: instret reg[%d] address registered\n", i);

		ehld->reg_instrun[i] = of_iomap(np, nr_subset + i);
		if (ehld->reg_instrun[i])
			ehld_info(1, "ehld: instrun reg[%d] address registered\n", i);
	}

	cpu_end = kmalloc_array(nr_subset, sizeof(unsigned int), GFP_KERNEL);
	if (!cpu_end) {
		ehld_err(1, "ehld: memory alloc failed\n");
		ret = -ENOMEM;
		goto err_alloc_cpuend;
	}

	ret = of_property_read_u32_array(np, "cpu-end", cpu_end, nr_subset);
	if (ret < 0) {
		ehld_err(1, "ehld: failed to get cpu end\n");
		ret = -EINVAL;
	...
	if (of_property_read_bool(child, "support-clear-data")) {
		ehld->dbgc.support_clear_data = true;
		ehld_info(1, "Support dbgc clear EHLD data\n");
	} else {
		ehld->dbgc.support_clear_data = false;
		ehld_info(1, "Not support dbgc clear EHLD data\n");
	}
	...
	for_each_possible_cpu(cpu) {
		...
		for (i = 0; i < nr_subset; i++) {
			if (cpu <= cpu_end[i]) {
				ctrl->reg_instret = ehld->reg_instret[i];
				ctrl->reg_instrun = ehld->reg_instrun[i];

				cpu_start = (i == 0) ? 0 : cpu_end[i - 1] + 1;
				ctrl->subset_id = cpu - cpu_start;
				break;
			}
		}
...
err_setup_ctrl:
	free_percpu(ehld_ctrl);
err_ipc_init:
err_read_property:
	kfree(cpu_end);
err_alloc_cpuend:
	kfree(ehld->reg_instrun);
err_alloc_instrun:
	kfree(ehld->reg_instret);
...
static int ehld_dbgc_setup(void)
{
	...
	if (ehld->dbgc.support_clear_data) {
		val = adv_tracer_ehld_clr_data();
		if (val < 0) {
			return -EINVAL;
		}
	}
