...
static int __init exynos_ssp_init(void)
{
	...
	pr_info("[EXYNOS][CMSSP][ INFO] %s function start\n" , __func__);

	ret = platform_driver_register(&exynos_ssp_driver);
	if (ret) {
		pr_err("[EXYNOS][CMSSP][ ERR] %s: fail to register driver. ret = 0x%x\n", __func__, ret);
		return ret;
	}

	pr_info("[EXYNOS][CMSSP][ INFO] driver, (c) 2024 Samsung Electronics\n");
