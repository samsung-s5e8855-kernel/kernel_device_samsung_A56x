...
static int profile_thread(void *data)
{
	...
	int temp_g3d = 0, temp_bat = 0, temp_npu = 0;
	...
	while (p_info.is_running) {
		...
		// npu temperature
		tz = thermal_zone_get_zone_by_name("NPU");
		if (!IS_ERR(tz)) {
			thermal_zone_get_temp(tz, &temp_npu);
			temp_npu = (temp_npu < 0) ? 0 : temp_npu / 1000;
		}
		pbuf_data += snprintf(pbuf + pbuf_data, pbuf_sz - pbuf_data, "%d ", temp_npu);
		...
		// gpu util, gpu clock
		if (gpu_dvfs_fn) {
			gpu_util = gpu_dvfs_fn->get_utilization();
			gpu_clock = gpu_dvfs_fn->get_cur_freq() / 1000;
		} else {
			gpu_util = 0;
			gpu_clock = 0;
		}
		...
		for_each_possible_cpu(cpu)
			pbuf_data += snprintf(pbuf + pbuf_data, pbuf_sz - pbuf_data, "%lu ", ml_cpu_util(cpu));
		...
		// llc way
		pbuf_data += snprintf(pbuf + pbuf_data, pbuf_sz - pbuf_data, "%d ", llc_get_region_info(LLC_REGION_CPU));

		// dsu opmode
		pbuf_data += snprintf(pbuf + pbuf_data, pbuf_sz - pbuf_data, "%d ", get_dsu_opmode());
...
int alloc_buf_for_profile(void)
{
	int try_alloc_cnt = 3;
	int ret = 0;
...
void prepare_header_profile(void)
{
	...
	if (cl_cnt > 3)
		pbuf_data += snprintf(pbuf + pbuf_data, pbuf_sz - pbuf_data, "%02d-temp_MIDH ", grp_num);

	pbuf_data += snprintf(pbuf + pbuf_data, pbuf_sz - pbuf_data, "%02d-temp_G3D ", grp_num);
	pbuf_data += snprintf(pbuf + pbuf_data, pbuf_sz - pbuf_data, "%02d-temp_NPU ", grp_num);
	...
	// llc way
	pbuf_data += snprintf(pbuf + pbuf_data, pbuf_sz - pbuf_data, "%02d-llc_way ", grp_num);
	grp_num++;

	// dsu opmode
	pbuf_data += snprintf(pbuf + pbuf_data, pbuf_sz - pbuf_data, "%02d-ems_dsumode ", grp_num);
...
int xperf_gpu_fn_init(const struct platform_device *pdev, const struct gpu_dvfs_fn* fn)
{
	if (!fn) {
		pr_err("[%s] %s: failed init gpu_dvfs_fn\n", prefix, __func__);
		return -EINVAL;
	}
	gpu_dvfs_fn = fn;
	return 0;
}
EXPORT_SYMBOL(xperf_gpu_fn_init);
