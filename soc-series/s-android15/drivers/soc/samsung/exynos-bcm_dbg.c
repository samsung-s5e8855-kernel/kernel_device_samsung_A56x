...
|| IS_ENABLED(CONFIG_EXYNOS_ESCAV1) || IS_ENABLED(CONFIG_EXYNOS_ESCA)
...
out:
	mutex_unlock(&bcm_dbg_data->lock);
					...
					struct exynos_bcm_dbg_data *data)
{
	...
	if (likely(ipc_base_info->direction == BCM_EVT_GET || *bcm_run != BCM_HALT))
		mutex_lock(&data->lock);

	if (unlikely(data->bcm_run_state == BCM_HALT)) {
		if (ipc_base_info->direction == BCM_EVT_GET)
			*bcm_run = BCM_HALT;
		goto out;
	}
	...
	if (ipc_base_info->direction == BCM_EVT_SET) {
		...
		cmd[0] |= BCM_CMD_SET(run, BCM_EVT_RUN_CONT_MASK,
	...
	if (ipc_base_info->direction == BCM_EVT_GET) {
		run = BCM_CMD_GET(cmd[2], BCM_EVT_RUN_CONT_MASK,
					BCM_EVT_RUN_CONT_SHIFT);
		*bcm_run = run;
...
void exynos_bcm_dbg_stop(unsigned int bcm_stop_owner)
{
	...
	if (unlikely(bcm_dbg_data->available_halt_owner[bcm_stop_owner]))
		stop = BCM_HALT;

	ret = exynos_bcm_dbg_run(stop, bcm_dbg_data);
	if (ret) {
...
static ssize_t show_bcm_dbg_data_df_filter_id(struct file *fp, struct kobject *kobj,
		struct bin_attribute *battr, char *buf, loff_t off, size_t size)
{
	struct device *dev = container_of(kobj, struct device, kobj);
	struct platform_device *pdev = container_of(dev,
...
static ssize_t show_bcm_dbg_data_df_filter_others(struct file *fp, struct kobject *kobj,
		struct bin_attribute *battr, char *buf, loff_t off, size_t size)
{
	struct device *dev = container_of(kobj, struct device, kobj);
	struct platform_device *pdev = container_of(dev,
		...
		struct bin_attribute *battr, char *buf, loff_t off, size_t size)
{
	...
	count += snprintf(buf + count, PAGE_SIZE, "\n= Usage: echo [type] > enable_stop_owner (0: disable, 1: stop, 2: stop+halt)\n");
	...
	count += snprintf(buf + count, PAGE_SIZE, "\n= BCM Available halt owner =\n");
	for (i = 0; i < STOP_OWNER_MAX; i++)
		count += snprintf(buf + count, PAGE_SIZE, " halt owner[%d]: %s\n",
				i, data->available_halt_owner[i] ? "true" : "false");
		...
		struct bin_attribute *battr, char *buf, loff_t off, size_t size)
{
	...
	if (enable == 1) {
		...
		data->available_halt_owner[owner_index] = false;
	} else if (enable == 2) {
		data->available_stop_owner[owner_index] = true;
		data->available_halt_owner[owner_index] = true;
	} else {
...
static BIN_ATTR(bcm_dbg_data_df_filter_id, 0440,
			show_bcm_dbg_data_df_filter_id, NULL, 0);
static BIN_ATTR(bcm_dbg_data_df_filter_others, 0440,
			show_bcm_dbg_data_df_filter_others, NULL, 0);
...
static struct bin_attribute *exynos_bcm_dbg_sysfs_entries[] = {
	...
	&bin_attr_bcm_dbg_data_df_filter_id,
	&bin_attr_bcm_dbg_data_df_filter_others,
...
module_platform_driver(exynos_bcm_dbg_driver);
