				...
				struct exynos_bcm_dbg_data *data)
{
	...
	for_each_child_of_node(np, child_np) {
		...
		if (!strcmp(node_name, "bcm_esca_ipc")) {
			data->ipc_node = child_np;
		} else if (!strcmp(node_name, "bcm_esca_noti_ipc")){
			data->ipc_noti_node = child_np;
		} else {
		...
		node_name = child_np->name;
		BCM_DBG("%s: child node name: %s\n", __func__, node_name);
		if (!strcmp(node_name, "bcm_ipc_bcm_event")) {
			data->ipc_node = child_np;
		} else {
			BCM_ERR("%s: No device node name: %s\n", __func__, node_name);
			return -ENODEV;
				...
				struct exynos_bcm_dbg_data *data)
{
	...
	for (i = 0; i < data->bcm_ip_nr; i++)
		data->initial_run_ip[i] = BCM_IP_EN;

	if(of_property_read_bool(np, "unused_bcm_ip")) {
		size = of_property_count_u32_elems(np, "unused_bcm_ip");
		BCM_INFO("%s: number of unused BCM IPs=%d\n", __func__, size);

		if (size < 0) {
			BCM_ERR("%s: Failed get number of unused BCM IPs\n", __func__);
		...
		for (i = 0; i < size; i++) {
			ret = of_property_read_u32_index(np,
					"unused_bcm_ip", i, &unused_ip);
			if (ret) {
				BCM_ERR("%s: Failed parse unused BCM IP(%d)\n",	__func__, i);
				kfree(data->initial_run_ip);
				return ret;
			}

			data->initial_run_ip[unused_ip] = BCM_IP_DIS;
		}
	}
				...
				struct exynos_bcm_dbg_data *data)
{
	...
	/* Get Halt Owner */
	size = of_property_count_u32_elems(np, "halt_owner");
	if (size < 0) {
		BCM_ERR("%s: Failed get number of halt_owner\n",
				__func__);
		return size;
	} else if (size > STOP_OWNER_MAX) {
		BCM_ERR("%s: Invalid halt owner size (%u)\n", __func__, size);
		return -EINVAL;
	} else if (size > 0) {
		u32 halt_owner_data[STOP_OWNER_MAX];

		of_property_read_u32_array(np, "halt_owner", halt_owner_data, size);

		for (i = 0; i < size; i++) {
			owner_index = halt_owner_data[i];
			if (owner_index >= STOP_OWNER_MAX) {
				BCM_ERR("%s: Invalid halt owner (%d:%u)\n",
						__func__, i, owner_index);
				return -EINVAL;
			}
...
#if !IS_ENABLED(CONFIG_EXYNOS_BCM_DBG_GNR)
static int exynos_bcm_rtc_parse_dt(struct device_node *np,
				struct exynos_bcm_dbg_data *data)
{
	int base, offset, ret;

	ret = of_property_read_u32_index(np, "rtc_addr", 0, &base);
	ret |= of_property_read_u32_index(np, "rtc_addr", 1, &offset);
	data->rtc_addr = ioremap((phys_addr_t)base + (unsigned long)offset, 0x4);
...
#if !IS_ENABLED(CONFIG_EXYNOS_BCM_DBG_GNR)
	ret = exynos_bcm_rtc_parse_dt(np, data);
	if (ret) {
		BCM_ERR("%s: Failed parse rtc address\n", __func__);
		return ret;
	}
#endif
