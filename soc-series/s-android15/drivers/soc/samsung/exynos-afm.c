...
struct afm_domain {
	...
	unsigned int			pmic_afm_en_offset;
	unsigned int			pmic_afm_level_offset;
	unsigned int			afm_warn_lowest_level;
	unsigned int			afm_warn_level_mask;
	struct kobject			kobj;
	struct list_head		list;

	unsigned int			pmic;
	unsigned int			pmic_no;
	struct cpumask			interrupt_affinity;

	unsigned int			orig_afm_warn_level;
	unsigned int			orig_afm_enable;
	bool				gen_interrupt_enabled;

	int (*pmic_read_reg)(u32, u8, u8*);
	int (*pmic_update_reg)(u32, u8, u8, u8);
};
...
{
	int ret;
	ret = afm_dom->pmic_update_reg(afm_dom->pmic_no, afm_dom->pmic_afm_en_offset,
...
static int afm_gen_interrupt_control(struct afm_domain *afm_dom, int enable)
{
	if (enable) {
		...
		if (!afm_dom->orig_afm_enable)
			control_afm_warn(afm_dom, AFM_WARN_ENABLE);

		if (afm_dom->pmic_update_reg(afm_dom->pmic_no, afm_dom->pmic_afm_level_offset,
					afm_dom->afm_warn_lowest_level, afm_dom->afm_warn_level_mask)) {
		...
		if (afm_dom->pmic_update_reg(afm_dom->pmic_no, afm_dom->pmic_afm_level_offset,
					afm_dom->orig_afm_warn_level, afm_dom->afm_warn_level_mask)) {
			pr_err("Failed to update pmic reg");
			return -1;
		}
...
static int afm_dt_parsing(struct device_node *dn, struct afm_domain *afm_dom)
{
	...
	ret |= of_property_read_u32(dn, "pmic-afm-en-offset", &afm_dom->pmic_afm_en_offset);
	ret |= of_property_read_u32(dn, "pmic-afm-level-offset", &afm_dom->pmic_afm_level_offset);
	ret |= of_property_read_u32(dn, "lowest-level", &afm_dom->afm_warn_lowest_level);
	ret |= of_property_read_u32(dn, "level-mask", &afm_dom->afm_warn_level_mask);
	ret |= of_property_read_u32(dn, "htu-base-addr", &afm_dom->base_addr);
	ret |= of_property_read_u32(dn, "pmic", &afm_dom->pmic);

	ret |= of_property_read_string(dn, "sibling-cpus", &buf);
	if (ret)
		return ret;

	if (of_property_read_u32(dn, "pmic-no", &afm_dom->pmic_no))
		afm_dom->pmic_no = DEFAULT_PMIC_NO;
...
static int init_afm_domain(struct device_node *dn, struct platform_device *pdev)
{
	...
	if (afm_dom->pmic_read_reg(afm_dom->pmic_no, afm_dom->pmic_afm_level_offset, &orig_pmic_reg)) {
		dev_err(&pdev->dev, "Failed to get pmic afm_level register\n");
		return -ENODEV;
	}
	afm_dom->orig_afm_warn_level = orig_pmic_reg & afm_dom->afm_warn_level_mask;

	if (afm_dom->pmic_read_reg(afm_dom->pmic_no, afm_dom->pmic_afm_en_offset, &orig_pmic_reg)) {
		dev_err(&pdev->dev, "Failed to get pmic afm_en register\n");
		return -ENODEV;
	}
	afm_dom->orig_afm_enable = orig_pmic_reg & (1 << S2MPS_AFM_WARN_EN_SHIFT);
...
static int check_regulator(struct device_node *dn)
{
	struct device_node *child;
	unsigned int pmic, pmic_no;

	for_each_child_of_node(dn, child) {
		if (of_property_read_u32(child, "pmic", &pmic))
			return -EINVAL;
		if (of_property_read_u32(child, "pmic-no", &pmic_no))
			pmic_no = DEFAULT_PMIC_NO;

		if (pmic == MAIN_PMIC) {
			if (!chk_main_pmic_info(pmic_no))
				return -EPROBE_DEFER;
		} else if (pmic == SUB_PMIC) {
			if (!chk_sub_pmic_info(pmic_no))
				return -EPROBE_DEFER;
		} else {
			return -EINVAL;
		}
...
static void exynos_afm_free_domain(struct platform_device *pdev, struct exynos_afm_data *afm_data)
{
	...
	while (!list_empty(&afm_data->afm_domain_list)) {
		afm_dom = list_last_entry(&afm_data->afm_domain_list, struct afm_domain, list);

		list_del(&afm_dom->list);
		...
		freq_qos_tracer_remove_request(&afm_dom->max_qos_req);

		if (afm_dom->kobj.ktype)
			kobject_del(&afm_dom->kobj);

		if (afm_dom->stats) {
			if (afm_dom->stats->time_in_state)
				kfree(afm_dom->stats->time_in_state);
			kfree(afm_dom->stats);
		}
