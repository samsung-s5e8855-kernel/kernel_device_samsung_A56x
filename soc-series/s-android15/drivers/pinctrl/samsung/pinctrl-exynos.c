 ...
 */
static const struct exynos_irq_chip exynos_wkup_irq_chip __initconst = {
	.chip = {
...
int exynos_eint_wkup_init(struct samsung_pinctrl_drv_data *d)
{
	...
	for (i = 0; i < d->nr_banks; ++i, ++bank) {
		...
		for (idx = 0; idx < bank->nr_pins; ++idx) {
			...
			if (!of_device_is_available(to_of_node(bank->fwnode))) {
			 ...
			 */
			usi_cnt = of_count_phandle_with_args(to_of_node(bank->fwnode), "usi", NULL);
			for (i = 0; i < usi_cnt; ++i) {
				usi_node = of_parse_phandle(to_of_node(bank->fwnode), "usi", i);
