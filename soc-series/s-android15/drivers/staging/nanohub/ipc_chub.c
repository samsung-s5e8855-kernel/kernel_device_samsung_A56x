...
void ipc_set_chub_boottime(struct device *dev, bool alive_mct_enabled)
{
	...
	if (alive_mct_enabled) {
		...
		if (!time_gap) {
#ifdef CONFIG_RISCV
			u32 mct_alive_base, mct_alive_offset, cnt_u_pre, cnt_u_cur, cnt_l;
			void *mct_alive = NULL;

			np = of_find_compatible_node(NULL, NULL, "samsung,exynos-nanohub");

			if (!dev) {
				CSP_PRINTF_INFO("%s: fail to get device info.\n", __func__);
				return;
			}

			if (of_property_read_bool(np, "chub_mct_alive_base")) {
				of_property_read_u32(np, "chub_mct_alive_base", &mct_alive_base);
			} else {
				CSP_PRINTF_INFO("cannot find property.(chub_mct_alive_base)\n");
				return;
			}

			if (of_property_read_bool(np, "chub_mct_alive_offset")) {
				of_property_read_u32(np, "chub_mct_alive_offset", &mct_alive_offset);
			} else {
				CSP_PRINTF_INFO("cannot find property.(chub_mct_alive_offset)\n");
				return;
			}

			mct_alive = devm_ioremap(dev, mct_alive_base + mct_alive_offset, 0x10);

			cnt_u_cur = readl_relaxed(mct_alive + 0x4);        // MCT_ALIVE_UPPER.
			do {
				cnt_u_pre = cnt_u_cur;
				cnt_l = readl_relaxed(mct_alive);          // MCT_ALIVE LOWER.
				cnt_u_cur = readl_relaxed(mct_alive + 0x4);// MCT_ALIVE_UPPER
			} while (cnt_u_pre != cnt_u_cur);
			ap = (((u64) cnt_u_cur) << 32) | cnt_l;
			...
			if (np)
				...
				np = of_find_node_by_name(NULL, "mct_alive");
				if (np) {
					struct clk *tick_clk;
					int div, ret;
					...
					if (ret || !div) {
						CSP_PRINTF_INFO("MCT: Fail to get the div value. set div to the default\n");
						div = DEFAULT_CLK_DIV;
					}
					tick_clk = of_clk_get_by_name(np, "fin_pll");
					if (!IS_ERR(tick_clk)) {
						rate = clk_get_rate(tick_clk);
						CSP_PRINTF_INFO("MCT: clk_get_rate = %u div = %d\n", rate, div);
						rate = rate / div;
					} else {
						CSP_PRINTF_INFO("MCT: Fail to get clock info\n");
					}
				}
			}
			if (!rate) {
...
void ipc_write_shared_reg(uint32_t reg, u32 value)
{
	if (reg >= SR_MAX)
		return;
	ipc_hw_write_shared_reg(ipc.mb_base, value, reg);
}
...
bool ipc_chub_cache_flush_done(void)
{
#if defined(CACHE_SUPPORT)
	struct ipc_map_area *map = ipc_get_base(IPC_REG_IPC);
	return map->cache.flush == CHUB_CACHE_FLUSH_DONE ? true : false;
#else
	return true;
#endif //CACHE_SUPPORT
}

void ipc_chub_set_cache_flush_status(bool flushed)
{
#if defined(CACHE_SUPPORT)
	enum chub_cache_flush_req req = flushed ? CHUB_CACHE_FLUSH_DONE : CHUB_CACHE_FLUSH_REQ;
    ipc.ipc_map->cache.flush = req;
#endif
}
