...
static inline void CIPC_USER_MEMCPY(void *dst, void *src, int size, int dst_io,
				    int src_io)
{
	cipc.user_func->user_memcpy(dst, src, size, dst_io, src_io);
}

static inline void CIPC_USER_STRNCPY(char *dst, char *src, int size, int dst_io,
				     int src_io)
{
	cipc.user_func->user_strncpy(dst, src, size, dst_io, src_io);
}

static inline int CIPC_USER_STRNCMP(char *dst, char *src, int size)
{
	return cipc.user_func->user_strncmp(dst, src, size);
}
...
{
	cipc.user_func->user_memset(buf, val, size, io);
}
...
VISIBLE_IF_KUNIT inline enum cipc_region user_to_reg(enum cipc_user_id id)
{
	switch (id) {
...
EXPORT_SYMBOL_IF_KUNIT(user_to_reg);
...
static int cipc_set_map(int master)
{
	...
	if (CIPC_USER_STRNCMP(cipc.chub_bootargs->magic, OS_UPDT_MAGIC, (int)sizeof(OS_UPDT_MAGIC))) {
...
if (reg == CIPC_REG_DATA_CHUB2ABOX) {
	...
	if (start) {
		...
		if (len != sizeof(ipc_testbuf)) {
			return CIPC_ERR;
		} else if (!CIPC_USER_STRNCMP(local_ipc_testbuf.magic, IPC_TEST_MAGIC, (int)sizeof(IPC_TEST_MAGIC))) {
