...
struct contexthub_symbol_table {
	...
	struct contexthub_symbol_addr symbol[];
};
...
struct chub_baracs {
	...
	void __iomem *addr;
	unsigned int size;
	unsigned int values[BARAC_MAX_WINDOWS * 5];
};
...
struct chub_iomem {
	...
	struct chub_baracs chub_baracs[BARAC_MAX];
};
