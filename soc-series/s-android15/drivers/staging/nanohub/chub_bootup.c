...
{
	int ret;
	struct chub_ext *ext = container_of(to_delayed_work(work),
					       struct chub_ext,
					       sensor_alive_work);

	struct contexthub_ipc_info *ipc = container_of(ext,
	...
	schedule_delayed_work(&ext->sensor_alive_work, msecs_to_jiffies(SENSORTASK_KICK_MS));
}
#endif
...
static void contexthub_set_barac(struct contexthub_ipc_info *chub)
{
	int i, j, offset, ret;
	...
	for (j = 0 ; j < BARAC_MAX; j++) {
		if (IS_ERR_OR_NULL(chub->iomem.chub_baracs[j].addr))
			break;

		for (i = 0 ; i < chub->iomem.chub_baracs[j].size * 5; i++) {
			if (!(i % 5)) {
				offset = chub->iomem.chub_baracs[j].values[i];
				continue;
			}
			nanohub_dev_debug(chub->dev, "%s: %s offset +%x %x\n", __func__,
					  chub->iomem.chub_baracs[j].name,
					  offset, chub->iomem.chub_baracs[j].values[i]);
			__raw_writel(chub->iomem.chub_baracs[j].values[i], chub->iomem.chub_baracs[j].addr + offset);
			offset += 4;
		}
		__raw_writel(BA_CTRL_UNMAP_BLOCK_ENABLE, chub->iomem.chub_baracs[j].addr + REG_BARAC_BA_CTRL_OFFSET);
		__raw_writel(BA_IRQ_MASK_IRQ_MASK, chub->iomem.chub_baracs[j].addr + REG_BARAC_BA_IRQ_MASK_OFFSET);
		__raw_writel(BA_LOG_CTRL_AW_CAPT_MODE_FIRST | BA_LOG_CTRL_AW_CAPT_EN_ENABLE |
				BA_LOG_CTRL_AR_CAPT_MODE_FIRST| BA_LOG_CTRL_AR_CAPT_EN_ENABLE
			     , chub->iomem.chub_baracs[j].addr + REG_BARAC_BA_LOG_CTRL_OFFSET);
	}
}

static void contexthub_set_address_remapper(struct contexthub_ipc_info *chub)
{
	nanohub_dev_info(chub->dev, "%s: tzpc and %s set\n", __func__, chub->iomem.is_baaw ? "baaw": "barac");

	if (chub->iomem.is_baaw)
		contexthub_set_baaw(chub);
	else
		contexthub_set_barac(chub);
}
...
static void contexthub_imgloader_checker(struct imgloader_desc *desc, phys_addr_t fw_phys_base, size_t fw_bin_size, size_t fw_mem_size) {
	...
	nanohub_dev_info(desc->dev, "=====compare SRAM and F/W binary=====\n");
	nanohub_dev_info(desc->dev, "loaded fw name : %s\n", desc->fw_name);
	nanohub_dev_info(desc->dev, "loaded fw size : 0x%X\n", fw_bin_size);
	...
	sram_phy = devm_ioremap(desc->dev, fw_phys_base, fw_mem_size);
	if (!sram_phy) {
		nanohub_dev_err(desc->dev, "fail to ioremap for sram\n");
		goto err2;
	}

	sram = (uint8_t*)kmalloc(fw_bin_size, GFP_KERNEL);
	if (!sram) {
		nanohub_dev_err(desc->dev, "fail to alloc for fw_bin\n");
		goto err3;
	}

	memcpy_fromio(sram, sram_phy, fw_bin_size);

	for (int i = 0; i < fw_bin_size; i++) {
		if (sram[i] != entry->data[i]) {
			nanohub_dev_info(desc->dev, "diff offset : 0x%x\n", i);
			nanohub_dev_info(desc->dev, "sram val : 0x%x, bin val : 0x%x\n", sram[i], entry->data[i]);
			diff_check++;
		}
	...
	if (diff_check == 0)
		nanohub_dev_info(desc->dev, "no diff between chub sram and binary\n");
	else
		nanohub_dev_err(desc->dev, "has %d diff between chub sram and binary\n", diff_check);
...
err1:
	...
	nanohub_dev_info(desc->dev, "===compare SRAM and F/W binary end===\n");
}
						 ...
						 size_t *fw_bin_size, size_t *fw_mem_size)
{
	...
	*fw_bin_size = size;
	*fw_mem_size = chub->iomem.sram_size - SZ_16K;
		     ...
		     enum chub_err_type err)
{
	...
	if (!atomic_read(&chub->atomic.reset_ctrl)) {
		nanohub_dev_info(chub->dev, "reset is disabled. skip this.\n");
		return 0;
	}
	...
	ipc_hw_write_shared_reg(chub->iomem.mailbox, 0, SR_3);
	/* shutdown */
	...
	/* set boottime */
	ipc_set_chub_boottime(chub->dev, chub->misc.alive_mct);
...
int contexthub_bootup_init(struct contexthub_ipc_info *chub)
{
	...
	contexthub_set_address_remapper(chub);
#if IS_ENABLED(CONFIG_EXYNOS_S2MPU)
...
#ifdef CONFIG_CONTEXTHUB_SENSOR_DEBUG
	INIT_DELAYED_WORK(&chub->ext.sensor_alive_work, sensor_alive_check_func);
#endif
