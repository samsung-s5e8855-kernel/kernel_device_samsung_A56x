...
static ssize_t chub_reset_ctrl_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	...
	if (atomic_read(&chub->atomic.reset_ctrl))
		return sprintf(buf, "1: reset is enabled\n");
	else
		return sprintf(buf, "0: reset is disabled\n");

}

static ssize_t chub_reset_ctrl_store(struct device *dev,
			      struct device_attribute *attr,
			      const char *buf, size_t count)
{
	struct nanohub_data *data = dev_get_nanohub_data(dev);
	struct contexthub_ipc_info *chub = data->pdata->mailbox_client;
	int val;
	int err = kstrtoint(&buf[0], 10, &val);

	nanohub_dev_info(dev, "%s: reset_ctrl:%d\n", __func__, val);
	if (!err) {
		if (val)
			atomic_set(&chub->atomic.reset_ctrl, 1);
		else
			atomic_set(&chub->atomic.reset_ctrl, 0);
...
static struct device_attribute attributes[] = {
	...
	__ATTR(reset_ctrl, 0660, chub_reset_ctrl_show, chub_reset_ctrl_store),
