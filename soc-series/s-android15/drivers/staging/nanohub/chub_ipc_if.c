	...
	case CIPC_REG_DATA_CHUB2AP:
		{
			...
			if (size < PACKET_SIZE_MAX) {
				...
				if (!rx_buf) {
					nanohub_dev_err(chub->dev, "%s: kzalloc to rx_buf(%d)\n", __func__, size);
				} else {				
				...
				nanohub_dev_err(chub->dev, "%s: PACKET_SIZE_MAX(%d) size(%d)\n", __func__, PACKET_SIZE_MAX, size);
			}
...
struct cipc_funcs cipc_func = {
	...
	.user_memcpy = cipc_func_memcpy,
	.user_memset = cipc_func_memset,
	...
	.user_strncmp = cipc_func_strncmp,
	.user_strncpy = cipc_func_strncpy,
...
int contexthub_get_sensortype(struct contexthub_ipc_info *chub, char *buf)
{
	...
	return (int)sizeof(struct sensor_map_pack);
}
...
bool contexthub_request_chub_flush_cache(struct contexthub_ipc_info *chub)
{
	...
	if (IS_ERR_OR_NULL(chub)) {
		nanohub_info("%s:invalied contexthub ipc info\n", __func__);
		return false;
	}

	if (ipc_hw_read_shared_reg(chub->iomem.mailbox, SR_3) == 0xFFFFFFFF) {
		nanohub_info("%s:HardFault handler has already flushed cache. skip the request.\n", __func__);
		return true;
	}

	nanohub_info("%s:request chub to flush cache\n", __func__);
	...
	ipc_chub_set_cache_flush_status(false);
	cipc_add_evt(CIPC_REG_EVT_AP2CHUB, IRQ_EVT_A2C_CACHE_FLUSH);

	do {
		if(ipc_chub_cache_flush_done()) {
			contexthub_put_token(chub);
			nanohub_info("%s:flush cache Done\n", __func__);
			return true;
		}
		mdelay(10);
	} while (count --);
	...
	contexthub_put_token(chub);
	nanohub_info("%s:flush cache in failed\n", __func__);
	return false;
#else
	return true;
#endif
