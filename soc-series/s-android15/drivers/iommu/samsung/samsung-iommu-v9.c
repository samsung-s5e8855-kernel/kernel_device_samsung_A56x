...
VISIBLE_IF_KUNIT void samsung_iommu_deinit_log(struct samsung_iommu_log *log)
{
	struct page *page;
...
EXPORT_SYMBOL_IF_KUNIT(samsung_iommu_deinit_log);

VISIBLE_IF_KUNIT int samsung_iommu_init_log(struct samsung_iommu_log *log, int len)
{
	struct page *page;
...
EXPORT_SYMBOL_IF_KUNIT(samsung_iommu_init_log);
...
#define IOVAD_VH_BESTFIT	BIT(0)
#define IOVAD_VH_NO_SIZE_ALIGN	BIT(1)
static void samsung_sysmmu_dma_info_to_prot(void *data, unsigned long attrs, int *prot)
{
					  ...
					  struct iova *new_iova, bool size_aligned, int *ret)
{
	...
	if (!(iovad->android_vendor_data1 & IOVAD_VH_BESTFIT)) {
		*ret = -EINVAL;
		return;
	}
...
static void limit_align_shift(void *data, struct iova_domain *iovad, unsigned long size,
			      unsigned long *shift)
{
	if (iovad->android_vendor_data1 & IOVAD_VH_NO_SIZE_ALIGN)
		*shift = 0;
}
...
static void iovad_init_best_fit(void *data, struct device *dev, struct iova_domain *iovad)
{
	if (of_property_read_bool(dev->of_node, "sysmmu,best-fit"))
		iovad->android_vendor_data1 |= IOVAD_VH_BESTFIT;

	if (of_property_read_bool(dev->of_node, "sysmmu,no-size-align"))
		iovad->android_vendor_data1 |= IOVAD_VH_NO_SIZE_ALIGN;
}

struct memlog *sysmmu_memlog_desc;
struct memlog_obj *sysmmu_memlog_obj;
static void __init samsung_sysmmu_memlog_register(void)
{
	int ret;

	ret = memlog_register("iommu", &sync_dev, &sysmmu_memlog_desc);
	if (ret) {
		pr_info("%s: failed to register memlog, %d\n", __func__, ret);
		return;
	}
	sysmmu_memlog_obj = memlog_alloc_printf(sysmmu_memlog_desc, SZ_4K, NULL, "fault", 0);
	if (!sysmmu_memlog_obj)
		pr_info("%s: failed to allocate memlog buffer\n", __func__);
}
...
static int __init samsung_sysmmu_init(void)
{
	...
	register_trace_android_rvh_iommu_dma_info_to_prot(samsung_sysmmu_dma_info_to_prot, NULL);
	...
	register_trace_android_rvh_iommu_limit_align_shift(limit_align_shift, NULL);
	...
	samsung_sysmmu_memlog_register();
