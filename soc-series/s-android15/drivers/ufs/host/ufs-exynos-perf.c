		...
		ufs_perf_op op)
{
	...
	int index;
	struct ufs_perf_v2 *stat_v2 = &perf->stat_v2;
	unsigned long len;
...
void ufs_init_cpufreq_request(struct ufs_perf *perf, bool add_knob)
{
	...
	for (i = 0; i < perf->num_clusters; i++) {
...
#if IS_ENABLED(CONFIG_EXYNOS_CPU_FREQ) || IS_ENABLED(CONFIG_FREQ_QOS_TRACER)
		if (add_knob)
		...
		pr_info("%s: cluster %d = %u\n", __func__, i, pos->frequency);
	}
}
...
#elif IS_ENABLED(CONFIG_EXYNOS_CPU_FREQ)
						...
						enum ufs_perf_request_mode mode)
{
	...
	if (perf->gear_scale_sup && prev != perf->exynos_gear_scale) {
