...
extern bool ufs_is_suspend_prepare(struct exynos_ufs *ufs);
void exynos_get_devfreq_noti(struct work_struct *work)
{
	struct ufs_perf_v2 *stat = container_of(to_delayed_work(work),
				struct ufs_perf_v2, devfreq_work);
...
static int exynos_ufs_reboot_handler(struct notifier_block *nb,	unsigned long l,
				void *p)
{
	struct ufs_perf_v2 *stat = container_of(nb,
				struct ufs_perf_v2, reboot_nb);
...
static int ufs_gear_change(struct ufs_hba *hba, bool en)
{
	struct exynos_ufs *ufs = to_exynos_ufs(hba);
	struct uic_pwr_mode *pmd = &ufs->hci_pmd_parm;
	struct uic_pwr_mode *act_pmd = &ufs->device_pmd_parm;
	struct ufs_perf *perf = ufs->perf;
	struct ufs_perf_v2 *stat = &perf->stat_v2;
	struct irq_inform *vendor_irq = &ufs->vendor_irq;
	unsigned long flags;
	int prev, ret = 0, val;
	bool is_suspend_prepare = ufs_is_suspend_prepare(ufs);

	if (is_suspend_prepare)
		return ret;
	...
	val = ufs->pm_qos_int_value[pmd->gear];
...
{
	struct ufs_perf_v2 *stat = container_of(work,
			struct ufs_perf_v2, gear_work);
	struct ufs_perf *perf = container_of(stat, struct ufs_perf, stat_v2);
	struct ufs_hba *hba = perf->hba;
	unsigned long flags;
	int ret = 0;
...
static policy_res __update_v2(struct ufs_perf *perf, u32 chunk_size,
			ufs_perf_op op, ufs_perf_entry entry)
{
#if 0
	struct ufs_perf_v1 *stat_v1 = &perf->stat_v1;
	struct ufs_perf_v2 *stat = &perf->stat_v2;
	unsigned long flags;
	traffic traffic = 0;
...
{
	struct ufs_perf_v2 *stat2 = &perf->stat_v2;
	int ret = 0;
