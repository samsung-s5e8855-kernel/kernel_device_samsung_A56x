...
{									\
	writel(val, VM_SFR(handle->name + ofs));			\
}									\
static inline u32 name##_readl(struct ufs_vs_handle *handle, u32 ofs)	\
{									\
	return readl(VM_SFR(handle->name + ofs));			\
}
#else
#define EXYNOS_UFS_MMIO_FUNC(name)					\
static inline void name##_writel(struct ufs_vs_handle *handle, u32 val, u32 ofs)	\
...
{
	u32 clkstop_ctrl = readl(VM_SFR(handle->std + MISC_CAL);

	writel(clkstop_ctrl & ~MPHY_APBCLK_CAL, VM_SFR(handle->std + MISC_CAL));
	writel(val, VM_SFR(handle->pma + ofs));
	writel(clkstop_ctrl | MPHY_APBCLK_CAL, VM_SFR(handle->std + MISC_CAL));
}
...
static inline u32 pma_readl(struct ufs_vs_handle *handle, u32 ofs)
{
	...
	writel(clkstop_ctrl & ~MPHY_APBCLK_CAL, VM_SFR(handle->std + MISC_CAL));
	val = readl(VM_SFR(handle->pma + ofs));
	writel(clkstop_ctrl | MPHY_APBCLK_CAL, VM_SFR(handle->std + MISC_CAL));
	return val;
}
#else
