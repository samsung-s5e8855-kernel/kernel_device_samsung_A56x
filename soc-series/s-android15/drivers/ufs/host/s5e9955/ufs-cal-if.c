...
static struct _ufs_cal_mgr {
	u32 oc_save_complete[GEAR_MAX + GEAR_1];
...
ufs_snr_save_pma_oc_code(struct ufs_vs_handle *handle,
			 struct ufs_cal_param *p, u32 hs_gear)
{
	u32 i, j, loop_cnt;
	struct ufs_cal_phy_cfg *cfg;
	struct _ufs_cal_mgr *mgr = &ufs_cal_mgr;

	hs_gear = hs_gear - PMD_HS_G1 + GEAR_1;
	if (mgr->oc_save_complete[hs_gear])
		return UFS_CAL_NO_ERROR;
	...
	for (i = 0; i < p->available_lane; i++) {
		...
		for (j = 0; j < loop_cnt; j++) {
			...
			if (cfg->lyr != PHY_PMA_TRSV)
				return UFS_CAL_ERROR;
			p->backup_oc_code[hs_gear][i][j] =
				pma_readl(handle,
					  PHY_PMA_TRSV_ADDR(cfg->addr, i));
			cfg++;
		}
	}
	mgr->oc_save_complete[hs_gear] = 1;

	return UFS_CAL_NO_ERROR;
}

static ufs_cal_errno
ufs_cal_set_pma_from_otp_hedge(struct ufs_vs_handle *handle,
							struct ufs_cal_param *p)
{
	u8 otp_data, pma_temp_data, pma_data;
	otp_hedge_pma_cfg *cfg;

	cfg = pma_otp_hedge;

	if (p->pma_otp[PMA_OTP_SIZE-1] != 0x0F)
		return UFS_CAL_NO_ERROR;

	for (; cfg->etype != PMA_SET_TYPE_NONE; cfg++) {
		otp_data = p->pma_otp[cfg->otp_addr - PMA_OTP_BASE];
		otp_data = (otp_data & (cfg->otp_mask << cfg->otp_bitpos)) >> cfg->otp_bitpos;

		switch(cfg->etype) {
			case PMA_SET_TYPE_CMN:
				pma_temp_data = (u8)pma_readl(handle, cfg->pma_addr);
				pma_temp_data = pma_temp_data & (~(cfg->pma_mask << cfg->pma_bitpos));
				pma_data = pma_temp_data | (otp_data << cfg->pma_bitpos);

				pma_writel(handle, (u32)pma_data,  PHY_PMA_COMN_ADDR(cfg->pma_addr));
				/*printf("{0x%x,0x%x,0x%02x,0x%02x,0x%04x,0x%03x,0x%02x},CMN  R OTP[0x%02x] W PMA[0x%02x]\n",cfg->etype, cfg->otp_addr, cfg->otp_mask, cfg->otp_bitpos, cfg->pma_addr, cfg->pma_mask, cfg->pma_bitpos, otp_data, pma_data);*/
				break;

			case PMA_SET_TYPE_TRSV:
				/*Caution....!!!!
				/TRSV_lane0/1 setting is all included in cfg(pma_otp_hedge) table.
				/So, lane para. of the rMPHY_PMA_TRSV_ADDR(offset, lane) is fixed to 0.*/
				pma_temp_data = (u8)pma_readl(handle, cfg->pma_addr);
				pma_temp_data = pma_temp_data & (~(cfg->pma_mask << cfg->pma_bitpos));
				pma_data = pma_temp_data | (otp_data << cfg->pma_bitpos);

				pma_writel(handle, (u32)pma_data,  PHY_PMA_TRSV_ADDR(cfg->pma_addr, 0));
				/*printf("{0x%x,0x%x,0x%02x,0x%02x,0x%04x,0x%03x,0x%02x},TRSV R OTP[0x%02x] W PMA[0x%02x]\n",cfg->etype, cfg->otp_addr, cfg->otp_mask, cfg->otp_bitpos, cfg->pma_addr, cfg->pma_mask, cfg->pma_bitpos, otp_data, pma_data);*/
				break;
				       ...
				       struct ufs_cal_param *p)
{
	...
	case PHY_PMA_OC_CODE:
		...
		if (ufs_snr_save_pma_oc_code(handle, p, cfg->flg) == UFS_CAL_ERROR)
			ret = UFS_CAL_TIMEOUT;
#endif
		break;
	case PHY_PMA_OTP_HEDGE:
		/* read OTP */
		if (ufs_cal_set_pma_from_otp_hedge(handle, p) == UFS_CAL_ERROR)
			ret = UFS_CAL_TIMEOUT;
		break;
	default:
		break;
...
ufs_cal_errno ufs_cal_pre_pmc(struct ufs_cal_param *p)
{
	...
	struct _ufs_cal_mgr *mgr = &ufs_cal_mgr;
	int len_tbl = sizeof(ufs_cal_vnd_tbl) / sizeof(struct __ufs_cal_vnd_tbl *);
	...
	for (i = 0; i < len_tbl; i++) {
		...
		if (mgr->vnd_tbl_cached)
			break;

		/*
		 * check part id and manufactorer id
		 * if part id is UFS_ANY_MODEL, not check it
		 */
		if (p_vnd_tbl->vendor == p->vendor &&
		    (!strcmp(p_vnd_tbl->model, UFS_ANY_MODEL) ||
		     !strcmp(p_vnd_tbl->model, p->model)))
			ufs_cal_mgr.vnd_tbl_cached = p_vnd_tbl->tbl;
	}

	/* override */
	if (ufs_cal_mgr.vnd_tbl_cached)
		__override_table(cfg, ufs_cal_mgr.vnd_tbl_cached);
...
ufs_cal_errno ufs_cal_pre_link(struct ufs_cal_param *p)
{
	...
	for (k = GEAR_1; k <= GEAR_MAX; k++)
		for (i = 0; i < p->available_lane; i++)
			for (j = 0; j < PMA_OC_CODE_SAVECNT; j++)
				p->backup_oc_code[k][i][j] = 0x80;
...
ufs_cal_errno ufs_cal_init(struct ufs_cal_param *p)
{
	...
	if (p->overwrite) {
		...
		cfg = p->evt_ver ? init_cfg_evt1 : init_cfg_evt0;
		__override_table(cfg, cfg_over);
	}

	for (int i = 0; i < GEAR_MAX + GEAR_1; i++)
		mgr->oc_save_complete[i] = 0;
