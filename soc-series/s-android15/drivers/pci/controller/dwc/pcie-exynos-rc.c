		...
		unsigned long attrs)
{
	...
	if (coherent_ptr != NULL) {
		ret = pcie_iommu_map(*dma_handle, *dma_handle, size,
				0, ch_num);
		if (ret != 0) {
			dev_err(dev, "DMA alloc - Can't map PCIe SysMMU table!!!\n");
		...
		unsigned long attrs)
{
	...
	if (dev_addr != DMA_MAPPING_ERROR) {
		...
		if (ret != 0) {
			dev_err(dev, "DMA map - Can't map PCIe SysMMU table!!!\n");
...
void exynos_pcie_set_ready_cto_recovery(int ch_num)
{
	...
	val = exynos_elbi_read(exynos_pcie, PCIE_APP_LTSSM_ENABLE);
	val &= ~(0x1 << 0);
	exynos_elbi_write(exynos_pcie, val, PCIE_APP_LTSSM_ENABLE);
...
static int exynos_pcie_rc_parse_dt(struct exynos_pcie *exynos_pcie, struct device *dev)
{
	...
	if (of_property_read_u32(np, "pmu-hw-ctrl-en",
				&exynos_pcie->pmu_hw_ctrl_en)) {
		pcie_err("Failed to parse PMU HW CTRL EN offset\n");
	}
	...
	if (!of_property_read_string(np, "use-l1ss", &use_l1ss)) {
		if (!strcmp(use_l1ss, "true")) {
			pcie_info("## PCIe L1SS is ENABLED.\n");
			exynos_pcie->use_l1ss = true;
		} else {
			pcie_info("## PCIe L1SS is DISABLED.\n");
	...
	/* set dbg range */
	err = of_property_read_u32_array(np, "elbi", dbg_range,
						ARRAY_SIZE(dbg_range));
	if (err) {
		dev_err(dev, "Failed to parse the number of elbi\n");
		exynos_pcie->dbg_range_elbi.start = 0x0;
		exynos_pcie->dbg_range_elbi.end = 0x0;
	} else {
		exynos_pcie->dbg_range_elbi.start = dbg_range[0];
		exynos_pcie->dbg_range_elbi.end = dbg_range[1];
	}

	err = of_property_read_u32_array(np, "phy", dbg_range,
						ARRAY_SIZE(dbg_range));
	if (err) {
		dev_err(dev, "Failed to parse the number of phy\n");
		exynos_pcie->dbg_range_phy.start = 0x0;
		exynos_pcie->dbg_range_phy.end = 0x0;
	} else {
		exynos_pcie->dbg_range_phy.start = dbg_range[0];
		exynos_pcie->dbg_range_phy.end = dbg_range[1];
	}

	err = of_property_read_u32_array(np, "pcs", dbg_range,
						ARRAY_SIZE(dbg_range));
	if (err) {
		dev_err(dev, "Failed to parse the number of phy\n");
		exynos_pcie->dbg_range_pcs.start = 0x0;
		exynos_pcie->dbg_range_pcs.end = 0x0;
	} else {
		exynos_pcie->dbg_range_pcs.start = dbg_range[0];
		exynos_pcie->dbg_range_pcs.end = dbg_range[1];
	}

	err = of_property_read_u32_array(np, "udbg", dbg_range,
						ARRAY_SIZE(dbg_range));
	if (err) {
		dev_err(dev, "Failed to parse the number of phy\n");
		exynos_pcie->dbg_range_udbg.start = 0x0;
		exynos_pcie->dbg_range_udbg.end = 0x0;
	} else {
		exynos_pcie->dbg_range_udbg.start = dbg_range[0];
		exynos_pcie->dbg_range_udbg.end = dbg_range[1];
	}

	err = of_property_read_u32_array(np, "dbi", dbg_range,
						ARRAY_SIZE(dbg_range));
	if (err) {
		dev_err(dev, "Failed to parse the number of phy\n");
		exynos_pcie->dbg_range_dbi.start = 0x0;
		exynos_pcie->dbg_range_dbi.end = 0x0;
	} else {
		exynos_pcie->dbg_range_dbi.start = dbg_range[0];
		exynos_pcie->dbg_range_dbi.end = dbg_range[1];
	}
	...
	/* SSD & WIFI power control */
	exynos_pcie->ep_reset_gpio = of_get_named_gpio(np, "pcie,ep-reset-gpio", 0);
	if (exynos_pcie->ep_reset_gpio < 0) {
		pcie_err("wlan reset gpio is not defined -> don't use wifi through pcie#%d\n",
				exynos_pcie->ch_num);
	} else {
				       ...
				       struct platform_device *pdev)
{
	...
	if (exynos_pcie->use_ia) {
		...
		temp_rsc = platform_get_resource_byname(pdev, IORESOURCE_MEM, "ia2");
		exynos_pcie->ia2_base =
				devm_ioremap_resource(&pdev->dev, temp_rsc);
		if (IS_ERR(exynos_pcie->ia2_base))
			pcie_err("IA2 is not defined.\n");
	}
...
static irqreturn_t exynos_pcie_rc_irq_handler(int irq, void *arg)
{
	...
	struct dw_pcie *pci = to_dw_pcie_from_pp(pp);
	struct exynos_pcie *exynos_pcie = to_exynos_pcie(pci);
	u32 val_irq0, val_irq1, val_irq2, val, i;
	...
	for (i = 0; i < 10; i++) {
		val = exynos_soc_read(exynos_pcie, PCIE_QCH_CTRL_DEBUG_1);
		val &= SLV_BRESP_RREST_PEND_MASK;
		if (val == 0x0)
			break;
	}
	...
	if (val != 0x0) {
		val = exynos_soc_read(exynos_pcie, PCIE_QCH_CTRL_DEBUG_1);
		pcie_info("Check SLV_XFER_PEND - %#x\n", val);
	}
						...
						int where, u32 *val)
{
	...
	if (where == PCI_CLASS_REVISION && (*val >> 8) == 0x0) {
		dev_dbg(&pdev->dev,
			"CLASS REV. of CP is 0x0, Fake return as a Network device...\n");
	...
	if (where == pdev->msi_cap + PCI_MSI_FLAGS &&
		...
		dev_dbg(&pdev->dev,
			"MSI_FLAGS_QMASK of CP is 0x0, Fake return as supporting 16 MSI...\n");
		*val |= (0x4 << 1);
	}
							...
							int where, u32 *val)
{
	...
	if (where == PCI_CLASS_REVISION && (*val >> 8) == 0x0) {
		dev_info(&pdev->dev,
			"CLASS REV. of SamsungWIFI is 0x0, Fake return as a Network device...\n");
		*val |=(PCI_CLASS_NETWORK_OTHER << 16);
	}
	...
	if (where == pdev->msi_cap + PCI_MSI_FLAGS &&
		(*val & PCI_MSI_FLAGS_QMASK) == 0x0) {
		dev_info(&pdev->dev,
			"MSI_FLAGS_QMASK of SamsungWIFI is 0x0, Fake return as supporting 16 MSI...\n");
		*val |= (0x5 << 1);
	}
}
...
static void exynos_pcie_rc_send_pme_turn_off(struct exynos_pcie *exynos_pcie)
{
	...
	val = exynos_elbi_read(exynos_pcie, PCIE_ELBI_RDLH_LINKUP) & 0x1f;
	pcie_dbg("%s: LTSSM: 0x%x\n", __func__, val);
	if (!(val >= S_RCVRY_LOCK && val <= S_L1_IDLE)) {
		pcie_info("%s, pcie link is not up(0x%x)\n", __func__,
				val);
		return;
	}
...
static inline void exynos_pcie_rc_qch_control(struct exynos_pcie *exynos_pcie, int enable)
{
	...
	pcie_dbg("Qch control : %d\n", enable);

	if (enable) {
		val = exynos_elbi_read(exynos_pcie, 0x3A8);
		val &= ~(0xf << 4);
		val |= 0x7;
		exynos_elbi_write(exynos_pcie, val, 0x3A8);
		val = exynos_soc_read(exynos_pcie, 0x8010);
		val &= ~(0xf << 0);
		val |= 0x7;
		exynos_soc_write(exynos_pcie, val, 0x8010);
	} else {
		val = exynos_elbi_read(exynos_pcie, 0x3A8);
		val &= ~(0xf << 4);
		exynos_elbi_write(exynos_pcie, val, 0x3A8);
						...
						int enable)
{
	if (exynos_pcie->phy->dbg_ops != NULL) {
		if (enable)
			exynos_pcie->phy->dbg_ops(exynos_pcie, HISTORY_EN);
		else
			exynos_pcie->phy->dbg_ops(exynos_pcie, HISTORY_DIS);
	} else {
		pcie_err("SKIP History buffer control !!\n");
	}
}
...
recheck_speed:
	exynos_pcie_rc_rd_own_conf(pp, exynos_pcie->pci_dev->pcie_cap +
			PCI_EXP_LNKCTL, 4, &val);
	current_lane = (val >> 20) & 0x3f;
	current_speed = (val >> 16) & 0xf;
	pcie_dbg("Current Link Speed: GEN%d-%dlane(MAX GEN%d)\n",
	...
	if (current_speed < exynos_pcie->max_link_speed || val != S_L0) {
		...
		if (count < MAX_TIMEOUT) { /* Wait until MAX link-up time */
			count++;
			udelay(10);
			goto recheck_speed;
		}
...
skip_speed_check:
	...
	current_speed = (val >> 16) & 0xf;
	current_lane = (val >> 20) & 0x3f;
	pcie_info("Current Link Speed: GEN%d-%dlane(MAX GEN%d)\n",
			current_speed, current_lane, exynos_pcie->max_link_speed);
...
void exynos_pcie_rc_check_payload(int ch_num)
{
	struct exynos_pcie *exynos_pcie = &g_pcie_rc[ch_num];
	...
	if (rc_mps != ep_mps) {
		pcie_err("MPS(Max Payload Size) is NOT equal(rc:%d/ep:%d)\n", rc_mps, ep_mps);
...
EXPORT_SYMBOL_GPL(exynos_pcie_rc_check_payload);
...
int exynos_pcie_rc_poweron(int ch_num)
{
	...
	pcie_dbg("%s, start of poweron, pcie state:%d(%d)\n", __func__,
					exynos_pcie->state, exynos_pcie->target_speed);
	if (!exynos_pcie->is_ep_scaned) {
	...
	if (exynos_pcie->state == STATE_LINK_DOWN) {
		...
		pcie_dbg("### PCIe(HSI1) block power : %d (0:ACTIVE, 2:SUSPENDED)\n",
			  exynos_pcie->pci->dev->power.runtime_status);
		...
		exynos_pcie_rc_check_payload(exynos_pcie->ch_num);
	}
...
static int exynos_pcie_set_l1ss(struct exynos_pcie *exynos_pcie, int enable, int id)
{
	...
	pcie_dbg("%s:L1SS_START(l1ss_ctrl_id_state=%#x, id=%#x, enable=%d)\n",
...
int exynos_pcie_l1ss_ctrl(int enable, int id, int ch_num)
{
	...
	if (!exynos_pcie->use_l1ss) {
		pcie_err("%s: 'use_l1ss' is false in DT(not support L1.2)\n", __func__);
		return -EINVAL;
	}
...
int exynos_pcie_rc_speed_change(int ch_num, int req_speed)
{
	...
	while (count < 3000) {
		ltssm = exynos_elbi_read(exynos_pcie, PCIE_ELBI_RDLH_LINKUP) & 0x1f;
	...
	pcie_info("Start Speed Change %d --> %d (0x%x)\n", current_speed, req_speed, ltssm);
	...
	if (current_speed != req_speed) {
		...
		pcie_info("Speed Change Successful(0x%x, %d).\n", ltssm, current_speed);
...
int exynos_pcie_rc_lane_change(int ch_num, int req_lane)
{
	...
	struct dw_pcie *pci;
	struct dw_pcie_rp *pp;
	u32 current_lane, max_lane, ltssm;
	u32 val, i, count = 0;
	...
	/* Check LTSSM before lane change */
	while (count < 3000) {
		ltssm = exynos_elbi_read(exynos_pcie, PCIE_ELBI_RDLH_LINKUP) & 0x1f;
	...
	if (ltssm >= S_CFG_LINKWD_START && ltssm < S_L2_IDLE) {
		pcie_info("%s : Link-up (0x%x) on dynamic lane change!\n",
								__func__, ltssm);
	} else {
...
static inline void exynos_pcie_enable_sep_msi(struct exynos_pcie *exynos_pcie)
{
	...
	pcie_dbg("Enable Separated MSI IRQs(Disable MSI RISING EDGE).\n");
	val = exynos_elbi_read(exynos_pcie, PCIE_IRQ2_EN);
	val &= ~IRQ_MSI_CTRL_EN_RISING_EDG;
	exynos_elbi_write(exynos_pcie, val, PCIE_IRQ2_EN);
	...
	for (i = exynos_pcie->separated_msi_start_num; i < MAX_MSI_CTRLS; i++) {
		if (sep_msi_vec[exynos_pcie->ch_num][i].is_used) {
			...
			pcie_info("Separated MSI%d is Enabled.\n", i);
...
void exynos_pcie_rc_check_function_validity(struct exynos_pcie *exynos_pcie)
{
	...
	exynos_pcie_host_v1_poweron(exynos_pcie->ch_num, exynos_pcie->max_link_speed);
					  ...
					  enum exynos_pcie_opr_state state)
{
	...
	case OPR_STATE_LINK:
		...
		exynos_pcie_rc_rd_own_conf(pp, PCIE_PORT_PHY_INTEROP_CTRL_OFF, 4, &val);
		val &= ~P2NOBEACON_ENABLE;
		exynos_pcie_rc_wr_own_conf(pp, PCIE_PORT_PHY_INTEROP_CTRL_OFF, 4, val);
		exynos_pmu_update(exynos_pcie->pmu_hw_ctrl_en,
				PCIE_HW_CTRL_EN, PCIE_HW_CTRL_EN);
		break;
	case OPR_STATE_SETUP_RC:
		...
		exynos_pcie_rc_rd_own_conf(pp, PCIE_PORT_VC0_P_RX_Q_CTRL_OFF, 4, &val);
		val &= ~PCIE_DATA_CREDIT;
		val |= 0x60;
		val &= ~PCIE_HEADER_CREDIT;
		val |= (0x34 << 12);
		exynos_pcie_rc_wr_own_conf(pp, PCIE_PORT_VC0_P_RX_Q_CTRL_OFF, 4, val);

		exynos_pcie_rc_rd_own_conf(pp, PCIE_PORT_VC0_NP_RX_Q_CTRL_OFF, 4, &val);
		val &= ~PCIE_DATA_CREDIT;
		val |= 0xD;
		val &= ~PCIE_HEADER_CREDIT;
		val |= (0x34 << 12);
		exynos_pcie_rc_wr_own_conf(pp, PCIE_PORT_VC0_NP_RX_Q_CTRL_OFF, 4, val);
...
static int exynos_pcie_rc_probe(struct platform_device *pdev)
{
	...
	exynos_pcie_dbg_memlog_init(exynos_pcie);
	...
	ret = exynos_pcie_rc_phy_init(exynos_pcie, 0);
	if (ret)
		goto probe_fail;
	...
	if (dev_is_dma_coherent(pci->dev)) {
		exynos_pcie_rc_set_iocc(exynos_pcie, 1);
		pcie_info("## PCIe IOCC is ENABLED.\n");
	} else {
		pcie_info("## PCIe IOCC is DISABLED.\n");
	}
	...
	exynos_pcie->ltssm_test_wq = create_workqueue("ltssm_test");
	if (!exynos_pcie->ltssm_test_wq) {
		dev_err(&pdev->dev, "unable to create workqueue: ltssm_test\n");
		return -ENOMEM;
	}

	INIT_WORK(&exynos_pcie->ltssm_test_work, exynos_pcie_rc_ltssm_test_work);
