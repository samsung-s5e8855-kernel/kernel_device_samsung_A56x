...
#define PCIE_PORT_VC0_P_RX_Q_CTRL_OFF	0x748
#define PCIE_PORT_VC0_NP_RX_Q_CTRL_OFF	0x74C
#define PCIE_DATA_CREDIT		(0xFFF << 0)
#define PCIE_HEADER_CREDIT		(0xFF << 12)

#define PCIE_PORT_MSI_CTRL_INT1_MASK	0x838
#define PCIE_PORT_COHERENCY_CTR_3	0x8E8

#define PCIE_PORT_PHY_INTEROP_CTRL_OFF  0x8C4
#define P2NOBEACON_ENABLE		(0x1 << 11)
...
#define PCIE_EOM_PH_SEL_MAX		128 /* phase max */
#define PCIE_EOM_DEF_VREF_MAX		256 /* vref max */
...
#define PCIE_QCH_CTRL_DEBUG_1          0x8024
#define SLV_BRESP_RREST_PEND_MASK      (0x3 << 30)
...
#define pcie_info(fmt, args...)					\
	do {							\
		...
		exynos_pcie_dbg_memlog_print(exynos_pcie,	\
				MEMLOG_LEVEL_INFO,		\
				fmt, ##args);			\
	} while (0);
...
#define pcie_dump(fmt, args...)					\
	do {							\
		dev_err(exynos_pcie->pci->dev, fmt, ##args);	\
		exynos_pcie_dbg_memlog_dump_print(exynos_pcie,	\
				MEMLOG_LEVEL_ERR,		\
				fmt, ##args);			\
	} while(0);
...
enum exynos_pcie_unit_test {
	...
	TEST_HISTORY_BUF,
	TEST_RECOVERY,
	TEST_SPEED_LANE_CHECK,
	TEST_TEM,
	TEST_END = TEST_TEM,
};

#define EXYNOS_PCIE_TEST_CASE_NAME(state)	\
	(state == TEST_ALL)		?	"TEST_ALL" :		\
	(state == TEST_LINK)		?	"TEST_LINK" :		\
	(state == TEST_DIS_LINK)	?	"TEST_DIS_LINK" :	\
	(state == TEST_CHK_LTSSM)	?	"TEST_CHK_LTSSM" :	\
	(state == TEST_DBG_OPS)		?	"TEST_DBG_OPS" :	\
	(state == TEST_LOAD_BIN)	?	"TEST_LOAD_BIN" :	\
	(state == TEST_L1SS_DIS)	?	"TEST_L1SS_DIS" :	\
	(state == TEST_L1SS_EN)		?	"TEST_L1SS_EN" :		\
	(state == TEST_L1SS_CTRL_STAT)	?	"TEST_L1SS_CTRL_STAT" :	\
	(state == TEST_FORCE_PERST)	?	"TEST_FORCE_PERST" :	\
	(state == TEST_FORCE_ALLPHYDWN)	?	"TEST_FORCE_ALLPHYDWN" :	\
	(state == TEST_ALL_DUMP)	?	"TEST_ALL_DUMP" :	\
	(state == TEST_SPEED_CHNG)	?	"TEST_SPEED_CHNG" :	\
	(state == TEST_LANE_CHNG)	?	"TEST_LANE_CHNG" :	\
	(state == TEST_LINKUP_W_SPD)	?	"TEST_LINKUP_W_SPD" :	\
	(state == TEST_HISTORY_BUF)	?	"TEST_HISTORY_BUF" :	\
	(state == TEST_RECOVERY)	?	"TEST_RECOVERY" :	\
	(state == TEST_SPEED_LANE_CHECK)?	"TEST_SPEED_LANE_CHECK" :	\
	(state == TEST_TEM)		?	"TEST_TEM" :		\
	" Unknown state ...!!"
...
struct exynos_pcie_eom_info {
	...
	u32 vref_start;
	u32 vref_stop;
	u32 phase, vref, error;
	u32 real_vref_even, real_vref_odd;
	int test_cnt;
};
...
struct exynos_pcie {
	...
	void __iomem		*ia1_base;
	void __iomem		*ia2_base;
	...
	struct work_struct ltssm_test_work;
	struct workqueue_struct *ltssm_test_wq;
	int ltssm_test_time_sec;
	struct exynos_pcie_ltssm_test *ltssm_test_data;

	struct exynos_pcie_dbg_range dbg_range_elbi;
	struct exynos_pcie_dbg_range dbg_range_phy;
	struct exynos_pcie_dbg_range dbg_range_pcs;
	struct exynos_pcie_dbg_range dbg_range_udbg;
	struct exynos_pcie_dbg_range dbg_range_dbi;
};
