...
static inline struct mbulk *mbulk_pool_get(struct mbulk_pool *pool, enum mbulk_class clas)
{
	...
	if (m->next_offset == 0) {
		...
		if (track_free_list[pool->pid][SECOND] == NULL) {
			SLSI_DBG3_NODEV(SLSI_MBULK, "There is no more mbulk to free\n");
			spin_unlock_bh(&mbulk_pool_lock);
			return NULL;
		} else {
			track_free_list[pool->pid][FIRST] = track_free_list[pool->pid][SECOND];
...
static inline void mbulk_pool_put(struct mbulk_pool *pool, struct mbulk *m)
{
	...
	if (m->pid >= MBULK_POOL_ID_MAX) {
		WLBT_WARN_ON(m->pid >= MBULK_POOL_ID_MAX);
		goto end;
	}
 ...
 *
 */
struct mbulk *mbulk_with_signal_alloc_by_pool(struct scsc_service *service, u8 pool_id, mbulk_colour colour,
					      enum mbulk_class clas, size_t sig_bufsz_req, size_t dat_bufsz)
{
	struct mbulk_pool *pool;
	size_t            sig_bufsz;
	size_t            tot_bufsz;
	struct mbulk      *m_ret = NULL, *head = NULL;
	u32		  index;
	...
	if (dat_bufsz != MBULK_DAT_BUFSZ_REQ_BEST_MAGIC &&
		...
		SLSI_ERR_NODEV("too large. Max:%d, sig:%d, payload:%d, total:%d try chained mbulk\n",
			       pool->seg_size, sig_bufsz,
			       dat_bufsz, tot_bufsz);
		head = mbulk_chain_with_signal_alloc_by_pool(service, pool_id, colour, clas,
							     sig_bufsz_req, dat_bufsz);
		if (!head)
			SLSI_ERR_NODEV("chained mbulk alloc failed\n");
		return head;
	}
	...
	if (index >= pool->tot_seg_num) {
		SLSI_ERR_NODEV("No free mbulk. tot_seg_num:%d, pool_end_addr:%p m_ret:%p index:%d\n",
		               pool->tot_seg_num, pool->end_addr, m_ret, index);
		return NULL;
	}
 ...
 *
 */
struct mbulk *mbulk_chain_with_signal_alloc_by_pool(struct scsc_service *service, u8 pool_id, mbulk_colour colour,
						    enum mbulk_class clas, size_t sig_bufsz, size_t dat_bufsz)
{
	size_t       tot_len;
	scsc_mifram_ref ref = 0;
	struct mbulk *m = NULL, *head = NULL, *pre = NULL;

	head = mbulk_with_signal_alloc_by_pool(service, pool_id, clas,  colour, sig_bufsz,
					       MBULK_DAT_BUFSZ_REQ_BEST_MAGIC);
	if (head == NULL || MBULK_SEG_TAILROOM(head) >= dat_bufsz) {
		SLSI_ERR_NODEV("Either head is NULL or tailroom %d, dat_bufsz %d\n",
			   MBULK_SEG_TAILROOM(head), dat_bufsz);
		mbulk_free_virt_host(head);
		return NULL;
	}
	...
	while (tot_len < dat_bufsz) {
		m = mbulk_with_signal_alloc_by_pool(service, pool_id, clas, colour, 0,
						    MBULK_DAT_BUFSZ_REQ_BEST_MAGIC);
		if (m == NULL)
			break;
		...
		scsc_mx_service_mif_ptr_to_addr(service, m, &ref);
		pre->chain_next_offset = ref;
		pre = m;
	}
 ...
 */
void mbulk_chain_free(struct mbulk *sg, struct scsc_service *service)
{
	struct mbulk *curr_mbulk = NULL;
	int i = 0;
	scsc_mifram_ref next_mbulk_ref = 0;
	...
	while ((curr_mbulk) && (i < MBULK_MAX_CHAIN)) {
		next_mbulk_ref = mbulk_chain_next(curr_mbulk);
		curr_mbulk->flag &= ~(MBULK_F_CHAIN | MBULK_F_CHAIN_HEAD);
		mbulk_free_virt_host(curr_mbulk);
		if (!next_mbulk_ref)
			break;
		curr_mbulk = scsc_mx_service_mif_addr_to_ptr(service, next_mbulk_ref);
		i++;
	}
}
