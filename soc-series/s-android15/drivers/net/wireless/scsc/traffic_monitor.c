...
static uint fake_tx_tput;
module_param(fake_tx_tput, uint, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(fake_tx_tput, "default is 0. only for test purpose");

static uint fake_rx_tput;
module_param(fake_rx_tput, uint, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(fake_rx_tput, "default is 0. only for test purpose");

static int fake_override_trigger;
/* Write callback */
static int fake_override_trigger_set(const char *val, const struct kernel_param *kp) {
	struct slsi_dev *sdev = slsi_get_sdev();

	SLSI_UNUSED_PARAMETER(val);
	SLSI_UNUSED_PARAMETER(kp);

	SLSI_INFO_NODEV("Trigger manual override\n");
	slsi_traffic_mon_override(sdev);
	return 0;
}

/* Set up the kernel parameter ops structure */
static struct kernel_param_ops fake_override_trigger_ops = {
	.get = NULL,
	.set = fake_override_trigger_set,
};

/* Declare the module parameter and its read/write callbacks */
module_param_cb(fake_override_trigger, &fake_override_trigger_ops, &fake_override_trigger, S_IRUGO | S_IWUSR);
		...
		if (dev) {
			...
			if (ndev_vif) {
				...
				if (ndev_vif->report_time >= 1000) {
					ndev_vif->tput_rx_bytes_per_sec = (ndev_vif->num_bytes_rx_per_sec / ndev_vif->report_time) * 1000;
					ndev_vif->tput_tx_bytes_per_sec = (ndev_vif->num_bytes_tx_per_sec / ndev_vif->report_time) * 1000;
				...
				ndev_vif->throughput_tx = (ndev_vif->num_bytes_tx_per_timer / time_in_ms) * 1000;
				ndev_vif->throughput_rx = (ndev_vif->num_bytes_rx_per_timer / time_in_ms) * 1000;
...
#ifdef CONFIG_SCSC_WLAN_TPUT_MONITOR
	/* override tput_tx / tput_rx with fake tput value */
	if (fake_tx_tput)
		tput_tx = fake_tx_tput;
	if (fake_rx_tput)
		tput_rx = fake_rx_tput;
#endif
	...
	void (*traffic_mon_client_cb)(void *client_ctx, u32 state, u32 tput_tx, u32 tput_rx))
{
		...
		for (i = 1; i <= CONFIG_SCSC_WLAN_MAX_INTERFACES; i++) {
			...
			if (dev) {
				...
				if (ndev_vif) {
					...
					ndev_vif->num_bytes_rx_per_sec = 0;
					ndev_vif->num_bytes_tx_per_sec = 0;
					ndev_vif->tput_rx_bytes_per_sec = 0;
					ndev_vif->tput_tx_bytes_per_sec = 0;
					ndev_vif->report_time = 0;
				}
			}
		}
	...
	spin_unlock_bh(&sdev->traffic_mon_clients.lock);
	return (void *)traffic_mon_client;
}

void slsi_traffic_mon_client_unregister(struct slsi_dev *sdev, void *client_addr)
{
	struct slsi_traffic_mon_client_entry *traffic_mon_client;
	struct net_device *dev;
	struct netdev_vif *ndev_vif;
	...
	traffic_mon_client = (struct slsi_traffic_mon_client_entry *)client_addr;

	if (!traffic_mon_client) {
		SLSI_ERR(sdev, "Invalid traffic mon client address\n");
		spin_unlock_bh(&sdev->traffic_mon_clients.lock);
		return;
	}
...
void slsi_traffic_mon_init(struct slsi_dev *sdev)
{
	...
	tm_data.tm_data_client = slsi_traffic_mon_client_register(sdev, (void *)&tm_data, TRAFFIC_MON_CLIENT_MODE_EVENTS,
								  tm_logger_mid, tm_logger_high, TRAFFIC_MON_DIR_DEFAULT,
								  slsi_traffic_mon_logger_cb);

	if (!(tm_data.tm_data_client))
		SLSI_WARN(sdev, "failed to add tm_data client to traffic monitor\n");
}
...
void slsi_traffic_mon_deinit(struct slsi_dev *sdev)
{
	...
	slsi_traffic_mon_client_unregister(sdev, tm_data.tm_data_client);
	tm_data.sdev = NULL;
}
#endif
