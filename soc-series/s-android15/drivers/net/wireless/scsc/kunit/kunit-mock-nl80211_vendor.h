...
#define slsi_tas_notify_sar_ind(args...)		kunit_mock_slsi_tas_notify_sar_ind(args)
#define slsi_tas_notify_sar_limit_upper(args...)	kunit_mock_slsi_tas_notify_sar_limit_upper(args)
#define slsi_tas_notify_sar_target(args...)		kunit_mock_slsi_tas_notify_sar_target(args)
#define slsi_tas_notify_wifi_status(arg)		kunit_mock_slsi_tas_notify_wifi_status(arg)
#endif
...
#if defined(CONFIG_SCSC_WLAN_TAS)
static int slsi_tas_notify_sar_ind(struct slsi_dev *sdev, struct net_device *dev, struct sk_buff *skb)
{
	if (skb) {
		kfree_skb(skb);
		skb = NULL;
...
static int slsi_tas_notify_sar_limit_upper(struct slsi_dev *sdev, struct net_device *dev, struct sk_buff *skb)
{
	if (skb) {
		kfree_skb(skb);
		skb = NULL;
...
static int slsi_tas_notify_sar_target(struct slsi_dev *sdev, struct net_device *dev, struct sk_buff *skb)
{
	if (skb) {
		kfree_skb(skb);
		skb = NULL;
...
static void slsi_tas_notify_wifi_status(bool enabled)
{
}
#endif
#endif
