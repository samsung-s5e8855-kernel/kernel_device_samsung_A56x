...
static void test_slsi_regd_deinit(struct kunit *test)
{
	...
	sdev->device_config.domain_info.countrylist = kzalloc(sizeof(*sdev->device_config.domain_info.countrylist), GFP_KERNEL);
	sdev->regdb.freq_ranges = kzalloc(sizeof(*sdev->regdb.freq_ranges), GFP_KERNEL);
	sdev->regdb.reg_rules = kzalloc(sizeof(*sdev->regdb.reg_rules), GFP_KERNEL);
	sdev->regdb.rules_collection = kzalloc(sizeof(*sdev->regdb.rules_collection), GFP_KERNEL);
	sdev->regdb.country = kzalloc(sizeof(*sdev->regdb.country), GFP_KERNEL);
