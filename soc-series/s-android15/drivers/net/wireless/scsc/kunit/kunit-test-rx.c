...
static void test_slsi_populate_bssid_info(struct kunit *test)
{
	...
	KUNIT_EXPECT_EQ(test, 0, slsi_populate_bssid_info(sdev, ndev_vif, skb, mld_addr, &bssid_list));
...
static void test_slsi_remove_assoc_disallowed_bssid(struct kunit *test)
{
	...
	struct slsi_ssid_info *new_ssid_info;
	struct slsi_bssid_info *new_bssid_info;
	struct slsi_scan_result test_scan;

	new_ssid_info = kunit_kzalloc(test, sizeof(struct slsi_ssid_info), GFP_KERNEL);
	if (!new_ssid_info) {
		KUNIT_EXPECT_TRUE (test, NULL != new_ssid_info);
		return;
	}

	new_bssid_info = kunit_kzalloc(test, sizeof(struct slsi_bssid_info), GFP_KERNEL);
	if (!new_bssid_info) {
		KUNIT_EXPECT_TRUE (test, NULL != new_bssid_info);
		return;
	}

	ndev_vif->ifnum = SLSI_NET_INDEX_WLAN;
	test_scan.ssid_length = IEEE80211_MAX_SSID_LEN;
	test_scan.akm_type = 1;
	memset(test_scan.ssid, 0x0, IEEE80211_MAX_SSID_LEN);

	INIT_LIST_HEAD(&ndev_vif->sta.ssid_info);
	list_add(&new_ssid_info->list, &ndev_vif->sta.ssid_info);

	new_ssid_info->ssid.ssid_len = IEEE80211_MAX_SSID_LEN;
	new_ssid_info->akm_type = 1;
	memset(new_ssid_info->ssid.ssid, 0x0, IEEE80211_MAX_SSID_LEN);
	memset(new_bssid_info->bssid, 0x0, sizeof(char) * ETH_ALEN);

	INIT_LIST_HEAD(&new_ssid_info->bssid_list);
	list_add(&new_bssid_info->list, &new_ssid_info->bssid_list);
	memset(test_scan.bssid, 0x1, sizeof(char) * ETH_ALEN);

	slsi_remove_assoc_disallowed_bssid(sdev, ndev_vif, &test_scan);
	KUNIT_EXPECT_EQ (test, 0, list_empty(&new_ssid_info->bssid_list));
}
...
static void test_slsi_add_to_scan_list(struct kunit *test)
{
	...
	struct sk_buff *skb;
	struct ieee80211_mgmt *mgmt;
	struct slsi_scan_result *scan_results = ndev_vif->scan[SLSI_SCAN_HW_ID].scan_results;
	int i = 0;

	scan_results = kunit_kzalloc(test, sizeof(struct slsi_scan_result), GFP_KERNEL);
	for (i = 0; i < ARRAY_SIZE(scan_list_tests); i++) {
		...
		if (scan_results->beacon)
			kfree_skb(scan_results->beacon);
		if (scan_results->probe_resp)
...
static void test_slsi_scan_update_ssid_map(struct kunit *test)
{
	...
	skb = fapi_alloc(mlme_add_scan_req, MLME_ADD_SCAN_REQ, 0, 110);
...
static void test_slsi_send_roam_vendor_event(struct kunit *test)
{
	...
	slsi_send_roam_vendor_event(sdev, dev, bssid, req_ie, req_ie_len, resp_ie, resp_ie_len,
...
static void test_slsi_rx_received_frame_logging(struct kunit *test)
{
	...
	u8 ml_link_band = SLSI_UC_MAC_2_4_BAND;
	...
	slsi_rx_received_frame_logging(dev, skb, NULL, 0, ml_link_band);
...
static void test_slsi_rx_blocking_signals(struct kunit *test)
{
	...
	sdev->sig_wait.cfm_id = MLME_DEL_VIF_CFM;
	sdev->sig_wait.process_id = SLSI_TX_PROCESS_ID_MIN;
	fapi_set_u16(skb, receiver_pid, SLSI_TX_PROCESS_ID_MIN);
	KUNIT_EXPECT_EQ(test, 0, slsi_rx_blocking_signals(sdev, skb));
	KUNIT_EXPECT_PTR_EQ(test, sdev->sig_wait.cfm, skb);

	sdev->sig_wait.cfm_id = MLME_ADD_VIF_CFM;
	...
	rcu_assign_pointer(sdev->netdev[0], dev);
	KUNIT_EXPECT_EQ(test, -EINVAL, slsi_rx_blocking_signals(sdev, skb));

	ndev_vif->sig_wait.ind_id = DEBUG_SPARE_SIGNAL_1_IND;
	fapi_set_u16(skb, receiver_pid, SLSI_TX_PROCESS_ID_MIN);
	ndev_vif->sig_wait.process_id = SLSI_TX_PROCESS_ID_MIN;
	KUNIT_EXPECT_EQ(test, 0, slsi_rx_blocking_signals(sdev, skb));
