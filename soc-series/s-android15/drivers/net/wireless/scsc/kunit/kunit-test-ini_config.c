...
static void test_set_mibs(struct kunit *test)
{
	...
	sdev->sig_wait.cfm = fapi_alloc(mlme_get_cfm, MLME_GET_CFM, 0, 128);
	((struct slsi_skb_cb *)sdev->sig_wait.cfm->cb)->data_length = 1;
...
static void test_encode_mib_octet_with_2indices(struct kunit *test)
{
	...
	KUNIT_EXPECT_EQ(test, 0, encode_mib_octet_with_2indices(sdev, buf, &pos, value, index1, index2,
								octet_index, psid, unit_con_factor));
	KUNIT_EXPECT_EQ(test, -EINVAL, encode_mib_octet_with_2indices(sdev, buf, &pos, "", index1, index2,
								      octet_index, psid, unit_con_factor));
}
...
static void test_encode_mib_band3_rssi_factor_value1(struct kunit *test)
{
	struct slsi_dev *sdev = TEST_TO_SDEV(test);
	u8 buf[20] = {0,};
	u16 pos = 0;
	...
	KUNIT_EXPECT_EQ(test, 0, encode_mib_band3_rssi_factor_value1(sdev, buf, &pos, value, lookup_entry));
	KUNIT_EXPECT_EQ(test, -EINVAL, encode_mib_band3_rssi_factor_value1(sdev, buf, &pos, "", lookup_entry));
}

static void test_encode_mib_band3_rssi_factor_value2(struct kunit *test)
{
	struct slsi_dev *sdev = TEST_TO_SDEV(test);
	u8 buf[20] = {0,};
	u16 pos = 0;
	...
	KUNIT_EXPECT_EQ(test, 0, encode_mib_band3_rssi_factor_value2(sdev, buf, &pos, value, lookup_entry));
	KUNIT_EXPECT_EQ(test, -EINVAL, encode_mib_band3_rssi_factor_value2(sdev, buf, &pos, "", lookup_entry));
}

static void test_encode_mib_band3_rssi_factor_value3(struct kunit *test)
{
	struct slsi_dev *sdev = TEST_TO_SDEV(test);
	u8 buf[20] = {0,};
	u16 pos = 0;
	...
	KUNIT_EXPECT_EQ(test, 0, encode_mib_band3_rssi_factor_value3(sdev, buf, &pos, value, lookup_entry));
	KUNIT_EXPECT_EQ(test, -EINVAL, encode_mib_band3_rssi_factor_value3(sdev, buf, &pos, "", lookup_entry));
}

static void test_encode_mib_band3_rssi_factor_value4(struct kunit *test)
{
	struct slsi_dev *sdev = TEST_TO_SDEV(test);
	u8 buf[20] = {0,};
	u16 pos = 0;
	...
	KUNIT_EXPECT_EQ(test, 0, encode_mib_band3_rssi_factor_value4(sdev, buf, &pos, value, lookup_entry));
	KUNIT_EXPECT_EQ(test, -EINVAL, encode_mib_band3_rssi_factor_value4(sdev, buf, &pos, "", lookup_entry));
}

static void test_encode_mib_band3_rssi_factor_score1(struct kunit *test)
{
	struct slsi_dev *sdev = TEST_TO_SDEV(test);
	u8 buf[20] = {0,};
	u16 pos = 0;
	...
	KUNIT_EXPECT_EQ(test, 0, encode_mib_band3_rssi_factor_score1(sdev, buf, &pos, value, lookup_entry));
	KUNIT_EXPECT_EQ(test, -EINVAL, encode_mib_band3_rssi_factor_score1(sdev, buf, &pos, "", lookup_entry));
}

static void test_encode_mib_band3_rssi_factor_score2(struct kunit *test)
{
	struct slsi_dev *sdev = TEST_TO_SDEV(test);
	u8 buf[20] = {0,};
	u16 pos = 0;
	...
	KUNIT_EXPECT_EQ(test, 0, encode_mib_band3_rssi_factor_score2(sdev, buf, &pos, value, lookup_entry));
	KUNIT_EXPECT_EQ(test, -EINVAL, encode_mib_band3_rssi_factor_score2(sdev, buf, &pos, "", lookup_entry));
}

static void test_encode_mib_band3_rssi_factor_score3(struct kunit *test)
{
	struct slsi_dev *sdev = TEST_TO_SDEV(test);
	u8 buf[20] = {0,};
	u16 pos = 0;
	...
	KUNIT_EXPECT_EQ(test, 0, encode_mib_band3_rssi_factor_score3(sdev, buf, &pos, value, lookup_entry));
	KUNIT_EXPECT_EQ(test, -EINVAL, encode_mib_band3_rssi_factor_score3(sdev, buf, &pos, "", lookup_entry));
}

static void test_encode_mib_band3_rssi_factor_score4(struct kunit *test)
{
	struct slsi_dev *sdev = TEST_TO_SDEV(test);
	u8 buf[20] = {0,};
	u16 pos = 0;
	...
	KUNIT_EXPECT_EQ(test, 0, encode_mib_band3_rssi_factor_score4(sdev, buf, &pos, value, lookup_entry));
	KUNIT_EXPECT_EQ(test, -EINVAL, encode_mib_band3_rssi_factor_score4(sdev, buf, &pos, "", lookup_entry));
}

static void test_save_roam_wtc_scan_mode(struct kunit *test)
{
	struct slsi_dev *sdev = TEST_TO_SDEV(test);
	u8 buf[20] = {0,};
	u16 pos = 0;
	...
	KUNIT_EXPECT_EQ(test, 0, save_roam_wtc_scan_mode(sdev, buf, &pos, value, lookup_entry));
	KUNIT_EXPECT_EQ(test, -EINVAL, save_roam_wtc_scan_mode(sdev, buf, &pos, NULL, lookup_entry));
}

static void test_save_roam_wtc_handling_rssi_threshold(struct kunit *test)
{
	struct slsi_dev *sdev = TEST_TO_SDEV(test);
	u8 buf[20] = {0,};
	u16 pos = 0;
	...
	KUNIT_EXPECT_EQ(test, 0, save_roam_wtc_handling_rssi_threshold(sdev, buf, &pos, value, lookup_entry));
	KUNIT_EXPECT_EQ(test, -EINVAL, save_roam_wtc_handling_rssi_threshold(sdev, buf, &pos, NULL, lookup_entry));
}

static void test_save_roam_wtc_24g_candi_rssi_threshold(struct kunit *test)
{
	struct slsi_dev *sdev = TEST_TO_SDEV(test);
	u8 buf[20] = {0,};
	u16 pos = 0;
	...
	KUNIT_EXPECT_EQ(test, 0, save_roam_wtc_24g_candi_rssi_threshold(sdev, buf, &pos, value, lookup_entry));
	KUNIT_EXPECT_EQ(test, -EINVAL, save_roam_wtc_24g_candi_rssi_threshold(sdev, buf, &pos, NULL, lookup_entry));
}

static void test_save_roam_wtc_5g_candi_rssi_threshold(struct kunit *test)
{
	struct slsi_dev *sdev = TEST_TO_SDEV(test);
	u8 buf[20] = {0,};
	u16 pos = 0;
	...
	KUNIT_EXPECT_EQ(test, 0, save_roam_wtc_5g_candi_rssi_threshold(sdev, buf, &pos, value, lookup_entry));
	KUNIT_EXPECT_EQ(test, -EINVAL, save_roam_wtc_5g_candi_rssi_threshold(sdev, buf, &pos, NULL, lookup_entry));
}

static void test_save_roam_wtc_6g_candi_rssi_threshold(struct kunit *test)
{
	struct slsi_dev *sdev = TEST_TO_SDEV(test);
	u8 buf[20] = {0,};
	u16 pos = 0;
	...
	KUNIT_EXPECT_EQ(test, 0, save_roam_wtc_6g_candi_rssi_threshold(sdev, buf, &pos, value, lookup_entry));
	KUNIT_EXPECT_EQ(test, -EINVAL, save_roam_wtc_6g_candi_rssi_threshold(sdev, buf, &pos, NULL, lookup_entry));
}

static void test_save_conn_non_hint_target_min_rssi(struct kunit *test)
{
		struct slsi_dev *sdev = TEST_TO_SDEV(test);
	u8 buf[20] = {0,};
	u16 pos = 0;
	...
	KUNIT_EXPECT_EQ(test, 0, save_conn_non_hint_target_min_rssi(sdev, buf, &pos, value, lookup_entry));
	KUNIT_EXPECT_EQ(test, -EINVAL, save_conn_non_hint_target_min_rssi(sdev, buf, &pos, NULL, lookup_entry));
}
...
static void test_slsi_ini_set_defaults(struct kunit *test)
{
	struct slsi_dev *sdev = TEST_TO_SDEV(test);
...
static struct kunit_case ini_config_test_cases[] = {
	...
	KUNIT_CASE(test_encode_mib_band3_rssi_factor_value1),
	KUNIT_CASE(test_encode_mib_band3_rssi_factor_value2),
	KUNIT_CASE(test_encode_mib_band3_rssi_factor_value3),
	KUNIT_CASE(test_encode_mib_band3_rssi_factor_value4),
	KUNIT_CASE(test_encode_mib_band3_rssi_factor_score1),
	KUNIT_CASE(test_encode_mib_band3_rssi_factor_score2),
	KUNIT_CASE(test_encode_mib_band3_rssi_factor_score3),
	KUNIT_CASE(test_encode_mib_band3_rssi_factor_score4),
	KUNIT_CASE(test_save_roam_wtc_scan_mode),
	KUNIT_CASE(test_save_roam_wtc_handling_rssi_threshold),
	KUNIT_CASE(test_save_roam_wtc_24g_candi_rssi_threshold),
	KUNIT_CASE(test_save_roam_wtc_5g_candi_rssi_threshold),
	KUNIT_CASE(test_save_roam_wtc_6g_candi_rssi_threshold),
	KUNIT_CASE(test_save_conn_non_hint_target_min_rssi),
	...
	KUNIT_CASE(test_slsi_ini_set_defaults),
