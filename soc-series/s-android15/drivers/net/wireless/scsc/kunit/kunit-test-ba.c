...
static void test_slsi_rx_ba_init(struct kunit *test)
{
	...
	KUNIT_EXPECT_EQ(test, 0, slsi_rx_ba_init(sdev));

	if (sdev->rx_ba_buffer_pool)
		vfree(sdev->rx_ba_buffer_pool);
}

static void test_slsi_rx_ba_deinit(struct kunit *test)
{
	struct slsi_dev *sdev = TEST_TO_SDEV(test);

	sdev->rx_ba_buffer_pool = vmalloc(sizeof(struct slsi_ba_session_rx) * SLSI_MAX_RX_BA_SESSIONS);
...
static void test_slsi_rx_ba_alloc_buffer(struct kunit *test)
{
	...
	for (i = 0; i < SLSI_MAX_RX_BA_SESSIONS; i++) {
		sdev->rx_ba_buffer_pool[i].used = false;
...
static void test_ba_update_expected_sn(struct kunit *test)
{
	...
	peer->ba_session_rx[TEST_TID]->buffer[0].signal = alloc_skb(100, GFP_KERNEL);
...
static void test_ba_scroll_window(struct kunit *test)
{
	...
	skb = skb_dequeue(&ndev_vif->ba_complete);
	kfree_skb(skb);
}
...
static void test_ba_consume_frame_or_get_buffer_index(struct kunit *test)
{
	...
	frame_desc->signal = alloc_skb(100, GFP_KERNEL);
...
static void test_slsi_ba_aging_timeout_handler(struct kunit *test)
{
	...
	ba_session_rx->buffer[0].active = true;
	ba_session_rx->buffer[0].signal = alloc_skb(100, GFP_KERNEL);
...
static void test_slsi_ba_process_frame(struct kunit *test)
{
	...
	KUNIT_EXPECT_EQ(test, -EINVAL, slsi_ba_process_frame(dev, peer, skb, sequence_number, TEST_TID));
	...
	KUNIT_EXPECT_EQ(test, 0, slsi_ba_process_frame(dev, peer, skb, sequence_number, TEST_TID));
...
static void test_slsi_rx_ba_stop_all(struct kunit *test)
{
	...
	for (i = 0; i < NUM_BA_SESSIONS_PER_PEER; i++) {
		...
		peer->ba_session_rx[i]->buffer[0].signal = NULL;
	}

	slsi_rx_ba_stop_all(dev, peer);

	for (i = 0; i < NUM_BA_SESSIONS_PER_PEER; i++)
		KUNIT_EXPECT_PTR_EQ(test, NULL, peer->ba_session_rx[i]);
}
...
static struct kunit_case ba_test_cases[] = {
	...
	KUNIT_CASE(test_slsi_rx_ba_deinit),
