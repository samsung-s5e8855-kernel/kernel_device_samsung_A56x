...
/* TestCase */
static void test_slsi_is_mhs_active(struct kunit *test)
{
	struct slsi_dev *sdev = TEST_TO_SDEV(test);
	struct net_device *dev = TEST_TO_DEV(test);
	u8 *ifname = (u8 *)dev->name;

	snprintf(dev->name, IFNAMSIZ, "wlan0");
	sdev->netdev[SLSI_NET_INDEX_WLAN] = dev;

	KUNIT_EXPECT_EQ(test, 0, slsi_is_mhs_active(sdev, &ifname));
}

static void test_slsi_is_160mhz_supported(struct kunit *test)
{
	struct slsi_dev *sdev = TEST_TO_SDEV(test);

	KUNIT_EXPECT_EQ(test, 0, slsi_is_160mhz_supported(sdev));
}
...
static void test_slsi_add_virtual_intf(struct kunit *test)
{
	...
	ndev_vif->activated = false;
	sdev->num_ap_vlan = SLSI_MAX_AP_VLAN;
	KUNIT_EXPECT_PTR_EQ(test,
			    ERR_PTR(-EINVAL),
			    slsi_add_virtual_intf(wiphy, name, '0', NL80211_IFTYPE_AP_VLAN, params));
	...
	ndev_vif = netdev_priv(dev);
	ndev_vif->activated = true;
KUNIT_EXPECT_PTR_EQ(test, ERR_PTR(-ENODEV), slsi_add_virtual_intf(wiphy, name, '0', NL80211_IFTYPE_AP_VLAN, params));
	sdev->netdev[NL80211_IFTYPE_AP_VLAN] = dev;
KUNIT_EXPECT_LT(test, 0, slsi_add_virtual_intf(wiphy, name, '0', NL80211_IFTYPE_AP_VLAN, params));
}
...
static void test_slsi_change_station(struct kunit *test)
{
	struct slsi_dev *sdev = TEST_TO_SDEV(test);
	struct net_device *dev = TEST_TO_DEV(test);
	struct netdev_vif *ndev_vif = netdev_priv(dev);
	struct wiphy *wiphy = sdev->wiphy;
	struct station_parameters *params = kunit_kzalloc(test, sizeof(*params), GFP_KERNEL);
	u8 mac[ETH_ALEN] = {0x0C, 0x0C, 0x1C, 0xAA, 0xB1, 0x1F};

	ndev_vif->iftype = NL80211_IFTYPE_ADHOC;
	KUNIT_EXPECT_EQ(test, -EINVAL, slsi_change_station(wiphy, dev, mac, params));

	ndev_vif->iftype = NL80211_IFTYPE_STATION;
	KUNIT_EXPECT_EQ(test, -EINVAL, slsi_change_station(wiphy, dev, mac, params));

	ndev_vif->iftype = NL80211_IFTYPE_AP_VLAN;
	KUNIT_EXPECT_EQ(test, 0, slsi_change_station(wiphy, dev, mac, params));

	ndev_vif->iftype = NL80211_IFTYPE_UNSPECIFIED;
	KUNIT_EXPECT_EQ(test, -EOPNOTSUPP, slsi_change_station(wiphy, dev, mac, params));
}
...
static void test_slsi_channel_switch(struct kunit *test)
{
	...
	ndev_vif->iftype = NL80211_IFTYPE_STATION;
	KUNIT_EXPECT_EQ(test, -EINVAL, slsi_channel_switch(wiphy, ndev, params));
	...
	KUNIT_EXPECT_EQ(test, 0, slsi_channel_switch(wiphy, ndev, params));
}
...
static void test_slsi_connect(struct kunit *test)
{
	...
	//slsi_check_wificonnect_to_connectedGo() != 0
	sme->bssid = SLSI_DEFAULT_BSSID;
	sme->ssid = "CNT_KUNT_SSID";
	sme->ssid_len = 14;
	...
	//slsi_check_valid_netdev_vif_state != 0
	ndev_vif->ifnum = SLSI_NET_INDEX_WLAN;
	ndev_vif->activated = true;
	KUNIT_EXPECT_EQ(test, -EINVAL, slsi_connect(wiphy, ndev, sme));

	//slsi_vif_activated() != 0
	ndev_vif->vif_type = FAPI_VIFTYPE_AP;
	...
	ndev_vif->vif_type = FAPI_VIFTYPE_STATION;
	ndev_vif->sta.vif_status = SLSI_VIF_STATUS_UNSPECIFIED;
	sdev->chip_info_mib.chip_version = 0;
	...
	//slsi_mlme_register_action_frame() != 0
	...
	//slsi_is_bssid_in_blacklist() != 0
	ndev_vif->power_mode = 12;
	ndev_vif->set_power_mode = 43;
	KUNIT_EXPECT_EQ(test, -EINVAL, slsi_connect(wiphy, ndev, sme));

	//slsi_is_test_mode_enabled() != 0
	test_cm_if_set_test_mode(true);
	KUNIT_EXPECT_EQ(test, -EOPNOTSUPP, slsi_connect(wiphy, ndev, sme));
...
static void test_slsi_set_pmksa(struct kunit *test)
{
	...
	ndev_vif->sta.rsn_ie = "\x30\x1a\x01\x00\x00\x0f\xac\x04\x01\x00\x00\x0f\xac\x04\x01\x00\x00\x0f\xac\x08\xc0\x00\x01\x00\x45\xAB\x7D\x9A\x24\x0A\x99\x12\x23\x84\xA5\xBC\x34\x00\x02\x61\x00\x0f\xac\x06";
...
static void test_slsi_p2p_group_mgmt_tx(struct kunit *test)
{
	struct net_device *ndev = TEST_TO_DEV(test);
	struct netdev_vif *ndev_vif = netdev_priv(ndev);
	struct slsi_dev *sdev = ndev_vif->sdev;
...
static void test_slsi_set_mac_acl(struct kunit *test)
{
	...
	params = kunit_kzalloc(test, sizeof(struct cfg80211_acl_data) +
...
static void test_slsi_ap_chandef_vht_ht(struct kunit *test)
{
	...
	sdev->fw_vht_cap[2] = 0;
	sdev->fw_vht_cap[3] = 0;
	KUNIT_EXPECT_TRUE(test, slsi_ap_chandef_vht_ht(ndev, sdev, 1));
	...
	KUNIT_EXPECT_FALSE(test, slsi_ap_chandef_vht_ht(ndev, sdev, 1));
}
...
static struct kunit_case cfg80211_ops_test_cases[] = {
	KUNIT_CASE(test_slsi_is_mhs_active),
	KUNIT_CASE(test_slsi_is_160mhz_supported),
	...
	KUNIT_CASE(test_slsi_change_station),
	...
	KUNIT_CASE(test_slsi_p2p_group_mgmt_tx),
