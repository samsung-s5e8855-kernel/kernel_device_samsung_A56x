...
static void test_cac_delete_tspec(struct kunit *test)
{
	...
	KUNIT_EXPECT_EQ(test, 0, cac_delete_tspec(sdev, id));
	KUNIT_EXPECT_EQ(test, -1, cac_delete_tspec(sdev, id+1));
}
...
static void test_cac_send_delts(struct kunit *test)
{
	...
	itr->accepted = 1;
	KUNIT_EXPECT_EQ(test, 255, cac_send_delts(sdev, id));
...
static void test_cac_delete_tspec_by_state(struct kunit *test)
{
	...
	KUNIT_EXPECT_EQ(test, 0, cac_delete_tspec_by_state(sdev, id1, 0));
	KUNIT_EXPECT_EQ(test, 0, cac_delete_tspec_by_state(sdev, id2, 0));
}
...
static void test_cac_get_active_tspecs(struct kunit *test)
{
	...
	struct slsi_dev *sdev = TEST_TO_SDEV(test);
	struct netdev_vif *ndev_vif = netdev_priv(dev);
	struct cac_activated_tspec *tspecs;
	struct cac_tspec *itr;
	int id;
