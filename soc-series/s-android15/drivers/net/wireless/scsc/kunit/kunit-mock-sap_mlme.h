...
#define slsi_rx_scheduled_pm_teardown_indication(args...)		kunit_mock_slsi_rx_scheduled_pm_teardown_indication(args)
#define slsi_rx_scheduled_pm_leaky_ap_detect_indication(args...)	kunit_mock_slsi_rx_scheduled_pm_leaky_ap_detect_indication(args)
#define slsi_rx_delayed_wakeup_indication(args...)			kunit_mock_slsi_rx_delayed_wakeup_indication(args)
#define slsi_rx_sr_params_changed_indication(args...)			kunit_mock_slsi_rx_sr_params_changed_indication(args)
#ifdef CONFIG_SCSC_WLAN_EHT
#define slsi_rx_mlo_link_info_ind(args...)				kunit_mock_slsi_rx_mlo_link_info_ind(args)
#define slsi_rx_mlo_link_measurement_ind(args...)			kunit_mock_slsi_rx_mlo_link_measurement_ind(args)
#define slsi_rx_mlo_set_ttlm_ind(args...)				kunit_mock_slsi_rx_mlo_set_ttlm_ind(args)
#endif
...
static void kunit_mock_slsi_rx_scheduled_pm_teardown_indication(struct slsi_dev *sdev, struct net_device *dev,
								struct sk_buff *skb)
{
	if (skb) {
		kfree_skb(skb);
...
static void kunit_mock_slsi_rx_scheduled_pm_leaky_ap_detect_indication(struct slsi_dev *sdev, struct net_device *dev,
								       struct sk_buff *skb)
{
	if (skb) {
		kfree_skb(skb);
...
static void kunit_mock_slsi_rx_delayed_wakeup_indication(struct slsi_dev *sdev, struct net_device *dev,
				       struct sk_buff *skb)
{
	if (skb) {
		kfree_skb(skb);
...
static void kunit_mock_slsi_rx_sr_params_changed_indication(struct slsi_dev *sdev, struct net_device *dev,
							    struct sk_buff *skb)
{
	if (skb) {
		kfree_skb(skb);
...
#ifdef CONFIG_SCSC_WLAN_EHT
static void kunit_mock_slsi_rx_mlo_link_info_ind(struct slsi_dev *sdev, struct net_device *dev, struct sk_buff *skb)
{
	if (skb) {
		kfree_skb(skb);
		skb = NULL;
...
static void kunit_mock_slsi_rx_mlo_link_measurement_ind(struct slsi_dev *sdev, struct net_device *dev, struct sk_buff *skb)
{
	if (skb) {
		kfree_skb(skb);
		skb = NULL;
...
static void kunit_mock_slsi_rx_mlo_set_ttlm_ind(struct slsi_dev *sdev, struct net_device *dev, struct sk_buff *skb)
{
	if (skb) {
		kfree_skb(skb);
		skb = NULL;
