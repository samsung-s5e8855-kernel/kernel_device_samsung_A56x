...
#define slsi_mlme_set_keepalive_parameters(args...)             kunit_mock_slsi_mlme_set_keepalive_parameters(args)
...
#define slsi_mlme_set_max_bw(args...)				kunit_mock_slsi_mlme_set_max_bw(args)
#define slsi_mlme_set_with_vifnum(args...)			kunit_mock_slsi_mlme_set_with_vifnum(args)
#define slsi_mlme_set_scan_timing_req(args...)			kunit_mock_slsi_mlme_set_scan_timing_req(args)
#define slsi_mlme_sar_get_avg_tx_power(args...)			kunit_mock_slsi_mlme_sar_get_avg_tx_power(args)
#define slsi_mlme_sar_get_tx_power_limit(args...)		kunit_mock_slsi_mlme_sar_get_tx_power_limit(args)
#define slsi_mlme_sar_set_index(args...)			kunic_mock_slsi_mlme_sar_set_index(args)
#define slsi_mlme_tas_deferred_tx_sar_limit(args...)		kunit_mock_slsi_mlme_tas_deferred_tx_sar_limit(args)
...
{
	*host_dst = kzalloc(sizeof(u8), GFP_KERNEL);
	*datalen = 10;
	return 0;
}
						      ...
						      u32 af_bitmap_active, u32 af_bitmap_suspended)
{
	...
	if (!ndev_vif->activated) {
		...
		if (sdev->chip_info_mib.chip_version > 0 && ndev_vif->iftype == NL80211_IFTYPE_P2P_CLIENT)
			return -EINVAL;
	}
...
static int kunit_mock_slsi_mlme_delba_req(struct slsi_dev *sdev, struct net_device *dev,
					  u16 vif, u8 *peer_qsta_address,
					  u16 priority, u16 direction,
					  u16 sequence_number, u16 reason_code)
{
	return 0;
}
					...
					const u8 *bssid)
{
	...
	if (ndev_vif->ifnum == SLSI_NET_INDEX_AP && sdev->p2p_state == P2P_IDLE_VIF_ACTIVE)
		return -EINVAL;
					...
					u8 *interface_address, u8 *device_address)
{
	...
	if (!ndev_vif->activated) {
		if (ndev_vif->sta.vif_status == SLSI_VIF_STATUS_DISCONNECTING)
			r = -EINVAL;
		if (ndev_vif->vif_type == FAPI_VIFTYPE_STATION &&
		    ndev_vif->sta.vif_status == SLSI_VIF_STATUS_UNSPECIFIED) {
			if (sdev->chip_info_mib.chip_version < 1)
				r = -EINVAL;
		}
	}
...

{
	...
	if (ndev_vif->iftype == NL80211_IFTYPE_AP)
		*command_pos += snprintf(command + *command_pos, command_len - *command_pos, "%d 0 0 0\n", setup_id);
	else
		*command_pos += snprintf(command + *command_pos, command_len - *command_pos, "%d 0 0 0 0\n", setup_id);
							...
							int mib_count, struct slsi_mib_data *mibrsp)
{
	...
	if (!sdev)
		return NULL;
	if (mibrsp && mibrsp->data)
		memset(mibrsp->data, 0, mibrsp->dataLength);
	for (i = 0; i < mib_count; i++) {
		if (sdev->recovery_timeout == 1) {
			...
			values[0].u.octetValue.data = mibrsp->data;
		...
		} else if (sdev->recovery_timeout == 5) {	// slsi_get_sr_per_obss_statistics
			values[i].type = SLSI_MIB_TYPE_OCTET;
			values[i].u.octetValue.data = mibrsp->data;
			values[i].u.octetValue.data[0] = 1;
			for (j = 1; j < 37; j += 4)
				values[i].u.octetValue.data[j] = j;
			for (j = 37; j < 43; j++)
				values[i].u.octetValue.data[j] = j;
			values[i].u.octetValue.dataLength = mibrsp->dataLength;
		} else if (sdev->recovery_timeout == 6) {	// slsi_get_obss_statistics
			values[i].type = SLSI_MIB_TYPE_OCTET;
			values[i].u.octetValue.data = mibrsp->data;
			values[i].u.octetValue.data[0] = 1;
			for (j = 1; j < 42; j++)
				values[i].u.octetValue.data[j] = j;
			values[i].u.octetValue.dataLength = mibrsp->dataLength;
		} else if (sdev->recovery_timeout == 7) {	// test_ioctl_slsi_get_regulatory
			values[i].type = SLSI_MIB_TYPE_OCTET;
			values[i].u.octetValue.data = mibrsp->data;
			values[i].u.octetValue.dataLength = 10;
			for (j = 0; j < 10; j += 2) {
				values[i].u.octetValue.data[j] = j;
				values[i].u.octetValue.data[j + 1] = j;
			}
		} else if (sdev->recovery_timeout == 99) {	// OCTET default
			values[i].type = SLSI_MIB_TYPE_OCTET;
			values[i].u.octetValue.data = mibrsp->data;
			for (j = 0; j < mibrsp->dataLength; j++)
				values[i].u.octetValue.data[j] = j;
			values[i].u.octetValue.dataLength = mibrsp->dataLength;
		} else {
...
static int kunit_mock_slsi_mlme_get_sinfo_mib(struct slsi_dev *sdev, struct net_device *dev,
			    struct slsi_peer *peer, u16 ifnum)
{
...
static struct sk_buff *kunit_mock_slsi_mlme_set_with_cfm(struct slsi_dev *sdev, struct net_device *dev,
							 u8 *mib, int mib_len, u16 vifnum)
{
	if (!sdev)
		return NULL;
	else if (dev) {
		struct netdev_vif *ndev_vif = netdev_priv(dev);
		if (!vifnum || !ndev_vif->is_available)
			return NULL;
	}
...
static int kunit_mock_slsi_mlme_set_keepalive_parameters(struct slsi_dev *sdev, struct net_device *dev, unsigned int keepalive_time)
{
		return 0;
}
...
{
	int result_code = FAPI_RESULTCODE_SUCCESS;
...
static int kunit_mock_slsi_mlme_set_max_bw(struct slsi_dev *sdev, struct net_device *dev, u8 link_id, u16 protection_scope,
...
static int kunit_mock_slsi_mlme_set_with_vifnum(struct slsi_dev *sdev, struct net_device *dev, u8 *mib, int mib_len, u16 vifnum)
{
	return 0;
}

static int kunit_mock_slsi_mlme_set_scan_timing_req(struct slsi_dev *sdev, struct net_device *dev, u16 scan_mode, u16 max_channel_time,
				  u16 home_away_time, u16 home_time, u16 max_channel_passive_time)
{
	return 0;
}

static int kunit_mock_slsi_mlme_sar_get_avg_tx_power(struct slsi_dev *sdev, struct net_device *dev, u8 dsi_id, u8 report_mode,
				   struct slsi_sar_avg_tx_power *avg_tx_pwr)
{
	return 0;
}

static int kunit_mock_slsi_mlme_sar_get_tx_power_limit(struct slsi_dev *sdev, struct net_device *dev, u16 op_class, u8 dsi_id,
				     struct slsi_mlme_sar_tx_power_limit *limits)
{
	return 0;
}

static int kunic_mock_slsi_mlme_sar_set_index(struct slsi_dev *sdev, struct net_device *dev, u8 dsi_id)
{
	return 0;
}

static void kunit_mock_slsi_mlme_tas_deferred_tx_sar_limit(struct slsi_dev *sdev)
{
}
#endif
