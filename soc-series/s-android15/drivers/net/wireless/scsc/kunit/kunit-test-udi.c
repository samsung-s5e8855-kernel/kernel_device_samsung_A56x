...
static void test_slsi_udi_deinit(struct kunit *test)
{
	...
	KUNIT_EXPECT_EQ(test, -1, slsi_udi_deinit());
}
...
static void test_send_signal_to_log_filter(struct kunit *test)
{
	...
	((struct fapi_signal *)skb->data)->id = cpu_to_le16(1);
	log_client->min_signal_id = 0;
	log_client->max_signal_id = 2;
	KUNIT_EXPECT_EQ(test, 0, send_signal_to_log_filter(log_client, skb, UDI_FROM_HOST));
...
static void test_send_signal_to_inverse_log_filter(struct kunit *test)
{
	...
	log_client->min_signal_id = 0;
	log_client->max_signal_id = 2;
	KUNIT_EXPECT_EQ(test, -ECANCELED, send_signal_to_inverse_log_filter(log_client, skb, UDI_FROM_HOST));

	cdev_client->ma_unitdata_filter_config = UDI_MA_UNITDATA_FILTER_ALLOW_MGT_ID;
	KUNIT_EXPECT_EQ(test, 0, send_signal_to_inverse_log_filter(log_client, skb, UDI_FROM_HOST));
...
static void test_udi_log_event(struct kunit *test)
{
	...
	skb = fapi_alloc(ma_blockackreq_ind, MA_BLOCKACKREQ_IND, 0, fapi_sig_size(ma_blockackreq_ind));
	KUNIT_EXPECT_EQ(test, 0, udi_log_event(log_client, skb, UDI_FROM_HOST));
	kfree_skb(skb);

	skb = fapi_alloc(ma_unitdata_ind, MA_UNITDATA_IND, 0, fapi_sig_size(ma_unitdata_ind));
