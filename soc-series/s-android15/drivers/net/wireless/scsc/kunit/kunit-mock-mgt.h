...
#define slsi_get_ifnum_by_vifid(args...)				kunit_mock_slsi_get_ifnum_by_vifid(args)
#define slsi_mlme_clear_vif(args...)					kunit_mock_slsi_mlme_clear_vif(args)
#define slsi_mlme_assign_vif(args...)					kunit_mock_slsi_mlme_assign_vif(args)
#define slsi_is_valid_vifnum(args...)					kunit_mock_slsi_is_valid_vifnum(args)
#ifdef CONFIG_SCSC_WLAN_SUPPORT_6G
#define slsi_set_mib_6g_safe_mode(args...)				kunit_mock_slsi_set_mib_6g_safe_mode(args)
#endif
#define slsi_set_mib_srp_non_srg_obss_pd_prohibited(args...)		kunit_mock_slsi_set_mib_srp_non_srg_obss_pd_prohibited(args)
#define slsi_set_mib_obss_pd_enable_per_obss(args...)			kunit_mock_slsi_set_mib_obss_pd_enable_per_obss(args)
#define slsi_fill_current_ap_detail(args...)				kunit_mock_slsi_fill_current_ap_detail(args)
...
static int kunit_mock_slsi_peer_remove(struct slsi_dev *sdev, struct net_device *dev, struct slsi_peer *peer)
{
	...
	queueset = MAP_AID_TO_QS(peer->aid);
...
static int kunit_mock_slsi_set_boost(struct slsi_dev *sdev, struct net_device *dev)
{
	...
	if (!ndev_vif->activated) {
		if (sdev->device_state == SLSI_DEVICE_STATE_STARTED && sdev->chip_info_mib.chip_version == 22)
			return 0;
		return 1;
	} else {
		if (sdev->chip_info_mib.chip_version < 22)
			return 1;
	}
...
static void alloc_sta_record(struct net_device *dev, u16 queueset)
{
	struct netdev_vif *ndev_vif = netdev_priv(dev);

	if (ndev_vif->peer_sta_record[queueset]) {
		SLSI_NET_WARN(dev, "peer is exist in queuset(%d)\n", queueset);
		return;
	}
						  ...
						  u8 *peer_address, u16 aid)
{
	...
	if (sdev->device_config.user_suspend_mode < 0)
		return NULL;

	if (sdev->device_config.qos_info == 981)
		alloc_sta_record(dev, queueset);
...
static int kunit_mock_slsi_read_regulatory_rules(struct slsi_dev *sdev,
						 ...
						 const char *alpha2)
{
	if (!domain_info || !domain_info->regdomain)
		return 1;

	domain_info->regdomain->n_reg_rules = 2;
	return 0;
}
...
static int kunit_mock_slsi_get_ifnum_by_vifid(struct slsi_dev *sdev, u16 vif_id)
{
...
static void kunit_mock_slsi_mlme_clear_vif(struct slsi_dev *sdev, struct net_device *dev, u16 vif_id)
{
}

static void kunit_mock_slsi_mlme_assign_vif(struct slsi_dev *sdev, struct net_device *dev, u16 vif_id)
{
}

static bool kunit_mock_slsi_is_valid_vifnum(struct slsi_dev *sdev, struct net_device *dev)
{
	struct netdev_vif *ndev_vif = netdev_priv(dev);
...
#ifdef CONFIG_SCSC_WLAN_SUPPORT_6G
static int kunit_mock_slsi_set_mib_6g_safe_mode(struct slsi_dev *sdev, bool enable)
{
	return 0;
}
#endif

static int kunit_mock_slsi_set_mib_srp_non_srg_obss_pd_prohibited(struct slsi_dev *sdev, bool enable)
{
	return 0;
}

static int kunit_mock_slsi_set_mib_obss_pd_enable_per_obss(struct slsi_dev *sdev, struct net_device *dev,
					 int config_cnt, int *rssi, u8 mac_addr[][ETH_ALEN])
{
	return 0;
}

static int kunit_mock_slsi_fill_current_ap_detail(struct slsi_dev *sdev, struct net_device *dev,
