...
static void test_slsi_nl80211_vendor_init_deinit(struct kunit *test)
{
	...
	nla_attr = kunit_nla_set_nested_attr(nla_attr, LLS_ATTRIBUTE_STATS_VERSION, (void *)&version, 0, 0, &len);
	KUNIT_EXPECT_EQ(test, -EINVAL, sdev->wiphy->vendor_commands[17].doit(sdev->wiphy, &ndev_vif->wdev, nla_attr, len));
...
static void test_slsi_gscan_hash_remove(struct kunit *test)
{
	...
	struct sk_buff           *skb = fapi_alloc(mlme_scan_ind, MLME_SCAN_IND, 0, 50);
	struct ieee80211_mgmt    *buf = kunit_kzalloc(test, sizeof(struct ieee80211_mgmt) + sizeof(u8) * 12, GFP_KERNEL);
	struct slsi_gscan_result *scan_res = kmalloc(100, GFP_KERNEL);
	...
	slsi_gscan_hash_add(sdev, scan_res);
	slsi_gscan_hash_remove(sdev, fapi_get_mgmt(skb)->bssid);
	kfree(scan_res);

	scan_res = slsi_prepare_scan_result(skb, 0, 1);
	if (scan_res != NULL)
		slsi_gscan_hash_add(sdev, scan_res);
...
static void test_slsi_check_scan_result(struct kunit *test)
{
	...
	if (scan_res) {
		slsi_gscan_hash_add(sdev, scan_res);
		scan_res->scan_cycle = 0;
	}

	KUNIT_EXPECT_EQ(test, SLSI_KEEP_SCAN_RESULT, slsi_check_scan_result(sdev, bucket, scan_res));
	kfree_skb(skb);
}
...
static void test_slsi_gscan_add_mlme(struct kunit *test)
{
	struct slsi_dev *sdev = TEST_TO_SDEV(test);
	struct slsi_nl_gscan_param *nl_gscan_param = kunit_kzalloc(test, sizeof(struct slsi_nl_gscan_param),
								   GFP_KERNEL);
	struct slsi_gscan *gscan = kunit_kzalloc(test, sizeof(struct slsi_gscan), GFP_KERNEL);
	...
	gscan->bucket[0]->report_events = 8;
	KUNIT_EXPECT_EQ(test, -EINVAL, slsi_gscan_add_mlme(sdev, nl_gscan_param, gscan));

	gscan->bucket[0]->report_events = 0;
	sdev->epno_active = true;
	KUNIT_EXPECT_EQ(test, 0, slsi_gscan_add_mlme(sdev, nl_gscan_param, gscan));

	gscan->bucket[0]->report_events =
		SLSI_REPORT_EVENTS_EACH_SCAN | SLSI_REPORT_EVENTS_FULL_RESULTS | SLSI_REPORT_EVENTS_NO_BATCH;
#ifdef CONFIG_SCSC_WLAN_ENABLE_MAC_RANDOMISATION
	sdev->scan_addr_set = true;
#endif
	nl_gscan_param->nl_bucket[0].num_channels = 2;
	KUNIT_EXPECT_EQ(test, 1, slsi_gscan_add_mlme(sdev, nl_gscan_param, gscan));
}
...
static void test_slsi_gscan_add(struct kunit *test)
{
	...
	nla_attr = kunit_nla_set_nested_attr(nla_attr, GSCAN_ATTRIBUTE_BASE_PERIOD, &data, 0, 0, &len);
	init_len = len;
	nla_attr = kunit_nla_set_nested_attr(nla_attr, GSCAN_ATTRIBUTE_NUM_AP_PER_SCAN, &data, 0, 0, &len);
	nla_attr = kunit_nla_set_nested_attr(nla_attr, GSCAN_ATTRIBUTE_REPORT_THRESHOLD_NUM_SCANS, &data, 0, 0, &len);
	nla_attr = kunit_nla_set_nested_attr(nla_attr, GSCAN_ATTRIBUTE_NUM_BUCKETS, &data, 0, 0, &len);
	nla_attr = kunit_nla_set_nested_attr(nla_attr, GSCAN_ATTRIBUTE_CH_BUCKET_8, &data, 0, 0, &len);
	nla_attr = kunit_nla_set_nested_attr(nla_attr, GSCAN_ATTRIBUTE_MAX, &data, 0, 0, &len);
	nla_attr = (struct nlattr *)((char *)nla_attr - len + init_len);

	kunit_set_gscan_disabled(true);
	KUNIT_EXPECT_EQ(test, -ENOTSUPP, slsi_gscan_add(sdev->wiphy, wdev, nla_attr, len));

	kunit_set_gscan_disabled(false);
	KUNIT_EXPECT_EQ(test, -EINVAL, slsi_gscan_add(sdev->wiphy, wdev, nla_attr, len));
}
...
static void test_slsi_lls_clear_stats(struct kunit *test)
{
	...
	KUNIT_EXPECT_EQ(test, -EINVAL, slsi_lls_clear_stats(wiphy, wdev, nla_attr, len));
}
...
static void test_slsi_lls_get_stats(struct kunit *test)
{
	...
	nla_attr = kunit_nla_set_nested_attr(nla_attr, LLS_ATTRIBUTE_STATS_VERSION, (void *)&version, 0, 0, &len);
	init_len = len;
	nla_attr = kunit_nla_set_nested_attr(nla_attr, LLS_ATTRIBUTE_GET_STATS_TYPE, 0, 0, 0, &len);
	nla_attr = (struct nlattr *)((char *)nla_attr - len + init_len);

	KUNIT_EXPECT_EQ(test, -EINVAL, slsi_lls_get_stats(sdev->wiphy, &ndev_vif->wdev, nla_attr, len));
}
...
static void test_slsi_apf_read_filter(struct kunit *test)
{
	...
	KUNIT_EXPECT_EQ(test, -ENOTSUPP, slsi_apf_read_filter(wiphy, wdev, 0, 0));

	sdev->device_config.fw_apf_supported = true;
	KUNIT_EXPECT_EQ(test, 1, slsi_apf_read_filter(wiphy, wdev, 0, 0));
}
...
static void test_slsi_rtt_set_get(struct kunit *test)
{
	...
	sdev->device_config.fw_apf_supported = false;
	KUNIT_EXPECT_LE(test, 1, slsi_rtt_get_capabilities(wiphy, 0, 0, 0));

	kunit_set_rtt_disabled(true);
	KUNIT_EXPECT_EQ(test, WIFI_HAL_ERROR_NOT_SUPPORTED, slsi_rtt_set_config(wiphy, wdev, 0, 0));

	kunit_set_rtt_disabled(false);
	KUNIT_EXPECT_EQ(test, -EINVAL, slsi_rtt_set_config(wiphy, wdev, 0, 0));

	nl_skb = nlmsg_new(100, GFP_KERNEL);
	nl_hdr = nlmsg_hdr(nl_skb);
	nlmsg_put(nl_skb, 0, 0, 0, 0, 0);
	nla_put_u8(nl_skb, SLSI_RTT_ATTRIBUTE_TARGET_CNT, 1);
	nla_put_u16(nl_skb, SLSI_RTT_ATTRIBUTE_TARGET_ID, 3);
	nlattr_nested = nla_nest_start(nl_skb, SLSI_RTT_ATTRIBUTE_TARGET_INFO);
	nlattr_nested2 = nla_nest_start(nl_skb, SLSI_RTT_ATTRIBUTE_TARGET_INFO);
	nla_put(nl_skb, SLSI_RTT_ATTRIBUTE_TARGET_MAC, ETH_ALEN, "aabbccddeeff");
	nla_put_u8(nl_skb, SLSI_RTT_ATTRIBUTE_TARGET_TYPE, 1);
	nla_put_u8(nl_skb, SLSI_RTT_ATTRIBUTE_TARGET_PEER, 2);
	nla_put_u16(nl_skb, SLSI_RTT_ATTRIBUTE_TARGET_CHAN_FREQ, 3);
	nla_put_u8(nl_skb, SLSI_RTT_ATTRIBUTE_TARGET_PERIOD, 4);
	nla_put_u8(nl_skb, SLSI_RTT_ATTRIBUTE_TARGET_NUM_BURST, 5);
	nla_put_u8(nl_skb, SLSI_RTT_ATTRIBUTE_TARGET_NUM_FTM_BURST, 6);
	nla_put_u8(nl_skb, SLSI_RTT_ATTRIBUTE_TARGET_NUM_RETRY_FTMR, 7);
	nla_put_u8(nl_skb, SLSI_RTT_ATTRIBUTE_TARGET_BURST_DURATION, 8);
	nla_put_u16(nl_skb, SLSI_RTT_ATTRIBUTE_TARGET_PREAMBLE, 9);
	nla_put_u16(nl_skb, SLSI_RTT_ATTRIBUTE_TARGET_BW, 10);
	nla_put_u16(nl_skb, SLSI_RTT_ATTRIBUTE_TARGET_LCI, 11);
	nla_put_u16(nl_skb, SLSI_RTT_ATTRIBUTE_TARGET_LCR, 12);
	nla_nest_end(nl_skb, nlattr_nested2);
	nla_nest_end(nl_skb, nlattr_nested);
	nlmsg_end(nl_skb, nl_hdr);
	KUNIT_EXPECT_EQ(test, 0, slsi_rtt_set_config(wiphy, wdev, nlmsg_data(nl_hdr), nlmsg_len(nl_hdr)));
	nlmsg_free(nl_skb);

	kunit_set_rtt_disabled(true);
	KUNIT_EXPECT_EQ(test, -ENOTSUPP, slsi_rtt_cancel_config(wiphy, wdev, 0, 0));

	kunit_set_rtt_disabled(false);
	KUNIT_EXPECT_EQ(test, 1, slsi_rtt_cancel_config(wiphy, wdev, 0, 0));

	nl_skb = nlmsg_new(100, GFP_KERNEL);
	nl_hdr = nlmsg_hdr(nl_skb);
	nlmsg_put(nl_skb, 0, 0, 0, 0, 0);
	nla_put_u16(nl_skb, SLSI_RTT_ATTRIBUTE_TARGET_CNT, 1);
	nlmsg_end(nl_skb, nl_hdr);
	KUNIT_EXPECT_EQ(test, 1, slsi_rtt_cancel_config(wiphy, wdev, nlmsg_data(nl_hdr), nlmsg_len(nl_hdr)));
	nlmsg_free(nl_skb);
	...
	nla_put(nl_skb, SLSI_RTT_ATTRIBUTE_TARGET_MAC, ETH_ALEN, "aabbccddeeff");
	nlmsg_end(nl_skb, nl_hdr);
	KUNIT_EXPECT_EQ(test, 0, slsi_rtt_cancel_config(wiphy, wdev, nlmsg_data(nl_hdr), nlmsg_len(nl_hdr)));
	nlmsg_free(nl_skb);
}
...
static void test_slsi_rx_range_ind(struct kunit *test)
{
	...
	struct slsi_dev   *sdev = TEST_TO_SDEV(test);
	struct sk_buff    *skb;
	u8 data[SLSI_WIFI_RTT_RESULT_LENGTH + 2] = {0};

	skb = fapi_alloc(mlme_range_ind, MLME_RANGE_IND, 0, 100);
	fapi_set_u16(skb, u.mlme_range_ind.entries, 10);
	...
	fapi_set_u16(skb, u.mlme_range_ind.entries, 1);
	fapi_set_u16(skb, u.mlme_range_ind.rtt_id, 2);
	data[0] = SLSI_WIFI_RTT_RESULT_ID;
	fapi_append_data(skb, data, sizeof(data));
	slsi_rx_range_ind(sdev, dev, skb);
	...
	fapi_set_u16(skb, u.mlme_range_ind.rtt_id, 2);
	data[0] = SLSI_WIFI_RTT_RESULT_ID;
	data[1] = SLSI_WIFI_RTT_RESULT_LENGTH;
	fapi_append_data(skb, data, sizeof(data));
	slsi_rx_range_ind(sdev, dev, skb);
}
...
static void test_str_type_check(struct kunit *test)
{
	...
	KUNIT_EXPECT_STREQ(test, "UNKNOWN_REASON", slsi_get_roam_reason_str(-1, reason_code));
	KUNIT_EXPECT_STREQ(test, "WIFI_ROAMING_SEARCH_REASON_FORCED_ROAMING", slsi_get_roam_reason_str(0, reason_code));
	KUNIT_EXPECT_STREQ(test, "WIFI_ROAMING_SEARCH_REASON_LOW_RSSI", slsi_get_roam_reason_str(1, reason_code));
	KUNIT_EXPECT_STREQ(test, "WIFI_ROAMING_SEARCH_REASON_CHANNEL_SWITCH_FAILURE",
			   slsi_get_roam_reason_str(3, FAPI_REASONCODE_CHANNEL_SWITCH_FAILURE));
	KUNIT_EXPECT_STREQ(test, "WIFI_ROAMING_SEARCH_REASON_BTM_REQ", slsi_get_roam_reason_str(5, reason_code));
	KUNIT_EXPECT_STREQ(test, "WIFI_ROAMING_SEARCH_REASON_CU_TRIGGER", slsi_get_roam_reason_str(2, reason_code));
	KUNIT_EXPECT_STREQ(test, "WIFI_ROAMING_SEARCH_REASON_EMERGENCY", slsi_get_roam_reason_str(4, reason_code));
	KUNIT_EXPECT_STREQ(test, "WIFI_ROAMING_SEARCH_REASON_IDLE", slsi_get_roam_reason_str(6, reason_code));
	KUNIT_EXPECT_STREQ(test, "WIFI_ROAMING_SEARCH_REASON_WTC", slsi_get_roam_reason_str(7, reason_code));
	KUNIT_EXPECT_STREQ(test, "WIFI_ROAMING_SEARCH_REASON_INACTIVITY_TIMER",
			   slsi_get_roam_reason_str(8, reason_code));
	KUNIT_EXPECT_STREQ(test, "WIFI_ROAMING_SEARCH_REASON_SCAN_TIMER", slsi_get_roam_reason_str(9, reason_code));
	KUNIT_EXPECT_STREQ(test, "WIFI_ROAMING_SEARCH_REASON_BT_COEX", slsi_get_roam_reason_str(10, reason_code));

	KUNIT_EXPECT_STREQ(test, "Not Set", slsi_get_nan_role_str(0));
	KUNIT_EXPECT_STREQ(test, "Anchor Master", slsi_get_nan_role_str(1));
	KUNIT_EXPECT_STREQ(test, "Master", slsi_get_nan_role_str(2));
	KUNIT_EXPECT_STREQ(test, "Sync", slsi_get_nan_role_str(3));
	KUNIT_EXPECT_STREQ(test, "Non Sync", slsi_get_nan_role_str(4));
	KUNIT_EXPECT_STREQ(test, "Undefined", slsi_get_nan_role_str(-1));

	KUNIT_EXPECT_STREQ(test, "eap_message", slsi_frame_transmit_failure_message_type(1));
	KUNIT_EXPECT_STREQ(test, "eapol_key_m123", slsi_frame_transmit_failure_message_type(2));
	KUNIT_EXPECT_STREQ(test, "eapol_key_m4", slsi_frame_transmit_failure_message_type(3));
	KUNIT_EXPECT_STREQ(test, "arp", slsi_frame_transmit_failure_message_type(4));
	KUNIT_EXPECT_STREQ(test, "dhcp", slsi_frame_transmit_failure_message_type(5));
	KUNIT_EXPECT_STREQ(test, "neighbor_discovery", slsi_frame_transmit_failure_message_type(6));
	KUNIT_EXPECT_STREQ(test, "wai_message", slsi_frame_transmit_failure_message_type(7));
	KUNIT_EXPECT_STREQ(test, "any_other", slsi_frame_transmit_failure_message_type(8));
	KUNIT_EXPECT_STREQ(test, "Undefined", slsi_frame_transmit_failure_message_type(-1));

	KUNIT_EXPECT_STREQ(test, "Soft Cached scan", slsi_get_scan_type(FAPI_SCANTYPE_SOFT_CACHED_ROAMING_SCAN));
	KUNIT_EXPECT_STREQ(test, "Soft Full scan", slsi_get_scan_type(FAPI_SCANTYPE_SOFT_FULL_ROAMING_SCAN));
	KUNIT_EXPECT_STREQ(test, "Hard Cached scan", slsi_get_scan_type(FAPI_SCANTYPE_HARD_CACHED_ROAMING_SCAN));
	KUNIT_EXPECT_STREQ(test, "Hard Full scan", slsi_get_scan_type(FAPI_SCANTYPE_HARD_FULL_ROAMING_SCAN));
	KUNIT_EXPECT_STREQ(test, "Undefined", slsi_get_scan_type(-1));

	KUNIT_EXPECT_STREQ(test, "passive", slsi_get_measure_mode(0));
	KUNIT_EXPECT_STREQ(test, "active", slsi_get_measure_mode(1));
	KUNIT_EXPECT_STREQ(test, "beacon_table", slsi_get_measure_mode(2));
	KUNIT_EXPECT_STREQ(test, "Undefined", slsi_get_measure_mode(-1));

	KUNIT_EXPECT_STREQ(test, "NAN_FAW", slsi_get_nan_schedule_type_str(1));
	KUNIT_EXPECT_STREQ(test, "NAN_NDC", slsi_get_nan_schedule_type_str(2));
	KUNIT_EXPECT_STREQ(test, "NAN_DW", slsi_get_nan_schedule_type_str(3));
	KUNIT_EXPECT_STREQ(test, "Undefined", slsi_get_nan_schedule_type_str(4));

	KUNIT_EXPECT_STREQ(test, "Peer Requested", slsi_get_nan_ulw_reason_str(1));
	KUNIT_EXPECT_STREQ(test, "Concurrent Operation", slsi_get_nan_ulw_reason_str(2));
	KUNIT_EXPECT_STREQ(test, "Scan", slsi_get_nan_ulw_reason_str(3));
	KUNIT_EXPECT_STREQ(test, "BT_COEX", slsi_get_nan_ulw_reason_str(4));
	KUNIT_EXPECT_STREQ(test, "Power Saving", slsi_get_nan_ulw_reason_str(5));
	KUNIT_EXPECT_STREQ(test, "Deleted", slsi_get_nan_ulw_reason_str(6));
	KUNIT_EXPECT_STREQ(test, "Undefined", slsi_get_nan_ulw_reason_str(-1));
}

static void test_slsi_print_availability_log_ind(struct kunit *test)
{
	struct slsi_dev *sdev = TEST_TO_SDEV(test);
	struct parameter_set param;

	param.width = 176;
	param.slot_bitmap = 1;
	slsi_print_availability_log_ind(sdev, &param, "KUNIT", 1234);

	param.slot_bitmap = 0;
	slsi_print_availability_log_ind(sdev, &param, "KUNIT", 5678);
}
...
static void test_slsi_rx_event_log_parse_vsie(struct kunit *test)
{
	struct slsi_dev   *sdev = TEST_TO_SDEV(test);
	struct slsi_rx_evt_info evt_info;

	evt_info.vd.vs_ie = NULL;
	slsi_rx_event_log_parse_vsie(sdev, "KUNIT", &evt_info, sizeof("KUNIT"));
...
static void test_dump_roam_scan_result(struct kunit *test)
{
	...
	dump_roam_scan_result(sdev, dev, &curr, bssid, freq, rssi, cu, score, tp_score, eligible_value, mld_ap);
...
static void test_slsi_on_alert(struct kunit *test)
{
	...
	sdev->wiphy->n_vendor_events = SLSI_NL80211_LOGGER_FW_DUMP_EVENT + 1;
	slsi_on_alert(buffer, buffer_size, err_code, sdev);
}
...
static void test_slsi_get_fw_mem_dump(struct kunit *test)
{
	...
	if (!mem_dump_buffer) {
		mem_dump_buffer = kmalloc(strlen(buffer) - 1, GFP_KERNEL);
...
static void test_slsi_get_usable_channels(struct kunit *test)
{
	...
	KUNIT_EXPECT_EQ(test, -EINVAL, slsi_get_usable_channels(wiphy, wdev, NULL, 0));

	nl_skb = nlmsg_new(100, GFP_KERNEL);
	nl_hdr = nlmsg_hdr(nl_skb);
	nlmsg_put(nl_skb, 0, 0, 0, 0, 0);
	nla_put_u32(nl_skb, SLSI_UC_ATTRIBUTE_BAND, 1);
	nla_put_u32(nl_skb, SLSI_UC_ATTRIBUTE_IFACE_MODE, -1);
	nla_put_u32(nl_skb, SLSI_UC_ATTRIBUTE_FILTER, 3);
	nla_put_u32(nl_skb, SLSI_UC_ATTRIBUTE_MAX_NUM, 4);
	nla_put_u32(nl_skb, -1, 5);
	nlmsg_end(nl_skb, nl_hdr);
	KUNIT_EXPECT_EQ(test, -EINVAL, slsi_get_usable_channels(wiphy, wdev, nlmsg_data(nl_hdr), nlmsg_len(nl_hdr)));
	nlmsg_free(nl_skb);
	...
	KUNIT_EXPECT_EQ(test, -EOPNOTSUPP,
			slsi_get_usable_channels(wiphy, wdev, nlmsg_data(nl_hdr), nlmsg_len(nl_hdr)));
	nlmsg_free(nl_skb);
	...
	nlmsg_put(nl_skb, 0, 0, 0, 0, 0);
	nla_put_u32(nl_skb, SLSI_UC_ATTRIBUTE_BAND, 1);
	nla_put_u32(nl_skb, SLSI_UC_ATTRIBUTE_IFACE_MODE, 2);
	nla_put_u32(nl_skb, SLSI_UC_ATTRIBUTE_FILTER, 3);
	nla_put_u32(nl_skb, SLSI_UC_ATTRIBUTE_MAX_NUM, 4);
	nlmsg_end(nl_skb, nl_hdr);
	KUNIT_EXPECT_EQ(test, -EOPNOTSUPP,
	...
	wiphy->bands[NL80211_BAND_5GHZ]->n_channels = 1;
#ifdef CONFIG_SCSC_WLAN_SUPPORT_6G
	wiphy->bands[NL80211_BAND_6GHZ] = kunit_kzalloc(test, sizeof(struct ieee80211_supported_band), GFP_KERNEL);
	wiphy->bands[NL80211_BAND_6GHZ]->channels = kunit_kzalloc(test, sizeof(struct ieee80211_channel), GFP_KERNEL);
	wiphy->bands[NL80211_BAND_6GHZ]->n_channels = 1;
#endif
#endif
	KUNIT_EXPECT_EQ(test, 1, slsi_get_usable_channels(wiphy, wdev, nlmsg_data(nl_hdr), nlmsg_len(nl_hdr)));
	nlmsg_free(nl_skb);
}
...
static void test_slsi_set_dtim_config(struct kunit *test)
{
	...
	ndev_vif->vif_type = FAPI_VIFTYPE_AP;
	KUNIT_EXPECT_EQ(test, -EINVAL, slsi_set_dtim_config(wiphy, wdev, NULL, 0));
	...
	nl_hdr = nlmsg_hdr(nl_skb);
	nlmsg_put(nl_skb, 0, 0, 0, 0, 0);
	nla_put_u16(nl_skb, SLSI_VENDOR_ATTR_DTIM_MULTIPLIER, 0);
	nlmsg_end(nl_skb, nl_hdr);
	KUNIT_EXPECT_EQ(test, -EINVAL, slsi_set_dtim_config(wiphy, wdev, nlmsg_data(nl_hdr), nlmsg_len(nl_hdr)));
	nlmsg_free(nl_skb);

	nl_skb = nlmsg_new(100, GFP_KERNEL);
	nl_hdr = nlmsg_hdr(nl_skb);
	nlmsg_put(nl_skb, 0, 0, 0, 0, 0);
	nla_put_u32(nl_skb, SLSI_VENDOR_ATTR_DTIM_MULTIPLIER, 0);
	nlmsg_end(nl_skb, nl_hdr);
	KUNIT_EXPECT_EQ(test, -EINVAL, slsi_set_dtim_config(wiphy, wdev, nlmsg_data(nl_hdr), nlmsg_len(nl_hdr)));
	nlmsg_free(nl_skb);
	...
	nl_hdr = nlmsg_hdr(nl_skb);
	nlmsg_put(nl_skb, 0, 0, 0, 0, 0);
	nla_put_u32(nl_skb, SLSI_VENDOR_ATTR_DTIM_MULTIPLIER, 8);
	nla_put_u32(nl_skb, SLSI_VENDOR_ATTR_DTIM_MAX, 1);
	nlmsg_end(nl_skb, nl_hdr);
	KUNIT_EXPECT_EQ(test, -EINVAL, slsi_set_dtim_config(wiphy, wdev, nlmsg_data(nl_hdr), nlmsg_len(nl_hdr)));
	nlmsg_free(nl_skb);
	...
	nla_put_u32(nl_skb, SLSI_VENDOR_ATTR_DTIM_MULTIPLIER, 8);
	nlmsg_end(nl_skb, nl_hdr);
	KUNIT_EXPECT_EQ(test, -EIO, slsi_set_dtim_config(wiphy, wdev, nlmsg_data(nl_hdr), nlmsg_len(nl_hdr)));
	nlmsg_free(nl_skb);

	nl_skb = nlmsg_new(100, GFP_KERNEL);
	nl_hdr = nlmsg_hdr(nl_skb);
	nlmsg_put(nl_skb, 0, 0, 0, 0, 0);
	nla_put_u32(nl_skb, SLSI_VENDOR_ATTR_DTIM_MULTIPLIER, 5);
	nlmsg_end(nl_skb, nl_hdr);
	KUNIT_EXPECT_EQ(test, 0, slsi_set_dtim_config(wiphy, wdev, nlmsg_data(nl_hdr), nlmsg_len(nl_hdr)));
	nlmsg_free(nl_skb);
}
...
static void test_slsi_select_tx_power_scenario(struct kunit *test)
{
	...
	nl_hdr = nlmsg_hdr(nl_skb);
	nlmsg_put(nl_skb, 0, 0, 0, 0, 0);
	nla_put_u16(nl_skb, SLSI_NL_ATTRIBUTE_TX_POWER_SCENARIO, 0);
	nlmsg_end(nl_skb, nl_hdr);
	KUNIT_EXPECT_EQ(test, -EINVAL,
			slsi_select_tx_power_scenario(wiphy, wdev, nlmsg_data(nl_hdr), nlmsg_len(nl_hdr)));
	nlmsg_free(nl_skb);

	nl_skb = nlmsg_new(100, GFP_KERNEL);
	nl_hdr = nlmsg_hdr(nl_skb);
	nlmsg_put(nl_skb, 0, 0, 0, 0, 0);
	nla_put_u8(nl_skb, SLSI_NL_ATTRIBUTE_TX_POWER_SCENARIO, 0);
	nla_put_u8(nl_skb, -1, 0);
	nlmsg_end(nl_skb, nl_hdr);
	KUNIT_EXPECT_EQ(test, -EOPNOTSUPP,
			slsi_select_tx_power_scenario(wiphy, wdev, nlmsg_data(nl_hdr), nlmsg_len(nl_hdr)));
	nlmsg_free(nl_skb);
	...
	nl_hdr = nlmsg_hdr(nl_skb);
	nlmsg_put(nl_skb, 0, 0, 0, 0, 0);
	nla_put_u8(nl_skb, SLSI_NL_ATTRIBUTE_TX_POWER_SCENARIO, 5);
	nlmsg_end(nl_skb, nl_hdr);
	KUNIT_EXPECT_EQ(test, -EINVAL,
			slsi_select_tx_power_scenario(wiphy, wdev, nlmsg_data(nl_hdr), nlmsg_len(nl_hdr)));
	nlmsg_free(nl_skb);
	...
	nl_hdr = nlmsg_hdr(nl_skb);
	nlmsg_put(nl_skb, 0, 0, 0, 0, 0);
	nla_put_u8(nl_skb, SLSI_NL_ATTRIBUTE_TX_POWER_SCENARIO, 1);
	nlmsg_end(nl_skb, nl_hdr);
	KUNIT_EXPECT_EQ(test, 1, slsi_select_tx_power_scenario(wiphy, wdev, nlmsg_data(nl_hdr), nlmsg_len(nl_hdr)));
	nlmsg_free(nl_skb);
	...
	nla_put_u8(nl_skb, SLSI_NL_ATTRIBUTE_TX_POWER_SCENARIO, 0);
	nlmsg_end(nl_skb, nl_hdr);
	KUNIT_EXPECT_EQ(test, 0, slsi_select_tx_power_scenario(wiphy, wdev, nlmsg_data(nl_hdr), nlmsg_len(nl_hdr)));
	nlmsg_free(nl_skb);
}
...
static void test_slsi_vendor_delay_wakeup_event(struct kunit *test)
{
	...
	slsi_vendor_delay_wakeup_event(sdev, dev, delayed_wakeup_ind);

	KUNIT_EXPECT_EQ(test, 1, called_cfg80211_vendor_event);
}

static void test_slsi_vendor_change_sr_parameter_event(struct kunit *test)
{
	struct net_device	*dev = TEST_TO_DEV(test);
	struct slsi_dev		*sdev = TEST_TO_SDEV(test);
	struct slsi_spatial_reuse_params spatial_reuse_ind;

	slsi_vendor_change_sr_parameter_event(sdev, dev, spatial_reuse_ind);
...
{
	test_dev_init(test);
	slsi_nl80211_vendor_init(TEST_TO_SDEV(test));
...
{
	slsi_nl80211_vendor_deinit(TEST_TO_SDEV(test));
...
static struct kunit_case nl80211_vendor_test_cases[] = {
	...
	KUNIT_CASE(test_slsi_gscan_add_mlme),
	...
	KUNIT_CASE(test_slsi_print_availability_log_ind),
#ifdef CONFIG_SCSC_WIFI_NAN_ENABLE
	KUNIT_CASE(test_slsi_handle_nan_rx_event_log_ind),
#endif
	KUNIT_CASE(test_slsi_rx_event_log_parse_vsie),
	...
	KUNIT_CASE(test_slsi_vendor_change_sr_parameter_event),
	{}
};
