...
{
	if (data) {
		buffer->data = kzalloc(dataLength, GFP_KERNEL);
		memcpy(buffer->data, data, dataLength);
		buffer->dataLength = (u32)dataLength;

		return SLSI_MIB_STATUS_SUCCESS;
	}

	return SLSI_MIB_STATUS_ERROR;
}
...
{
	buffer->data = kmalloc(10, GFP_KERNEL);
	return 0;
}
...
{
	if (buffer) {
		if (psid == SLSI_PSID_UNIFI24_G40_MHZ_CHANNELS)
			buffer->dataLength = 1;
	}
