...
static void test_hip4_skb_to_mbulk(struct kunit *test)
{
	...
	KUNIT_EXPECT_PTR_EQ(test, NULL, hip4_skb_to_mbulk(&sdev->hip.hip_priv, skb,
...
static void test_hip4_tl_fb(struct kunit *test)
{
	...
	hip4_update_index(&sdev->hip, HIP4_MIF_Q_FH_RFB, ridx, 161);
	hip4_update_index(&sdev->hip, HIP4_MIF_Q_FH_RFB, widx, 162);
	hip4_tl_fb(&sdev->hip);
}
...
static void test_hip4_wq_ctrl(struct kunit *test)
{
	...
	hip4_update_index(&sdev->hip, HIP4_MIF_Q_TH_CTRL, ridx, 161);
	hip4_update_index(&sdev->hip, HIP4_MIF_Q_TH_CTRL, widx, 162);
...
static void test_hip_sched_wq_ctrl(struct kunit *test)
{
	...
	sdev->hip.hip_priv->hip4_wake_lock_ctrl.ws = kunit_kzalloc(test, sizeof(struct wakeup_source), GFP_KERNEL);
	wakeup_source_add(sdev->hip.hip_priv->hip4_wake_lock_ctrl.ws);
	sdev->hip.hip_priv->hip4_wake_lock_ctrl.ws->active = true;

	slsi_hip_sched_wq_ctrl(&sdev->hip);
}

static void test_hip4_napi_poll(struct kunit *test)
{
	struct slsi_dev *sdev = kunit_kzalloc(test, sizeof(struct slsi_dev), GFP_KERNEL);
	...
	hip4_napi_poll(napi, 60);

	*((u16 *)(scbrd_base + q_idx_layout[HIP4_MIF_Q_TH_DAT][1])) = 0;
	*((u16 *)(scbrd_base + q_idx_layout[HIP4_MIF_Q_TH_DAT][0])) = 2049;

	hip4_update_index(&sdev->hip, HIP4_MIF_Q_TH_DAT, ridx, 161);
	hip4_update_index(&sdev->hip, HIP4_MIF_Q_TH_DAT, widx, 162);
	hip4_napi_poll(napi, 60);
}
...
static void test_hip_from_host_dpd_intr_set(struct kunit *test)
{
	struct slsi_dev *sdev = kunit_kzalloc(test, sizeof(struct slsi_dev), GFP_KERNEL);
	...
	slsi_hip_from_host_dpd_intr_set(sdev->service, &sdev->hip);
}
static void test_hip_init(struct kunit *test)
{
	...
	kfree(sdev->hip.hip_priv->bh_ctl);
	kfree(sdev->hip.hip_priv->bh_rfb);
...
static void test_slsi_hip_transmit_frame(struct kunit *test)
{
	...
	KUNIT_EXPECT_EQ(test, -EINVAL, slsi_hip_transmit_frame(&sdev->hip, skb, true, 0, 1, 1));
}
...
static struct kunit_case hip4_test_cases[] = {
	...
	KUNIT_CASE(test_hip4_napi_poll),
	...
	KUNIT_CASE(test_hip_from_host_dpd_intr_set),
