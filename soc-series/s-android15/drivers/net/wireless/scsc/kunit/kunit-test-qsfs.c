...
static void test_slsi_qsf_encode_hw_feature(struct kunit *test)
{
	...
	KUNIT_EXPECT_EQ(test, 0,
			slsi_qsf_encode_hw_feature(sdev, misc_features_activated, he_active, buf, bytes,
						   buf_size, eht_caps, he_caps, vht_caps, ht_caps));
...
static void test_slsi_qsf_encode_sw_feature_4(struct kunit *test)
{
	...
	u32 nan_std_plus_versions = 0x0101;
	char buf[1024] = {0};
	u32 bytes = 0;
	u32 buf_size = sizeof(buf);
	...
	KUNIT_EXPECT_LT(test, 0,
			slsi_qsf_encode_sw_feature_4(sdev, misc_features_activated,
						     ...
						     nan_fast_connect, nan_std_plus_versions,
						     buf, bytes, buf_size));
}
