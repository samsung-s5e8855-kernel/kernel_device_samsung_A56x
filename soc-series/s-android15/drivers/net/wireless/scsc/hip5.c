...
MODULE_PARM_DESC(hip5_tx_zero_copy, "Tx zero copy (0: disable, 1: enable)");
...
static int hip4_dynamic_logging_tput_in_mbps = 50;
...
void slsi_trigger_pcie_interrupt(u32 intr_from_host, struct slsi_hip *hip)
{
	struct hip_priv	*hip_priv	= hip->hip_priv;
	struct slsi_dev	*sdev		= container_of(hip, struct slsi_dev, hip);
	u32 fromhost_intr		= intr_from_host - hip->hip_priv->intr_from_host_ctrl;
	...
	if (!test_bit(0, hip_priv->pcie_claim_bit)) {
		...
		scsc_service_mifintrbit_bit_set(sdev->service, intr_from_host, SCSC_MIFINTR_TARGET_WLAN);
		read_unlock_bh(&hip_priv->rw_pcie);
	}
}
#endif
...
static struct mbulk *hip5_skb_to_mbulk(struct scsc_service *service, struct hip_priv *hip, struct sk_buff *skb, bool ctrl_packet, mbulk_colour colour)
{
	struct mbulk        *m = NULL;
	...
	if (payload > 0) {
		...
		m = mbulk_with_signal_alloc_by_pool(service, pool_id, colour, clas, cb->sig_length + 4,
		...
		m = mbulk_with_signal_alloc_by_pool(service, pool_id, colour, clas, cb->sig_length + 4, 0);
		if (!m)
			return NULL;
	}
                                                ...
                                                struct sk_buff_head *skb_list)
{
	...
	while (num_bd) {
		if (!(bulk_desc->flag & HIP5_OPT_BD_CHAIN_START)) {
			...
			SLSI_DBG4(sdev, SLSI_HIP, "BD: buf_addr:0x%X, offset:%d, data_len:%d, flag:%d\n", bulk_desc->buf_addr, bulk_desc->offset, bulk_desc->data_len, bulk_desc->flag);
			...
			do {
				...
				if (HIP5_OPT_BD_BUF_ADDR_IS_SKB(bulk_desc->buf_addr)) {
					...
					tmp_skb = hip5_rx_zero_copy_get_skb_from_addr(sdev, hip_priv, bulk_desc->buf_addr);
					if (!tmp_skb) {
					...
					*skb_cnt = (*skb_cnt) + 1;
					if (bulk_desc->data_len) {
						fapi_append_data(skb, tmp_skb->data + bulk_desc->offset, bulk_desc->data_len);
						consume_skb(tmp_skb);
					}
				} else {
					mem = scsc_mx_service_mif_addr_to_ptr(service, bulk_desc->buf_addr);
...
static inline int hip5_opt_zero_copy_dma_desc_to_bd(struct hip_priv *hip_priv, struct hip5_tx_dma_desc * dma_desc, struct hip5_opt_bulk_desc *bulk_desc)
{
	...
	skb_meta.idx = i;
	memcpy(dma_desc->skb->head, &skb_meta, sizeof(struct hip5_skb_meta));
...
static int hip5_opt_hip_signal_add(struct slsi_hip *hip, enum hip5_hip_q_conf conf, scsc_mifram_ref phy_m, struct scsc_service *service, void *m_ptr, struct sk_buff *skb_fapi, bool zero_copy)
{
	...
	if (conf == HIP5_MIF_Q_FH_CTRL) {
#ifdef CONFIG_SCSC_PCIE_CHIP
		slsi_trigger_pcie_interrupt(hip_priv->intr_from_host_ctrl, hip);
#else
...
static void hip5_tl_fb(unsigned long data)
{
	...
	while (idx_r != idx_w) {
		...
		if (HIP5_OPT_BD_BUF_ADDR_IS_SKB(ref)) {
			...
			if (!colour) {
				spin_lock_bh(&hip_priv->rx_lock);
				skb = hip5_rx_zero_copy_get_skb_from_addr(sdev, hip_priv, ref);
				if (skb) {
					num_skbs++;
					consume_skb(skb);
				}
...
void slsi_pcie_claim_release(struct work_struct *work)
{
	...
	while ((i = find_first_bit(hip_priv->fromhost_intr_bit, FROMHOST_INTR_NUM)) != FROMHOST_INTR_NUM) {
		...
		if (!test_bit(0, hip_priv->pcie_claim_bit)) {
			while (scsc_mx_service_claim(WLAN_HOST)) {
				if (++ retry_cnt >= 3) {
					/* If you want to delete triggering fw panic, then bitmap should be restored. */
					SLSI_ERR_NODEV("Failed to claim PCIE. Trigger fw panic\n");
					SLSI_FW_BUG_ON_WQ(sdev, 1);
					return;
				}
			}
	...
	/* PCIE release when traffic state is not high */
	if (hip_priv->in_low_traffic_state) {
...
static void pcie_traffic_monitor_cb(void *client_ctx, u32 state, u32 tput_tx, u32 tput_rx)
{
	...
	if (state >= TRAFFIC_MON_CLIENT_STATE_HIGH && state <= TRAFFIC_MON_CLIENT_STATE_OVERRIDE) {
		hip->hip_priv->in_low_traffic_state = 0;
	} else {
		/* PCIE release when traffic state is not high */
		hip->hip_priv->in_low_traffic_state = 1;
		queue_work(hip_priv->pcie_wq, &hip_priv->pcie_ctrl.pcie_work);
	}
}
#endif
...
static void hip5_traffic_monitor_logring_cb(void *client_ctx, u32 state, u32 tput_tx, u32 tput_rx)
{
	...
	if (state == TRAFFIC_MON_CLIENT_STATE_HIGH || state == TRAFFIC_MON_CLIENT_STATE_MID || state == TRAFFIC_MON_CLIENT_STATE_OVERRIDE) {
		if (hip4_dynamic_logging) {
			atomic_set(&hip->hip_priv->in_logring_disable, 1);
			scsc_logring_enable(false);
		}
	} else {
		atomic_set(&hip->hip_priv->in_logring_disable, 0);
...
#ifdef CONFIG_SCSC_PCIE_CHIP
	slsi_trigger_pcie_interrupt(hip->hip_priv->intr_from_host_dpd, hip);
#else
...
int slsi_hip_init(struct slsi_hip *hip)
{
	...
	/* traffic monitor state for checking tput */
	hip->hip_priv->in_low_traffic_state = 1;
	/* register traffic monitor to control PCIE cliam/release */
	hip->hip_priv->pcie_client = slsi_traffic_mon_client_register(sdev,
								      hip->hip_priv,
...
static bool is_skb_dma_able(struct slsi_dev *sdev, struct slsi_hip *hip, struct sk_buff *skb)
{
	struct slsi_skb_cb *cb = slsi_skb_cb_get(skb);
	dma_addr_t skb_dma_addr;
	...
	skb_dma_map_len = (skb_headroom(skb) - move_start_addr_by) + skb_headlen(skb) + hip->hip_priv->unidat_req_tailroom;
...
bool hip5_opt_aggr_check(struct slsi_dev *sdev, struct slsi_hip *hip, struct sk_buff *skb)
{
	...
	if(!is_skb_dma_able(sdev, hip, skb)) {
		return false;
	}
...
int hip5_opt_tx_frame(struct slsi_hip *hip, struct sk_buff *skb, bool ctrl_packet, u8 vif_index, u8 peer_index, u8 priority)
{
	...
	if (payload) {
		 if (!ctrl_packet && cb->skb_dma_addr) {
			...
			m = hip5_skb_to_mbulk(service, hip->hip_priv, skb, ctrl_packet, colour);
			if (!m) {
	...
	if (payload && !ctrl_packet && cb->skb_dma_addr) {
		if (hip5_opt_hip_signal_add(hip, HIP5_MIF_Q_FH_DAT0, 0, service, &tx_dma_desc, skb_fapi, true)) {
			SLSI_ERR_NODEV("No space\n");
	...
	if (ctrl_packet)
		SCSC_BIN_TAG_DEBUG(BIN_WIFI_CTRL_TX, skb_udi->data, skb_headlen(skb_udi));
	else
		SCSC_BIN_TAG_DEBUG(BIN_WIFI_DATA_TX, skb_udi->data, skb_headlen(skb_udi));
#endif
...
void hip5_opt_aggr_tx_frame(struct scsc_service *service, struct slsi_hip *hip)
{
	...
	for (i = 0; i < SLSI_HIP_HIP5_OPT_TX_Q_MAX; i++) {
		...
		while ((idx < HIP5_OPT_NUM_BD_PER_SIGNAL_MAX) && !skb_queue_empty(&hip_priv->hip5_opt_tx_q[i].tx_q)) {
			...
			if (!skb) {
				SLSI_ERR(sdev, "no SKB in q:%d\n", i);
				goto send_signal;
			}

			/* clone the SKB for logging only if there is a logging client */
			...
			if (hip5_tx_zero_copy) {
				...
				tx_desc_list[idx].offset =  slsi_skb_cb_get(skb)->offset + fapi_sig_size(ma_unitdata_req);
				tx_desc_list[idx].dma_map_len = slsi_skb_cb_get(skb)->dma_map_len;
				tx_desc_list[idx].skb_dma_addr = slsi_skb_cb_get(skb)->skb_dma_addr;
				m_arr[idx] = (struct mbulk *)&tx_desc_list[idx];
				idx++;
			} else {
				/* create a Mbulk only if the Signal has a payload (bulk data) */
				m = hip5_skb_to_mbulk(service, hip->hip_priv, skb, false, colour);
				if (!m) {
					SLSI_ERR_NODEV("mbulk is NULL\n");
					if (skb_udi)
						consume_skb(skb_udi);
...
send_signal:
		if (skb_fapi) {
			if (idx) {
				 ...
				 * BD will be initialized when it is used in below for loop.
				 */
				hip5_q_entry = (struct hip5_opt_hip_signal *)&ctrl->q_tlv[0].array[idx_w];
				...
				hip5_q_entry->sig_len = skb_fapi->len;
				if (!atomic_read(&hip->hip_priv->in_logring_disable))
...
#ifdef CONFIG_SCSC_PCIE_CHIP
	slsi_trigger_pcie_interrupt(hip->hip_priv->intr_from_host_ctrl, hip);
#else
...
static int hip5_rx_zero_copy_init_skb_buffers(struct slsi_dev *sdev, struct slsi_hip *hip, struct scsc_service *service)
{
	...
	for (i = 0; i < (MAX_NUM - 1); i++) {
		...
		skb_reserve(skb, SLSI_NETIF_SKB_HEADROOM);
		/* Get DMA'able address of SKB */
		hip_priv->rx_skb_table[i].skb_dma_addr = (dma_map_single(scsc_service_get_pcie_dev(), skb->data, HIP5_DAT_MBULK_SIZE, DMA_FROM_DEVICE) >> 4) | 0x1;
		if (dma_mapping_error(scsc_service_get_pcie_dev(), hip_priv->rx_skb_table[i].skb_dma_addr)) {
			SLSI_ERR_NODEV("DMA mapping error\n");
			consume_skb(skb);
			return -EFAULT;
		}
...
int slsi_hip_setup(struct slsi_hip *hip)
{
	...
	compat_flag = scsc_wifi_get_hip_config_version_5_u32(&sdev->hip.hip_control->config_v5, compat_flag);
	SLSI_INFO_NODEV("compat_flag:0x%X\n", compat_flag);
	if (compat_flag & MIF_HIP_COMPAT_FLAG_RX_ZERO_COPY) {
...
#endif
#ifdef CONFIG_SCSC_PCIE_CHIP
	slsi_traffic_mon_client_unregister(sdev, hip->hip_priv->pcie_client);
#endif
