...
static int slsi_reassoc_frequency_write_legacy(struct net_device *dev, char *command, int buf_len)
{
	...
	if (!ndev_vif->activated || ndev_vif->vif_type != FAPI_VIFTYPE_STATION ||
	    ndev_vif->sta.vif_status != SLSI_VIF_STATUS_CONNECTED) {
		SLSI_NET_ERR(dev, "STA not connected: vif_type:%d vif_activated:%d vif_status:%d\n",
			     ndev_vif->vif_type, ndev_vif->activated, ndev_vif->sta.vif_status);
		SLSI_MUTEX_UNLOCK(ndev_vif->vif_mutex);
		kfree(ioctl_args);
		return -EPERM;
	}
...
int slsi_set_tx_power_calling(struct net_device *dev, char *command, int buf_len)
{
	...
	if (ioctl_args->arg_count > 1) {
		if (slsi_str2int(ioctl_args->args[1], &ant)) {
			SLSI_ERR(sdev, "Invalid string: '%s'\n", ioctl_args->args[1]);
			kfree(ioctl_args);
			return -EINVAL;
		}
...
int slsi_set_max_bw(struct net_device *dev, char *command, int cmd_len)
{
	...
	ret = slsi_mlme_set_max_bw(sdev, dev, (u8)link_id, FULL_BSS_BANDWIDTH, leaky_ap_detect, bandwidth);
	if (ret < 0) {
		ret = -1;
	} else {
#ifdef CONFIG_SCSC_WLAN_EHT
