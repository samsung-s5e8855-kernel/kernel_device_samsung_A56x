		...
		for (i = 1; i <= CONFIG_SCSC_WLAN_MAX_INTERFACES; i++)
			if (sdev->netdev[i]) {
				...
				if (ndev_vif->detect_vifnum)
					slsi_mlme_clear_detect_vif(sdev, sdev->netdev[i], ndev_vif->detect_vifnum);
...
static int slsi_rx_netdev_mlme(struct slsi_dev *sdev, struct net_device *dev, struct sk_buff *skb)
{
	...
	case MLME_NAN_BOOTSTRAPPING_REQUESTED_IND:
		slsi_nan_bootstrapping_requested_ind(sdev, dev, skb);
		break;
	case MLME_NAN_BOOTSTRAPPING_CONFIRM_IND:
		slsi_nan_bootstrapping_confirm_ind(sdev, dev, skb);
		break;
	case MLME_NAN_PAIRING_REQUESTED_IND:
		slsi_nan_pairing_requested_ind(sdev, dev, skb);
		break;
	case MLME_NAN_PAIRING_REQUEST_IND:
		slsi_nan_pairing_request_ind(sdev, dev, skb);
		break;
	case MLME_NAN_PAIRING_RESPONSE_IND:
		slsi_nan_pairing_response_ind(sdev, dev, skb);
		break;
	case MLME_NAN_PAIRING_CONFIRM_IND:
		slsi_nan_pairing_confirm_ind(sdev, dev, skb);
		break;
#endif
#endif
	...
	case MLME_MLO_SET_TTLM_IND:
		...
		break;
#endif
	case MLME_SET_BSS_MAX_CHANNEL_WIDTH_IND:
		slsi_rx_set_bss_max_channel_width_event(sdev, dev, skb);
		break;
	default:
		kfree_skb(skb);
