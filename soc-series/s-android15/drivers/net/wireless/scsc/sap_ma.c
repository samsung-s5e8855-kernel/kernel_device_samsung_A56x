...
#if IS_ENABLED(CONFIG_SCSC_LOG_COLLECTION)
		scsc_log_collector_schedule_collection(SCSC_LOG_HOST_WLAN, SCSC_LOG_HOST_COMMON_REASON_START);
#endif
#ifdef CONFIG_SCSC_WLAN_ENHANCED_PKT_FILTER
...
#if defined(CONFIG_SCSC_WLAN_RX_NAPI_GRO)
		u16 drop_proto;
#endif
	...
	if (to_ndev_vif->enhanced_arp_detect_enabled && (to_ndev_vif->vif_type == FAPI_VIFTYPE_STATION)) {
		...
		if (to_ndev_vif->gro_enabled == SLSI_GRO_ENABLED) {
			...
			if (ctx_napi) {
				...
				napi = &sdev->hip.hip_priv->bh_dat->bh_priv.napi.napi_instance;
#else
