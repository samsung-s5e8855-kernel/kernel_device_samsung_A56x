...
static u32 slsi_qsf_encode_hw_feature(struct slsi_dev *sdev, u8 *misc_features_activated,
				      ...
				      u8 *eht_caps, u8 *he_caps, u8 *vht_caps, u8 *ht_caps)
{
	u8 wifi_standard = 0;
	u32 hw_feature = 0;
	u8 low_rx_core = 0;
...
#ifdef CONFIG_SCSC_WLAN_EHT
	u8 supports_80mhz = 0;
	u8 supports_160mhz = 0;
	u8 supports_320mhz = 0;
...
#ifdef CONFIG_SCSC_WLAN_EHT
	case SLSI_WIFI_7:
		supports_80mhz = he_caps[6] & IEEE80211_HE_PHY_CAP0_CHANNEL_WIDTH_SET_40MHZ_80MHZ_IN_5G;
		supports_160mhz = he_caps[6] & IEEE80211_HE_PHY_CAP0_CHANNEL_WIDTH_SET_160MHZ_IN_5G;
		supports_320mhz = eht_caps[2] & IEEE80211_EHT_PHY_CAP0_320MHZ_IN_6GHZ;
		eht_mcs_nss_supp = (struct ieee80211_eht_mcs_nss_supp *)&sdev->fw_sta_eht_cap[11];
		antenna = slsi_get_antenna_from_eht_caps(sdev, supports_80mhz, supports_160mhz,
							 supports_320mhz, eht_mcs_nss_supp);
		break;
#endif
	case SLSI_WIFI_6E:
	case SLSI_WIFI_6:
...
static u32 slsi_qsf_encode_sw_feature_4(struct slsi_dev *sdev, u8 *misc_features_activated,
					...
					u32 nan_std_plus_versions, bool nan_std_plus_fast_recovery,
					char *buf, u32 bytes, u32 buf_size)
{
	...
	u32 nan_std_plus_major_number = SLSI_GET_STD_NAN_MAJOR_NUMBER(nan_std_plus_versions);
	u32 nan_std_plus_minor_number = SLSI_GET_STD_NAN_MINOR_NUMBER(nan_std_plus_versions);
	...
	std_nan[0] = misc_features_activated[18];
	std_nan[1] = misc_features_activated[19];
	std_nan[1] |= (nan_std_plus_fast_recovery ? BIT(2) : 0x0);
	std_nan[2] = misc_features_activated[20] & BIT(0);
	std_nan[2] |= misc_features_activated[20] & BIT(1);
	std_nan[3] = 0x00;
	std_nan[4] = 0x00;
	std_nan[5] = nan_std_plus_minor_number;
	std_nan[6] = nan_std_plus_major_number;

	bytes += scnprintf(buf + bytes, buf_size - bytes, "%02X%02X%02X%02X%02X%02X%02X%02X%02X",
		SLSI_QSF_SW_FEATURE_STANDARD_PLUS_ID, SLSI_QSF_SW_FEATURE_STANDARD_PLUS_LEN, std_nan[6],
		std_nan[5],std_nan[4],std_nan[3],std_nan[2],std_nan[1],std_nan[0]);
	return bytes;
}
...
static int slsi_get_qsf_mib_data(struct slsi_dev *sdev, struct slsi_qsf_mib_data *qsf_data)
{
	...
	struct slsi_mib_get_entry get_values[] = { { SLSI_PSID_UNIFI_HE_ACTIVATED, { 0, 0 } },
						   ...
						   { SLSI_PSID_UNIFI_NAN_STD_PLUS_VERSION, {0, 0} },
						   { SLSI_PSID_UNIFI_NAN_STD_PLUS_FR_MAX_DEVICES, {0, 0} },
						 };
	...
	if (values[11].type == SLSI_MIB_TYPE_OCTET && values[11].u.octetValue.dataLength >= 14)
		memcpy(qsf_data->eht_caps, values[11].u.octetValue.data, values[11].u.octetValue.dataLength);
	else
		SLSI_ERR(sdev, "invalid type or len for index: %d len: %d\n", 11,
			 values[11].u.octetValue.dataLength);
	if (values[12].type == SLSI_MIB_TYPE_OCTET && values[12].u.octetValue.dataLength >= 21)
		memcpy(qsf_data->misc_features_activated, values[12].u.octetValue.data, 21);
	else
		SLSI_ERR(sdev, "invalid type or len for index: %d len: %d\n", 12,
			 values[12].u.octetValue.dataLength);
	if (values[13].type == SLSI_MIB_TYPE_OCTET && values[13].u.octetValue.dataLength >= 4)
		memcpy(&qsf_data->appendix_versions, values[13].u.octetValue.data, 4);
	else
		SLSI_ERR(sdev, "invalid type or len for index: %d len: %d\n", 13,
			 values[13].u.octetValue.dataLength);

	SLSI_CHECK_TYPE(sdev, values[16].type, SLSI_MIB_TYPE_UINT);
	qsf_data->nan_std_plus_versions = values[16].u.uintValue;
	SLSI_CHECK_TYPE(sdev, values[17].type, SLSI_MIB_TYPE_UINT);
	qsf_data->nan_std_plus_fast_recovery = (values[17].u.uintValue > 0 ? true : false);

	SLSI_CHECK_TYPE(sdev, values[14].type, SLSI_MIB_TYPE_UINT);
	sdev->fw_build_id = values[14].u.uintValue;
	SLSI_CHECK_TYPE(sdev, values[15].type, SLSI_MIB_TYPE_UINT);
	qsf_data->twt_control = values[15].u.uintValue;
	kfree(mibrsp.data);
	kfree(values);
	return 0;
}
...
void slsi_get_qsfs_feature_set(struct slsi_dev *sdev)
{
	...
	u32 pos = 0;
	int ret = 0;
	u32 buf_size = SLSI_QSF_BUFF_LEN;
	...
	pos = slsi_qsf_encode_hw_feature(sdev, qsf_data.misc_features_activated, qsf_data.he_active,
					 buf, pos, buf_size, qsf_data.eht_caps, qsf_data.he_caps,
	...
	pos = slsi_qsf_encode_sw_feature_4(sdev, qsf_data.misc_features_activated, qsf_data.tdls_active,
					   ...
					   qsf_data.nan_fast_connect, qsf_data.nan_std_plus_versions, qsf_data.nan_std_plus_fast_recovery,
					   buf, pos, buf_size);
