...
void tcp_retransmit_skb_callback(void *ignore, const struct sock *sk, const struct sk_buff *skb)
{
	...
	if (sk->sk_state == TCP_ESTABLISHED) {
		SLSI_INFO_NODEV("%pI4:%hu->%pI4:%hu len:%d snd_una:%u snd_cwnd:%u prior_cwnd:%u\n",
				&saddr, ntohs(inet_sk(sk)->inet_sport), &daddr, ntohs(inet_sk(sk)->inet_dport), skb->len,
				tcp_sk(sk)->snd_una, tcp_sk(sk)->snd_cwnd, tcp_sk(sk)->prior_cwnd);
	}
}
...
void qdisc_create_callback(void *ignore, const struct Qdisc_ops *ops, struct net_device *dev, unsigned int parent)
{
	...
	if (!strcmp(ops->id, "htb")) {
		...
		ndev_vif->gro_stat.qdisc_stat = SLSI_QDISC_CREATED;
		slsi_dynamic_gro_ctrl(ndev_vif);
		write_unlock_bh(&ndev_vif->gro_lock);
		SLSI_INFO_NODEV("GRO %s (If_index:%d, qdisc:%s, latency mode:%s)\n",
