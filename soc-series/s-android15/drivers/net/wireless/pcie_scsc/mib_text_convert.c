...
static bool CsrWifiMibConvertTextParseLine(const char *linestr, struct slsi_mib_data *mibDataSet, struct slsi_mib_data *mibDataGet)
{
		...
		if (!processingStr) {
			...
			if (!equals && (current_char[0] == '.')) {
				if (!dot1) {
					...
					SLSI_ERR_NODEV("CsrWifiMibConvertTextParseLine('%s') only 2 indexes supported\n", trimmed);
					result = false;
				}
			}
			if (!equals && (current_char[0] == '='))
	...
	if (result) {
		...
		if (!CsrWifiMibConvertStrToUint16(trimmed, &entry.psid)) {
			SLSI_ERR_NODEV("CsrWifiMibConvertTextParseLine('%s') Convert <psid> failed\n", trimmed);
			result = false;
		}
		if (dot1 && !CsrWifiMibConvertStrToUint16(&trimmed[dot1 + 1], &entry.index[0])) {
			SLSI_ERR_NODEV("CsrWifiMibConvertTextParseLine('%s') Convert <index 1> failed\n", trimmed);
			result = false;
		}
		if (dot2 && !CsrWifiMibConvertStrToUint16(&trimmed[dot2 + 1], &entry.index[1])) {
			SLSI_ERR_NODEV("CsrWifiMibConvertTextParseLine('%s') Convert <index 2> failed\n", trimmed);
			result = false;
		}
			...
			if (CsrStrNICmp(data, "true", 4) == 0) {
				...
				for (i = 0; i < octetLen; i++)
					if (!CsrHexStrToUint8(&data[1 + (i * 2)], &entry.value.u.octetValue.data[i])) {
						SLSI_ERR_NODEV("CsrWifiMibConvertTextParseLine('%s') Convert Hex Bytes <data> failed\n", trimmed);
						result = false;
						break;
					}
				...
				if (!CsrWifiMibConvertStrToUint32(&data[1], &entry.value.u.uintValue)) {
					SLSI_ERR_NODEV("CsrWifiMibConvertTextParseLine('%s') Convert Integer <data> failed\n", trimmed);
					result = false;
				} else {
				...
				SLSI_ERR_NODEV("CsrWifiMibConvertTextParseLine('%s') Convert Unsigned Integer <data> failed\n", trimmed);
				result = false;
			} else {
		...
		if ((lineEnd[0] == '\n') || (lineEnd[0] == '\r') || (lineEnd[1] == '\0')) {
			...
			if (strSize > 2) {
				...
				if (!CsrWifiMibConvertTextParseLine(line, mibDataSet, mibDataGet)) {
					SLSI_ERR_NODEV("CsrWifiMibConvertTextParseLine() Failed for line '%s'\n", line);
					result = false;
				}
				kfree(line);
			}
