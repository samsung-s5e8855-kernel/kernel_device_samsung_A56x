...
static void test_slsi_mlme_get_sinfo_mib(struct kunit *test)
{
	...
	KUNIT_EXPECT_EQ(test, -EINVAL, slsi_mlme_get_sinfo_mib(sdev, dev, peer, ndev_vif->ifnum));
	...
	KUNIT_EXPECT_EQ(test, -EIO, slsi_mlme_get_sinfo_mib(sdev, dev, peer, ndev_vif->ifnum));
}
