...
#define SLSI_WLAN_DPD_MAX_SIZE	  2*1024*1024
#else
#define SLSI_WLAN_DPD_MAX_SIZE	  512*1024
#endif
#define SLSI_WLAN_DPD_MAGIC_STR   "MDPD"

static uint slsi_dpd_msg_num_max = 16;
module_param(slsi_dpd_msg_num_max, uint, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(slsi_dpd_msg_num_max, "Max number of User messages queued in driver (default:16)");
...
static int slsi_wlan_dpd_mmap(struct file *filp, struct vm_area_struct *vma)
{
	...
	if ((size > slsi_wlan_dpd_mmap_size) || !(IS_ALIGNED(size, PAGE_SIZE))) {
		SLSI_ERR_NODEV("size is invalid! (req:%ld, allow:%ld)\n", size, SLSI_WLAN_DPD_MAX_SIZE);
		goto error;
	}
...
void slsi_wlan_dpd_mmap_user_space_event(int msg_id)
{
	...
	if (skb_queue_len(&client->msg_list) >= slsi_dpd_msg_num_max) {
...
int slsi_wlan_dpd_mmap_create(void)
{
	...
	slsi_wlan_dpd_mmap_class = class_create("slsi_wlan_dpd_mmap_class");
#else
