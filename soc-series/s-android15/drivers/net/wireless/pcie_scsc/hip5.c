...
static struct sk_buff *hip5_rx_zero_copy_get_skb_from_addr(struct slsi_dev *sdev, struct hip_priv *hip_priv, u32 addr)
{
	...
	if (i < MAX_NUM) {
		SLSI_DBG4(sdev, SLSI_HIP, "SKB:0x%X\n", addr);
		skb = hip_priv->rx_skb_table[i].skb;
		atomic_set(&hip_priv->rx_skb_table[i].in_use, 0);
		skb_dma_addr = addr;
		skb_dma_addr = (skb_dma_addr & ~0x1) << 4;
		dma_unmap_single(scsc_service_get_pcie_dev(), skb_dma_addr, HIP5_DAT_MBULK_SIZE, DMA_FROM_DEVICE);
		return skb;
	}
	return NULL;
}
...
{
	struct hip5_tx_skb_meta skb_meta;
	u16 i = 0;
	...
	skb_meta.idx = i;
	memcpy(dma_desc->skb->head, &skb_meta, sizeof(struct hip5_tx_skb_meta));
...
static mbulk_colour hip5_opt_zero_copy_free_buffer(struct slsi_dev *sdev, struct hip_priv *hip_priv, u32 addr)
{
	...
	skb_meta = (struct hip5_tx_skb_meta *)phys_to_virt(dma_addr);
	i = skb_meta->idx;
...
static int hip5_napi_poll(struct napi_struct *napi, int budget)
{
	...
	if (!hip_priv->mx_pci_claim_data) {
		if (scsc_mx_service_claim_deferred(sdev->service, hip5_napi_mx_claim_complete_cb, (void *)hip_priv, WLAN_RX_DATA)) {
			SLSI_DBG3(sdev, SLSI_RX, "PCIE need to be claimed\n");
			hip5_napi_complete(sdev, napi, hip);
			return 0;
		}
		hip_priv->mx_pci_claim_data = true;
...
static bool is_skb_dma_able(struct slsi_dev *sdev, struct hip_priv *hip_priv, struct sk_buff *skb)
{
	...
	if (dma_mapping_error(sdev->dev, skb_dma_addr)) {
		SLSI_ERR_NODEV("DMA mapping error\n");
		return false;
	}
