			  ...
			  )
{
	...
	case FAPI_VIFTYPE_STATION:
	{
		...
		ndev_vif->sta.connecting_links = 0;
...
#if !(defined(SCSC_SEP_VERSION) && SCSC_SEP_VERSION < 11)
static struct slsi_ssid_info *slsi_get_valid_ssid_info_from_list(struct net_device *dev)
{
	struct netdev_vif *ndev_vif = netdev_priv(dev);
...
static struct slsi_bssid_info *slsi_get_valid_bssid_info_from_list(struct slsi_dev *sdev, struct net_device *dev,
								   struct list_head *bssid_list, bool is_mld)
{
#ifdef CONFIG_SCSC_WLAN_EHT
	struct netdev_vif *ndev_vif = netdev_priv(dev);
#endif
				   ...
				   struct ieee80211_channel **channel, bool retry)
{
	...
	if (ndev_vif->sta.ml_connection) {
		if (ndev_vif->sta.connecting_links > 1) {
			num_ml_link = ndev_vif->sta.connecting_links - 1;
			ret = slsi_set_uint_mib(sdev, NULL, SLSI_PSID_UNIFI_MULTILINK_NUMBER_OF_LINKS, num_ml_link);
			if (!ret) {
				SLSI_INFO(sdev, "[MLD] Trying again to connect to BSSID: " MACSTR " with %d link\n",
					  MAC2STR(ndev_vif->sta.bssid), num_ml_link);
				ndev_vif->sta.connecting_links = num_ml_link;
				slsi_update_sta_sme(sdev, dev, ndev_vif->sta.ssid, ndev_vif->sta.ssid_len,
						    ndev_vif->sta.bssid, ndev_vif->chan);
				return true;
			}
		} else {
			ret = slsi_set_uint_mib(sdev, NULL, SLSI_PSID_UNIFI_MULTILINK_NUMBER_OF_LINKS,
						ndev_vif->sta.max_ml_link);
			if (!ret)
...
#endif
	{
		bssid_info = slsi_get_valid_bssid_info_from_list(sdev, dev, &ssid_info->bssid_list, false);
		if (!bssid_info) {
...
#ifdef CONFIG_SCSC_WLAN_EHT
	/* ML connection with multi-link. Try to get bssid_info from same AP MLD.
	 * If not found, go for next valid bssid_info.
	 */
	bssid_info = slsi_get_valid_bssid_info_from_list(sdev, dev, &ssid_info->bssid_list, true);
	if (!bssid_info) {
		bssid_info = slsi_get_valid_bssid_info_from_list(sdev, dev, &ssid_info->bssid_list, false);
		if (!bssid_info) {
			SLSI_DBG1(sdev, SLSI_MLME, "No bssid to try for retry connection\n");
