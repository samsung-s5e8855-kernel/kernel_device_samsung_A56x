/*
 *  s2se910_adc.c - Support for ADC in S2SE910 PMIC
 *
...
#if IS_ENABLED(CONFIG_EXYNOS_ESCA)
static int s2se910_adc_read_data(struct s2p_adc *info, unsigned long chan)
{
...
static const struct s2p_adc_data s2se910_adc_info = {
	.num_channels	= ADC_MAX_CHANNELS,
	.read_data	= s2se910_adc_read_data,
};

static const struct of_device_id s2se910_adc_match[] = {
	{
		.compatible = "s2se910-gpadc",
		.data = &s2se910_adc_info,
	},
	{},
};
MODULE_DEVICE_TABLE(of, s2se910_adc_match);

static struct s2p_adc_data *s2se910_adc_get_data(void)
{
	return (struct s2p_adc_data *)&s2se910_adc_info;
}

static int s2se910_read_raw(struct iio_dev *indio_dev,
...
static const struct iio_info s2se910_adc_iio_info = {
	.read_raw = &s2se910_read_raw,
};

static const struct iio_chan_spec s2se910_adc_iio_channels[] = {
	ADC_CHANNEL(0, "adc0"),
	ADC_CHANNEL(1, "adc1"),
	ADC_CHANNEL(2, "adc2"),
	ADC_CHANNEL(3, "adc3"),
...
static int s2se910_adc_remove_devices(struct device *dev, void *c)
{
	int ret = 0;
...
static int s2se910_adc_parse_dt(struct s2se910_dev *iodev, struct s2p_adc *info)
{
	...
	gpadc_np = of_find_node_by_name(mfd_np, "s2se910-gpadc");
	if (!gpadc_np) {
...
static int s2se910_adc_probe(struct platform_device *pdev)
{
	...
	info->data = s2se910_adc_get_data();

	ret = s2se910_adc_parse_dt(iodev, info);
	if (ret < 0) {
		pr_err("%s: s2se910_adc_parse_dt fail\n", __func__);
		return -ENODEV;
	}
	...
	indio_dev->info = &s2se910_adc_iio_info;
	indio_dev->modes = INDIO_DIRECT_MODE;
	indio_dev->channels = s2se910_adc_iio_channels;
	...
	ret = of_platform_populate(info->gpadc_node, s2se910_adc_match, NULL, &indio_dev->dev);
	if (ret < 0) {
...
err_of_populate:
	device_for_each_child(&indio_dev->dev, NULL, s2se910_adc_remove_devices);
err_mutex:
...
static int s2se910_adc_remove(struct platform_device *pdev)
{
...
static struct platform_driver s2se910_adc_driver = {
	.probe		= s2se910_adc_probe,
	.remove		= s2se910_adc_remove,
	.driver		= {
		.name	= "s2se910-gpadc",
		.of_match_table = s2se910_adc_match,
	},
};

module_platform_driver(s2se910_adc_driver);

MODULE_AUTHOR("Samsung Electronics");
MODULE_DESCRIPTION("GP-ADC driver for s2se910");
