...
static int set_charging_current(struct s2m_chg_manager_info *battery, int coeff)
{
	...
	if (battery->input_current != input_current) {
		...
		ret = psy_do_property(battery->pdata->charger_name, set, POWER_SUPPLY_PROP_CURRENT_MAX, value);
		if (ret < 0)
	...
	if (battery->charging_current != charging_current) {
		...
		ret = psy_do_property(battery->pdata->charger_name, set, POWER_SUPPLY_PROP_CURRENT_NOW, value);
		if (ret < 0)
			pr_err("%s: Fail to execute property\n", __func__);
	...
	if (battery->topoff_current != topoff_current) {
		...
		ret = psy_do_property(battery->pdata->charger_name, set, (enum power_supply_property)POWER_SUPPLY_S2M_PROP_CURRENT_FULL,
						value);
		if (ret < 0)
			pr_err("%s: Fail to execute property\n", __func__);
	...
	if (battery->small_input_flag == 0) {
		...
		ret = psy_do_property(battery->pdata->smallcharger_name, set, POWER_SUPPLY_S2M_PROP_CHARGING_ENABLED, value);
		if (ret < 0)
			pr_err("%s: Fail to execute property\n", __func__);
	} else {
		...
		ret = psy_do_property(battery->pdata->smallcharger_name, set, POWER_SUPPLY_PROP_CURRENT_MAX, value);
		if (ret < 0)
			pr_err("%s: Fail to execute property\n", __func__);
		...
		ret = psy_do_property(battery->pdata->smallcharger_name, set, POWER_SUPPLY_PROP_CURRENT_NOW, value);
		if (ret < 0)
			pr_err("%s: Fail to execute property\n", __func__);
	}
#endif
...
static int set_charger_mode(struct s2m_chg_manager_info *battery, int charger_mode)
{
	...
	if (!psy)
		return -EINVAL;
	ret = psy_do_property(battery->pdata->charger_name, set, (enum power_supply_property)POWER_SUPPLY_S2M_PROP_CHARGING_ENABLED, val);
	if (ret < 0)
		pr_err("%s: Fail to execute property\n", __func__);
	...
	case S2M_BAT_CHG_MODE_CHARGING:
		if (battery->cable_type == POWER_SUPPLY_TYPE_PREPARE_TA ||
			...
			if (!psy)
				return -EINVAL;
			ret = psy_do_property(battery->pdata->smallcharger_name, set, POWER_SUPPLY_S2M_PROP_CHARGING_ENABLED, val);
			if (ret < 0)
				pr_err("%s: Fail to execute property\n", __func__);
		}
		break;
...
static int set_battery_status(struct s2m_chg_manager_info *battery, int status)
{
	...
	case POWER_SUPPLY_STATUS_CHARGING:
        ...
        if (!psy)
            ...
            ret = psy_do_property(battery->pdata->charger_name, set, POWER_SUPPLY_PROP_VOLTAGE_MAX, value);
            if (ret < 0)
                pr_err("%s: Fail to execute property\n", __func__);
        }
		...
		if (!psy)
			return -EINVAL;
		ret = psy_do_property(battery->pdata->charger_name, set, POWER_SUPPLY_PROP_ONLINE, value);
		if (ret < 0)
			pr_err("%s: Fail to execute property\n", __func__);
		...
		if (battery->cable_type == POWER_SUPPLY_TYPE_PREPARE_TA ||
			...
			if (!psy)
				return -EINVAL;
			ret = psy_do_property(battery->pdata->smallcharger_name, set, POWER_SUPPLY_PROP_ONLINE, value);
			if (ret < 0)
				pr_err("%s: Fail to execute property\n", __func__);
		}
#endif
	...
	if (!psy)
		return -EINVAL;
	ret = psy_do_property(battery->pdata->charger_name, set, POWER_SUPPLY_PROP_STATUS, value);
	if (ret < 0)
		pr_err("%s: Fail to execute property\n", __func__);
	return 0;
}
					...
					union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGE_COUNTER:
		...
		if (!psy)
			return -EINVAL;
		ret = psy_do_property(battery->pdata->fuelgauge_name, get, (enum power_supply_property)POWER_SUPPLY_PROP_CHARGE_COUNTER,
						value);
		if (ret < 0)
			pr_err("%s: Fail to execute property\n", __func__);
					...
					union power_supply_propval *val)
{
	struct s2m_chg_manager_info *battery = power_supply_get_drvdata(psy);
	enum s2m_power_supply_property s2m_psp = (enum s2m_power_supply_property) psp;
	char *sc_name = battery->pdata->switching_charger_name;
	char *dc_name = battery->pdata->direct_charger_name;
	char *chg_name;
	union power_supply_propval value = {};
	int ret = 0;
	...
	switch ((int)psp) {
	case POWER_SUPPLY_PROP_STATUS:
		psy_do_property(chg_name, get, POWER_SUPPLY_PROP_STATUS, value);
		val->intval = value.intval;
		break;
	case POWER_SUPPLY_PROP_ONLINE:
		break;
	case POWER_SUPPLY_PROP_HEALTH:
		psy_do_property(chg_name, get, POWER_SUPPLY_PROP_HEALTH, value);
		val->intval = value.intval;
	...
	case POWER_SUPPLY_PROP_CURRENT_MAX:
		psy_do_property(chg_name, get, POWER_SUPPLY_PROP_CURRENT_MAX, value);
		val->intval = value.intval;
		break;
	case POWER_SUPPLY_PROP_CURRENT_NOW:
		psy_do_property(chg_name, get, POWER_SUPPLY_PROP_CURRENT_NOW, value);
		val->intval = value.intval;
		break;
	case POWER_SUPPLY_PROP_VOLTAGE_MAX:
		psy_do_property(chg_name, get, POWER_SUPPLY_PROP_VOLTAGE_MAX, value);
		val->intval = value.intval;
		break;
	case POWER_SUPPLY_PROP_PRESENT:
		psy_do_property(sc_name, get, POWER_SUPPLY_PROP_PRESENT, value);
		val->intval = value.intval;
		break;
	case POWER_SUPPLY_S2M_PROP_MIN ... POWER_SUPPLY_S2M_PROP_MAX:
		switch (s2m_psp) {
		...
		return ret;
	default:
		psy_do_property(sc_name, get, psp, value);
		break;
	}
	return ret;
}
					...
					const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_STATUS:
		if (psy_sc)
			psy_do_property(sc_name, set, POWER_SUPPLY_PROP_STATUS, val);
		if (psy_dc)
			psy_do_property(dc_name, set, POWER_SUPPLY_PROP_STATUS, val);
		break;
	case POWER_SUPPLY_PROP_ONLINE:
		if (battery->cable_type == POWER_SUPPLY_TYPE_BATTERY){
		...
		if (psy_sc)
			psy_do_property(sc_name, set, POWER_SUPPLY_PROP_ONLINE, val);
		if (psy_dc)
			psy_do_property(dc_name, set, POWER_SUPPLY_PROP_ONLINE, val);
		break;
	case POWER_SUPPLY_PROP_VOLTAGE_MAX:
		pr_info("%s, set float voltage\n", __func__);
		if (psy_sc)
			psy_do_property(sc_name, set, POWER_SUPPLY_PROP_VOLTAGE_MAX, val);
		if (psy_dc)
			psy_do_property(dc_name, set, POWER_SUPPLY_PROP_VOLTAGE_MAX, val);
		break;
	case POWER_SUPPLY_PROP_CURRENT_MAX:
		if (psy_sc)
			psy_do_property(sc_name, set, POWER_SUPPLY_PROP_CURRENT_MAX, val);
		break;
	case POWER_SUPPLY_PROP_CURRENT_NOW:
		if (psy_sc)
			psy_do_property(sc_name, set, POWER_SUPPLY_PROP_CURRENT_NOW, val);
		break;
	case POWER_SUPPLY_S2M_PROP_MIN ... POWER_SUPPLY_S2M_PROP_MAX:
		switch (s2m_psp) {
		case POWER_SUPPLY_S2M_PROP_DIRECT_CHARGER_MODE:
		...
		case POWER_SUPPLY_S2M_PROP_CURRENT_FULL:
			...
			if (psy_sc)
				psy_do_property(sc_name, set, (enum power_supply_property) s2m_psp, value);
			break;
		...
		case POWER_SUPPLY_S2M_PROP_CHARGING_ENABLED:
			...
			if (val->intval == S2M_BAT_CHG_MODE_CHARGING) {
				if (battery->is_apdo &&
					...
					value.intval = val->intval;
					if (psy_dc)
						psy_do_property(dc_name, set, (enum power_supply_property) s2m_psp, value);
				} else {
					battery->chg_src = S2M_SC;
					value.intval = val->intval;
					if (psy_sc)
		...
		default:
			if (psy_sc)
				psy_do_property(sc_name, set, psp, val);
			break;
		}
		return ret;
	default:
...
static int get_battery_info(struct s2m_chg_manager_info *battery)
{
	...
	chg_name = battery->pdata->switching_charger_name;
#else
	psy_sc = power_supply_get_by_name(battery->pdata->charger_name);
	chg_name = battery->pdata->charger_name;
#endif
	if (!psy_sc) {
	...
	ret = psy_do_property(chg_name, set, (enum power_supply_property)POWER_SUPPLY_S2M_PROP_USBPD_TEST_READ, value);
	if (ret < 0)
	...
	if (!psy) {
		...
		ret = psy_do_property("muic-manager", get, (enum power_supply_property)POWER_SUPPLY_S2M_PROP_MUIC_PATH, value);
		if (ret < 0)
	...
	ret = psy_do_property(battery->pdata->fuelgauge_name, get, POWER_SUPPLY_PROP_VOLTAGE_NOW, value);
	if (ret < 0)
		pr_err("%s: Fail to execute property\n", __func__);
	battery->voltage_now = value.intval;

	value.intval = S2M_BATTERY_VOLTAGE_AVERAGE;
	ret = psy_do_property(battery->pdata->fuelgauge_name, get, POWER_SUPPLY_PROP_VOLTAGE_AVG, value);
	if (ret < 0)
		pr_err("%s: Fail to execute property\n", __func__);
	battery->voltage_avg = value.intval;

	value.intval = S2M_BATTERY_CURRENT_MA;
	ret = psy_do_property(battery->pdata->fuelgauge_name, get, POWER_SUPPLY_PROP_CURRENT_NOW, value);
	if (ret < 0)
		pr_err("%s: Fail to execute property\n", __func__);
	battery->current_now = value.intval;

	value.intval = S2M_BATTERY_CURRENT_MA;
	ret = psy_do_property(battery->pdata->fuelgauge_name, get, POWER_SUPPLY_PROP_CURRENT_AVG, value);
	if (ret < 0)
		pr_err("%s: Fail to execute property\n", __func__);
	battery->current_avg = value.intval;

	/* Get temperature info */
	ret = psy_do_property(battery->pdata->fuelgauge_name, get, POWER_SUPPLY_PROP_TEMP, value);
	if (ret < 0)
		pr_err("%s: Fail to execute property\n", __func__);
	battery->temperature = value.intval;
	get_battery_capacity(battery);

	ret = psy_do_property(battery->pdata->fuelgauge_name, get, (enum power_supply_property)POWER_SUPPLY_S2M_PROP_SOH, value);
	if (ret < 0)
		pr_err("%s: Fail to execute property\n", __func__);
	...
	ret = psy_do_property(battery->pdata->charger_name, get, POWER_SUPPLY_PROP_CURRENT_MAX, value);
	if (ret < 0)
		pr_err("%s: Fail to execute property\n", __func__);
	battery->current_max = value.intval;

	/* Get charge current limit */
	ret = psy_do_property(battery->pdata->charger_name, get, POWER_SUPPLY_PROP_CURRENT_NOW, value);
	if (ret < 0)
		pr_err("%s: Fail to execute property\n", __func__);
	battery->current_chg = value.intval;

	/* Get charger status*/
	ret = psy_do_property(battery->pdata->charger_name, get, POWER_SUPPLY_PROP_STATUS, value);
	if (ret < 0)
		pr_err("%s: Fail to execute property\n", __func__);
	...
	ret = psy_do_property(battery->pdata->pmeter_name, get, (enum power_supply_property)POWER_SUPPLY_S2M_PROP_VCHGIN, value);

	/* Get input voltage & current from powermeter */
	battery->vchg_voltage = value.intval;

	ret = psy_do_property(battery->pdata->pmeter_name, get, (enum power_supply_property)POWER_SUPPLY_S2M_PROP_ICHGIN, value);
	...
	/* Get input current limit */
	ret = psy_do_property(battery->pdata->smallcharger_name, get, POWER_SUPPLY_PROP_CURRENT_MAX, value);
	if (ret < 0)
		pr_err("%s: Fail to execute property\n", __func__);
	battery->small_input = value.intval;

	/* Get charge current limit */
	ret = psy_do_property(battery->pdata->smallcharger_name, get, POWER_SUPPLY_PROP_CURRENT_NOW, value);
	if (ret < 0)
		pr_err("%s: Fail to execute property\n", __func__);
...
static int get_battery_health(struct s2m_chg_manager_info *battery)
{
	...
	if (!psy)
		return -EINVAL;
	ret = psy_do_property(battery->pdata->charger_name, get, POWER_SUPPLY_PROP_HEALTH, value);
	if (ret < 0)
		pr_err("%s: Fail to execute property\n", __func__);
...
static void bat_monitor_work(struct work_struct *work)
{
	...
	if (is_smdk) {
		...
		ret = psy_do_property(battery->pdata->charger_name, get, POWER_SUPPLY_PROP_PRESENT, value);
		if (ret < 0)
			pr_err("%s: Fail to execute property\n", __func__);
...
static int s2m_chg_manager_parse_dt(struct device *dev, struct s2m_chg_manager_info *battery)
{
	...
	if (!pdata->charging_current_expand)
		return -ENOMEM;
