...
static void s2mf301_test_read(struct i2c_client *i2c)
{
	...
	s2mf301_read_reg(i2c, 0x6C, &data);
	sprintf(str+strlen(str), "0x6C:0x%02x, ", data);
...
static bool s2mf301_chg_init(struct s2mf301_charger_data *charger)
{
	...
	/* Set battery OCP 10A */
	s2mf301_update_reg(charger->i2c, S2MF301_CHG_CTRL13, S2MF301_SET_BAT_OCP_10000mA, BAT_OCP_MASK);
		...
		struct s2mf301_charger_data *charger)
{
	...
	ret = psy_do_property(charger->pdata->fuelgauge_name, get, POWER_SUPPLY_PROP_CURRENT_AVG, value);
	if (ret < 0)
...
static void s2mf301_set_2lv_3lv_chg_mode(struct s2mf301_charger_data *charger, int onoff)
{
	...
	ret = psy_do_property(charger->pdata->pmeter_name, (enum power_supply_property)POWER_SUPPLY_S2M_PROP_VCHGIN, value);
	if (ret < 0) {
		pr_err("%s: Fail to execute property\n", __func__);
		return;
	}
				...
				const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ONLINE:
		...
		if (charger->cable_type != POWER_SUPPLY_TYPE_OTG) {
			...
			ret = psy_do_property(charger->pdata->fuelgauge_name, set, POWER_SUPPLY_PROP_ENERGY_AVG, value);
			if (ret < 0)
				pr_err("%s: Fail to execute property\n", __func__);
		}
		break;
	...
	case POWER_SUPPLY_S2M_PROP_MIN ... POWER_SUPPLY_S2M_PROP_MAX:
		...
		case POWER_SUPPLY_S2M_PROP_CHARGING_ENABLED:
			...
			if (value.intval != POWER_SUPPLY_TYPE_OTG) {
				...
				ret = psy_do_property(charger->pdata->fuelgauge_name, set, psp_t, value);
				if (ret < 0)
					pr_err("%s: Fail to execute property\n", __func__);
				...
				const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ONLINE:
		...
		ret = psy_do_property(charger->pdata->charger_name, set, psp_t, value);
		if (ret < 0)
			pr_err("%s: Fail to execute property\n", __func__);
...
static irqreturn_t s2mf301_event_isr(int irq, void *data)
{
	...
	if (val & WDT_TIMER_FAULT_STATUS_MASK) {
		...
		ret = psy_do_property("s2mf301-usbpd", set, psp_t, value);
		if (ret < 0)
			pr_err("%s: Fail to execute property\n", __func__);
	}
...
static irqreturn_t s2mf301_ivr_isr(int irq, void *data)
{
	...
	ret = psy_do_property(charger->pdata->pmeter_name, get, (enum power_supply_property)POWER_SUPPLY_S2M_PROP_ICHGIN, value);
	if (ret < 0) {
		pr_err("%s: Fail to execute property\n", __func__);
		return IRQ_HANDLED;
	}
