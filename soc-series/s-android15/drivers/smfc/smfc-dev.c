...
static int smfc_vb2_buf_init(struct vb2_buffer *vb)
{
	...
	for (plane = 0; plane < vb->num_planes; ++plane) {
		...
		sbuf->info[plane].sgt =
			dma_buf_map_attachment_unlocked(sbuf->info[plane].dba,
...
static int smfc_init_v4l2(struct device *dev, struct smfc_dev *smfc)
{
	...
	smfc->videodev->device_caps = smfc->devdata->device_caps |
		V4L2_CAP_EXYNOS_JPEG_DMABUF_OFFSET | V4L2_CAP_STREAMING;
...
{
	__maybe_unused struct bts_bw bw;
	...
	if (smfc->core_dvfs_class != PM_QOS_DEVICE_THROUGHPUT)
		smfc_pm_qos_update_request(&smfc->qosreq_core,
			smfc->core_dvfs_class, smfc->qosreq_core_level);
}
...
static void smfc_remove_devfreq(struct smfc_dev *smfc)
{
	...
	if (smfc->core_dvfs_class != PM_QOS_DEVICE_THROUGHPUT)
		smfc_pm_qos_remove_request(&smfc->qosreq_core);
}
...
static int exynos_smfc_probe(struct platform_device *pdev)
{
	...
	ret = devm_request_irq(&pdev->dev, irq_num, exynos_smfc_irq_handler,
				0, pdev->name, smfc);
	if (ret) {
	...
	if (of_property_read_u32(pdev->dev.of_node, "smfc,core_qos_minlock",
				(u32 *)&smfc->qosreq_core_level))
		smfc->qosreq_core_level = 0;
	...
	dev_info(&pdev->dev, "Minlock level(INT=%d, CORE=%d, MIF=%d)\n",
		smfc->qosreq_int_level, smfc->qosreq_core_level,
		smfc->qosreq_mif_level);
	dev_info(&pdev->dev, "CORE clk(%d), BPC(%d)\n",
		smfc->core_clk, smfc->bpc);
