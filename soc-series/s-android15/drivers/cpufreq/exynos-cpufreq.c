...
int exynos_cpufreq_get_minfreq(int domain_id, u32 *min_freq)
{
	struct exynos_cpufreq_domain *domain;

	list_for_each_entry(domain, &domains, list) {
		if (domain->id == domain_id) {
			*min_freq = domain->min_freq;
			return 0;
		}
	}
...
EXPORT_SYMBOL_GPL(exynos_cpufreq_get_minfreq);
...
static void debug_pre_scale(struct exynos_cpufreq_domain *domain, unsigned int target_freq)
{
	...
	dbg_snapshot_freq(domain->dss_type, domain->curr_freq, target_freq, DSS_FLAG_IN);
#endif
	dbg_snapshot_printk("ID %d: %d -> %d (%d), reason=%d, flag=%d\n",
			domain->id, domain->curr_freq, target_freq, DSS_FLAG_IN,
			domain->reason, domain->flag);
}
		...
		int result)
{
	dbg_snapshot_printk("ID %d: %d -> %d (%d)\n",
			domain->id, domain->curr_freq, target_freq, DSS_FLAG_OUT);
#if !IS_ENABLED(CONFIG_PRECISE_DVFS_LOGGING)
	dbg_snapshot_freq(domain->dss_type, domain->curr_freq, target_freq,
		...
		unsigned int relation, struct cpufreq_policy *policy)
{
	...
	if (domain->curr_freq == target_freq) {
		ret = -EINVAL;
		goto out;
	}
	...
	if (ret < 0) {
		pr_err("Failed to scale frequency of domain%d (%d -> %d)\n",
				domain->id, domain->curr_freq, target_freq);
		goto out;
	}
		...
		unsigned int relation)
{
	...
	if (domain->request_freq == target_freq)
		return -EINVAL;

	domain->request_freq = target_freq;
...
/**
 * dm_scaler - Carry out a frequency update.
 *
 * This function is called by DVFS manager to request frequency update.
 * This function is only used when "CONFIG_EXYNOS_DVFS_MANAGER" is enabled.
 */
		...
		unsigned int relation)
{
	...
	domain->reason = ego_get_update_reason(policy);
	domain->flag = ego_get_flag(policy);
...
/**
 * dm_scaler - Receive frequency transition for the update request.
 *
 * This function is called by "ESCA" DVFS manager to nofity cpufreq transition.
 * This function is only used when "CONFIG_EXYNOS_ESCA_DVFS_MANAGER" is enabled.
 */
static int dm_scaler(int dm_type, void *devdata, unsigned int target_freq,
		unsigned int duration)
{
	struct exynos_cpufreq_domain *domain = devdata;
	struct cpufreq_policy *policy = get_cpufreq_policy(domain);
	unsigned long latency = (unsigned long)ktime_to_us(duration);
	unsigned long flags;
	...
	if (domain->curr_freq == target_freq) {
		trace_cpufreq_scale_freq(domain->id, domain->curr_freq, target_freq,
					"early-return", latency);
	...
	/*
	 * Update request_freq to curr_freq.
	 * Frequency may actually be applied differently than
	 * requested vaule, for example, skew, etc.
	 */
	domain->curr_freq = target_freq;
	domain->request_freq = target_freq;
...
VISIBLE_IF_KUNIT int exynos_cpufreq_verify(struct cpufreq_policy_data *new_policy)
{
	struct exynos_cpufreq_domain *domain;
	struct cpufreq_policy *policy;
...
EXPORT_SYMBOL_IF_KUNIT(exynos_cpufreq_verify);
 ...
 * customer requirement: min/max control by cpufreq policy node
 * for support freq-qos-tracer, create this sysfs
 */
...
#define store_one(file_name, type)					\
...
{									\
	...
	if (unlikely(!domain))						\
		return -EINVAL;						\
									\
	ret = freq_qos_update_request(&domain->user_##type##_qos_req, val);	\
		...
		struct cpufreq_policy *policy)
{
	...
	ret = sysfs_create_file(&policy->kobj, &user_min_freq.attr);
	if (ret < 0)
		return ret;

	ret = sysfs_create_file(&policy->kobj, &user_max_freq.attr);
	if (ret < 0)
		return ret;
	...
	pr_info("domain%d operates at %dKHz for %lldmsecs\n",
