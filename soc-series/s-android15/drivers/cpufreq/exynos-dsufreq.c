...
#if !IS_ENABLED(CONFIG_PRECISE_DVFS_LOGGING)
	dbg_snapshot_freq(dsufreq.dss_type, prev_freq, target_freq, DSS_FLAG_OUT);
#endif
...
VISIBLE_IF_KUNIT int dsufreq_init_stats(struct device_node *dn)
{
	struct dsufreq_stats *stats;
...
EXPORT_SYMBOL_IF_KUNIT(dsufreq_init_stats);
