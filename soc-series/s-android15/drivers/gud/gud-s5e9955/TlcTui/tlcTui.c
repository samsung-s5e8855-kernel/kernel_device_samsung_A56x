...
static bool tlc_open_driver(void)
{
	...
	mc_ret = mc_malloc_wsm(DEVICE_ID, 0, (u32)sizeof(struct tui_dci_msg_t),
			       (uint8_t **)&dci, 0);
	if (mc_ret != MC_DRV_OK) {
...
static int main_thread(void *uarg)
{
	...
	for (;;) {
		...
		ret = tlc_wait_cmd_from_driver();
		if (ret) {
			if (++fail_count < retry_max) {
				tui_dev_err(ret, "%d fail count = %d", __LINE__, fail_count);
				continue;
			}
			else{
				tui_dev_err(ret, "%d fail limit exceeeded. ending thread", __LINE__);
				break;
			}
		}
		fail_count = 0;
