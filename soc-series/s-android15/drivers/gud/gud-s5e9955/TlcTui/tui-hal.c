...
#include "../../../../drivers/dpu/exynos_drm_tui.h"
...
/* ExySp : USE_DMA_HEAP_ALLOC */
	...
	if (!dma_heap) {
		tui_dev_err(!dma_heap, "fail to get dma_heap(%p)\n", dma_heap);
		return TUI_DCI_ERR_INTERNAL_ERROR;
	}
	...
	if (IS_ERR(g_dma_buf)){
		tui_dev_err(IS_ERR(g_dma_buf), "fail to allocate dma buffer(%p)\n", g_dma_buf);
		goto err_alloc;
	}

	g_attachment = dma_buf_attach(g_dma_buf, dev_tlc_tui);
	if (IS_ERR(g_attachment)){
		tui_dev_err(IS_ERR(g_attachment), "fail to dma buf attachment(%p)\n", g_attachment);
		goto err_attach;
	}

	g_sgt = dma_buf_map_attachment_unlocked(g_attachment, DMA_BIDIRECTIONAL);
	if (IS_ERR(g_sgt)){
		tui_dev_err(IS_ERR(g_sgt), "Failed to get sgt(%p)\n", g_sgt);
		goto err_attachment;
	}

	phys_addr = sg_phys(g_sgt->sgl);
	if (IS_ERR_VALUE(phys_addr)) {
		tui_dev_err((int)IS_ERR_VALUE(phys_addr), "Failed to get iova, phys_addr: %llu\n", phys_addr);
		goto err_daddr;
	}
...
err_daddr:
	phys_addr = 0;
err_attachment:
	dma_buf_unmap_attachment_unlocked(g_attachment, g_sgt, DMA_BIDIRECTIONAL);
err_attach:
