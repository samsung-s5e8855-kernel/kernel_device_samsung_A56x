...
extern struct eth_dev *g_eth_dev;
extern struct workqueue_struct	*uether_wq;
static int rndis_module_init;
...
invalid:
	...
	if (value >= 0) {
		pr_debug("rndis req%02x.%02x v%04x i%04x l%d\n",
			ctrl->bRequestType, ctrl->bRequest,
			w_value, w_index, w_length);
...
static int rndis_set_alt(struct usb_function *f, unsigned intf, unsigned alt)
{
	...
	if (intf == rndis->ctrl_id) {
		pr_debug("reset rndis control %d\n", intf);
		usb_ep_disable(rndis->notify);

		if (!rndis->notify->desc) {
			pr_debug("init rndis ctrl %d\n", intf);
		...
		if (rndis->port.in_ep->enabled) {
			pr_debug("reset rndis\n");
			mp_gether_disconnect(&rndis->port);
		}
		...
		net = mp_gether_connect(&rndis->port);
		if (IS_ERR(net))
			return PTR_ERR(net);
...
rndis_bind(struct usb_configuration *c, struct usb_function *f)
{
	 ...
	 */
	if (!rndis_opts->bound) {
		mp_gether_set_gadget(rndis_opts->net, cdev->gadget);
		status = mp_gether_register_netdev(rndis_opts->net);
		if (status)
			goto fail;
...
void rndis_borrow_net(struct usb_function_instance *f, struct net_device *net)
{
	...
	if (opts->bound)
		mp_gether_cleanup(netdev_priv(opts->net));
	else
...
/* f_rndis_item_ops */
USB_ETHERNET_CONFIGFS_MP_ITEM(rndis);

/* f_rndis_opts_dev_addr */
USB_ETHERNET_CONFIGFS_MP_ITEM_ATTR_DEV_ADDR(rndis);

/* f_rndis_opts_host_addr */
USB_ETHERNET_CONFIGFS_MP_ITEM_ATTR_HOST_ADDR(rndis);

/* f_rndis_opts_qmult */
USB_ETHERNET_CONFIGFS_MP_ITEM_ATTR_QMULT(rndis);

/* f_rndis_opts_ifname */
USB_ETHERNET_CONFIGFS_MP_ITEM_ATTR_IFNAME(rndis);

/* f_rndis_opts_class */
USB_ETHER_CONFIGFS_MP_ITEM_ATTR_U8_RW(rndis, class);

/* f_rndis_opts_subclass */
USB_ETHER_CONFIGFS_MP_ITEM_ATTR_U8_RW(rndis, subclass);

/* f_rndis_opts_protocol */
USB_ETHER_CONFIGFS_MP_ITEM_ATTR_U8_RW(rndis, protocol);
...
static int mp_gether_init(void)
{
...
/*static void mp_gether_exit(void)
{
...
static struct usb_function_instance *rndis_alloc_inst(void)
{
	...
	if (!rndis_module_init) {
		mp_gether_init();
		rndis_module_init = true;
	}
	...
	opts->net = mp_gether_setup_default();
	if (IS_ERR(opts->net)) {
...
static struct usb_function *rndis_alloc(struct usb_function_instance *fi)
{
	...
	mp_gether_get_host_addr_u8(opts->net, rndis->ethaddr);
