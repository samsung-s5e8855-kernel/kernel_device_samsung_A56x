...
#endif
#ifdef CONFIG_SLSI_BT_USE_HCI_UART_INTERFACE
#include "hci_uart_slsi.h"
#endif
...
static void slsi_bt_error_handle(int reason, bool req_restart)
{
	...
	slsi_hci_uart_block(bt_trans.htr);
#endif
	mutex_unlock(&bt_start_mutex);
...
static int slsi_bt_transport_init(unsigned int en_trs)
{
	...
	if (en_trs & SLSI_BT_TR_EN_HCI_UART) {
		htr = hci_trans_new("hci_uart_slsi");
		hci_trans_add_tail(htr, bt_trans.htr);
	}
#endif
...
#ifdef CONFIG_SLSI_BT_USE_UART_INTERFACE
	ret = slsi_bt_open(
		(SLSI_BT_TR_EN_H4 | SLSI_BT_TR_EN_PROP | SLSI_BT_TR_EN_BCSP |
		 SLSI_BT_TR_EN_TTY),
		NULL);
#endif
#ifdef CONFIG_SLSI_BT_USE_HCI_UART_INTERFACE
	ret = slsi_hci_uart_open_io();
#endif
