...
inline struct sk_buff *alloc_hci_h4_pkt_skb(int size)
{
...
static int hci_h4_skb_pack(struct hci_trans *htr, struct sk_buff *skb)
{
	...
	h4->send_skb_count++;
	return h4->reverse ?
		hci_trans_recv_skb(htr, skb) : hci_trans_send_skb(htr, skb);
}

static int hci_h4_pkt_status(char *data, size_t len)
{
	if (!data)
		return HCI_PKT_STATUS_NO_DATA;
	if (len < HCI_H4_PKT_TYPE_SIZE)
		return HCI_PKT_STATUS_NOT_ENOUGH_LENGTH;
	return hci_pkt_status(data[0], &data[1], len - 1);
}
...
static int hci_h4_skb_unpack(struct hci_trans *htr, struct sk_buff *skb)
{
	...
	return h4->reverse ?
		hci_trans_send_skb(htr, skb) : hci_trans_recv_skb(htr, skb);
}
		...
		size_t size, int is_h4)
{
	...
	if (skb == NULL) {
		...
		*ref_cnt = 0;
		...
		TR_DBG("expand skb to %zu\n", size);
		nskb = skb_copy_expand(skb, 0, size, GFP_ATOMIC);
		if (nskb == NULL)
			BT_ERR("failed to skb expand\n");
		kfree_skb(skb);
		skb = nskb;
	}
	...
	if (*ref_cnt > SLSI_BT_WR_SKB_MAX_REUSE_CNT) {
		TR_ERR("Reusing ref has reached its limit (%d)\n",
		...
		size_t count, unsigned int arg)
{
	...
	TR_DBG("count: %zu, flags: %u\n", count, flags);
	if (htr == NULL|| data == NULL || h4 == NULL)
		return -EINVAL;
	...
	ret = hci_pkt_status(GET_HCI_PKT_TYPE(skb), skb->data, skb->len);
	if (ret != HCI_PKT_STATUS_COMPLETE) {
		TR_DBG("HCI hci packet is not completed. wait more data\n");
		return count;
	}

	// completed HCI packet
	h4->wskb = NULL;
	TR_DBG("write %d bytes\n", skb->len);
		...
		size_t count, unsigned int flags)
{
	...
	ret = hci_h4_pkt_status(skb->data, skb->len);
	while (ret == HCI_PKT_STATUS_OVERRUN_LENGTH) {
		int split = HCI_H4_PKT_TYPE_SIZE +
		      hci_pkt_get_size(skb->data[0], &skb->data[1], skb->len-1);
		h4->rskb = alloc_hci_h4_pkt_skb(skb->len - split);
		skb_split(skb, h4->rskb, split);
		TR_DBG("received %d bytes\n", skb->len);
		if (hci_h4_skb_unpack(htr, skb) < 0)
			break;
		skb = h4->rskb;
		ret = hci_h4_pkt_status(skb->data, skb->len);
	}
	...
	if (ret != HCI_PKT_STATUS_COMPLETE) {
		TR_DBG("HCI H4 packet is not completed. wait more data\n");
		return count;
	}
