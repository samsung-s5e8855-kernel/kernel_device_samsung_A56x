...
static ssize_t muic_sysfs_show_usb_open(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct muic_platform_data *muic_pdata = dev_get_drvdata(dev);

	pr_info("%s, %s\n", __func__, muic_pdata->force_usb_open ? "OPEN" : "NOT_OPEN");

	return sprintf(buf, "%s\n", muic_pdata->force_usb_open ? "OPEN" : "NOT_OPEN");
}

static ssize_t muic_sysfs_set_usb_open(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	struct muic_platform_data *muic_pdata = dev_get_drvdata(dev);
	struct muic_interface_t *muic_if = muic_pdata->muic_if;
...
static DEVICE_ATTR(force_usb_open, 0664,
		muic_sysfs_show_usb_open,
		muic_sysfs_set_usb_open);
