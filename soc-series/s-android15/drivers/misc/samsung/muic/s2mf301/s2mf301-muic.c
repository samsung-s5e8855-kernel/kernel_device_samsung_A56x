    ...
    t_path_data path_data)
{
	...
	if (muic_pdata->force_usb_open) {
		pr_info("%s, force usb open\n", __func__);
...
/* FIXME */
#if !IS_ENABLED(CONFIG_MUIC_UART_SWITCH)
	if (gpiod_count(dev->parent, NULL) < 1) {
		pr_err("%s : could not find muic gpio\n", __func__);
		muic_data->pdata->gpio_uart_sel = 0;
	} else
		muic_data->pdata->gpio_uart_sel = of_get_named_gpio(np_muic, "gpio_uart_sel",  0);
#else
