...
#endif
/* when using request_firmware we need to hardcode the folders */
#elif (LINUX_VERSION_CODE < KERNEL_VERSION(6, 6, 30))
#define MX140_FW_BASE_DIR_SYSTEM_ETC_WIFI	"/vendor/etc/wifi"
#define MX140_FW_BASE_DIR_VENDOR_ETC_WIFI	"/vendor/etc/wifi"
#else
#define MX140_FW_BASE_DIR_SYSTEM_ETC_WIFI	"/vendor/firmware/wifi"
#define MX140_FW_BASE_DIR_VENDOR_ETC_WIFI	"/vendor/firmware/wifi"
#endif
...
static const struct fw_suffix fw_suffixes[] = {
	...
	{ .suffix = "_evt0.0", .hw_ver = 0xff, },
	{ .suffix = "_evt0.1", .hw_ver = 0xff, },
};
...
#else
static char *fw_base_dir = "/vendor/firmware/wifi";
#endif
#endif /* End of IS_ENABLED(CONFIG_SCSC_PCIE) */
#endif
...
#else
static char base_dir[] = "/vendor/firmware/wifi";
static char exe_dir[] = "/usr/bin/";
#endif
#endif /* End of IS_ENABLED(CONFIG_SCSC_PCIE) */
#endif
...
#ifdef CONFIG_WLBT_DUAL_FIRMWARE
...
static u32 mx140_get_board_rev(struct device *dev) {
	u32 board_rev = 0;
	if (of_property_read_u32(dev->of_node, "board_rev", &board_rev)) {
		SCSC_TAG_DEBUG(MX_FILE, "board revision reading ERROR, treat as 0\n");
	} else {
...
static void mx140_get_fw_suffix(const char *hw_rev_suffix) {
	int i;
...
void mx140_update_suffix(struct scsc_mx *mx) {
	...
	dev = scsc_mx_get_device(mx);
	board_rev = mx140_get_board_rev(dev);
#ifdef CONFIG_ARCH_EXYNOS
	SCSC_TAG_INFO_DEV(MX_FILE, dev,
			  "EXYNOS SOC_VERSION: product_id 0x%x, rev 0x%x\n",
			   exynos_soc_info.product_id, exynos_soc_info.revision);
#endif

#if defined(CONFIG_SOC_EXYNOS9955)
	if (board_rev == 0)
		chip_rev = 0;
		...
		chip_rev = 0;
#elif defined(CONFIG_SOC_EXYNOS5535)
	chip_rev = exynos_soc_info.main_rev;
#else
	chip_rev = board_rev;
#endif
	...
	mx140_get_fw_suffix(hw_rev_suffix);
}
#endif
