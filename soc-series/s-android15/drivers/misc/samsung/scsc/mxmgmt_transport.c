...
#define MEM_LENGTH 1024
	...
	SCSC_TAG_INFO(MXMGT_TRANS, "FH: allocated irq bit %d for %s\n", mxmgmt_transport->mif_istream.write_bit_idx,
		      (target == SCSC_MIF_ABS_TARGET_WLAN) ? "MXMGMT_WLAN_INPUT_TYPE" : "MXMGMT_WPAN_INPUT_TYPE");
	if(target == SCSC_MIF_ABS_TARGET_WLAN) {
	...
	SCSC_TAG_INFO(MXMGT_TRANS, "FH: allocated irq bit %d for %s\n", mxmgmt_transport->mif_ostream.write_bit_idx,
		      (target == SCSC_MIF_ABS_TARGET_WLAN) ? "MXMGMT_WLAN_OUTPUT_TYPE" : "MXMGMT_WPAN_OUTPUT_TYPE");
