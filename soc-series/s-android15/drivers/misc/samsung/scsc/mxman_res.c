...
#if IS_ENABLED(CONFIG_SCSC_MXLOGGER)
#if IS_ENABLED(CONFIG_SCSC_MEMLOG)
static void *mxman_res_memlog_get_start_vaddr(struct memlog_obj *obj)
{
	return obj->vaddr;
}
...
int mxman_res_mappings_logger_init(struct mxman *mxman, void *start_dram)
{
	...
	memset(mxman_res_memlog_get_start_vaddr(obj), 0, MX_DRAM_SIZE_SECTION_LOG);
...
cont_no_bt:
	...
	whdr_if->invalidate_error_records(whdr_if);
	if (bhdr_if) {
		bhdr_if->copy_fw(bhdr_if, (char *)bt_fw, bt_length, start_dram);
		bhdr_if->invalidate_error_records(bhdr_if);
	}
...
void mxman_res_fw_record_reset(struct mxman *mxman, enum scsc_subsystem sub)
{
	...
	case SCSC_SUBSYSTEM_WLAN:
		if (!mxman->fw_wlan) {
			SCSC_TAG_ERR(MXMAN, "Not present in the WLAN FW image\n");
			return;
		}
		fw_if = mxman->fw_wlan;
		break;
	case SCSC_SUBSYSTEM_WPAN:
		if (!mxman->fw_wpan) {
			SCSC_TAG_ERR(MXMAN, "Not present in the WPAN FW image\n");
			return;
		}
		fw_if = mxman->fw_wpan;
		break;
	...
	fw_if->invalidate_error_records(fw_if);
}
...
{
	int r;
	r = mifpmuman_recovery_mode_subsystem(scsc_mx_get_mifpmuman(mxman->mx), enable_indep);
	if (r) {
		SCSC_TAG_ERR(MXMAN, "PMU error\n");
	}
	return r;
}
#endif
...
int mxman_res_pmu_boot(struct mxman *mxman, enum scsc_subsystem sub)
{
	...
	int r = mifpmuman_start_subsystem(scsc_mx_get_mifpmuman(mxman->mx), sub);
	if (r)
		SCSC_TAG_INFO(MXMAN, "PMU error\n");
	return r;
}
...
int mxman_res_pmu_scan2mem(struct mxman *mxman, uint32_t enable_scan2mem_dump, bool dump)
{
	...
	/* This should be a blocking call, mifpmu should do the waitqueue */
	r = mifpmuman_trigger_scan2mem(scsc_mx_get_mifpmuman(mxman->mx), enable_scan2mem_dump, dump);
	if (r)
		SCSC_TAG_INFO(MXMAN, "PMU error\n");
	return r;
}
...
#endif
#if defined(CONFIG_SCSC_PCIE_CHIP)
	mif->reg_pcie_recovery_notifier(mif, notifier);
...
int mxman_res_deinit_common(struct mxman *mxman)
{
	...
	mif->unreg_pcie_recovery_notifier(mif);
#endif
#ifdef CONFIG_SCSC_SMAPPER
