...
void whdr_invalidate_error_records(struct fwhdr_if *interface)
{
}
...
struct fwhdr_if *test_alloc_fwif(struct kunit *test)
{
	...
	interface->invalidate_error_records = whdr_invalidate_error_records;
...
void *mock_platform_get_mifram_ptr(struct scsc_mif_abs *interface,  scsc_mifram_ref ref)
{
	if (ref == -1)
		return sharedmem;
...
void *mock_platform_get_mifram_phy_ptr(struct scsc_mif_abs *interface,  scsc_mifram_ref ref)
{
	return 0;
}
...
void set_visible_sharedmem(struct kunit *test, size_t size, bool visible)
{
	if (visible && !sharedmem)
		sharedmem = kunit_kzalloc(test, size * sizeof(u32), GFP_KERNEL);
	else if (!visible)
		sharedmem = NULL;
}
