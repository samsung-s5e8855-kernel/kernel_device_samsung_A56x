...
extern ssize_t (*fp_mx_procfs_mx_boot_count_wlan_read)(struct file *file, char __user *user_buf, size_t count, loff_t *ppos);
extern ssize_t (*fp_mx_procfs_mx_boot_count_wpan_read)(struct file *file, char __user *user_buf, size_t count, loff_t *ppos);
...
static void test_all(struct kunit *test)
{
	...
	fp_mx_procfs_mx_boot_count_wlan_read(&file, &user_buf, 1, &ppos);

	fp_mx_procfs_mx_boot_count_wpan_read(&file, &user_buf, 1, &ppos);
