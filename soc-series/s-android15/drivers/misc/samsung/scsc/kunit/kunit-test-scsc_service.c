...
#include "../srvman.h"
#include "../../scsc_bt/scsc_bt_priv.h"
...
static void test_scsc_service_register_check_bt_status_cb(struct kunit *test)
{
	scsc_service_register_check_bt_status_cb(1);
	scsc_service_unregister_check_bt_status_cb();
	scsc_service_register_check_bt_status_cb(0);
	scsc_service_unregister_check_bt_status_cb();
}

static void test_srvman_forward_bt_fw_log(struct kunit *test)
{
	struct mxman *mx;
	struct scsc_mx *scscmx;
	struct srvman srvman;
	...
	srvman_forward_bt_fw_log(&srvman, NULL, 5, NULL);
}

static void test_srvman_check_silent_recovery(struct kunit *test)
{
	struct mxman *mx;
	struct scsc_mx *scscmx;
	struct srvman srvman;
	...
	srvman_check_silent_recovery(&srvman, &syserr);
}

static void test_scsc_service_mxlogger_buff_size(struct kunit *test)
{
	struct scsc_service *service;

	service = test_alloc_scscservice(test);

	scsc_service_mxlogger_buff_size(service, SCSC_LOG_CHUNK_UDI, SCSC_MIFINTR_TARGET_WLAN);
	scsc_service_mxlogger_buff_size(service, SCSC_LOG_CHUNK_UDI, SCSC_MIFINTR_TARGET_WPAN);
	scsc_service_collect_buffer(service, SCSC_LOG_CHUNK_UDI, NULL, NULL, SCSC_MIFINTR_TARGET_WLAN);
	scsc_service_collect_buffer(service, SCSC_LOG_CHUNK_UDI, NULL, NULL, SCSC_MIFINTR_TARGET_WPAN);
}
...
#endif
	KUNIT_CASE(test_scsc_service_register_check_bt_status_cb),
	KUNIT_CASE(test_srvman_forward_bt_fw_log),
	KUNIT_CASE(test_srvman_check_silent_recovery),
	KUNIT_CASE(test_scsc_service_mxlogger_buff_size),
	{}
};
