...
#define READ_BINARY_TIMEOUT	1000 /* in milisecounds */
...
#define MAX_READ_BINARY_SIZE	(10*1024)
...
static DECLARE_COMPLETION(read_binary_done);
...
static DEFINE_MUTEX(read_binary_lock);
static DEFINE_MUTEX(read_binary_copy_lock);
static char *read_binary;
static size_t read_binary_bytes;
...
static int msg_from_wlbtd_read_binary_cb(struct sk_buff *skb, struct genl_info *info)
{
	struct nlattr *attr_binary = info ? info->attrs[ATTR_CONTENT] : NULL;
	char *filepath = "Unknown";
	int ret_code = 0;

	if (!attr_binary || nla_len(attr_binary) > MAX_READ_BINARY_SIZE){
		SCSC_TAG_ERR(WLBTD, "failed.\n");
		ret_code = -EINVAL;
		goto error_complete;
	}

	if (info && nla_len(info->attrs[ATTR_PATH]))
		filepath = nla_data(info->attrs[ATTR_PATH]);

	mutex_lock(&read_binary_copy_lock);

	/* The previous value is left over. The last trial probably timeout. */
	if (read_binary)
		kfree(read_binary);

	read_binary = kmalloc(nla_len(attr_binary), GFP_KERNEL);
	if (!read_binary) {
		SCSC_TAG_ERR(WLBTD, "kmalloc failed: read_binary = NULL\n");
		ret_code = -ENOMEM;
		goto error_complete;
	}
	read_binary_bytes = (size_t)nla_len(attr_binary);
	memcpy(read_binary, nla_data(attr_binary), read_binary_bytes);
	SCSC_TAG_INFO(WLBTD, "load %s: %zu\n", filepath, read_binary_bytes);

error_complete:
	mutex_unlock(&read_binary_copy_lock);
	complete(&read_binary_done);
	return ret_code;
}
...
static struct nla_policy policy_read_binary[] = {
	[ATTR_PATH] = { .type = NLA_STRING, },
	[ATTR_CONTENT] = { .type = NLA_BINARY, },
};
...
const struct genl_ops scsc_ops[] = {
	...
	{
		.cmd = EVENT_READ_BINARY,
		.flags = 0,
#if (LINUX_VERSION_CODE < KERNEL_VERSION(5, 4, 0) || LINUX_VERSION_CODE >= KERNEL_VERSION(6, 1, 25))
		.policy = policy_read_binary,
#endif
		.doit = msg_from_wlbtd_read_binary_cb,
		.dumpit = NULL,
	},
};
...
int call_wlbtd_sable(u8 trigger_code, u16 reason_code, bool sable, bool moredump)
{
	...
	if (trigger_code == SCSC_LOG_FW_PANIC)
		...
		wait_for_completion(&event_done);
...
int call_wlbtd_request_binary(const char *file_path, const uint8_t **binary, size_t *bytes)
{
	unsigned long completion_jiffies = 0;
	unsigned long max_timeout_jiffies = msecs_to_jiffies(READ_BINARY_TIMEOUT);
	struct sk_buff *skb;
	void *msg;
	int rc = 0;
	...
	*bytes = 0;
	...
	SCSC_TAG_DEBUG(WLBTD, "create message to read %s\n", file_path);
	msg = genlmsg_put(skb,
			0,		 // PID is whatever
			0,		 // Sequence number (don't care)
			&scsc_nlfamily,	 // Pointer to family struct
			0,		 // Flags
			EVENT_READ_BINARY// Generic netlink command
			);
	if (!msg) {
		SCSC_TAG_ERR(WLBTD, "Failed to create message\n");
		goto error;
	...
	/* reinit so completion can be re-used */
	reinit_completion(&read_binary_done);
	...
	SCSC_TAG_DEBUG(WLBTD, "waiting for completion\n");
	completion_jiffies = wait_for_completion_timeout(&read_binary_done,
						max_timeout_jiffies);
	...
	if (completion_jiffies == 0)
		...
		/* The received binary. User needs to free it */
		mutex_lock(&read_binary_copy_lock);
		*binary = read_binary;
		*bytes = read_binary_bytes;

		read_binary = NULL;
		read_binary_bytes = 0;
		mutex_unlock(&read_binary_copy_lock);

		SCSC_TAG_INFO(WLBTD, "read %s in %dms\n", file_path,
			completion_jiffies ? completion_jiffies : 1);
	}
...
EXPORT_SYMBOL(call_wlbtd_request_binary);
