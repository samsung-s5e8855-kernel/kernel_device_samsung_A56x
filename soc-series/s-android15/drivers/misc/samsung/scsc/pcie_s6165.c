...
#include "mif_reg_redwood_pcie.h"
#else
#error "unknown chip"
#endif
...
static uint pcie_ssc_disable = 1;
module_param(pcie_ssc_disable, uint, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(pcie_ssc_disable, "disable PCIe SSC");
#endif

#ifdef CONFIG_SCSC_BB_PAEAN
static bool enable_l1ss = true;
#elif CONFIG_SCSC_BB_REDWOOD
static bool enable_l1ss = false;
#endif
module_param(enable_l1ss, bool, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(enable_l1ss, "Enables l1ss");

static uint check_l1ss_delay = 1000;
module_param(check_l1ss_delay, uint, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(enable_l1ss, "check_l1ss_delay us, default 1000us(1ms)");

static DEFINE_RWLOCK(pcie_on_lock);
static DEFINE_RWLOCK(pcie_change_lock);
...
#ifdef CONFIG_SCSC_BB_PAEAN
#define PCI_DEVICE_ID_WLBT 0xA465
#elif CONFIG_SCSC_BB_REDWOOD
...
static uint32_t pmu_boot_flags;

void pcie_set_link_down_cb_flag(struct pcie_mif *pcie, bool val)
{
	pcie->link_down_cb_flag = val;
}

bool pcie_get_link_down_cb_flag(struct pcie_mif *pcie)
{
	return pcie->link_down_cb_flag;
}

void pcie_s6165_irqdump(struct pcie_mif *pcie)
{
	u32 val;
	if (get_pcie_link_state(pcie) != PCIE_LINK_STATE_ON){
...
static void pcie_mask_wlan_wpan_th_irq(struct pcie_mif *pcie)
{
	...
	for (i = 0; i < NUM_TO_HOST_IRQ_WLAN; i++) {
		if(!pcie->wlan_msi[i].is_masked) {
			...
			disable_irq_nosync(pcie->wlan_msi[i].msi_vec);
		}
	}
	for (i = 0; i < NUM_TO_HOST_IRQ_WPAN; i++) {
		if(!pcie->wpan_msi[i].is_masked) {
			pcie->wpan_msi[i].is_masked = true;
			disable_irq_nosync(pcie->wpan_msi[i].msi_vec);
		}
	}
}
...
#ifdef CONFIG_SCSC_BB_PAEAN
static void pcie_set_paean_remappers(struct pcie_mif *pcie)
{
	u32 cmd_dw;
...
#elif CONFIG_SCSC_BB_REDWOOD
static void pcie_set_redwood_remappers(struct pcie_mif *pcie)
{
	u32 cmd_dw;
...
void pcie_prepare_boot(struct pcie_mif *pcie)
{
	...
	/* set pmu boot flags here */
	SCSC_TAG_INFO_DEV(PCIE_MIF, pcie->dev, "Set PMU boot flags 0x%x\n", pmu_boot_flags);
	writel(pmu_boot_flags, pcie->registers_ramrp + RAMRP_HOSTIF_PMU_BOOTFLAGS);
	msleep(100);
...
#ifdef CONFIG_SCSC_BB_PAEAN
	pcie_set_paean_remappers(pcie);
#elif CONFIG_SCSC_BB_REDWOOD
	pcie_set_redwood_remappers(pcie);
#endif

#ifdef CONFIG_SOC_S5E9955
	SCSC_TAG_INFO_DEV(PCIE_MIF, pcie->dev, "SSC disable :%d\n", pcie_ssc_disable);
	SCSC_TAG_INFO_DEV(PCIE_MIF, pcie->dev, "TRIGGER IRQ %d: PMU_INTERRUPT_FROM_HOST_INTGR\n", PMU_INT_FROM_HOST_PCIE_SSC_CONFIG);
	writel(pcie_ssc_disable, pcie->registers_ramrp + RAMRP_HOSTIF_PCIE_SSC_DISABLE);
	writel(PMU_INT_FROM_HOST_PCIE_SSC_CONFIG, pcie->registers_pert + PMU_INTERRUPT_FROM_HOST_INTGR);
#endif
...
int pcie_set_mbox_pmu(struct pcie_mif *pcie, u32 val)
{
	...
	if(pcie_is_change(pcie)){
		SCSC_TAG_INFO_DEV(PCIE_MIF, pcie->dev, "Changing pcie speed. reject interrupt to pmu. Try again\n");
		return -EAGAIN;
	}
	...
	writel(PMU_INT_FROM_HOST_PMU_MBOX_REQ_MASK, pcie->registers_pert + PMU_INTERRUPT_FROM_HOST_INTGR);

	return 0;
}

int pcie_get_mbox_pmu_pcie(struct pcie_mif *pcie)
{
	u32 val = 0;
...
int pcie_set_mbox_pmu_pcie(struct pcie_mif *pcie, u32 val)
{
	SCSC_TAG_INFO_DEV(PCIE_MIF, pcie->dev, "Write CMD %x at offset 0x%x\n", val,
			  RAMRP_HOSTIF_PMU_MBOX_FROM_HOST_PTR);
	...
	SCSC_TAG_INFO_DEV(PCIE_MIF, pcie->dev, "TRIGGER IRQ 0x%x: PMU_INTERRUPT_FROM_HOST_INTGR\n",
...
int pcie_load_pmu_fw_flags(struct pcie_mif *pcie, u32 *ka_patch, size_t ka_patch_len, uint32_t flags)
{
	...
	memcpy(pcie->ka_patch_fw, ka_patch, ka_patch_len);
	pcie->ka_patch_len = ka_patch_len;
	/* save the passed flags */
	pmu_boot_flags = flags;
...
void *pcie_map(struct pcie_mif *pcie, size_t *allocated)
{
	...
	pcie_iommu_map(pcie->mem_start, pcie->mem_start, pcie->mem_sz, 0, pcie->ch_num);
...
void pcie_unmap(struct pcie_mif *pcie)
{
	...
	pcie_iommu_unmap(pcie->mem_start, pcie->mem_sz, pcie->ch_num);
...
static void wlbt_pcie_linkdown_cb(struct exynos_pcie_notify *noti)
{
	...
	pcie->link_state = PCIE_LINK_STATE_ERROR;
	...
	pcie->link_down_cb_flag = true;
	pcie->pcie_recovery_notifier();
}
...
irqreturn_t pcie_mif_pmu_pcie_isr(int irq, void *data)
{
	struct pcie_mif *pcie = (struct pcie_mif *)data;

	SCSC_TAG_DEBUG_DEV(PCIE_MIF, pcie->dev, "PCIE_OFF INT %d received, boot_state = %u\n", irq, pcie->boot_state);

	if (pcie->pmu_pcie_handler != pcie_mif_irq_default_handler)
		pcie->pmu_pcie_handler(irq, pcie->irq_dev_pmu);
	else
...
static int pcie_mif_enable_msi(struct pcie_mif *pcie)
{
	...
	for (i = 0; i < NUM_TO_HOST_IRQ_PMU; i++, acc++) {
		...
		if (acc == PMU_MBOX_PCIE) {
			...
			ret = devm_request_irq(&pcie->pdev->dev, pcie->pmu_msi[i].msi_vec, pcie_mif_pmu_pcie_isr, IRQF_TRIGGER_NONE, "PMU_MBOX_PCIE", pcie);
...
static bool pcie_mif_check_handler(struct irq_desc *desc)
{
	return desc && desc->action && (desc->action->handler == pcie_mif_isr_wlan
		|| desc->action->handler == pcie_mif_isr_wpan || desc->action->handler == pcie_mif_pmu_pcie_isr
...
static int pcie_mif_speed_change(int ch_num, int val)
{
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(6, 1, 0))
	return exynos_pcie_rc_speed_change(ch_num, val);
#else
	return -1;
#endif
}

static int pcie_mif_lane_change(int ch_num, int val)
{
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(6, 1, 0))
	return exynos_pcie_rc_lane_change(ch_num, val);
#else
	return -1;
#endif
}

extern void exynos_pcie_rc_check_l1ss(int ch_num);
static int check_l1ss_kthread(void * data)
{
	while(true){
		if (kthread_should_stop()) {
			break;
		}
		exynos_pcie_rc_check_l1ss(pcie->ch_num);
		udelay(check_l1ss_delay);
	}
	return 0;
}

static int start_check_l1ss = 0;
static ssize_t sysfs_show_start_check_l1ss(struct kobject *kobj, struct kobj_attribute *attr, char *buf);
static ssize_t sysfs_store_start_check_l1ss(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count);
static struct kobj_attribute start_check_l1ss_attr = __ATTR(start_check_l1ss, 0660, sysfs_show_start_check_l1ss, sysfs_store_start_check_l1ss);

/* Retrieve start_check_l1ss in sysfs global */
static ssize_t sysfs_show_start_check_l1ss(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", start_check_l1ss);
}

/* Update start_check_l1ss in sysfs global */
static ssize_t sysfs_store_start_check_l1ss(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t count)
{
	int r;

	r = kstrtoint(buf, 10, &start_check_l1ss);

	if (r) {
		SCSC_TAG_ERR(PLAT_MIF, "failed to read start_check_l1ss value. error %d\n", r);
		return r;
	}

	if (start_check_l1ss == 1) {
		SCSC_TAG_INFO(PCIE_MIF, "check l1ss ON\n");
		pcie->t = kthread_run(check_l1ss_kthread, NULL, "check_l1ss_kt");
	} else if(start_check_l1ss == 0) {
		SCSC_TAG_INFO(PCIE_MIF, "check l1ss OFF\n");
		kthread_stop(pcie->t);
	}
	return count;
}

/* Register start_check_l1ss override */
void pcie_create_sysfs_start_check_l1ss(void)
{
	int r;

	/* Create sysfs file /sys/kernel/start_check_l1ss */
	r = sysfs_create_file(kernel_kobj, &start_check_l1ss_attr.attr);
	if (r) {
		/* Failed, so clean up dir */
		SCSC_TAG_ERR(MXMAN, "Can't create /sys/kernel/start_check_l1ss\n");
	}
}

/* Unregister start_check_l1ss override */
void pcie_destroy_sysfs_start_check_l1ss(void)
{
	if (start_check_l1ss) {
		kthread_stop(pcie->t);
		SCSC_TAG_INFO(PCIE_MIF, "check l1ss OFF\n");
	...
	/* Destroy /sys/kernel/start_check_l1ss file */
	sysfs_remove_file(kernel_kobj, &start_check_l1ss_attr.attr);
}
...
static int pcie_mif_module_probe(struct pci_dev *pdev, const struct pci_device_id *id)
{
	...
	if(exynos_pcie_register_event(&pcie->pcie_link_down_event)) {
		SCSC_TAG_ERR_DEV(PCIE_MIF, pcie->dev, "Register LINKDOWN event FAILED!!\n");
	}
	...
	pcie_create_sysfs_start_check_l1ss();
	return 0;
}
...
static void pcie_mif_module_remove(struct pci_dev *pcie_dev)
{
	...
	pcie_destroy_sysfs_start_check_l1ss();
}
...
void pcie_irq_reg_pmu_pcie_handler(struct pcie_mif *pcie, void (*handler)(int irq, void *data), void *dev)
{
	SCSC_TAG_INFO_DEV(PCIE_MIF, pcie->dev, "Registering mif pmu pcie_off int handler %pS\n", handler);
	pcie->pmu_pcie_handler = handler;
	pcie->irq_dev_pmu = dev;
}
...
void pcie_reg_pcie_recovery_notifier(struct pcie_mif *pcie, void (*notifier)(void))
{
	SCSC_TAG_INFO_DEV(PCIE_MIF, pcie->dev, "Registering mif recovery notifier %pS\n", notifier);
	pcie->pcie_recovery_notifier = notifier;
}
...
static void set_pcie_on(int state)
{
	unsigned long flags;

	write_lock_irqsave(&pcie_on_lock, flags);
	pcie->link_state = state;
	write_unlock_irqrestore(&pcie_on_lock, flags);
}

int get_pcie_link_state(struct pcie_mif *pcie)
{
	int  ret;
	unsigned long flags;
...
EXPORT_SYMBOL(get_pcie_link_state);

void set_pcie_change(struct pcie_mif *pcie, bool change)
{
	unsigned long flags;

	write_lock_irqsave(&pcie_change_lock, flags);
	pcie->is_change = change;
	write_unlock_irqrestore(&pcie_change_lock, flags);
}
EXPORT_SYMBOL(set_pcie_change);

bool pcie_is_change(struct pcie_mif *pcie)
{
	bool ret;
	unsigned long flags;

	read_lock_irqsave(&pcie_change_lock, flags);
	ret = pcie->is_change;
	read_unlock_irqrestore(&pcie_change_lock, flags);

	return ret;
}
EXPORT_SYMBOL(pcie_is_change);
...
static void pcie_mif_save_state(struct pci_dev *pdev, int ch_num)
{
	...
	if(pcie->pci_saved_configs) {
		kfree(pcie->pci_saved_configs);
		pcie->pci_saved_configs = NULL;
	}
...
u32 pcie_mif_link_change(struct pcie_mif *pcie)
{
	int ret = 0;
	u32 msg = PMU_AP_MSG_LINK_CHANGE_SUCCESS;
	uint16_t gen;
	uint16_t lane;

	SCSC_TAG_INFO(PCIE_MIF, "start\n");
	pcie_mif_l1ss_ctrl(pcie, 0);

	gen = readw(pcie->registers_ramrp + RAMRP_HOSTIF_GEN);
	lane = readw(pcie->registers_ramrp + RAMRP_HOSTIF_LANE);

	ret = pcie_mif_speed_change(pcie->ch_num, gen);
	if(ret) {
		msg = PMU_AP_MSG_LINK_CHANGE_ERROR;
		goto done;
	}

	ret = pcie_mif_lane_change(pcie->ch_num, lane);
	if(ret) {
		msg = PMU_AP_MSG_LINK_CHANGE_ERROR;
	}
...
EXPORT_SYMBOL(pcie_mif_link_change);
...
int pcie_mif_poweron(struct pcie_mif *pcie)
{
	...
	if (ret) {
		set_pcie_on(PCIE_LINK_STATE_OFF);
	...
	set_pcie_on(PCIE_LINK_STATE_ON);
...
void pcie_mif_poweroff(struct pcie_mif *pcie)
{
	...
	if (get_pcie_link_state(pcie) == PCIE_LINK_STATE_OFF) {
	...
	if (get_pcie_link_state(pcie) == PCIE_LINK_STATE_ERROR) {

		set_pcie_on(PCIE_LINK_STATE_OFF);
		exynos_pcie_set_ready_cto_recovery(pcie->ch_num);
...
int pcie_mif_set_fw_runtime_pm(struct pcie_mif *pcie, int runtime_pm)
{
	if(get_pcie_link_state(pcie) == PCIE_LINK_STATE_OFF) {
		SCSC_TAG_ERR(PCIE_MIF, "PCIE is OFF. Can't set runtime_pm\n");
		return -ENODEV;
	}

	/* safe to communicate with PMU as PCIE is On */
	writel((u32)runtime_pm, pcie->registers_ramrp + RAMRP_HOSTIF_RUNTIME_PM);
	writel(PMU_INT_FROM_HOST_PCIE_TIMER_SET_MASK, pcie->registers_pert + PMU_INTERRUPT_FROM_HOST_INTGR);

	return 0;
}
#endif

struct device *pcie_mif_get_pcie_dev(struct pcie_mif *pcie)
{
	return pcie->dev;
}
EXPORT_SYMBOL(pcie_mif_get_pcie_dev);
