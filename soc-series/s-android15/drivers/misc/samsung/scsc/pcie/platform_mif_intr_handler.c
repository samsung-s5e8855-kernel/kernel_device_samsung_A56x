...
static int platform_mif_link_change_cb(void * first, void * second)
{
	...
	blocking_notifier_call_chain(&link_change_chain, WLBT_LINK_CHANGE_START, NULL);
	pcie_mif_link_change(g_platform->pcie);
	blocking_notifier_call_chain(&link_change_chain, WLBT_LINK_CHANGE_END, NULL);
	return 0;
}

void platform_mif_pmu_pcie_isr(int irq, void *data)
{
	struct platform_mif *platform = (struct platform_mif *)data;
