...
int mifqos_cluster_update_request(struct mifqos *qos, enum scsc_service_id id, enum scsc_qos_config config)
{
	struct scsc_mif_abs *mif;
	struct scsc_mifqos_request *req;
	int ret = 0;
	...
	mif = qos->mif;
	req = &qos->qos_req[id];
	if (mif->mif_pm_qos_cluster_update_request)
		ret = mif->mif_pm_qos_cluster_update_request(mif, req, config);
	mutex_unlock(&qos->lock);
	return ret;
}
#endif
