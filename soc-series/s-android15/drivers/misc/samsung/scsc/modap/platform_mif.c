...
/**
 * Pointer to the reserved allocated ramrp buffer.
*/
static void *ramrp_buff;
/**
 * A pointer stored localy for boot config, it will be nulled out when platform driver gets
 * unitialized
*/
static struct regmap *boot_cfg = NULL;
#endif
...
static int platform_load_pmu_fw_flags(struct scsc_mif_abs *interface, u32 *ka_patch, size_t ka_patch_len, u32 flags)
{
	...
	memcpy(platform->ka_patch_fw, ka_patch, ka_patch_len);
	*(int *)platform->ka_patch_fw = flags;
	platform->ka_patch_len = ka_patch_len;
...
static int platform_mif_set_remap(struct platform_mif *platform)
{
	...
	regmap_write(regmap,
		 WLAN_PROC_RMP_BOOT,
		(WLBT_DBUS_BARAC_0_START + platform->remap_addr_wlan) >> 12);
	regmap_read(regmap, WLAN_PROC_RMP_BOOT, &val);
	COMP_RET(((WLBT_DBUS_BARAC_0_START + platform->remap_addr_wlan) >> 12), val);
	SCSC_TAG_INFO_DEV(PLAT_MIF, platform->dev, "Updated WLAN_REMAP: 0x%x.\n", val);

	regmap_write(regmap,
		 WPAN_PROC_RMP_BOOT,
		(WLBT_DBUS_BARAC_0_START + platform->remap_addr_wpan) >> 12);
	regmap_read(regmap, WPAN_PROC_RMP_BOOT, &val);
	COMP_RET(((WLBT_DBUS_BARAC_0_START + platform->remap_addr_wpan) >> 12), val);
...
#if IS_ENABLED(CONFIG_SOC_S5E8535) || IS_ENABLED(CONFIG_SOC_S5E8835) || IS_ENABLED(CONFIG_SOC_S5E8845) || IS_ENABLED(CONFIG_SOC_S5E8855)
...
#define CHECK(x) do { \
	if (x != 0) { \
		SCSC_TAG_ERR_DEV(PLAT_MIF, platform->dev, "%s failed at L%d\n", __FUNCTION__, __LINE__); \
		goto cfg_error; \
	} \
	else { \
		SCSC_TAG_INFO_DEV(PLAT_MIF, platform->dev, "pass L%d\n", __LINE__); \
	} \
} while (0)
	u64 ret64 = 0;
...
#ifdef CONFIG_EXYNOS_EMULATOR
	/* disable S2MPU in Hybrid Zebu if it is not already diasbled by Harx team */
...
#if defined(CONFIG_SCSC_RAMRP_DUMP)
int mif_get_ramrp_buff(struct platform_mif *platform, void** buff, int count, u64 offset)
{
	int i = 0;
	const uint16_t MAX_RAMRP_SZ = PMU_BOOT_RAM_END - PMU_BOOT_RAM_START + 1;
	const uint16_t MAX_RAMRP_SZ_ALIGNED	= ((CEILING(MAX_RAMRP_SZ, PAGE_SIZE)) * (PAGE_SIZE));
	u32 val;
	u8 intsz = sizeof(unsigned int);
	/* Adjust the 'count' */
	if((offset + count) > MAX_RAMRP_SZ)
		count = MAX_RAMRP_SZ - offset;

	if (count % intsz)
		count = ((count/intsz) + 1) * intsz;

	if ((offset % intsz) && (offset > intsz))
		offset = ((offset/intsz) - 1) * intsz;
	else if (offset <= (intsz - 1))
		offset = 0;

	if (boot_cfg == NULL)
		boot_cfg = platform_mif_get_regmap(platform, BOOT_CFG);

	if (*buff == NULL) {
		/** will be allocated once and freed on releasing the diver */
	...
	/**
	 * AP has now has granted the access to RAMRP
	*/
	regmap_write(boot_cfg, PMU_BOOT, PMU_BOOT_AP_ACC);
	regmap_read(boot_cfg, PMU_BOOT, &val);
	for (i = 0; i < (count / intsz); i++) {
		regmap_read(boot_cfg, PMU_BOOT_RAM_START + (intsz * i) + (offset), ((unsigned int*) ramrp_buff + i));
		udelay(10);
	}
	/**
	 * Release the access of RAMRP to the PMU
	*/
	regmap_write(boot_cfg, PMU_BOOT, PMU_BOOT_PMU_ACC);
	regmap_read(boot_cfg, PMU_BOOT, &val);
	return count;
}

int platform_mif_get_ramrp_buff(struct scsc_mif_abs *interface, void** buff, int count, u64 offset)
{
	struct platform_mif *platform = platform_mif_from_mif_abs(interface);

	return mif_get_ramrp_buff(platform, buff, count, offset);
}
...
struct scsc_mif_abs *platform_mif_create(struct platform_device *pdev)
{
...
#if defined(CONFIG_SCSC_RAMRP_DUMP)
	platform_if->get_ramrp_buff = platform_mif_get_ramrp_buff;
#endif
#if IS_ENABLED(CONFIG_SCSC_PMU_BOOTFLAGS)
	...
	err = platform_mif_init_ioresource(pdev, platform);
	if (err)
		goto error_exit;
...
#if defined(CONFIG_WLBT_DCXO_TUNE)
	err = platform_mif_dcxo_tune_init(pdev, platform_if);
	if (err)
		goto error_exit;
#endif
...
{
	struct platform_mif *platform = NULL;

	if (!interface) {
		SCSC_TAG_ERR(PLAT_MIF, "Invalid interface object\n");
		BUG_ON(!interface);
		return NULL;
	}

	platform = platform_mif_from_mif_abs(interface);

	if (!platform) {
		SCSC_TAG_ERR(PLAT_MIF, "Invalid platform object\n");
		BUG_ON(!platform);
		return NULL;
	}
