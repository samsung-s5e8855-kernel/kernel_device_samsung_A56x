...
static ssize_t mx_procfs_mx_boot_count_wlan_read(struct file *file, char __user *user_buf, size_t count, loff_t *ppos)
{
	struct mxproc *mxproc = file->private_data;
	int pos = 0;
	char buf[32];
	...
	boot_count_wlan = atomic_read(&mxproc->mxman->boot_count_wlan);
	SCSC_TAG_INFO(MX_PROC, "boot_count_wlan : %u\n", boot_count_wlan);
	pos += scnprintf(buf + pos, bufsz - pos, "%u\n", boot_count_wlan);

	return simple_read_from_buffer(user_buf, count, ppos, buf, pos);
}

static ssize_t mx_procfs_mx_boot_count_wpan_read(struct file *file, char __user *user_buf, size_t count, loff_t *ppos)
{
	struct mxproc *mxproc = file->private_data;
	int pos = 0;
	char buf[32];
	...
	boot_count_wpan = atomic_read(&mxproc->mxman->boot_count_wpan);
	SCSC_TAG_INFO(MX_PROC, "boot_count_wpan : %u\n", boot_count_wpan);
	pos += scnprintf(buf + pos, bufsz - pos, "%u\n", boot_count_wpan);
...
MX_PROCFS_RO_FILE_OPS(mx_boot_count_wlan);
MX_PROCFS_RO_FILE_OPS(mx_boot_count_wpan);
...
int mxproc_create_info_proc_dir(struct mxproc *mxproc, struct mxman *mxman)
{
	...
	MX_PROCFS_ADD_FILE(mxproc, mx_boot_count_wlan, parent, S_IRUSR | S_IRGRP | S_IROTH);
	MX_PROCFS_ADD_FILE(mxproc, mx_boot_count_wpan, parent, S_IRUSR | S_IRGRP | S_IROTH);
...
void mxproc_remove_info_proc_dir(struct mxproc *mxproc)
{
	if (mxproc->procfs_info_dir) {
		...
		MX_PROCFS_REMOVE_FILE(mx_boot_count_wlan, mxproc->procfs_info_dir);
		MX_PROCFS_REMOVE_FILE(mx_boot_count_wpan, mxproc->procfs_info_dir);
