...
#if defined(CONFIG_SCSC_RAMRP_DUMP) || defined(CONFIG_WLBT_SPLIT_RECOVERY)
#include "mxman.h"
#endif
...
static inline int read_ramrp(struct mx_mmap_dev* mx_dev, int count, loff_t f_pos)
{
	...
	{
		SCSC_TAG_INFO(MX_MMAP, "error in getting mx_dev %p and get_ramrp_buff %p and get_ramrp_buff %p\n"
								, mx_dev, mx_dev->mif_abs->get_ramrp_buff);
		return -EINVAL;
	}
...
int mx_ramrp_mmap(struct file *filp, struct vm_area_struct *vma)
{
	...
	SCSC_TAG_INFO(MX_MMAP, "%d Bytes has been dumped successfully from RAMRP\n", bytes);
