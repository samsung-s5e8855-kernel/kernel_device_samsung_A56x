...
static void platform_mif_pmu_pcie_isr(int irq, void *data)
{
	...
	case PMU_AP_MSG_LINK_CHANGE:
		...
		spin_lock_irqsave(&platform->link_spinlock, flags);
		set_pcie_change(platform->pcie, true);
		msg = pcie_mif_link_change(platform->pcie);
		pcie_set_mbox_pmu_pcie(platform->pcie, msg);
		set_pcie_change(platform->pcie, false);
		spin_unlock_irqrestore(&platform->link_spinlock, flags);
		break;
	default:
		break;
	}
