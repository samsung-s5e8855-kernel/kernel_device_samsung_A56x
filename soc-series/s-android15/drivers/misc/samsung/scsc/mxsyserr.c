...
#if IS_ENABLED(CONFIG_SCSC_INDEPENDENT_SUBSYSTEM)
static unsigned long syserr_reset_history[MAX_NUM_SUBSYS][SYSERR_RESET_HISTORY_SIZE] = {0, };
static int syserr_reset_history_index[MAX_NUM_SUBSYS] = {0, };
#else
static unsigned long syserr_reset_history[SYSERR_RESET_HISTORY_SIZE] = {0};
static int syserr_reset_history_index = 0;
#endif
...
void mx_syserr_handler(struct mxman *mxman, const void *message)
{
	...
	if (decode.level >= MX_SYSERR_LEVEL_5) {
		...
		int i;
#if IS_ENABLED(CONFIG_SCSC_INDEPENDENT_SUBSYSTEM)
		unsigned int subsys_idx = decode.subsys - 1;
		enum scsc_subsystem sub = (decode.subsys == SYSERR_SUBSYS_WLAN) ? SCSC_SUBSYSTEM_WLAN : SCSC_SUBSYSTEM_WPAN;
#endif
		...
		if ((decode.level >= MX_SYSERR_LEVEL_7) || (mxman->syserr_recovery_in_progress)) {
			...
			mxman_recover_subsystem(mxman, SCSC_PANIC_CODE_HOST << 15, __func__, sub);
#else
			mxman_fail(mxman, SCSC_PANIC_CODE_HOST << 15, __func__, sub);
#endif
#else
...
#if IS_ENABLED(CONFIG_SCSC_INDEPENDENT_SUBSYSTEM)
		if (mxman->last_syserr_recovery_time[subsys_idx]) {
#else
		if (mxman->last_syserr_recovery_time) {
			 ...
			 */
			if (syserr_reset_min_interval) {
#if IS_ENABLED(CONFIG_SCSC_INDEPENDENT_SUBSYSTEM)
				if (time_in_range(now, mxman->last_syserr_recovery_time[subsys_idx],
						mxman->last_syserr_recovery_time[subsys_idx] + msecs_to_jiffies(syserr_reset_min_interval))) {
#else
				if ((time_in_range(now, mxman->last_syserr_recovery_time,
			...
			if (syserr_reset_monitor_period) {
...
#if IS_ENABLED(CONFIG_SCSC_INDEPENDENT_SUBSYSTEM)
					out_of_danger_period_found = ((!syserr_reset_history[subsys_idx][i]) ||
							    (!time_in_range(now, syserr_reset_history[subsys_idx][i],
								syserr_reset_history[subsys_idx][i] + msecs_to_jiffies(syserr_reset_monitor_period))));
#else
				...
				if (!out_of_danger_period_found) {
					...
					SCSC_TAG_INFO(MXMAN, "Within %dms, the count of service level reset for %s is %d\n",
						syserr_reset_monitor_period, (decode.subsys == SYSERR_SUBSYS_WLAN) ? "WiFi" : "BT", i + 1);
			}
		} else
			/* First syserr service reset since chip was (re)started - zap history */
			for (i = 0; i < SYSERR_RESET_HISTORY_SIZE; i++)
#if IS_ENABLED(CONFIG_SCSC_INDEPENDENT_SUBSYSTEM)
				syserr_reset_history[subsys_idx][i] = 0;
#else
...
#if IS_ENABLED(CONFIG_SCSC_INDEPENDENT_SUBSYSTEM)
		syserr_reset_history[subsys_idx][(syserr_reset_history_index[subsys_idx])++ % SYSERR_RESET_HISTORY_SIZE] = now;
		mxman->last_syserr_recovery_time[subsys_idx] = now;
#else
