...
static void pcie_mif_cleanup(struct scsc_mif_abs *interface)
{
}

static void pcie_mif_restart(struct scsc_mif_abs *interface)
{
}

static void pcie_mif_suspend_reg_handler(struct scsc_mif_abs *interface,
					     ...
					     void (*resume)(struct scsc_mif_abs *abs, void *data), void *data)
{
	...
	SCSC_TAG_ERR_DEV(PCIE_MIF, pcie->dev, "Registering mif suspend/resume handlers in %p %p\n", pcie,
			  interface);
	pcie->suspend_handler = suspend;
	pcie->resume_handler = resume;
	pcie->suspendresume_data = data;
}

static void pcie_mif_suspend_unreg_handler(struct scsc_mif_abs *interface)
{
	struct pcie_mif *pcie = pcie_mif_from_mif_abs(interface);

	SCSC_TAG_ERR_DEV(PCIE_MIF, pcie->dev, "Unregistering mif suspend/resume handlers in %p %p\n", pcie,
			  interface);
	pcie->suspend_handler = NULL;
	pcie->resume_handler = NULL;
	pcie->suspendresume_data = NULL;
}

#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 0))
static void pcie_recovery_disabled_reg(struct scsc_mif_abs *interface, bool (*handler)(void))
{
	struct pcie_mif *pcie = pcie_mif_from_mif_abs(interface);
	unsigned long flags;

	SCSC_TAG_ERR_DEV(PCIE_MIF, pcie->dev, "MUST NOT BE CALLED FOR x86, Registering mif recovery %pS\n", handler);
	spin_lock_irqsave(&pcie->mif_spinlock, flags);
	pcie->recovery_disabled = handler;
	spin_unlock_irqrestore(&pcie->mif_spinlock, flags);
}

static void pcie_recovery_disabled_unreg(struct scsc_mif_abs *interface)
{
	struct pcie_mif *pcie = pcie_mif_from_mif_abs(interface);
	unsigned long flags;

	SCSC_TAG_INFO_DEV(PCIE_MIF, pcie->dev, "Unregistering mif recovery\n");
	spin_lock_irqsave(&pcie->mif_spinlock, flags);
	pcie->recovery_disabled = NULL;
	spin_unlock_irqrestore(&pcie->mif_spinlock, flags);
}
#endif
...
static int pcie_mif_get_mbox_pmu_pcie(struct scsc_mif_abs *interface)
{
	struct pcie_mif *pcie = pcie_mif_from_mif_abs(interface);
	u32 val = 0;

	val = pcie_get_mbox_pmu_pcie(pcie);

	SCSC_TAG_DEBUG_DEV(PLAT_MIF, pcie->dev, "Read PMU_PCIE_MAILBOX: %u\n", val);
	return val;
}

static int pcie_mif_set_mbox_pmu_pcie(struct scsc_mif_abs *interface, u32 val)
{
	struct pcie_mif *pcie = pcie_mif_from_mif_abs(interface);
	SCSC_TAG_ERR_DEV(PCIE_MIF, pcie->dev, "MUST NOT BE CALLED FOR x86, Write PMU_PCIE_MAILBOX: %u\n", val);
...
int pcie_get_mbox_pmu_error(struct pcie_mif *pcie)
{
	...
	SCSC_TAG_INFO_DEV(PCIE_MIF, pcie->dev, "Read RAMRP_HOSTIF_ERROR_MBOX_TO_HOST_PTR: %u\n", val);
	return val;
}

static int pcie_mif_get_mbox_pmu_error(struct scsc_mif_abs *interface)
{
	struct pcie_mif *pcie = pcie_mif_from_mif_abs(interface);
	u32 val = 0;

	val = pcie_get_mbox_pmu_error(pcie);

	SCSC_TAG_INFO_DEV(PCIE_MIF, pcie->dev, "Read PMU_ERROR_MAILBOX: %u\n", val);
	return val;
}

void pcie_x86_irqdump(struct pcie_mif *pcie)
{
	u32 val;
...
static void pcie_mif_irqdump(struct scsc_mif_abs *interface)
{
	struct pcie_mif *pcie = pcie_mif_from_mif_abs(interface);
	pcie_x86_irqdump(pcie);
}
...
static int pcie_set_mbox_pmu_pcie(struct pcie_mif *pcie, u32 val)
{
...
irqreturn_t pcie_mif_pmu_isr(int irq, void *data)
{
	...
	if (pcie->pmu_handler && pcie->pmu_handler != pcie_mif_irq_default_handler)
...
irqreturn_t pcie_mif_pmu_pcie_isr(int irq, void *data)
{
	...
	SCSC_TAG_INFO_DEV(PCIE_MIF, pcie->dev, "PCIE_OFF INT %d received, boot_state = %u\n", irq, pcie->boot_state);
	pcie_set_mbox_pmu_pcie(pcie, PMU_AP_MSG_WLBT_PCIE_OFF_REJECT);

	if (pcie->pmu_pcie_handler && pcie->pmu_pcie_handler != pcie_mif_irq_default_handler)
		pcie->pmu_pcie_handler(irq, pcie->irq_dev_pmu);
	else
		SCSC_TAG_INFO_DEV(PCIE_MIF, pcie->dev, "MIF PMU PCIE_OFF Interrupt Handler not registered\n");
...
irqreturn_t pcie_mif_pmu_error_isr(int irq, void *data)
{
	...
	SCSC_TAG_INFO_DEV(PCIE_MIF, pcie->dev, "PCIE_ERROR INT %d received, boot_state = %u\n", irq, pcie->boot_state);
	if (pcie->pmu_error_handler && pcie->pmu_error_handler != pcie_mif_irq_default_handler)
		pcie->pmu_error_handler(irq, pcie->irq_dev_pmu);
	else
		SCSC_TAG_INFO_DEV(PCIE_MIF, pcie->dev, "MIF PMU PCIE_ERROR Interrupt Handler not registered\n");
...
irqreturn_t pcie_mif_isr_wlan(int irq, void *data)
{
	...
	if (pcie->wlan_handler && pcie->wlan_handler != pcie_mif_irq_default_handler) {
...
irqreturn_t pcie_mif_isr_wpan(int irq, void *data)
{
	...
	if (pcie->wpan_handler && pcie->wpan_handler != pcie_mif_irq_default_handler)
...
static void pcie_mif_irq_reg_pmu_error_handler(struct scsc_mif_abs *interface, void (*handler)(int irq, void *data),
					     void *dev)
{
	struct pcie_mif *pcie = pcie_mif_from_mif_abs(interface);
	unsigned long flags;

	SCSC_TAG_INFO_DEV(PCIE_MIF, pcie->dev, "Registering mif pmu pcie_error int handler %pS in %p %p\n", handler, pcie,
			  interface);
	spin_lock_irqsave(&pcie->mif_spinlock, flags);
...
static int pcie_mif_enable_msi(struct pcie_mif *pcie)
{
	...
	for (i = 0; i < NUM_TO_HOST_IRQ_PMU; i++, acc++) {
		...
		if (acc == PMU_MBOX_PCIE) {
			...
			ret = request_irq(pcie->pmu_msi[i].msi_vec, pcie_mif_pmu_pcie_isr, IRQF_TRIGGER_NONE, "PMU_MBOX_PCIE", pcie);
		} else if (acc == PMU_MBOX_ERROR) {
			SCSC_TAG_ERR_DEV(PCIE_MIF, pcie->dev, "Setting MSI %d interrupt vector line %d for PMU\n", acc,
					 pcie->pmu_msi[i].msi_vec);
			ret = request_irq(pcie->pmu_msi[i].msi_vec, pcie_mif_pmu_error_isr, IRQF_TRIGGER_NONE, "PMU_MBOX_ERROR", pcie);
		} else {
			SCSC_TAG_ERR_DEV(PCIE_MIF, pcie->dev, "Setting MSI %d interrupt vector line %d for PMU\n", acc,
					 pcie->pmu_msi[i].msi_vec);
			ret = request_irq(pcie->pmu_msi[i].msi_vec, pcie_mif_pmu_isr, IRQF_TRIGGER_NONE, "PMU_MBOX_PMU", pcie);
		}
		if (ret) {
			SCSC_TAG_ERR_DEV(PCIE_MIF, pcie->dev, "Failed to register MSI handler. Aborting.\n");
			return -EINVAL;
...
struct scsc_mif_abs *pcie_mif_create(struct pci_dev *pdev, const struct pci_device_id *id)
{
	...
	pcie_if->mif_cleanup = pcie_mif_cleanup;
	pcie_if->mif_restart = pcie_mif_restart;
	/* Suspend/resume not supported in PCIe MIF */
	pcie_if->suspend_reg_handler = pcie_mif_suspend_reg_handler;
	pcie_if->suspend_unreg_handler = pcie_mif_suspend_unreg_handler;
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 4, 0))
	pcie_if->recovery_disabled_reg = pcie_recovery_disabled_reg;
	pcie_if->recovery_disabled_unreg = pcie_recovery_disabled_unreg;
#endif
	...
	pcie_if->get_mbox_pmu_pcie = pcie_mif_get_mbox_pmu_pcie;
	pcie_if->set_mbox_pmu_pcie = pcie_mif_set_mbox_pmu_pcie;
	pcie_if->load_pmu_fw = pcie_load_pmu_fw;
	pcie_if->irq_reg_pmu_handler = pcie_mif_irq_reg_pmu_handler;
	pcie_if->irq_reg_pmu_error_handler = pcie_mif_irq_reg_pmu_error_handler;
	pcie_if->get_mbox_pmu_error = pcie_mif_get_mbox_pmu_error;

	pcie->pmu_handler = pcie_mif_irq_default_handler;
	pcie->pmu_error_handler = pcie_mif_irq_default_handler;
	pcie->pmu_pcie_handler = pcie_mif_irq_default_handler;
	pcie->irq_dev_pmu = NULL;

	pcie_if->wlbt_irqdump = pcie_mif_irqdump;
...
/* Stubs for x86 suspend/resume v3 */
