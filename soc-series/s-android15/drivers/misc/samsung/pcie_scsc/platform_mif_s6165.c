...
static const char *events[8] = {"RST ON", "RST OFF", "CLAIM", "CLAIM CB", "RELEASE", "PMU OFF", "PMU ON", "LINK CHANGE"};
...
static int __platform_mif_send_event_to_fsm(struct platform_mif *platform, int event, bool comp)
{
	...
	if(event == PCIE_EVT_LINK_CHANGE) {
		ev.pcie_wakeup_cb = platform->pcie_wakeup_cb;
	}else {
...
static int platform_mif_hostif_wakeup(struct scsc_mif_abs *interface, int (*cb)(void *first, void *second), void *service, void *dev)
{
	...
	if (platform->off_req == false && get_pcie_link_state(platform->pcie) == PCIE_LINK_STATE_ON) {
...
static int platform_mif_link_change_cb(void * first, void * second)
{
	u32 msg;
	unsigned long flags;
	(void)first;
	...
	msg = pcie_mif_link_change(g_platform->pcie);
	pcie_set_mbox_pmu_pcie(g_platform->pcie, msg);
...
static void platform_mif_pmu_pcie_isr(int irq, void *data)
{
	...
	case PMU_AP_MSG_LINK_CHANGE:
		...
		platform->pcie_wakeup_cb = platform_mif_link_change_cb;
					     ...
					     void *dev)
{
	...
	SCSC_TAG_INFO_DEV(PLAT_MIF, platform->dev, "Registering mif pmu pcie int handler %pS in %p %p\n", handler, platform,
			  interface);
	spin_lock_irqsave(&platform->mif_spinlock, flags);
...
static void platform_mif_reg_pcie_recovery_notifier(struct scsc_mif_abs *interface, void (*notifier)(void))
{
	...
	SCSC_TAG_INFO_DEV(PLAT_MIF, platform->dev, "Registering mif recovery notifier %pS in %p\n", notifier, interface);
	spin_lock_irqsave(&platform->mif_spinlock, flags);
...
static void platform_mif_unreg_pcie_recovery_notifier(struct scsc_mif_abs *interface)
{
	...
	SCSC_TAG_INFO_DEV(PLAT_MIF, platform->dev, "Unregistering mif recovery notifier in %p\n", interface);
	spin_lock_irqsave(&platform->mif_spinlock, flags);
...
{
	return platform_mif_get_is_parked() && !scsc_pcie_link_state_on();
}
