...
static void pcie_mif_default_notifier(void)
{
	SCSC_TAG_INFO_DEV(PCIE_MIF, pcie->dev, "\n");
}

static void pcie_mif_reg_pcie_recovery_notifier(struct scsc_mif_abs *interface, void (*notifier)(void))
{
	struct pcie_mif *pcie = pcie_mif_from_mif_abs(interface);
...
struct scsc_mif_abs *pcie_mif_create(struct pci_dev *pdev, const struct pci_device_id *id)
{
	...
	pcie_if->get_pcie_link_state = get_pcie_link_state;
	pcie_if->reg_pcie_recovery_notifier = pcie_mif_reg_pcie_recovery_notifier;
	pcie->pcie_recovery_notifier = pcie_mif_default_notifier;
