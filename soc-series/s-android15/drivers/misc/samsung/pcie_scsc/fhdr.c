...
const void *fhdr_lookup_tag(const void *blob, enum fhdr_tag tag, uint32_t *length)
{
	...
	//SCSC_TAG_DBG4(FW_LOAD, "blob: %p fhdr->tag: 0x%x\n", blob, fhdr->tag);
	...
	//SCSC_TAG_DBG4(FW_LOAD, "fhdr_lookup_tag 0x%x %u\n", tag, fhdr_length);

	while (item != NULL) {
		//SCSC_TAG_DBG4(FW_LOAD, "blob: %p item->tag: 0x%x item->length: %u\n", blob, item->tag, item->length);
		if (item->tag == tag) {
			if (length != NULL) {
				*length = item->length;
			}
			SCSC_TAG_DBG4(FW_LOAD, "fhdr_lookup_tag 0x%x found with size %u\n", item->tag, item->length);
