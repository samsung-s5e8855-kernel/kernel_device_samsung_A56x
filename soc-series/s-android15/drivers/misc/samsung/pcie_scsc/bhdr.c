...
static const void *bhdr_item_value(const struct bhdr_tag_length *item)
{
	...
	//SCSC_TAG_DBG4(FW_LOAD, "bhdr_lookup_tag current tag %u length %u\n", item->tag, item->length);
...
static void bhdr_invalidate_error_records(struct fwhdr_if *interface)
{
	struct bhdr *bhdr = bhdr_from_fwhdr_if(interface);
	void *area_start;
	size_t area_len;

	if (bhdr->fw_dram_addr == NULL) {
		SCSC_TAG_WARNING(FW_LOAD, "There is no address for fw dram\n");
		return;
	} else if (bhdr->bt_fw_runtime_size_val <= bhdr->fw_size) {
		SCSC_TAG_WARNING(FW_LOAD, "Wrong parameters: fw_len 0x%x runtime_size 0x%x\n",
								bhdr->fw_size, bhdr->bt_fw_runtime_size_val);
		return;
	}

	/* 	The area that can be derived below means the memory region that BT FW uses during runtime.
		When collecting minimoredump during creation of moredump, then the host accesses this area.	*/
	area_start = bhdr->fw_dram_addr + bhdr->bt_fw_offset_val + bhdr->fw_size;
	area_len = bhdr->bt_fw_runtime_size_val - bhdr->fw_size;

	SCSC_TAG_DBG4(FW_LOAD, "BT fw_len 0x%x runtime_size 0x%x offset 0x%x from start of Shared DRAM\n",
							bhdr->fw_size, bhdr->bt_fw_runtime_size_val, bhdr->bt_fw_offset_val);
...
struct fwhdr_if *bhdr_create(void)
{
	...
	fw_if->invalidate_error_records = bhdr_invalidate_error_records;
