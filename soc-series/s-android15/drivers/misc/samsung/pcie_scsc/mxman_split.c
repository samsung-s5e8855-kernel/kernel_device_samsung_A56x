...
#else
static bool indep_subsys_recovery_enabled = false;
#endif
...
static bool moredump_collection_on_recovery_enable = false;
...
#if IS_ENABLED(CONFIG_SCSC_BB_REDWOOD)
static int disable_recovery_handling = 0;
#else
...
static int mxman_rcvry_fsm_complete(struct mxman *mxman, bool full)
{
	...
	if (!full && th->error_during_recovery) {
...
int mxman_set_recovery_mode(struct mxman *mxman, bool enable_indep)
{
	...
	if (enable_indep) {
		...
		if (disable_recovery_handling) {
			SCSC_TAG_WARNING(MXMAN, "Disable recovery handling!\n");
			goto exit;
		}
...
#endif
		if (disable_recovery_handling) {
			SCSC_TAG_INFO(MXMAN, "Enable moredump due to disabling recovery\n");
...
static void mxman_failure_work(struct work_struct *work)
{
	...
	bool error_during_recovery = mxman->rcvry_thread.error_during_recovery;
#else
	bool enable_moredump = true;
#endif
...
#endif
		if (disable_recovery_handling) {
			SCSC_TAG_INFO(MXMAN, "Enable moredump\n");
			enable_moredump = true;
		} else if (enable_scan2mem_dump) {
			SCSC_TAG_INFO(MXMAN, "Enable moredump due to scan2mem recovery\n");
			enable_moredump = true;
#if defined(CONFIG_WLBT_SPLIT_RECOVERY)
		} else if (!error_during_recovery && srvman_check_silent_recovery(srvman, &mxman->last_syserr)) {
#else
		} else if (srvman_check_silent_recovery(srvman, &mxman->last_syserr)) {
#endif
			SCSC_TAG_INFO(MXMAN, "Disable moredump due to silent recovery\n");
...
static void mxman_init_rcvry_fsm_params(struct mxman *mxman)
{
	...
	th->error_during_recovery = false;
}
...
static int mxman_handle_rcvry_fsm(void *data)
{
	...
	while (true) {
		...
		/* PRI-22811 : Have to reset error_during_recovery flag.
		 * It makes false moredump trigger even useless condition.
		 */
		th->error_during_recovery = false;
