...
#endif
#include <pcie_scsc/scsc_debug-snapshot.h>
...
#else
static uint pcie_ssc_disable = 0;
#endif
...
#define SHARED_MEM1_SIZE (16 * 1024 * 1024)
#define SHARED_MEM2_SIZE (4 * 1024 * 1024)
#endif
...
#if defined(CONFIG_WLBT_RX_CACHEABLE)
	void *mem_2; /* DMA memory mapped to PCIe space for MX-AP comms */
#endif
...
#if defined(CONFIG_WLBT_RX_CACHEABLE)
		if (ref >= SHARED_MEM1_SIZE) {
			ref = ref - SHARED_MEM1_SIZE;
			return (void *)((uintptr_t)pcie->mem_2 + (uintptr_t)ref);
		}
#endif
...
static void __iomem *pcie_mif_map_region(unsigned long phys_addr, size_t size)
{
	...
	vmem = vmap(pages, size >> PAGE_SHIFT, VM_MAP, PAGE_KERNEL);
...
void *pcie_map(struct pcie_mif *pcie, size_t *allocated)
{
	if (allocated)
		*allocated = 0;
#if defined(CONFIG_WLBT_RX_CACHEABLE)
	pcie->mem = pcie_mif_map_region(pcie->mem_start, SHARED_MEM1_SIZE);
	pcie->mem_2 = pcie_mif_map_region(pcie->mem_start + SHARED_MEM1_SIZE, SHARED_MEM2_SIZE);
#else
...
#if defined(CONFIG_WLBT_RX_CACHEABLE)
	if (!pcie->mem_2) {
		SCSC_TAG_ERR_DEV(PCIE_MIF, pcie->dev, "Error remaping shared memory2\n");
		return NULL;
        }
#endif
...
#if defined(CONFIG_WLBT_RX_CACHEABLE)
        SCSC_TAG_INFO_DEV(PCIE_MIF, pcie->dev, "Map2: virt %p phys %lx\n", pcie->mem_2, (uintptr_t)pcie->mem_start + SHARED_MEM1_SIZE);
#endif
...
#if defined(CONFIG_WLBT_RX_CACHEABLE)
        SCSC_TAG_INFO_DEV(PCIE_MIF, pcie->dev, "Unmap2: virt %p phys %lx\n", pcie->mem_2, (uintptr_t)pcie->mem_start + SHARED_MEM1_SIZE);
#endif
	pcie_iommu_unmap(pcie->mem_start, pcie->mem_sz, pcie->ch_num);
	pcie_mif_unmap_region(pcie->mem);
	pcie->mem = NULL;
#if defined(CONFIG_WLBT_RX_CACHEABLE)
	pcie_mif_unmap_region(pcie->mem_2);
	pcie->mem_2 = NULL;
#endif
}
