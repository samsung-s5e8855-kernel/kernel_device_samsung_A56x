...
/* The buffer area for each user is different, so handler can be selected */
static int (*send_data_copy)(void *dst, const void *src, size_t len);
static int (*recv_data_copy)(void *dst, const void *src, size_t len);

static int send_data_copy_from_user(void *dst, const void *src, size_t len)
{
	return copy_from_user(dst, src, len);
}

static int recv_data_copy_to_user(void *dst, const void *src, size_t len)
{
	return copy_to_user(dst, src, len);
}

static int data_copy_memcpy(void *dst, const void *src, size_t len)
{
	return memcpy(dst, src, len) != dst;
}
...
static void scsc_bt_fw_log_update_filter(u8 trigger, u32 bt1_high, u32 bt1_low, u32 bt0_high, u32 bt0_low)
{
	...
	if (!bt_svc->fw_log.btlog_enables_used) {
		if (bt_svc->service) {
...
#ifndef CONFIG_SCSC_BT_USE_HCI_UART
	if (!waitqueue_active(&bt_svc->read_wait))
		return;
#endif

	/* Wake the reader operation */
	if (scsc_read_bt_fw_data_available() || atomic_read(&bt_svc->error_count) != 0 ||
	    bt_svc->bsmhcp_protocol->header.panic_deathbed_confession) {
		if (!wake_lock_active(&bt_svc->read_wake_lock))
			wake_lock(&bt_svc->read_wake_lock);
		wake_up(&bt_svc->read_wait);
...
static ssize_t scsc_iq_report_evt_read(char __user *buf, size_t len)
{
	...
	ret = recv_data_copy(buf, &h4_iq_report_evt[bt_svc->read_offset], len);
	if (ret == 0) {
		...
		SCSC_TAG_ERR(BT_H4, "recv_data_copy returned: %zu\n", ret);
		ret = -EACCES;
	}
...
static ssize_t scsc_hci_evt_read(char __user *buf, size_t len)
{
	...
	if (bt_svc->read_offset == 0) {
		...
		copy_len = min(sizeof(h4_hci_event_header), len);
		ret = recv_data_copy(buf, &h4_hci_event_header, copy_len);
		if (ret == 0) {
			/* All good - Update our consumed information */
			consumed = copy_len;
			bt_svc->read_offset = copy_len;
		} else {
			SCSC_TAG_WARNING(BT_H4, "recv_data_copy returned: %zu\n", ret);
			ret = -EACCES;
		}
	}

	/* Can more data be put into the userspace buffer */
	if (ret == 0 && bt_svc->read_offset >= sizeof(h4_hci_event_header) && (len > consumed)) {
		/* Calculate the amount of data that can be transferred */
		len = min((td->length - (bt_svc->read_offset - sizeof(h4_hci_event_header))), (len - consumed));

		/* Copy the data to the user buffer */
		ret = recv_data_copy(&buf[consumed],
...
static ssize_t scsc_hci_evt_error_read(char __user *buf, size_t len)
{
	...
	if (bt_svc->system_error_info != NULL) {
		...
		/* Copy the data to the user buffer */
		ret = recv_data_copy(buf, &bt_svc->system_error_info[bt_svc->read_offset], len);
	} else {
		/* Calculate the amount of data that can be transferred */
		len = min(sizeof(h4_hci_event_hardware_error) - bt_svc->read_offset, len);
		/* Copy the data to the user buffer */
		ret = recv_data_copy(buf, &h4_hci_event_hardware_error[bt_svc->read_offset], len);
	}
...
static ssize_t scsc_rx_read_data(char __user *buf, size_t len, uint16_t data_length, uint8_t *data)
{
	...
	if (bt_svc->read_offset < h4_read_data_header_len) {
		...
		ret = recv_data_copy(buf, &h4_read_data_header[bt_svc->read_offset], copy_len);
...
static ssize_t scsc_read_credit(char __user *buf, size_t len)
{
	...
	/* Copy the data to the user buffer */
	ret = recv_data_copy(buf, &h4_hci_event_ncp_header[bt_svc->read_offset], len);
	if (ret == 0) {
		/* All good - Update our consumed information */
		bt_svc->read_offset += len;
		consumed = len;
...
static ssize_t scsc_bt_shm_h4_read_hci_evt(char __user *buf, size_t len)
{
	...
	while (bt_svc->read_operation == BT_READ_OP_NONE &&
		...
		if (event_type == BSMHCP_EVENT_TYPE_CONNECTED ||
			...
			if (td->hci_connection_handle >= SCSC_BT_CONNECTION_INFO_MAX) {
				SCSC_TAG_WARNING(BT_H4,
					     "disconnect handle is beyond max (hci_connection_handle=0x%03x)\n",
					     td->hci_connection_handle);
...
static ssize_t scsc_bt_circ_h4_read_fw_log_data(char __user *buf, size_t len)
{
	...
	while (bt_svc->read_operation == BT_READ_OP_NONE &&
		...
		if (bt_svc->fw_log.fw_acl_read_state & FW_READ_VSC) {
			/* Set mxlog for Fwsnoop */
			scsc_bt_fw_log_update_filter(bt_svc->fw_log.fwlog_enables,
				bt_svc->fw_log.fwlog_enables1_high,
				bt_svc->fw_log.fwlog_enables1_low,
				bt_svc->fw_log.fwlog_enables0_high,
				bt_svc->fw_log.fwlog_enables0_low);
...
static int scsc_update_read_indexes(void)
{
	bool    gen_bg_int = false;
	bool    gen_fg_int = false;

	/* If anything was read, generate the appropriate interrupt(s) */
	if (bt_svc->bsmhcp_protocol->header.mailbox_hci_evt_read != bt_svc->mailbox_hci_evt_read)
		gen_bg_int = true;

	if (bt_svc->bsmhcp_protocol->header.mailbox_acl_rx_read != bt_svc->mailbox_acl_rx_read ||
	...
	if (bt_svc->bsmhcp_protocol->header.mailbox_iq_report_read != bt_svc->mailbox_iq_report_read)
		gen_fg_int = true;
	...
	return (gen_bg_int || gen_fg_int);
}

static inline void debug_msg_mailbox_status(void)
{
	if (bt_svc == NULL)
		return;

	SCSC_TAG_DEBUG(BT_H4, "hci_evt_read=%u, hci_evt_write=%u, acl_rx_read=%u, acl_rx_write=%u, iso_rx_read=%u, iso_rx_write=%u\n",
		bt_svc->mailbox_hci_evt_read, bt_svc->mailbox_hci_evt_write, bt_svc->mailbox_acl_rx_read,
		bt_svc->mailbox_acl_rx_write, bt_svc->mailbox_iso_rx_read, bt_svc->mailbox_iso_rx_write);

	SCSC_TAG_DEBUG(BT_H4, "acl_free_read=%u, acl_free_write=%u, iso_free_read=%u, iso_free_write=%u, iq_report_read=%u iq_report_write=%u\n",
		bt_svc->mailbox_acl_free_read, bt_svc->mailbox_acl_free_write, bt_svc->mailbox_iso_free_read,
		bt_svc->mailbox_iso_free_write, bt_svc->mailbox_iq_report_read, bt_svc->mailbox_iq_report_write);

	SCSC_TAG_DEBUG(BT_H4, "read_operation=%u, hci_event_paused=%u, data_paused=%u, fw_acl_read_state=%u\n",
		bt_svc->read_operation, bt_svc->hci_event_paused,
		bt_svc->data_paused, bt_svc->fw_log.fw_acl_read_state);
}
...
ssize_t scsc_bt_shm_h4_read(struct file *file, char __user *buf, size_t len, loff_t *offset)
{
	...
	while (0 == atomic_read(&bt_svc->error_count) && 0 == consumed) {
		...
		/* Show the mail box status */
		debug_msg_mailbox_status();
	...
	updated = scsc_update_read_indexes();
...
static void _scsc_bt_shm_h4_rx(void)
{
	unsigned char buf[DATA_PKTS_MAX_LEN] = {0, };
	ssize_t ret = 0;

	if (atomic_read(&bt_svc->error_count) != 0 && !bt_svc->recovery_waiting && bt_svc->read_operation == BT_READ_OP_NONE)
		bt_svc->read_operation = BT_READ_OP_HCI_EVT_ERROR;

	do {
		if (bt_svc->read_operation != BT_READ_OP_NONE) {
			ret = scsc_bt_shm_h4_read_continue(buf, sizeof(buf));
			if (ret != 0)
				break;
		}

		ret = scsc_bt_shm_h4_queue_sync_helper(buf, sizeof(buf));
		if (ret != 0)
			break;
		...
		/* Check each channel and read data */
		ret = scsc_bt_shm_h4_read_hci_evt(buf, sizeof(buf));
		if (ret > 0) {
			/* Handling fswnoop acl log after processing all */
			if (bt_svc->read_operation == BT_READ_OP_NONE) {
				if (bt_svc->fw_log.observers_registered == true &&
					bt_svc->fw_log.fw_acl_read_state == FW_READ_ENABLE) {
						srvman_wake_up_mxlog_thread_for_fwsnoop(bt_svc->service);
		...
		ret = scsc_bt_shm_h4_read_iso_data(buf, sizeof(buf));
		if (ret > 0)
			break;

		ret = scsc_bt_shm_h4_read_acl_data(buf, sizeof(buf));
		if (ret > 0)
			break;

		ret = scsc_bt_shm_h4_read_credit(buf, sizeof(buf));
		if (ret > 0)
			break;

		ret = scsc_bt_shm_h4_read_iq_report_evt(buf, sizeof(buf));
		if (ret > 0)
			break;

		ret = scsc_bt_circ_h4_read_fw_log_data(buf, sizeof(buf));
		if (ret > 0) {
			/* All done - reset the read interrupt to read the new FW log */
			if (bt_svc->read_operation == BT_READ_OP_NONE)
				bt_svc->fw_log.fw_acl_read_state &= FW_READ_ENABLE;
	...
	/* HCI_EVT_ERROR handle */
	if (bt_svc->read_operation == BT_READ_OP_HCI_EVT_ERROR) {
		SCSC_TAG_ERR(BT_H4, "BT_READ_OP_HCI_EVT_ERROR\n");
		ret = scsc_hci_evt_error_read(buf, sizeof(buf));
		if (bt_svc->read_operation == BT_READ_OP_NONE)
			bt_svc->read_operation = BT_READ_OP_STOP;
	}

	if (BT_READ_OP_STOP != bt_svc->read_operation && BT_READ_OP_FW_LOG_DATA != bt_svc->read_operation)
		SCSC_TAG_DEBUG(BT_H4, "hci_evt_read=%u, acl_rx_read=%u, acl_free_read=%u, read_operation=%u, ret=%zd\n",
		       bt_svc->mailbox_hci_evt_read, bt_svc->mailbox_acl_rx_read,
		       bt_svc->mailbox_acl_free_read, bt_svc->read_operation, ret);

#ifdef CONFIG_SCSC_BT_USE_HCI_UART
	if (ret > 0)
		/* Let's send it to user */
		scsc_bt_virtual_serial_rx_drain(buf, ret);
#endif
}

static void scsc_bt_shm_h4_rx(struct work_struct *work)
{
	if (bt_svc == NULL || !bt_svc->service_started)
		return;

	/* In case of the open type is SCSC_USER_TYPE_FILE_IO, the user
	 * determines the length and reads it, but in HCI_UART, we can upload it
	 * all at once. Don't make a new interrupt again, let's upload
	 * everything we see here
	 */
	while (scsc_read_data_available() || scsc_read_bt_fw_data_available() ||
	       (atomic_read(&bt_svc->error_count) && !bt_svc->recovery_waiting)) {
		scsc_update_cached_write();
		_scsc_bt_shm_h4_rx();
		scsc_update_read_indexes();
		wmb();

		if (bt_svc->read_operation == BT_READ_OP_STOP)
			break;
	}
...
ssize_t scsc_bt_shm_h4_write(struct file *file, const char __user *buf, size_t count, loff_t *offset)
{
	...
	if (atomic_read(&bt_svc->error_count) != 0) {
		...
		if (bt_svc->recovery_waiting) {
			...
			ret = wait_event_interruptible_timeout(bt_svc->read_wait,
							       !(bt_svc->recovery_waiting), HZ);
			if (ret == 0)
				ret = -EAGAIN;
		}
#endif
	...
	while (written != count && ret == 0) {
		...
		ret = send_data_copy(&h4_write_buffer[bt_svc->h4_write_offset], &buf[written], length);
		if (ret == 0) {
			...
			if ((length + bt_svc->h4_write_offset) >= H4DMUX_HEADER_HCI &&
				...
				if ((hci_pkt_len + 1) <= (length + bt_svc->h4_write_offset)) {
					...
					if (HCI_COMMAND_GET_OPCODE(h4_write_buffer) == HCI_VSC_SET_FW_LOG_BTSNOOP &&
						...
						bt_svc->fw_log.fw_acl_read_state |= FW_READ_VSC;
						bt_svc->fw_log.fwlog_enables = h4_write_buffer[4];
						bt_svc->fw_log.fwlog_enables1_high = HCI_FW_LOG_GET_FILTER(&h4_write_buffer[5]);
						bt_svc->fw_log.fwlog_enables1_low = HCI_FW_LOG_GET_FILTER(&h4_write_buffer[9]);
						bt_svc->fw_log.fwlog_enables0_high = HCI_FW_LOG_GET_FILTER(&h4_write_buffer[13]);
						bt_svc->fw_log.fwlog_enables0_low = HCI_FW_LOG_GET_FILTER(&h4_write_buffer[17]);
			...
			SCSC_TAG_WARNING(BT_H4, "send_data_copy returned: %zu\n", ret);
			ret = -EACCES;
		}
	}

	SCSC_TAG_DEBUG(BT_H4,
		       "h4_write_offset=%zu, ret=%zd, written=%zd\n",
...
int scsc_bt_shm_init(void)
{
	...
	if (bt_svc->user_type == SCSC_USER_TYPE_FILE_IO) {
		send_data_copy = send_data_copy_from_user;
		recv_data_copy = recv_data_copy_to_user;
	} else { /* SCSC_USER_TYPE_HCIUART */
		send_data_copy = data_copy_memcpy;
		recv_data_copy = data_copy_memcpy;

		bt_svc->wq = create_singlethread_workqueue("scsc_bt_wq");
		if (bt_svc->wq == NULL) {
			SCSC_TAG_WARNING(BT_H4, "Fail to create workqueue\n");
			return -ENOMEM;
		}
		INIT_WORK(&bt_svc->work_rx, scsc_bt_shm_h4_rx);
	}
...
void scsc_bt_shm_exit(void)
{
	...
	/* Release IRQs for fwsnoop */
	bt_svc->fw_log.fw_acl_read_state = FW_READ_DISABLE;
	scsc_bt_fw_log_update_filter(0, 0, 0, 0, 0);

	/* Release workqueue for work_rx */
	if (bt_svc->wq) {
		cancel_work_sync(&bt_svc->work_rx);
		flush_workqueue(bt_svc->wq);
		destroy_workqueue(bt_svc->wq);
		bt_svc->wq = NULL;
	}
