...
#include "cmu_mfc/s5e9955_cmu_mfc.h"
#endif
...
void tsmux_ioremap_wfd_pactive_sfr(struct tsmux_device *tsmux_dev) {
	...
	if (!of_property_read_u32(tsmux_dev->dev->of_node, "wfd_pactive_base", &wfd_pactive)) {
		tsmux_dev->regs_base_wfd_pactive = ioremap(wfd_pactive, sizeof(u32));
		if (tsmux_dev->regs_base_wfd_pactive == NULL)
			print_tsmux(TSMUX_ERR, "failed to ioremap wfd_pactive\n");
	}
}

void tsmux_set_wfd_pactive(struct tsmux_device *tsmux_dev, bool enable) {
	if (tsmux_dev->regs_base_wfd_pactive) {
		if (enable)
			writel(0x1, tsmux_dev->regs_base_wfd_pactive);
		else
			writel(0x0, tsmux_dev->regs_base_wfd_pactive);
	}
}
...
uint32_t tsmux_get_hw_version(struct tsmux_device *tsmux_dev)
{
	...
	while (total_udelay < 1000) {
		...
		dbg_sel_regs = TSMUX_READL(TSMUX_DBG_SEL_ADDR);
		if (dbg_sel_regs == 0xE1) {
			udelay(10);
		...
		udelay(100);
		total_udelay += 100;
		print_tsmux(TSMUX_ERR, "delay to set dbg_sel_regs\n");
	}
...
{
	return tsmux_check_interrupt_stat(tsmux_dev, job_id, false /* job done */);
}
...
{
	return tsmux_check_interrupt_stat(tsmux_dev, job_id, true /* partial done */);
}
