...
VISIBLE_IF_KUNIT inline bool is_otf_job_done(struct tsmux_context *ctx)
{
	int i;
	bool ret = false;
...
EXPORT_SYMBOL_IF_KUNIT(is_otf_job_done);
...
VISIBLE_IF_KUNIT void tsmux_handle_otf_job_done(struct tsmux_device *tsmux_dev, int job_id)
{
	int i;
	int dst_len;
	struct tsmux_context *ctx = tsmux_dev->ctx[TSMUX_CUR_CONTEXTS_NUM];
...
EXPORT_SYMBOL_IF_KUNIT(tsmux_handle_otf_job_done);

static void tsmux_handle_m2m_job_done(struct tsmux_device *tsmux_dev, int job_id)
{
	struct tsmux_context *ctx = tsmux_dev->ctx[TSMUX_CUR_CONTEXTS_NUM];
...
VISIBLE_IF_KUNIT bool tsmux_handle_otf_partial_done(struct tsmux_device *tsmux_dev, int job_id)
{
	int i;
	int dst_len = 0;
...
EXPORT_SYMBOL_IF_KUNIT(tsmux_handle_otf_partial_done);

static void tsmux_handle_m2m_partial_done(struct tsmux_device *tsmux_dev, int job_id)
{
	print_tsmux_wfdlogger(TSMUX_DBG, "Job ID %d is partial done\n", job_id);
	tsmux_clear_partial_done(tsmux_dev, job_id);
}
...
irqreturn_t tsmux_irq(int irq, void *priv)
{
	...
	for (job_id = 0; job_id < JOB_ID_COUNT; job_id++) {
		if (job_id == 0) {
			if (tsmux_is_job_done_id(tsmux_dev, job_id))
				tsmux_handle_otf_job_done(tsmux_dev, job_id);

			if (tsmux_is_partial_done_id(tsmux_dev, job_id))
				partial_done = tsmux_handle_otf_partial_done(tsmux_dev, job_id);
			continue;
		}

		if (tsmux_is_job_done_id(tsmux_dev, job_id))
			tsmux_handle_m2m_job_done(tsmux_dev, job_id);

		if (tsmux_is_partial_done_id(tsmux_dev, job_id))
			tsmux_handle_m2m_partial_done(tsmux_dev, job_id);
	}
...
VISIBLE_IF_KUNIT void tsmux_set_global_tsmux_dev(struct tsmux_device *tsmux_dev)
{
	g_tsmux_dev = tsmux_dev;
}
EXPORT_SYMBOL_IF_KUNIT(tsmux_set_global_tsmux_dev);
...
static int tsmux_open(struct inode *inode, struct file *filp)
{
	...
	tsmux_set_wfd_pactive(tsmux_dev, true);
...
static int tsmux_release(struct inode *inode, struct file *filp)
{
	...
	tsmux_set_wfd_pactive(tsmux_dev, false);
	...
	struct tsmux_buffer_info *buf_info, int dma_direction)
{
	...
	if (IS_ERR(temp_dmabuf_att)) {
		...
		temp_sgt = dma_buf_map_attachment_unlocked(temp_dmabuf_att, dma_direction);
	...
	struct tsmux_buffer_info *buf_info, int dma_direction)
{
	...
	if (!IS_ERR_VALUE(temp_dma_addr) && temp_dma_addr) {
		...
		dma_buf_unmap_attachment_unlocked(temp_dmabuf_att, temp_sgt, dma_direction);
	}
...
#else
static void tsmux_make_psi_info(struct tsmux_context *ctx, struct packetizing_param *param)
{
	uint64_t pcr;
	uint64_t pcr_base;
	uint32_t pcr_ext;
	...
	if (is_psi_invalid(ctx->psi_info.pat_len, ctx->psi_info.pmt_len, ctx->psi_info.pcr_len)) {
		print_tsmux_wfdlogger(TSMUX_ERR, "psi data is invalid\n");
		return;
	}
...
static int tsmux_get_free_buf_index(struct tsmux_context *ctx)
{
	int i = 0;
	int index = -1;
...
int tsmux_packetize(struct packetizing_param *param)
{
	...
	if ((index = tsmux_get_free_buf_index(ctx)) < 0)
	{
		print_tsmux_wfdlogger(TSMUX_ERR, "cannot packetize\n");
...
#else
static void tsmux_set_dummy_data_for_test_mode(struct tsmux_context *ctx,
		struct packetizing_param *param, int index)
{
...
static void set_pts_in_pes_hdr(struct tsmux_otf_config *config,
		struct packetizing_param *param)
{
	uint64_t pts = (param->time_stamp * 9ll) / 100ll;
...
int tsmux_packetize(struct packetizing_param *param)
{
	...
	tsmux_set_dummy_data_for_test_mode(ctx, param, index);
	...
	if (ctx->otf_cmd_queue.config.pkt_ctrl.psi_en) {
		tsmux_make_psi_info(ctx, param);
	}

	/* in case of otf, PTS should be set by tsmux device driver */
	set_pts_in_pes_hdr(config, param);
...
static int tsmux_ioctl_otf_dq_buf(struct tsmux_context *ctx, struct tsmux_otf_cmd_queue *arg)
{
...
#ifdef ASB_TEST
	// increase dq_buf wait time for asb test
	ctx->otf_cmd_queue.max_otf_job_done_wait_time_us = 100000000;
#endif
	...
	if (wait_time > 0 || index != -1) {
		...
		if (ctx->otf_outbuf_info[index].buf_state == BUF_JOB_DONE) {
			...
			ctx->otf_cmd_queue.out_buf[index].kernel_end_stamp = get_ktimestamp();
...
static int tsmux_ioctl_otf_map_buf(struct tsmux_context *ctx, struct tsmux_otf_cmd_queue *arg)
{
	...
	for (i = 0; i < TSMUX_OUT_BUF_CNT; i++) {
		...
		if (IS_ERR(temp_dmabuf_att)) {
			...
			temp_sgt = dma_buf_map_attachment_unlocked(temp_dmabuf_att, DMA_FROM_DEVICE);
			temp_dma_addr = sg_dma_address(temp_sgt->sgl);
		...
		if (IS_ERR_VALUE(temp_dma_addr) || temp_dma_addr == 0) {
			...
			ret = dma_buf_vmap_unlocked(temp_dmabuf, &temp_iosys_map);
			if (ret) {
	...
	for (i = 0; i < TSMUX_OUT_BUF_CNT; i++) {
		...
		if (temp_vaddr) {
			print_tsmux(TSMUX_DBG, "dma_buf_vunmap(%pK, %pK)\n",
					temp_dmabuf, temp_vaddr);
			dma_buf_vunmap_unlocked(temp_dmabuf, &temp_iosys_map);
		}

		if (!IS_ERR_VALUE(temp_dma_addr) && temp_dma_addr) {
			print_tsmux(TSMUX_DBG, "dma_buf_unmap_attachment(%pK, %llx)\n",
					temp_dmabuf_att, temp_dma_addr);
			dma_buf_unmap_attachment_unlocked(temp_dmabuf_att, temp_sgt, DMA_FROM_DEVICE);
		}
...
static int tsmux_ioctl_sfr_dump_unmap_buf(struct tsmux_context *ctx)
{
	...
	if (ctx->sfr_dump_buf_info.vaddr) {
		...
		dma_buf_vunmap_unlocked(ctx->sfr_dump_buf_info.dmabuf, &ctx->sfr_dump_buf_info.map);
	}
	...
	if (!IS_ERR_VALUE(ctx->sfr_dump_buf_info.dma_addr) && ctx->sfr_dump_buf_info.dma_addr) {
		...
		dma_buf_unmap_attachment_unlocked(ctx->sfr_dump_buf_info.dmabuf_att, ctx->sfr_dump_buf_info.sgt, DMA_FROM_DEVICE);
	}
...
static int tsmux_probe(struct platform_device *pdev)
{
	...
	tsmux_ioremap_wfd_pactive_sfr(tsmux_dev);
