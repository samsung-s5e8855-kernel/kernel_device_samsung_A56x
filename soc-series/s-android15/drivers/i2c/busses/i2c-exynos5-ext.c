...
static int change_i2c_gpio_io(struct exynos5_i2c *i2c)
{
	struct pinctrl_state *io_i2c_pins;
	struct pinctrl *io_i2c_pinctrl;
	int status = 0;

	io_i2c_pinctrl = devm_pinctrl_get(i2c->dev);
	if (IS_ERR(io_i2c_pinctrl)) {
		dev_err(i2c->dev, "Can't get i2c pinctrl!!!\n");
		return -EINVAL;
	}

	io_i2c_pins = pinctrl_lookup_state(io_i2c_pinctrl, "bus_io");
	if (!IS_ERR(io_i2c_pins)) {
		status = pinctrl_select_state(io_i2c_pinctrl, io_i2c_pins);
		if (status) {
			dev_err(i2c->dev, "Can't set io i2c pins!!!\n");
			return -EINVAL;
		}
	} else {
		dev_err(i2c->dev, "Can't get io pinstate!!!\n");
		return -EINVAL;
	}
...
static void recover_gpio_pins(struct exynos5_i2c *i2c)
{
	...
	if (sda_val == 0) {
		if (i2c->bus_clear) {
			/* To make SCL pulses, set i2c pinctrl as IO mode */
			if (change_i2c_gpio_io(i2c) < 0) {
				dev_err(i2c->dev, "i2c pinctrl for io mode is no set - skip bus clear\n");
			} else {
			...
			dev_err(i2c->dev, "i2c bus clear is not enabled - skip bus clear\n");
		}
	}
...
VISIBLE_IF_KUNIT u32 exynos5_i2c_func(struct i2c_adapter *adap)
{
	return I2C_FUNC_I2C | (I2C_FUNC_SMBUS_EMUL & ~I2C_FUNC_SMBUS_QUICK);
}
EXPORT_SYMBOL_IF_KUNIT(exynos5_i2c_func);
...
static int exynos5_i2c_probe(struct platform_device *pdev)
{
	...
	ret = of_property_read_u32(np, "samsung,i2c-bus-clear", &i2c->bus_clear);
	if (ret) {
		dev_info(&pdev->dev, "i2c bus clear is not defined - set as default(Enabled)\n");
		i2c->bus_clear = 1;
	}

	dev_info(&pdev->dev, "i2c bus clear : %s\n", i2c->bus_clear ? "Enabled" : "Disabled");
	...
	/* Set device as Asynchronous suspend */
