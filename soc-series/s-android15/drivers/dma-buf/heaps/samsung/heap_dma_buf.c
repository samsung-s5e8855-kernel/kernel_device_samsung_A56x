...
static int samsung_heap_mmap(struct dma_buf *dmabuf, struct vm_area_struct *vma)
{
	...
	for_each_sgtable_page(table, &piter, vma->vm_pgoff) {
		...
		if (dma_heap_flags_direct_io(buffer->flags))
...
void samsung_heap_deferred_free(struct deferred_freelist_item *item, enum df_reason reason)
{
	struct samsung_dma_buffer *buffer;

	buffer = container_of(item, struct samsung_dma_buffer, deferred_free);

	if (dma_heap_flags_uncached(buffer->flags) && !dma_heap_flags_protected(buffer->flags))
		heap_cache_flush(buffer);

	buffer->heap->ops->release(buffer, reason);
...
static void samsung_heap_dma_buf_release(struct dma_buf *dmabuf)
{
	...
	if (dma_heap_flags_deferred_free(buffer->flags)) {
		deferred_free(&buffer->deferred_free, samsung_heap_deferred_free, npages);
		return;
	}
	...
	buffer->heap->ops->release(buffer, 0);
