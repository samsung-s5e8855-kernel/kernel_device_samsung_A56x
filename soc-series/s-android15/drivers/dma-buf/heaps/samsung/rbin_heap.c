...
static int rbin_heap_allocate(struct samsung_dma_buffer *buffer)
{
	struct samsung_dma_heap *heap = buffer->heap;
	struct rbin_heap *rbin_heap = heap->priv;
	...
	size_remain = last_size = PAGE_ALIGN(buffer->len);
...
got_pg:
	...
	if (sg_alloc_table(&buffer->sg_table, i, GFP_KERNEL)) {
		perrfn("failed to allocate sgtable (nents : %d)", i);
		goto free_buffer;
	}
	...
	atomic_add(buffer->len >> PAGE_SHIFT, &rbin_alloced_pages);
...
static void rbin_heap_release(struct samsung_dma_buffer *buffer, enum df_reason reason)
{
...
static const struct samsung_heap_ops rbin_heap_ops = {
	.allocate = rbin_heap_allocate,
	.release = rbin_heap_release,
};

static struct samsung_dma_heap rbin_dma_heap = {
	.name = "camera",
...
static struct samsung_dma_heap rbin_uncached_dma_heap = {
	.name = "camera-uncached",
...
static int rbin_heap_probe(struct platform_device *pdev)
{
	...
	if (!kobj) {
		perrdev(&pdev->dev, "failed to create kobject");
		goto out;
	}
	...
	rbin_dma_heap.priv = rbin_heap;
	if (samsung_heap_add(&rbin_dma_heap))
		goto out;
	rbin_uncached_dma_heap.priv = rbin_heap;
	if (samsung_heap_add(&rbin_uncached_dma_heap))
		goto out;

	pr_info("%s created %s\n", __func__, rmem->name);
	return 0;
out:
	rbin_dma_heap.priv = NULL;
	rbin_uncached_dma_heap.priv = NULL;
