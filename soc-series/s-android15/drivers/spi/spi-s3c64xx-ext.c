...
#include <soc/samsung/samsung-pdma.h>
...
#define S3C64XX_SPI_OPTION		0x80
...
#define S3C64XX_SPI_PACKET_CNT_MASK		((1ULL << 16) - 1)

#define S3C64XX_V2_SPI_PACKET_CNT_EN		(1ULL << 31)
#define S3C64XX_V2_SPI_PACKET_CNT_MASK		((1ULL << 24) - 1)
...
#define S3C64XX_SPI_CSN_STREAM		(1<<1)
...
VISIBLE_IF_KUNIT unsigned int s3c64xx_spi_get_max_transfer_packet(struct s3c64xx_spi_driver_data *sdd)
{
	struct s3c64xx_spi_info *sci = sdd->cntrlr_info;
	unsigned int max_transfer_packet;

	max_transfer_packet = S3C64XX_SPI_PACKET_CNT_MAX;
	if (sci->usi_version > 2)
		max_transfer_packet = (S3C64XX_V2_SPI_PACKET_CNT_MASK & ~(1 << 4));

	return max_transfer_packet;
}
EXPORT_SYMBOL_IF_KUNIT(s3c64xx_spi_get_max_transfer_packet);
				...
				struct spi_transfer *xfer, int dma_mode)
{
	...
	if (dma_mode) {
		...
		if (sci->usi_version == 2) {
			...
			packet_cnt_en = readl(regs + S3C64XX_SPI_PACKET_CNT);
			packet_cnt_en &= ~S3C64XX_V2_SPI_PACKET_CNT_EN;
			writel(packet_cnt_en, regs + S3C64XX_SPI_PACKET_CNT);

			writel(((xfer->len * 8 / sdd->cur_bpw) & S3C64XX_V2_SPI_PACKET_CNT_MASK),
				regs + S3C64XX_SPI_PACKET_CNT);
					    ...
					    struct spi_message *msg)
{
	...
	max_packet = s3c64xx_spi_get_max_transfer_packet(sdd);
	...
	list_for_each_entry(xfer, &msg->transfers, transfer_list) {
		...
		if ((sci->dma_mode != DMA_MODE) || !(sdd->rx_dma.ch && sdd->tx_dma.ch)) {
			...
			target_len = xfer->len;
			if (xfer->len > max_packet * sdd->cur_bpw / 8)
				xfer->len = max_packet * sdd->cur_bpw / 8;
		}
try_transfer:
		if (sci->dma_mode == DMA_MODE) {
		...
		if (sci->dma_mode != DMA_MODE) {
			...
			if (target_len > 0) {
				if (target_len > max_packet * sdd->cur_bpw / 8)
					xfer->len = max_packet * sdd->cur_bpw / 8;
				else
					xfer->len = target_len;
				goto try_transfer;
...
static void exynos_usi_init(struct s3c64xx_spi_driver_data *sdd)
{
	...
	/* From USIv3, USI does not support legacy scheme for nCS_TIME_COUNT control scheme
	 * Consider it as new scheme with nCS driven as high untile nCS_TIME_COUNT is expired
	 */
	if (sci->usi_version == 2) {
...
static void s3c64xx_spi_hwinit(struct s3c64xx_spi_driver_data *sdd, int channel)
{
	...
	if (sci->usi_version > 2) {
		val = readl(regs + S3C64XX_SPI_OPTION);
		if (sci->cs_stream == NCS_STREAM_ENABLE)
			val |= S3C64XX_SPI_CSN_STREAM;
		else
			val &= ~S3C64XX_SPI_CSN_STREAM;

		writel(val, regs + S3C64XX_SPI_OPTION);
	}
...
static struct s3c64xx_spi_info *s3c64xx_spi_parse_dt(struct device *dev)
{
	...
	/* SPI USI version, default is 2 */
	if (of_property_read_u32(dev->of_node, "samsung,usi-version", &sci->usi_version))
		sci->usi_version = 2;

	if (sci->usi_version > 2) {
		if (of_get_property(dev->of_node, "samsung,spi-cs-stream", NULL))
			sci->cs_stream = NCS_STREAM_ENABLE;
		else
			sci->cs_stream = NCS_STREAM_DISABLE;
	}

	dev_info(dev, "set spi usi version as %d\n", sci->usi_version);
