...
static int s2mpb02_enable(struct regulator_dev *rdev)
{
...
static int s2mpb02_disable_regmap(struct regulator_dev *rdev)
{
	struct s2mpb02_data *info = rdev_get_drvdata(rdev);
	struct i2c_client *i2c = info->iodev->i2c;
	u8 val;
...
static int s2mpb02_is_enabled_regmap(struct regulator_dev *rdev)
{
	struct s2mpb02_data *info = rdev_get_drvdata(rdev);
	struct i2c_client *i2c = info->iodev->i2c;
	struct regulator_dev *s2mpb02_rdev = rdev;
	int ret;
	u8 val;

	ret = s2mpb02_read_reg(i2c, s2mpb02_rdev->desc->enable_reg, &val);
	if (ret < 0)
		return ret;

	val &= s2mpb02_rdev->desc->enable_mask;

	return (val == s2mpb02_rdev->desc->enable_mask);
}

static int s2mpb02_get_voltage_sel_regmap(struct regulator_dev *rdev)
{
	struct s2mpb02_data *info = rdev_get_drvdata(rdev);
	struct i2c_client *i2c = info->iodev->i2c;
	int ret;
...
static int s2mpb02_set_voltage_sel_regmap(struct regulator_dev *rdev, unsigned sel)
{
	struct s2mpb02_data *info = rdev_get_drvdata(rdev);
	struct i2c_client *i2c = info->iodev->i2c;
	int ret;
...
static int s2mpb02_set_voltage_sel_regmap_buck(struct regulator_dev *rdev,
					unsigned sel)
{
	struct s2mpb02_data *info = rdev_get_drvdata(rdev);
	struct i2c_client *i2c = info->iodev->i2c;
...
static int s2mpb02_set_voltage_time_sel(struct regulator_dev *rdev,
				   ...
				   unsigned int new_selector)
{
	...
	/* sanity check */
	if (!s2mpb02_rdev->desc->ops->list_voltage)
		return -EINVAL;

	old_volt = s2mpb02_rdev->desc->ops->list_voltage(s2mpb02_rdev, old_selector);
	new_volt = s2mpb02_rdev->desc->ops->list_voltage(s2mpb02_rdev, new_selector);
...
static struct regulator_ops s2mpb02_ldo_ops = {
	...
	.is_enabled		= s2mpb02_is_enabled_regmap,
	.enable			= s2mpb02_enable,
	.disable		= s2mpb02_disable_regmap,
	.get_voltage_sel	= s2mpb02_get_voltage_sel_regmap,
	.set_voltage_sel	= s2mpb02_set_voltage_sel_regmap,
	.set_voltage_time_sel	= s2mpb02_set_voltage_time_sel,
};
...
static struct regulator_ops s2mpb02_buck_ops = {
	...
	.disable		= s2mpb02_disable_regmap,
	.get_voltage_sel	= s2mpb02_get_voltage_sel_regmap,
	.set_voltage_sel	= s2mpb02_set_voltage_sel_regmap_buck,
	.set_voltage_time_sel	= s2mpb02_set_voltage_time_sel,
};
...
{
	struct device_node *pmic_np = NULL, *s2mpb02_regulators_np = NULL, *reg_np = NULL;
	struct s2mpb02_regulator_data *s2mpb02_rdata = NULL;
	size_t i = 0;
	...
	s2mpb02_regulators_np = of_find_node_by_name(pmic_np, "regulators");
	if (!s2mpb02_regulators_np) {
	...
	for_each_child_of_node(s2mpb02_regulators_np, reg_np) {
		pdata->num_regulators++;
	}

	s2mpb02_rdata = devm_kzalloc(iodev->dev, sizeof(*s2mpb02_rdata) *
				pdata->num_regulators, GFP_KERNEL);
	if (!s2mpb02_rdata)
		return -ENOMEM;

	pdata->regulators = s2mpb02_rdata;
	pdata->num_rdata = 0;
	for_each_child_of_node(s2mpb02_regulators_np, reg_np) {
		...
		s2mpb02_rdata->id = i;
		s2mpb02_rdata->initdata = of_get_regulator_init_data(iodev->dev, reg_np,
							     &regulators[i]);
		s2mpb02_rdata->reg_node = reg_np;
		s2mpb02_rdata++;
		pdata->num_rdata++;
	}
	of_node_put(s2mpb02_regulators_np);
...
static ssize_t s2mpb02_read_store(struct device *dev,
				  ...
				  const char *s2mpb02_buf, size_t s2mpb02_size)
{
	...
	ret = kstrtou8(s2mpb02_buf, 0, &reg_addr);
	if (ret < 0)
				   ...
				   const char *s2mpb02_buf, size_t s2mpb02_size)
{
	...
	ret = sscanf(s2mpb02_buf, "0x%02hhx 0x%02hhx", &reg, &data);
	if (ret != 2) {
...
static struct pmic_device_attribute regulator_attr[] = {
	PMIC_ATTR(write, 0644, s2mpb02_write_show, s2mpb02_write_store),
	PMIC_ATTR(read, 0644, s2mpb02_read_show, s2mpb02_read_store),
};
