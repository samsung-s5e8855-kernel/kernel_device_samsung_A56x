...
void phy_exynos_snps_usbdp_tune_each_param(struct exynos_usbphy_info *info, char *name, int val)
{
	...
	void __iomem *usbphy_ctrl_base = info->regs_base;
	u32 cr_reg = 0;
	...
	*/
	if (!strcmp(name, "tx_eq_main_g1")) {
		cr_reg = readl(usbphy_ctrl_base + SNPS_USBDPPHY_REG_PHY_EXT_CONFIG18);
		cr_reg &= ~((0x3 << 24) | (0xfff << 0));
		cr_reg |= (3 << 24) | (val << 6) | (val << 0);
		writel(cr_reg, usbphy_ctrl_base + SNPS_USBDPPHY_REG_PHY_EXT_CONFIG18);
	} else if (!strcmp(name, "tx_eq_post_g1")) {
		cr_reg = readl(usbphy_ctrl_base + SNPS_USBDPPHY_REG_PHY_EXT_CONFIG19);
		cr_reg &= ~((0xfff << 0));
		cr_reg |= (val << 6) | (val << 0);
		writel(cr_reg, usbphy_ctrl_base + SNPS_USBDPPHY_REG_PHY_EXT_CONFIG19);
	} else if (!strcmp(name, "tx_eq_pre_g1")) {
		cr_reg = readl(usbphy_ctrl_base + SNPS_USBDPPHY_REG_PHY_EXT_CONFIG20);
		cr_reg &= ~((0xfff << 0));
		cr_reg |= (val << 6) | (val << 0);
		writel(cr_reg, usbphy_ctrl_base + SNPS_USBDPPHY_REG_PHY_EXT_CONFIG20);
...
static int update_fw_to_sram(struct exynos_usbphy_info *info)
{
	...
	if (EXYNOS_USBCON_VER_MINOR(info->version) == 0) {
		...
		code = phy_fw_code;
			...
			(EXYNOS_USBCON_VER_MINOR(info->version) == 2)) {
		code_size = sizeof(phy_fw_u1u2_enabled) / sizeof(struct ram_code);
		code = phy_fw_u1u2_enabled;
		skip_check_ack = 1;
	} else if ((EXYNOS_USBCON_VER_MINOR(info->version) == 3) ||
       			(EXYNOS_USBCON_VER_MINOR(info->version) == 4)) {
		code_size = sizeof(phy_fw_abnormal_term_fix) / sizeof(struct ram_code);
		code = phy_fw_abnormal_term_fix;
		skip_check_ack = 1;
	}
...
static void phy_exynos_snps_usbdp_tune(struct exynos_usbphy_info *info)
{
	if (!info)
		return;
	if (!info->tune_param)
...
static int additional_cr_reg_update(struct exynos_usbphy_info *info)
{
	...
	/* tx_vswing_lvl adjust : Do not use ovride by CR */
	if ((EXYNOS_USBCON_VER_MINOR(info->version) == 2) ||
			(EXYNOS_USBCON_VER_MINOR(info->version) == 4)) {
		/*Lane0:
			- phy_ext_tx_eq_ovrd_g1[0:0] / phy_ext_tx_eq_pre_g1[5:0] 
			- phy_ext_tx_eq_main_g1[5:0] / phy_ext_tx_eq_post_g1[5:0]

			Lane1:
			- phy_ext_tx_eq_ovrd_g1[1:1] / phy_ext_tx_eq_pre_g1[11:6]
			- phy_ext_tx_eq_main_g1[11:6] / phy_ext_tx_eq_post_g1[11:6]
		*/
		// TX_EQ_MAIN : 51-->47
		writel(0x03CF3BEF, info->regs_base + SNPS_USBDPPHY_REG_PHY_EXT_CONFIG18);
		// TX_EQ_POST : 10-->14
		writel(0x0028A38E, info->regs_base + SNPS_USBDPPHY_REG_PHY_EXT_CONFIG19);
		// TX_EQ_PRE
		writel(0x0000000, info->regs_base + SNPS_USBDPPHY_REG_PHY_EXT_CONFIG20);
	}
...
int phy_exynos_snps_usbdp_phy_enable(struct exynos_usbphy_info *info)
{
	...
	if ((EXYNOS_USBCON_VER_MINOR(info->version) == 1) ||
			(EXYNOS_USBCON_VER_MINOR(info->version) == 3)) {
		/* 4-lpp mplla/b_ctr_cp_int_ref x2*/
		phy_exynos_snps_usbdp_cr_write(info, 0x005c, 0xc4c4);
        } else if ((EXYNOS_USBCON_VER_MINOR(info->version) == 2) ||
		       (EXYNOS_USBCON_VER_MINOR(info->version) == 4)) {
		/* 3-gap mplla/b_ctr_cp_int_ref x2*/
		phy_exynos_snps_usbdp_cr_write(info, 0x005c, 0xc1c1);
	}
