...
struct exynos_mipi_phy {
	...
	struct mipi_phy_desc {
		...
		unsigned int bias_offset;
		void __iomem *regs; /* clock */
		void __iomem *regs_lane;
		void __iomem *regs_bias;
		void __iomem *regs_cmn;
		void __iomem *regs_cal;
	} phys;
};
...
#define to_mipi_video_phy(desc) \
	container_of((desc), struct exynos_mipi_phy, phys)
...
static int exynos_mipi_phy_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(state->reg_pmu)) {
		...
		if (of_property_read_bool(node, "isolation")) {
			ret = of_property_read_u32(node, "isolation", &iso);
			if (ret) {
		...
		if (of_property_read_bool(node, "power-gating")) {
	...
	if (IS_ERR(state->reg_reset)) {
		...
		if (!of_property_read_bool(node, "reset"))
			return -EINVAL;

		ret = of_property_read_u32(node, "reset", &rst);
		if (ret) {
	...
	state->phys.sz_pg = elements;
	state->phys.iso_offset = iso;
	state->phys.rst_bit = rst;

	if (state->reg_pmu) {
		dev_info(dev, "isolation: 0x%x\n", state->phys.iso_offset);

		for (i = 0; i < elements; i++) {
			state->phys.pg_offset[i] = pg[i];

			if (pg[i] > 0) {
				dev_info(dev, "power-gating[%d]: 0x%x\n",
						i, state->phys.pg_offset[i]);
	...
	if (res) {
		/* clock */
		dev_info(dev, "clock defalut 0 to get: %pR", res);
		state->phys.regs = devm_ioremap_resource(dev, res);
		if (IS_ERR(state->phys.regs))
			return PTR_ERR(state->phys.regs);
	}

	res = platform_get_resource_byname(pdev, IORESOURCE_MEM, "lane");
	if (res) {
		/* lane */
		dev_info(dev, "mipi phy lane: %pR", res);
		state->phys.regs_lane = devm_ioremap(dev, res->start, resource_size(res));
		if (!state->phys.regs_lane) {
			dev_err(dev, "can't devm_ioremap for lane %pR\n", res);
			return -ENOMEM;
		}
	} else {
		/* default lane */
		state->phys.regs_lane = state->phys.regs + 0x100;
	}

	res = platform_get_resource_byname(pdev, IORESOURCE_MEM, "cmn");
	if (res) {
		dev_info(dev, "mipi phy cmn: %pR", res);
		state->phys.regs_cmn = devm_ioremap(dev, res->start, resource_size(res));
		if (!state->phys.regs_cmn) {
			dev_err(dev, "can't devm_ioremap for cmn %pR\n", res);
			return -ENOMEM;
		}
	} else {
		dev_err(dev, "can't get resource for cmn %pR\n", res);
	}
	res = platform_get_resource_byname(pdev, IORESOURCE_MEM, "cal");
	if (res) {
		dev_info(dev, "mipi phy cal: %pR", res);
		state->phys.regs_cal = devm_ioremap(dev, res->start, resource_size(res));
		if (!state->phys.regs_cal) {
			dev_err(dev, "can't devm_ioremap for cal %pR\n", res);
			return -ENOMEM;
		}
	} else {
		dev_err(dev, "can't get resource for cal %pR\n", res);
	}
	res = platform_get_resource_byname(pdev, IORESOURCE_MEM, "bias");
	if (res) {
		/* bais */
		state->phys.regs_bias = devm_ioremap(dev, res->start, resource_size(res));
		if (!state->phys.regs_bias) {
			dev_err(dev, "can't devm_ioremap for lane %pR\n", res);
			return -ENOMEM;
		}
	} else {
		/* for default bias */
		state->phys.bias_offset = 0x1000;
	}
	...
	state->phys.data = phy_data;
	phy_set_drvdata(generic_phy, &state->phys);

	phy_provider = devm_of_phy_provider_register(dev, exynos_mipi_phy_of_xlate);
