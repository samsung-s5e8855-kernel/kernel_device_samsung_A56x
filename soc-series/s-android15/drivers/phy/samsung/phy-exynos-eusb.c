...
void phy_exynos_eusb_tune(struct exynos_usbphy_info *info)
{
	...
	for (; info->tune_param[cnt].value != EXYNOS_USB_TUNE_LAST; cnt++) {
		...
		if (!strcmp(para_name, "tx_fsls_slew_rate")) {
		...
		} else if (!strcmp(para_name, "tx_hs_drv_ofst")) {
			((EUSBPHY_REG_TXTUNE_p) (&phy_cfg_tx))->b.hs_drv_ofst = val;
		...
		} else if (!strcmp(para_name, "tx_vpsf_vref_tune")) {
			((EUSBPHY_REG_TXTUNE_p) (&phy_cfg_tx))->b.vpsf_vref_tune = val;
			...
			phy_cfg_tx = val & 0x3ffff;
			phy_cfg_rx = ((val >> 21) & 0x7) << 8 | ((val >> 20) & 0x1) << 4 | ((val >> 18) & 0x3);
			break;
		}
	}
...
void phy_exynos_eusb_initiate(struct exynos_usbphy_info *info)
{
	...
	((EUSBPHY_REG_TXTUNE_p) (&reg))->b.hs_drv_ofst = 0;
