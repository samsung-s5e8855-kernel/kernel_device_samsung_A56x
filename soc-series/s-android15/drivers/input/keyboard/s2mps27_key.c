/*
 * Driver for Power key/Vol Down key on s2mps27 IC by PWRON rising, falling interrupts.
 *
 * drivers/input/keyboard/s2mps27_key.c
 * S2MPS27 Keyboard Driver
 *
...
static void s2mps27_keys_work_func(struct work_struct *work)
{
...
static irqreturn_t s2mps27_keys_press_irq_handler(int irq, void *dev_id)
{
...
static irqreturn_t s2mps27_keys_release_irq_handler(int irq, void *dev_id)
{
	struct s2p_keys_drvdata *ddata = dev_id;
...
of_s2mps27_keys_parse_dt(struct s2mps27_dev *iodev)
{
	#define S2MPS27_SUPPORT_KEY_NUM	(2)	// power, vol-down key
	struct device *dev = iodev->dev;
	struct device_node *mfd_np, *key_np;
	struct s2p_keys_platform_data *pdata;
	int error, nbuttons;
	size_t size;
	...
	key_np = of_find_node_by_name(mfd_np, "s2mps27-keys");
	if (!key_np) {
	...
	if (nbuttons != S2MPS27_SUPPORT_KEY_NUM) {
		dev_warn(dev, "%s: s2mps27-keys support only two button(%d)\n",
			__func__, nbuttons);
		error = -ENODEV;
		goto err_out;
	}
...
static void s2mps27_keys_set_input_device(struct platform_device *pdev,
		struct s2p_keys_platform_data *pdata, struct input_dev *input)
{
	input->name		= pdata->name ? : pdev->name;
	input->phys 		= "s2mps27-keys/input0";
...
static int s2mps27_keys_set_interrupt(struct s2p_keys_drvdata *ddata, int irq_base)
{
	...
	for (i = 0; i < ddata->pdata->nbuttons; i++) {
		...
		idx = i * 2;
		if (i == POWER_KEY_IDX) {
			bdata->irq_key_r = irq_base + S2MPS27_PMIC_IRQ_PWRONR_INT1;
			bdata->irq_key_p = irq_base + S2MPS27_PMIC_IRQ_PWRONP_INT1;
		} else if (i == VOLDN_KEY_IDX) {
			bdata->irq_key_r = irq_base + S2MPS27_PMIC_IRQ_VOLDNR_INT3;
			bdata->irq_key_p = irq_base + S2MPS27_PMIC_IRQ_VOLDNP_INT3;
		} else {
		...
		ret = devm_request_threaded_irq(dev, bdata->irq_key_r, NULL,
						s2mps27_keys_release_irq_handler, 0,
						irq_name[idx], ddata);
		if (ret < 0) {
		...
		ret = devm_request_threaded_irq(dev, bdata->irq_key_p, NULL,
						s2mps27_keys_press_irq_handler, 0,
...
static int s2mps27_keys_set_buttondata(struct s2p_keys_drvdata *ddata,
				     struct input_dev *input, bool *wakeup)
{
	...
	for (cnt = 0; cnt < ddata->pdata->nbuttons; cnt++) {
		...
		snprintf(device_name, sizeof(device_name) - 1,
			"s2mps27-keys-wq%d@%s", cnt, dev_name(ddata->dev));
		...
		INIT_DELAYED_WORK(&bdata->key_work, s2mps27_keys_work_func);
...
static int s2mps27_keys_probe(struct platform_device *pdev)
{
	...
	pdata = of_s2mps27_keys_parse_dt(iodev);
	if (IS_ERR(pdata))
	...
	s2mps27_keys_set_input_device(pdev, pdata, input);
	...
	ret = s2mps27_keys_set_buttondata(ddata, input, &wakeup);
	if (ret < 0) {
		pr_err("%s: s2mps27_keys_set_buttondata fail\n", __func__);
		return ret;
	}
	...
	ret = s2mps27_keys_set_interrupt(ddata, iodev->sdev->irq_base);
	if (ret < 0) {
		pr_err("%s: s2mps27_keys_set_interrupt fail\n", __func__);
		goto fail;
	}
...
static int s2mps27_keys_remove(struct platform_device *pdev)
{
...
static int s2mps27_keys_suspend(struct device *dev)
{
...
static int s2mps27_keys_resume(struct device *dev)
{
	struct s2p_keys_drvdata *ddata = dev_get_drvdata(dev);
...
static struct of_device_id s2mps27_keys_of_match[] = {
	{ .compatible = "s2mps27-power-keys", },
	{ },
};
MODULE_DEVICE_TABLE(of, s2mps27_keys_of_match);

static SIMPLE_DEV_PM_OPS(s2mps27_keys_pm_ops, s2mps27_keys_suspend, s2mps27_keys_resume);

static struct platform_driver s2mps27_keys_device_driver = {
	.probe		= s2mps27_keys_probe,
	.remove		= s2mps27_keys_remove,
	.driver		= {
		.name	= "s2mps27-power-keys",
		.owner	= THIS_MODULE,
		.pm	= &s2mps27_keys_pm_ops,
		.of_match_table = of_match_ptr(s2mps27_keys_of_match),
	}
};

module_platform_driver(s2mps27_keys_device_driver);

MODULE_LICENSE("GPL");
MODULE_AUTHOR("Samsung Electronics");
MODULE_DESCRIPTION("Keyboard driver for s2mps27");
MODULE_ALIAS("platform:s2mps27 Power/Vol-down key");
