/*
 * Driver for Power key/Vol Down key on s2mpu15 IC by PWRON,Vol-down rising, falling interrupts.
 *
 * drivers/input/keyboard/s2mpu15_key.c
...
of_s2mpu15_keys_parse_dt(struct s2mpu15_dev *iodev)
{
...
static void s2mpu15_keys_set_input_device(struct platform_device *pdev,
...
static int s2mpu15_keys_set_interrupt(struct s2p_keys_drvdata *ddata, int irq_base)
{
...
static int s2mpu15_keys_set_buttondata(struct s2p_keys_drvdata *ddata,
...
static int s2mpu15_keys_probe(struct platform_device *pdev)
{
	...
	pdata = of_s2mpu15_keys_parse_dt(iodev);
	if (IS_ERR(pdata))
	...
	s2mpu15_keys_set_input_device(pdev, pdata, input);
	...
	ret = s2mpu15_keys_set_interrupt(ddata, iodev->sdev->irq_base);
	if (ret < 0) {
