...
static void dsim_reg_set_md_ana_con(u32 id, const u32 *blk_ctl)
{
	...
	dsim_phy_write(id, DSIM_PHY_MD_ANA_CON3(3), 0x0);
}
...
static void dsim_reg_set_ext_vt_sync(u32 id, struct dsim_reg_config *config)
{
	...
	mask = DSIM_OPTION_SUITE_OPT_EXT_VT_SYNC_MASK;
						...
						struct dsim_clks *clks)
{
	...
	dsim_reg_enable_shadow_vss_update(id, 0);
		...
		struct dsim_clks *clks)
{
	...
	dsim_reg_enable_shadow_vss_update(id, 1);
	...
	if (!is_cmd) {
		...
		dsim_write(id, DSIM_VIDEO_TIMER, 0);
		dsim_reg_set_cmd_ctrl(id, config, clks);
	}
}
