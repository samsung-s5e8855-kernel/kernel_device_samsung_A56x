...
void dp_hex_dump(void __iomem *base_addr, int size)
{
	...
	num_word = size / 4;
	for (i = 0; i < height; i++) {
		cal_log_info(0, "[0x%p] ", base_addr + i * 32);
		for (j = 0; j < 8; j++) {
			if (num_word <= 0) break;
...
int dp_hsi0_domain_power_check(void)
{
	...
	if (pwr_regs == NULL) {
		pwr_regs = ioremap((phys_addr_t)0x12860000 + 0x2000, 0x24);
		if (!pwr_regs) {
	...
	pr_info("HSI0_CONFIGURATION[0x1286_2000](0x%08x)\n", readl(pwr_regs));
	pr_info("HSI0_STATUS	   [0x1286_2004](0x%08x)\n", readl(pwr_regs + 0x4));
	pr_info("HSI0_STATES       [0x1286_2008](0x%08x)\n", readl(pwr_regs + 0x8));
	if (readl(pwr_regs + 0x8) == 0)
		ret = 0;
	pr_info("HSI0_CTRL         [0x1286_2010](0x%08x)\n", readl(pwr_regs + 0x10));
	pr_info("HSI0_OUT          [0x1286_2020](0x%08x)\n", readl(pwr_regs + 0x20));

	return ret;
}

void dp_print_hsi0_domain_power(void)
{
	if (pwr_regs) {
		pr_info("HSI0_CONFIGURATION[0x1286_2000](0x%08x)\n", readl(pwr_regs));
		pr_info("HSI0_STATUS	   [0x1286_2004](0x%08x)\n", readl(pwr_regs + 0x4));
		...
		pr_info("HSI0_PWR SFR is not remapped\n");
}
...
{
	if (clk_regs1 == NULL) {
		clk_regs1 = ioremap((phys_addr_t)0x17800000 + 0x0620, 0x10);
		if (!clk_regs1) {
	...
	pr_info("PLL_CON0_MUX_CLKCMU_HSI0_DPOSC_USER[0x1780_0620](0x%08x)\n", readl(clk_regs1));
	pr_info("PLL_CON1_MUX_CLKCMU_HSI0_DPOSC_USER[0x1780_0624](0x%08x)\n", readl(clk_regs1 + 0x4));

	if (clk_regs2 == NULL) {
		clk_regs2 = ioremap((phys_addr_t)0x22b20000 + 0x10c0, 0x10);
		if (!clk_regs2) {
			pr_err("HSI0_CLK SFR ioremap is failed\n");
			return;
		}
	...
	pr_info("CLK_CON_MUX_CLKCMU_HSI0_DPOSC[0x22b2_10c0](0x%08x)\n", readl(clk_regs2));

	if (clk_regs3 == NULL) {
		clk_regs3 = ioremap((phys_addr_t)0x22b20000 + 0x18a0, 0x10);
		if (!clk_regs3) {
			pr_err("HSI0_CLK SFR ioremap is failed\n");
			return;
		}
	...
	pr_info("CLK_CON_DIV_CLKCMU_HSI0_DPOSC[0x22b2_18a0](0x%08x)\n", readl(clk_regs3));
}

void dp_print_hsi0_clk(void)
{
	if (clk_regs1) {
		pr_info("PLL_CON0_MUX_CLKCMU_HSI0_DPOSC_USER[0x1780_0620](0x%08x)\n", readl(clk_regs1));
		pr_info("PLL_CON1_MUX_CLKCMU_HSI0_DPOSC_USER[0x1780_0624](0x%08x)\n", readl(clk_regs1 + 0x4));
	} else
		pr_info("PLL_CON0_MUX_CLKCMU_HSI0_DPOSC_USER[0x1780_0620] is not remapped\n");

	if (clk_regs2)
		pr_info("CLK_CON_MUX_CLKCMU_HSI0_DPOSC[0x22b2_10c0](0x%08x)\n", readl(clk_regs2));
	else
		pr_info("CLK_CON_MUX_CLKCMU_HSI0_DPOSC[0x22b2_10c0] is not remapped\n");

	if (clk_regs3)
		pr_info("CLK_CON_DIV_CLKCMU_HSI0_DPOSC[0x22b2_18a0](0x%08x)\n", readl(clk_regs3));
	else
		pr_info("CLK_CON_DIV_CLKCMU_HSI0_DPOSC[0x22b2_18a0] is not remapped\n");
}
...
static u32 phy_eq_hbr2_3[4][4][5] = {
	/* {eq main, eq post, eq pre, eq vswing lvl, rboost en} */
	{	/* Swing Level_0 */
		{ 25, 0, 0 , 5, 3 },/* Pre-emphasis Level_0 */
		{ 31, 0, 6 , 5, 3 },/* Pre-emphasis Level_1 */
		{ 37, 0, 12, 5, 3 },/* Pre-emphasis Level_2 */
		{ 43, 0, 19, 5, 3 },/* Pre-emphasis Level_3 */
	},
	{	/* Swing Level_1 */
		{ 37, 0, 0 , 5, 3 },
		{ 46, 0, 9 , 5, 3 },
...
void dp_reg_sw_reset(struct dp_cal_res *cal_res)
{
	...
	cal_log_debug(0, "SYSTEM_SW_RESET_CONTROL(-)[0x%p]:0x%08x\n",
			(void __iomem *)0x17880000 + SYSTEM_SW_RESET_CONTROL,
			dp_read(SYSTEM_SW_RESET_CONTROL));

	cal_log_debug(0, "SYSTEM_CLK_CONTROL[0x%p]:0x%08x\n",
			(void __iomem *)0x17880000 + SYSTEM_CLK_CONTROL,
...
int dp_reg_usbdrd_qch_en(u32 en)
{
	...
	cmu_regs = ioremap((phys_addr_t)0x17800000 + 0x3054, 0x8);
	if (!cmu_regs) {
	...
	if (en) {
		...
		pr_info("QCH_CON_USB32DRD_QCH_S_CTRL[0x17803054](0x%08x)\n", readl(cmu_regs));
		writel(0x6, cmu_regs);
		pr_info("QCH_CON_USB32DRD_QCH_S_CTRL[0x17803054](0x%08x)\n", readl(cmu_regs));
	} else {
...
void dp_reg_phy_pll_disable(u32 disable)
{
...
#if IS_ENABLED(CONFIG_USB_DWC3_EXYNOS_MODULE)
		dwc3_exynos_otg_inform_dp_use(1, 4);
#endif
		tcpc_mux_con = 0x2;
...
#if IS_ENABLED(CONFIG_USB_DWC3_EXYNOS_MODULE)
		dwc3_exynos_otg_inform_dp_use(1, 2);
#endif
		tcpc_mux_con = 0x3;
		if (cal_res->dp_sw_sel) {
			tcpc_orientation = 0;
 ...
 */
static void dp_reg_phy_enable_cp_current_boost(void)
{
	dp_reg_cr_write_mask(0x5c, 0x400, 0x700);
	dp_reg_cr_write_mask(0x5c, 0x4, 0x7);
}
	...
	for (i = EQ_MAIN; i <= RBOOST; i++) {
		...
		if (link_rate <=  LINK_RATE_2_7Gbps)
			...
			if (exynos_soc_info.main_rev == 0)
				val = phy_eq_hbr2_3_evt0[amp_lvl][pre_emp_lvl][i];
			else
...
u64 dp_reg_get_video_clk(void)
{
	...
	return (u64)dp->cur_mode.clock * 1000;
#else
	struct dp_device *dp = get_dp_drvdata();
...
int dp_reg_edid_read(struct dp_cal_res *cal_res, u8 block_cnt, u32 length, u8 *data)
{
	...
	while (retry_cnt > 0) {
		...
		dp_reg_set_aux_ch_operation_enable();
...
void dp_reg_phy_post_init(struct dp_cal_res *cal_res)
{
	...
	case CCIC_NOTIFY_DP_PIN_E:
		...
		datapath_control_val[0] = 0x00000f00;
	...
	case CCIC_NOTIFY_DP_PIN_F:
		...
		pstate_mask = pstate_val =
			DP_TX2_PSTATE | DP_TX3_PSTATE;
		datapath_control_val[0] = 0x00000300;
...
void dp_reg_phy_init(struct dp_cal_res *cal_res)
{
	...
	dp_reg_phy_enable_cp_current_boost();
...
#if IS_ENABLED(CONFIG_USB_DWC3_EXYNOS_MODULE)
	dwc3_exynos_otg_inform_dp_use(0, dp_reg_get_lane_count());
	dwc3_exynos_phy_enable(1, 0);
	atomic_set(&cal_res->usbdp_phy_en_cnt, 0);
#endif
}

void dp_reg_set_fec_ready(u32 en)
{
	u32 val = en ? ~0 : 0;

	dp_write_mask(PCS_CONTROL, val, FEC_READY);
}

void dp_reg_set_fec_func_en(u32 en)
{
	u32 val = en ? ~0 : 0;

	dp_write_mask(PCS_CONTROL, val, FEC_FUNC_EN);
}
		...
		u8 bpc, u8 range)
{
	...
	dp_write_mask(SST1_VIDEO_CONTROL, bpc, BPC);	/* 0x0 : 6bits, 0x1 : 8bits, 0x2 : 10bits */
...
#endif
	dp_reg_dsc_enable(cal_res->dsc_en, cal_res->slice_count, cal_res->chunk_size);
...
void dp_reg_set_hdr_infoframe(struct hdr_infoframe hdr_infoframe, u32 en)
{
	int i, j;
	u32 hdr_infoframe_data = 0;
	u8 *data = (u8*)&hdr_infoframe;
...
void dp_reg_set_pps_sdp(u32 *sdp_payload)
{
	int i;

	for (i = 0; i < 32; i++)
		dp_write(SST1_PPS_SDP_PAYLOAD_0 + (i * 4), sdp_payload[i]);
	cal_log_info(0, "DEBUG FOR ====== SDP for PPS ====== \n");
	dp_print_hex_dump(regs_dp[REGS_LINK][SST1].regs,
			regs_dp[REGS_LINK][SST1].regs + SST1_PPS_SDP_PAYLOAD_0,
			SST1_VSC_SDP_DATA_PAYLOAD_FIFO - SST1_PPS_SDP_PAYLOAD_0);
	dp_write_mask(SST1_PPS_SDP_CONTROL, 1, PPS_SDP_UPDATE);
	dp_write_mask(SST1_PPS_SDP_CONTROL, 1, PPS_SDP_FRAME_SEND_ENABLE);
}

void dp_reg_dsc_enable(u32 en, u32 slice_count, u32 chunk_size) {
	int i;
	u32 mask;

	dp_write_mask(SST1_VIDEO_DSC_STREAM_CONTROL_0, slice_count, SLICE_COUNT_PER_LINE);
	for (i = 0; i < slice_count; i++) {
		mask = (i & 1) ? CHUNK_SIZE_1 : CHUNK_SIZE_0;
		dp_write_mask(SST1_VIDEO_DSC_STREAM_CONTROL_1 + (i / 2) * 4, chunk_size, mask);
	}
	dp_write_mask(SST1_VIDEO_DSC_STREAM_CONTROL_0, en, DSC_ENABLE);
}
