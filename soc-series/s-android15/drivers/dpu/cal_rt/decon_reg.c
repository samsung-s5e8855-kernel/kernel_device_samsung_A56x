...
void decon_reg_set_scaler_enable(u32 id, bool en)
{
	...
	val = en ? POST_SCALER_ON : 0;
	decon_write_mask(id, DATA_PATH_CON, val, POST_SCALER_ON);
}
		...
		u32 swrst)
{
	...
	decon_reg_config_data_path_size(id, dsc_enc.width_per_enc,
			...
			overlap_w, &dsc_enc, &config->dsc, config->out_bpc);
...
static void decon_reg_set_vmc_mode(u32 id, struct decon_vmc_mode *vmc_mode)
{
	u32 val, mask;
...
int decon_reg_init(u32 id, struct decon_config *config)
{
	...
	decon_reg_set_vmc_mode(id, &config->mode.vmc_mode);
		...
		bool in_slice[], u32 partial_w, u32 partial_h)
{
	...
	decon_reg_set_data_path_size(id, partial_w,
			config->pscaler ? config->desired_height : partial_h,
...
void __decon_dump(u32 id, struct decon_regs *regs, bool dsc_en)
{
	...
	dpu_print_hex_dump(main_regs, main_regs + 0x0000, 0x364);
