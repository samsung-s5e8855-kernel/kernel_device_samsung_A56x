...
#define IS_ARRAY(arg) ((((void *) &arg) == ((void *) arg)) && IS_INDEXABLE(arg))
...
static inline bool IS_RCD_REQUIRED(struct exynos_dqe *dqe)
{
	return (dqe->cfg.rcd_en > 0);
}

static inline bool IS_SCL_REQUIRED(struct exynos_dqe *dqe)
{
	return (dqe->cfg.in.width != dqe->cfg.out.width ||
		dqe->cfg.in.height != dqe->cfg.out.height);
}
...
static u32 dqe_gen_crc_key(struct exynos_dqe *dqe, u8 id, const u8 attr[])
{
	u32 attr0 = attr[0];
	u32 attr1 = attr[1];
	...
	case DQE_COLORMODE_ID_GAMMA:
		attr0 = (attr0 == 8 || attr0 == 1) ? 8 : 10;
		break;
	default:
		break;
	}

	return (attr0 << 16) | (attr1 << 8) | id;
}
...
static bool dqe_check_crc(struct exynos_dqe *dqe, u8 id, const u8 attr[], u16 value)
{
	...
	u32 key = dqe_gen_crc_key(dqe, id, attr);
...
static struct dqe_ctx_int *dqe_get_lut(struct exynos_dqe *dqe, u32 type)
{
	if (!dqe || !dqe->lut || type >= dqe->num_lut)
		return NULL;
	return &(((struct dqe_ctx_int *)dqe->lut)[type]);
}
...
static int dqe_restore_context(struct exynos_dqe *dqe)
{
	...
	if (!dqe->cfg.cur_bpc || !dqe->cfg.in.width || !dqe->cfg.in.height) // no received bpc info yet
		return -1;
	...
	dqe_reg_init(dqe->id, &dqe->cfg.in, &dqe->cfg.out);
	...
	/* enable rcd if required */
	decon_reg_set_rcd_enable(dqe->decon->id, IS_RCD_REQUIRED(dqe));
	/* enable scl if required */
	decon_reg_set_scaler_enable(dqe->decon->id, IS_SCL_REQUIRED(dqe));
	...
	if (IS_DQE_ON(&dqe->ctx.atc_on)) {
		dqe_reg_set_atc_img_size(id, dqe->cfg.in.width, dqe->cfg.in.height);
	...
	snprintf(log_msg, sizeof(log_msg),
		"DI[%s,%s] LM[%s] GM[%s] DG[%s] CGC%s[%s] RG[%s] HSC[%s] ATC[%s] SCL[%s] DE[%s] RCD[%s]",
		...
		dqe_print_onoff(&dqe->ctx.rcd_on));

	dqe_debug(dqe, "update-%d,%d/%d "KTIME_MSEC_FMT" %s\n",
		atomic_read(&dqe->update_cnt),
		dqe->cfg.in_bpc, dqe->cfg.cur_bpc,
		KTIME_MSEC_ARG(time_c), log_msg);

	DPU_EVENT_LOG("DQE_RESTORE", dqe->decon->crtc, 0, "%s", log_msg);
					...
					struct drm_crtc_state *crtc_state)
{
	...
	iosys_map_clear(&map);
	dqe_fd = exynos_crtc_state->color_fd_slot0;
	if (dqe_fd <= 0) {
	...
	if (dqe->colormode.dma_buf) {
		if (dqe->colormode.vaddr) {
			...
			dma_buf_vunmap_unlocked(dqe->colormode.dma_buf, &old_map);
		}
				...
				struct drm_crtc_state *crtc_state)
{
	...
	while (size < hdr->total_size) {
		...
		ctx_i = dqe_get_lut(dqe, DQE_MODE_MAIN);
		switch (data_h->id) {
		...
		case DQE_COLORMODE_ID_CGC17_CON:
			lut = ctx_i->cgc17_con;
			len = ARRAY_SIZE_SAFE(ctx_i->cgc17_con);
			break;
		case DQE_COLORMODE_ID_DEGAMMA:
			if (data_h->attr[0] == 8 || data_h->attr[0] == 1) {
			...
			len = ARRAY_SIZE_SAFE(ctx_i->degamma_lut[0]);
			break;
		...
		case DQE_COLORMODE_ID_SCL:
			lut = ctx_i->scl_input;
			len = ARRAY_SIZE_SAFE(ctx_i->scl_input);
			break;
		case DQE_COLORMODE_ID_DE:
			lut = ctx_i->de_lut;
			len = ARRAY_SIZE_SAFE(ctx_i->de_lut);
			break;
		case DQE_COLORMODE_ID_ATC:
			lut = ctx_i->atc_lut;
			len = ARRAY_SIZE(ctx_i->atc_lut);
			break;
		default:
			lut = NULL;
	...
	if (likely(updated)) {
		...
		DPU_EVENT_LOG("DQE_COLORMODE", dqe->decon->crtc, 0, "UPDATED[%x]", updated);
					...
					struct drm_crtc_state *crtc_state)
{
	...
	for (mode_type = DQE_MODE_PRESET; mode_type < dqe->num_lut; mode_type++) {
		attr = dqe_get_lut(dqe, mode_type)->mode_attr;
				...
				struct drm_crtc_state *crtc_state)
{
	...
	mutex_lock(&dqe->lock);
	ctx_from = dqe_get_lut(dqe, mode_type);
	ctx_to = dqe_get_lut(dqe, DQE_MODE_MAIN);
	...
	if (unlikely(updated)) {
		dqe_debug(dqe, "context updated (%x)\n", updated);
		DPU_EVENT_LOG("DQE_PRESET", dqe->decon->crtc, 0, "UPDATED[%x]", updated);
		updated = DQE_UPDATED_CONTEXT;
	}
	return updated;
}
				...
				struct drm_crtc_state *crtc_state)
{
	...
	if (decon->crtc->partial && exynos_crtc_state->partial) {
		in.width = drm_rect_width(&exynos_crtc_state->partial_region);
		in.height = drm_rect_height(&exynos_crtc_state->partial_region);
	} else {
		in.width = crtc_state->adjusted_mode.hdisplay;
		in.height = crtc_state->adjusted_mode.vdisplay;
	}

	if (exynos_crtc_state->pscaler.type == EXYNOS_SCALER_DQE) {
		out.width = exynos_crtc_state->pscaler.hdisplay;
		out.height = exynos_crtc_state->pscaler.vdisplay;
	} else {
		out.width = in.width;
		out.height = in.height;
	}

	if (unlikely(dqe->cfg.in.width != in.width) ||
		unlikely(dqe->cfg.in.height != in.height) ||
		unlikely(dqe->cfg.out.width != out.width) ||
		unlikely(dqe->cfg.out.height != out.height) ||
		unlikely(dqe->cfg.in_bpc != cfg->in_bpc) ||
		unlikely(dqe->cfg.out_bpc != cfg->out_bpc) ||
		unlikely(dqe->cfg.rcd_en != cfg->rcd_en)) {

		dqe->cfg.in.width = in.width;
		dqe->cfg.in.height = in.height;
		dqe->cfg.out.width = out.width;
		dqe->cfg.out.height = out.height;

		dqe->cfg.in_bpc = cfg->in_bpc;
		dqe->cfg.cur_bpc = IS_DQE_VER_RANGE(dqe, DQE_VER_500, DQE_VER_610) ?
					dqe->cfg.in_bpc : 10;
		if (dqe->decon->config.terminal_unit == DECON_TERMINAL_UNIT_DQE)
			dqe->cfg.out_bpc = cfg->out_bpc;
		else
			dqe->cfg.out_bpc = cfg->in_bpc;
		dqe->cfg.rcd_en = cfg->rcd_en;

		snprintf(log_msg, sizeof(log_msg),
			"RES[%ux%u to %ux%u] BPC[%u-%u-%u] RCD[%u]",
			dqe->cfg.in.width, dqe->cfg.in.height,
			dqe->cfg.out.width, dqe->cfg.out.height,
			dqe->cfg.in_bpc, dqe->cfg.cur_bpc, dqe->cfg.out_bpc,
			dqe->cfg.rcd_en);
		dqe_debug(dqe, "%s\n", log_msg);
		DPU_EVENT_LOG("DQE_CONFIG", dqe->decon->crtc, 0, "%s", log_msg);
...
static int dqe_parse_preset(struct exynos_dqe *dqe, struct device *dev)
{
	...
	dqe->lut = kcalloc(dqe->num_lut, sizeof(struct dqe_ctx_int), GFP_KERNEL);
	if (!dqe->lut)
		goto err;

	for (i = DQE_MODE_PRESET; i < dqe->num_lut; i++) {
		attr = dqe_get_lut(dqe, i)->mode_attr;
...
static int dqe_init_resources(struct exynos_dqe *dqe, struct device *dev)
{
	...
	if (!dqe_cls)
		dqe_cls = class_create(name);
	if (IS_ERR_OR_NULL(dqe_cls)) {
		dqe_err(dqe, "failed to create dqe class\n");
		goto err;
	}

	if (dqe->id > 0) // uses dqe if id = 0 for backward compatible
		sprintf(name, "dqe%d", dqe->id);
	dqe->dev = device_create(dqe_cls, NULL, 0, &dqe, name);
...
static int dqe_dec_cgc17(struct dqe_ctx_int *lut, int mask, int wr_mode)
{
	...
	int decoded[17] = {0,};
	int row = ARRAY_SIZE_SAFE(lut->cgc17_encoded[0]);
	int col = ARRAY_SIZE_SAFE(lut->cgc17_encoded[0][0]);
	int size = ARRAY_SIZE_SAFE(decoded);
				...
				struct dqe_ctx_int *lut, bool clrForced)
{
	...
	if (!IS_ARRAY(lut->scl_input) || ARRAY_SIZE_SAFE(lut->scl_input) == 1)
		return;
...
static void dqe_set_all_luts(struct exynos_dqe *dqe)
{
	...
	mutex_lock(&dqe->lock);
	lut = dqe_get_lut(dqe, DQE_MODE_MAIN);
...
#define DQE_CREATE_SYSFS_FUNC(name) \
	...
	{ \
		...
		if (dqe == NULL || buf == NULL || dqe->lut == NULL) \
			return -1;\
		mutex_lock(&dqe->lock); \
		dqe_debug(dqe, "+\n"); \
		ret = dqe_ ## name ## _show(dqe, dqe_get_lut(dqe, dqe->mode_idx), buf); \
		if (ret < 0) \
			dqe_err(dqe, "err -\n"); \
		else \
			dqe_debug(dqe, "-\n"); \
	...
	{ \
		int ret = 0; \
		struct exynos_dqe *dqe = dev_get_drvdata(dev); \
		if (dqe == NULL || count <= 0 || buffer == NULL || dqe->lut == NULL) \
			return -1; \
		mutex_lock(&dqe->lock); \
		dqe_debug(dqe, "+\n"); \
		ret = dqe_ ## name ## _store(dqe, dqe_get_lut(dqe, dqe->mode_idx), buffer, count); \
		if (ret < 0) \
			dqe_err(dqe, "err -\n"); \
		else \
			dqe_debug(dqe, "-\n"); \
...
static ssize_t dqe_conv_cgc2str(struct dqe_ctx_int *lut, char *out)
{
	...
	int row = ARRAY_SIZE_SAFE(lut->cgc17_encoded[0][0]);
	int col = ARRAY_SIZE_SAFE(lut->cgc17_encoded[0][0][0]);
					...
					struct dqe_ctx_int *lut, char *buf)
{
	...
	return dqe_conv_lut2str(lut->disp_dither, ARRAY_SIZE_SAFE(lut->disp_dither), buf);
}
			...
			struct dqe_ctx_int *lut, const char *buffer, size_t count)
{
	...
	ret = dqe_conv_str2lut(buffer, lut->disp_dither, ARRAY_SIZE_SAFE(lut->disp_dither), true);
	if (ret < 0)
		return ret;
	if (dqe->mode_idx == DQE_MODE_MAIN) // do not update dqe_ctx_reg for preset LUTs
					...
					struct dqe_ctx_int *lut, char *buf)
{
	...
	return dqe_conv_lut2str(lut->cgc_dither, ARRAY_SIZE_SAFE(lut->cgc_dither), buf);
}
			...
			struct dqe_ctx_int *lut, const char *buffer, size_t count)
{
	...
	ret = dqe_conv_str2lut(buffer, lut->cgc_dither, ARRAY_SIZE_SAFE(lut->cgc_dither), true);
	if (ret < 0)
		return ret;
	if (dqe->mode_idx == DQE_MODE_MAIN) { // do not update dqe_ctx_reg for preset LUTs
				...
				struct dqe_ctx_int *lut, char *buf)
{
	...
	return dqe_conv_lut2str(lut->cgc17_con, ARRAY_SIZE_SAFE(lut->cgc17_con), buf);
}
			...
			struct dqe_ctx_int *lut, const char *buffer, size_t count)
{
	...
	ret = dqe_conv_str2lut(buffer, lut->cgc17_con, ARRAY_SIZE_SAFE(lut->cgc17_con), true);
	if (ret < 0)
		return ret;
	if (dqe->mode_idx == DQE_MODE_MAIN) { // do not update dqe_ctx_reg for preset LUTs
				...
				struct dqe_ctx_int *lut, char *buf)
{
	...
	return dqe_conv_lut2str(lut->regamma_lut[bpc], ARRAY_SIZE_SAFE(lut->regamma_lut[0]), buf);
}
			...
			struct dqe_ctx_int *lut, const char *buffer, size_t count)
{
	...
	ret = dqe_conv_str2lut(buffer, lut->regamma_lut[bpc], ARRAY_SIZE_SAFE(lut->regamma_lut[0]), false);
	if (ret < 0)
		return ret;
	if (dqe->mode_idx == DQE_MODE_MAIN) { // do not update dqe_ctx_reg for preset LUTs
				...
				struct dqe_ctx_int *lut, char *buf)
{
	...
	return dqe_conv_lut2str(lut->degamma_lut[bpc], ARRAY_SIZE_SAFE(lut->degamma_lut[0]), buf);
}
			...
			struct dqe_ctx_int *lut, const char *buffer, size_t count)
{
	...
	ret = dqe_conv_str2lut(buffer, lut->degamma_lut[bpc], ARRAY_SIZE_SAFE(lut->degamma_lut[0]), false);
	if (ret < 0)
		return ret;
	if (dqe->mode_idx == DQE_MODE_MAIN) { // do not update dqe_ctx_reg for preset LUTs
				...
				struct dqe_ctx_int *lut, char *buf)
{
	...
	return dqe_conv_lut2str(lut->hsc48_lcg[idx], ARRAY_SIZE_SAFE(lut->hsc48_lcg[0]), buf);
}
			...
			struct dqe_ctx_int *lut, const char *buffer, size_t count)
{
	...
	ret = dqe_conv_str2lut(buffer, lut->hsc48_lcg[idx], ARRAY_SIZE_SAFE(lut->hsc48_lcg[0]), true);
	if (ret < 0)
		return ret;
	return count;
}
			...
			struct dqe_ctx_int *lut, char *buf)
{
	...
	return dqe_conv_lut2str(lut->atc_lut, ARRAY_SIZE_SAFE(lut->atc_lut), buf);
}
			...
			struct dqe_ctx_int *lut, const char *buffer, size_t count)
{
	...
	ret = dqe_conv_str2lut(buffer, lut->atc_lut, ARRAY_SIZE_SAFE(lut->atc_lut), false);
	if (ret < 0)
		return ret;
	if (dqe->mode_idx == DQE_MODE_MAIN) // do not update dqe_ctx_reg for preset LUTs
			...
			struct dqe_ctx_int *lut, char *buf)
{
	...
	return dqe_conv_lut2str(lut->scl_input, ARRAY_SIZE_SAFE(lut->scl_input), buf);
}
			...
			struct dqe_ctx_int *lut, const char *buffer, size_t count)
{
	...
	ret = dqe_conv_str2lut(buffer, lut->scl_input, ARRAY_SIZE_SAFE(lut->scl_input), true);
	if (ret < 0)
		return ret;
	if (dqe->mode_idx == DQE_MODE_MAIN) { // do not update dqe_ctx_reg for preset LUTs
			...
			struct dqe_ctx_int *lut, char *buf)
{
	...
	return dqe_conv_lut2str(lut->de_lut, ARRAY_SIZE_SAFE(lut->de_lut), buf);
}
			...
			struct dqe_ctx_int *lut, const char *buffer, size_t count)
{
	...
	ret = dqe_conv_str2lut(buffer, lut->de_lut, ARRAY_SIZE_SAFE(lut->de_lut), false);
	if (ret < 0)
		return ret;
	if (dqe->mode_idx == DQE_MODE_MAIN) { // do not update dqe_ctx_reg for preset LUTs
			...
			struct dqe_ctx_int *lut, char *buf)
{
	...
	return dqe_conv_lut2str(lut->rcd_lut, ARRAY_SIZE_SAFE(lut->rcd_lut), buf);
}
			...
			struct dqe_ctx_int *lut, const char *buffer, size_t count)
{
	...
	ret = dqe_conv_str2lut(buffer, lut->rcd_lut, ARRAY_SIZE_SAFE(lut->rcd_lut), false);
	if (ret < 0)
		return ret;
	if (dqe->mode_idx == DQE_MODE_MAIN) { // do not update dqe_ctx_reg for preset LUTs
...
struct exynos_dqe *exynos_dqe_register(struct decon_device *decon)
{
	...
	if (decon->id > 1 || !(decon->config.out_type & DECON_OUT_DSI)) {
