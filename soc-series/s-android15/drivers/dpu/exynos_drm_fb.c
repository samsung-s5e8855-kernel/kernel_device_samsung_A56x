...
VISIBLE_IF_KUNIT void exynos_drm_fb_set_offsets(struct drm_framebuffer *fb)
{
...
EXPORT_SYMBOL_IF_KUNIT(exynos_drm_fb_set_offsets);
...
exynos_get_format_info(const struct drm_mode_fb_cmd2 *cmd)
{
	...
	} else if (has_all_bits(DRM_FORMAT_MOD_SAMSUNG_SAJC(0, 0, 0), cmd->modifier[0])) {
		int i;
...
EXPORT_SYMBOL_IF_KUNIT(exynos_get_format_info);
...
EXPORT_SYMBOL_IF_KUNIT(exynos_drm_fb_dma_addr);
