				     ...
				     uint64_t modifier)
{
	...
	if (has_all_bits(DRM_FORMAT_MOD_SAMSUNG_SAJC(0, 0, 0), modifier))
				     ...
				     struct drm_atomic_state *state)
{
	...
	if (!new_crtc_state->planes_changed || !new_crtc_state->active ||
			new_exynos_crtc_state->skip_frameupdate) {
				       ...
				       struct drm_atomic_state *old_state)
{
	...
	new_exynos_crtc_state =
		exynos_drm_atomic_get_new_crtc_state(old_state, exynos_crtc);
							 ...
							 const struct dpp_restrict *res)
{
	...
	dpu_res_add_u32(&ptr, &size, DPU_RES_COMP_TRASFORM, &res->comp_transform_support);
