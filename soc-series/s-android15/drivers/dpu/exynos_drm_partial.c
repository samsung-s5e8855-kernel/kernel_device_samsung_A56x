...
static bool exynos_partial_init(struct exynos_partial *partial,
			...
			const struct exynos_display_mode *exynos_mode)
{
	...
	if (exynos_mode->vhm) {
		partial_debug(partial, "cannot support partial update for vhm\n");
		return false;
	}
...
// 		partial_debug("Partial update was enabled, but it will be disabled due to te_shfit of svsync.\n");
// 		return false;
// 	}
// #endif
		...
		const struct drm_rect *partial_r)
{
	struct decon_device *decon = partial->exynos_crtc->ctx;
	struct dsim_device *dsim;
	struct decon_config decon_config;
	...
	memcpy(&decon_config, &decon->config, sizeof(struct decon_config));
	decon_config.image_width = partial_w;
	decon_config.image_height = partial_h;
	decon_config.dsc_cfg.pic_height = partial_h;
	decon_reg_set_mres(decon->id, &decon_config);
                               ...
                               const struct drm_crtc_state *new_crtc_state)
{
	...
	const struct exynos_display_mode *exynos_mode = &new_exynos_crtc_state->exynos_mode;

	partial->enabled = exynos_partial_init(partial, mode, exynos_mode);
	if (!partial->enabled)
		return;

	partial_debug(partial, "INIT: min rect(%dx%d) en(%d)\n",
			partial->min_w, partial->min_h, partial->enabled);
	DPU_EVENT_LOG("PARTIAL_INIT", partial->exynos_crtc, 0,
		"minimum rect size[%dx%d] en(%d)",
		partial->min_w, partial->min_h, partial->enabled);
}
...
EXPORT_SYMBOL_IF_KUNIT(exynos_partial_is_full);
				  ...
				  struct drm_crtc_state *new_crtc_state)
{
	...
	if (IS_PSCALER_ENABLED(new_exynos_crtc_state->pscaler.type))
		return false;
