...
int convert_pps_to_dsc_config(struct drm_dsc_config *cfg, const u8 *table)
{
	...
	cfg->rc_edge_factor = (table[40] << 1);
	/* PPS 41 */
	cfg->rc_quant_incr_limit0 = table[41];
	/* PPS 42 */
	cfg->rc_quant_incr_limit1 = table[42];
	/* PPS 43 */
	...
	for (i = 0; i < DSC_NUM_BUF_RANGES - 1; ++i)
		cfg->rc_buf_thresh[i] = (table[44 + i]);
	/* PPS 58 ~ 87 */
	...
	exynos_drm_dsc_print_pps_info(cfg, table);
#endif
