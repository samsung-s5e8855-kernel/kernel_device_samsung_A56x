...
static bool hdr_reg_verify_v2p4(u32 id, u32 start_offset, u32 length)
{
	switch (start_offset) {
	case HDR_HDR_CON:
		if (length != 1)
...
bool hdr_reg_verify(u32 id, u32 hw_type, u32 start_offset, u32 length)
{
	...
	if (hw_type == HDR_HW_TYPE_BASE) {	// S.LSI
		if (IS_HDR_LAYER(id))
			return hdr_reg_verify_v2p4(id, start_offset, length);
		else
...
bool hdr_reg_check_premultiplied_out(u32 id)
{
	// v2p4: pre img -> depre img -> hdr/wcg -> depre img -> pre img -> blend
	if (IS_HDR_LAYER(id))
		return true;

	// v1p2: pre img -> depre img -> hdr/wcg -> depre img -> blend
	return false;
}
