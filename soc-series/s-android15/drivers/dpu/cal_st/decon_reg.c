...
/* [8855]
 * #1) When a DECON operates with concurrent write back or dual blender mode,
 *     SRAM_EN_OF_SEC_# should be configured for SRAM allocation
 *     of secondary OUTFIFO path.
 * #2) SRAM pool : index 0 ~ 4 -> PRI and SEC have to share SRAMs
 *     - if sram0 is enabled at PRI, SEC can not enable sram0
 *
 * Following are the scenarios:
 *  < DECON0 > - Single DSC (LCD only) - 3 SRAMs
 *  < DECON0 > - Dual DSC (LCD + CWB)  - 3 SRAMs for LCD & 2 SRAMs for CWB
 *  < DECON1 > - SWB		       - 2 SRAMs
 *  Note: SWB & CWB are mutually exclusive
 */
static void decon_reg_set_sram_enable(u32 id)
{
	if (id == 0) {
		...
		/* In case of SWB, below configurations are used.
		 * In case of CWB, the same SRAMs are configured
		 * in SRAM_EN_OF_SEC_0 of DECON0.
		 */
		decon_write_mask(id, SRAM_EN_OF_PRI_0, ~0,
				SRAM3_EN_F | SRAM4_EN_F);
	}
}
...
static void decon_reg_set_outfifo_size_ctl1(u32 id, u32 width)
{
	...
	/* In Santa, OF_SIZE_1 is available only in decon0 */
	if (id != 0)
		return;
...
static void decon_reg_set_outfifo_size_ctl2(u32 id, u32 width, u32 height)
{
	...
	/* In Santa, OF_SIZE_2 is available only in decon0 */
	if (id != 0)
		return;
...
/* bias sleep can be enabled only when pll seep is enabled */
	...
	if (out_type & DECON_OUT_DSI) {
		 ...
		 * But, in case of DPHY 6410, the BIAS block has been
		 * removed, so, no delay is required.
		 */
		dsimif_start_time = dsim->clk_param.hs_clk / 16 * 0;
	}
#endif

	switch (out_type) {
	case DECON_OUT_DSI0:
		/* s5e8855 has DECON0/OUT0 connected only to DSIMIF0 */
		val = OUTIF_DSI0;
...
{
	cal_log_info(id, "BLD_WIN_IDX_BASE is not supported\n");
}
/*
		...
		u32 bpc)
{
	u32 width_f;
	u32 comp_slice_width; /* compressed slice width */
	u32 align = (id == 0) ? 4 : 2; /* LCD: 4, Other(DP): 2 */
...
static u32 decon_get_vfp_th(u32 svsync_us)
{
	...
	cnt = svsync_us * 768 / 10; //OSC Clk Rate
...
static u32 decon_get_ewr_cycle(int fps, int wakeup_us)
{
	...
	cnt = ((1000000 / fps) - wakeup_us) * 768 / 10; //OSC Clk Rate
		...
		bool in_slice[4], u32 ds_en[2], u32 sm_ch[2])
{
	...
	case 4:
		...
		/* ENC1 is not supported in S5E8855 */
	...
	case 1:
		ds_en[DECON_DSC_ENC0] = 0;
		sm_ch[DECON_DSC_ENC0] = 0;
		/* ENC1 is not supported in S5E8845 */
		ds_en[DECON_DSC_ENC1] = 0;
		sm_ch[DECON_DSC_ENC1] = 0;
		break;
	default:
				...
				struct decon_dsc *dsc_enc)
{
	...
	dsc_enc->pic_width = dsc_cfg->pic_width; // TBD: will it change for CWB ??
			...
			struct decon_dsc *dsc_enc, bool gen_pps)
{
	...
	/* gen_pps is made false in S5E8855 */
	if (gen_pps)
	...
	if (id == 1) {
		...
		if (!gen_pps)
			dsc_reg_set_pps_36_87(DECON_DSC_ENC1, &config->dsc_cfg);
		...
		for (dsc_id = 0; dsc_id < config->dsc.dsc_count; dsc_id++) {
			...
			if (!gen_pps)
				dsc_reg_set_pps_36_87(dsc_id, &config->dsc_cfg);
		}
	}
...
// is the below function applicable for new DSIM version 2.93 ?
...
/*
 * PLL_SLEEP_CON register is changed
 * - PLL_WAKEUP_EN
...
void decon_reg_set_bpc_and_dither(u32 id, struct decon_config *config)
{
	...
	/* deprecated dither settings as moved to DQE */
}
...
void __decon_dump(u32 id, struct decon_regs *regs, bool dsc_en)
{
	...
	dpu_print_hex_dump(main_regs, main_regs + 0x0700, 0x04);
	...
	/* decon_win & decon_wincon : 8EA */
	for (i = 0; i < MAX_WIN_PER_DECON; i++) {
		cal_log_info(id, "\n=== DECON_WIN%d SFR DUMP ===\n", i);
		dpu_print_hex_dump(win_regs, win_regs + WIN_OFFSET(i), 0x24);
		...
		dpu_print_hex_dump(win_regs,
				win_regs + WIN_OFFSET(i) + SHADOW_OFFSET, 0x24);
...
bool decon_reg_check_th_error(u32 id) { return 0; }
void sramc_d_reg_get_irq_and_clear(u32 id) { return; }
void decon_reg_opmode_switch(u32 id, struct decon_config *config) { return; }
