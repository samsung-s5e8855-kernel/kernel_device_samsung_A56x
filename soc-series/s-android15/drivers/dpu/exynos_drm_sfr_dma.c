...
static void exynos_drm_sfr_dma_enable_sync(void)
{
	...
	bool external_connected = dma_private[0].external_connected;

	for (dpuf = 0; dpuf < MAX_DPUF_CNT; ++dpuf) {
		pdma = &dma_private[dpuf];
		pdma->enabled = dma_switch && !external_connected;
	}
}
...
static void __dump_dataspace(void)
{
	...
	for (dpuf = 0; dpuf < MAX_DPUF_CNT; ++dpuf) {
		...
		__sfr_dma_dump(pdma->sfr_dma_regs);
	}
...
int exynos_drm_sfr_dma_request(int id, u32 offset, u32 val, enum dpp_regs_type type)
{
	...
	if (idx >= pdma->total_size || pdma->external_connected) {
		goto exit;
	}
...
static int drm_dma_alloc_map_buf(struct device *dev, struct dpuf_dma *dma, size_t size)
{
	...
	if (IS_ERR_OR_NULL(buf)) {
		pr_err("dma_buf allocation is failed [%p]\n", buf);
		return PTR_ERR(buf);
	}
	...
	if (IS_ERR_OR_NULL(attachment)) {
		pr_err("failed to attach dma_buf [%p]\n", attachment);
		return -EINVAL;
	}
	...
	if (IS_ERR_VALUE(dma_addr)) {
		pr_err("failed to map iovmm [%llu]\n", dma_addr);
		return -EINVAL;
	}
...
void exynos_drm_sfr_dma_config(bool changed, bool active)
{
	...
	if (changed) {
		if (active)
			pdma->external_connected = true;
		else
			pdma->external_connected = false;
	}
}

int exynos_drm_sfr_dma_initialize(struct device *dev, void __iomem *reg,
		                  size_t size, bool en)
{
	...
	pdma->dma_switch = en;
	pdma->external_connected = false;
	pdma->sfr_dma_regs = reg;
	pdma->total_size = size;

	pr_info("%s:DMA vaddr:%p paddr:0x%llx\n", __func__, pdma->data, pdma->addr);
