...
void dpu_profile_hiber_inc_frame_cnt(struct exynos_drm_crtc *exynos_crtc)
{
	...
	if (exynos_crtc->hibernation->profile.started)
...
static struct memlog_obj *
__memlog_alloc_printf(struct dpu_memlog *memlog, const char *name, struct device_node *np)
{
	struct memlog_obj *obj;
	u32 size;

	if (of_property_read_u32(np, name, &size)) {
		pr_info("%s: failed to alloc dev memlog memory for %s\n",
				__func__, name);
		return NULL;
	}

	obj = memlog_alloc_printf(memlog->desc, size, NULL, name, 0);
...
static void dpu_init_memlogger(struct exynos_drm_crtc *exynos_crtc)
{
	...
	struct device_node *np = exynos_crtc->dev->of_node;
	int ret;
	...
	if (!g_log_obj) {
		g_log_obj = __memlog_alloc_printf(memlog, "log-mem0", np);
		if (!g_log_obj)
			return;
	}

	if (!g_errlog_obj) {
		g_errlog_obj = __memlog_alloc_printf(memlog, "log-mem1", np);
		if (!g_errlog_obj)
			return;
	}

	memlog->event_log.obj = __memlog_alloc_printf(memlog, "evt-mem", np);
	...
	memlog->fevent_log.obj = __memlog_alloc_printf(memlog, "fevt-mem", np);
