...
int exynos_dpuf_set_votf(u32 dpuf_idx, bool en)
{
	...
	pr_info("%s: dpuf[%d] en[%d]\n", __func__, dpuf_idx, en);
...
static uint64_t *dpp_get_modifiers(struct dpp_device *dpp)
{
	...
	if (test_bit(DPP_ATTR_SAJC, &dpp->attr))
		add_modifier(DRM_FORMAT_MOD_SAMSUNG_SAJC(0, 0, 0), modifiers, dpp, i);
		...
		struct dpp_params_info *config, const struct drm_framebuffer *fb)
{
	...
	if (has_all_bits(DRM_FORMAT_MOD_ARM_AFBC(0), fb->modifier)) {
	...
	} else if (has_all_bits(DRM_FORMAT_MOD_SAMSUNG_SAJC(0, 0, 0), fb->modifier)) {
		...
		config->sajc_addr_mode = SAJC_MOD_ADDRESS_MODE_GET(fb->modifier);
...
static void dpp_print_restriction(struct device *dev, struct dpp_restrict *res)
{
	...
	dev_info(dev, "compressed format support transform(0x%x)",
			res->comp_transform_support);
}
...
static int exynos_parse_restrictions(struct device *dev, struct dpp_restrict *res)
{
	...
	if (of_property_read_u32(np, "dpp,comp-transform-support",
				&res->comp_transform_support))
		res->comp_transform_support = 0;
...
static irqreturn_t dpp_irq_handler(int irq, void *priv)
{
	...
	struct exynos_drm_crtc *exynos_crtc = to_exynos_crtc(exynos_plane->base.state->crtc);
	u32 dpp_irq = 0;
	...
	if (dpp_irq & DPP_CFG_ERROR_IRQ)
		DPU_EVENT_LOG("DPP_CFG_ERR", exynos_crtc, EVENT_FLAG_ERROR, NULL);
...
static irqreturn_t dma_irq_handler(int irq, void *priv)
{
	...
	if (irqs & IDMA_RECOVERY_START_IRQ) {
		...
		DPU_EVENT_LOG("DMA_RECOVERY", exynos_crtc, EVENT_FLAG_ERROR,
				"ID:%d SRC:%s COUNT:%d",
	...
	if (irqs & IDMA_STATUS_FRAMEDONE_IRQ) {
		...
		DPU_EVENT_LOG("DPP_FRAMEDONE", exynos_crtc, 0, "ID:%d", dpp->id);
	}
	if ((irqs & IDMA_READ_SLAVE_ERROR) || (irqs & IDMA_STATUS_DEADLOCK_IRQ)) {
		...
		DPU_EVENT_LOG("DMA_ERR", exynos_crtc, EVENT_FLAG_ERROR,	NULL);
...
void dpp_set_all_irqs(bool en)
{
	struct dpp_device *dpp;
	u32 id;

	for (id = 0; id < MAX_DPP_CNT; id++) {
		dpp = get_dpp_drvdata(id);
		if (!dpp)
			continue;
		if (en)
			dpp_reg_enable_all_irqs(id, dpp->attr);
		else
			dpp_reg_disable_all_irqs(id, dpp->attr);
	}
}
