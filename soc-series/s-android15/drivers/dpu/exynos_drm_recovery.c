...
__weak bool peri_condition_check(const struct drm_crtc *crtc)
{
	...
	if (conn) {
		...
		if (funcs && funcs->check_connection)
			timedout = !funcs->check_connection(exynos_conn);
	}
...
struct recovery_mapping recovery_map[RECOVERY_MODE_MAX] = {
	...
	{"dpp", dpp_condition_check},
	{"peri", peri_condition_check},
	{"decon", decon_condition_check},
#if defined(CONFIG_EXYNOS_UEVENT_RECOVERY_SOLUTION)
	{"uevent_r", skip_condition_check},
#endif
...
const char recovery_state_string[RECOVERY_STATE_NUM][RECOVERY_NAME_LEN] = {
	"not_supported", "idle", "trigger", "begin", "restore",
#if defined(CONFIG_EXYNOS_UEVENT_RECOVERY_SOLUTION)
	"uevent_trigger",
#endif
	"max_error"
...
static inline bool __is_dual_display(struct decon_device *decon)
{
	return (decon->config.mode.dsi_mode == DSI_MODE_DUAL_DISPLAY) ? true : false;
}

static inline bool is_builtin_crtc(const struct drm_crtc *crtc)
{
	struct exynos_drm_crtc *exynos_crtc = to_exynos_crtc(crtc);
	struct decon_device *decon = exynos_crtc->ctx;
	int idx = 0;

	if (__is_dual_display(decon))
		++idx;

	return ((drm_crtc_index(crtc) <= idx) ? true : false);
}
...
#if defined(CONFIG_EXYNOS_UEVENT_RECOVERY_SOLUTION)
static void exynos_recovery_send_uevent(struct drm_device *dev)
{
	char *reset_event[] = {"LCM_DIE=1", NULL };
	kobject_uevent_env(&dev->primary->kdev->kobj, KOBJ_CHANGE, reset_event);
}

static void exynos_recovery_send_dual_uevent(struct drm_device *dev, int id)
{
	char *reset_event0[] = {"DUAL_LCM_DIE=0", NULL };
	char *reset_event1[] = {"DUAL_LCM_DIE=1", NULL };

	if (id == 1)
		kobject_uevent_env(&dev->primary->kdev->kobj, KOBJ_CHANGE, reset_event1);
	else
		kobject_uevent_env(&dev->primary->kdev->kobj, KOBJ_CHANGE, reset_event0);
}
#endif
				...
				struct exynos_recovery *recovery)
{
	...
	recovery->skip_snapshot = cond->disable_watchdog;
...
static void exynos_recovery_handler(struct work_struct *work)
{
	...
	if (!exynos_recovery_condition_check(crtc, recovery)) {
		if (recovery->always) {
			...
			if (!recovery->skip_snapshot)
				exynos_drm_report_dpu_fault(exynos_crtc);
			recovery->skip_snapshot = false;
			exynos_recovery_set_state(decon, RECOVERY_IDLE);
			return;
		}
	}
...
#if defined(CONFIG_EXYNOS_UEVENT_RECOVERY_SOLUTION)
	if (recovery->r_cond[recovery->req_mode_id].send_uevent) {
		exynos_recovery_set_state(decon, RECOVERY_UEVENT);
		if (__is_dual_display(decon))
			exynos_recovery_send_dual_uevent(dev, decon->id);
		else
			exynos_recovery_send_uevent(dev);
		recovery->r_cond[recovery->req_mode_id].count++;
		recovery->count++;
		recov_info("uevent recovery(#%d) \n",recovery->r_cond[recovery->req_mode_id].count);
		return;
	}
#endif

	if (recovery->r_cond[recovery->req_mode_id].refresh_panel ||
		recovery->r_cond[recovery->req_mode_id].reset_vblank)
		intf_reset = true;

	__set_panel_reset(decon, intf_reset);
...
retry:
	...
	recov_info("recovery(#%d) is successfully finished "KTIME_MSEC_FMT"\n",
			recovery->count, KTIME_MSEC_ARG(time_c));
...
static int __set_recovery_mode(struct exynos_recovery *recovery, char *mode)
{
...
int exynos_recovery_set_mode(struct decon_device *decon, char *mode)
{
	...
	funcs = recovery->funcs;
	if (funcs)
		ret = funcs->set_mode(recovery, mode);
...
static int exynos_recovery_parse_dt_sub(struct device_node *np,
					struct exynos_recovery *recovery, int id)
{
	struct exynos_recovery_cond *cond;
	u32 data[2];
	int ret, i;
	...
	cond = &recovery->r_cond[id];
	cond->id = id;
	strncpy(&cond->name[0], np->name, RECOVERY_NAME_LEN - 1);
...
#if defined(CONFIG_EXYNOS_UEVENT_RECOVERY_SOLUTION)
	cond->send_uevent = of_property_read_bool(np, "send-uevent");
	recov_info("send_uevent=%d\n", cond->send_uevent);
#endif

	cond->disable_cond_chk = of_property_read_bool(np, "disable-condition-check");
	recov_info("disable condition func=%d\n", cond->disable_cond_chk);

	cond->disable_watchdog = of_property_read_bool(np, "disable-watchdog");
	recov_info("disable watchdog=%d\n", cond->disable_watchdog);
					...
					struct decon_device *decon)
{
	...
	struct device_node *child_np, *cchild_np, *prev = NULL;
	int num_cond, i, ret;

	recovery->modes = num_cond = 0;
	child_np = of_get_child_by_name(np, "recovery-condition");

	if (child_np) {
		do {
			cchild_np = of_get_next_available_child(child_np, prev);
			if (cchild_np) {
				i = num_cond++;
				ret = exynos_recovery_parse_dt_sub(cchild_np, recovery, i);
				if (ret) {
					recov_err("failed to parse recovery-condition (%s)\n", cchild_np->name);
					break;
				}
				recovery->modes = num_cond;
				recovery->max_req_mode |= (1 << i);
				if (strncmp(cchild_np->name, "always", RECOVERY_NAME_LEN) == 0) {
					recovery->always = true;
					recovery->always_id = i;
				}
				recov_info("recovery[%d]: (%s) registered\n", i, cchild_np->name);
				prev = cchild_np;
				of_node_put(cchild_np);
			}
		} while (cchild_np != NULL && num_cond < RECOVERY_MODE_MAX);
