...
static void dp_check_hdr_support(struct dp_device *dp)
{
	...
	u8 eotf = hdmi_type1->eotf;

	if (dp->dfp_type != DFP_TYPE_DP) {
		dp_info(dp, "not support HDR DFP: %d\n", dp->dfp_type);
		return;
	}

	if (eotf & HDR_SUPPORT_TYPE) {
		...
		dp_info(dp, "Support HDR(bpc: %d, max: %hu, max_fall: %hu, min: %hu)\n",
				dp->cal_res.bpc, hdmi_type1->max_cll,
				hdmi_type1->max_fall, hdmi_type1->min_cll);
	}
}
