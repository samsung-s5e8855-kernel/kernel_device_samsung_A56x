...
void dp_set_tui_status(bool status)
{
	struct dp_device *dp = get_dp_drvdata();
	dp->tui_status = status;
}
...
int dp_check_pixel_clock_for_hdr(struct dp_device *dp)
{
	...
	if (pixel_clock > bandwidth) {
		dp_info(dp, "not enough bandwidth for HDR (%llu/%llu)\n",
...
static void dp_check_dsc_capability(struct dp_device *dp)
{
	...
	/* HACK : DSC with 10bpc is not implemented now */
	if (dp->cal_res.bpc == BPC_10)
		goto err;
...
static int dp_full_link_training(struct dp_device *dp)
{
	...
	if (val[0] & DP_EXTENDED_RECEIVER_CAP_FIELD_PRESENT) {
		...
		dp_info(dp, "Extended Receiver Capability Field\n");
...
#define USB_ININ_WAIT_TIME 10000
	...
	do {
		state = dwc3_exynos_otg_is_usb_ready();
...
{
	struct dp_device *dp = get_dp_drvdata();
	struct hdr_infoframe hdr_infoframe = {0,};
	u8 buf[HDMI_INFOFRAME_HEADER_SIZE + HDR_INFOFRAME_LENGTH];
	int ret = 0;

	if (dp->rx_edid_data.hdr_support) {
		dp_info(dp, "Send HDR Infoframe\n");
		hdmi_drm_infoframe_pack_only(&dp->drm_hdr_infoframe, (void*)(buf), sizeof(buf));
		hdr_infoframe.version = dp->drm_hdr_infoframe.version;
		hdr_infoframe.length = dp->drm_hdr_infoframe.length;
		memcpy(hdr_infoframe.data, &buf[HDMI_INFOFRAME_HEADER_SIZE], HDR_INFOFRAME_LENGTH);
...
static void dp_on_by_hpd_high(struct dp_device *dp)
{
	...
	if (dp->bist_used) {
		...
		timeout = dp_wait_state_change(dp, 5000, DP_STATE_ON);
...
void dp_off_by_hpd_low(struct dp_device *dp)
{
	...
	if (dp->state == DP_STATE_ON
		...
		if (dp->bist_used == 0) {
			...
			timeout = dp_wait_state_change(dp, 5000, DP_STATE_OFF);
	...
	if (dp->bist_used == 0) {
		...
		if (irq_status_reg & VSYNC_DET)
			if (decon->config.mode.op_mode == DECON_VIDEO_MODE)
				exynos_drm_crtc_handle_vblank(decon->crtc);
	}
...
#define HACK
	...
	dp->res.phy_regs = ioremap((phys_addr_t)0x188D0000, 0x250);
	...
	dp->res.phy_tca_regs = ioremap((phys_addr_t)0x188E0000, 0xFC);
...
static ssize_t dp_sbu_sw_sel_store(const struct class *dev,
			     ...
			     struct drm_connector_state *state)
{
	...
	if (crtc_state->mode_changed) {
		...
		if (dp->rx_edid_data.hdr_support && dp->cal_res.bpc == BPC_10) {
	...
	if (dp->send_hdr_infoframe) {
		...
		dp_info(dp, "hdr infoframe eotf: %x, meta type: %x, version: %d, length: %d\n",
				dp->drm_hdr_infoframe.eotf, dp->drm_hdr_infoframe.metadata_type,
				dp->drm_hdr_infoframe.version, dp->drm_hdr_infoframe.length);
		dp_set_hdr_infoframe();
	}

	if ((crtc_state->active_changed == true) && (crtc_state->active == false) &&
			dp->rx_edid_data.hdr_support) {
		exynos_drm_replace_property_blob_from_id(state->connector->dev,
			&state->hdr_output_metadata, 0, sizeof(struct hdr_output_metadata));
		dp_info(dp, "Clear hdr_output_metadata\n");
	}
