...
static void
exynos_drm_crtc_hiber_trig_reset(struct drm_device *dev, struct drm_atomic_state *state)
{
	...
	for_each_oldnew_crtc_in_state(state, crtc, old_crtc_state, new_crtc_state, i) {
		...
		hibernation_trig_reset(exynos_crtc->hibernation, exynos_mode);
	}
}
...
exynos_atomic_check_windows(struct drm_device *dev, struct drm_atomic_state *state)
{
	...
	for_each_oldnew_crtc_in_state(state, crtc, old_crtc_state, new_crtc_state, i) {
		...
		dpu_debug(dev, "%s: win cnt changed: %d -> %d\n", crtc->name, old_win_cnt, new_win_cnt);
		...
		if (old_win_cnt > new_win_cnt) {
			...
			dpu_debug(dev, "%s: current win_mask=0x%x new=0x%x avail=0x%x\n", crtc->name,
					new_exynos_crtc_state->reserved_win_mask, win_mask,
					priv->available_win_mask);
	...
	for_each_new_crtc_in_state(state, crtc, new_crtc_state, i) {
		...
		dpu_debug(dev, "visible_win_mask(0x%x)\n",
...
static void exynos_atomic_prepare_partial_update(struct drm_atomic_state *state)
{
	...
	for_each_oldnew_crtc_in_state(state, crtc, old_crtc_state, new_crtc_state, i) {
		...
		if (drm_atomic_crtc_needs_modeset(new_crtc_state) ||
			...
			exynos_partial_initialize(partial, new_crtc_state);
					  ...
					  struct drm_crtc_state *crtc_state)
{
	...
	for (i = 0; i < n; i++) {
		...
		dpu_debug(dev, "[PLANE:%d:%s] normalized zpos value %d\n",
...
static void commit_tail(struct drm_atomic_state *old_state)
{
	...
	for_each_crtc_in_state(old_state, crtc, i) {
		...
		exynos_drm_profiler_update_ems_fence_cnt(exynos_crtc);
		recovery = check_recovery_state(crtc);
	}

	if (unlikely(recovery))
		dpu_info(dev, "recovery, skip for all preceding commits\n");
	else
...
static int exynos_atomic_skip_commit(struct drm_atomic_state *state)
{
	struct drm_crtc *crtc;
	struct drm_crtc_state *new_crtc_state;
	int i = 0;
	...
	for_each_new_crtc_in_state(state, crtc, new_crtc_state, i) {
		...
		if (new_exynos_crtc_state->shutdown_inprogress)
			return -ENODEV;
	}
...
static void __release_windows(struct drm_device *dev, unsigned int win_mask)
{
...
static void exynos_drm_rollback_windows(struct drm_atomic_state *old_state)
{
	...
	for_each_oldnew_crtc_in_state(old_state, crtc, old_crtc_state, new_crtc_state, i) {
		...
		win_mask = new_exynos_crtc_state->reserved_win_mask &
			(~old_exynos_crtc_state->reserved_win_mask);
		if (!win_mask)
			continue;
				...
				bool nonblock)
{
	...
	ret = exynos_atomic_skip_commit(state);
	if (ret) {
		drm_atomic_helper_cleanup_planes(dev, state);
		goto err;
	}

	exynos_drm_crtc_hiber_trig_reset(dev, state);
				      ...
				      const struct drm_plane_state *plane_state)
{
	...
	if (has_all_bits(DRM_FORMAT_MOD_SAMSUNG_SAJC(0, 0, 0), fb->modifier) ||
...
static bool __needs_wait_vblank(struct drm_crtc *crtc, struct drm_crtc_state *new_crtc_state)
{
	struct exynos_drm_crtc_state *new_exynos_crtc_state;
	struct exynos_drm_crtc *exynos_crtc;
	...
	return drm_crtc_vblank_get(crtc) == 0;
}

#define DEFAULT_TIMEOUT_FPS 60
#define MAX_VBLANK_CNT      12
static ktime_t __get_vblank_timeout(struct drm_crtc_state *crtc_state)
{
	struct exynos_drm_crtc_state *exynos_crtc_state = to_exynos_crtc_state(crtc_state);
	ktime_t interval_ns;

	if (exynos_crtc_state->frame_interval_ns) {
		interval_ns = exynos_crtc_state->frame_interval_ns;
	} else {
		u64 fps;

		fps = drm_mode_vrefresh(&crtc_state->mode) ?: DEFAULT_TIMEOUT_FPS;
		interval_ns = HZ2NS(fps);
	}

	if (IS_ENABLED(CONFIG_BOARD_EMULATOR))
		interval_ns *= 1000;

	return MAX_VBLANK_CNT * interval_ns;
}
		...
		struct drm_atomic_state *old_state)
{
	...
	DPU_ATRACE_BEGIN(__func__);
	for_each_new_crtc_in_state(old_state, crtc, new_crtc_state, i) {
		ktime_t vblank_timeout_ns, diff_ns, timestamp_s;

		if (!__needs_wait_vblank(crtc, new_crtc_state))
			continue;

		old_state->crtcs[i].last_vblank_count = drm_crtc_vblank_count(crtc);

		vblank_timeout_ns = __get_vblank_timeout(new_crtc_state);

		timestamp_s = ktime_get();
		ret = wait_event_timeout(dev->vblank[i].queue,
				old_state->crtcs[i].last_vblank_count !=
				drm_crtc_vblank_count(crtc),
				nsecs_to_jiffies(vblank_timeout_ns));

		diff_ns = ktime_sub(ktime_get(), timestamp_s);
		if (diff_ns >= (vblank_timeout_ns >> 1))
			pr_warn("[CRTC:%d:%s] vblank wait timed out!!!"
					KTIME_MSEC_FMT"/"KTIME_MSEC_FMT"\n",
				crtc->base.id, crtc->name,
				KTIME_MSEC_ARG(diff_ns), KTIME_MSEC_ARG(vblank_timeout_ns >> 1));
				      ...
				      unsigned int *still_crtc_mask)
{
	...
	for_each_oldnew_crtc_in_state(old_state, crtc, old_crtc_state,
		...
		if (exynos_crtc->vmc) {
			vmc_still_block(exynos_crtc->vmc, true);
			*still_crtc_mask |= drm_crtc_mask(crtc);
		}
	}
}
					...
					unsigned int *still_crtc_mask)
{
	...
	for_each_new_crtc_in_state(old_state, crtc, new_crtc_state, i) {
		...
		if (*still_crtc_mask & drm_crtc_mask(crtc))
...
static void exynos_atomic_wait_for_framestart(struct drm_atomic_state *old_state)
{
	...
	for_each_new_crtc_in_state(old_state, crtc, new_crtc_state, i) {
		...
		if (exynos_crtc_state->skip_hw_update)
			continue;

		if (exynos_crtc_state->shutdown_inprogress)
			continue;
...
static void exynos_atomic_update_brightness(struct drm_atomic_state *old_state)
{
	...
	struct exynos_drm_connector *exynos_conn;
	const struct exynos_drm_connector_funcs *funcs;
	const struct exynos_drm_connector_state *new_exynos_conn_state;
	int i;
	...
	for_each_new_connector_in_state(old_state, conn, new_conn_state, i) {
		...
		if (!new_conn_state->crtc || !new_conn_state->crtc->state->active)
			continue;

		new_exynos_conn_state = to_exynos_connector_state(new_conn_state);
		if (!new_exynos_conn_state->brightness_level_changed)
			continue;
		...
		if (funcs->update_brightness)
			funcs->update_brightness(exynos_conn, true);
	}
}
...
static void exynos_atomic_commit_tail(struct drm_atomic_state *old_state)
{
	...
	unsigned int still_crtc_mask = 0;

	DPU_ATRACE_BEGIN(__func__);

	DPU_ATRACE_BEGIN("modeset");
	exynos_atomic_block_hiber(old_state, &hibernation_crtc_mask, &still_crtc_mask);
	...
	exynos_atomic_unblock_hiber(old_state, &hibernation_crtc_mask, &still_crtc_mask);
...
extern struct platform_driver vmc_driver;
...
static struct exynos_drm_driver_info exynos_drm_drivers[] = {
	...
	{ DRV_PTR(vmc_driver, CONFIG_EXYNOS_VMC), 		0},
...
static int exynos_drm_platform_probe(struct platform_device *pdev)
{
	...
	platform_set_drvdata(pdev, &exynos_dev->base);
