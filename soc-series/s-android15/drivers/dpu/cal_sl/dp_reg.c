...
int dp_hsi0_domain_power_check(void)
{
	...
	if (pwr_regs == NULL) {
		pwr_regs = ioremap((phys_addr_t)0x13860000 + 0x1f40, 0x24);
		if (!pwr_regs) {
	...
	pr_info("HSI0_CONFIGURATION[0x1386_1f40](0x%08x)\n", readl(pwr_regs));
	pr_info("HSI0_STATUS	   [0x1386_1f44](0x%08x)\n", readl(pwr_regs + 0x4));
	pr_info("HSI0_STATES       [0x1386_1f48](0x%08x)\n", readl(pwr_regs + 0x8));
	if (readl(pwr_regs + 0x8) == 0)
		ret = 0;
	pr_info("HSI0_CTRL         [0x1386_1f50](0x%08x)\n", readl(pwr_regs + 0x10));
	pr_info("HSI0_OUT          [0x1386_1f60](0x%08x)\n", readl(pwr_regs + 0x20));
...
{
	if (clk_regs1 == NULL) {
		clk_regs1 = ioremap((phys_addr_t)0x18800000 + 0x0620, 0x10);
		if (!clk_regs1) {
	...
	pr_info("PLL_CON0_MUX_CLKCMU_HSI0_DPOSC_USER[0x1880_0620](0x%08x)\n", readl(clk_regs1));
	pr_info("PLL_CON1_MUX_CLKCMU_HSI0_DPOSC_USER[0x1880_0624](0x%08x)\n", readl(clk_regs1 + 0x4));

	if (clk_regs2 == NULL) {
		clk_regs2 = ioremap((phys_addr_t)0x2a0a0000 + 0x10ec, 0x10);
		if (!clk_regs2) {
			pr_err("HSI0_CLK SFR ioremap is failed\n");
			return;
		}
	...
	pr_info("CLK_CON_MUX_CLKCMU_HSI0_DPOSC[0x22b2_10ec](0x%08x)\n", readl(clk_regs2));

	if (clk_regs3 == NULL) {
		clk_regs3 = ioremap((phys_addr_t)0x2a0a0000 + 0x18b4, 0x10);
		if (!clk_regs3) {
			pr_err("HSI0_CLK SFR ioremap is failed\n");
			return;
		}
	...
	pr_info("CLK_CON_DIV_CLKCMU_HSI0_DPOSC[0x2a0a_18b4](0x%08x)\n", readl(clk_regs3));
}
...
void dp_print_hsi0_clk(void)
{
	if (clk_regs1) {
		...
		pr_info("PLL_CON0_MUX_CLKCMU_HSI0_DPOSC_USER[0x1880_0620] is not remapped\n");

	if (clk_regs2)
		pr_info("CLK_CON_MUX_CLKCMU_HSI0_DPOSC[0x22b2_10ec](0x%08x)\n", readl(clk_regs2));
	else
		pr_info("CLK_CON_MUX_CLKCMU_HSI0_DPOSC[0x22b2_10ec] is not remapped\n");

	if (clk_regs3)
		pr_info("CLK_CON_DIV_CLKCMU_HSI0_DPOSC[0x2a0a_18b4](0x%08x)\n", readl(clk_regs3));
	else
		pr_info("CLK_CON_DIV_CLKCMU_HSI0_DPOSC[0x2a0a_18b4] is not remapped\n");
}
...
void dp_reg_sw_reset(struct dp_cal_res *cal_res)
{
	...
	cal_log_debug(0, "SYSTEM_SW_RESET_CONTROL(-)[0x%p]:0x%08x\n",
			(void __iomem *)0x18880000 + SYSTEM_SW_RESET_CONTROL,
			dp_read(SYSTEM_SW_RESET_CONTROL));

	cal_log_debug(0, "SYSTEM_CLK_CONTROL[0x%p]:0x%08x\n",
			(void __iomem *)0x18880000 + SYSTEM_CLK_CONTROL,
...
int dp_reg_usbdrd_qch_en(u32 en)
{
	...
	cmu_regs = ioremap((phys_addr_t)0x18800000 + 0x3060, 0x8);
	if (!cmu_regs) {
	...
	if (en) {
		...
		pr_info("QCH_CON_USB32DRD_QCH_S_CTRL[0x18803060](0x%08x)\n", readl(cmu_regs));
		writel(0x6, cmu_regs);
		pr_info("QCH_CON_USB32DRD_QCH_S_CTRL[0x18803060](0x%08x)\n", readl(cmu_regs));
	} else {
