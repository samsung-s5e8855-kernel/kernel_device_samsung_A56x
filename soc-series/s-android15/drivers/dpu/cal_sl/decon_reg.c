...
//#define S5E3HAF_10BPC_COMPRESSION
...
static void sramc_g_reg_init(u32 id)
{
	...
	if ((id != REGS_DECON0_ID) && (id != REGS_DECON1_ID))
		return;
 ...
 * #3) SRAM pool : index 0 ~ 15 -> PRI and SEC have to share SRAMs
 *     - if sram0 is enabled at PRI, SEC can not enable sram0
 *     - PRI_0 : index 0 ~ 7 / PRI_1 : index 8 ~ 15
 *
 * [ EVT0 ]
 * < SRAM index : 0 ~ 15 >
 *  - entry : 8(384) / 10(384) / 13(352) / others(512: 4K pixels)
 ...
 *    12  | DECON1: Dual DSC disabled  | DECON1
 *    13  | DECON1: Dual DSC disabled  | DECON1
 *    14  | DECON2: Dual DSC disabled  | DECON2
 *    15  | DECON2: Dual DSC disabled  | DECON2
 *    16  | Without conflict           | DECON0
 *    17  | Without conflict           | DECON0
 *  __________________________________________________________________
 *
...
{
	u32 sram_pri0 = 0, sram_pri1 = 0;
	u32 sram_sec0 = 0, sram_sec1 = 0;

	switch (id) {
	case 0:
		sram_pri0 = SRAM0_EN_F | SRAM1_EN_F;
		sram_pri1 = SRAM16_EN_F | SRAM17_EN_F;
		break;
	case 1:
		sram_pri0 = SRAM2_EN_F | SRAM3_EN_F;
		break;
	case 2: //DP
	case 3: //SWB
	default:
		sram_pri0 = SRAM4_EN_F | SRAM5_EN_F | SRAM6_EN_F;
		if (is_dual_blender(config)) {
			sram_pri0 = SRAM4_EN_F | SRAM5_EN_F;
			sram_sec0 = SRAM6_EN_F;
			sram_sec1 = SRAM12_EN_F;
		}
		break;
	}

	decon_write_mask(id, SRAM_EN_OF_PRI_0, ~0, sram_pri0);
	decon_write_mask(id, SRAM_EN_OF_PRI_1, ~0, sram_pri1);
	decon_write_mask(id, SRAM_EN_OF_SEC_0, ~0, sram_sec0);
	decon_write_mask(id, SRAM_EN_OF_SEC_1, ~0, sram_sec1);
}
...
{
	u32 data_path0 = en ? ~0 : 0;
	u32 data_path1 = CWB_SRC_F(CWB_SRC_ENH_OUT) |
		         CWB_OF_IDX_F(CWB_OF_IDX_OF3) |
                         WB_SEL_IF_F(WB_SEL_IF_0);
	u32 sram_pri1 = en ? 0 : ~0;
	u32 sram_sec1 = en ? ~0 : 0;

	decon_write_mask(id, DATA_PATH_CON, data_path0, OUTIF_WB);
	if (en)
		decon_write_mask(id, DATA_PATH_CON_1, data_path1,
				CWB_SRC_MASK | CWB_OF_IDX_MASK | WB_SEL_IF_MASK);
	decon_write_mask(id, SRAM_EN_OF_PRI_1, sram_pri1, SRAM16_EN_F | SRAM17_EN_F);
	decon_write_mask(id, SRAM_EN_OF_SEC_1, sram_sec1,
			SRAM12_EN_F | SRAM13_EN_F | SRAM14_EN_F |
			SRAM15_EN_F | SRAM16_EN_F | SRAM17_EN_F);
}
...
void decon_reg_set_scaler_enable(u32 id, bool en)
{
	...
	val = en ? DQE_SCALER_ON : 0;
	decon_write_mask(id, DATA_PATH_CON, val, DQE_SCALER_ON);
}
...
void decon_reg_set_aiqe_enable(u32 id, bool en)
{
	...
	val = en ? AIQE_ON : 0;
	decon_write_mask(id, DATA_PATH_CON, val, AIQE_ON);
}
...
void decon_reg_set_aiqe_scaler_enable(u32 id, bool en)
{
	...
	val = en ? AIQE_SCALER_ON : 0;
	decon_write_mask(id, DATA_PATH_CON, val, AIQE_SCALER_ON);
}
...
static void decon_reg_config_data_path_size(u32 id, u32 width, u32 height,
		struct decon_dsc *p, struct exynos_dsc *dsc, u32 bpc)
{
	u32 width_f;
	u32 comp_slice_width; /* compressed slice width */
	u32 align = (id == 0 || id == 1) ? 4 : 2; /* LCD: 4, Other(DP): 2 */
			...
			enum decon_opmode_option option)
{
	...
	if (cfg->svsync_block) {
		decon_write_mask(id, OPMODE_OPTION, 0,
			CMD_FRAME_CON_F_MASK | VDO_FRAME_CON_F_MASK);
		return;
	}
...
void decon_reg_init_trigger(u32 id, struct decon_config *cfg)
{
	...
	mask = HW_TRIG_EN | SW_TRIG_EN | HW_TRIG_SEL_MASK | HW_TRIG_MASK_DECON;
	/*
	 * In case of vhm from EVT1 of 9955
	 * HW_TRIG_EN should be cleared and HW_TRIG_MASK should be unmasked
	 */
	if (cfg->mode.vmc_mode.en) {
		val = 0;
	} else {
		val = (mode == DECON_SW_TRIG) ? SW_TRIG_EN : HW_TRIG_EN;
				...
				struct decon_dsc *dsc_enc)
{
	...
	dsc_enc->bits_per_component = dsc_cfg->bits_per_component;
	dsc_enc->line_buf_depth = dsc_cfg->line_buf_depth;
	...
	struct decon_dsc *dsc_enc)
{
	...
	width = config->pscaler ? config->desired_width : config->image_width;
	height = config->pscaler ? config->desired_height : config->image_height;
...
#if defined(S5E3HAF_10BPC_COMPRESSION)
	dsc_enc->bits_per_component = 10;
#else
	dsc_enc->bits_per_component = 8;
#endif
	dsc_enc->line_buf_depth = dsc_enc->bits_per_component + 1;
...
static void dsc_reg_set_pps(u32 dsc_id, struct decon_dsc *dsc_enc)
{
	...
	val &= ~(PPS00_VER_MASK | PPS03_BPC_LBD_MASK);
	val |= PPS00_VER(dsc_enc->version);
	val |= PPS03_BPC_LBD(dsc_enc->bits_per_component << 4 | dsc_enc->line_buf_depth);
	...
	/* VESA SCR v4 was included after DSC 1.2a */
	if (dsc_enc->version <= 0x11 && dsc_enc->initial_dec_delay == 0)
	...
	/*
	 * SLSI was generated PPS table in probing time based on the PPS cmds
	 * or dsc configuration in dt. If new QP table is required by force,
	 * then the following code should be activated based on verified QP value.
	 */
#if defined(S5E3HAF_10BPC_COMPRESSION)
	if (dsc_enc->bits_per_component > 8) {
		val = PPS36_FLATNESS_MIN_QP(0x07);
		val |= PPS37_FLATNESS_MAX_QP(0x10);
		val |= PPS38_39_RC_MODEL_SIZE(0x2000);
		dsc_write(DSC_PPS36_39(dsc_id), val);


		val = PPS40_RC_EDGE_FACTOR(0x06);
		val |= PPS41_RC_QUANT_INCR_LIMIT0(0x0F);
		val |= PPS42_RC_QUANT_INCR_LIMIT1(0x0F);
		val |= PPS44_RC_TGT_OFFSET_HI(0x3);
		val |= PPS44_RC_TGT_OFFSET_LO(0x3);
		dsc_write(DSC_PPS40_43(dsc_id), val);

		dsc_write(DSC_PPS60_63(dsc_id), 0x22002A40);
		dsc_write(DSC_PPS64_67(dsc_id), 0x2ABE3AFC);
		dsc_write(DSC_PPS68_71(dsc_id), 0x3AFA3AF8);

		dsc_write(DSC_PPS72_75(dsc_id), 0x3B383B78);
		dsc_write(DSC_PPS76_79(dsc_id), 0x3BB64BF6);
		dsc_write(DSC_PPS80_83(dsc_id), 0x4C344C74);
		dsc_write(DSC_PPS84_87(dsc_id), 0x5C748CF4);
	}
#endif
}
...
static int dsc_reg_init(u32 id, struct decon_config *config)
{
	struct decon_dsc dsc_enc;
	...
	decon_reg_config_data_path_size(id, dsc_enc.width_per_enc,
			config->pscaler ? config->desired_height : config->image_height,
			&dsc_enc, &config->dsc, config->out_bpc);
...
static void decon_reg_configure_lcd(u32 id, struct decon_config *config)
{
	...
	if (config->dsc.enabled) {
		dsc_reg_init(id, config);
	} else {
		decon_reg_config_data_path_size(id, config->image_width >> is_dual_blender(config),
...
u32 decon_reg_get_enable_windows(u32 id)
{
	...
	for (win_idx = 0; win_idx < MAX_DECON_CNT; win_idx++) {
		if (wincon_read_mask(id, DECON_CON_WIN(win_idx), _WIN_EN_F))
			enable_wins |= (1 << win_idx);
	}

	if (!enable_wins)
		cal_log_info(id, "no enabled window\n");
...
{
	u32 mask = SHD_REG_UP_REQ_GLOBAL | SHD_REG_UP_REQ_FOR_DECON;
	struct decon_device *decon = get_decon_drvdata(id);
	 ...
	 * In DP scenario, decon dither can be enabled by ENHANCE_DITHER_ON.
	 */

	if ((id <= 1) && decon_read_mask(id, DATA_PATH_CON, ENHANCE_DQE_ON))
		mask |= (SHD_REG_UP_REQ_DQE | SHD_REG_UP_REQ_DQE_CGC);

	if (decon_read_mask(id, DATA_PATH_CON, COMP_DSC_MASK))
		mask |= SHD_REG_UP_REQ_CMP;

	if (decon_read_mask(id, DATA_PATH_CON, AIQE_ON_SCALER_ON))
		mask |= SHD_REG_UP_REQ_AIQE;
...
static void decon_reg_set_urgent(u32 id, struct decon_config *config)
{
	...
	if (id == 0) {
		decon_reg_set_rd_urgent_enable(id,
				config->mode.op_mode ? 0 : config->urgent.rd_en);
...
static void decon_reg_set_vmc_mode(u32 id, bool en)
{
	u32 val, mask;

	val = en ? ~0 : 0;
	mask = VMC_ALIGN_VSYNC | VMC_MODE_EN;
	decon_write_mask(id, VMC_CNTL, val, mask);
}
...
int decon_reg_init(u32 id, struct decon_config *config)
{
	...
	decon_reg_set_vmc_mode(id, config->mode.vmc_mode.en);
	...
	if ((config->out_type & DECON_OUT_DSI) &&
		...
		(config->svsync_time) && !config->svsync_block) {
	...
	/*
	 * decon idle update is required
	 * for decon to send esync_dsim to dsim
	 * even before decon runs.
	 */
	if (config->mode.vmc_mode.en && !decon_reg_get_run_status(id))
...
{
	int ret = 0;
	struct decon_vmc_mode *vmc_mode = &config->mode.vmc_mode;
	bool decon_running = decon_reg_get_run_status(id);
	bool decon_update_req = false;
#if IS_ENABLED(CONFIG_DRM_SAMSUNG_DP)
	if (config->out_type & DECON_OUT_DP)
	...
	if (vmc_mode->en) {
		if (decon_reg_get_enable_windows(id) &&
			(!decon_running || !vmc_mode->sync_shd_up)) {
			decon_reg_update_req_global(id);
			decon_update_req = true;
		}

		vmc_mode->sync_shd_up = false;
		...
		enum decon_set_trig trig)
{
	...
	if (mode->trig_mode == DECON_SW_TRIG) {
		val = (trig == DECON_TRIG_UNMASK) ? SW_TRIG : 0;
		mask = SW_TRIG;
	} else { /* DECON_HW_TRIG */
		val = (trig == DECON_TRIG_UNMASK) ? 0 : HW_TRIG_MASK_DECON;
...
void decon_reg_set_all_interrupts(u32 en, bool is_video_mode[])
{
	...
	for (id = 0; id < MAX_DECON_CNT; id++) {
		if (is_video_mode[id])
			decon_reg_set_interrupts(id, en);
	}
}
...
{
	/*
	 * When global update request write occurs during frame blank period,
	 * the update is postponed to next frame blank start.
	 */
	decon_write_mask(id, DECON_SHD_UP_OPTION, ~0,
		SHD_UP_OPTION_MASK_FRAME_BLANK_UPDATE);

	if (unmask && !decon_read(id, SHD_REG_UP_REQ)) {
			decon_reg_update_req_global(id);
			mode->vmc_mode.sync_shd_up = true;
	}
}
...
{
	config->svsync_block = block;

	if (block) {
		decon_reg_set_vfp_threshold(id, 0);
		decon_reg_set_opmode_option(id, config, 0);
	}
}
#endif
