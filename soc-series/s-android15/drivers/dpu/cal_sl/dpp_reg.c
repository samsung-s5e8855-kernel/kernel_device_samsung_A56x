...
static void idma_reg_set_comp(u32 id, const struct dpp_params_info *p)
{
	...
	case COMP_TYPE_SAJC:
		...
		dma_write(id, RDMA_SAJC_PARAM, IDMA_INDEPENDENT_BLK(p->blk_size) |
				...
				IDMA_SAJC_ADDRESS_MODE(p->sajc_addr_mode));
		break;
	default:
		break;
	}
...
void hdrc_reg_set_dither_en(u32 id, u32 en)
{
...
static void hdrc_reg_set_hdr_bypass(u32 id, bool bypass)
{
	u32 val = bypass ? 1 : 0;

	hdr_comm_write(id, LSI_COMM_HDR_BYPASS, COMM_HDR_BYPASS(val));
}
		...
		const unsigned long attr)
{
	...
	if (test_bit(DPP_ATTR_IDMA, &attr)) {
		...
		if (test_bit(DPP_ATTR_DPP, &attr)) {
			...
			hdrc_reg_set_hdr_bypass(id, !p->hdr_en);
...
void dpp_reg_init(u32 id, const unsigned long attr)
{
	...
	if (test_bit(DPP_ATTR_HDR_COMM, &attr)) {
		...
		hdrc_reg_set_hdr_bypass(id, true); /* disable if customer's HDR is used */
	}
