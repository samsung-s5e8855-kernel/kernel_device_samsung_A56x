...
#define HIBERNATION_ENTRY_VHM_FPS		240
...
void exynos_hibernation_queue_enter_work(struct exynos_drm_crtc *exynos_crtc)
{
	struct exynos_hibernation *hiber = exynos_crtc->hibernation;

	if (!hiber || is_hibernation_blocked(hiber))
		return;

	if (IS_DECON_HIBER_STATE(exynos_crtc))
		return;

	kthread_queue_work(&exynos_crtc->worker, &hiber->work);
}
...
static bool exynos_hibernation_check(struct exynos_hibernation *hiber)
{
	...
	/* Hibernation is supported only if all instances are activated. */
	if (!list_empty(&inactive_list))
		return false;

	if (!(dpu_hiber_enable_mask & (1 << hiber->id))) {
		hiber_debug(hiber, "hibernation is intentionally disabled\n");
		return false;
	}

	return (!is_hibernation_blocked(hiber) && !is_dsr_operating(hiber) &&
...
retry:
	...
	exynos_crtc_state = to_exynos_crtc_state(crtc_state);
	exynos_crtc_state->skip_hw_update = !sr_active;
...
/* Hibernation is activated in the first call of decon_enable() */
void exynos_hibernation_activate(struct exynos_hibernation *hiber)
{
	struct exynos_hibernation *pos, *t;

	if (!hiber)
		return;

	list_for_each_entry_safe(pos, t, &inactive_list, node) {
		if (pos == hiber) {
			list_del(&pos->node);
			return;
		}
...
exynos_hibernation_register(struct exynos_drm_crtc *exynos_crtc)
{
	...
	hibernation_trig_reset(hibernation, NULL);

	atomic_set(&hibernation->block_cnt, 0);
	INIT_LIST_HEAD(&hibernation->node);
	list_add_tail(&hibernation->node, &inactive_list);
