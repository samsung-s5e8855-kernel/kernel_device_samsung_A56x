...
/*===== for legacy model build ====*/
__weak bool hdr_reg_check_premultiplied_out(u32 id)
{
	return false;
}
/*===== for legacy model build ====*/
				...
				const struct exynos_drm_plane_state *exynos_plane_state)
{
	...
	if (hdr->dma_buf) {
		if (hdr->vaddr) {
			...
			dma_buf_vunmap_unlocked(hdr->dma_buf, &old_map);
		}
...
{
	return ((coef_h != NULL) &&
		(coef_h->type.unpack.hw_type == HDR_HW_TYPE_BASE) &&
		(coef_h->sfr_con.unpack.mod_en != 0) &&
		(coef_h->sfr_con.unpack.mul_en != 0) &&
		(hdr_reg_check_premultiplied_out(coef_h->type.unpack.layer_index) == false));
}
...
error:
	...
	exynos_plane_state->hdr_ctx = NULL;
	return -1;
}
...
static int hdr_init_resources(struct exynos_hdr *hdr, struct device *dev)
{
	...
	if (!hdr_cls)
		hdr_cls = class_create("hdr");
	if (IS_ERR_OR_NULL(hdr_cls)) {
		hdr_err(hdr, "failed to create hdr class\n");
		goto err;
	}

	hdr->dev = device_create(hdr_cls, NULL, 0, &hdr, "hdr%d", hdr->id);
