...
TRACE_EVENT(ufs_perf_update_v1,
	TP_PROTO(const char *event, int chunk,
		 u32 count, u32 density, int freq_state, s64 s_time, int ret),

	TP_ARGS(event, chunk, count, density, freq_state, s_time, ret),
	...
	TP_fast_assign(
		...
		__entry->chunk = chunk;
		__entry->count = count;
		__entry->density = density;
		__entry->freq_state = freq_state;
		__entry->s_time = s_time;
		__entry->ret = ret;
	),

	TP_printk(
		"%s: %s, %u, %u, %s, %lld nsec, %d",
		__get_str(event),
		__print_symbolic(__entry->chunk, CHUNK_TYPE),
		__entry->count,
		__entry->density,
