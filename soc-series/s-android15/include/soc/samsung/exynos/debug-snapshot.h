...
typedef void (*dss_el1_undef_hook_fn)(struct pt_regs *regs, unsigned long esr);
...
extern void dbg_snapshot_ecc_dump(int policy);
...
extern int register_dss_el1_undef_hook(dss_el1_undef_hook_fn fn);
...
extern void dbg_snapshot_irq(int irq, void *fn, int en);
...
extern void dbg_snapshot_esca(unsigned long long timestamp, const char *log,
				unsigned int data);
#else
...
extern void dbg_snapshot_freq(int type, u64 time, int seq_no, unsigned int old_freq,
				unsigned int target_freq, int en);
#else
extern void dbg_snapshot_freq(int type, unsigned int old_freq,
				unsigned int target_freq, int en);
#endif
extern void dbg_snapshot_dm(int req_type, int dm_type,
			    unsigned int min, unsigned int max);
extern void dbg_snapshot_pmic(int irq, const char *str, int data1, int data2);
extern void dbg_snapshot_printk(const char *fmt, ...);
extern struct dbg_snapshot_log_item *dbg_snapshot_get_log_item(char *name);
extern char dss_freq_name[SZ_32][SZ_8];
extern unsigned int dss_freq_size;
...
/* debug-snapshot-pmsg functions */
extern void dbg_snapshot_pmsg_chain_register(struct notifier_block *nb);

#else /* CONFIG_DEBUG_SNAPSHOT */
#define dbg_snapshot_pmsg_chain_register(a)	do { } while (0)
#define dbg_snapshot_cpuidle(a, b, c, d)	do { } while (0)
#define dbg_snapshot_irq(a, b, c)		do { } while (0)
#if IS_ENABLED(CONFIG_EXYNOS_ESCA)
...
#if IS_ENABLED(CONFIG_PRECISE_DVFS_LOGGING)
#define dbg_snapshot_freq(a, b, c, d, e, f)		do { } while (0)
#else
#define dbg_snapshot_freq(a, b, c, d)		do { } while (0)
#endif
#define dbg_snapshot_dm(a, b, c, d)		do { } while (0)
#define dbg_snapshot_pmic(a, b, c, d)		do { } while (0)
...
#define register_dss_el1_undef_hook(a)		(0)
...
#define ARM_CPU_PART_HAYES		0xD80
#define ARM_CPU_PART_HUNTER		0xD81
#define ARM_CPU_PART_HUNTER_ELP		0xD82
#define ARM_CPU_PART_BLACKHAWK		0xD85
#define ARM_CPU_PART_CHABERTON		0xD87
