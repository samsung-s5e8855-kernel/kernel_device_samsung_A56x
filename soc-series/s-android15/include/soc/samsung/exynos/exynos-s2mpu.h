...
/* S2MPU version */
#define S2MPU_V1_2				(0x1200)
#define S2MPU_V2_0				(0x2000)
#define S2MPU_V9_0				(0x9000)
#define S2MPU_V9_0_1				(0x9001)

#define S2MPU_VERSION_MAJOR_MINOR_SHIFT		(0x8)
...
#define ERROR_INVALID_SUBSYSTEM_PD		(0x6)
#define ERROR_INVALID_ALLOWLIST_INFO		(0x7)
...
#define OWNER_IS_STAGE2_LV3_TABLE		(97)
#define OWNER_IS_TZMP2				(96)
#define OWNER_IS_PKVM				(95)
...
/* v9.0.1 */
#define S2MPU_REGION_ACCESS_PERMISSION_FAULT	(0x0)
#define S2MPU_S1PTW_RANGE_CHK_FAULT		(0x1)
#define S2MPU_PROTECTION_TABLE_ENTRY_CHK_FAULT	(0x2)

#define FAULT_TYPE_PAGE_FAULT			(0x1)
#define S2MPU_PTW_OUT_OF_PA_RANGE_ACCESS	(0x1)
 ...
 * S2MPU v9.0.1
 *
 * BY_S1PTW, AP_FAULT_SRC are added
 *******************************
 */
struct __s2mpu_fault_info_v9_0_1 {
	unsigned int fault_addr_low;
	unsigned int fault_addr_high;
	unsigned int vid;
	unsigned int type;
	...
	unsigned int subsystem;
	unsigned int blocklist_owner;
	unsigned int by_s1ptw;
	unsigned int ap_fault_src;
	unsigned int ptw_fault_src;
	struct s2mpu_fault_regs fault_regs;
	struct s2mpu_fault_tlb fault_tlb[S2MPU_TLB_TYPE_NUM];
	struct s2mpu_fault_map_info fault_map_info;
};

struct s2mpu_fault_info_v9_0_1 {
	unsigned int fault_cnt;
	enum s2mpu_type s2mpu_type;
	struct __s2mpu_fault_info_v9_0_1 info[MAX_VID_OF_S2MPU_FAULT_INFO];
};
...
/**
 * exynos_s2mpu_subsystem_set_allowlist - Set allowlist of sub-system.
 *
 * @subsystem:	 Sub-system name.
 *		 Please refer to subsystem-names property of s2mpu node
 *		 in exynosXXXX-security.dtsi.
 * @base: allowlist base address.
 *		 It's physical address(PA) and should be aligned with 64KB
 *		 because of S2MPU granule.
 * @size: allowlist size.
 *		 It should be aligned with 64KB like base
 *		 because of S2MPU granlue.
 */
...
/**
 * exynos_s2mpu_backup_subsystem - Backup S2MPU context for sub-system
 *
 * @subsystem:	Sub-system name
 *		Please refer to subsystem-names property of
 *		s2mpu node in exynosXXXX-security.dtsi.
 *
 * This function must be called in case that sub-system block power off.
 */
unsigned long exynos_s2mpu_backup_subsystem(const char *subsystem);
#else
static inline unsigned long
exynos_s2mpu_verify_subsystem_fw(const char *subsystem,
				 unsigned int fw_id,
				 unsigned long fw_base,
				 size_t fw_bin_size,
				 size_t fw_mem_size)
...
static inline unsigned long
exynos_s2mpu_request_fw_stage2_ap(const char *subsystem)
{
	return 0;
}
...
static inline unsigned long
exynos_s2mpu_release_fw_stage2_ap(const char *subsystem,
				  unsigned int fw_id)
{
	return 0;
}
static inline unsigned long
exynos_s2mpu_backup_subsystem(const char *subsystem)
{
	return 0;
}
#endif
