...
#else
static inline int exynos_thermal_get_tz_temps(int *temps, int size)
{
	return -ENODEV;
}
#endif
...
extern int exynos_tmu_isp_remove_notifier(struct notifier_block *n);
#else
static inline int exynos_tmu_isp_add_notifier(struct notifier_block *n)
{
	return 0;
}
static inline int exynos_tmu_isp_remove_notifier(struct notifier_block *n)
{
	return 0;
}
#endif
