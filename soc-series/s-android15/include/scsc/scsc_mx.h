...
#define WLBT_LINK_CHANGE_START 0x0
#define WLBT_LINK_CHANGE_END 0x1
...
struct scsc_service_client {
	...
	/* called when recovery level needs to check in order to decide silent recovery */
	u8 (*check_reset_level)(struct scsc_service_client *client, u8 level);
};
...
int scsc_service_register_link_change_notifier(struct scsc_service *service, struct notifier_block *nb);
int scsc_service_unregister_link_change_notifier(struct scsc_service *service, struct notifier_block *nb);
