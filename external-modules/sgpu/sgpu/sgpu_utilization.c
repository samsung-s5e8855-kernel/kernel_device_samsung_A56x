...
{
	struct utilization_data data = ((struct sgpu_devfreq_data *)stat->private_data)->udata;
...
static int sgpu_dvfs_get_fake_random_utilization(struct devfreq_dev_status *stat, bool interval)
{
	...
	struct utilization_data data = df_data->udata;
	struct utilization_timeinfo *timeinfo = &data.timeinfo[SGPU_TIMEINFO_SW];
	struct devfreq_dev_profile *dp = df_data->devfreq->profile;
...
int sgpu_utilization_job_start(struct devfreq *df, uint32_t job_count, bool cu_job)
{
	...
	struct amdgpu_device *adev = df_data->adev;
	int ret = 0;
	...
	if (cu_job) {
		SGPU_LOG(adev, DMSG_INFO, DMSG_POWER,
				"amdgpu_ib_schedule cu_active_cnt %d, usage_cnt %d",
				data->cu_active, df_data->adev->dev->power.usage_count);
	} else {
		SGPU_LOG(adev, DMSG_INFO, DMSG_POWER,
				"amdgpu_ib_schedule active_cnt %d, usage_cnt %d",
				data->active, df_data->adev->dev->power.usage_count);
	}
...
int sgpu_utilization_job_end(struct devfreq *df, uint32_t job_count, bool cu_job)
{
	...
	if (data->active == 0) {
		...
		SGPU_LOG(adev, DMSG_INFO, DMSG_ETC,
				"amdgpu_fence_process active_cnt %d, usage_cnt %d",
				data->active, df_data->adev->dev->power.usage_count);
	} else if (cu_job && data->cu_active == 0) {
		...
		data->cu_last_time = current_time;
		SGPU_LOG(adev, DMSG_INFO, DMSG_ETC,
				"amdgpu_fence_process cu active_cnt %d, usage_cnt %d",
				data->cu_active, df_data->adev->dev->power.usage_count);
	}
...
int sgpu_utilization_capture(struct devfreq_dev_status *stat)
{
	...
	if (sw_info->total_time - sw_info->prev_total_time >=
	    df_data->valid_time * NSEC_PER_MSEC) {
	...
	SGPU_LOG(adev, DMSG_INFO, DMSG_DVFS, "cur_freq=%8lu, load=%4u",
		 stat->current_frequency,
		 div64_u64(sw_info->busy_time * 100, sw_info->total_time));
...
void sgpu_utilization_trace_before(struct devfreq_dev_status *stat, unsigned long freq)
{
	...
	trace_sgpu_devfreq_monitor(df_data->devfreq, df_data->min_freq,
				   df_data->max_freq,
...
void sgpu_utilization_trace_after(struct devfreq_dev_status *stat, unsigned long freq)
{
	...
	SGPU_LOG(adev, DMSG_INFO, DMSG_DVFS, "min_freq=%8lu, max_freq=%8lu, cur_freq=%8lu",
		 df_data->min_freq, df_data->max_freq,
		 stat->current_frequency);
...
void sgpu_utilization_init(struct utilization_data *udata)
{
	spin_lock_init(&udata->lock);
	udata->active = 0;
	udata->cu_active = 0;
	udata->utilization_src = &utilization_src_info[SGPU_DVFS_SRC_JIFFIES];
}
