...
#define SGPU_BPMD_LAYOUT_MINOR_VERSION 6
 ...
 * @grbm_gfx_index: Value written to GRBM_GFX_INDEX when the register is read
 * @index_register_val: value of the index register
 ...
 * IH Ring Packet
 */
struct sgpu_bpmd_layout_ih_ring_packet {
	struct sgpu_bpmd_layout_packet_header header;
	uint32_t ring_size;
	uint32_t rptr;
	uint32_t wptr;
	uint8_t use_doorbell;
	uint8_t use_bus_addr;
	uint8_t use_write_back;
	uint8_t enabled;
} __attribute__((packed, aligned(1)));
