...
#else
		def = data = RREG32_SOC15(GC, 0, mmCGTT_TCP_CLK_CTRL_0);
		data |= CGTT_TCP_CLK_CTRL_0__SOFT_OVERRIDE_17_MASK;
		if (def != data)
			WREG32_SOC15(GC, 0, mmCGTT_TCP_CLK_CTRL_0, data);
#endif
	}
}
...
#if defined(CONFIG_DRM_SGPU_EXYNOS) && defined(CONFIG_DRM_SGPU_EMULATION_FULL_SOC)
