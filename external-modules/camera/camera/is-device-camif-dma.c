				...
				struct is_camif_dma_data *data)
{
	...
	struct is_camif_wdma *wdma;
	int ret, vc;
	char *name __free(__putname) = __getname();
	size_t name_len;
	struct device_node *module_node __free(device_node)
				= of_parse_phandle(node, "module", 0);
...
static int is_camif_wdma_remove(struct platform_device *pdev,
				struct is_camif_dma_data *data)
{
	return 0;
}
...
static int is_camif_wdma_module_remove(struct platform_device *pdev,
					struct is_camif_dma_data *data)
{
	return 0;
}
...
static struct is_camif_dma_data is_camif_wdma_module = {
	...
	.remove	= is_camif_wdma_module_remove,
};
