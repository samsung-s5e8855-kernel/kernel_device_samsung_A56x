	...
	u32 width, u32 height, u32 bpp)
{
	...
	sd = (struct pablo_internal_subdev *)crta_bufmgr->priv;
...
static void __pablo_crta_bufmgr_free_buf(struct pablo_internal_subdev *sd)
{
...
static int pablo_crta_bufmgr_open(struct pablo_crta_bufmgr *crta_bufmgr)
{
	...
	/* alloc priv */
	sd = vzalloc(sizeof(struct pablo_internal_subdev));
	if (!sd) {
		err_adt("failed to alloc pablo_internal_subdev");
		return -ENOMEM;
	}
	crta_bufmgr->priv = (void *)sd;

	if (is_hw_get_crta_buf_size(crta_bufmgr->type, &width, &height, &bpp))
		return -ENODEV;
...
static int pablo_crta_bufmgr_close(struct pablo_crta_bufmgr *crta_bufmgr)
{
	...
	if (test_bit(PABLO_SUBDEV_ALLOC, &sd->state)) {
		...
		__pablo_crta_bufmgr_free_buf(sd);
	}
...
err_alloc:
	while (buf_type-- > PABLO_CRTA_BUF_BASE)
		vfree(__crta_bufmgr[buf_type]);

	return -ENOMEM;
}

void pablo_crta_bufmgr_remove(void)
{
	u32 buf_type;

	for (buf_type = 0; buf_type < PABLO_CRTA_BUF_MAX; buf_type++) {
		vfree(__crta_bufmgr[buf_type]);
		__crta_bufmgr[buf_type] = NULL;
	}

	if (__pb_ss_static) {
		CALL_VOID_BUFOP(__pb_ss_static, detach_ext, __pb_ss_static);
		CALL_VOID_BUFOP(__pb_ss_static, free, __pb_ss_static);
		__pb_ss_static = NULL;
	}
}
