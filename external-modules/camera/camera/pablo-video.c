	...
	const struct v4l2_ioctl_ops *ioctl_ops)
{
	...
	video->vd.minor		= -1;
	video->vd.release	= video_device_release_empty;
...
static inline void is_video_remove(struct is_video *video)
{
	video_unregister_device(&video->vd);
}
...
static int pablo_video_probe(struct platform_device *pdev)
{
	...
	if (!is_get_is_dev()) {
		warn("is_dev is not yet probed(video)");
		return -EPROBE_DEFER;
	}
...
static int pablo_video_remove(struct platform_device *pdev)
{
	struct device *dev = &pdev->dev;
	struct is_video *iv = (struct is_video *)platform_get_drvdata(pdev);

	is_video_remove(iv);

	dev_info(dev, "removing %s", iv->vd.name);
