...
static int __is_resource_cam_pwr_notifier(bool on)
{
	int ret;
...
static int is_resource_cam_pwr_notifier(struct is_core *core, bool on)
{
	int ret;
	...
	/*
	 * In kernel 6.6, NPU cannot register camera notifier
	 * because camera is moved to external-module driver
	 */
	if (PKV_VER_GE(6, 6, 0)) {
		ret = npu_cam_notify_on_off(on);
		if (ret)
			err("npu_cam_notify_on_off(%s) is fail (%d)", on ? "true" : "false", ret);
	} else {
		ret = core->noti_register_state ? __is_resource_cam_pwr_notifier(on) : 0;
		if (ret)
			err("__is_resource_cam_pwr_notifier is fail (%d)", ret);
	}
...
static int is_tmu_notifier(struct notifier_block *nb, unsigned long state, void *data)
{
	...
	case ISP_THROTTLING:
		...
		if (fps == ISP_FPS_INVALID) {
			warn("[RSC] THROTTLING : Invalid FPS, use Unlimited FPS");
			fps = 60;
		}
...
static int pablo_resource_dump_per_core(void)
{
...
static void pablo_rscmgr_deinit_rmem(struct is_resourcemgr *rscmgr, struct device_node *np)
{
	struct of_phandle_iterator i;
	...
	while (!of_phandle_iterator_next(&i)) {
		...
		if (!strcmp(i.node->name, "camera_rmem"))
			pablo_rscmgr_deinit_log_rmem(rscmgr);
		else if (!strcmp(i.node->name, "camera_ddk") || !strcmp(i.node->name, "camera-bin"))
			pablo_lib_deinit_reserved_mem();
	}
}
			 ...
			 struct platform_device *pdev)
{
	...
	is_debug_register_dump_func(is_resource_dump);
	...
	INIT_LIST_HEAD(&resourcemgr->share_resource_list);
...
void pablo_resourcemgr_remove(
	struct is_resourcemgr *resourcemgr, void *private_data, struct platform_device *pdev)
{
	struct device_node *np = pdev->dev.of_node;

#ifdef ENABLE_KERNEL_LOG_DUMP
#if IS_ENABLED(CONFIG_EXYNOS_SNAPSHOT)
	kfree(resourcemgr->kernel_log_buf);
#elif IS_ENABLED(CONFIG_DEBUG_SNAPSHOT)
	kfree(resourcemgr->kernel_log_buf);
#endif
#endif
	unregister_reboot_notifier(&notify_reboot_block);
	atomic_notifier_chain_unregister(&panic_notifier_list, &notify_panic_block);

	pablo_dvfs_deinit(&resourcemgr->dvfs_ctrl);

	pablo_crta_bufmgr_remove();

	pablo_resourcemgr_deinit_mem(resourcemgr);

	pablo_rscmgr_deinit_rmem(resourcemgr, np);

#if IS_ENABLED(CONFIG_EXYNOS_THERMAL) || IS_ENABLED(CONFIG_EXYNOS_THERMAL_V2)
#if IS_ENABLED(CONFIG_PABLO_V13_0_0)
	exynos_tmu_isp_remove_notifier(&resourcemgr->tmu_notifier);
#endif
#endif

#if IS_ENABLED(CONFIG_EXYNOS_ITMON)
	itmon_notifier_chain_unregister(&resourcemgr->itmon_notifier);
#endif
}
...
static int pablo_resource_get_ischain_type(struct is_resourcemgr *resourcemgr)
{
	...
	ret = is_resource_cam_pwr_notifier(core, true);
	if (ret) {
...
static void pablo_resource_put_ischain_type(struct is_resourcemgr *resourcemgr)
{
	...
	ret = is_resource_cam_pwr_notifier(core, false);
	if (ret)
		err("is_resource_cam_pwr_notifier is fail(%d)", ret);
