...
static void mdbgv_video(struct is_video_ctx *ivc, struct is_video *iv, const char *msg)
{
	if (iv->device_type == IS_DEVICE_ISCHAIN)
		mdbgv_ischain("%s\n", ivc, msg);
	else
...
static int is_queue_set_format_mplane(struct is_video_ctx *vctx, struct is_queue *queue,
				      void *device, struct v4l2_format *format)
{
	int ret = 0;
	u32 plane;
	...
	mvinfo("[%s]pixelformat(%c%c%c%c) bit(%d) size(%dx%d) flag(0x%x)\n", vctx, video,
	       queue->name, (char)((fmt->pixelformat >> 0) & 0xFF),
	       (char)((fmt->pixelformat >> 8) & 0xFF), (char)((fmt->pixelformat >> 16) & 0xFF),
	       (char)((fmt->pixelformat >> 24) & 0xFF), queue->framecfg.format->hw_bitwidth,
	       queue->framecfg.width, queue->framecfg.height, queue->framecfg.hw_pixeltype);
p_err:
	return ret;
}
...
int is_vidioc_querycap(struct file *file, void *fh, struct v4l2_capability *cap)
{
	...
	if (iv->video_type == IS_VIDEO_TYPE_LEADER)
		cap->capabilities |=
			V4L2_CAP_STREAMING | V4L2_CAP_VIDEO_OUTPUT | V4L2_CAP_VIDEO_OUTPUT_MPLANE;
	else
		cap->capabilities |=
			V4L2_CAP_STREAMING | V4L2_CAP_VIDEO_CAPTURE | V4L2_CAP_VIDEO_CAPTURE_MPLANE;
...
static int queue_init(void *priv, struct vb2_queue *vbq, struct vb2_queue *vbq_dst)
{
	int ret = 0;
...
int is_vidioc_s_fmt_mplane(struct file *file, void *fh, struct v4l2_format *f)
{
	...
	if (iv->video_type == IS_VIDEO_TYPE_LEADER)
		condition = BIT(IS_VIDEO_S_INPUT) | BIT(IS_VIDEO_S_FORMAT) | BIT(IS_VIDEO_S_BUFS);
	else
		condition = BIT(IS_VIDEO_S_INPUT) | BIT(IS_VIDEO_S_BUFS) | BIT(IS_VIDEO_S_FORMAT) |
			    BIT(IS_VIDEO_OPEN) | BIT(IS_VIDEO_STOP);
	...
	mdbgv_vid("set_format(%d x %d)\n", ivc->queue->framecfg.width, ivc->queue->framecfg.height);
...
int is_vidioc_reqbufs(struct file *file, void *fh, struct v4l2_requestbuffers *b)
{
	...
	if (iv->video_type == IS_VIDEO_TYPE_CAPTURE) {
		if (iv->device_type == IS_DEVICE_ISCHAIN) {
			...
			if (leader && test_bit(IS_SUBDEV_START, &leader->state)) {
				merr("leader%d still running, subdev%d req is not applied", idi,
				     leader->id, ivc->subdev->id);
				return -EBUSY;
			}
		} else {
	...
	if (!(ivc->state & (BIT(IS_VIDEO_S_FORMAT) | BIT(IS_VIDEO_STOP) | BIT(IS_VIDEO_S_BUFS)))) {
		mverr("invalid state(%lX)", ivc, iv, ivc->state);
		return -EINVAL;
	}
...
int is_vidioc_streamoff(struct file *file, void *fh, enum v4l2_buf_type i)
{
	...
	if (rcnt + pcnt + ccnt > 0)
		mvwarn("streamoff: queued buffer is not empty(R%d, P%d, C%d)", ivc, iv, rcnt, pcnt,
		       ccnt);
...
int is_vidioc_s_input(struct file *file, void *fs, unsigned int i)
{
	...
	if (iv->video_type == IS_VIDEO_TYPE_LEADER) {
		...
		if (iv->device_type == IS_DEVICE_ISCHAIN) {
			mdbgv_ischain("%s(input: %08X) - stream: %d, position: %d, "
				      "intype: %d, leader: %d\n",
				      ivc, __func__, i, stream, position, intype, leader);

			ret = is_ischain_group_s_input(ivc->device, ivc->group, stream, position,
						       intype, leader);
			if (ret) {
			...
			mdbgv_sensor("%s(input: %08X) - scenario: %d, stream: %d, position: %d, "
				     "intype: %d, leader: %d\n",
				     ivc, __func__, scenario, i, stream, position, intype, leader);
	...
	if (!(ivc->state & (BIT(IS_VIDEO_OPEN) | BIT(IS_VIDEO_S_INPUT) | BIT(IS_VIDEO_S_BUFS)))) {
		mverr("invalid state(%lX)", ivc, iv, ivc->state);
		return -EINVAL;
	}
...
int is_vidioc_g_ctrl(struct file *file, void *fh, struct v4l2_control *a)
{
	...
	case V4L2_CID_IS_END_OF_STREAM:
		...
		break;
	case V4L2_CID_IS_G_EXYNOS_REVISION:
#if !IS_ENABLED(CONFIG_ARCH_VELOCE_HYCON)
		a->value = exynos_soc_info.revision;
#endif
		break;
	default:
...
static int is_g_ctrl_capture_meta(struct file *file, struct v4l2_ext_control *ext_ctrl)
{
	...
	if (IS_ERR_OR_NULL(dma_buf)) {
		err("Failed to get dmabuf. fd %d ret %ld", dma_buf_fd, PTR_ERR(dma_buf));
		ret = -EINVAL;
		goto err_get_dbuf;
	}
	...
	mdbgv_ischain("%s: request capture meta update. fcount(%d), size(%d), addr(%lx)\n", ivc,
		      __func__, fcount, size, cap_shot_kva);

	ret = is_ischain_g_ddk_capture_meta_update(group, device, fcount, size,
						   (ulong)cap_shot_kva);
	if (ret) {
...
static int is_s_ctrl_setfile(struct file *file, struct v4l2_control *a)
{
	...
	mvinfo("[S_CTRL] setfile(%d), scenario(%d)\n", idi, iv, idi->setfile & IS_SETFILE_MASK,
	       scenario);
...
static void is_s_ctrl_dvfs_scenario(struct file *file, struct v4l2_control *a)
{
	...
	mvinfo("[S_CTRL][DVFS] value(%x), common(%d), vendor(%d)\n", ivc, iv,
	       ...
	       dvfs_ctrl->dvfs_scenario & IS_DVFS_SCENARIO_COMMON_MODE_MASK, vendor);
}
...
int is_vidioc_s_ctrl(struct file *file, void *fh, struct v4l2_control *a)
{
	...
	case V4L2_CID_IS_DVFS_LOCK:
		idi = GET_DEVICE_ISCHAIN(ivc);
		if (idi) {
			is_pm_qos_add_request(&dvfs_ctrl->user_qos, PM_QOS_DEVICE_THROUGHPUT,
					      a->value);
	...
	case V4L2_CID_IS_HF_MAX_SIZE:
		...
		mvinfo("[S_CTRL] max buffer size: %d x %d (0x%08X)", ivc, iv,
		       core->vendor.isp_max_width, core->vendor.isp_max_height, a->value);
		break;
	case V4L2_CID_IS_YUV_MAX_SIZE:
		...
		if (idi) {
			...
			mvinfo("[S_CTRL] max yuv buffer size: %d x %d (0x%08X)", ivc, iv,
			       idi->yuv_max_width, idi->yuv_max_height, a->value);
		}
		break;
	...
	case V4L2_CID_IS_CHANGE_CHAIN_PATH: /* Change OTF HW path */
		ret = CALL_GROUP_OPS(ivc->group, change_chain, a->value);
		if (ret)
			mierr("is_group_change_chain is fail(%d)", instance, ret);
		break;
...
static int pablo_s_ext_ctrl_nfd_data(struct file *fp, struct v4l2_ext_control *e)
{
	...
	if (access_ok(e->ptr, sizeof(struct nfd_info))) {
		...
		ret = __copy_from_user_inatomic(nfd_data, e->ptr, sizeof(struct nfd_info));
		if (ret) {
...
static int pablo_s_ext_ctrl_od_data(struct file *fp, struct v4l2_ext_control *e)
{
	...
	if (access_ok(e->ptr, sizeof(struct od_info))) {
		...
		ret = __copy_from_user_inatomic(od_data, e->ptr, sizeof(struct od_info));
		if (ret) {
...
static int pablo_s_ext_ctrl_capture_intent_info(struct file *fp, struct v4l2_ext_control *e)
{
	...
	mginfo("[S_CTRL] capture intent: intent(%d) count(%d) EV(%d) remaincount(%d)\n", head, head,
	       info.captureIntent, info.captureCount, info.captureEV,
	       head->remainCaptureIntentCount);
...
static int is_ssx_video_check_3aa_state(struct is_device_sensor *device, u32 next_id)
{
	...
	if (next_id >= (GROUP_ID_3AA_MAX - GROUP_ID_3AA0)) {
		merr("The next_id is too big.(next_id: %d)", device, next_id);
		ret = -ENODEV;
		goto p_err;
	}

	if (test_bit(IS_SENSOR_FRONT_START, &device->state)) {
		merr("current sensor is not in stop state", device);
		ret = -EBUSY;
		goto p_err;
	}
