...
static int parse_qos_table(struct is_dvfs_ctrl *dvfs_ctrl, struct device_node *np)
{
	...
	for_each_child_of_node(np, next) {
		...
		elems_lv = of_property_count_u32_elems(next, "lev");
		if (elems_lv >= IS_DVFS_LV_END) {
			probe_warn("too many elements");
			continue;
		}

		elems_path = of_property_count_u32_elems(next, "path");
		for (path = 0; path < elems_path; ++path) {
			of_property_read_u32_index(next, "path", path, &dvfs_path);
			set_bit(dvfs_path, &dvfs_ctrl->dvfs_info.qos_otf[dvfs_typ]);
		}
		dvfs_ctrl->dvfs_info.qos_name[dvfs_typ] = next->name;

		for (lv = 0; lv < elems_lv; lv++) {
...
static void free_phy_ldos(struct regulator **ldos, int num)
{
	int i;
...
int is_chain_dev_parse_dt(struct platform_device *pdev)
{
	...
	of_property_read_u32(np, "num_of_mtnr0", &pdata->num_of_ip.mtnr0);
	of_property_read_u32(np, "num_of_mtnr1", &pdata->num_of_ip.mtnr1);
	...
	vendor_np = of_get_child_by_name(np, "vendor");
	if (vendor_np) {
		ret = is_vendor_dt(vendor_np);
		if (ret)
...
void is_chain_dev_free_dt(struct platform_device *pdev)
{
	struct is_core *core = dev_get_drvdata(&pdev->dev);
	struct device *dev = &pdev->dev;
	struct is_bts_scen *bts_scen = core->resourcemgr.bts_scen;
	const char **name = core->resourcemgr.itmon_port_name;

	free_phy_ldos(core->resourcemgr.phy_ldos, core->resourcemgr.num_phy_ldos);
	kfree(name);
	kfree(bts_scen);
	kfree(dev_get_platdata(dev));
}
...
void is_sensor_dev_free_dt(struct platform_device *pdev)
{
	struct device *dev = &pdev->dev;
		...
		struct exynos_platform_is_module *pdata, struct is_sensor_cfg *cfg)
{
	...
	if (opt_np) {
		...
		if (of_find_property(opt_np, "hw_bns", NULL))
			of_property_read_u32(opt_np, "hw_bns", &cfg->hw_bns);
		...
		if (of_find_property(opt_np, "fid_loc", NULL)) {
			...
			of_property_read_u32_index(opt_np, "fid_loc", 2, &cfg->fid_loc.nfi_line);
			of_property_read_u32_index(opt_np, "fid_loc", 3, &cfg->fid_loc.nfi_byte);
			cfg->fid_loc.valid = 1;
		}
				     ...
				     u32 idx_mode, u32 idx_dma)
{
	...
	for (idx_link_vc = 0; idx_link_vc < CSI_VIRTUAL_CH_MAX; idx_link_vc++) {
		...
		cfg->dma_idx[idx_link_vc] = idx_dma;
		cfg->dma_vc[idx_link_vc] = idx_dma_vc;
...
static bool check_available_rsc_share(struct exynos_sensor_pin *pin)
{
	bool is_auto_share_available = true;
	struct is_core *core = is_get_is_core();
	struct is_vendor_private *vendor_priv = core->vendor.private_data;

	if (pin->act <= PIN_NONE || pin->act > PIN_REGULATOR_VOLTAGE
		|| vendor_priv->use_manual_share_rsc)
		is_auto_share_available = false;

	return is_auto_share_available;
}

static void init_resource_share(struct is_resource_share *is_resource_share,
	u32 id, char *name)
{
	struct is_core *core = is_get_is_core();

	is_resource_share = kzalloc(sizeof(struct is_resource_share), GFP_KERNEL);
	if (!is_resource_share) {
		err("failed to alloc is_resource_share");
		return;
	}

	strcpy(is_resource_share->name, name);
	list_add_tail(&is_resource_share->list, &core->resourcemgr.share_resource_list);
	info("[%s]is registered in resource_share_list", is_resource_share->name);
	is_resource_share->init_sensor_id = id;
	is_resource_share->shared_rsc_lock = NULL;
	is_resource_share->shared_rsc_count = NULL;
}

void is_rename_shared_rsc(struct exynos_sensor_pin *pin,
	struct exynos_platform_is_module *pdata, char *rename)
{
	if (!check_available_rsc_share(pin))
		return;
	...
	case PIN_INPUT:
	case PIN_RESET:
		snprintf(rename, RESOURSE_NAME_LEN, "%s_%lu", pin->name, pin->pin);
		break;
	case PIN_FUNCTION:
	case PIN_MCLK:
		snprintf(rename, RESOURSE_NAME_LEN, "%s_%u", pin->name, pdata->mclk_ch);
		break;
	case PIN_I2C:
		snprintf(rename, RESOURSE_NAME_LEN, "%s_%u", pin->name, pdata->sensor_i2c_ch);
		break;
	default:
		snprintf(rename, RESOURSE_NAME_LEN, "%s", pin->name);
		break;
	}
}

int is_check_shared_rsc(struct exynos_sensor_pin *pin,
	struct exynos_platform_is_module *pdata)
{
	struct is_core *core = is_get_is_core();
	struct is_resource_share *is_resource_share;
	bool reg_cap = true;
	char name[RESOURSE_NAME_LEN] = {0};

	if (!check_available_rsc_share(pin))
		return 0;

	is_rename_shared_rsc(pin, pdata, name);

	list_for_each_entry(is_resource_share, &core->resourcemgr.share_resource_list, list) {
		if (!strcmp(is_resource_share->name, name)) {
			reg_cap = false;

			if (!is_resource_share->shared_rsc_lock
				&& is_resource_share->init_sensor_id != pdata->id) {

				is_resource_share->shared_rsc_lock
					= &core->shared_rsc_lock[core->shared_rsc_idx];
				is_resource_share->shared_rsc_count
					= &core->shared_rsc_count[core->shared_rsc_idx];

				is_resource_share->is_shared = true;

				info("[%s] share regulator(%s), share_idx(%d)\n",
					__func__,
					is_resource_share->name,
					core->shared_rsc_idx);
				core->shared_rsc_idx++;

				if (core->shared_rsc_idx == MAX_SENSOR_SHARED_RSC)
					panic("Exceed max sensor share_idx(%d)", core->shared_rsc_idx);
			}
			break;
		}
	}
	...
	if (reg_cap)
		init_resource_share(is_resource_share, pdata->id, name);
			...
			if (pin->act == PIN_REGULATOR) {
				...
				if (reg_cap) {
					...
					if (IS_ERR_OR_NULL(regulator)) {
						err("failed to get regulator : %s",
							(pin->name ? pin->name : "null"));
						kfree(node_table);
						return -EPROBE_DEFER;
					}
					...
					pdata->module_regulator = is_module_regulator;
...
void is_sensor_module_free_dt(struct device *dev)
{
	struct exynos_platform_is_module *pdata = dev_get_platdata(dev);

	kfree(pdata->module_regulator);
	kfree(pdata->cfg);
	kfree(pdata);
}
