...
void pablo_icpu_debug_trace_dump(void)
{
	u32 size = __profile.size;
	void *kva = __profile.kva;
	...
	if (!__profile.outfile) {
		__profile.outfile = debugfs_create_blob("dbg15_out", 0400,
	...
	if (__profile.blob.data == NULL) {
		__profile.blob.data = vmalloc(size);
		if (!__profile.blob.data) {
			ICPU_ERR("blob->data alloc failed, size(%d)", size);
			return;
		}
		__profile.blob.size = size;
	}

	memcpy(__profile.blob.data, kva, size);

	ICPU_INFO("dbg15 dump done!");
}

bool pablo_icpu_debug_trace_enabled(void)
{
	return icpu_fw_trace_dbg;
}

void pablo_icpu_debug_trace_dump_info(void *kva, u32 size)
{
	__profile.kva = kva;
	__profile.size = size;
}
...
static int __set_fw_trace_dbg(const char *val, const struct kernel_param *kp)
{
	int ret;
	long new;
	...
	ret = kstrtol(val, 0, &new);
	if (ret) {
		ICPU_ERR("check size, ret(%d)", ret);
		return -EINVAL;
	}
	...
	if (icpu_fw_trace_dbg && !new)
		pablo_icpu_debug_trace_dump();

	icpu_fw_trace_dbg = new;

	return 0;
}

static int __get_fw_trace_dbg(char *buffer, const struct kernel_param *kp)
{
	int len;
...
static const struct kernel_param_ops __fw_trace_param_ops = {
	.set = __set_fw_trace_dbg,
	.get = __get_fw_trace_dbg,
};
module_param_cb(icpu_fw_trace, &__fw_trace_param_ops, NULL, 0644);
