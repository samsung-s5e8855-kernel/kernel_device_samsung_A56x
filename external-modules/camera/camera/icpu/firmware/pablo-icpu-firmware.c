...
static struct icpu_firmware {
	...
	u32 bin_size_max;
	bool aarch64;
	struct icpu_firmware_mem_desc mem_desc[ICPU_FW_MEM_MAX];
...
static int __alloc_firmware_mem(enum icpu_firmware_mem_id id)
{
	...
	if (id >= ICPU_FW_MEM_MAX) {
		ICPU_ERR("id(%d) is not valid", id);
		return -EINVAL;
	}

	desc = &_icpu_fw.mem_desc[id];
	if (!desc->size) {
		ICPU_WARN("id(%d) description is not valid, please check dtsi, skip!", id);
		return 0;
	}
...
static enum icpu_firmware_mem_id __get_secure_mem_id(void)
{
	...
	if (_icpu_fw.mem_desc[ICPU_FW_MEM_SECURE_HEAP].size)
...
{
	return __get_buf(ICPU_FW_MEM_DBG);
}
...
static int __load_firmware_heap(bool secure_mode)
{
	...
	if (pablo_icpu_debug_trace_enabled()) {
		ret = __alloc_firmware_mem(ICPU_FW_MEM_DBG);
		if (ret) {
			ICPU_ERR("Fail to alloc log dbg, ret(%d)", ret);
			return ret;
		}
	}
