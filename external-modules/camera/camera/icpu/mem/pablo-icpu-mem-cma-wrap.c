...
int __iommu_map_cma(struct device *dev, struct icpu_cma_buf *fw_buf)
{
	...
	fw_buf->map_size = pkv_iommu_map_sg(iommu_get_domain_for_dev(dev), reserved_base,
			fw_buf->sgt->sgl,
...
#define ICPU_CMA_RETRY_CNT 100
static int _icpu_mem_cma_alloc(struct device *dev, struct icpu_buf *special_buf)
{
	...
	while (retry_count <= ICPU_CMA_RETRY_CNT) {
		ICPU_INFO_IF(!retry_count, "cma_alloc: E");
		fw_pages = cma_alloc(buf->cma_area, nr_pages, align, false);
		ICPU_INFO_IF(!retry_count, "cma_alloc: X");
...
static void _icpu_mem_cma_free(struct icpu_buf *buf)
{
...
struct icpu_buf_ops icpu_buf_ops_cma = {
	.alloc = _icpu_mem_cma_alloc,
	.free = _icpu_mem_cma_free,
	.sync_for_device = _icpu_mem_cma_sync_for_device,
};
