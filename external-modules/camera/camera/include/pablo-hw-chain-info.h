...
enum hw_slot_scenario {
	HW_SLOT_SCENARIO_NORMAL = 0,
	HW_SLOT_SCENARIO_2NR = 1,
};
...
struct pablo_hw_chain_info_ops {
	...
	bool (*check_crta_hw)(struct is_hardware *hw, u32 hw_id);
	int (*get_pdaf_buf_num)(struct is_hardware *hw, int max_target_fps);
	bool (*skip_setfile_load)(struct is_hardware *hw, int hw_id);
	int (*get_dynamic_hw_slot)(struct is_hardware *hw, u32 hw_id, u32 scenario);
	bool (*check_mlsc_group)(struct is_hardware *hw, int group_id);
};
