...
#define BYRP_CR_NUM 4000 // Solomon: 1792 + 1650(LUT) = 3442
#define RGBP_CR_NUM 5000 // Solomon: 1818 + 1650(LUT) + 825(LUT) = 4293
#define YUVSC_CR_NUM 500 // Solomon: 447
#define MLSC_CR_NUM 2000 // Solomon: 1671
...
enum pablo_sensor_control_id {
	...
	SS_CTRL_CIS_REQUEST_SEAMLESS_MODE_CHANGE, /* request_seamless_mode_change */
...
struct is_byrp_config {
	...
	u32 thstat_pre_bypass;
	u32 thstat_pre_grid_w;
	u32 thstat_pre_grid_h;

	u32 thstat_awb_bypass;
	u32 thstat_awb_grid_w;
	u32 thstat_awb_grid_h;

	u32 thstat_ae_bypass;
	u32 thstat_ae_grid_w;
	u32 thstat_ae_grid_h;

	u32 rgbyhist_bypass;
	u32 rgbyhist_bin_num;
	u32 rgbyhist_hist_num;
...
struct is_rgbp_config {
	...
	u32 thstat_awb_hdr_bypass;
	u32 thstat_awb_hdr_grid_w;
	u32 thstat_awb_hdr_grid_h;

	u32 rgbyhist_hdr_bypass;
	u32 rgbyhist_hdr_bin_num;
	u32 rgbyhist_hdr_hist_num;
...
struct pablo_rta_frame_info {
	...
	struct pablo_area byrp_crop_in;
	u32 byrpZslBit;
	...
	/* YUVSC */
	struct pablo_size yuvsc_output_size;

	/* Buffer available */
	u32 rgbp_out_drcclct_buffer;
	u32 mlsc_out_svhist_buffer;
	u32 mlsc_out_meds_buffer;
...
struct pablo_crta_frame_info {
	...
	/* config */
	struct is_byrp_config byrp_cfg;
	struct is_rgbp_config rgbp_cfg;
	struct is_yuvsc_config yuvsc_cfg;
	struct is_mlsc_config mlsc_cfg;

	/* reg */
	u32 pdp_cr_size;
	struct pablo_cr_set pdp_cr[PDP_CR_NUM];
	u32 byrp_cr_size;
	struct pablo_cr_set byrp_cr[BYRP_CR_NUM];
	u32 rgbp_cr_size;
	struct pablo_cr_set rgbp_cr[RGBP_CR_NUM];
	u32 yuvsc_cr_size;
	struct pablo_cr_set yuvsc_cr[YUVSC_CR_NUM];
	u32 mlsc_cr_size;
	struct pablo_cr_set mlsc_cr[MLSC_CR_NUM];
