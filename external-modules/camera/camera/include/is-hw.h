...
#define IS_MAX_SCENARIO             (140)
#define IS_MAX_SETFILE              (140)
/*
 ...
 *         |_|      |_||     |_|           |
 *                     |___________________|
 * @HW_OTF : setbit @ set_hw_map / clearbit @ set_hw_map
 *           define HW working domain.
 *           In OTF domain, HW uses the controlling concept based on Sensor OTF input.
 */
enum is_hw_state {
	HW_OPEN,
	HW_INIT,
...
enum is_hardware_id {
	...
	DEV_HW_LME1,
	DEV_HW_BYRP,
	DEV_HW_BYRP0 = DEV_HW_BYRP,
	DEV_HW_BYRP1,
	DEV_HW_BYRP2,
	DEV_HW_BYRP3,
	DEV_HW_BYRP4,
	DEV_HW_BYRP_MAX = DEV_HW_BYRP4,
	DEV_HW_RGBP,
	DEV_HW_RGBP0 = DEV_HW_RGBP,
	DEV_HW_RGBP1,
	DEV_HW_RGBP2,
	DEV_HW_RGBP3,
	DEV_HW_RGBP_MAX = DEV_HW_RGBP3,
	DEV_HW_MCFP,
	DEV_HW_ORB0,
	DEV_HW_ORB1,
	DEV_HW_DLFE,
	...
	DEV_HW_YUVSC2,
	DEV_HW_YUVSC3,
	DEV_HW_YUVSC_MAX = DEV_HW_YUVSC3,
	DEV_HW_MLSC0,
	DEV_HW_MLSC1,
	DEV_HW_MLSC2,
	DEV_HW_MLSC3,
	DEV_HW_MLSC_MAX = DEV_HW_MLSC3,
	DEV_HW_MTNR0,
	DEV_HW_MTNR1,
	DEV_HW_MSNR,
	DEV_HW_END
...
struct is_hw_ip {
	...
	struct is_hw_ip *locomotive;

	void (*remove)(struct is_hw_ip *hw_ip);
};
...
struct is_hw_ip_ops {
	...
	int (*cmp_fcount)(struct is_hw_ip *hw_ip, u32 fcount);
	size_t (*dump_params)(struct is_hw_ip *hw_ip, u32 instance, char *buf, size_t size);
	int (*recover)(struct is_hw_ip *hw_ip, u32 fcount);
};

/* query types */
enum pablo_query_type {
	PABLO_QUERY_GET_PCFI,
	PABLO_QUERY_GET_EDGE_SCORE,
};
...
typedef void (*hw_ip_remove_fn_t)(struct is_hw_ip *hw_ip);
...
struct is_hardware_ops {
	...
	int (*clear_crc_status)(u32 instance, struct is_hw_ip *hw_ip);
	void (*sfr_dump)(struct is_hardware *hardware, u32 hw_id, bool flag_print_log);
};
...
struct is_hardware {
	...
	/* for test double */
	void (*fake_m2m_frame_start)(struct is_hw_ip *hw_ip, u32 instance);
	void (*fake_m2m_frame_end)(struct is_hw_ip *hw_ip, u32 instance);
	void (*fake_otf_frame_start)(struct is_hw_ip *hw_ip, u32 instance);
	void (*fake_otf_frame_end)(u32 *hw_slot_id, u32 instance);
};
...
#define IS_PRINT_MULTI_TARGET_DVA(TARGET) \
	if (leader_frame->TARGET[j][i]) \
		pr_err("[@][%d] %s[%d][%d]: %pad\n", \
			instance, #TARGET, j, i, &leader_frame->TARGET[j][i]);
...
void is_hw_check_iteration_state(
	struct is_frame *frame, struct is_dvfs_ctrl *dvfs_ctrl, struct is_group *group);
