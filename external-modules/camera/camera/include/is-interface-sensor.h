...
enum mf_hdr_mode {
	MF_HDR_MODE_2AEB_2VC = 0,
};

struct is_sensor_seamless_ctl {
	u32 mode; // each bit is controlled with pablo_sensor_mode enum

	enum sf_hdr_mode sf_hdr_mode;
	enum mf_hdr_mode mf_hdr_mode;
	u32 remosaic_crop_zoom_ratio;
...
enum is_rta_sensor_mode {
	...
	SENSOR_MODE_AEB = (1 << 5),
	SENSOR_MODE_ADC11 = (1 << 6),
	SENSOR_MODE_MAX,
};

struct is_sensor_mode_info {
	u32 mode;
	u32 extra[3]; /* write in param order corresponding to enabled bit information in mode variable */
	u32 min_expo;
	u32 max_expo;
	u32 min_again;
	u32 max_again;
...
struct crop_shift_info {
	int32_t crop_shift_x;
	int32_t crop_shift_y;
	int32_t crop_shift_sign;
};
...
typedef struct {
	...
	unsigned int pre_frame_length_lines;
	unsigned int frame_time; // unit: ms
	unsigned int fll_long;
	unsigned int fll_short;

	/** The length of a line is specified as a number of pixel clocks, line_length_pck. */
	unsigned int line_length_pck;
	unsigned int pre_line_length_pck;
	unsigned int line_length_inck;
	...
	unsigned int base_min_frame_us_time;
	...
	bool use_fast_first_frame;
	bool use_print_control_log;
	u32 fll_margin_ratio;
	u32 fll_margin_ratio_aeb;
	...
	struct is_sensor_seamless_ctl seamless_ctl_before_stream;
	struct is_sensor_seamless_ctl cur_seamless_ctl;
	struct is_sensor_seamless_ctl seamless_history[NUM_FRAMES];

	struct is_sensor_mode_info nfi_sensor_mode_info[SEAMLESS_MODE_MAX];
	u32 nfi_sensor_mode_cnt;
	bool update_nfi_seamless_mode;
} cis_shared_data;
...
struct is_cis_ops {
	...
	int (*cis_init_state)(struct v4l2_subdev *subdev, int mode);
	int (*cis_mode_change)(struct v4l2_subdev *subdev, u32 mode);
	int (*cis_set_flip_mode)(struct v4l2_subdev *subdev);
	...
	int (*cis_check_cit_contraint)(struct v4l2_subdev *subdev, u16 *cit);
	...
	int (*cis_adjust_cit_for_extremely_br)(struct v4l2_subdev *subdev, u32 *cit);
	...
	void (*cis_get_crop_shift_data)(struct v4l2_subdev *subdev, struct crop_shift_info *data);
#endif
	...
	int (*cis_seamless_ctl_before_stream)(struct v4l2_subdev *subdev,
	...
	int (*cis_update_seamless_history)(struct v4l2_subdev *subdev);
	int (*cis_get_updated_binning_ratio)(struct v4l2_subdev *subdev, u32 fcount, u32 *binning_ratio);
	int (*cis_notify_vsync)(struct v4l2_subdev *subdev);
	int (*cis_read_bpc_otp)(struct v4l2_subdev *subdev, char *bpc_data, u32 *bpc_size);
...
#endif
	int (*ois_set_dev_ctrl)(struct v4l2_subdev *subdev, int forceMode);
...
#endif
	bool (*ois_get_active)(struct v4l2_subdev *subdev);
...
struct is_cis_ext2_interface_ops {
	...
	int (*set_seamless_mode)(struct is_sensor_interface *itf,
		struct is_sensor_seamless_ctl *seamless_mode);
	int (*get_nfi_sensor_mode_info)(struct is_sensor_interface *itf,
		struct is_sensor_mode_info *nfi_sensor_mode_info, u32 *nfi_sensor_mode_cnt);
};
...
struct is_eeprom_ops {
	...
	int (*eeprom_get_cal_buf)(struct v4l2_subdev *subdev, char **buf);
};
