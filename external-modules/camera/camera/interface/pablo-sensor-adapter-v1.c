...
static int __pablo_sensor_adt_request_seamless_mode_change(
	struct is_sensor_interface *sensor_itf, u32 instance, u32 *ctrl_param, u32 *param_idx)
{
	int ret = 0;
	...
	mdbg_adt(2, "[%s] LN2[%d] LN4[%d] ADC11[%d] 12bit[%d] AEB[%d] RM[%d,%d]\n", instance, __func__,
		(seamless_mode.mode & SENSOR_MODE_LN2),
		(seamless_mode.mode & SENSOR_MODE_LN4),
		(seamless_mode.mode & SENSOR_MODE_ADC11),
	...
	ret |= sensor_itf->cis_ext2_itf_ops.set_seamless_mode(sensor_itf, &seamless_mode);
...
static const struct constrol_sensor_fn_table fn_table[SS_CTRL_END] = {
	...
	{ SS_CTRL_CIS_SET_NUM_OF_FRAME_PER_ONE_RTA,
		__pablo_sensor_adt_set_num_of_frame_per_one_3aa },
	...
	{ SS_CTRL_CIS_REQUEST_SENSOR_INFO_MFHDR_MODE_CHANGE,
		__pablo_sensor_adt_set_sensor_info_mfhdr_mode_change },
	{ SS_CTRL_CIS_REQUEST_SEAMLESS_MODE_CHANGE,
		__pablo_sensor_adt_request_seamless_mode_change },
		...
		struct pablo_pdaf_info *buf_info)
{
	...
	wdma_idx = sensor_cfg->dma_idx[stat_vc];
	wdma_vc = sensor_cfg->dma_vc[stat_vc];

	buf_info->sensor_mode = module->vc_extra_info[VC_BUF_DATA_TYPE_GENERAL_STAT1].sensor_mode;
	buf_info->hpd_width = sensor_cfg->input[stat_vc].width;
	buf_info->hpd_height = sensor_cfg->input[stat_vc].height;
	buf_info->hpd_data_size = sensor_cfg->output[wdma_idx][wdma_vc].stride *
				  sensor_cfg->output[wdma_idx][wdma_vc].height;

	for (wdma_vc = DMA_VIRTUAL_CH_0; wdma_vc < DMA_VIRTUAL_CH_MAX; wdma_vc++) {
		if (sensor_cfg->output[wdma_idx][wdma_vc].type == VC_VPDAF) {
			link_vc = sensor_cfg->link_vc[wdma_idx][wdma_vc];

			buf_info->sensor_mode = module->vpd_sensor_mode;
			buf_info->vpd_width = sensor_cfg->input[link_vc].width;
			buf_info->vpd_height = sensor_cfg->input[link_vc].height;
			buf_info->vpd_data_size = sensor_cfg->output[wdma_idx][wdma_vc].stride *
						  sensor_cfg->output[wdma_idx][wdma_vc].height;
		}
	}

	if (sensor_cfg->special_vc_sensor_mode != VC_SENSOR_MODE_INVALID)
		buf_info->sensor_mode = sensor_cfg->special_vc_sensor_mode;
