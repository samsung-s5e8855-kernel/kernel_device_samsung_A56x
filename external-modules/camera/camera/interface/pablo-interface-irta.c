...
int pablo_interface_irta_dma_buf_attach(struct pablo_interface_irta *pii, u32 idx, int fd)
{
	...
	pii->sgt[idx] = pkv_dma_buf_map_attachment(pii->attachment[idx], DMA_BIDIRECTIONAL);
...
err_address:
	pkv_dma_buf_unmap_attachment(pii->attachment[idx], pii->sgt[idx], DMA_BIDIRECTIONAL);
