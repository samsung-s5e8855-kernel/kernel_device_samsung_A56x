...
static struct vm_struct *pablo_bin_vm_rsv;
int pablo_lib_init_reserved_mem(struct reserved_mem *rmem)
{
	...
	pablo_bin_vm_rsv = __get_vm_area_caller(PAGE_ALIGN(rmem->size), 0, LIB_START,
					VMALLOC_END, __builtin_return_address(0));
#else
	pablo_bin_vm_rsv = __get_vm_area(PAGE_ALIGN(rmem->size), 0, LIB_START, VMALLOC_END);
#endif

	if (pablo_bin_vm_rsv->size < LIB_SIZE) {
		probe_err("insufficient bin rmem (0x%lx < 0x%x)", pablo_bin_vm_rsv->size, LIB_SIZE);
		return -ENOMEM;
	}

	pablo_bin_vm.phys_addr = rmem->base;
	pablo_bin_vm.addr = pablo_bin_vm_rsv->addr;
	pablo_bin_vm.size = pablo_bin_vm_rsv->size;
