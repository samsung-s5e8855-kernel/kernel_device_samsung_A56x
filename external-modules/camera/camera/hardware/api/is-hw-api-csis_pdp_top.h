...
#define csis_pdp_top_enable_ibuf_ptrn_gen(base_reg, sensor_cfg, otf_info, clk, fps, on) do_nothing
...
#define csis_pdp_top_nfi_en(csi, en, fid_loc) do_nothing_0
#define csis_pdp_top_nfi_sw_trigger(csi) do_nothing
#define csis_pdp_top_nfi_irq_msk(base_reg, csi_ch, on) do_nothing
#define csis_pdp_top_nfi_irq_src(base_reg, str, end) do_nothing
#endif
...
struct pablo_kunit_hw_csis_pdp_top_func {
	...
	int (*irq_src)(void __iomem *regs);
	void (*nfi_irq_msk)(void __iomem *base_reg, u32 csi_ch, bool on);
	void (*nfi_irq_src)(void __iomem *base_reg, ulong *str, ulong *end);
};
