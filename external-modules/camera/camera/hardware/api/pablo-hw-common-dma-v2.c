...
static int is_hw_dma_set_msb_align(struct is_common_dma *dma, u32 sign, u32 msb_align)
{
	...
	if (sign)
		...
		SET_F(base, DMA_R_DATA_FORMAT, DMA_F_DATA_FORMAT_MSBALIGN_UNSIGN, msb_align);
...
static int is_hw_dma_get_size(struct is_common_dma *dma, u32 *width, u32 *height)
{
	void __iomem *base = __is_hw_dma_get_base_addr(dma);

	*width = GET_R(base, DMA_R_WIDTH);
	*height = GET_R(base, DMA_R_HEIGHT);
...
static int is_hw_dma_set_comp_quality(struct is_common_dma *dma,
	enum lossy_comp_quality_type quality_control)
{
	void __iomem *base = __is_hw_dma_get_base_addr(dma);

	if (quality_control >= LOSSY_COMP_QUALITY_TYPE_MAX) {
		warn_hw("SBWC does not support quality mode :%d", quality_control);
		return DMA_OPS_ERROR;
	}

	/* 0 : Quality prioritzed mode(default), 1: normal mode  2 : footprint mode */
...
static const struct is_common_dma_ops is_hw_dma_ops = {
	...
	.dma_get_size			= is_hw_dma_get_size,
...
static int pmio_dma_set_msb_align(struct is_common_dma *dma, u32 sign, u32 msb_align)
{
	...
	if (sign)
		...
		val = SET_V(val, DMA_F_DATA_FORMAT_MSBALIGN_UNSIGN, msb_align);
...
static int pmio_dma_set_comp_quality(struct is_common_dma *dma,
	enum lossy_comp_quality_type quality_control)
{
	void __iomem *base = __pmio_dma_get_base_addr(dma);
	u32 val;
	...
	/* 0 : Quality prioritzed mode(default), 1: normal mode  2 : footprint mode */
	val = PMIO_GET_R(base, corex_offset +
		dma->reg_ofs + dma_regs[DMA_R_COMP_LOSSY_QUALITY_CONTROL].sfr_offset);
	val = SET_V(val, DMA_F_COMP_LOSSY_QUALITY_CONTROL, quality_control);
	PMIO_SET_R(base, corex_offset +
		dma->reg_ofs + dma_regs[DMA_R_COMP_LOSSY_QUALITY_CONTROL].sfr_offset, val);
...
static int pmio_dma_get_size(struct is_common_dma *dma, u32 *width, u32 *height)
{
	void __iomem *base = __pmio_dma_get_base_addr(dma);
	unsigned int corex_offset = GET_COREX_OFFSET(dma->set_id);

	*width = PMIO_GET_R(base, corex_offset + dma->reg_ofs + dma_regs[DMA_R_WIDTH].sfr_offset);
	*height = PMIO_GET_R(base, corex_offset + dma->reg_ofs + dma_regs[DMA_R_HEIGHT].sfr_offset);
...
static int pmio_dma_set_img_addr(struct is_common_dma *dma, dma_addr_t *addr, u32 plane, int buf_idx, u32 num_buffers)
{
	...
	do {
		...
		dbg_dma(4, "[%s] image addr[%d]: %pad, high: 0x%x, low: 0x%x",
				dma->name, i + buf_idx,
	...
	u32 plane, int buf_idx, u32 num_buffers)
{
	...
	do {
		...
		dbg_dma(4, "[%s] header addr[%d]: %pad, high: 0x%x low: 0x%x",
				dma->name, i + buf_idx,
						&addr[i + buf_idx], dva_high, dva_low);
	} while (++i < num_buffers);
...
const struct is_common_dma_ops pmio_dma_ops = {
	...
	.dma_get_size		= pmio_dma_get_size,
...
{
	return (sbwc_type >> SBWC_LOSSY_FR_SHIFT) ? 1 : 0;
}
EXPORT_SYMBOL_GPL(is_hw_dma_get_comp_sbwc_fr);
			       ...
			       u32 comp_sbwc_en, bool is_msb, u32 *fmt)
{
	...
	if (hw_format == DMA_INOUT_FORMAT_IMG_WITH_WGT) {
		*fmt = DMA_FMT_U16BIT_PACK;
		return 0;
	}
...
u32 is_hw_dma_get_payload_stride(u32 comp_sbwc_en, u32 bit_depth, u32 width,
	u32 comp_align_type, u32 quality_control,
	u32 block_width, u32 block_height)
{
	...
	if (comp_sbwc_en == COMP) {
		...
		if (block_width == 256 && block_height == 1) {
			...
			/* 32x4 */
			if (quality_control == LOSSY_COMP_FOOTPRINT) {
				if (bit_depth < 12) {
					compressed_data = 64;
				} else {
					if (comp_align_type == SBWC_ALIGN_TYPE_32B)
