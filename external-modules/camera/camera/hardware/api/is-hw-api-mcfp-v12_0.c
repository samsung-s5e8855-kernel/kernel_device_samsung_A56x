...
#include "sfr/is-sfr-mcfp-v12_1.h"
#else
...
static void __mcfp_hw_s_int_mask(struct pablo_mmio *base)
{
	...
	MCFP_SET_F(base, MCFP_R_INT_REQ_INT1_ENABLE, MCFP_F_INT_REQ_INT1_ENABLE, MCFP_INT1_EN_MASK);
}
...
#if IS_ENABLED(CONFIG_PABLO_V12_1_0)
	MCFP_SET_R(base, MCFP_R_STAT_CINFIFODECOMP_ENABLE, 0);
	MCFP_SET_R(base, MCFP_R_STAT_COUTFIFODECOMP_ENABLE, 0);
#endif
	...
	struct is_mcfp_config *config)
{
	...
	case MCFP_RDMA_CUR_IN_UV:
		...
		/* MCFP CUR YUV use 12bit only so set msb_align_unsign to 1 */
	...
	*header_offset = strip_header_offset_in_byte;
	...
	dbg_hw(3, "%s : dma_id %d, sbwc(%d) st_ofs %d, width %d, height %d, img_ofs %d, header_ofs %d, stride (%d, %d)\n",
			__func__, dma->id, comp_sbwc_en, strip_start_pos, width, height, *strip_offset, *header_offset,
...
KUNIT_EXPORT_SYMBOL(mcfp_hw_s_block_bypass);

int mcfp_hw_g_block_bypass(struct pablo_mmio *base, u32 get_val[], size_t val_size)
{
	if (val_size < MCFP_BLK_BYPASS_MAX)
		return -EINVAL;

	get_val[MCFP_GEOMATCH_EN] =
		MCFP_GET_F(base, MCFP_R_YUV_GEOMATCH_EN, MCFP_F_YUV_GEOMATCH_EN);
	get_val[MCFP_GEOMATCH_BYPASS] =
		MCFP_GET_F(base, MCFP_R_YUV_GEOMATCH_BYPASS, MCFP_F_YUV_GEOMATCH_BYPASS);
	get_val[MCFP_GEOMATCH_MATCH_ENABLE] = MCFP_GET_F(base, MCFP_R_YUV_GEOMATCH_MATCH_ENABLE,
							 MCFP_F_YUV_GEOMATCH_MATCH_ENABLE);
	get_val[MCFP_YUV422TO444_BYPASS] =
		MCFP_GET_F(base, MCFP_R_YUV_YUV422TO444_BYPASS, MCFP_F_YUV_YUV422TO444_BYPASS);
	get_val[MCFP_YUVTORGB_BYPASS] =
		MCFP_GET_F(base, MCFP_R_YUV_YUVTORGB_BYPASS, MCFP_F_YUV_YUVTORGB_BYPASS);
	get_val[MCFP_GAMMARGB_BYPASS] =
		MCFP_GET_F(base, MCFP_R_RGB_GAMMARGB_BYPASS, MCFP_F_RGB_GAMMARGB_BYPASS);
	get_val[MCFP_RGBTOYUV_BYPASS] =
		MCFP_GET_F(base, MCFP_R_RGB_RGBTOYUV_BYPASS, MCFP_F_RGB_RGBTOYUV_BYPASS);
	get_val[MCFP_YUV444TO422_BYPASS] =
		MCFP_GET_F(base, MCFP_R_YUV_YUV444TO422_BYPASS, MCFP_F_YUV_YUV444TO422_BYPASS);

	return 0;
}
KUNIT_EXPORT_SYMBOL(mcfp_hw_g_block_bypass);
...
{
	MCFP_SET_R(base, MCFP_R_YUV_MAIN_CTRL_HF_DECOMP_EN, enable);

	MCFP_SET_R(base, MCFP_R_STAT_CINFIFODECOMP_ENABLE, enable);
	MCFP_SET_F(base, MCFP_R_IP_USE_OTF_PATH_01, MCFP_F_IP_USE_OTF_IN_FOR_PATH_1, enable);

	MCFP_SET_F(base, MCFP_R_STAT_CINFIFODECOMP_CONFIG,
		MCFP_F_STAT_CINFIFODECOMP_STALL_BEFORE_FRAME_START_EN, enable);
	MCFP_SET_F(base, MCFP_R_STAT_CINFIFODECOMP_CONFIG,
		MCFP_F_STAT_CINFIFODECOMP_AUTO_RECOVERY_EN, 0);
	MCFP_SET_F(base, MCFP_R_STAT_CINFIFODECOMP_CONFIG, MCFP_F_STAT_CINFIFODECOMP_DEBUG_EN, 1);
	MCFP_SET_F(base, MCFP_R_STAT_CINFIFODECOMP_INTERVALS,
		MCFP_F_STAT_CINFIFODECOMP_INTERVAL_HBLANK, HBLANK_CYCLE);
	MCFP_SET_F(base, MCFP_R_STAT_CINFIFODECOMP_INTERVALS,
		MCFP_F_STAT_CINFIFODECOMP_INTERVAL_PIXEL, PBLANK_CYCLE);

	MCFP_SET_R(base, MCFP_R_STAT_COUTFIFODECOMP_ENABLE, enable);
	MCFP_SET_F(base, MCFP_R_STAT_COUTFIFODECOMP_CONFIG, MCFP_F_STAT_COUTFIFODECOMP_VVALID_RISE_AT_FIRST_DATA_EN, 1);
	MCFP_SET_F(base, MCFP_R_STAT_COUTFIFODECOMP_CONFIG, MCFP_F_STAT_COUTFIFODECOMP_DEBUG_EN, 1);

	MCFP_SET_F(base, MCFP_R_IP_USE_OTF_PATH_23, MCFP_F_IP_USE_OTF_OUT_FOR_PATH_3, enable);

	MCFP_SET_F(base, MCFP_R_STAT_COUTFIFODECOMP_INTERVAL_VBLANK,
		MCFP_F_STAT_COUTFIFODECOMP_INTERVAL_VBLANK, VBLANK_CYCLE);
	MCFP_SET_F(base, MCFP_R_STAT_COUTFIFODECOMP_AUTO_VBLANK,
		MCFP_F_STAT_COUTFIFODECOMP_AUTO_VBLANK, 0);
	MCFP_SET_F(base, MCFP_R_STAT_COUTFIFODECOMP_INTERVALS,
		MCFP_F_STAT_COUTFIFODECOMP_INTERVAL_HBLANK, HBLANK_CYCLE);
	MCFP_SET_F(base, MCFP_R_STAT_COUTFIFODECOMP_INTERVALS,
		MCFP_F_STAT_COUTFIFODECOMP_INTERVAL_PIXEL, PBLANK_CYCLE);
}

void mcfp_hw_s_hf_input_size(struct pablo_mmio *base, u32 set_id, u32 width, u32 height)
{
	MCFP_SET_F(base, MCFP_R_YUV_MAIN_CTRL_HF_DECOMP_SZ_WIDTH, MCFP_F_YUV_HF_DECOMP_SZ_WIDTH, width);
	MCFP_SET_F(base, MCFP_R_YUV_MAIN_CTRL_HF_DECOMP_SZ_HEIGHT, MCFP_F_YUV_HF_DECOMP_SZ_HEIGHT, height);
}
#endif
...
#if IS_ENABLED(CONFIG_PABLO_V12_1_0)
	MCFP_SET_F(base, MCFP_R_RGB_GAMMARGB_CRC,
			MCFP_F_RGB_GAMMARGB_CRC_SEED, seed);
	MCFP_SET_F(base, MCFP_R_YUV_CINFIFODLNE_STREAM_CRC,
			MCFP_F_YUV_CINFIFODLNE_CRC_SEED, seed);
	MCFP_SET_F(base, MCFP_R_STAT_CINFIFODLFEWGT_STREAM_CRC,
			MCFP_F_STAT_CINFIFODLFEWGT_CRC_SEED, seed);
	MCFP_SET_F(base, MCFP_R_YUV_DTP_STREAM_CRC,
			MCFP_F_YUV_DTP_CRC_SEED, seed);
	MCFP_SET_F(base, MCFP_R_YUV_COUTFIFODLFEPREV_STREAM_CRC,
			MCFP_F_YUV_COUTFIFODLFEPREV_CRC_SEED, seed);
	MCFP_SET_F(base, MCFP_R_STAT_COUTFIFODECOMP_STREAM_CRC,
			MCFP_F_STAT_COUTFIFODECOMP_CRC_SEED, seed);
#endif
}
...
void mcfp_hw_s_dlfe_path(struct pablo_mmio *base, u32 en)
{
	...
	MCFP_SET_R(base, MCFP_R_YUV_COUTFIFODLFECURR_ENABLE, en);
	MCFP_SET_F(base, MCFP_R_YUV_COUTFIFODLFECURR_CONFIG, MCFP_F_YUV_COUTFIFODLFECURR_VVALID_RISE_AT_FIRST_DATA_EN, en);
	MCFP_SET_F(base, MCFP_R_YUV_COUTFIFODLFECURR_CONFIG, MCFP_F_YUV_COUTFIFODLFECURR_DEBUG_EN, 1);
	MCFP_SET_F(base, MCFP_R_IP_USE_OTF_PATH_01, MCFP_F_IP_USE_OTF_OUT_FOR_PATH_1, en);

	/* Prev YUV COUTFIFO setting */
	MCFP_SET_R(base, MCFP_R_YUV_COUTFIFODLFEPREV_ENABLE, en);
	MCFP_SET_F(base, MCFP_R_YUV_COUTFIFODLFEPREV_CONFIG, MCFP_F_YUV_COUTFIFODLFEPREV_VVALID_RISE_AT_FIRST_DATA_EN, en);
	MCFP_SET_F(base, MCFP_R_YUV_COUTFIFODLFEPREV_CONFIG, MCFP_F_YUV_COUTFIFODLFEPREV_DEBUG_EN, 1);
	MCFP_SET_F(base, MCFP_R_IP_USE_OTF_PATH_23, MCFP_F_IP_USE_OTF_OUT_FOR_PATH_2, en);

	/* WGT CINFIFO setting */
	MCFP_SET_R(base, MCFP_R_STAT_CINFIFODLFEWGT_ENABLE, en);
	MCFP_SET_F(base, MCFP_R_STAT_CINFIFODLFEWGT_CONFIG, MCFP_F_STAT_CINFIFODLFEWGT_STALL_BEFORE_FRAME_START_EN, en);
	MCFP_SET_F(base, MCFP_R_STAT_CINFIFODLFEWGT_CONFIG, MCFP_F_STAT_CINFIFODLFEWGT_AUTO_RECOVERY_EN, 0);
	MCFP_SET_F(base, MCFP_R_STAT_CINFIFODLFEWGT_CONFIG, MCFP_F_STAT_CINFIFODLFEWGT_DEBUG_EN, 1);
	MCFP_SET_F(base, MCFP_R_IP_USE_OTF_PATH_23, MCFP_F_IP_USE_OTF_IN_FOR_PATH_3, en);
}
KUNIT_EXPORT_SYMBOL(mcfp_hw_s_dlfe_path);
...
void mcfp_hw_s_strgen(struct pablo_mmio *base, u32 set_id)
{
	...
	/* To use STRGEN, VValid rise must be asserted ASAP. */
	MCFP_SET_F(base, MCFP_R_IP_USE_CINFIFO_NEW_FRAME_IN, MCFP_F_IP_USE_CINFIFO_NEW_FRAME_IN, 0);
}
