...
#include "sfr/is-sfr-cstat-v3_20.h"
...
u32 cstat_hw_is_occurred(ulong state, enum cstat_event_type type)
{
	...
	case CSTAT_LINE:
		...
		break;
	case CSTAT_FE:
		mask = BIT_MASK(FRAME_END_INTERRUPT);
		break;
	case CSTAT_RGBY:
	...
	case CSTAT_ERR:
		mask = (BIT_MASK(IRQ_CORRUPTED) |
			BIT_MASK(COREX_ERROR_INT) |
			BIT_MASK(SDC_ERROR) |
			BIT_MASK(LIC_CONTROLLER_OVERFLOW_ERROR) |
			BIT_MASK(LIC_CONTROLLER_ROW_ERROR) |
			BIT_MASK(C_LOADER_AND_COREX_OVERLAP) |
			BIT_MASK(CINFIFO_INPUT_PROTOCOL_ERROR) |
			BIT_MASK(CINFIFO_PIXEL_CNT_ERROR) |
			BIT_MASK(CINFIFO_OVERFLOW_ERROR) |
			BIT_MASK(CINFIFO_OVERLAP_ERROR));
		break;
	case CSTAT_LIC_ERR:
		mask = (BIT_MASK(LIC_CONTROLLER_OVERFLOW_ERROR) |
			BIT_MASK(LIC_CONTROLLER_ROW_ERROR) |
			BIT_MASK(CINFIFO_PIXEL_CNT_ERROR));
		break;
	case CSTAT_CDAF:
...
void cstat_hw_print_err(char *name, u32 instance, u32 fcount, ulong src1, ulong src2)
{
	...
	while (err < CSTAT_INT1_CNT) {
		switch (err) {
		case IRQ_CORRUPTED:
		case COREX_ERROR_INT:
		case SDC_ERROR:
		case LIC_CONTROLLER_OVERFLOW_ERROR:
		case LIC_CONTROLLER_ROW_ERROR:
		case C_LOADER_AND_COREX_OVERLAP:
		case CINFIFO_INPUT_PROTOCOL_ERROR:
		case CINFIFO_PIXEL_CNT_ERROR:
		case CINFIFO_OVERFLOW_ERROR:
		case CINFIFO_OVERLAP_ERROR:
...
void cstat_hw_s_dma_cfg(struct cstat_param_set *p_set, struct is_cstat_config *conf)
{
	...
	if (p_set->dma_output_drc.cmd && conf->drcclct_bypass) {
		...
		p_set->dma_output_drc.width = (conf->drc_grid_w + 1) / 2 * 2;
