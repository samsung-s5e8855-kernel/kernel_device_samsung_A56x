...
#include "sfr/is-sfr-byrp-v13_20.h"
#include "pmio.h"
...
void byrp_hw_s_init(struct pablo_mmio *base, u32 set_id)
{
	...
	BYRP_SET_R(base, BYRP_R_BYR_RDMABYRIN_MAX_MO, 0xa);
}
...
void byrp_hw_s_strgen(struct pablo_mmio *base, u32 set_id)
{
	...
	/* DTP path select */
	BYRP_SET_R(base, GET_COREX_OFFSET(set_id) + BYRP_R_CHAIN_INPUT_0_SELECT, 0);
...
{
	u32 val = 0;
	u32 input_0 = 0x1;  /* rdma : 0x1 */
	u32 output_0 = 0x0; /* PREGAMMA : 0x0 */
	bool dma_input_en, mpc_en, dng_en;

	/* TODO: MPC on/off is checked from cis */
	mpc_en = 0; /* TBD */
	dng_en = param_set->dma_output_byr.cmd;
	dma_input_en = param_set->dma_input.cmd;
...
static void byrp_hw_s_int_mask(struct pablo_mmio *base, u32 set_id)
{
	BYRP_SET_F(base, GET_COREX_OFFSET(set_id) + BYRP_R_INT_REQ_INT0_ENABLE,
		BYRP_F_INT_REQ_INT0_ENABLE, INT0_EN_MASK);
	BYRP_SET_F(base, GET_COREX_OFFSET(set_id) + BYRP_R_INT_REQ_INT1_ENABLE,
		BYRP_F_INT_REQ_INT1_ENABLE, INT1_EN_MASK);
}
...
void byrp_hw_init_pmio_config(struct pmio_config *cfg)
{
	...
	cfg->volatile_table = &byrp_volatile_ranges_table;
	cfg->wr_noinc_table = &byrp_wr_noinc_table;

	cfg->max_register = BYRP_R_BYR_CROPZSL_STREAM_CRC;
	cfg->num_reg_defaults_raw = (BYRP_R_BYR_CROPZSL_STREAM_CRC >> 2) + 1;
