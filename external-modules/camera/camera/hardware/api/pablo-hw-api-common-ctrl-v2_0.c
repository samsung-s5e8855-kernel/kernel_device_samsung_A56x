...
#define pr_fmt(fmt) "[@][PCC(v2.0)] " fmt
...
#define pcc_apg_off(pcc)                                                                           \
	do {                                                                                       \
		if (pcc->apg_mode)                                                                 \
			_pcc_set_qch(pcc, true);                                                   \
	} while (0)
#define pcc_apg_on(pcc)                                                                            \
	do {                                                                                       \
		if (pcc->apg_mode)                                                                 \
...
static const struct cr_field_map cotf_out_cr[MAX_COTF_OUT_NUM] = {
	...
	{
		.cr_offset = CMN_CTRL_R_IP_USE_OTF_PATH_67,
		.field_id = CMN_CTRL_F_IP_USE_OTF_OUT_FOR_PATH_6,
	},
	{
		.cr_offset = CMN_CTRL_R_IP_USE_OTF_PATH_67,
		.field_id = CMN_CTRL_F_IP_USE_OTF_OUT_FOR_PATH_7,
	},
};
...
static void _pcc_s_apg_mode(struct pablo_common_ctrl *pcc, u32 apg_mode)
{
	struct pablo_mmio *pmio = pcc->pmio;
	bool hwapg = false, ictrl = false;
	...
	pcc->apg_mode = apg_mode;

	SET_CR_F(pmio, CMN_CTRL_R_IP_APG_MODE, CMN_CTRL_F_IP_APG_MODE_HWAPG, hwapg);
	SET_CR_F(pmio, CMN_CTRL_R_IP_APG_MODE, CMN_CTRL_F_IP_APG_MODE_ICTRL, ictrl);
}
...
static int _sw_reset(struct pablo_common_ctrl *pcc, enum sw_reset_mode mode)
{
	struct pablo_mmio *pmio = pcc->pmio;
	u32 retry = PCC_TRY_COUNT;
	u32 val;
	...
	do {
		val = GET_CR(pmio, sw_reset_cr[mode].cr_offset);
		if (val)
			udelay(1);
		else
			break;
...
/* PCC API functions */
static int pcc_otf_enable(struct pablo_common_ctrl *pcc, const struct pablo_common_ctrl_cfg *cfg)
{
	_pcc_enable(pcc, cfg);
...
static int pcc_m2m_enable(struct pablo_common_ctrl *pcc, const struct pablo_common_ctrl_cfg *cfg)
{
	_pcc_enable(pcc, cfg);
	_pcc_s_apg_mode(pcc, cfg->apg_mode);

	pcc_info(
		"fs_mode(%d) apg_mode(%d) int0(0x%08x) int1(0x%08x) cmdq_int(0x%08x) corex_int(0x%08x)",
		pcc, cfg->fs_mode, cfg->apg_mode, cfg->int_en[PCC_INT_0], cfg->int_en[PCC_INT_1],
		cfg->int_en[PCC_CMDQ_INT], cfg->int_en[PCC_COREX_INT]);
...
static int pcc_reset(struct pablo_common_ctrl *pcc)
{
	...
	ret = _sw_reset(pcc, SW_RESET_ALL);
	if (ret)
	...
	struct pablo_common_ctrl *pcc, struct pablo_common_ctrl_frame_cfg *frame_cfg)
{
	 ...
	 */
	if (frame_cfg->num_buffers > 1 && _get_cmdq_queue_fullness(pcc) > 1)
		return 0;

	return _prepare_real_cmd(pcc, frame_cfg);
}
...
static const struct of_device_id pablo_common_ctrl_api_of_table_v2_0[] = {
	{
		.name = "pablo_common_ctrl_v2.0",
		.compatible = "samsung,pablo-common-ctrl-v2.0",
	},
	{},
};
MODULE_DEVICE_TABLE(of, pablo_common_ctrl_api_of_table_v2_0);
...
static struct platform_driver pablo_common_ctrl_api_driver = {
	...
	.driver = {
		.name = "pablo_common_ctrl_v2.0",
		.owner = THIS_MODULE,
		.of_match_table = pablo_common_ctrl_api_of_table_v2_0,
	},
};
...
KUNIT_EXPORT_SYMBOL(pablo_common_ctrl_api_get_platform_driver_v2_0);
