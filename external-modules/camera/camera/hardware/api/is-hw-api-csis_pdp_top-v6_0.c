...
void csis_pdp_top_s_otf_lc(void __iomem *regs, u32 otf_ch, u32 *lc)
{
	...
	/* EMB LC: EMB doesn't have MUX of PDP input */
	if (lc[CAMIF_VC_EMB] < CSIS_OTF_CH_LC_NUM)
		printk(KERN_CONT ", EMB_LC%d", lc[CAMIF_VC_EMB]);
	...
	u32 otf_out_id, struct is_sensor_cfg *sensor_cfg, bool csi_potf)
{
	...
	for (lc = 0; lc < CSIS_OTF_CH_LC_NUM; lc++) {
		...
		if (potf && sensor_cfg->input[link_vc].data == DATA_EMB)
			user_emb = true;
...
int csis_pdp_top_nfi_en(struct is_device_csi *csi, bool en, struct is_fid_loc *fid_loc)
{
	...
	if (!en) {
		is_hw_set_reg(top->regs, &csis_top_regs[CSIS_R_CSIS0_NFI_EN + otf_info->csi_ch], 0);
		return 0;
	}
	...
	val = is_hw_set_field_value(val, &csis_top_fields[CSIS_F_FID_LOC_BYTE], fid_loc->byte);
	val = is_hw_set_field_value(val, &csis_top_fields[CSIS_F_FID_LOC_LINE], fid_loc->line);
	is_hw_set_reg(top->regs, &csis_top_regs[CSIS_R_CSIS0_FRAME_ID_EN + otf_info->csi_ch], val);

	val = is_hw_set_field_value(val, &csis_top_fields[CSIS_F_NFI_LOC_BYTE], fid_loc->nfi_byte);
	val = is_hw_set_field_value(val, &csis_top_fields[CSIS_F_NFI_LOC_LINE], fid_loc->nfi_line);
	val = is_hw_set_field_value(val, &csis_top_fields[CSIS_F_NFI_EN_CSIS], en);
	is_hw_set_reg(top->regs, &csis_top_regs[CSIS_R_CSIS0_NFI_EN + otf_info->csi_ch], val);
...
void csis_pdp_top_nfi_sw_trigger(struct is_device_csi *csi)
{
	...
	is_hw_set_field(top->regs, &csis_top_regs[CSIS_R_NFI_CTRL_0],
		&csis_top_fields[CSIS_F_SW_TRIGGER], val);
}
...
void csis_pdp_top_nfi_irq_msk(void __iomem *base_reg, u32 csi_ch, bool on)
{
	...
	l_msk = is_hw_get_reg(base_reg, &csis_top_regs[CSIS_R_NFI_INT_ENABLE_0]);
	u_msk = is_hw_get_reg(base_reg, &csis_top_regs[CSIS_R_NFI_INT_ENABLE_1]);
	...
	is_hw_set_reg(base_reg, &csis_top_regs[CSIS_R_NFI_INT_ENABLE_0], l_msk);
	is_hw_set_reg(base_reg, &csis_top_regs[CSIS_R_NFI_INT_ENABLE_1], u_msk);
...
void csis_pdp_top_nfi_irq_src(void __iomem *base_reg, ulong *str, ulong *end)
{
	...
	l_src = is_hw_get_reg(base_reg, &csis_top_regs[CSIS_R_NFI_INT_SOURCE_0]);
	u_src = is_hw_get_reg(base_reg, &csis_top_regs[CSIS_R_NFI_INT_SOURCE_1]);

	/* clear */
	is_hw_set_reg(base_reg, &csis_top_regs[CSIS_R_NFI_INT_SOURCE_0], l_src);
	is_hw_set_reg(base_reg, &csis_top_regs[CSIS_R_NFI_INT_SOURCE_1], u_src);
