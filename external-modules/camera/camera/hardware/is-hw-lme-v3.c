...
static int param_debug_lme_usage(char *buffer, const size_t buf_size)
{
	const char *usage_msg = "[value] number value, set debug param value\n"
				"\t0 : turn off debug\n"
				"\t1 : dump sfr\n";
...
static struct pablo_debug_param debug_lme = {
	.type = IS_DEBUG_PARAM_TYPE_NUM,
	.max_value = 1,
	.ops.usage = param_debug_lme_usage,
};
module_param_cb(debug_lme, &pablo_debug_param_ops, &debug_lme, 0644);
...
static size_t is_hw_lme_dump_params(struct is_hw_ip *hw_ip, u32 instance, char *buf, size_t buf_size)
{
	struct is_hw_lme *hw;
	struct lme_param_set *param_set;
	...
	hw = (struct is_hw_lme *)hw_ip->priv_info;
	...
	p = pablo_json_object_open(p, "param_lme_dma", &rem);
	p = pablo_json_uint(p, "cur_input_cmd", dma->cur_input_cmd, &rem);
	p = pablo_json_uint(p, "prev_input_cmd", dma->prev_input_cmd, &rem);
	p = pablo_json_uint(p, "output_cmd", dma->output_cmd, &rem);
	p = pablo_json_uint(p, "sad_output_cmd", dma->sad_output_cmd, &rem);
	p = pablo_json_uint(p, "input_format", dma->input_format, &rem);
	p = pablo_json_uint(p, "input_bitwidth", dma->input_bitwidth, &rem);
	p = pablo_json_uint(p, "input_order", dma->input_order, &rem);
	p = pablo_json_uint(p, "input_plane", dma->input_plane, &rem);
	p = pablo_json_uint(p, "input_msb", dma->input_msb, &rem);
	p = pablo_json_uint(p, "cur_input_width", dma->cur_input_width, &rem);
	p = pablo_json_uint(p, "cur_input_height", dma->cur_input_height, &rem);
	p = pablo_json_uint(p, "prev_input_width", dma->prev_input_width, &rem);
	p = pablo_json_uint(p, "prev_input_height", dma->prev_input_height, &rem);
	p = pablo_json_uint(p, "output_format", dma->output_format, &rem);
	p = pablo_json_uint(p, "output_bitwidth", dma->output_bitwidth, &rem);
	p = pablo_json_uint(p, "output_order", dma->output_order, &rem);
	p = pablo_json_uint(p, "output_plane", dma->output_plane, &rem);
	p = pablo_json_uint(p, "sad_output_plane", dma->sad_output_plane, &rem);
	p = pablo_json_uint(p, "output_width", dma->output_width, &rem);
	p = pablo_json_uint(p, "output_height", dma->output_height, &rem);
	p = pablo_json_uint(p, "output_msb", dma->output_msb, &rem);
	p = pablo_json_uint(p, "processed_output_cmd", dma->processed_output_cmd, &rem);
	p = pablo_json_uint(p, "err", dma->err, &rem);
	p = pablo_json_object_close(p, &rem);
	p = pablo_json_uint(p, "instance_id", param_set->instance_id, &rem);
	p = pablo_json_uint(p, "fcount", param_set->fcount, &rem);
	p = pablo_json_uint(p, "tnr_mode", param_set->tnr_mode, &rem);
	p = pablo_json_bool(p, "reprocessing", param_set->reprocessing, &rem);
...
const struct is_hw_ip_ops is_hw_lme_ops = {
	...
	.dump_params		= is_hw_lme_dump_params,
};
...
EXPORT_SYMBOL_GPL(is_hw_lme_remove);
