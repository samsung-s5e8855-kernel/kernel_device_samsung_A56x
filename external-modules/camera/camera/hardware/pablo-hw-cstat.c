...
static inline void is_hw_cstat_check_dbg_info(
	struct is_hw_ip *hw_ip, u32 int_src1, u32 int_src2, u32 instance, u32 fcount)
{
	bool is_int;
	bool is_ins;
	bool is_fcount;
	...
	msinfo_hw("check_dbg_info: 0x%x(0x%x) 0x%x(0x%x) fcount %d(%d)\n", instance, hw_ip,
		int_src1, dbg_info.src1, int_src2, dbg_info.src2, fcount, dbg_info.fcount);
...
static int is_hw_cstat_dbg_set(const char *val)
{
	int ret, argc;
	char **argv;
	u32 dbg_type;
...
static int is_hw_cstat_dbg_usage(char *buffer, const size_t buf_size)
{
	const char *usage_msg =
		"> Set regdump/scandump trigger\n"
		"  # echo <type> <instance> <fcount> <int1_msk> <int2_msk> > debug_cstat\n"
		"    - type: 1=REG_DUMP, 2=S2D\n"
		"    - fcount 0: Trigger for every interrupt.\n"
		"    - int1/2_msk: Trigger when the specific bit is set.\n"
		"================================================================\n"
		"> Disable specific WDMA\n"
		"  # echo 3 <dma_id> > debug_cstat\n"
		"    - dma_id: 1 << Actual WDMA ID\n"
		"================================================================\n";

	return scnprintf(buffer, buf_size, usage_msg);
}

static int is_hw_cstat_dbg_get(char *buffer, const size_t buf_size)
{
	return scnprintf(buffer, buf_size,
		"type %d instance %d fcount %d int1_msk 0x%x int2_msk 0x%x dma_id 0x%lx\n",
		dbg_info.type, dbg_info.instance, dbg_info.fcount, dbg_info.src1, dbg_info.src2,
		dbg_info.dma_id);
}

static struct pablo_debug_param debug_cstat = {
	.type = IS_DEBUG_PARAM_TYPE_STR,
	.ops.set = is_hw_cstat_dbg_set,
	.ops.get = is_hw_cstat_dbg_get,
	.ops.usage = is_hw_cstat_dbg_usage,
};
module_param_cb(debug_cstat, &pablo_debug_param_ops, &debug_cstat, 0644);

static inline void is_hw_cstat_put_vc_buf(
...
static inline void is_hw_cstat_frame_end(struct is_hw_ip *hw_ip, u32 instance)
{
	...
	if (IS_ENABLED(CRTA_CALL)) {
		...
		if (test_and_clear_bit(HW_OVERFLOW_RECOVERY, &hw_ip->state)) {
			...
			cstat_hw_g_stat_data(
				hw_ip->pmio, CSTAT_STAT_EDGE_SCORE, (void *)&edge_score);

			for (buf_idx = PABLO_CRTA_CSTAT_END_PRE_THUMB;
				buf_idx < PABLO_CRTA_CSTAT_END_VPDAF; buf_idx++) {
				bufmgr = pablo_get_crta_bufmgr(
					data_to_buf[buf_idx], instance, hw_ip->ch);
				CALL_CRTA_BUFMGR_OPS(
					bufmgr, get_process_buf, start_fcount, &stat_buf[buf_idx]);
			}
			...
			if (sensor_itf) {
				...
				if (!ret)
					get_vc_dma_buf_by_fcount(sensor_itf,
						VC_BUF_DATA_TYPE_SENSOR_STAT3, start_fcount, &index,
		...
		ret = CALL_ADT_MSG_OPS(hw->icpu_adt, send_msg_cstat_frame_end, instance,
			(void *)hw_ip, NULL, start_fcount, &shot_buf, stat_buf, edge_score);
		if (ret)
...
static int is_hw_cstat_frame_end_callback(void *caller, void *ctx, void *rsp_msg)
{
	...
	for (buf_idx = PABLO_CRTA_CSTAT_END_PRE_THUMB; buf_idx < PABLO_CRTA_CSTAT_END_CDAF_MW;
		buf_idx++) {
...
static int is_hw_cstat_isr1(u32 hw_id, void *data)
{
	...
	int1 = (ulong)cstat_hw_g_int1_status(hw_ip->pmio, true, hw->hw_fro_en);
	int2 = (ulong)cstat_hw_g_int2_status(hw_ip->pmio, false, hw->hw_fro_en);
	hw->irq_state[CSTAT_INT1] = int1;
	hw->irq_state[CSTAT_INT2] = int2;

	msdbg_hw(2, "[INT1][F%d] src 0x%lx 0x%lx\n", instance, hw_ip, hw_fcount, int1, int2);
	...
	if (test_bit(HW_OVERFLOW_RECOVERY, &hardware->hw_recovery_flag)) {
		mserr_hw("[INT1][F%d] during recovery. src 0x%lx 0x%lx", instance, hw_ip, hw_fcount,
			int1, int2);
		atomic_dec(&hw->isr_run_count);
		wake_up(&hw->isr_wait_queue);
	...
	if (corex_end)
		cstat_hw_s_corex_type(hw_ip->pmio, COREX_IGNORE);
	...
	while (int_status) {
		...
		case CSTAT_LINE:
			if (corex_end) {
				...
				if (!test_bit(HW_SUSPEND, &hw_ip->state))
					is_hw_cstat_set_config_lock_delay(
						hw_ip, instance, hw_fcount);
...
skip_isr_event:
	if (int_status)
		mswarn_hw("[F%d] invalid interrupt: event_state(%ld), int_status(%x)", instance,
			hw_ip, hw_fcount, hw->event_state, int_status);
...
static inline void is_hw_cstat_cdaf(struct is_hw_ip *hw_ip, u32 instance)
{
	...
	ret = CALL_CRTA_BUFMGR_OPS(
		bufmgr, get_free_buf, hw_fcount, true, &cdaf_buf[PABLO_CRTA_CDAF_RAW]);
	...
	if (cstat_hw_g_stat_data(hw_ip->pmio, CSTAT_STAT_CDAF, (void *)&cdaf)) {
	...
	if (!ret)
		get_vc_dma_buf_by_fcount(sensor_itf, VC_BUF_DATA_TYPE_SENSOR_STAT1, hw_fcount,
			&index, &cdaf_buf[PABLO_CRTA_PDAF_TAIL].dva);
	...
	if (sensor_itf->cis_itf_ops.is_laser_af_available(sensor_itf)) {
		...
		ret = CALL_CRTA_BUFMGR_OPS(
			bufmgr, get_free_buf, hw_fcount, true, &cdaf_buf[PABLO_CRTA_LASER_AF_DATA]);
	...
	ret = CALL_ADT_MSG_OPS(hw->icpu_adt, send_msg_cstat_cdaf_end, instance, (void *)hw_ip, NULL,
		hw_fcount, cdaf_buf);
	if (ret)
...
static int is_hw_cstat_isr2(u32 hw_id, void *data)
{
	...
	src = cstat_hw_g_int2_status(hw_ip->pmio, true, hw->hw_fro_en);
	...
	if (!test_bit(HW_OPEN, &hw_ip->state) || !test_bit(HW_RUN, &hw_ip->state)) {
		mserr_hw("[INT2][F%d] invalid HW state 0x%lx src 0x%x", instance, hw_ip, hw_fcount,
			hw_ip->state, src);
		atomic_dec(&hw->isr_run_count);
		wake_up(&hw->isr_wait_queue);
	...
	if (test_bit(HW_OVERFLOW_RECOVERY, &hw_ip->hardware->hw_recovery_flag)) {
		mserr_hw("[INT2][F%d] during recovery. src 0x%x", instance, hw_ip, hw_fcount, src);
		atomic_dec(&hw->isr_run_count);
		wake_up(&hw->isr_wait_queue);
...
static int is_hw_cstat_open(struct is_hw_ip *hw_ip, u32 instance)
{
	...
	if (ret) {
		mserr_hw("failed to probe internal sub-device for %s: %d", instance, hw_ip,
			pis->name, ret);
		goto err_c_loader_alloc;
	}

	pis->width = 0x2000 + 0x8000;
	pis->height = 1;
	pis->num_planes = 1;
	pis->num_batch = 1;
	pis->num_buffers = 1;
	pis->bits_per_pixel = 8;
	pis->memory_bitwidth = 8;
...
static int is_hw_cstat_register_icpu_msg_cb(struct is_hw_ip *hw_ip, u32 instance)
{
	...
	ret = CALL_ADT_MSG_OPS(icpu_adt, register_response_msg_cb, instance, cmd_id,
		is_hw_cstat_frame_start_callback);
	if (ret)
		goto exit;

	cmd_id = PABLO_HIC_CSTAT_FRAME_END;
	ret = CALL_ADT_MSG_OPS(icpu_adt, register_response_msg_cb, instance, cmd_id,
		is_hw_cstat_frame_end_callback);
	if (ret)
		goto exit;

	cmd_id = PABLO_HIC_CSTAT_CDAF_END;
	ret = CALL_ADT_MSG_OPS(
		icpu_adt, register_response_msg_cb, instance, cmd_id, is_hw_cstat_cdaf_callback);
	if (ret)
		goto exit;
...
static int is_hw_cstat_init(struct is_hw_ip *hw_ip, u32 instance, bool flag, u32 f_type)
{
	int ret;
	struct is_hw_cstat *hw;
	u32 dma_id;
	...
	for (dma_id = CSTAT_R_CL; dma_id < CSTAT_DMA_NUM; dma_id++) {
		if (cstat_hw_create_dma(hw_ip->pmio, dma_id, &hw->dma[dma_id])) {
...
static int is_hw_cstat_close(struct is_hw_ip *hw_ip, u32 instance)
{
	...
	for (i = 0; i < max_num; i++) {
		struct is_hw_ip *hw_ip_phys =
			CALL_HW_CHAIN_INFO_OPS(hardware, get_hw_ip, DEV_HW_3AA0 + i);
	...
	if (i == max_num) {
		if (cstat_hw_s_reset(hw_ip->locomotive->pmio))
...
static int is_hw_cstat_core_init(struct is_hw_ip *hw_ip, u32 instance)
{
	...
	cstat_hw_core_init(hw_ip->locomotive->pmio);
...
{
	cstat_hw_ctx_init(hw_ip->pmio);

	cstat_hw_s_corex_enable(hw_ip->pmio, true);
}
...
static void is_hw_cstat_s_global_enable(struct is_hw_ip *hw_ip, bool enable, bool fro_en)
{
	...
	cstat_hw_s_global_enable(hw_ip->pmio, enable, fro_en);
}
...
static void is_hw_cstat_wait_isr_done(struct is_hw_ip *hw_ip)
{
	...
	cstat_hw_wait_isr_clear(hw_ip->pmio, hw->hw_fro_en);

	timetowait = wait_event_timeout(
		hw->isr_wait_queue, !atomic_read(&hw->isr_run_count), IS_HW_STOP_TIMEOUT);
	if (!timetowait)
...
static int is_hw_cstat_enable(struct is_hw_ip *hw_ip, u32 instance, ulong hw_map)
{
	...
	hw_ip->pmio_config.cache_type = PMIO_CACHE_FLAT_COREX;
	if (pmio_reinit_cache(hw_ip->pmio, &hw_ip->pmio_config)) {
		err("failed to reinit PMIO cache");
		return -EINVAL;
	}
...
static int is_hw_cstat_disable(struct is_hw_ip *hw_ip, u32 instance, ulong hw_map)
{
	...
	msinfo_hw(
		"%s: Vvalid(%d)\n", instance, hw_ip, __func__, atomic_read(&hw_ip->status.Vvalid));
...
static void is_hw_cstat_update_param(struct is_hw_ip *hw_ip, struct is_param_region *p_region,
	struct cstat_param_set *p_set, IS_DECLARE_PMAP(pmap), u32 instance, u32 ctx)
{
	...
	if (test_bit(pindex, pmap))
		memcpy(&p_set->dma_output_drc, &param->drc, sizeof(struct param_dma_output));

#if defined(ENABLE_SAT_CAV)
	pindex = GET_CSTAT_CTX_PINDEX(PARAM_CSTAT_SAT, ctx);
	if (test_bit(pindex, pmap))
		memcpy(&p_set->dma_output_sat, &param->sat, sizeof(struct param_dma_output));
	...
	if (test_bit(pindex, pmap))
		memcpy(&p_set->dma_output_cds, &param->cds, sizeof(struct param_dma_output));
#endif

#if defined(ENABLE_AWB_HAL)
	pindex = GET_CSTAT_CTX_PINDEX(PARAM_CSTAT_PRE, ctx);
	if (test_bit(pindex, pmap))
		memcpy(&p_set->dma_output_pre, &param->pre, sizeof(struct param_dma_output));

	pindex = GET_CSTAT_CTX_PINDEX(PARAM_CSTAT_AWB, ctx);
	if (test_bit(pindex, pmap))
		memcpy(&p_set->dma_output_awb, &param->awb, sizeof(struct param_dma_output));
#endif
}

static void is_hw_cstat_internal_shot(
	struct is_hw_ip *hw_ip, struct is_frame *frame, struct cstat_param_set *p_set)
{
...
static void is_hw_cstat_external_shot(
	struct is_hw_ip *hw_ip, struct is_frame *frame, struct cstat_param_set *p_set)
{
	...
	/* DMA settings */
	CALL_HW_OPS(hw_ip, dma_cfg, "cstat_byr_in", hw_ip, frame, b_idx, frame->num_buffers,
		&p_set->dma_input.cmd, p_set->dma_input.plane, p_set->input_dva,
		frame->dvaddr_buffer);

	CALL_HW_OPS(hw_ip, dma_cfg, "cstat_drc", hw_ip, frame, b_idx, frame->num_buffers,
		&p_set->dma_output_drc.cmd, p_set->dma_output_drc.plane, p_set->output_dva_drc,
		frame->txdgrTargetAddress[ctx_idx]);

	CALL_HW_OPS(hw_ip, dma_cfg, "cstat_lme_ds0", hw_ip, frame, 0, 1,
		&p_set->dma_output_lme_ds0.cmd, p_set->dma_output_lme_ds0.plane,
		p_set->output_dva_lme_ds0, frame->txldsTargetAddress[ctx_idx]);

	CALL_HW_OPS(hw_ip, dma_cfg, "cstat_lme_ds1", hw_ip, frame, 0, 1,
		&p_set->dma_output_lme_ds1.cmd, p_set->dma_output_lme_ds1.plane,
		p_set->output_dva_lme_ds1, frame->dva_cstat_lmeds1[ctx_idx]);

	CALL_HW_OPS(hw_ip, dma_cfg, "cstat_fdpig", hw_ip, frame, 0, 1, &p_set->dma_output_fdpig.cmd,
		p_set->dma_output_fdpig.plane, p_set->output_dva_fdpig,
		frame->efdTargetAddress[ctx_idx]);

#if defined(ENABLE_SAT_CAV)
	CALL_HW_OPS(hw_ip, dma_cfg, "cstat_sat", hw_ip, frame, 0, 1, &p_set->dma_output_sat.cmd,
		p_set->dma_output_sat.plane, p_set->output_dva_sat, frame->dva_cstat_sat[ctx_idx]);

	CALL_HW_OPS(hw_ip, dma_cfg, "cstat_cav", hw_ip, frame, 0, 1, &p_set->dma_output_cav.cmd,
		p_set->dma_output_cav.plane, p_set->output_dva_cav, frame->dva_cstat_cav[ctx_idx]);
#else
	CALL_HW_OPS(hw_ip, dma_cfg, "cstat_cds", hw_ip, frame, 0, 1, &p_set->dma_output_cds.cmd,
		p_set->dma_output_cds.plane, p_set->output_dva_cds,
		frame->txpTargetAddress[ctx_idx]);
#endif

#if defined(ENABLE_AWB_HAL)
	CALL_HW_OPS(hw_ip, dma_cfg, "cstat_pre", hw_ip, frame, 0, 1, &p_set->dma_output_pre.cmd,
		p_set->dma_output_pre.plane, p_set->output_dva_pre, frame->dva_cstat_pre[ctx_idx]);

	CALL_HW_OPS(hw_ip, dma_cfg, "cstat_awb", hw_ip, frame, 0, 1, &p_set->dma_output_awb.cmd,
		p_set->dma_output_awb.plane, p_set->output_dva_awb, frame->dva_cstat_awb[ctx_idx]);
#endif
}
...
static void is_hw_cstat_check_ignore_delay_corex(struct is_hw_ip *hw_ip, u32 instance, u32 fcount)
{
	...
	if (hw->sensor_itf[instance]) {
		hw->sensor_itf[instance]->cis_itf_ops.get_sensor_frame_timing(
			hw->sensor_itf[instance], &vvalid_time, &vblank_time);
		if (test_bit(HW_CONFIG, &hw_ip->state) && (vvalid_time != hw->vvalid_time)) {
			hw->ignore_corex_delay = IGNORE_COREX_DEALY;
			msinfo_hw("[F%d] detect seamless modechange & disable corex delay",
				instance, hw_ip, fcount);
		}
	}
	...
	u32 *vvalid_time, u32 *corex_delay_time, u32 *cinrow_time, u32 *cinrow_ratio)
{
	...
	u32 req_min_frame_time =
		REQ_START_2_SHOT + REQ_SHOT_2_END + (REQ_END_2_START * DVFS_LEVEL_RATIO);

	hw = (struct is_hw_cstat *)hw_ip->priv_info;

	*vvalid_time = hw->vvalid_time;
	frame_time = 1000000U / frame_rate;
	vblank_time = (frame_time > *vvalid_time) ? (frame_time - *vvalid_time) : 0;
	...
	if (!test_bit(HW_CONFIG, &hw_ip->state))
		msinfo_hw("vvalid(%d+%d->%d), cinrow(%dus/%d%%), vblank(%d->%d), frame(%d): ",
			instance, hw_ip, *vvalid_time, *corex_delay_time,
			(*vvalid_time + *corex_delay_time), *cinrow_time, *cinrow_ratio,
			vblank_time, (vblank_time - *corex_delay_time), frame_time);
}
/*
 ...
 */
static void is_hw_cstat_s_col_row(void __iomem *base, bool en, u32 width, u32 height,
	u32 vvalid_time, u32 cinrow_time, u32 cinrow_ratio)
{
...
static int is_hw_cstat_init_config(struct is_hw_ip *hw_ip, u32 instance, struct is_frame *frame)
{
	...
	if (cstat_hw_s_sram_offset(hw_ip->locomotive->pmio))
		return -EINVAL;

	atomic_set(&hw->start_fcount, 0);

	msinfo_hw("init_config: %s_IN size %dx%d lic_mode %d\n", atomic_read(&hw_ip->instance),
		hw_ip, (hw->input == OTF) ? "OTF" : "DMA", width, height, lic_mode);

	return 0;
}

static void is_hw_cstat_s_stat_cfg(
	struct is_hw_ip *hw_ip, u32 instance, struct cstat_param_set *p_set)
{
	int ret;
...
static int is_hw_cstat_s_iq_regs(struct is_hw_ip *hw_ip, u32 instance, struct c_loader_buffer *clb)
{
	...
	if (test_and_clear_bit(CR_SET_CONFIG, &iq_set->state)) {
		...
		for (i = 0; i < regs_size; i++) {
			if (cstat_hw_is_corex_offset(regs[i].reg_addr)) {
				hw->cur_iq_set.regs[i].reg_addr =
					regs[i].reg_addr + CSTAT_COREX_SDW_OFFSET;
				hw->cur_iq_set.regs[i].reg_data = regs[i].reg_data;
			} else {
				hw->cur_iq_set.regs[i].reg_addr = regs[i].reg_addr;
				hw->cur_iq_set.regs[i].reg_data = regs[i].reg_data;
			}
		}
		hw->cur_iq_set.size = regs_size;

		pmio_cache_fsync_ext(hw_ip->pmio, clb, hw->cur_iq_set.regs, regs_size);
...
static void is_hw_cstat_s_block_reg(struct is_hw_ip *hw_ip, struct cstat_param_set *p_set)
{
	...
	cstat_hw_s_default_blk_cfg(hw_ip->pmio);
}

static int is_hw_cstat_s_size_regs(
	struct is_hw_ip *hw_ip, struct cstat_param_set *p_set, struct is_frame *frame)
{
	struct is_hw_cstat *hw;
	void __iomem *base;
	...
	if (sensor_center_x == 0 || sensor_center_y == 0) {
		...
		msdbg_hw(2, "%s: cal_center(0,0) from DDK. Fix to (%d,%d)", p_set->instance_id,
			hw_ip, __func__, sensor_center_x, sensor_center_y);
	}

	/* Total_binning = sensor_binning * csis_bns_binning */
	grid_cfg.binning_x =
		(1024ULL * sensor_binning_ratio_x * p_set->sensor_config.bns_binning_ratio_x) /
		1000 / 1000;
	grid_cfg.binning_y =
		(1024ULL * sensor_binning_ratio_y * p_set->sensor_config.bns_binning_ratio_y) /
		1000 / 1000;
	...
	if (p_set->sensor_config.freeform_sensor_crop_enable == 1) {
		...
		binned_sensor_width =
			p_set->sensor_config.calibrated_width * 1000 / sensor_binning_ratio_x;
		binned_sensor_height =
			p_set->sensor_config.calibrated_height * 1000 / sensor_binning_ratio_y;
		sensor_crop_x = ((binned_sensor_width - p_set->sensor_config.width) >> 1) & (~0x1);
		sensor_crop_y =
			((binned_sensor_height - p_set->sensor_config.height) >> 1) & (~0x1);
	}
	...
	msdbg_hw(2,
		...
		p_set->instance_id, hw_ip, __func__, p_set->sensor_config.calibrated_width,
		p_set->sensor_config.calibrated_height, sensor_center_x, sensor_center_y,
		sensor_crop_x, sensor_crop_y, start_pos_x, start_pos_y);

	msdbg_hw(2, "%s:[CGR] sfr: binning(%dx%d), step(%dx%d), crop(%d,%d), crop_radial(%d,%d)\n",
		p_set->instance_id, hw_ip, __func__, grid_cfg.binning_x, grid_cfg.binning_y,
		grid_cfg.step_x, grid_cfg.step_y, grid_cfg.crop_x, grid_cfg.crop_y,
		grid_cfg.crop_radial_x, grid_cfg.crop_radial_y);
	...
	msdbg_hw(2,
		"%s:[LSC] dbg: calibrated_size(%dx%d), cal_center(%d,%d), sensor_crop(%d,%d), start_pos(%d,%d)\n",
		p_set->instance_id, hw_ip, __func__, p_set->sensor_config.calibrated_width,
		p_set->sensor_config.calibrated_height, sensor_center_x, sensor_center_y,
		sensor_crop_x, sensor_crop_y, start_pos_x, start_pos_y);
	...
	msdbg_hw(2, "%s:[CGR,LSC] bcrop: (%d,%d %dx%d), bns: (%dx%d, ratio:%d)\n",
		p_set->instance_id, hw_ip, __func__, size_cfg->bcrop.x, size_cfg->bcrop.y,
		size_cfg->bcrop.w, size_cfg->bcrop.h, cur_crop.w, cur_crop.h,
		size_cfg->bns_binning);
...
static int is_hw_cstat_s_dma(struct is_hw_ip *hw_ip, struct cstat_param_set *p_set)
{
	...
	for (dma_id = CSTAT_W_RGBYHIST; dma_id < CSTAT_DMA_NUM; dma_id++) {
		...
		if (cstat_hw_s_wdma_cfg(hw_ip->pmio, &hw->dma[dma_id], p_set,
			    (hw_ip->num_buffers & 0xffff), disable))
			return -EINVAL;
	}
...
{
	return cstat_hw_s_fro(hw_ip->pmio, (hw_ip->num_buffers & 0xffff), 1);
}

static int is_hw_cstat_trigger(
	struct is_hw_ip *hw_ip, u32 instance, u32 fcount, struct c_loader_buffer *clb)
{
	struct is_hw_cstat *hw = (struct is_hw_cstat *)hw_ip->priv_info;
	void __iomem *base = hw_ip->pmio;
	struct param_sensor_config *sensor_cfg;
	struct cstat_time_cfg time_cfg;
	u32 cinrow_time, cinrow_ratio;
	struct is_hardware *hardware = hw_ip->hardware;

	cstat_hw_s_cloader(hw_ip->pmio, clb->header_dva, clb->num_of_headers);
	...
	case VOTF:
		...
		is_hw_cstat_s_col_row(base, true, hw->param_set[instance].otf_input.width,
			hw->param_set[instance].otf_input.height, time_cfg.vvalid, cinrow_time,
			cinrow_ratio);
...
static int is_hw_cstat_set_config(
	struct is_hw_ip *hw_ip, u32 chain_id, u32 instance, u32 fcount, void *conf)
{
	...
	if (hw->config.sensor_center_x != cstat_config->sensor_center_x ||
		...
		msinfo_hw("[F:%d] CSTAT cgras/luma_shading sensor_center(%d x %d)\n", instance,
			hw_ip, fcount, cstat_config->sensor_center_x,
	...
	if (hw->config.lmeds_bypass != cstat_config->lmeds_bypass ||
		...
		msinfo_hw("[F:%d] CSTAT lmeds_bypass(%d), lmeds0(%d x %d)\n", instance, hw_ip,
			fcount, cstat_config->lmeds_bypass, cstat_config->lmeds_w,
			cstat_config->lmeds_h);
	...
	if (hw->config.cdaf_mw_bypass != cstat_config->cdaf_mw_bypass)
		msinfo_hw("[F:%d] CSTAT cdafmw bypass(%d), grid(%d x %d)\n", instance, hw_ip,
			fcount, cstat_config->cdaf_mw_bypass, cstat_config->cdaf_mw_x,
			cstat_config->cdaf_mw_x);
...
static int _is_hw_cstat_shot(struct is_hw_ip *hw_ip, struct is_frame *frame, ulong hw_map)
{
	...
	if (IS_ENABLED(CRTA_CALL)) {
		if (likely(!test_bit(hw_ip->id, &debug_iq))) {
			...
			ret = is_hw_cstat_s_iq_regs(hw_ip, instance, &clb);
	...
	if (dbg_info.type == CSTAT_DBG_REG_DUMP) {
		pr_info("header dva: %pad\n", &clb.header_dva);
	...
	ret = is_hw_cstat_trigger(hw_ip, instance, frame->fcount, &clb);
	if (ret) {
...
static int is_hw_cstat_frame_ndone(
...
static int is_hw_cstat_change_chain(
...
static int is_hw_cstat_set_regs(struct is_hw_ip *hw_ip, u32 chain_id, u32 instance, u32 fcount,
...
int is_hw_cstat_dump_regs(struct is_hw_ip *hw_ip, u32 instance, u32 fcount, struct cr_set *regs,
 ...
 * The bayer crop control, however, which is decided by user, doesn't care the modified FOV
 * by sensor. So, driver should move the user's bayer crop region into sensor out bayer region
	...
	struct cstat_size_cfg *size_cfg)
{
	 ...
	 */
	for (i = 0; i < 2; i++)
		bcrop[i] = (bcrop[i] > ss_crop[i]) ? (bcrop[i] - ss_crop[i]) : 0;
	...
	struct pablo_rta_frame_info *prfi)
{
	...
	if (!sensor_itf) {
		...
		get_remosaic_zoom_ratio(sensor_itf, frame->fcount,
	...
	msdbg_hw(2, "[F%d] cstat_out_drcclct_buffer: %d, cstat_out_meds_buffer: %d\n", instance,
		hw_ip, frame->fcount, prfi->cstat_out_drcclct_buffer, prfi->cstat_out_meds_buffer);
}
...
static size_t is_hw_cstat_dump_params(struct is_hw_ip *hw_ip, u32 instance, char *buf, size_t size)
{
	struct is_hw_cstat *hw = (struct is_hw_cstat *)hw_ip->priv_info;
	...
	p = dump_param_dma_output(p, "dma_output_lme_ds0", &param->dma_output_lme_ds0, &rem);
	p = dump_param_dma_output(p, "dma_output_lme_ds1", &param->dma_output_lme_ds1, &rem);
...
#endif
	p = dump_param_dma_output(p, "ae", &param->ae, &rem);
	p = dump_param_dma_output(p, "rgby", &param->rgby, &rem);
...
static const struct is_hw_ip_ops is_hw_cstat_ops = {
	...
	.dump_params = is_hw_cstat_dump_params,
};
	...
	struct is_interface_ischain *itfc, int id, const char *name)
{
	...
	cstat_hw_init_pmio_config(&hw_ip->pmio_config);

	hw_ip->pmio = pmio_init(NULL, NULL, &hw_ip->pmio_config);
	if (IS_ERR(hw_ip->pmio)) {
		err("failed to init cstat PMIO: %ld", PTR_ERR(hw_ip->pmio));
		return -ENOMEM;
	}
