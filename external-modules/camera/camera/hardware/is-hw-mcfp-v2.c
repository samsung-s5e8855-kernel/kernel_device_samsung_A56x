...
static int param_debug_mcfp_usage(char *buffer, const size_t buf_size)
{
	const char *usage_msg = "MCFP debug features\n"
				"\tb[0] : dump sfr\n"
				"\tb[1] : dump sfr once\n"
				"\tb[2] : s2d\n"
				"\tb[3] : skip ddk\n"
...
static struct pablo_debug_param debug_mcfp = {
	.type = IS_DEBUG_PARAM_TYPE_BIT,
	.max_value = 0x7F,
	.ops.usage = param_debug_mcfp_usage,
};

module_param_cb(debug_mcfp, &pablo_debug_param_ops, &debug_mcfp, 0644);

#if IS_ENABLED(CONFIG_PABLO_KUNIT_TEST)
const struct kernel_param *is_hw_mcfp_get_debug_kernel_param_kunit_wrapper(void)
{
	return G_KERNEL_PARAM(debug_mcfp);
}
KUNIT_EXPORT_SYMBOL(is_hw_mcfp_get_debug_kernel_param_kunit_wrapper);
#endif
...
static int is_hw_mcfp_handle_interrupt(u32 id, void *context)
{
	...
	if (mcfp_hw_is_occurred(status, INTR_FRAME_END)) {
		...
		if (unlikely(test_bit(MCFP_DBG_S2D, &debug_mcfp.value)))
	...
	struct mcfp_param_set *param_set, u32 instance, u32 set_id)
{
	...
	if (unlikely(test_bit(MCFP_DBG_TNR, &debug_mcfp.value))) {
	...
	ulong hw_map)
{
	...
	if (unlikely(test_bit(MCFP_DBG_DTP, &debug_mcfp.value)))
	...
	if (unlikely(test_bit(MCFP_DBG_DUMP_REG, &debug_mcfp.value) ||
		     test_bit(MCFP_DBG_DUMP_REG_ONCE, &debug_mcfp.value))) {
		mcfp_hw_dump(hw_ip->pmio, HW_DUMP_CR);

		if (test_bit(MCFP_DBG_DUMP_REG_ONCE, &debug_mcfp.value))
			clear_bit(MCFP_DBG_DUMP_REG_ONCE, &debug_mcfp.value);
	}
...
static size_t is_hw_mcfp_dump_params(struct is_hw_ip *hw_ip, u32 instance, char *buf, size_t size)
{
	struct is_hw_mcfp *hw;
	struct mcfp_param_set *param;
	size_t rem = size;
	char *p = buf;

	hw = (struct is_hw_mcfp *)hw_ip->priv_info;
	...
	p = dump_param_otf_input(p, "fto_input", &param->fto_input, &rem);
	p = dump_param_stripe_input(p, "stripe_input", &param->stripe_input, &rem);
	p = dump_param_dma_input(p, "dma_input_prev_yuv", &param->dma_input_prev_yuv, &rem);
	p = dump_param_dma_input(p, "dma_input_prev_w", &param->dma_input_prev_w, &rem);
	p = dump_param_dma_input(p, "dma_input_cur_w", &param->dma_input_cur_w, &rem);
	p = dump_param_dma_input(p, "dma_input_drc_prv_map", &param->dma_input_drc_prv_map, &rem);
	p = dump_param_dma_input(p, "dma_input_drc_cur_map", &param->dma_input_drc_cur_map, &rem);
	p = dump_param_dma_input(p, "dma_input_mv", &param->dma_input_mv, &rem);
	p = dump_param_dma_input(p, "dma_input_mv_mixer", &param->dma_input_mv_mixer, &rem);
	p = dump_param_dma_input(p, "dma_input_sf", &param->dma_input_sf, &rem);
	p = dump_param_dma_output(p, "dma_output_w", &param->dma_output_w, &rem);
...
const struct is_hw_ip_ops is_hw_mcfp_ops = {
	...
	.dump_params		= is_hw_mcfp_dump_params,
};
...
EXPORT_SYMBOL_GPL(is_hw_mcfp_remove);
