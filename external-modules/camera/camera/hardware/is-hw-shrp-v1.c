...
static size_t is_hw_shrp_dump_params(struct is_hw_ip *hw_ip, u32 instance, char *buf, size_t size)
{
	struct is_hw_shrp *hw = (struct is_hw_shrp *)hw_ip->priv_info;
	struct shrp_param_set *param;
	size_t rem = size;
	char *p = buf;
	...
	p = dump_param_dma_input(p, "dma_input_hf", &param->dma_input_hf, &rem);
	p = dump_param_otf_input(p, "otf_input_hf", &param->otf_input_hf, &rem);
	p = dump_param_otf_output(p, "otf_output_hf", &param->otf_output_hf, &rem);
...
const struct is_hw_ip_ops is_hw_shrp_ops = {
	...
	.dump_params		= is_hw_shrp_dump_params,
};
...
EXPORT_SYMBOL_GPL(is_hw_shrp_remove);
