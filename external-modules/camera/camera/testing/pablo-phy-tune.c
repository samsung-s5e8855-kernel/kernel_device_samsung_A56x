...
static u32 phy_tune_enable_value;
...
static void __prepare_skew_code_sweep(struct is_device_csi *csi)
{
	int ret;
	struct device *dev;
	...
	device = v4l2_get_subdev_hostdata(*csi->subdev);
	dev = &device->pdev->dev;
	dnode = dev->of_node;
	phy_tune_enable_value = 0;
	ret = of_property_read_u32(dnode, "phy_tune_enable_value", &phy_tune_enable_value);
	if (ret) {
...
static void __reset_release_skew_code_sweep(struct phy *phy, u32 enable_value, bool enable)
{
	int ret = 0;
	struct phy_sfr_info sf_info;
	struct mipi_phy_desc *phy_desc;
	void __iomem *regs_cmn;
	...
	phy_desc = phy_get_drvdata(phy);
	regs_cmn = phy_desc->regs_cmn;
	sf_info.addr = 0x0004;
	sf_info.start = 0;
	sf_info.width = 32;
	sf_info.val = enable_value;
	if (!enable) {
		update_bits(regs_cmn + sf_info.addr, sf_info.start,sf_info.width, 0x0);
        /* reset assertion */
        ret = phy_reset(phy);
        if (ret)
		...
		update_bits(regs_cmn + sf_info.addr, sf_info.start,sf_info.width, sf_info.val);
		/* reset release */
...
static void __skew_code_sweep(u32 num_skew_code, u32 skew_test_time, u32 max_lane,
			...
			u32 start, u32 width, bool is_data)
{
	struct phy_sfr_info sf_info;
	u32 i, lane;
	u32 data_val_lsb;
	...
	sf_info.addr = PHY_ADDR_DESKEW_CAL_CODE;
	sf_info.start = start;
	sf_info.width = width;

	for (i = 0; i < num_skew_code; i++) {

		__reset_release_skew_code_sweep(csi->phy, phy_tune_enable_value, false);
		for (lane = 0; lane < max_lane; lane++) {
			sf_info.val = i;
			if (IS_ENABLED(USE_SKEW_CAL_DATA_LSB) && is_data) {
				data_val_lsb = sf_info.val & 0x3;
				update_bits(regs + (lane * PHY_LANE_OFFSET) + sf_info.addr,
					SKEW_CAL_DATA_START_POS_LSB, SKEW_CAL_DATA_WIDTH_LSB, data_val_lsb);
				sf_info.val = (sf_info.val & 0x1C) >> SKEW_CAL_DATA_WIDTH_LSB;
			}
			update_bits(regs + (lane * PHY_LANE_OFFSET) + sf_info.addr,
				sf_info.start, sf_info.width, sf_info.val);
		}
		__reset_release_skew_code_sweep(csi->phy, phy_tune_enable_value, true);
		...
		info("%s[SKEW_CAL_%s_CODE: %d] s:%d, e:%d, err:%d\n", __func__,
			is_data ? "DATA" : "CLK", i, csi->state_cnt.str,
	...
	for (lane = 0; lane < max_lane; lane++) {
		if (IS_ENABLED(USE_SKEW_CAL_DATA_LSB) && is_data)
			update_bits(regs + (lane * PHY_LANE_OFFSET) + sf_info.addr,
				SKEW_CAL_DATA_START_POS_LSB, SKEW_CAL_DATA_WIDTH_LSB, sf_info.val);
			...
			struct v4l2_subdev *sd_mod, void __iomem *regs)
{
	...
	/* Enable SKEW_CAL */
	sf_info.addr = PHY_ADDR_DESKEW_CAL_EN;
	sf_info.start = SKEW_CAL_ENABLE_START_POS;
	sf_info.width = SKEW_CAL_ENABLE_WIDTH;
	sf_info.val = 0;

	__prepare_skew_code_sweep(csi);
	for (lane = 0; lane < max_lane; lane++)
		update_bits(regs + (lane * PHY_LANE_OFFSET) + sf_info.addr,
				sf_info.start, sf_info.width, sf_info.val);
	...
	__skew_code_sweep(num_skew_code, skew_test_time, max_lane, csi, sd_csi,
			sd_mod, regs, SKEW_CAL_DATA_START_POS, SKEW_CAL_DATA_WIDTH, true);

	/* Sweep SKEW_CAL_CLK_CODE */
	__skew_code_sweep(num_skew_code, skew_test_time, max_lane, csi, sd_csi,
			sd_mod, regs, SKEW_CAL_CLK_START_POS, SKEW_CAL_CLK_WIDTH, false);
			...
			struct v4l2_subdev *sd_mod, void __iomem *regs)
{
	...
	regs += ADDR_CRC_FORCE_OFFSET;
	sf_info.addr = PHY_ADDR_CRC_FORCE_CODE;
	sf_info.start = CRC_FORCE_CODE_START_POS;
	sf_info.width = CRC_FORCE_CODE_WIDTH;
	for (i = 0; i < num_crc_code; i++) {
		sf_info.val = i << CRC_FORCE_CODE_OFFSET | 0x1;
...
int pablo_test_set_phy_tune(const char *val, const struct kernel_param *kp)
{
	...
	u32 prev, next;
	const u32 elem_sz = sizeof(struct phy_sfr_info) / sizeof(u32);
	...
	data_lane = csi->sensor_cfg[csi->nfi_toggle]->lanes + 1;
