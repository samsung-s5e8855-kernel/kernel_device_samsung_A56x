...
static void pst_start_hw_ip(
	struct is_hw_ip *hw_ip, struct is_frame *frame, const struct pst_callback_ops *pst_cb)
{
	u32 instance = 0;
	ulong hw_map = 0;
			...
			const struct pst_callback_ops *pst_cb)
{
	...
	if (hw_id >= DEV_HW_END) {
		...
		pst_start_hw_ip(hw_ip, frame, pst_cb);
...
void pst_get_size_by_format(
	enum dma_inout_format format, u32 plane, u32 stride, u32 height, size_t *size)
{
	switch (format) {
