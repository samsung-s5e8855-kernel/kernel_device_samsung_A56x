...
static const unsigned int pst_byrp_dma[] = {
	[P_BYRP_B_RDMA] = BYRP_R_BYR_RDMABYRIN_EN,
};
...
static int pst_set_hw_byrp(const char *val, const struct kernel_param *kp)
{
	return pst_set_hw_ip(val, DEV_HW_BYRP, frame_byrp, byrp_param, &byrp_cr_set,
		ARRAY_SIZE(byrp_param_preset), result, &pst_cb_byrp);
}
...
static int pst_get_hw_byrp(char *buffer, const struct kernel_param *kp)
{
	...
	ret = pst_get_hw_ip(buffer, "BYRP", ARRAY_SIZE(byrp_param_preset), result);
