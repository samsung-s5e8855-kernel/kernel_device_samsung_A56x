...
static int pst_set_hw_rgbp(const char *val, const struct kernel_param *kp)
{
	return pst_set_hw_ip(val, DEV_HW_RGBP, frame_rgbp, rgbp_param, &rgbp_cr_set,
		ARRAY_SIZE(rgbp_param_preset), result, &pst_cb_rgbp);
}
...
static int pst_get_hw_rgbp(char *buffer, const struct kernel_param *kp)
{
	...
	ret = pst_get_hw_ip(buffer, "RGBP", ARRAY_SIZE(rgbp_param_preset), result);
