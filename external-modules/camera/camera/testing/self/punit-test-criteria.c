...
#define CHECK_DQUE_COND(crit, group, frame)                                                        \
	(CHECK_EQ(crit->group_id, group->id) && CHECK_FCOUNT(crit->fcount, frame->fcount))
...
static int punit_add_criteria(struct punit_test_criteria in)
{
	u32 index = 0;
	struct punit_test_criteria *criteria;

	if (in.check_point >= PUNIT_CHECK_POINT_MAX || in.group_id >= GROUP_ID_MAX ||
	    in.check_type >= PUNIT_CHECK_TYPE_MAX || in.check_operator >= PUNIT_OP_MAX) {
		pr_err("Invalid parameter\n");
		return -EINVAL;
	}

	if (ZERO_OR_NULL_PTR(point_criteria[in.instance][in.check_point])) {
		point_criteria[in.instance][in.check_point] =
		...
		if (point_criteria[in.instance][in.check_point]->count == PUNIT_CRITERIA_MAX) {
			pr_err("exceeded the number of available criteria\n");
			return -ENOMEM;
		}
	}

	index = point_criteria[in.instance][in.check_point]->count;
	point_criteria[in.instance][in.check_point]->count++;

	criteria = &point_criteria[in.instance][in.check_point]->criteria[index];
	memcpy(criteria, &in, sizeof(struct punit_test_criteria));
...
static void punit_reset_criteria(struct punit_test_criteria *crit)
{
	crit->instance = 0;
	crit->check_point = PUNIT_CHECK_POINT_MAX;
	crit->group_id = GROUP_ID_MAX;
	crit->fcount = 0;
	crit->check_type = PUNIT_CHECK_TYPE_MAX;
	crit->check_variable = PUNIT_DVFS_VAR_MAX;
	crit->check_operator = PUNIT_OP_MAX;
	crit->expect_value = 0;
	crit->actual_value = -1;
	crit->check_result = -1;
}
...
int punit_set_test_criteria_info(int argc, char **argv)
{
	...
	int ret = 0;
	int arg_i = 1;
	struct punit_test_criteria crit = {0,};

	punit_reset_criteria(&crit);
	...
	ret = kstrtouint(argv[arg_i], 0, &crit.instance);
	if (ret) {
		pr_err("%s: Invalid parameter[%s]\n", __func__, argv[arg_i]);
		return ret;
	}

	if (crit.instance >= IS_STREAM_COUNT) {
		pr_err("%s: Invalid parameters. instance %d >= %d\n", __func__, crit.instance,
		       IS_STREAM_COUNT);
		return -EINVAL;
	}
	...
	for (i = arg_i; i < argc; i += 2) {
		if (strcmp(argv[i], "-p") == 0) {
			ret = kstrtouint(argv[i + 1], 0, &crit.check_point);
			if (ret) {
				pr_err("%s: Invalid parameter[%s]\n", __func__, argv[i + 1]);
				return ret;
			}

			if (crit.check_point >= PUNIT_CHECK_POINT_MAX) {
				pr_err("%s: Invalid parameters. check_point %d >= %d\n", __func__,
				       crit.check_point, PUNIT_CHECK_POINT_MAX);
				return -EINVAL;
			}
		} else if (strcmp(argv[i], "-g") == 0) {
			ret = kstrtouint(argv[i + 1], 0, &crit.group_id);
			if (ret) {
				pr_err("%s: Invalid parameter[%s]\n", __func__, argv[i + 1]);
				return ret;
			}
		} else if (strcmp(argv[i], "-f") == 0) {
			ret = kstrtouint(argv[i + 1], 0, &crit.fcount);
			if (ret) {
				pr_err("%s: Invalid parameter[%s]\n", __func__, argv[i + 1]);
				return ret;
			}
		} else if (strcmp(argv[i], "-t") == 0) {
			ret = kstrtouint(argv[i + 1], 0, &crit.check_type);
			if (ret) {
				pr_err("%s: Invalid parameter[%s]\n", __func__, argv[i + 1]);
				return ret;
			}
		} else if (strcmp(argv[i], "-v") == 0) {
			ret = kstrtouint(argv[i + 1], 0, &crit.check_variable);
			if (ret) {
				pr_err("%s: Invalid parameter[%s]\n", __func__, argv[i + 1]);
				return ret;
			}
		} else if (strcmp(argv[i], "-o") == 0) {
			ret = kstrtouint(argv[i + 1], 0, &crit.check_operator);
			if (ret) {
				pr_err("%s: Invalid parameter[%s]\n", __func__, argv[i + 1]);
				return ret;
			}
		} else if (strcmp(argv[i], "-e") == 0) {
			ret = kstrtouint(argv[i + 1], 0, &crit.expect_value);
			if (ret) {
				pr_err("%s: Invalid parameter[%s]\n", __func__, argv[i + 1]);
				return ret;
			}
		} else if (strcmp(argv[i], "-d") == 0) {
			strreplace(argv[i + 1], '_', ' ');
			desc = argv[i + 1];
			if (!desc) {
				pr_err("%s: Invalid desc parameter\n", __func__);
				return -EINVAL;
			}
			memcpy(crit.desc, desc, strlen(crit.desc));
		} else {
			pr_err("%s: Invalid parameter[%s]\n", __func__, argv[i]);
			return -EINVAL;
		}
...
static int punit_default_res_formatter(struct punit_test_criteria *criteria, char *buffer)
{
	...
	ret = sprintf(buffer, "%s [I#%d][G#%d][F#%d] actual : %d / expect : %d %s",
		CHECK_RES(criteria->check_result), criteria->instance, criteria->group_id,
		criteria->fcount, criteria->actual_value, criteria->expect_value, criteria->desc);
	return ret;
}
