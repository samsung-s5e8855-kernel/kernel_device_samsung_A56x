...
static int punit_set_param_handler(u32 cmd, u32 value)
{
	...
	case V4L2_CID_IS_S_DVFS_RECORDING_SIZE: {
		...
		dvfs_ctrl->dvfs_rec_size = value;
		pr_info("[S_PARAM][DVFS] dvfs_rec_size width(%d), height(%d)\n",
			dvfs_ctrl->dvfs_rec_size & 0xffff,
			dvfs_ctrl->dvfs_rec_size >> IS_DVFS_SCENARIO_HEIGHT_SHIFT);
	} break;
	case V4L2_CID_IS_SET_SETFILE: {
		struct is_device_ischain *device = is_get_ischain_device(0);

		if (!device) {
			pr_err("[S_PARAM][DVFS] device is NULL");
			return -EINVAL;
		}
		device->setfile = value;
		pr_info("[S_PARAM][DVFS] V4L2_CID_IS_SET_SETFILE (%d)\n", device->setfile);
	} break;
	default:
