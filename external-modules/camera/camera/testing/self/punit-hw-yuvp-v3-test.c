...
#define NUM_OF_YUVP_PARAM (PARAM_YUVP_PCCHIST_W - PARAM_YUVP_CONTROL + 1)
...
static const struct yuvp_param yuvp_param_preset_grp[] = {
	...
	[0].pcchist_in.cmd = DMA_OUTPUT_COMMAND_DISABLE,

	[0].pcchist_out.cmd = DMA_OUTPUT_COMMAND_DISABLE,
};
...
static const struct yuvp_param yuvp_param_preset[] = {
	...
	[1].otf_input.physical_width = 1920,
	[1].otf_input.physical_height = 1440,
	[1].otf_input.offset_x = 0,
	[1].otf_input.offset_y = 0,

	[1].otf_output.cmd = OTF_OUTPUT_COMMAND_ENABLE,
	...
	[1].fto_output.cmd = OTF_OUTPUT_COMMAND_DISABLE,

	[1].stripe_input.total_count = 0,

	[1].seg.cmd = DMA_OUTPUT_COMMAND_DISABLE,

	[1].drc.cmd = DMA_OUTPUT_COMMAND_DISABLE,

	[1].clahe.cmd = DMA_OUTPUT_COMMAND_DISABLE,

	[1].pcchist_in.cmd = DMA_OUTPUT_COMMAND_DISABLE,

	[1].pcchist_out.cmd = DMA_OUTPUT_COMMAND_DISABLE,

	/* Param set: YUV RDMA bypass 422 10bit to MCSC */
	...
	[2].otf_input.physical_width = 1920,
	[2].otf_input.physical_height = 1440,
	[2].otf_input.offset_x = 0,
	[2].otf_input.offset_y = 0,

	[2].otf_output.cmd = OTF_OUTPUT_COMMAND_ENABLE,
	...
	[2].dma_input.plane = DMA_INOUT_PLANE_2,
	...
	[2].fto_output.cmd = OTF_OUTPUT_COMMAND_DISABLE,

	[2].stripe_input.total_count = 0,

	[2].seg.cmd = DMA_OUTPUT_COMMAND_DISABLE,

	[2].drc.cmd = DMA_OUTPUT_COMMAND_DISABLE,

	[2].clahe.cmd = DMA_OUTPUT_COMMAND_DISABLE,

	[2].pcchist_in.cmd = DMA_OUTPUT_COMMAND_DISABLE,

	[2].pcchist_out.cmd = DMA_OUTPUT_COMMAND_DISABLE,
};
...
static void pst_set_buf_yuvp(struct is_frame *frame, u32 param_idx)
{
	...
	case PARAM_YUVP_CLAHE:
		...
		pst_get_size_of_dma_input(&yuvp_param[param_idx], align, block_w, block_h, size);
		break;
	case PARAM_YUVP_PCCHIST_R:
		dva = frame->dva_yuvp_out_pcchist;
		pst_get_size_of_dma_output(&yuvp_param[param_idx], align, size);
		break;
	case PARAM_YUVP_PCCHIST_W:
		dva = frame->dva_yuvp_cap_pcchist;
		pst_get_size_of_dma_output(&yuvp_param[param_idx], align, size);
		break;
...
static void pst_init_param_yuvp(unsigned int index, enum pst_hw_ip_type type)
{
	...
	memcpy(yuvp_param[i++], (u32 *)&preset[index].pcchist_in, PARAMETER_MAX_SIZE);
	memcpy(yuvp_param[i++], (u32 *)&preset[index].pcchist_out, PARAMETER_MAX_SIZE);
}
