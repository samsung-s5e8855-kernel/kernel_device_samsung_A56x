...
static int pst_set_hw_mcsc(const char *val, const struct kernel_param *kp)
{
	return pst_set_hw_ip(val, DEV_HW_MCSC0, frame_mcsc, mcsc_param, &mcsc_cr_set,
		ARRAY_SIZE(mcsc_param_preset), result, &pst_cb_mcsc);
}
...
static int pst_get_hw_mcsc(char *buffer, const struct kernel_param *kp)
{
	...
	ret = pst_get_hw_ip(buffer, "MCSC", ARRAY_SIZE(mcsc_param_preset), result);
