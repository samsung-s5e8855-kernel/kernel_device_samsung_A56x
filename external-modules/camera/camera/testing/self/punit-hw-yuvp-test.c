...
static int pst_set_hw_yuvp(const char *val, const struct kernel_param *kp)
{
	return pst_set_hw_ip(val, DEV_HW_YPP, frame_yuvp, yuvp_param, &yuvp_cr_set,
		ARRAY_SIZE(yuvp_param_preset), result, &pst_cb_yuvp);
}
...
static int pst_get_hw_yuvp(char *buffer, const struct kernel_param *kp)
{
	...
	ret = pst_get_hw_ip(buffer, "YUVP", ARRAY_SIZE(yuvp_param_preset), result);
