...
static int pgt_qbuf_group(struct is_group *head, u32 fcount, struct camera2_shot *shot_meta)
{
	int ret;
	...
	if (!ZERO_OR_NULL_PTR(shot_meta))
		memcpy(frame->shot, shot_meta, sizeof(struct camera2_shot));
...
static int pgt_dqbuf_group(struct is_group *head)
{
	...
	for (i = 0; i < com_cnt; i++) {
		...
		/* qbuf next group */
		if (head->gnext && !ndone)
			pgt_qbuf_group(head->gnext, cur_fcount, frame->shot);
...
static void pgt_init_sensor(u32 instance)
{
	...
	/* s_input */
	if (st_info->enable[GROUP_SLOT_SENSOR + 1])
		scenario = SENSOR_SCENARIO_NORMAL;
	else
		scenario = SENSOR_SCENARIO_VISION;

	is_sensor_s_input(ids, sensor_position, scenario, 1);
	...
	ids->ex_mode = st_info->sensor_ex_mode;
	ids->ex_mode_format = st_info->sensor_ex_mode_format;
...
static void pgt_start_sensor(u32 instance)
{
	...
	for (i = 0; i < queue->buf_maxcount; i++) {
		...
		ret = pgt_qbuf_group(&ids->group_sensor, 0, NULL);
		if (ret)
			return;
	}
...
static int pgt_qbuf_ischain(u32 instance)
{
	...
	framemgr = GET_HEAD_GROUP_FRAMEMGR(head);
	for (i = 0; i < framemgr->num_frames; i++) {
		ret = pgt_qbuf_group(head, 0, NULL);
		if (ret)
			return ret;
	}
...
static int set_punit_group_test(const char *val, const struct kernel_param *kp)
{
	...
	case PUNIT_GROUP_TEST_ACT_SET_PARAM:
		...
		break;
	case PUNIT_GROUP_TEST_ACT_SET_NFI: {
		struct punit_group_test_static_info *st_info = &pgt_st_info[instance];
		struct is_device_sensor *ids = is_get_sensor_device(st_info->sensor_id);
		struct is_fid_loc *fid_loc, save;

		fid_loc = &ids->cfg[ids->nfi_toggle]->fid_loc;
		memcpy(&save, fid_loc, sizeof(struct is_fid_loc));

		fid_loc->valid = true;
		fid_loc->byte = 10;
		fid_loc->nfi_byte = 20;

		ids->cfg[!ids->nfi_toggle] = ids->cfg[ids->nfi_toggle];
		schedule_work(&ids->nfi_work);
		fsleep(100);

		memcpy(fid_loc, &save, sizeof(struct is_fid_loc));
		break;
	}
	default:
