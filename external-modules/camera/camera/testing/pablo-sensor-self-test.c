...
static DECLARE_BITMAP(result_sensor, sizeof(unsigned long) * 8);
static const u32 pst_csi_camdev_id[PST_CSI_MAX] = {
	1225, /* csi s_fmt */
};
...
static void pst_check_csi_input_size(void)
{
	bool cond;
	struct is_device_csi *csi;
	u32 width, height;
	...
	width = csi->sensor_cfg[csi->nfi_toggle]->input[0].width;
	height = csi->sensor_cfg[csi->nfi_toggle]->input[0].height;

	cond = (width == sensor->sensor_width) && (height == sensor->sensor_height);
	UTC_UPDATE_BIT(PST_CSI_SIZE, result_sensor, cond);
}
...
static int pablo_test_get_sensor_run(char *buffer, const struct kernel_param *kp)
{
	...
	for (i = 0; i < PST_CSI_MAX; i++)
		ret += sprintf(buffer + ret, "%d:%d\n", pst_csi_camdev_id[i],
			       test_bit(i, result_sensor) ? P_PASS : P_FAIL);
