...
static void pablo_hw_chain_info_cr_set_kunit_test(struct kunit *test)
{
	...
	frame->kva_mtnr0_rta_info[PLANE_INDEX_CR_SET] = (u64)addr_contents;
	scs = CALL_HW_CHAIN_INFO_OPS(hw, get_cr_set, DEV_HW_MTNR0, frame);
	KUNIT_EXPECT_EQ(test, (u64)scs, (u64)addr_contents);

	frame->kva_mtnr1_rta_info[PLANE_INDEX_CR_SET] = (u64)addr_contents;
	scs = CALL_HW_CHAIN_INFO_OPS(hw, get_cr_set, DEV_HW_MTNR1, frame);
	KUNIT_EXPECT_EQ(test, (u64)scs, (u64)addr_contents);
...
static void pablo_hw_chain_info_get_config_kunit_test(struct kunit *test)
{
	...
	frame->kva_mtnr0_rta_info[PLANE_INDEX_CONFIG] = (u64)addr_contents;
	config_data[0] = sizeof(struct is_mtnr0_config);
...
static void pablo_hw_chain_info_get_hw_slot_id_kunit_test(struct kunit *test)
{
	...
	slot_id = CALL_HW_CHAIN_INFO_OPS(hw, get_hw_slot_id, DEV_HW_BYRP0);
	KUNIT_EXPECT_TRUE(test, slot_id == 4);

	slot_id = CALL_HW_CHAIN_INFO_OPS(hw, get_hw_slot_id, DEV_HW_BYRP1);
	KUNIT_EXPECT_TRUE(test, slot_id == 5);

	slot_id = CALL_HW_CHAIN_INFO_OPS(hw, get_hw_slot_id, DEV_HW_BYRP2);
	KUNIT_EXPECT_TRUE(test, slot_id == 6);

	slot_id = CALL_HW_CHAIN_INFO_OPS(hw, get_hw_slot_id, DEV_HW_BYRP3);
	KUNIT_EXPECT_TRUE(test, slot_id == 7);

	slot_id = CALL_HW_CHAIN_INFO_OPS(hw, get_hw_slot_id, DEV_HW_BYRP4);
	KUNIT_EXPECT_TRUE(test, slot_id == 8);

	slot_id = CALL_HW_CHAIN_INFO_OPS(hw, get_hw_slot_id, DEV_HW_RGBP0);
	KUNIT_EXPECT_TRUE(test, slot_id == 9);

	slot_id = CALL_HW_CHAIN_INFO_OPS(hw, get_hw_slot_id, DEV_HW_RGBP1);
	KUNIT_EXPECT_TRUE(test, slot_id == 10);

	slot_id = CALL_HW_CHAIN_INFO_OPS(hw, get_hw_slot_id, DEV_HW_RGBP2);
	KUNIT_EXPECT_TRUE(test, slot_id == 11);

	slot_id = CALL_HW_CHAIN_INFO_OPS(hw, get_hw_slot_id, DEV_HW_RGBP3);
	KUNIT_EXPECT_TRUE(test, slot_id == 12);

	slot_id = CALL_HW_CHAIN_INFO_OPS(hw, get_hw_slot_id, DEV_HW_YUVSC0);
	KUNIT_EXPECT_TRUE(test, slot_id == 13);

	slot_id = CALL_HW_CHAIN_INFO_OPS(hw, get_hw_slot_id, DEV_HW_YUVSC1);
	KUNIT_EXPECT_TRUE(test, slot_id == 14);

	slot_id = CALL_HW_CHAIN_INFO_OPS(hw, get_hw_slot_id, DEV_HW_YUVSC2);
	KUNIT_EXPECT_TRUE(test, slot_id == 15);

	slot_id = CALL_HW_CHAIN_INFO_OPS(hw, get_hw_slot_id, DEV_HW_YUVSC3);
	KUNIT_EXPECT_TRUE(test, slot_id == 16);

	slot_id = CALL_HW_CHAIN_INFO_OPS(hw, get_hw_slot_id, DEV_HW_MLSC0);
	KUNIT_EXPECT_TRUE(test, slot_id == 17);

	slot_id = CALL_HW_CHAIN_INFO_OPS(hw, get_hw_slot_id, DEV_HW_MLSC1);
	KUNIT_EXPECT_TRUE(test, slot_id == 18);

	slot_id = CALL_HW_CHAIN_INFO_OPS(hw, get_hw_slot_id, DEV_HW_MLSC2);
	KUNIT_EXPECT_TRUE(test, slot_id == 19);

	slot_id = CALL_HW_CHAIN_INFO_OPS(hw, get_hw_slot_id, DEV_HW_MLSC3);
	KUNIT_EXPECT_TRUE(test, slot_id == 20);

	slot_id = CALL_HW_CHAIN_INFO_OPS(hw, get_hw_slot_id, DEV_HW_MTNR0);
	KUNIT_EXPECT_TRUE(test, slot_id == 21);

	slot_id = CALL_HW_CHAIN_INFO_OPS(hw, get_hw_slot_id, DEV_HW_MTNR1);
	KUNIT_EXPECT_TRUE(test, slot_id == 22);

	slot_id = CALL_HW_CHAIN_INFO_OPS(hw, get_hw_slot_id, DEV_HW_MSNR);
	KUNIT_EXPECT_TRUE(test, slot_id == 23);

	slot_id = CALL_HW_CHAIN_INFO_OPS(hw, get_hw_slot_id, DEV_HW_YPP);
	KUNIT_EXPECT_TRUE(test, slot_id == 24);

	slot_id = CALL_HW_CHAIN_INFO_OPS(hw, get_hw_slot_id, DEV_HW_MCSC0);
	KUNIT_EXPECT_TRUE(test, slot_id == 25);
...
static void pablo_hw_chain_info_check_crta_hw_kunit_test(struct kunit *test)
{
	bool ret;

	ret = CALL_HW_CHAIN_INFO_OPS(hw, check_crta_hw, DEV_HW_PAF0);
	KUNIT_EXPECT_TRUE(test, ret == true);
	ret = CALL_HW_CHAIN_INFO_OPS(hw, check_crta_hw, DEV_HW_PAF1);
	KUNIT_EXPECT_TRUE(test, ret == true);
	ret = CALL_HW_CHAIN_INFO_OPS(hw, check_crta_hw, DEV_HW_PAF2);
	KUNIT_EXPECT_TRUE(test, ret == true);
	ret = CALL_HW_CHAIN_INFO_OPS(hw, check_crta_hw, DEV_HW_PAF3);
	KUNIT_EXPECT_TRUE(test, ret == true);
	ret = CALL_HW_CHAIN_INFO_OPS(hw, check_crta_hw, DEV_HW_BYRP0);
	KUNIT_EXPECT_TRUE(test, ret == true);
	ret = CALL_HW_CHAIN_INFO_OPS(hw, check_crta_hw, DEV_HW_BYRP1);
	KUNIT_EXPECT_TRUE(test, ret == true);
	ret = CALL_HW_CHAIN_INFO_OPS(hw, check_crta_hw, DEV_HW_BYRP2);
	KUNIT_EXPECT_TRUE(test, ret == true);
	ret = CALL_HW_CHAIN_INFO_OPS(hw, check_crta_hw, DEV_HW_BYRP3);
	KUNIT_EXPECT_TRUE(test, ret == true);
	ret = CALL_HW_CHAIN_INFO_OPS(hw, check_crta_hw, DEV_HW_MTNR0);
...
static int pablo_hw_chain_info_kunit_test_init(struct kunit *test)
{
	...
	/* Consider MTNR0 + DLFE integrated CR buffer */
...
static struct kunit_case pablo_hw_chain_info_kunit_test_cases[] = {
	...
	KUNIT_CASE(pablo_hw_chain_info_check_crta_hw_kunit_test),
	{},
};
