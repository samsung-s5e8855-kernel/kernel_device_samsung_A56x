...
static void pablo_hw_dvfs_get_lv_kunit_test(struct kunit *test)
{
	...
	ids->cfg[NFI_TOGGLE_0]->max_vc = 4;
	ids->cfg[NFI_TOGGLE_0]->lanes = 2; /* C-phy 3 Trio */
	ids->cfg[NFI_TOGGLE_0]->mipi_speed = 2784;
	ids->cfg[NFI_TOGGLE_0]->max_fps = 60;
	ids->cfg[NFI_TOGGLE_0]->height = 4080;
	ids->cfg[NFI_TOGGLE_0]->width = 3060;
	ids->cfg[NFI_TOGGLE_0]->dvfs_lv_csis = IS_DVFS_LV_END;
	ids->cfg[NFI_TOGGLE_0]->input[0].hwformat = HW_FORMAT_RAW10;
	ids->cfg[NFI_TOGGLE_0]->output[0][0].hwformat = HW_FORMAT_RAW10;
	ids->cfg[NFI_TOGGLE_0]->dma_num = 1;
	ids->cfg[NFI_TOGGLE_0]->link_vc[0][0] = 0;
	ids->dma_output[0].fmt.hw_format = ids->cfg[NFI_TOGGLE_0]->output[0][0].hwformat;
	ids->dma_output[0].fmt.bitsperpixel[0] = 10;
	ids->dma_output[0].fmt.hw_bitwidth = 10;
	ids->state = 0xfffffff;
	backup_cphy = ids->pdata->use_cphy;
	ids->pdata->use_cphy = 1;

	ret = is_hw_dvfs_get_lv(dvfs_ctrl, IS_DVFS_CSIS);
	KUNIT_EXPECT_EQ(test, ret, 6);

	ids->cfg[NFI_TOGGLE_0]->input[0].extformat = HW_FORMAT_RAW10_POTF & HW_EXT_FORMAT_MASK;
	ids->cfg[NFI_TOGGLE_0]->output[0][0].extformat = HW_FORMAT_RAW10_POTF & HW_EXT_FORMAT_MASK;
	ids->dma_output[0].fmt.hw_bitwidth = 16;
	ret = is_hw_dvfs_get_lv(dvfs_ctrl, IS_DVFS_CSIS);
	KUNIT_EXPECT_EQ(test, ret, 6);

	ids->cfg[NFI_TOGGLE_0]->dvfs_lv_csis = 1;
	ret = is_hw_dvfs_get_lv(dvfs_ctrl, IS_DVFS_CSIS);
	KUNIT_EXPECT_EQ(test, ret, 1);

	ids->state = 0x0;
	ids->pdata->use_cphy = backup_cphy;
}
...
static void pablo_hw_dvfs_get_scenario_kunit_test(struct kunit *test)
{
	...
	struct is_sensor_cfg *cfg = ids1->cfg[NFI_TOGGLE_0];
...
static int pablo_hw_dvfs_kunit_test_init(struct kunit *test)
{
	...
	test_ctx.ids1->cfg[NFI_TOGGLE_0] = &test_ctx.cfg;
	test_ctx.ids2->cfg[NFI_TOGGLE_0] = &test_ctx.cfg;
	test_ctx.ids3->cfg[NFI_TOGGLE_0] = &test_ctx.cfg;
