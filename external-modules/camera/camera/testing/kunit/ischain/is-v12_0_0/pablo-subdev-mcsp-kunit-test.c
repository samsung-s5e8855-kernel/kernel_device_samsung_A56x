...
static void pablo_subdev_mcsp_tag_dma_kunit_test(struct kunit *test)
{
	...
	utc_flag = cap_node->input.cropRegion[0] == 0;
	KUNIT_EXPECT_EQ(test, cap_node->input.cropRegion[2], 1920);
	utc_flag = utc_flag ? cap_node->input.cropRegion[2] == 1920 : 0;
	KUNIT_EXPECT_EQ(test, cap_node->input.cropRegion[3], 1080);
	utc_flag = utc_flag ? cap_node->input.cropRegion[3] == 1080 : 0;
	KUNIT_EXPECT_EQ(test, cap_node->output.cropRegion[0], 0);
	utc_flag = utc_flag ? cap_node->output.cropRegion[0] == 0 : 0;
	KUNIT_EXPECT_EQ(test, cap_node->output.cropRegion[2], 1920);
	utc_flag = utc_flag ? cap_node->output.cropRegion[2] == 1920 : 0;
	KUNIT_EXPECT_EQ(test, cap_node->output.cropRegion[3], 1080);
	utc_flag = utc_flag ? cap_node->output.cropRegion[3] == 1080 : 0;

	set_utc_result(KUTC_MCSC_STRIDE_ALIGN, UTC_ID_MCSC_STRIDE_ALIGN, utc_flag);
