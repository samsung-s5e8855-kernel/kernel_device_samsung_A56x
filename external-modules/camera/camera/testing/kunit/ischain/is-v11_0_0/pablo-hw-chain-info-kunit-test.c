...
static void pablo_hw_chain_info_check_crta_hw_kunit_test(struct kunit *test)
{
	...
	KUNIT_EXPECT_TRUE(test, ret == true);
	ret = CALL_HW_CHAIN_INFO_OPS(hw, check_crta_hw, DEV_HW_3AA0);
	KUNIT_EXPECT_TRUE(test, ret == true);
	ret = CALL_HW_CHAIN_INFO_OPS(hw, check_crta_hw, DEV_HW_3AA1);
	KUNIT_EXPECT_TRUE(test, ret == true);
	ret = CALL_HW_CHAIN_INFO_OPS(hw, check_crta_hw, DEV_HW_3AA2);
	KUNIT_EXPECT_TRUE(test, ret == true);
	ret = CALL_HW_CHAIN_INFO_OPS(hw, check_crta_hw, DEV_HW_3AA3);
	KUNIT_EXPECT_TRUE(test, ret == true);
