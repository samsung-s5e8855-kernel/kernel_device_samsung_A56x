...
static void pablo_hw_dvfs_get_lv_kunit_test(struct kunit *test)
{
	...
	ids->cfg[NFI_TOGGLE_0]->lanes = 1;
	ids->cfg[NFI_TOGGLE_0]->mipi_speed = 1000;
	ids->cfg[NFI_TOGGLE_0]->max_fps = 120;
	ids->cfg[NFI_TOGGLE_0]->height = 1920;
	ids->cfg[NFI_TOGGLE_0]->width = 1080;
	ids->cfg[NFI_TOGGLE_0]->votf = 1;
	ids->cfg[NFI_TOGGLE_0]->dvfs_lv_csis = IS_DVFS_LV_END;
	ids->cfg[NFI_TOGGLE_0]->output[0][0].hwformat = 0x2B;
	ids->cfg[NFI_TOGGLE_0]->output[0][0].extformat = 0x300;
	...
	for (type = 0; type < IS_DVFS_END; type++) {
		...
		set_bit(PABLO_SUBDEV_ALLOC, &csi->i_subdev[NFI_TOGGLE_0][0][0].state);
		ret = is_hw_dvfs_get_lv(dvfs_ctrl, type);

		ids->pdata->use_cphy = 0;
		ret = is_hw_dvfs_get_lv(dvfs_ctrl, type);

		ids->cfg[NFI_TOGGLE_0]->votf = 0;
		ret = is_hw_dvfs_get_lv(dvfs_ctrl, type);
...
static int pablo_hw_dvfs_kunit_test_init(struct kunit *test)
{
	...
	*ids3 = is_get_sensor_device(KUNIT_TEST_SENSOR + 2);
	...
	(*ids1)->cfg[NFI_TOGGLE_0] = &test_ctx.cfg;
	(*idi)->sensor = *ids1;
