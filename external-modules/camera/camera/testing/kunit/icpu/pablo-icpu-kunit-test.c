...
#define SET_ICPU_CPU_WFI_STATUS(_pd)                                                               \
	do {                                                                                       \
		if (IS_ENABLED(CONFIG_PABLO_ICPU_HW_V1_0) ||                                       \
			(IS_ENABLED(CONFIG_PABLO_ICPU_HW_V2_0)))                                   \
			*(u32 *)(_pd.mcuctl_reg_base + 0xC4) = 1;                                  \
		else                                                                               \
			*(u32 *)(_pd.mcuctl_reg_base + 0xC4) = 3;                                  \
	} while (0)
...
{
	u32 ch;
	struct pablo_icpu_mbox_chan *tx_chan[2], *rx_chan[6];
	struct icpu_mbox_client cl;

	tx_chan[0] = pablo_icpu_request_mbox_chan(&cl, ICPU_MBOX_CHAN_TX);
	KUNIT_EXPECT_NOT_ERR_OR_NULL(test, tx_chan[0]);

	tx_chan[1] = pablo_icpu_request_mbox_chan(&cl, ICPU_MBOX_CHAN_TX);
	KUNIT_EXPECT_PTR_EQ(test, (void *)tx_chan[1], NULL);

	pablo_icpu_free_mbox_chan(tx_chan[0]);
	pablo_icpu_free_mbox_chan(tx_chan[1]);

	for (ch = 0 ; ch < pablo_icpu_get_num_rx_mbox(); ch++) {
		rx_chan[ch] = pablo_icpu_request_mbox_chan(&cl, ICPU_MBOX_CHAN_RX);
		KUNIT_EXPECT_NOT_ERR_OR_NULL(test, rx_chan[ch]);
	}
	rx_chan[ch] = pablo_icpu_request_mbox_chan(&cl, ICPU_MBOX_CHAN_RX);
	KUNIT_EXPECT_PTR_EQ(test, (void *)rx_chan[ch], NULL);

	for (ch = 0 ; ch < pablo_icpu_get_num_rx_mbox(); ch++)
		pablo_icpu_free_mbox_chan(rx_chan[ch]);
}
