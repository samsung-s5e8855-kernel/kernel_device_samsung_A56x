...
static void pablo_debug_get_debug_param_kunit_test(struct kunit *test)
{
	...
	const char *clk_usage_str = "debug_clk usage: echo [value|option] > <target param>";
	const char *clk_values_str = "type : NUM\nmax : 2\nvalue : 1";
	char *buf = (char *)kunit_kzalloc(test, PAGE_SIZE, GFP_KERNEL);
	...
	/* normal cases */
	kp = pablo_debug_get_kernel_param(IS_DEBUG_PARAM_CLK);
	/* TC #2. set/get value */
	kp->ops->set("1", kp);
	res = kp->ops->get(buf, kp);
	...
	/* check if debug_clk max value is 2 */
	res = kstrtoul(argv[1], 0, &value);
	KUNIT_EXPECT_EQ(test, res, 0);
	KUNIT_EXPECT_EQ(test, value, 2);

	/* TC #4. get type of debug_param */
	kp->ops->set("type", kp);
	len = kp->ops->get(buf, kp);
	KUNIT_EXPECT_GT(test, len, 0);
	buf[len - 1] = '\0';
	KUNIT_EXPECT_STREQ(test, buf, "NUM");

	/* TC #5. get usage of debug_param */
	kp->ops->set("usage", kp);
	len = kp->ops->get(buf, kp);
	KUNIT_EXPECT_GT(test, len, strlen(clk_usage_str));

	/* compare only the first line of the acquired buf */
	buf[strlen(clk_usage_str)] = '\0';
	KUNIT_EXPECT_STREQ(test, buf, (char *)clk_usage_str);

	/* TC #6. get type, max, value of debug_param with query command */
	kp->ops->set("query", kp);
	len = kp->ops->get(buf, kp);
	KUNIT_EXPECT_EQ(test, len, strlen(clk_values_str));
	KUNIT_EXPECT_STREQ(test, buf, (char *)clk_values_str);

	/* TC #7. get value again after getting max, type, usage */
	res = kp->ops->get(buf, kp);
	KUNIT_EXPECT_GT(test, res, 0);
	res = kstrtoul(buf, 0, &value);
	KUNIT_EXPECT_EQ(test, res, 0);
...
static void pablo_debug_memlog_alloc_dump_kunit_test(struct kunit *test)
{
	...
	/* TC #1. Invalid size */
	ret = is_debug_memlog_alloc_dump(0x12345678, 0, 0, "test");
	KUNIT_EXPECT_LT(test, ret, (int)0);
	KUNIT_EXPECT_PTR_EQ(test, (void *)mobj_dump->obj, NULL);

	/* TC #2. Normal case */
	ret = is_debug_memlog_alloc_dump(0x12345678, 0, 10, "test");
...
static void dummy_dump_func(void)
{
	/* Do nothing */
}

static void pablo_debug_register_dump_func_kunit_test(struct kunit *test)
{
	struct is_debug *debug = pkt_ctx.debug;

	if (debug->num_dump_func >= PABLO_DBG_DUMP_FUNC_NUM) {
		is_debug_register_dump_func(dummy_dump_func);
		KUNIT_EXPECT_PTR_NE(test, debug->dump_func[PABLO_DBG_DUMP_FUNC_NUM - 1],
			(void *)dummy_dump_func);
	} else {
		is_debug_register_dump_func(dummy_dump_func);
		KUNIT_EXPECT_PTR_EQ(
			test, debug->dump_func[debug->num_dump_func - 1], (void *)dummy_dump_func);
		debug->num_dump_func--;
	}
}
...
static struct kunit_case pablo_debug_kunit_test_cases[] = {
	...
	KUNIT_CASE(pablo_debug_register_dump_func_kunit_test),
	{},
};
