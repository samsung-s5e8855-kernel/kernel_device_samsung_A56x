...
static void pablo_lib_json_doc_kunit_test(struct kunit *test)
{
	const char *const stream[] = {
		"{\"stream\":[{\"id\":0,\"name\":\"preview0\",\"sensor\":\"S5KHP2\"},",
		"{\"id\":1,\"name\":\"preview1\",\"sensor\":\"S5KHP2\"}]}"
	};
	...
	for (i = 0; i < 2; i++) {
		p = pablo_json_object_open(p, NULL, &rem);
		p = pablo_json_int(p, "id", i, &rem);
		p = pablo_json_nstr(
			p, "name", name, scnprintf(name, sizeof(name), "preview%d", i), &rem);
		p = pablo_json_nstr(p, "sensor", "S5KHP2", 6, &rem);
		p = pablo_json_object_close(p, &rem);
	}
	p = pablo_json_array_close(p, &rem);
	p = pablo_json_object_close(p, &rem);
	...
	KUNIT_EXPECT_EQ(test, (ulong)(p - buf),
		(ulong)scnprintf(exp, BUF_SIZE, "%s%s", stream[0], stream[1]));
	KUNIT_EXPECT_STREQ(test, buf, (char *)exp);
...
static struct kunit_case pablo_lib_json_kunit_test_cases[] = {
	...
	KUNIT_CASE(pablo_lib_json_doc_kunit_test),
	{},
};
