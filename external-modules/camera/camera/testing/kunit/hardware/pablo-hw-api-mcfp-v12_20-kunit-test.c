...
static void pablo_hw_api_mcfp_hw_s_block_bypass_kunit_test(struct kunit *test)
{
	u32 val[MCFP_BLK_BYPASS_MAX];
	int ret;
	bool utc_flag = false;

	mcfp_hw_s_block_bypass(test_ctx.pmio, COREX_DIRECT);

	ret = mcfp_hw_g_block_bypass(test_ctx.pmio, val, ARRAY_SIZE(val) - 1);
	KUNIT_EXPECT_EQ(test, ret, -EINVAL);

	ret = mcfp_hw_g_block_bypass(test_ctx.pmio, val, ARRAY_SIZE(val));
	KUNIT_EXPECT_EQ(test, ret, 0);
	if (!ret) {
		utc_flag = true;
		KUNIT_EXPECT_EQ(test, val[MCFP_GEOMATCH_EN], 0);
		utc_flag = utc_flag && !val[MCFP_GEOMATCH_EN];
		KUNIT_EXPECT_EQ(test, val[MCFP_GEOMATCH_BYPASS], 1);
		utc_flag = utc_flag && val[MCFP_GEOMATCH_BYPASS];
		KUNIT_EXPECT_EQ(test, val[MCFP_GEOMATCH_MATCH_ENABLE], 0);
		utc_flag = utc_flag && !val[MCFP_GEOMATCH_MATCH_ENABLE];
		KUNIT_EXPECT_EQ(test, val[MCFP_YUV422TO444_BYPASS], 1);
		utc_flag = utc_flag && val[MCFP_YUV422TO444_BYPASS];
		KUNIT_EXPECT_EQ(test, val[MCFP_YUVTORGB_BYPASS], 1);
		utc_flag = utc_flag && val[MCFP_YUVTORGB_BYPASS];
		KUNIT_EXPECT_EQ(test, val[MCFP_GAMMARGB_BYPASS], 1);
		utc_flag = utc_flag && val[MCFP_GAMMARGB_BYPASS];
		KUNIT_EXPECT_EQ(test, val[MCFP_RGBTOYUV_BYPASS], 1);
		utc_flag = utc_flag && val[MCFP_RGBTOYUV_BYPASS];
		KUNIT_EXPECT_EQ(test, val[MCFP_YUV444TO422_BYPASS], 1);
		utc_flag = utc_flag && val[MCFP_YUV444TO422_BYPASS];
	}

	set_utc_result(KUTC_MCFP_BYPASS, UTC_ID_MCFP_BYPASS, utc_flag);
}
...
static struct kunit_case pablo_hw_api_mcfp_kunit_test_cases[] = {
	...
	KUNIT_CASE(pablo_hw_api_mcfp_hw_s_block_bypass_kunit_test),
	{},
};
