...
#define CSTAT_GET_R_COREX(base, R) PMIO_GET_R(base, CSTAT_COREX_SDW_OFFSET + R)
#define CSTAT_GET_F_COREX(base, R, F) PMIO_GET_F(base, CSTAT_COREX_SDW_OFFSET + R, F)
#define CSTAT_GET_F_DIRECT(base, R, F) PMIO_GET_F(base, R, F)
...
static void pablo_hw_api_cstat_hw_corex_resume_kunit_test(struct kunit *test)
{
	...
	cstat_hw_corex_resume(ctx->pmio);

	val = CSTAT_GET_F_DIRECT(
		ctx->pmio, CSTAT_R_COREX_UPDATE_TYPE_0, CSTAT_F_COREX_UPDATE_TYPE_0);
	KUNIT_EXPECT_EQ(test, val, COREX_COPY);
	val = CSTAT_GET_F_DIRECT(ctx->pmio, CSTAT_R_COREX_START_0, CSTAT_F_COREX_START_0);
	KUNIT_EXPECT_EQ(test, val, 1);
	val = CSTAT_GET_F_DIRECT(
		ctx->pmio, CSTAT_R_COREX_UPDATE_MODE_0, CSTAT_F_COREX_UPDATE_MODE_0);
...
static void pablo_hw_api_cstat_hw_s_corex_delay_kunit_test(struct kunit *test)
{
	...
	cstat_hw_s_corex_delay(ctx->pmio, &time_cfg);
...
static void pablo_hw_api_cstat_hw_s_lic_hblank_kunit_test(struct kunit *test)
{
	...
	time_cfg.clk = 267000000;
	cstat_hw_s_lic_hblank(ctx->pmio, &time_cfg);
}
...
static void pablo_hw_api_cstat_hw_is_corex_offset_kunit_test(struct kunit *test)
{
	...
	ret = cstat_hw_is_corex_offset(CSTAT_R_BYR_CGRAS_LUT_START_ADD);
	KUNIT_EXPECT_EQ(test, ret, false);

	ret = cstat_hw_is_corex_offset(CSTAT_R_BYR_CGRAS_LUT_ACCESS);
	KUNIT_EXPECT_EQ(test, ret, false);

	ret = cstat_hw_is_corex_offset(CSTAT_R_BYR_CGRAS_LUT_ACCESS_SETB);
	KUNIT_EXPECT_EQ(test, ret, false);

	ret = cstat_hw_is_corex_offset(CSTAT_R_RGB_LUMASHADING_LUT_START_ADD);
	KUNIT_EXPECT_EQ(test, ret, false);

	ret = cstat_hw_is_corex_offset(CSTAT_R_RGB_LUMASHADING_LUT_ACCESS_SETA);
	KUNIT_EXPECT_EQ(test, ret, false);

	ret = cstat_hw_is_corex_offset(CSTAT_R_RGB_LUMASHADING_LUT_ACCESS_SETB);
	KUNIT_EXPECT_EQ(test, ret, false);

	ret = cstat_hw_is_corex_offset(CSTAT_R_RGB_LUMASHADING_STREAM_CRC);
...
static void pablo_hw_api_cstat_hw_s_global_enable_kunit_test(struct kunit *test)
{
	...
	cstat_hw_s_global_enable(ctx->pmio, true, true);
	cstat_hw_s_global_enable(ctx->pmio, false, true);
}
...
static void pablo_hw_api_cstat_hw_s_one_shot_enabl_kunit_test(struct kunit *test)
{
	...
	/* set idleness status */
	CSTAT_SET_F_DIRECT(ctx->pmio, CSTAT_R_IDLENESS_STATUS, CSTAT_F_IDLENESS_STATUS, 1);

	ret = cstat_hw_s_one_shot_enable(ctx->pmio);
	KUNIT_EXPECT_EQ(test, ret, 0);

	/* set idleness status */
	CSTAT_SET_F_DIRECT(ctx->pmio, CSTAT_R_IDLENESS_STATUS, CSTAT_F_IDLENESS_STATUS, 0);
...
static void pablo_hw_api_cstat_hw_corex_trigger_kunit_test(struct kunit *test)
{
	...
	cstat_hw_corex_trigger(ctx->pmio);
}
...
static void pablo_hw_api_cstat_hw_s_post_frame_gap_kunit_test(struct kunit *test)
{
	...
	cstat_hw_s_post_frame_gap(ctx->pmio, 0);
}
...
static void pablo_hw_api_cstat_hw_s_int_on_col_row_kunit_test(struct kunit *test)
{
	...
	cstat_hw_s_int_on_col_row(ctx->pmio, true, CSTAT_BCROP0, 0, 0);
	cstat_hw_s_int_on_col_row(ctx->pmio, false, CSTAT_BCROP0, 0, 0);
}
...
static void pablo_hw_api_cstat_hw_s_freeze_on_col_row_kunit_test(struct kunit *test)
{
	...
	cstat_hw_s_freeze_on_col_row(ctx->pmio, CSTAT_BCROP0, 0, 0);
}
...
static void pablo_hw_api_cstat_hw_g_version_kunit_test(struct kunit *test)
{
	...
	version = cstat_hw_g_version(ctx->pmio);
...
static void pablo_hw_api_cstat_hw_s_input_kunit_test(struct kunit *test)
{
	...
	ret = cstat_hw_s_input(ctx->pmio, OTF);
	KUNIT_EXPECT_EQ(test, ret, 0);

	ret = cstat_hw_s_input(ctx->pmio, DMA);
	KUNIT_EXPECT_EQ(test, ret, 0);

	ret = cstat_hw_s_input(ctx->pmio, VOTF);
	KUNIT_EXPECT_EQ(test, ret, 0);

	ret = cstat_hw_s_input(ctx->pmio, CSTST_INPUT_PATH_NUM);
...
static void pablo_hw_api_cstat_hw_s_format_kunit_test(struct kunit *test)
{
	...
	ret = cstat_hw_s_format(ctx->pmio, 4032, 3024, INPUT_10B);
	KUNIT_EXPECT_EQ(test, ret, 0);

	ret = cstat_hw_s_format(ctx->pmio, 4032, 3024, CSTAT_INPUT_BITWIDTH_NUM);
...
static void pablo_hw_api_cstat_hw_s_sdc_enable_kunit_test(struct kunit *test)
{
	...
	cstat_hw_s_sdc_enable(ctx->pmio, enable);
}
...
static void pablo_hw_api_cstat_hw_s_fro_kunit_test(struct kunit *test)
{
	...
	/* enable fro */
	cstat_hw_s_fro(ctx->pmio, 2, 0x1F);
	/* disable fro */
	cstat_hw_s_fro(ctx->pmio, 1, 0x1F);
}
...
static void pablo_hw_api_cstat_hw_s_corex_enable_kunit_test(struct kunit *test)
{
	...
	/* enable corex */
	cstat_hw_s_corex_enable(ctx->pmio, true);
	/* disable corex */
	CSTAT_SET_F_DIRECT(ctx->pmio, CSTAT_R_COREX_STATUS_0, CSTAT_F_COREX_BUSY_0, 1);
	cstat_hw_s_corex_enable(ctx->pmio, false);
}
...
static void pablo_hw_api_cstat_hw_s_int_enable_kunit_test(struct kunit *test)
{
	...
	/* run */
	cstat_hw_s_int_enable(ctx->pmio);

	/* check output */
	set_val = CSTAT_GET_F_DIRECT(ctx->pmio, CSTAT_R_CONTINT_SIPUIPP1P0P0_LEVEL_PULSE_N_SEL,
	...
	set_val = CSTAT_GET_F_COREX(ctx->pmio, CSTAT_R_IP_USE_END_INTERRUPT_ENABLE,
		CSTAT_F_IP_USE_END_INTERRUPT_ENABLE);
	expected_val = 0;
	KUNIT_EXPECT_EQ(test, set_val, expected_val);

	set_val = CSTAT_GET_F_COREX(
		ctx->pmio, CSTAT_R_IP_END_INTERRUPT_ENABLE, CSTAT_F_IP_END_INTERRUPT_ENABLE);
	expected_val = 0;
	KUNIT_EXPECT_EQ(test, set_val, expected_val);

	set_val = CSTAT_GET_F_COREX(ctx->pmio, CSTAT_R_IP_CORRUPTED_INTERRUPT_ENABLE,
		CSTAT_F_IP_CORRUPTED_INTERRUPT_ENABLE);
	expected_val = INT_CRPT_EN_MASK;
	KUNIT_EXPECT_EQ(test, set_val, expected_val);

	set_val = CSTAT_GET_F_DIRECT(ctx->pmio, CSTAT_R_CONTINT_SIPUIPP1P0P0_INT1_ENABLE,
		CSTAT_F_CONTINT_SIPUIPP1P0P0_INT1_ENABLE);
	expected_val = INT1_EN_MASK;
	KUNIT_EXPECT_EQ(test, set_val, expected_val);

	set_val = CSTAT_GET_F_DIRECT(ctx->pmio, CSTAT_R_CONTINT_SIPUIPP1P0P0_INT2_ENABLE,
...
static void pablo_hw_api_cstat_hw_g_int1_status_kunit_test(struct kunit *test)
{
	...
	/* fro int status */
	ret = cstat_hw_g_int1_status(ctx->pmio, true, true);
	KUNIT_EXPECT_EQ(test, ret, 0);

	/* normal int status */
	ret = cstat_hw_g_int1_status(ctx->pmio, true, false);
	KUNIT_EXPECT_EQ(test, ret, 0);
}
...
static void pablo_hw_api_cstat_hw_g_int2_status_kunit_test(struct kunit *test)
{
	...
	/* fro int status */
	ret = cstat_hw_g_int2_status(ctx->pmio, true, true);
	KUNIT_EXPECT_EQ(test, ret, 0);

	/* normal int status */
	ret = cstat_hw_g_int2_status(ctx->pmio, true, false);
	KUNIT_EXPECT_EQ(test, ret, 0);
}
...
static void pablo_hw_api_cstat_hw_wait_isr_clear_kunit_test(struct kunit *test)
{
	...
	CSTAT_SET_F_DIRECT(
		ctx->pmio, CSTAT_R_CONTINT_SIPUIPP1P0P0_INT1, CSTAT_F_CONTINT_SIPUIPP1P0P0_INT1, 1);
	CSTAT_SET_F_DIRECT(
		ctx->pmio, CSTAT_R_CONTINT_SIPUIPP1P0P0_INT2, CSTAT_F_CONTINT_SIPUIPP1P0P0_INT2, 1);
	cstat_hw_wait_isr_clear(ctx->pmio, false);
}
...
static void pablo_hw_api_cstat_hw_s_crc_seed_kunit_test(struct kunit *test)
{
	...
	cstat_hw_s_crc_seed(ctx->pmio, seed);
}
...
static void pablo_hw_api_cstat_hw_s_cdaf_dma_kunit_test(struct kunit *test)
{
	...
	for (test_idx = 0; test_idx < CSTAT_TEST_SET; test_idx++) {
		...
		ret = cstat_hw_create_dma(ctx->pmio, dma_id, &dma[dma_id]);
		...
		ret = cstat_hw_s_wdma_cfg(ctx->pmio, &dma[dma_id], &param, 1, 0);

		/* check output */
		KUNIT_EXPECT_EQ(test, ret, 0);

		set_val = CSTAT_GET_F_COREX(
			ctx->pmio, CSTAT_R_STAT_WDMACDAF_EN, CSTAT_F_STAT_WDMACDAF_EN);
		expected_val = param.cdaf_mw.cmd;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(ctx->pmio, CSTAT_R_STAT_WDMACDAF_DATA_FORMAT,
			CSTAT_F_STAT_WDMACDAF_DATA_FORMAT_BAYER);
		expected_val = DMA_FMT_U8BIT_PACK;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(ctx->pmio, CSTAT_R_STAT_WDMACDAF_MONO_MODE,
			CSTAT_F_STAT_WDMACDAF_MONO_MODE);
		expected_val = 0;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(
			ctx->pmio, CSTAT_R_STAT_WDMACDAF_WIDTH, CSTAT_F_STAT_WDMACDAF_WIDTH);
		expected_val = param.cdaf_mw.width;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(
			ctx->pmio, CSTAT_R_STAT_WDMACDAF_HEIGHT, CSTAT_F_STAT_WDMACDAF_HEIGHT);
		expected_val = param.cdaf_mw.height;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(ctx->pmio, CSTAT_R_STAT_WDMACDAF_IMG_STRIDE_1P,
		...
		/* MSB */
		set_val = CSTAT_GET_F_COREX(ctx->pmio, CSTAT_R_STAT_WDMACDAF_IMG_BASE_ADDR_1P_FRO0,
			CSTAT_F_STAT_WDMACDAF_IMG_BASE_ADDR_1P_FRO0);
		/* MSB << 4 | LSB */
		set_val = (set_val << 4) |
			  CSTAT_GET_F_COREX(ctx->pmio,
...
static void pablo_hw_api_cstat_hw_s_drc_dma_kunit_test(struct kunit *test)
{
	...
	for (test_idx = 0; test_idx < CSTAT_TEST_SET; test_idx++) {
		...
		dma_out->bitwidth = DMA_INOUT_BIT_WIDTH_14BIT;
		...
		expected_val = DIV_ROUND_UP(conf.drc_grid_w, 4) * 4;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);
		...
		set_val = CSTAT_GET_F_COREX(
			ctx->pmio, CSTAT_R_STAT_WDMADRCGRID_EN, CSTAT_F_STAT_WDMADRCGRID_EN);
		expected_val = param.dma_output_drc.cmd;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(ctx->pmio, CSTAT_R_STAT_WDMADRCGRID_DATA_FORMAT,
			CSTAT_F_STAT_WDMADRCGRID_DATA_FORMAT_BAYER);
		expected_val = DMA_FMT_U14BIT_PACK;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(ctx->pmio, CSTAT_R_STAT_WDMADRCGRID_MONO_MODE,
			CSTAT_F_STAT_WDMADRCGRID_MONO_MODE);
		expected_val = 0;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(
			ctx->pmio, CSTAT_R_STAT_WDMADRCGRID_WIDTH, CSTAT_F_STAT_WDMADRCGRID_WIDTH);
		expected_val = param.dma_output_drc.width;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(ctx->pmio, CSTAT_R_STAT_WDMADRCGRID_HEIGHT,
			CSTAT_F_STAT_WDMADRCGRID_HEIGHT);
		expected_val = param.dma_output_drc.height;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(ctx->pmio, CSTAT_R_STAT_WDMADRCGRID_IMG_STRIDE_1P,
			CSTAT_F_STAT_WDMADRCGRID_IMG_STRIDE_1P);
		expected_val = DIV_ROUND_UP(param.dma_output_drc.width * param.dma_output_drc.bitwidth,
					BITS_PER_BYTE);
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		/* MSB */
		set_val =
			CSTAT_GET_F_COREX(ctx->pmio, CSTAT_R_STAT_WDMADRCGRID_IMG_BASE_ADDR_1P_FRO0,
...
static void pablo_hw_api_cstat_hw_s_sat_dma_kunit_test(struct kunit *test)
{
	...
	for (test_idx = 0; test_idx < CSTAT_TEST_SET; test_idx++) {
		...
		set_val = CSTAT_GET_F_COREX(
			ctx->pmio, CSTAT_R_YUV_WDMASAT_EN, CSTAT_F_YUV_WDMASAT_EN);
		expected_val = dma_out->cmd;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(ctx->pmio, CSTAT_R_YUV_WDMASAT_DATA_FORMAT,
		...
		set_val = CSTAT_GET_F_COREX(
			ctx->pmio, CSTAT_R_YUV_WDMASAT_MONO_MODE, CSTAT_F_YUV_WDMASAT_MONO_MODE);
		expected_val = 1;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(
			ctx->pmio, CSTAT_R_YUV_WDMASAT_WIDTH, CSTAT_F_YUV_WDMASAT_WIDTH);
		expected_val = dma_out->width;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(
			ctx->pmio, CSTAT_R_YUV_WDMASAT_HEIGHT, CSTAT_F_YUV_WDMASAT_HEIGHT);
		expected_val = dma_out->height;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(ctx->pmio, CSTAT_R_YUV_WDMASAT_IMG_STRIDE_1P,
			CSTAT_F_YUV_WDMASAT_IMG_STRIDE_1P);
		expected_val = ALIGN(DIV_ROUND_UP(dma_out->width * dma_out->bitwidth, BITS_PER_BYTE), 16);
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		/* MSB */
		set_val = CSTAT_GET_F_COREX(ctx->pmio, CSTAT_R_YUV_WDMASAT_IMG_BASE_ADDR_1P_FRO0,
			CSTAT_F_YUV_WDMASAT_IMG_BASE_ADDR_1P_FRO0);
		/* MSB << 4 | LSB */
		set_val =
			(set_val << 4) | CSTAT_GET_F_COREX(ctx->pmio,
		...
		set_val = CSTAT_GET_F_COREX(
			ctx->pmio, CSTAT_R_RGB_RGBTOYUVSAT_BYPASS, CSTAT_F_RGB_RGBTOYUVSAT_BYPASS);
		expected_val = 0;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(ctx->pmio, CSTAT_R_YUV_YUV444TO422SAT_BYPASS,
			CSTAT_F_YUV_YUV444TO422SAT_BYPASS);
		expected_val = 1;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(ctx->pmio, CSTAT_R_YUV_YUV422TO420SAT_BYPASS,
...
static void pablo_hw_api_cstat_hw_s_cav_dma_kunit_test(struct kunit *test)
{
	...
	for (test_idx = 0; test_idx < CSTAT_TEST_SET; test_idx++) {
		...
		set_val = CSTAT_GET_F_COREX(
			ctx->pmio, CSTAT_R_YUV_WDMACAV_EN, CSTAT_F_YUV_WDMACAV_EN);
		expected_val = dma_out->cmd;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(ctx->pmio, CSTAT_R_YUV_WDMACAV_DATA_FORMAT,
		...
		set_val = CSTAT_GET_F_COREX(
			ctx->pmio, CSTAT_R_YUV_WDMACAV_MONO_MODE, CSTAT_F_YUV_WDMACAV_MONO_MODE);
		expected_val = 0;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(
			ctx->pmio, CSTAT_R_YUV_WDMACAV_WIDTH, CSTAT_F_YUV_WDMACAV_WIDTH);
		expected_val = dma_out->width;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(
			ctx->pmio, CSTAT_R_YUV_WDMACAV_HEIGHT, CSTAT_F_YUV_WDMACAV_HEIGHT);
		expected_val = dma_out->height;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(ctx->pmio, CSTAT_R_YUV_WDMACAV_IMG_STRIDE_1P,
			CSTAT_F_YUV_WDMACAV_IMG_STRIDE_1P);
		expected_val = ALIGN(DIV_ROUND_UP(dma_out->width * dma_out->bitwidth, BITS_PER_BYTE), 16);
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		/* MSB */
		set_val = CSTAT_GET_F_COREX(ctx->pmio, CSTAT_R_YUV_WDMACAV_IMG_BASE_ADDR_1P_FRO0,
		...
		set_val = CSTAT_GET_F_COREX(
			ctx->pmio, CSTAT_R_RGB_RGBTOYUVCAV_BYPASS, CSTAT_F_RGB_RGBTOYUVCAV_BYPASS);
		expected_val = 1;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(ctx->pmio, CSTAT_R_YUV_YUV444TO422CAV_BYPASS,
			CSTAT_F_YUV_YUV444TO422CAV_BYPASS);
		expected_val = 1;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(ctx->pmio, CSTAT_R_YUV_YUV422TO420CAV_BYPASS,
...
static void pablo_hw_api_cstat_hw_s_fdpig_wdma_kunit_test(struct kunit *test)
{
	...
	for (test_idx = 0; test_idx < CSTAT_TEST_SET; test_idx++) {
		...
		set_val = CSTAT_GET_F_COREX(
			ctx->pmio, CSTAT_R_YUV_WDMAFDPIG_EN, CSTAT_F_YUV_WDMAFDPIG_EN);
		expected_val = dma_out->cmd;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(ctx->pmio, CSTAT_R_YUV_WDMAFDPIG_DATA_FORMAT,
		...
		set_val = CSTAT_GET_F_COREX(ctx->pmio, CSTAT_R_YUV_WDMAFDPIG_MONO_MODE,
			CSTAT_F_YUV_WDMAFDPIG_MONO_MODE);
		expected_val = 0;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(
			ctx->pmio, CSTAT_R_YUV_WDMAFDPIG_WIDTH, CSTAT_F_YUV_WDMAFDPIG_WIDTH);
		expected_val = dma_out->width;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(
			ctx->pmio, CSTAT_R_YUV_WDMAFDPIG_HEIGHT, CSTAT_F_YUV_WDMAFDPIG_HEIGHT);
		expected_val = dma_out->height;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(ctx->pmio, CSTAT_R_YUV_WDMAFDPIG_IMG_STRIDE_1P,
			CSTAT_F_YUV_WDMAFDPIG_IMG_STRIDE_1P);
		expected_val = ALIGN(DIV_ROUND_UP(dma_out->width * dma_out->bitwidth, BITS_PER_BYTE), 16);
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		/* MSB */
		set_val = CSTAT_GET_F_COREX(ctx->pmio, CSTAT_R_YUV_WDMAFDPIG_IMG_BASE_ADDR_1P_FRO0,
		...
		set_val = CSTAT_GET_F_COREX(ctx->pmio, CSTAT_R_RGB_RGBTOYUVFDPIG_BYPASS,
			CSTAT_F_RGB_RGBTOYUVFDPIG_BYPASS);
		expected_val = 1;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(ctx->pmio, CSTAT_R_YUV_YUV444TO422FDPIG_BYPASS,
			CSTAT_F_YUV_YUV444TO422FDPIG_BYPASS);
		expected_val = 1;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(ctx->pmio, CSTAT_R_YUV_YUV422TO420FDPIG_BYPASS,
...
static void pablo_hw_api_cstat_hw_s_byr_rdma_cfg_kunit_test(struct kunit *test)
{
	...
	for (test_idx = 0; test_idx < CSTAT_TEST_SET; test_idx++) {
		...
		set_val = CSTAT_GET_F_COREX(
			ctx->pmio, CSTAT_R_BYR_RDMABYRIN_EN, CSTAT_F_BYR_RDMABYRIN_EN);
		expected_val = dma_input->cmd;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(ctx->pmio, CSTAT_R_BYR_RDMABYRIN_DATA_FORMAT,
			CSTAT_F_BYR_RDMABYRIN_DATA_FORMAT_BAYER);
		expected_val = DMA_FMT_U10BIT_PACK;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(ctx->pmio, CSTAT_R_BYR_RDMABYRIN_COMP_CONTROL,
			CSTAT_F_BYR_RDMABYRIN_COMP_SBWC_EN);
		expected_val = 0;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(
			ctx->pmio, CSTAT_R_BYR_RDMABYRIN_WIDTH, CSTAT_F_BYR_RDMABYRIN_WIDTH);
		expected_val = dma_input->width;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(
			ctx->pmio, CSTAT_R_BYR_RDMABYRIN_HEIGHT, CSTAT_F_BYR_RDMABYRIN_HEIGHT);
		expected_val = dma_input->height;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(ctx->pmio, CSTAT_R_BYR_RDMABYRIN_IMG_STRIDE_1P,
		...
		/* MSB */
		set_val = CSTAT_GET_F_COREX(ctx->pmio, CSTAT_R_BYR_RDMABYRIN_IMG_BASE_ADDR_1P_FRO0,
...
static void pablo_hw_api_cstat_hw_s_ds_cfg_kunit_test(struct kunit *test)
{
	...
	for (dma_id = 0; dma_id < CSTAT_DMA_NUM; dma_id++) {
		for (test_idx = 0; test_idx < CSTAT_TEST_SET; test_idx++) {
			...
			case CSTAT_W_FDPIG:
				...
				dma_out->width = test_config[test_idx].fdpig.w;
				dma_out->height = test_config[test_idx].fdpig.h;
				crop_reg = CSTAT_R_RGB_FDPIGDS_CROP_EN;
				crop_field = CSTAT_F_RGB_FDPIGDS_CROP_EN;
				break;
			case CSTAT_W_SAT:
				dma_out = &param.dma_output_sat;
				dma_out->dma_crop_width = test_config[test_idx].bns.w;
				...
				dma_out->width = test_config[test_idx].sat.w;
				dma_out->height = test_config[test_idx].sat.h;
				crop_reg = CSTAT_R_RGB_SATDS_CROP_EN;
				crop_field = CSTAT_F_RGB_SATDS_CROP_EN;
				break;
			case CSTAT_W_CAV:
				dma_out = &param.dma_output_cav;
				dma_out->dma_crop_width = test_config[test_idx].bns.w;
				...
				dma_out->width = test_config[test_idx].cav.w;
				dma_out->height = test_config[test_idx].cav.h;
				crop_reg = CSTAT_R_RGB_CAVDS_CROP_EN;
				crop_field = CSTAT_F_RGB_CAVDS_CROP_EN;
				break;
			default:
				continue;
			}
			...
			dma_out->cmd = DMA_OUTPUT_COMMAND_ENABLE;
			ret = cstat_hw_s_ds_cfg(ctx->pmio, dma_id, &size_cfg, &param);
			KUNIT_EXPECT_EQ(test, ret, 0);

			if (crop_reg != CSTAT_REG_CNT) {
				set_val = CSTAT_GET_F_COREX(ctx->pmio, crop_reg, crop_field);
				KUNIT_EXPECT_EQ(test, set_val, 1);
			}

			dma_out->cmd = DMA_OUTPUT_COMMAND_DISABLE;
			cstat_hw_s_ds_cfg(ctx->pmio, dma_id, &size_cfg, &param);
			KUNIT_EXPECT_EQ(test, ret, 0);
	...
	dma_out->cmd = DMA_OUTPUT_COMMAND_ENABLE;
	ret = cstat_hw_s_ds_cfg(ctx->pmio, CSTAT_W_SAT, &size_cfg, &param);
	KUNIT_EXPECT_EQ(test, ret, -EINVAL);
	...
	for (dma_id = CSTAT_W_FDPIG; dma_id <= CSTAT_W_CAV; dma_id++) {
		...
		/* Invalid crop offset */
		dma_out->dma_crop_offset_x = size_cfg.bns.w;
		dma_out->dma_crop_offset_y = size_cfg.bns.h;
...
static void pablo_hw_api_cstat_hw_s_simple_lic_cfg_kunit_test(struct kunit *test)
{
	...
	for (test_idx = 0; test_idx < CSTAT_TEST_SET; test_idx++) {
		...
		/* run */
		cstat_hw_s_simple_lic_cfg(ctx->pmio, &simple_lic_cfg);

		/* check output */
		set_val = CSTAT_GET_F_COREX(ctx->pmio, CSTAT_R_LIC_INPUT_MODE, CSTAT_F_LIC_BYPASS);
		expected_val = (u32)simple_lic_cfg.bypass;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(
			ctx->pmio, CSTAT_R_LIC_INPUT_MODE, CSTAT_F_LIC_OTF_PRIORITY);
		expected_val = 1;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(ctx->pmio, CSTAT_R_LIC_INPUT_SIZE, CSTAT_F_LIC_WIDTH);
		expected_val = simple_lic_cfg.input_width;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(ctx->pmio, CSTAT_R_LIC_INPUT_SIZE, CSTAT_F_LIC_HEIGHT);
		expected_val = simple_lic_cfg.input_height;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(
			ctx->pmio, CSTAT_R_LIC_INPUT_CONFIG_0, CSTAT_F_LIC_BIT_MODE);
		expected_val = simple_lic_cfg.input_bitwidth;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(
			ctx->pmio, CSTAT_R_LIC_INPUT_CONFIG_0, CSTAT_F_LIC_VL_NUM_LINE);
		expected_val = 0;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(
			ctx->pmio, CSTAT_R_LIC_INPUT_CONFIG_0, CSTAT_F_LIC_RDMA_EN);
		expected_val = 0;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(
			ctx->pmio, CSTAT_R_LIC_INPUT_CONFIG_0, CSTAT_F_LIC_STALL_MEM_PORTION);
		expected_val = ((simple_lic_cfg.input_width + 16384) / 3) * 100 / 16384;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(
			ctx->pmio, CSTAT_R_LIC_INPUT_BLANK, CSTAT_F_LIC_IN_HBLANK_CYCLE);
		expected_val = 45;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(
			ctx->pmio, CSTAT_R_LIC_INPUT_BLANK, CSTAT_F_LIC_OUT_HBLANK_CYCLE);
		expected_val = 45;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);
	}
}
...
static void pablo_hw_api_cstat_hw_s_sram_offset_kunit_test(struct kunit *test)
{
	...
	cstat_hw_s_sram_offset(ctx->pmio);
}
...
static void pablo_hw_api_cstat_hw_s_crop_kunit_test(struct kunit *test)
{
	...
	for (crop_id = 0; crop_id <= CSTAT_CROP_NUM; crop_id++)
		cstat_hw_s_crop(ctx->pmio, crop_id, true, &crop, &param, &size_cfg);
}
...
static void pablo_hw_api_cstat_hw_s_grid_cfg_kunit_test(struct kunit *test)
{
	...
	for (grid_id = 0; grid_id <= CSTAT_GRID_LSC; grid_id++)
		cstat_hw_s_grid_cfg(ctx->pmio, grid_id, &cfg);
}
...
static void pablo_hw_api_cstat_hw_s_bns_cfg_kunit_test(struct kunit *test)
{
	...
	for (test_idx = 0; test_idx < CSTAT_TEST_SET; test_idx++) {
		...
		/* run */
		cstat_hw_s_bns_cfg(ctx->pmio, &crop_bns, &size_cfg);

		/* check output */
		set_val = CSTAT_GET_F_COREX(
			ctx->pmio, CSTAT_R_BYR_BNS_CONFIG, CSTAT_F_BYR_BNS_FACTORX);
		expected_val = 3;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(
			ctx->pmio, CSTAT_R_BYR_BNS_CONFIG, CSTAT_F_BYR_BNS_FACTORY);
		expected_val = 3;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(
			ctx->pmio, CSTAT_R_BYR_BNS_OUTPUTSIZE, CSTAT_F_BYR_BNS_NOUTPUTTOTALWIDTH);
		expected_val = out_width;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(
			ctx->pmio, CSTAT_R_BYR_BNS_OUTPUTSIZE, CSTAT_F_BYR_BNS_NOUTPUTTOTALHEIGHT);
		expected_val = out_height;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(
			ctx->pmio, CSTAT_R_BYR_BNS_LINEGAP, CSTAT_F_BYR_BNS_LINEGAP);
		expected_val = 4;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(
			ctx->pmio, CSTAT_R_BYR_BNS_WEIGHT_X_0, CSTAT_F_BYR_BNS_WEIGHT_X_0_0);
		expected_val = bns_weights[0];
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(
			ctx->pmio, CSTAT_R_BYR_BNS_WEIGHT_X_0, CSTAT_F_BYR_BNS_WEIGHT_X_0_1);
		expected_val = bns_weights[1];
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(
			ctx->pmio, CSTAT_R_BYR_BNS_WEIGHT_X_0, CSTAT_F_BYR_BNS_WEIGHT_X_0_2);
		expected_val = bns_weights[2];
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(
			ctx->pmio, CSTAT_R_BYR_BNS_WEIGHT_X_0, CSTAT_F_BYR_BNS_WEIGHT_X_0_3);
		expected_val = bns_weights[3];
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(
			ctx->pmio, CSTAT_R_BYR_BNS_WEIGHT_X_4, CSTAT_F_BYR_BNS_WEIGHT_X_0_4);
		expected_val = bns_weights[4];
		KUNIT_EXPECT_EQ(test, set_val, expected_val);

		set_val = CSTAT_GET_F_COREX(
			ctx->pmio, CSTAT_R_BYR_BNS_BYPASS, CSTAT_F_BYR_BNS_BYPASS);
		expected_val = 0;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);
	}
...
static void pablo_hw_api_cstat_hw_g_stat_data_kunit_test(struct kunit *test)
{
	...
	/* cdaf stat test */
	ret = cstat_hw_g_stat_data(ctx->pmio, CSTAT_STAT_CDAF, (void *)&cdaf);
	KUNIT_EXPECT_EQ(test, ret, 0);
	...
	/* edge score test */
	ret = cstat_hw_g_stat_data(ctx->pmio, CSTAT_STAT_EDGE_SCORE, (void *)&edge_score);
	KUNIT_EXPECT_EQ(test, ret, 0);

	/* error test */
	ret = cstat_hw_g_stat_data(ctx->pmio, CSTAT_STAT_ID_NUM, NULL);
	KUNIT_EXPECT_EQ(test, ret, -EINVAL);
}
...
static void pablo_hw_api_cstat_hw_s_default_blk_cfg_kunit_test(struct kunit *test)
{
	...
	cstat_hw_s_default_blk_cfg(ctx->pmio);

	/* FDPIG */
	set_val = CSTAT_GET_R_COREX(ctx->pmio, CSTAT_R_RGB_RGBTOYUVFDPIG_COEFF_R1);
	expected_val = 306;
	KUNIT_EXPECT_EQ(test, set_val, expected_val);

	set_val = CSTAT_GET_R_COREX(ctx->pmio, CSTAT_R_YUV_YUV444TO422FDPIG_MCOEFFS_0);
	expected_val = 0x20402000;
	KUNIT_EXPECT_EQ(test, set_val, expected_val);

	/* SAT */
	set_val = CSTAT_GET_R_COREX(ctx->pmio, CSTAT_R_RGB_RGBTOYUVSAT_COEFF_R1);
	expected_val = 306;
	KUNIT_EXPECT_EQ(test, set_val, expected_val);

	set_val = CSTAT_GET_R_COREX(ctx->pmio, CSTAT_R_YUV_YUV444TO422SAT_MCOEFFS_0);
	expected_val = 0x20402000;
	KUNIT_EXPECT_EQ(test, set_val, expected_val);

	/* CAV */
	set_val = CSTAT_GET_R_COREX(ctx->pmio, CSTAT_R_RGB_RGBTOYUVCAV_COEFF_R1);
	expected_val = 306;
	KUNIT_EXPECT_EQ(test, set_val, expected_val);

	set_val = CSTAT_GET_R_COREX(ctx->pmio, CSTAT_R_YUV_YUV444TO422CAV_MCOEFFS_0);
	expected_val = 0x20402000;
	KUNIT_EXPECT_EQ(test, set_val, expected_val);
}
...
static void pablo_hw_api_cstat_hw_s_pixel_order_kunit_test(struct kunit *test)
{
	...
	cstat_hw_s_pixel_order(ctx->pmio, 0);
}
...
static void pablo_hw_api_cstat_hw_s_mono_mode_kunit_test(struct kunit *test)
{
	...
	cstat_hw_s_mono_mode(ctx->pmio, 0);
}
...
static void pablo_hw_api_cstat_hw_print_stall_status_kunit_test(struct kunit *test)
{
	...
	cstat_hw_print_stall_status(ctx->pmio, ch);
}
...
static int pablo_hw_api_cstat_pmio_init(struct kunit *test)
{
	int ret;
	struct pablo_hw_api_cstat_test_ctx *ctx = test->priv;

	ctx->pmio_config.name = "cstat";
	ctx->pmio_config.mmio_base = ctx->test_addr;
	ctx->pmio_config.cache_type = PMIO_CACHE_NONE;

	cstat_hw_init_pmio_config(&ctx->pmio_config);

	ctx->pmio = pmio_init(NULL, NULL, &ctx->pmio_config);
	if (IS_ERR(ctx->pmio)) {
		err("failed to init cstat PMIO: %ld", PTR_ERR(ctx->pmio));
		return -ENOMEM;
	}

	ret = pmio_field_bulk_alloc(
		ctx->pmio, &ctx->pmio_fields, ctx->pmio_config.fields, ctx->pmio_config.num_fields);
	if (ret) {
		err("failed to alloc cstat PMIO fields: %d", ret);
		pmio_exit(ctx->pmio);
		return ret;
	}

	ctx->pmio_reg_seqs =
		kunit_kzalloc(test, sizeof(struct pmio_reg_seq) * cstat_hw_g_reg_cnt(), 0);
	if (!ctx->pmio_reg_seqs) {
		err("failed to alloc PMIO multiple write buffer");
		pmio_field_bulk_free(ctx->pmio, ctx->pmio_fields);
		pmio_exit(ctx->pmio);
		return -ENOMEM;
	}
...
static void pablo_hw_api_cstat_pmio_deinit(struct kunit *test)
{
	struct pablo_hw_api_cstat_test_ctx *ctx = test->priv;

	kunit_kfree(test, ctx->pmio_reg_seqs);
	pmio_field_bulk_free(ctx->pmio, ctx->pmio_fields);
	pmio_exit(ctx->pmio);
}
...
{
	struct pablo_hw_api_cstat_test_ctx *ctx;
	int ret;

	ctx = kunit_kzalloc(test, sizeof(struct pablo_hw_api_cstat_test_ctx), GFP_KERNEL);
	KUNIT_ASSERT_NOT_ERR_OR_NULL(test, ctx);

	test->priv = ctx;

	ctx->test_addr = kunit_kzalloc(test, 0x10000, 0);
	KUNIT_ASSERT_NOT_ERR_OR_NULL(test, ctx->test_addr);

	ret = pablo_hw_api_cstat_pmio_init(test);
...
static void pablo_hw_api_cstat_hw_kunit_test_exit(struct kunit *test)
{
	...
	pablo_hw_api_cstat_pmio_deinit(test);
