...
static void pablo_hw_api_pdp_hw_s_core_kunit_test(struct kunit *test)
{
	...
	pdp->otf_id = 0;
	cfg->img_vc[pdp->otf_id] = 0;
	cfg->hpd_vc[pdp->otf_id] = 2;
	cfg->input[cfg->hpd_vc[pdp->otf_id]].extformat = POTF_EN(pd_hwformat);
...
static void pablo_hw_api_pdp_hw_s_input_mux_kunit_test(struct kunit *test)
{
	...
	cfg->img_vc[pdp->otf_id] = 0;
	cfg->hpd_vc[pdp->otf_id] = 2;
	cfg->vpd_vc[pdp->otf_id] = 3;

	for(i = 0; i < pdp->mux_elems; i++)
		pdp->mux_val[i] = i;

	pdp_hw_s_input_mux(pdp, pdp->otf_id);

	if (IS_ENABLED(CONFIG_PDP_INPUT_MUX)) {
		KUNIT_EXPECT_EQ(test, cfg->img_vc[pdp->otf_id],
			PDP_GET_F(pdp->base, PDP_R_PDP_VC_CON, PDP_F_MUX_IMG_VC));
		KUNIT_EXPECT_EQ(test, cfg->hpd_vc[pdp->otf_id],
			PDP_GET_F(pdp->base, PDP_R_PDP_VC_CON, PDP_F_MUX_AF0_VC));
		KUNIT_EXPECT_EQ(test, cfg->vpd_vc[pdp->otf_id],
			PDP_GET_F(pdp->base, PDP_R_PDP_VC_CON, PDP_F_MUX_AF1_VC));
	} else if (!IS_ENABLED(CONFIG_PDP_V6_0)) {
...
static void pablo_hw_api_pdp_hw_s_default_blk_cfg_kunit_test(struct kunit *test)
{
	...
	KUNIT_EXPECT_EQ(test, val, 0);
	val = PDP_GET_R_COREX(ctx->test_addr, PDP_R_Y_PDBPC0_BYPASS);
	KUNIT_EXPECT_EQ(test, val, 1);
	val = PDP_GET_R_COREX(ctx->test_addr, PDP_R_Y_PDBPC1_BYPASS);
	KUNIT_EXPECT_EQ(test, val, 1);
	...
	KUNIT_EXPECT_EQ(test, val, 0);
	val = PDP_GET_R_COREX(ctx->test_addr, PDP_R_Y_PDSTAT_SAT_SUB_ON);
	KUNIT_EXPECT_EQ(test, val, 0);
	val = PDP_GET_R_COREX(ctx->test_addr, PDP_R_Y_PDSTAT_XCOR_H_ON);
	KUNIT_EXPECT_EQ(test, val, 0);
	val = PDP_GET_R_COREX(ctx->test_addr, PDP_R_Y_PDSTAT_XCOR_V_ON);
	KUNIT_EXPECT_EQ(test, val, 0);
}
