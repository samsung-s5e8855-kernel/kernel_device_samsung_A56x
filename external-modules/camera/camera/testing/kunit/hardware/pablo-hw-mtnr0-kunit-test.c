...
static struct pablo_hw_mtnr0_kunit_test_ctx {
...
static void pablo_hw_mtnr0_open_kunit_test(struct kunit *test)
{
	int ret;
...
static void pablo_hw_mtnr0_handle_interrupt_kunit_test(struct kunit *test)
{
	int ret;
	struct is_hw_ip *hw_ip = &test_ctx.hw_ip;
	u32 instance = 0;
...
static void pablo_hw_mtnr0_shot_kunit_test(struct kunit *test)
{
	int ret;
	struct is_hw_ip *hw_ip = &test_ctx.hw_ip;
	...
	hw_mtnr0 = (struct is_hw_mtnr0 *)hw_ip->priv_info;
	config = &hw_mtnr0->config[instance];
	config->mixerL0_en = 1;
	config->geomatchL0_en = 0;
	config->imgL0_bit = 12;
	param_region = &test_ctx.parameter;
	param_region->mtnr.rdma_cur_l0.width = 320;
	param_region->mtnr.rdma_cur_l0.height = 240;
	param_region->mtnr.rdma_cur_l0.bitwidth = config->imgL0_bit;
	param_region->mtnr.rdma_cur_l0.msb = config->imgL0_bit - 1;
	...
	param_region->mtnr.rdma_cur_l4.width = 40;
	param_region->mtnr.rdma_cur_l4.height = 30;
	...
	param_region->mtnr.wdma_prev_l0.width = 320;
	param_region->mtnr.wdma_prev_l0.height = 240;
	param_region->mtnr.wdma_prev_l0.bitwidth = param_region->mtnr.rdma_cur_l0.bitwidth;
	param_region->mtnr.wdma_prev_l0.msb = param_region->mtnr.rdma_cur_l0.msb;
	...
	is_hw_mtnr0_s_debug_type(MTNR0_DBG_DTP);

	ret = CALL_HWIP_OPS(hw_ip, shot, &test_ctx.frame, hw_map);
	KUNIT_EXPECT_EQ(test, ret, 0);

	is_hw_mtnr0_c_debug_type(MTNR0_DBG_DTP);

	is_hw_mtnr0_s_debug_type(MTNR0_DBG_TNR);

	ret = CALL_HWIP_OPS(hw_ip, shot, &test_ctx.frame, hw_map);
	KUNIT_EXPECT_EQ(test, ret, 0);

	config->mixerL0_mode = MTNR0_TNR_MODE_FIRST;
	ret = CALL_HWIP_OPS(hw_ip, shot, &test_ctx.frame, hw_map);
	KUNIT_EXPECT_EQ(test, ret, 0);
	...
	config->mixerL0_still_en = 1;
	ret = CALL_HWIP_OPS(hw_ip, shot, &test_ctx.frame, hw_map);
	KUNIT_EXPECT_EQ(test, ret, 0);

	for (buf_id = 0; buf_id < MTNR0_SUBDEV_END; buf_id++)
		set_bit(PABLO_SUBDEV_ALLOC, &hw_mtnr0->subdev[instance][buf_id].state);

	config->mixerL0_en = 0;
	param_set = &hw_mtnr0->param_set[instance];
	param_set->stripe_input.total_count = 2;

	ret = CALL_HWIP_OPS(hw_ip, shot, &test_ctx.frame, hw_map);
	KUNIT_EXPECT_EQ(test, ret, 0);

	is_hw_mtnr0_c_debug_type(MTNR0_DBG_TNR);
...
static void pablo_hw_mtnr0_enable_kunit_test(struct kunit *test)
{
	int ret;
	struct is_hw_ip *hw_ip = &test_ctx.hw_ip;
	u32 instance = 0;
...
static void pablo_hw_mtnr0_set_config_kunit_test(struct kunit *test)
{
	int ret;
	struct is_hw_ip *hw_ip = &test_ctx.hw_ip;
	u32 chain_id = 0;
...
static void pablo_hw_mtnr0_dump_regs_kunit_test(struct kunit *test)
{
	int ret;
	struct is_hw_ip *hw_ip = &test_ctx.hw_ip;
	u32 instance = 0;
...
static void pablo_hw_mtnr0_set_regs_kunit_test(struct kunit *test)
{
	int ret;
	struct is_hw_ip *hw_ip = &test_ctx.hw_ip;
	u32 chain_id = 0;
...
static void pablo_hw_mtnr0_notify_timeout_kunit_test(struct kunit *test)
{
	int ret;
	struct is_hw_ip *hw_ip = &test_ctx.hw_ip;
	u32 instance = 0;
...
static void pablo_hw_mtnr0_restore_kunit_test(struct kunit *test)
{
	int ret;
	struct is_hw_ip *hw_ip = &test_ctx.hw_ip;
	u32 instance = 0;
...
static void pablo_hw_mtnr0_setfile_kunit_test(struct kunit *test)
{
	int ret;
	struct is_hw_ip *hw_ip = &test_ctx.hw_ip;
	u32 scenario = 0;
...
static void pablo_hw_mtnr0_get_meta_kunit_test(struct kunit *test)
{
	int ret;
	struct is_hw_ip *hw_ip = &test_ctx.hw_ip;
	ulong hw_map = 0;
...
static void pablo_hw_mtnr0_get_cap_meta_kunit_test(struct kunit *test)
{
	int ret;
	struct is_hw_ip *hw_ip = &test_ctx.hw_ip;
	ulong hw_map = 0;
...
static void pablo_hw_mtnr0_set_param_kunit_test(struct kunit *test)
{
	int ret;
	struct is_hw_ip *hw_ip = &test_ctx.hw_ip;
	u32 instance = 0;
...
static void pablo_hw_mtnr0_frame_ndone_kunit_test(struct kunit *test)
{
	int ret;
	struct is_hw_ip *hw_ip = &test_ctx.hw_ip;
...
static void pablo_hw_mtnr_param_get_debug_mtnr0_kunit_test(struct kunit *test)
{
	int ret, len;
	u32 value;
	char *usage_str;
	...
	kp = is_hw_mtnr0_get_debug_kernel_param_kunit_wrapper();
	KUNIT_ASSERT_NOT_ERR_OR_NULL(test, kp);

	is_hw_mtnr0_s_debug_type(MTNR0_DBG_DUMP_REG);
	...
	len = scnprintf(usage_str, sizeof(usage_str),
		"debug_mtnr0 usage: echo [value|option] > <target param>");
	...
	is_hw_mtnr0_c_debug_type(MTNR0_DBG_DUMP_REG);

	kunit_kfree(test, buffer);
}

static void pablo_hw_mtnr0_reset_kunit_test(struct kunit *test)
{
	int ret;
	struct is_hw_ip *hw_ip = &test_ctx.hw_ip;
	u32 instance = 0;
	...
	hw = (struct is_hw_mtnr0 *)hw_ip->priv_info;
	hw->pcc->ops = NULL;
...
static void pablo_hw_mtnr0_recursive_nr_kunit_test(struct kunit *test)
{
	int ret;
	struct is_hw_ip *hw_ip = &test_ctx.hw_ip;
	struct pablo_hw_helper_ops ops;
	...
	for (buf_id = MTNR0_SUBDEV_PREV_YUV_2NR; buf_id < MTNR0_SUBDEV_MAX; buf_id++) {
		sd = &hw_mtnr0->subdev[instance][buf_id];
...
static struct kunit_case pablo_hw_mtnr0_kunit_test_cases[] = {
	KUNIT_CASE(pablo_hw_mtnr0_open_kunit_test),
	KUNIT_CASE(pablo_hw_mtnr0_handle_interrupt_kunit_test),
	KUNIT_CASE(pablo_hw_mtnr0_shot_kunit_test),
	KUNIT_CASE(pablo_hw_mtnr0_enable_kunit_test),
	KUNIT_CASE(pablo_hw_mtnr0_set_config_kunit_test),
	KUNIT_CASE(pablo_hw_mtnr0_dump_regs_kunit_test),
	KUNIT_CASE(pablo_hw_mtnr0_set_regs_kunit_test),
	KUNIT_CASE(pablo_hw_mtnr0_notify_timeout_kunit_test),
	KUNIT_CASE(pablo_hw_mtnr0_restore_kunit_test),
	KUNIT_CASE(pablo_hw_mtnr0_setfile_kunit_test),
	KUNIT_CASE(pablo_hw_mtnr0_get_meta_kunit_test),
	KUNIT_CASE(pablo_hw_mtnr0_get_cap_meta_kunit_test),
	KUNIT_CASE(pablo_hw_mtnr0_set_param_kunit_test),
	KUNIT_CASE(pablo_hw_mtnr0_frame_ndone_kunit_test),
	KUNIT_CASE(pablo_hw_mtnr_param_get_debug_mtnr0_kunit_test),
	KUNIT_CASE(pablo_hw_mtnr0_reset_kunit_test),
#if IS_ENABLED(ENABLE_RECURSIVE_NR)
	KUNIT_CASE(pablo_hw_mtnr0_recursive_nr_kunit_test),
#endif
	{},
};
...
{
	int ret;
	enum is_hardware_id hw_id = DEV_HW_MTNR0;
	...
	org_hw_ip = CALL_HW_CHAIN_INFO_OPS(hw_ip->hardware, get_hw_ip, DEV_HW_MTNR0);
	KUNIT_ASSERT_NOT_NULL(test, org_hw_ip);

	hw_ip->regs_start[REG_SETA] = org_hw_ip->regs_start[REG_SETA];

	ret = pablo_hw_mtnr0_probe(hw_ip, itf, itfc, hw_id, "MTNR0");
	KUNIT_ASSERT_EQ(test, ret, 0);

	hw_ip->id = hw_id;
	snprintf(hw_ip->name, sizeof(hw_ip->name), "MTNR0");
	hw_ip->itf = itf;
	hw_ip->itfc = itfc;
...
static int pablo_hw_mtnr0_kunit_test_init(struct kunit *test)
{
	int ret;
...
static void pablo_hw_mtnr0_kunit_test_exit(struct kunit *test)
{
...
struct kunit_suite pablo_hw_mtnr0_kunit_test_suite = {
	.name = "pablo-hw-mtnr0-kunit-test",
	.init = pablo_hw_mtnr0_kunit_test_init,
	.exit = pablo_hw_mtnr0_kunit_test_exit,
	.test_cases = pablo_hw_mtnr0_kunit_test_cases,
};
define_pablo_kunit_test_suites(&pablo_hw_mtnr0_kunit_test_suite);
