...
static void pablo_hw_api_pdp_hw_s_core_kunit_test(struct kunit *test)
{
	...
	u32 sensor_type, path, en_votf, num_buffers, potf_en, hpd_vc;
	bool enable;
	...
	hpd_vc = cfg->hpd_vc[pdp->otf_id] = 2;
	cfg->input[hpd_vc].extformat = (1 << POTF_EN_BIT);
	potf_en = CHECK_POTF_EN(cfg->input[hpd_vc].extformat);
	...
	cfg->input[hpd_vc].extformat = (0 << POTF_EN_BIT);
	potf_en = CHECK_POTF_EN(cfg->input[hpd_vc].extformat);
	pdp_hw_s_core(pdp, enable, cfg, pd_width, pd_height, pd_hwformat, sensor_type, path,
		en_votf, num_buffers);
	pablo_pdp_check_pd_size(test, pd_width, pd_height, pd_hwformat, potf_en);
