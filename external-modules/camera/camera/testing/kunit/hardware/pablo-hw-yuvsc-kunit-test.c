...
static struct pablo_hw_yuvsc_kunit_test_ctx {
	void *base;
...
/* Define testcases */
static void pablo_hw_yuvsc_open_kunit_test(struct kunit *test)
{
	int ret;
	...
	hw_ip = CALL_HW_CHAIN_INFO_OPS(&test_ctx.hardware, get_hw_ip, DEV_HW_YUVSC0);
	...
	hw = (struct pablo_hw_yuvsc *)hw_ip->priv_info;
...
static void pablo_hw_yuvsc_enable_kunit_test(struct kunit *test)
{
	int ret;
	...
	hw_ip[0] = CALL_HW_CHAIN_INFO_OPS(&test_ctx.hardware, get_hw_ip, DEV_HW_YUVSC0);
	...
	hw_ip[1] = CALL_HW_CHAIN_INFO_OPS(&test_ctx.hardware, get_hw_ip, DEV_HW_YUVSC1);
...
static void pablo_hw_yuvsc_disable_kunit_test(struct kunit *test)
{
	int ret;
	struct is_hw_ip *hw_ip = NULL;
	u32 instance = STREAM_ID(__LINE__);
...
static void pablo_hw_yuvsc_shot_kunit_test(struct kunit *test)
{
	int ret;
	struct is_hw_ip *hw_ip = NULL;
	...
	iq_set = &((struct pablo_hw_yuvsc *)hw_ip->priv_info)->iq_set;
...
static void pablo_hw_yuvsc_frame_ndone_kunit_test(struct kunit *test)
{
	int ret;
	struct is_hw_ip *hw_ip = NULL;
	struct is_frame *frame = &test_ctx.frame;
...
static void pablo_hw_yuvsc_notify_timeout_kunit_test(struct kunit *test)
{
	int ret;
	struct is_hw_ip *hw_ip = NULL;
	u32 instance = STREAM_ID(__LINE__);
...
static void pablo_hw_yuvsc_reset_kunit_test(struct kunit *test)
{
	int ret;
	struct is_hw_ip *hw_ip = NULL;
	u32 instance = STREAM_ID(__LINE__);
...
static void pablo_hw_yuvsc_restore_kunit_test(struct kunit *test)
{
	int ret;
	struct is_hw_ip *hw_ip = NULL;
	u32 instance = STREAM_ID(__LINE__);
...
static void pablo_hw_yuvsc_change_chain_kunit_test(struct kunit *test)
{
	struct is_hw_ip *hw_ip[2];
	u32 instance = STREAM_ID(__LINE__);
	u32 next_id;
...
static void pablo_hw_yuvsc_dump_regs_kunit_test(struct kunit *test)
{
	int ret;
	struct is_hw_ip *hw_ip = NULL;
	u32 instance = STREAM_ID(__LINE__);
...
static void pablo_hw_yuvsc_set_config_kunit_test(struct kunit *test)
{
	int ret;
	struct is_hw_ip *hw_ip = NULL;
	struct is_yuvsc_config conf, *hw_conf;
	u32 instance = STREAM_ID(__LINE__);
	u32 fcount = __LINE__;
	u32 chain_id = 0;
	...
	hw_conf = &((struct pablo_hw_yuvsc *)hw_ip->priv_info)->config[instance];
...
static void pablo_hw_yuvsc_cmp_fcount_kunit_test(struct kunit *test)
{
	int ret;
	struct is_hw_ip *hw_ip = NULL;
	u32 instance = STREAM_ID(__LINE__);
...
static void pablo_hw_yuvsc_query_kunit_test(struct kunit *test)
{
	struct is_hw_ip *hw_ip = NULL;
	...
	p_region->yuvsc.otf_output.width = otf.w;
	p_region->yuvsc.otf_output.height = otf.h;

	CALL_HWIP_OPS(hw_ip, query, instance, PABLO_QUERY_GET_PCFI, frame, prfi);
	KUNIT_EXPECT_EQ(test, prfi->yuvsc_output_size.width, otf.w);
	KUNIT_EXPECT_EQ(test, prfi->yuvsc_output_size.height, otf.h);
}

static void pablo_hw_yuvsc_handle_interrupt_kunit_test(struct kunit *test)
{
	int ret;
	struct is_hw_ip *hw_ip = NULL;
	struct is_interface_ischain *itfc = &test_ctx.itfc;
	enum is_hardware_id hw_id = DEV_HW_YUVSC0;
	u32 instance = 0;
	int hw_slot;
...
static struct kunit_case pablo_hw_yuvsc_kunit_test_cases[] = {
	KUNIT_CASE(pablo_hw_yuvsc_open_kunit_test),
	KUNIT_CASE(pablo_hw_yuvsc_enable_kunit_test),
	KUNIT_CASE(pablo_hw_yuvsc_disable_kunit_test),
	KUNIT_CASE(pablo_hw_yuvsc_shot_kunit_test),
	KUNIT_CASE(pablo_hw_yuvsc_frame_ndone_kunit_test),
	KUNIT_CASE(pablo_hw_yuvsc_notify_timeout_kunit_test),
	KUNIT_CASE(pablo_hw_yuvsc_reset_kunit_test),
	KUNIT_CASE(pablo_hw_yuvsc_restore_kunit_test),
	KUNIT_CASE(pablo_hw_yuvsc_change_chain_kunit_test),
	KUNIT_CASE(pablo_hw_yuvsc_dump_regs_kunit_test),
	KUNIT_CASE(pablo_hw_yuvsc_set_config_kunit_test),
	KUNIT_CASE(pablo_hw_yuvsc_cmp_fcount_kunit_test),
	KUNIT_CASE(pablo_hw_yuvsc_query_kunit_test),
	KUNIT_CASE(pablo_hw_yuvsc_handle_interrupt_kunit_test),
	{},
};
...
static int pablo_hw_yuvsc_kunit_test_init(struct kunit *test)
{
	int ret;
	struct is_hw_ip *hw_ip = NULL;
	...
	setup_hw_ip(test, hw_ip, DEV_HW_YUVSC0);

	ret = pablo_hw_yuvsc_probe(hw_ip, itf, itfc, DEV_HW_YUVSC0, "YUVSC");
	KUNIT_ASSERT_EQ(test, ret, 0);

	hw_ip = CALL_HW_CHAIN_INFO_OPS(&test_ctx.hardware, get_hw_ip, DEV_HW_YUVSC1);
	KUNIT_ASSERT_NOT_NULL(test, hw_ip);

	setup_hw_ip(test, hw_ip, DEV_HW_YUVSC1);

	ret = pablo_hw_yuvsc_probe(hw_ip, itf, itfc, DEV_HW_YUVSC1, "YUVSC");
	KUNIT_ASSERT_EQ(test, ret, 0);

	return 0;
}

static void pablo_hw_yuvsc_kunit_test_exit(struct kunit *test)
{
...
struct kunit_suite pablo_hw_yuvsc_kunit_test_suite = {
	.name = "pablo-hw-yuvsc-kunit-test",
	.init = pablo_hw_yuvsc_kunit_test_init,
	.exit = pablo_hw_yuvsc_kunit_test_exit,
	.test_cases = pablo_hw_yuvsc_kunit_test_cases,
};
define_pablo_kunit_test_suites(&pablo_hw_yuvsc_kunit_test_suite);
