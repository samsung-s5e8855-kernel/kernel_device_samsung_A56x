...
#include "hardware/sfr/is-sfr-cstat-v3_20.h"
...
static void pablo_hw_api_cstat_hw_is_occurred_kunit_test(struct kunit *test)
{
	...
	ret = pablo_run_cstat_hw_is_occurred(FRAME_END_INTERRUPT, CSTAT_FE);
	KUNIT_EXPECT_TRUE(test, ret);
	...
	ret = pablo_run_cstat_hw_is_occurred(COREX_ERROR_INT, CSTAT_ERR);
	KUNIT_EXPECT_TRUE(test, ret);

	ret = pablo_run_cstat_hw_is_occurred(SDC_ERROR, CSTAT_ERR);
	KUNIT_EXPECT_TRUE(test, ret);

	ret = pablo_run_cstat_hw_is_occurred(LIC_CONTROLLER_OVERFLOW_ERROR, CSTAT_ERR);
	KUNIT_EXPECT_TRUE(test, ret);

	ret = pablo_run_cstat_hw_is_occurred(LIC_CONTROLLER_ROW_ERROR, CSTAT_ERR);
	KUNIT_EXPECT_TRUE(test, ret);

	ret = pablo_run_cstat_hw_is_occurred(C_LOADER_AND_COREX_OVERLAP, CSTAT_ERR);
	KUNIT_EXPECT_TRUE(test, ret);

	ret = pablo_run_cstat_hw_is_occurred(CINFIFO_INPUT_PROTOCOL_ERROR, CSTAT_ERR);
	KUNIT_EXPECT_TRUE(test, ret);

	ret = pablo_run_cstat_hw_is_occurred(CINFIFO_PIXEL_CNT_ERROR, CSTAT_ERR);
	KUNIT_EXPECT_TRUE(test, ret);

	ret = pablo_run_cstat_hw_is_occurred(CINFIFO_OVERFLOW_ERROR, CSTAT_ERR);
	KUNIT_EXPECT_TRUE(test, ret);

	ret = pablo_run_cstat_hw_is_occurred(CINFIFO_OVERLAP_ERROR, CSTAT_ERR);
	KUNIT_EXPECT_TRUE(test, ret);
...
static void pablo_hw_api_cstat_hw_s_drc_dma_kunit_test(struct kunit *test)
{
	...
	for (test_idx = 0; test_idx < CSTAT_TEST_SET; test_idx++) {
		...
		expected_val = (conf.drc_grid_w + 1) / 2 * 2;
		KUNIT_EXPECT_EQ(test, set_val, expected_val);
