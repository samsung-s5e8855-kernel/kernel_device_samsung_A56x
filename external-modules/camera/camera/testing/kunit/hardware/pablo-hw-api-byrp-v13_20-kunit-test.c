...
#include "hardware/sfr/is-sfr-byrp-v13_20.h"
...
static void pablo_hw_api_byrp_hw_g_int_mask_kunit_test(struct kunit *test)
{
	...
	BYRP_SET_F(test_ctx.pmio, BYRP_R_INT_REQ_INT0_ENABLE, BYRP_F_INT_REQ_INT0_ENABLE,
		   INT0_EN_MASK);
	BYRP_SET_F(test_ctx.pmio, BYRP_R_INT_REQ_INT1_ENABLE, BYRP_F_INT_REQ_INT1_ENABLE,
		   INT1_EN_MASK);

	mask = byrp_hw_g_int0_mask(test_ctx.pmio);
	KUNIT_EXPECT_EQ(test, mask, (unsigned int)INT0_EN_MASK);

	mask = byrp_hw_g_int1_mask(test_ctx.pmio);
	KUNIT_EXPECT_EQ(test, mask, (unsigned int)INT1_EN_MASK);
}
...
static void pablo_hw_api_byrp_hw_s_wdma_addr_kunit_test(struct kunit *test)
{
	...
	for (test_idx = BYRP_WDMA_BYR; test_idx < BYRP_WDMA_MAX; test_idx++) {
		...
		comp_sbwc_en = false;
		ret = byrp_hw_s_wdma_addr(&dma, addr, plane, num_buffers, buf_idx, comp_sbwc_en,
					  payload_size, image_addr_offset, header_addr_offset);
		KUNIT_EXPECT_EQ(test, ret, 0);
...
static void pablo_hw_api_byrp_hw_s_rdma_addr_kunit_test(struct kunit *test)
{
	...
	for (test_idx = BYRP_RDMA_IMG; test_idx < BYRP_RDMA_MAX; test_idx++) {
		...
		comp_sbwc_en = false;
		ret = byrp_hw_s_rdma_addr(&dma, addr, plane, num_buffers, buf_idx, comp_sbwc_en,
					  payload_size, image_addr_offset, header_addr_offset);
		KUNIT_EXPECT_EQ(test, ret, 0);
...
static void pablo_hw_api_byrp_hw_is_occurred_kunit_test(struct kunit *test)
{
	u32 ret, status;
	int test_idx;
	u32 err_interrupt0_list[] = { INTR0_BYRP_CMDQ_ERROR_INT,
	...
	u32 err_interrupt1_list[] = { INTR1_BYRP_VOTF_LOST_FLUSH_INT, INTR1_BYRP_SBWC0_ERROR };
...
static void pablo_hw_api_byrp_hw_g_dma_param_ptr_kunit_test(struct kunit *test)
{
	...
	ret = byrp_hw_g_wdma_param_ptr(BYRP_WDMA_CFG_BYR_PROCESSED, dma_frame, param_set, name,
				       &dma_frame_dva, &pdo, &param_set_dva);
	KUNIT_EXPECT_EQ(test, ret, (int)0);
	KUNIT_EXPECT_PTR_EQ(test, (dma_addr_t *)dma_frame_dva,
...
static void pablo_hw_api_byrp_hw_update_param_kunit_test(struct kunit *test)
{
	...
	KUNIT_EXPECT_NE(test, byrp_param_set->dma_output_byr_processed.cmd,
			byrp_param->byr_processed.cmd);
	KUNIT_EXPECT_NE(test, byrp_param_set->stripe_input.total_count,
			byrp_param->stripe_input.total_count);
	...
	KUNIT_EXPECT_EQ(test, byrp_param_set->dma_output_byr_processed.cmd,
			byrp_param->byr_processed.cmd);
	KUNIT_EXPECT_EQ(test, byrp_param_set->stripe_input.total_count,
			byrp_param->stripe_input.total_count);
}
...
struct kunit_suite pablo_hw_api_byrp_kunit_test_suite = {
	.name = "pablo-hw-api-byrp-v13_20-kunit-test",
