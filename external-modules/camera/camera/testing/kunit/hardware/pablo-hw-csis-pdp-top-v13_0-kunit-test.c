...
{
	int ret;
	void *test_addr = (void *)kunit_kzalloc(test, 0xF000, 0);
	u32 cr_offset;
	...
	for (otf_out_id = 0; otf_out_id < CAMIF_OTF_OUT_ID_NUM; otf_out_id++) {
...
static void pablo_hw_csi_pdp_top_enable_ibuf_ptrn_gen_kunit_test(struct kunit *test)
{
	...
	csis_pdp_top_enable_ibuf_ptrn_gen(test_addr, &sensor_cfg, &otf_info, 0, 30, on);
...
static void pablo_hw_csi_pdp_top_nfi_irq_msk_kunit_test(struct kunit *test)
{
	void *test_addr = kunit_kzalloc(test, 0xF000, 0);
	struct pablo_kunit_hw_csis_pdp_top_func *func = pablo_kunit_get_hw_csis_pdp_top_test();
	u64 l_msk, u_msk, concat_msk, ch_msk = 0;
	u32 csi_ch;
	...
	/* set ch #0 */
	csi_ch = 0;
	func->nfi_irq_msk(test_addr, csi_ch, true);
	l_msk = is_hw_get_reg(test_addr, &is_csis_pdp_top_regs[CSIS_PDP_TOP_R_NFI_INT_ENABLE_0]);
	u_msk = is_hw_get_reg(test_addr, &is_csis_pdp_top_regs[CSIS_PDP_TOP_R_NFI_INT_ENABLE_1]);
	concat_msk = l_msk | (u_msk << 32);
	ch_msk |= CSIS_NFI_INTR_EN_MASK << (csi_ch * INTR_NFI_MAX);
	KUNIT_EXPECT_EQ(test, ch_msk, concat_msk);
	...
	/* clear ch #0 */
	csi_ch = 0;
	func->nfi_irq_msk(test_addr, csi_ch, false);
	l_msk = is_hw_get_reg(test_addr, &is_csis_pdp_top_regs[CSIS_PDP_TOP_R_NFI_INT_ENABLE_0]);
	u_msk = is_hw_get_reg(test_addr, &is_csis_pdp_top_regs[CSIS_PDP_TOP_R_NFI_INT_ENABLE_1]);
	concat_msk = l_msk | (u_msk << 32);
	ch_msk &= ~(CSIS_NFI_INTR_EN_MASK << (csi_ch * INTR_NFI_MAX));
	...
	/* clear ch #6 -> all clear */
	csi_ch = 6;
	func->nfi_irq_msk(test_addr, csi_ch, false);
	l_msk = is_hw_get_reg(test_addr, &is_csis_pdp_top_regs[CSIS_PDP_TOP_R_NFI_INT_ENABLE_0]);
	u_msk = is_hw_get_reg(test_addr, &is_csis_pdp_top_regs[CSIS_PDP_TOP_R_NFI_INT_ENABLE_1]);
	concat_msk = l_msk | (u_msk << 32);
	KUNIT_EXPECT_EQ(test, 0, concat_msk);

	kunit_kfree(test, test_addr);
}

static void pablo_hw_csi_pdp_top_nfi_irq_src_kunit_test(struct kunit *test)
{
	void *test_addr = kunit_kzalloc(test, 0xF000, 0);
	struct pablo_kunit_hw_csis_pdp_top_func *func = pablo_kunit_get_hw_csis_pdp_top_test();
	ulong l_src, str, end;
	...
	/* ch #0 IRQ */
	csi_ch = 0;
	l_src = CSIS_NFI_INTR_EN_MASK << (csi_ch * INTR_NFI_MAX);
	is_hw_set_reg(test_addr, &is_csis_pdp_top_regs[CSIS_PDP_TOP_R_NFI_INT_SOURCE_0], l_src);
	func->nfi_irq_src(test_addr, &str, &end);
	KUNIT_EXPECT_TRUE(test, test_bit(csi_ch, &str));
	KUNIT_EXPECT_TRUE(test, test_bit(csi_ch, &end));
	is_hw_set_reg(test_addr, &is_csis_pdp_top_regs[CSIS_PDP_TOP_R_NFI_INT_SOURCE_0], 0);
...
static struct kunit_case pablo_hw_csi_kunit_test_cases[] = {
	...
	KUNIT_CASE(pablo_hw_csi_pdp_top_nfi_irq_msk_kunit_test),
	KUNIT_CASE(pablo_hw_csi_pdp_top_nfi_irq_src_kunit_test),
	{},
};
