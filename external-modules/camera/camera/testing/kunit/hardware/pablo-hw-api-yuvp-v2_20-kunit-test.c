...
static void pablo_hw_api_yuvp_hw_s_block_bypass_kunit_test(struct kunit *test)
{
	...
	int list_size = ARRAY_SIZE(reg_list);
	bool utc_flag = false;
	int i;
	...
	for (i = 0; i < list_size; i++) {
		if (YUVP_GET_R(test_ctx.pmio, reg_list[i])) {
			utc_flag = true;
		} else {
			utc_flag = false;
			break;
	...
	KUNIT_EXPECT_TRUE(test, utc_flag);
	set_utc_result(KUTC_YUVP_BYPASS, UTC_ID_YUVP_BYPASS, utc_flag);
}
