...
static struct pkt_gdc_votf_ops *pkt_votf_ops;
...
static void pablo_gdc_run_next_job_kunit_test(struct kunit *test)
{
	...
	ctx->m2m_ctx = v4l2_m2m_ctx_init(gdc->m2m.m2m_dev, ctx, pkt_ops->queue_init);
...
static void pablo_gdc_out_votf_otf_kunit_test(struct kunit *test)
{
	...
	pkt_votf_ops->out_votf_otf(gdc, ctx);
	KUNIT_EXPECT_EQ(test, (int)atomic_read(&gdc->wait_mfc), 1);
...
static void pablo_gdc_device_run_kunit_test(struct kunit *test)
{
	...
	struct gdc_dev *gdc = &pkt_ctx.gdc;
	struct gdc_ctx *ctx = &pkt_ctx.ctx;
	struct device *backup = gdc_get_votf_device();
	int test_result;

	/* TC : no gdc_dev */
	test_result = pkt_votf_ops->device_run(0);
	KUNIT_EXPECT_EQ(test, test_result, -ENOMEM);

	/* TC : no votf_ctx */
	pdev->dev.driver_data = gdc;
	gdc_set_votf_device(&pdev->dev);
	test_result = pkt_votf_ops->device_run(0);
	KUNIT_EXPECT_EQ(test, test_result, -ENOMEM);
...
static void pablo_gdc_register_ready_frame_cb_kunit_test(struct kunit *test)
{
	...
	/* TC : no gdc_dev */
	test_result = pkt_votf_ops->register_ready_frame_cb(NULL);
	KUNIT_EXPECT_EQ(test, test_result, -ENOMEM);
	...
	test_result = pkt_votf_ops->register_ready_frame_cb(kunit_test_gdc_ready_frame_cb);
	KUNIT_EXPECT_EQ(test, test_result, 0);
...
static void pablo_gdc_unregister_ready_frame_cb_kunit_test(struct kunit *test)
{
	...
	/* TC : no gdc_dev */
	pkt_votf_ops->unregister_ready_frame_cb();

	/* TC : add successful TC */
	pdev->dev.driver_data = gdc;
	gdc_set_votf_device(&pdev->dev);
	gdc->mfc_votf.gdc_frame_ready = kunit_test_gdc_ready_frame_cb;
	pkt_votf_ops->unregister_ready_frame_cb();
	KUNIT_EXPECT_NULL(test, gdc->mfc_votf.gdc_frame_ready);
...
static int pablo_gdc_kunit_test_init(struct kunit *test)
{
	...
	pkt_votf_ops = pablo_kunit_get_gdc_votf_ops();
