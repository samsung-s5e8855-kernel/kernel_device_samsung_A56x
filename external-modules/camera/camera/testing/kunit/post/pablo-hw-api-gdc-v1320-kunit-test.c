...
#include "gdc/pablo-hw-reg-gdc-v1320.h"
#include "is-common-enum.h"
#include "is-type.h"

#define MAX_SUPPORT_SBWC_FMT 8
#define GDC_GET_F(base, R, F)  (0xdead == R ? -1 : PMIO_GET_F(base->pmio, R, F))
#define GDC_GET_R(base, R)  (0xdead == R ? -1 : PMIO_GET_R(base->pmio, R))
...
static void camerapp_hw_select_gdc_reg_kunit_test(struct kunit *test)
{
	...
	gdc->dev_id = 1;
	*(u32 *)(tctx->addr + GDC_REG(gdc->dev_id, R_YUV_GDC_GRID_MODE)) = 0x1;
	val = GDC_GET_R(gdc, GDC_L_R_YUV_GDC_GRID_MODE);
	KUNIT_EXPECT_EQ(test, val, 0x1);

	gdc->dev_id = 0;
	*(u32 *)(tctx->addr + GDC_REG(gdc->dev_id, R_YUV_GDC_GRID_MODE)) = 0x1;
	val = GDC_GET_R(gdc, GDC_O_R_YUV_GDC_GRID_MODE);
	KUNIT_EXPECT_EQ(test, val, 0x1);
}
...
static void camerapp_hw_api_gdc_get_size_constraints_kunit_test(struct kunit *test)
{
	...
	*(u32 *)(tctx->addr + GDC_REG(gdc->dev_id, R_IP_VERSION)) = gdc_variant.version;
...
static void camerapp_hw_api_gdc_start_kunit_test(struct kunit *test)
{
	...
	/* APB-DIRECT */
	*(u32 *)(tctx->addr + GDC_REG(gdc->dev_id, R_IP_PROCESSING)) = 0x1;

	camerapp_hw_gdc_start(gdc->pmio, &clb);

	val = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_CMDQ_ADD_TO_QUEUE_0));
	KUNIT_EXPECT_EQ(test, val, 1);
	val = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_IP_PROCESSING));
	KUNIT_EXPECT_EQ(test, val, 0);

	/* C-LOADER */
	*(u32 *)(tctx->addr + GDC_REG(gdc->dev_id, R_IP_PROCESSING)) = 0x1;
	*(u32 *)(tctx->addr + GDC_REG(gdc->dev_id, R_CMDQ_ADD_TO_QUEUE_0)) = 0x0;

	clb.header_dva = 0xDEADDEAD;
	clb.num_of_headers = 5;
	camerapp_hw_gdc_start(gdc->pmio, &clb);

	val = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_CMDQ_QUE_CMD_H));
	KUNIT_EXPECT_EQ(test, val, clb.header_dva >> 4);
	val = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_CMDQ_QUE_CMD_M));
...
static void camerapp_hw_api_gdc_sw_reset_kunit_test(struct kunit *test)
{
	...
	val2 = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_SW_RESET));
...
static void camerapp_hw_gdc_get_intr_status_and_clear_kunit_test(struct kunit *test)
{
	...
	*(u32 *)(tctx->addr + GDC_REG(gdc->dev_id, R_INT_REQ_INT0)) = int0;
	*(u32 *)(tctx->addr + GDC_REG(gdc->dev_id, R_INT_REQ_INT1)) = int1;

	val0 = camerapp_hw_gdc_get_intr_status_and_clear(gdc->pmio);

	c0 = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_INT_REQ_INT0_CLEAR));
	c1 = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_INT_REQ_INT1_CLEAR));
...
static void camerapp_hw_gdc_set_initialization_kunit_test(struct kunit *test)
{
	...
	val = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_IP_PROCESSING));
	KUNIT_EXPECT_EQ(test, val, (u32)1);
	val = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_C_LOADER_ENABLE));
	KUNIT_EXPECT_EQ(test, val, (u32)1);
	val = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_STAT_RDMACLOADER_EN));
	KUNIT_EXPECT_EQ(test, val, (u32)1);
	val = GDC_GET_F(gdc, GDC_REG(gdc->dev_id, R_CMDQ_QUE_CMD_L), GDC_F_CMDQ_QUE_CMD_INT_GROUP_ENABLE);
	KUNIT_EXPECT_EQ(test, val, (u32)0xFF);
	val = GDC_GET_F(gdc, GDC_REG(gdc->dev_id, R_CMDQ_QUE_CMD_M), GDC_F_CMDQ_QUE_CMD_SETTING_MODE);
	KUNIT_EXPECT_EQ(test, val, (u32)3);
	val = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_CMDQ_ENABLE));
	KUNIT_EXPECT_EQ(test, val, (u32)1);
	val = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_INT_REQ_INT0_ENABLE));
	KUNIT_EXPECT_EQ(test, val, (u32)GDC_INT0_EN);
	val = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_INT_REQ_INT1_ENABLE));
...
static void camerapp_hw_gdc_check_scale_parameters(struct kunit *test)
{
	...
	val1 = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_YUV_GDC_CONFIG));
	KUNIT_EXPECT_EQ(test, val1, (u32)0);
	val1 = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_YUV_GDC_INPUT_ORG_SIZE));
	val2 = (ctx->s_frame.height << 16) | ctx->s_frame.width;
	KUNIT_EXPECT_EQ(test, val1, val2);
	val1 = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_YUV_GDC_INPUT_CROP_START));
	val2 = (crop_param->crop_start_y << 16) | crop_param->crop_start_x;
	KUNIT_EXPECT_EQ(test, val1, val2);
	val1 = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_YUV_GDC_INPUT_CROP_SIZE));
	val2 = (crop_height << 16) | crop_width;
	KUNIT_EXPECT_EQ(test, val1, val2);
	val1 = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_YUV_GDC_SCALE));
	val2 = (gdc_scale_y << 16) | gdc_scale_x;
	KUNIT_EXPECT_EQ(test, val1, val2);
	val1 = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_YUV_GDC_SCALE_SHIFTER));
	val2 = (gdc_scale_shifter_y << 4) | gdc_scale_shifter_x;
	KUNIT_EXPECT_EQ(test, val1, val2);
	val1 = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_YUV_GDC_INV_SCALE));
	val2 = (gdc_inv_scale_y << 16) | gdc_inv_scale_x;
	KUNIT_EXPECT_EQ(test, val1, val2);
	val1 = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_YUV_GDC_OUT_CROP_START));
	val2 = (0x0 << 16) | 0x0;
	KUNIT_EXPECT_EQ(test, val1, val2);
	val1 = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_YUV_GDC_OUT_CROP_SIZE));
	val2 = (ctx->d_frame.height << 16) | ctx->d_frame.width;
	KUNIT_EXPECT_EQ(test, val1, val2);
	val1 = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_YUV_GDC_OUT_SCALE));
...
static void camerapp_hw_gdc_check_grid_parameters(struct kunit *test)
{
	...
	u32 sfr_start_x = GDC_REG(gdc->dev_id, R_YUV_GDC_GRID_DX_0_0);
	u32 sfr_start_y = GDC_REG(gdc->dev_id, R_YUV_GDC_GRID_DY_0_0);
...
static void camerapp_hw_gdc_check_compressor(struct kunit *test)
{
	...
	if (s_frame->extra && d_frame->extra) {
		...
		val1 = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_YUV_GDC_COMPRESSOR));
		val2 = (d_frame->extra << 8) | s_frame->extra;
		KUNIT_EXPECT_EQ(test, val1, val2);
		val1 = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_YUV_RDMAY_COMP_CONTROL));
		val2 = (align_64b << 3) | s_frame->extra;
		KUNIT_EXPECT_EQ(test, val1, val2);
		val1 = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_YUV_RDMAUV_COMP_CONTROL));
		val2 = (align_64b << 3) | s_frame->extra;
		KUNIT_EXPECT_EQ(test, val1, val2);
		val1 = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_YUV_RDMAY_COMP_LOSSY_QUALITY_CONTROL));
		KUNIT_EXPECT_EQ(test, val1, 0);
		val1 = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_YUV_RDMAUV_COMP_LOSSY_QUALITY_CONTROL));
		KUNIT_EXPECT_EQ(test, val1, 0);
		val1 = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_YUV_WDMA_COMP_CONTROL));
		val2 = (align_64b << 3) | d_frame->extra;
		KUNIT_EXPECT_EQ(test, val1, val2);
		val1 = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_YUV_WDMA_COMP_LOSSY_QUALITY_CONTROL));
		KUNIT_EXPECT_EQ(test, val1, 0);
	} else {
		val1 = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_YUV_GDC_COMPRESSOR));
		KUNIT_EXPECT_EQ(test, val1, 0);
...
static void camerapp_hw_gdc_check_format(struct kunit *test)
{
	...
	val = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_YUV_GDC_YUV_FORMAT));
	KUNIT_EXPECT_EQ(test, val, 0x1);
	val = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_YUV_WDMA_DATA_FORMAT));
	KUNIT_EXPECT_EQ(test, val, 0x800);
	val = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_YUV_RDMAY_DATA_FORMAT));
	KUNIT_EXPECT_EQ(test, val, 0x800);
	val = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_YUV_RDMAUV_DATA_FORMAT));
	KUNIT_EXPECT_EQ(test, val, 0x800);
	val = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_YUV_WDMA_MONO_MODE));
...
static void camerapp_hw_gdc_check_dma_addr(struct kunit *test)
{
	...
	/* CORE */
	val = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_YUV_WDMA_EN));
	KUNIT_EXPECT_EQ(test, val, 0x1);
	val = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_YUV_RDMAY_EN));
	KUNIT_EXPECT_EQ(test, val, 0x1);
	val = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_YUV_RDMAUV_EN));
	KUNIT_EXPECT_EQ(test, val, 0x1);

	/* WDMA */
	val = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_YUV_WDMA_IMG_BASE_ADDR_1P_FRO_0_0));
	KUNIT_EXPECT_EQ(test, val, d_frame->addr.y >> 4);
	val = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_YUV_WDMA_IMG_BASE_ADDR_2P_FRO_0_0));
	KUNIT_EXPECT_EQ(test, val, d_frame->addr.cb >> 4);
	val = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_YUV_WDMA_IMG_BASE_ADDR_1P_FRO_LSB_4B_0_0));
	KUNIT_EXPECT_EQ(test, val, d_frame->addr.y & 0xF);
	val = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_YUV_WDMA_IMG_BASE_ADDR_2P_FRO_LSB_4B_0_0));
	KUNIT_EXPECT_EQ(test, val, d_frame->addr.cb & 0xF);
	val = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_YUV_WDMA_HEADER_BASE_ADDR_1P_FRO_0_0));
	KUNIT_EXPECT_EQ(test, val, d_frame->addr.y_2bit >> 4);
	val = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_YUV_WDMA_HEADER_BASE_ADDR_2P_FRO_0_0));
	KUNIT_EXPECT_EQ(test, val, d_frame->addr.cbcr_2bit >> 4);
	val = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_YUV_WDMA_HEADER_BASE_ADDR_1P_FRO_LSB_4B_0_0));
	KUNIT_EXPECT_EQ(test, val, d_frame->addr.y_2bit & 0xF);
	val = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_YUV_WDMA_HEADER_BASE_ADDR_2P_FRO_LSB_4B_0_0));
	KUNIT_EXPECT_EQ(test, val, d_frame->addr.cbcr_2bit & 0xF);

	/* RDMA */
	val = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_YUV_RDMAY_IMG_BASE_ADDR_1P_FRO0));
	KUNIT_EXPECT_EQ(test, val, s_frame->addr.y_2bit >> 4);
	val = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_YUV_RDMAUV_IMG_BASE_ADDR_1P_FRO0));
	KUNIT_EXPECT_EQ(test, val, s_frame->addr.cbcr_2bit >> 4);
	val = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_YUV_RDMAY_IMG_BASE_ADDR_1P_FRO0_LSB_4B));
	KUNIT_EXPECT_EQ(test, val, s_frame->addr.y_2bit & 0xF);
	val = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_YUV_RDMAUV_IMG_BASE_ADDR_1P_FRO0_LSB_4B));
	KUNIT_EXPECT_EQ(test, val, s_frame->addr.cbcr_2bit & 0xF);
	val = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_YUV_RDMAY_HEADER_BASE_ADDR_1P_FRO0));
	KUNIT_EXPECT_EQ(test, val, s_frame->addr.y_2bit >> 4);
	val = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_YUV_RDMAUV_HEADER_BASE_ADDR_1P_FRO0));
	KUNIT_EXPECT_EQ(test, val, s_frame->addr.cbcr_2bit >> 4);
	val = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_YUV_RDMAY_HEADER_BASE_ADDR_1P_FRO0_LSB_4B));
	KUNIT_EXPECT_EQ(test, val, s_frame->addr.y_2bit & 0xF);
	val = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_YUV_RDMAUV_HEADER_BASE_ADDR_1P_FRO0_LSB_4B));
	KUNIT_EXPECT_EQ(test, val, s_frame->addr.cbcr_2bit & 0xF);
					   ...
					   u32 rdma_stride_header_2p)
{
	...
	val = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_YUV_WDMA_WIDTH));
	KUNIT_EXPECT_EQ(test, val, wdma_width);
	val = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_YUV_WDMA_HEIGHT));
	KUNIT_EXPECT_EQ(test, val, wdma_height);
	val = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_YUV_RDMAY_WIDTH));
	KUNIT_EXPECT_EQ(test, val, rdma_y_width);
	val = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_YUV_RDMAY_HEIGHT));
	KUNIT_EXPECT_EQ(test, val, rdma_y_height);
	val = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_YUV_RDMAUV_WIDTH));
	KUNIT_EXPECT_EQ(test, val, rdma_uv_width);
	val = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_YUV_RDMAUV_HEIGHT));
	KUNIT_EXPECT_EQ(test, val, rdma_uv_height);
	val = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_YUV_WDMA_IMG_STRIDE_1P));
	KUNIT_EXPECT_EQ(test, val, wdma_stride_1p);
	val = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_YUV_WDMA_IMG_STRIDE_2P));
	KUNIT_EXPECT_EQ(test, val, wdma_stride_2p);
	val = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_YUV_WDMA_HEADER_STRIDE_1P));
	KUNIT_EXPECT_EQ(test, val, wdma_stride_header_1p);
	val = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_YUV_WDMA_HEADER_STRIDE_2P));
	KUNIT_EXPECT_EQ(test, val, wdma_stride_header_2p);
	val = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_YUV_RDMAY_IMG_STRIDE_1P));
	KUNIT_EXPECT_EQ(test, val, rdma_stride_1p);
	val = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_YUV_RDMAUV_IMG_STRIDE_1P));
	KUNIT_EXPECT_EQ(test, val, rdma_stride_2p);
	val = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_YUV_RDMAY_HEADER_STRIDE_1P));
	KUNIT_EXPECT_EQ(test, val, rdma_stride_header_1p);
	val = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_YUV_RDMAUV_HEADER_STRIDE_1P));
...
static void camerapp_hw_gdc_check_core_param(struct kunit *test)
{
	...
	val = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_YUV_GDC_INTERPOLATION));
	KUNIT_EXPECT_EQ(test, val, 0x23);
	val = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_YUV_GDC_BOUNDARY_OPTION));
	KUNIT_EXPECT_EQ(test, val, 0x1);
	val = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_YUV_GDC_GRID_MODE));
	KUNIT_EXPECT_EQ(test, val, crop_param->is_grid_mode);
	val = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_YUV_GDC_YUV_FORMAT));
	KUNIT_EXPECT_EQ(test, val, 0x1);
	val = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_YUV_GDC_LUMA_MINMAX));
	KUNIT_EXPECT_EQ(test, val, 0xFF0000);
	val = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_YUV_GDC_CHROMA_MINMAX));
	KUNIT_EXPECT_EQ(test, val, 0xFF0000);
}
...
static void camerapp_hw_gdc_check_out_select(struct kunit *test)
{
	...
	val1 = GDC_GET_R(gdc, GDC_REG(gdc->dev_id, R_YUV_GDC_OUTPUT_SELECT));
	KUNIT_EXPECT_EQ(test, val1, val2);
}
...
static void camerapp_hw_gdc_init_pmio_config_kunit_test(struct kunit *test)
{
	...
	val = gdc->pmio_config.max_register;
	KUNIT_EXPECT_EQ(test, val, GDC_L_R_MAX_REGISTER);

	gdc->dev_id = 0;
	camerapp_hw_gdc_init_pmio_config(gdc);
	val = gdc->pmio_config.max_register;
	KUNIT_EXPECT_EQ(test, val, GDC_O_R_MAX_REGISTER);
}
...
static struct kunit_case camerapp_hw_api_gdc_kunit_test_cases[] = {
	KUNIT_CASE(camerapp_hw_select_gdc_reg_kunit_test),
...
struct kunit_suite camerapp_hw_api_gdc_kunit_test_suite = {
	.name = "pablo-hw-api-gdc-v1320-kunit-test",
