...
static struct putc_info putc_kunit_list[KUTC_MAX];
void set_utc_result(u32 idx, u32 id, bool pass)
{
	int result = pass ? P_PASS : P_FAIL;

	putc_kunit_list[idx].camdev_id = id;
	putc_kunit_list[idx].result = result;
}
...
{
	memset(putc_kunit_list, 0, sizeof(struct putc_info) * KUTC_MAX);
	if (val[0] == '1') {
		_save_origin_ctx();
		__run_kunit_test();
...
static int __get_pablo_kunit_test(char *buffer, const struct kernel_param *kp)
{
	int i, ret;

	ret = scnprintf(buffer, PAGE_SIZE, "%lu\n[UTC]\n", ARRAY_SIZE(putc_kunit_list));
	for (i = 0; i < ARRAY_SIZE(putc_kunit_list); i++) {
		if (putc_kunit_list[i].camdev_id)
			ret += sprintf(buffer + ret, "%d:%d\n", putc_kunit_list[i].camdev_id,
				putc_kunit_list[i].result);
	}
