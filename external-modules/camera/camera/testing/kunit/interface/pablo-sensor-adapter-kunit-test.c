...
static void pablo_sensor_adt_v1_request_seamless_mode_change_kunit_test(struct kunit *test)
{
	int ret = 0;
	u32 instance;
	struct test_ctx *test_ctx;
	...
	psci.sensor_control_size = 1;
	psci.sensor_control[0] = PABLO_CRTA_MAGIC_NUMBER;
	psci.sensor_control[1] = SS_CTRL_CIS_REQUEST_SEAMLESS_MODE_CHANGE;
	psci.sensor_control[2] = 0;
	psci.sensor_control[3] = 0;
	...
	psci.sensor_control[7] = 0;
	psci.sensor_control[8] = 0;
	psci.sensor_control[9] = 0;
	psci.sensor_control[10] = 0;
	psci.sensor_control[11] = SS_CTRL_END;
...
static struct kunit_case pablo_sensor_adt_v1_kunit_test_cases[] = {
	...
	KUNIT_CASE(pablo_sensor_adt_v1_request_seamless_mode_change_kunit_test),
...
static int pablo_sensor_adt_v1_kunit_test_init(struct kunit *test)
{
	...
	pablo_sensor_adt_test_ctx.backup_sensor_cfg = sensor->cfg[sensor->nfi_toggle];
	pablo_sensor_adt_test_ctx.sensor_cfg.input[0].width = 4032;
	pablo_sensor_adt_test_ctx.sensor_cfg.input[0].height = 3024;
	sensor->cfg[sensor->nfi_toggle] = &pablo_sensor_adt_test_ctx.sensor_cfg;
...
static void pablo_sensor_adt_v1_kunit_test_exit(struct kunit *test)
{
	...
	sensor->cfg[sensor->nfi_toggle] = pablo_sensor_adt_test_ctx.backup_sensor_cfg;
