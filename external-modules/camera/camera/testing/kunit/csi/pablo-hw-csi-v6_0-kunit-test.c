...
static void pablo_hw_csi_hw_s_config_sbwc_kunit_test(struct kunit *test)
{
	struct pablo_kunit_hw_csi_func *func = pablo_kunit_get_hw_csi_test();
	...
	wdma = kunit_kzalloc(test, sizeof(struct is_camif_wdma), GFP_KERNEL);
	wdma->regs_ctl = kunit_kmalloc(test, SZ_64K, flags);
	test_result = func->csi_hw_s_config_sbwc(2040, wdma->regs_ctl, 0);
	KUNIT_EXPECT_EQ(test, test_result, DMA_OUTPUT_SBWC_DISABLE);
	kunit_kfree(test, wdma->regs_ctl);
	kunit_kfree(test, wdma);
}

static void pablo_hw_csi_hw_get_dim_hwfmt_kunit_test(struct kunit *test)
{
	struct pablo_kunit_hw_csi_func *func = pablo_kunit_get_hw_csi_test();
	u32 dim, hw_format;

	func->csi_hw_get_dim_hwfmt(V4L2_PIX_FMT_SGRBG8, &dim, &hw_format);
	KUNIT_EXPECT_EQ(test, dim, CSIS_REG_DMA_1D_DMA);
	KUNIT_EXPECT_EQ(test, hw_format, DMA_INOUT_FORMAT_BAYER_PACKED);
	func->csi_hw_get_dim_hwfmt(V4L2_PIX_FMT_SBGGR8, &dim, &hw_format);
	KUNIT_EXPECT_EQ(test, dim, CSIS_REG_DMA_1D_DMA);
	KUNIT_EXPECT_EQ(test, hw_format, DMA_INOUT_FORMAT_BAYER_PACKED);

	func->csi_hw_get_dim_hwfmt(V4L2_PIX_FMT_SBGGR10, &dim, &hw_format);
	KUNIT_EXPECT_EQ(test, dim, CSIS_REG_DMA_2D_DMA);
	KUNIT_EXPECT_EQ(test, hw_format, DMA_INOUT_FORMAT_BAYER);
	func->csi_hw_get_dim_hwfmt(V4L2_PIX_FMT_SBGGR12, &dim, &hw_format);
	KUNIT_EXPECT_EQ(test, dim, CSIS_REG_DMA_2D_DMA);
	KUNIT_EXPECT_EQ(test, hw_format, DMA_INOUT_FORMAT_BAYER);
	func->csi_hw_get_dim_hwfmt(V4L2_PIX_FMT_SBGGR16, &dim, &hw_format);
	KUNIT_EXPECT_EQ(test, dim, CSIS_REG_DMA_2D_DMA);
	KUNIT_EXPECT_EQ(test, hw_format, DMA_INOUT_FORMAT_BAYER);

	func->csi_hw_get_dim_hwfmt(V4L2_PIX_FMT_SBGGR10P, &dim, &hw_format);
	KUNIT_EXPECT_EQ(test, dim, CSIS_REG_DMA_2D_DMA);
	KUNIT_EXPECT_EQ(test, hw_format, DMA_INOUT_FORMAT_BAYER_PACKED);
	func->csi_hw_get_dim_hwfmt(V4L2_PIX_FMT_SBGGR12P, &dim, &hw_format);
	KUNIT_EXPECT_EQ(test, dim, CSIS_REG_DMA_2D_DMA);
	KUNIT_EXPECT_EQ(test, hw_format, DMA_INOUT_FORMAT_BAYER_PACKED);
	func->csi_hw_get_dim_hwfmt(V4L2_PIX_FMT_PRIV_MAGIC, &dim, &hw_format);
	KUNIT_EXPECT_EQ(test, dim, CSIS_REG_DMA_2D_DMA);
	KUNIT_EXPECT_EQ(test, hw_format, DMA_INOUT_FORMAT_BAYER_PACKED);
	func->csi_hw_get_dim_hwfmt(V4L2_PIX_FMT_ARGB32, &dim, &hw_format);
	KUNIT_EXPECT_EQ(test, dim, CSIS_REG_DMA_2D_DMA);
	KUNIT_EXPECT_EQ(test, hw_format, DMA_INOUT_FORMAT_BAYER);
}

static bool hw_get_dma_fmt(
	struct kunit *test, u32 hwformat, u32 pack_fmt, u32 unpack_fmt, u32 pack_bitwdt)
{
	struct pablo_kunit_hw_csi_func *func = pablo_kunit_get_hw_csi_test();
	u32 dma_format, bitwidth;
	bool pack_flag = false, unpack_flag = false;

	func->csi_hw_get_dma_fmt(
		pack_bitwdt, hwformat, DMA_INOUT_FORMAT_BAYER_PACKED, &dma_format, &bitwidth);
	KUNIT_EXPECT_EQ(test, dma_format, pack_fmt);
	KUNIT_EXPECT_EQ(test, bitwidth, pack_bitwdt);
	if (dma_format == pack_fmt && bitwidth == pack_bitwdt)
		pack_flag = true;
	func->csi_hw_get_dma_fmt(
		pack_bitwdt, hwformat, DMA_INOUT_FORMAT_BAYER, &dma_format, &bitwidth);
	KUNIT_EXPECT_EQ(test, dma_format, unpack_fmt);
	KUNIT_EXPECT_EQ(test, bitwidth, 16);
	if (dma_format == unpack_fmt && bitwidth == 16)
		unpack_flag = true;
...
static void pablo_hw_csi_hw_get_dma_fmt_kunit_test(struct kunit *test)
{
	bool flag;

	/* RAW8 */
	flag = hw_get_dma_fmt(test, HW_FORMAT_RAW8, CSIS_DMA_FMT_U8BIT_PACK,
		CSIS_DMA_FMT_U8BIT_UNPACK_MSB_ZERO, 8);
	set_utc_result(KUTC_S_FMT_RAW8, UTC_ID_FMT_RAW8, flag == true);

	/* RAW10 */
	flag = hw_get_dma_fmt(test, HW_FORMAT_RAW10, CSIS_DMA_FMT_U10BIT_PACK,
		CSIS_DMA_FMT_U10BIT_UNPACK_MSB_ZERO, 10);
	set_utc_result(KUTC_S_FMT_RAW10, UTC_ID_FMT_RAW10, flag == true);

	/* RAW12 */
	flag = hw_get_dma_fmt(test, HW_FORMAT_RAW12, CSIS_DMA_FMT_U12BIT_PACK,
		CSIS_DMA_FMT_U12BIT_UNPACK_MSB_ZERO, 12);
	set_utc_result(KUTC_S_FMT_RAW12, UTC_ID_FMT_RAW12, flag == true);
}

static void pablo_hw_csi_hw_s_fmt_dma_kunit_test(struct kunit *test)
{
	struct pablo_kunit_hw_csi_func *func = pablo_kunit_get_hw_csi_test();
	struct is_camif_wdma *wdma;
	...
	wdma = kunit_kzalloc(test, sizeof(struct is_camif_wdma), GFP_KERNEL);
	wdma->regs_vc[0] = kunit_kmalloc(test, SZ_64K, flags);
	framecfg.format = kunit_kzalloc(test, sizeof(struct is_fmt), GFP_KERNEL);
	framecfg.format->pixelformat = V4L2_PIX_FMT_SBGGR10;
	test_result =
		func->csi_hw_s_fmt_dma(wdma->regs_vc[0], &framecfg, 0, HW_FORMAT_RAW10, &bitwidth);
	KUNIT_EXPECT_EQ(test, test_result, 0);
	KUNIT_EXPECT_EQ(test, bitwidth, 16);
	kunit_kfree(test, framecfg.format);
	kunit_kfree(test, wdma->regs_vc[0]);
	kunit_kfree(test, wdma);
}

static void pablo_hw_csi_hw_s_config_dma_kunit_tes(struct kunit *test)
{
	struct pablo_kunit_hw_csi_func *func = pablo_kunit_get_hw_csi_test();
	u32 width = 2040;
	struct is_camif_wdma *wdma;
	...
	wdma->regs_ctl = kunit_kmalloc(test, SZ_64K, flags);
	wdma->regs_vc[0] = kunit_kmalloc(test, SZ_64K, flags);
	framecfg.width = width;
	framecfg.format = kunit_kzalloc(test, sizeof(struct is_fmt), GFP_KERNEL);
	test_result = func->csi_hw_s_config_dma(
		wdma->regs_ctl, wdma->regs_vc[0], 0, &framecfg, HW_FORMAT_RAW8, &stride);
	KUNIT_EXPECT_EQ(test, test_result, 0);
	KUNIT_EXPECT_EQ(test, stride, ALIGN(width * 2, 32));
	kunit_kfree(test, framecfg.format);
	kunit_kfree(test, wdma->regs_vc[0]);
	kunit_kfree(test, wdma->regs_ctl);
	kunit_kfree(test, wdma);
...
static struct kunit_case pablo_hw_csi_kunit_test_cases[] = {
	...
	KUNIT_CASE(pablo_hw_csi_hw_s_config_sbwc_kunit_test),
	KUNIT_CASE(pablo_hw_csi_hw_get_dim_hwfmt_kunit_test),
	KUNIT_CASE(pablo_hw_csi_hw_get_dma_fmt_kunit_test),
	KUNIT_CASE(pablo_hw_csi_hw_s_fmt_dma_kunit_test),
	KUNIT_CASE(pablo_hw_csi_hw_s_config_dma_kunit_tes),
	{},
};
