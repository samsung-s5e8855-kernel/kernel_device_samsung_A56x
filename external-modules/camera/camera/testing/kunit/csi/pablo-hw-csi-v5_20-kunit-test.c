...
static void pablo_hw_csi_g_dam_common_frame_id_kunit_test(struct kunit *test)
{
	...
	batch_num = 8;
	*(u32 *)(test_addr +
		 csi_dma_cmn_regs[CSIS_DMA_CMN_R_CSIS_CMN_FRO_PREV_FRAME_ID0].sfr_offset) =
		0x23333333;
	...
	*(u32 *)(test_addr +
		...
		0x23333333;
	*(u32 *)(test_addr +
		 csi_dma_cmn_regs[CSIS_DMA_CMN_R_CSIS_CMN_FRO_PREV_FRAME_ID1].sfr_offset) =
		0x45555555;
	ret = func->csi_hw_g_dma_common_frame_id(test_addr, batch_num, frame_id);
	KUNIT_EXPECT_EQ(test, ret, 0);
...
static void pablo_hw_csi_g_frame_ptr_kunit_test(struct kunit *test)
{
	...
	for (f_no = 0; f_no < 0x1F; f_no++) {
		*(u32 *)(test_addr + csi_dmax_chx_regs[CSIS_DMAX_CHX_R_ACT_CTRL].sfr_offset) = f_no
											       << 2;
...
static void pablo_hw_csi_hw_s_dma_input_mux(struct kunit *test)
{
	...
	u32 mux_val_base, wdma_ch = 0;
	int link_vc_list[CSIS_OTF_CH_LC_NUM] = { 0, 1, 2, 3 };
	...
	wdma_idx = otf_out_id;
	func->csi_hw_s_dma_input_mux(reg_mux, wdma_idx, wdma_ch, bns_en, bns_dma_mux_val, csi_ch,
				     otf_ch, otf_out_id, link_vc_list, &wdma_info);
	val = *(u32 *)(reg_mux);
...
struct kunit_suite pablo_hw_csi_kunit_test_suite = {
	.name = "pablo-hw-csi-v5_20-kunit-test",
