...
static void pablo_device_sensor_v2_cmp_mode_kunit_test(struct kunit *test)
{
	...
	mode.ex_mode_format = EX_FORMAT_NONE;
...
static void pablo_device_sensor_v2_g_aeb_mode_kunit_test(struct kunit *test)
{
	...
	ids = &test_ctx.ids;
	ids->cfg[ids->nfi_toggle] = &test_ctx.cfg;

	/* Sub TC: true */
	ids->group_sensor.pnext = &test_ctx.group;
	ids->cfg[ids->nfi_toggle]->ex_mode = EX_AEB;
...
static void pablo_device_sensor_v2_g_ex_mode_kunit_test(struct kunit *test)
{
	...
	/* Sub TC: true */
	ids->cfg[ids->nfi_toggle]->ex_mode = EX_DRAMTEST;
...
static void pablo_device_sensor_v2_s_ext_ctrls_kunit_test(struct kunit *test)
{
	...
	struct is_module_enum *module;
	struct v4l2_ext_controls *ctrls;
	struct v4l2_ext_control *ext_ctrl = kunit_kzalloc(test, sizeof(struct v4l2_ext_control), 0);
	struct is_sensor_mode *mode = kunit_kzalloc(test, sizeof(struct is_sensor_mode), 0);
	struct is_sensor_cfg *cfg = kunit_kzalloc(test, sizeof(struct is_sensor_cfg), 0);
	u32 check_num = __LINE__;
	int ret;
	...
	/* Sub TC: V4L2_CID_SENSOR_SET_NFI */
	mode->width = cfg->width = 4000;
	mode->height = cfg->height = 3000;
	mode->framerate = cfg->framerate = 30;
	cfg->input[CSI_VIRTUAL_CH_0].hwformat = HW_FORMAT_RAW10;
	cfg->mode = check_num;
	module->cfgs = 1;
	module->cfg = cfg;
	...
	ctrls->count = 1;
	ctrls->controls = ext_ctrl;
	ctrls->controls->id = V4L2_CID_SENSOR_SET_NFI;
	ctrls->controls->ptr = mode;
	ret = fp->s_ext_ctrls(ids, ctrls);
	KUNIT_EXPECT_EQ(test, ret, 0);
	KUNIT_EXPECT_NOT_ERR_OR_NULL(test, ids->cfg[!ids->nfi_toggle]);
	KUNIT_EXPECT_EQ(test, ids->cfg[!ids->nfi_toggle]->mode, check_num);

	kunit_kfree(test, cfg);
	kunit_kfree(test, mode);
	kunit_kfree(test, ext_ctrl);
}
...
static void pablo_device_sensor_v2_notify_by_dma_end_kunit_test(struct kunit *test)
{
	...
	arg->pb_output = kunit_kzalloc(test, sizeof(struct is_priv_buf), 0);
	arg->pb_output->ops = kunit_kzalloc(test, sizeof(struct is_priv_buf_ops), 0);
	ret = fp->notify_by_dma_end(ids, arg, notification);
	KUNIT_EXPECT_EQ(test, ret, 0);
#endif
	...
	ret = fp->notify_by_dma_end(ids, arg, notification);
	KUNIT_EXPECT_EQ(test, ret, 0);
	kunit_kfree(test, arg->pb_output->ops);
	kunit_kfree(test, arg->pb_output);
...
{
	struct is_device_sensor *ids;
	unsigned int notification = CSIS_NOTIFY_FSTART;
...
static void pablo_device_sensor_v2_snr_kunit_test(struct kunit *test)
{
	...
	set_bit(IS_SENSOR_START, &ids->state);
	csi = &test_ctx.csi;
	ids->subdev_csi = &test_ctx.subdev_csi;
	fp->snr(&ids->snr_timer);
