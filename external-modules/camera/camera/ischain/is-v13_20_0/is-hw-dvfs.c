	...
	struct is_device_ischain *device, struct is_dvfs_scenario_param *param)
{
	...
	if (vendor == IS_DVFS_SCENARIO_VENDOR_SINGLE_TAKE)
		return IS_DVFS_SENSOR_SINGLETAKE;
	...
	struct is_device_ischain *device, struct is_dvfs_scenario_param *param)
{
	...
	else if (param->mode == IS_DVFS_MODE_VIDEO && resol < IS_DVFS_RESOL_HD_TH)
	...
	struct is_dvfs_scenario_param *param)
{
	...
	mdbg_dvfs(1, "[DVFS] %s() rear mask: 0x%x, front mask: 0x%x, tele mask: 0x%x\n", device,
		__func__, param->rear_mask, param->front_mask, param->tele_mask);
}
	...
	struct is_device_ischain *device, int flag_capture, struct is_dvfs_scenario_param *param)
{
	...
	for (device_id = 0; device_id < IS_SENSOR_COUNT; device_id++) {
		...
		if (ids->cfg[ids->nfi_toggle]->special_mode)
			param->sensor_mode = ids->cfg[ids->nfi_toggle]->special_mode;
...
static int is_hw_dvfs_get_rear_single_tele_scenario(
	struct is_device_ischain *device, struct is_dvfs_scenario_param *param)
{
...
static int is_hw_dvfs_get_rear_dual_wide_tele_scenario(
	struct is_device_ischain *device, struct is_dvfs_scenario_param *param)
{
	mdbg_dvfs(1, "[DVFS] %s() mode: %s, resol: %s, fps: %s\n", device, __func__,
		  str_mode[param->mode], str_resol[param->resol], str_fps[param->fps]);
...
static int is_hw_dvfs_get_rear_dual_wide_ultrawide_scenario(
	struct is_device_ischain *device, struct is_dvfs_scenario_param *param)
{
	mdbg_dvfs(1, "[DVFS] %s() mode: %s, resol: %s, fps: %s\n", device, __func__,
		  str_mode[param->mode], str_resol[param->resol], str_fps[param->fps]);
...
static int is_hw_dvfs_get_rear_dual_wide_macro_scenario(
	struct is_device_ischain *device, struct is_dvfs_scenario_param *param)
{
	mdbg_dvfs(1, "[DVFS] %s() mode: %s, resol: %s, fps: %s\n", device, __func__,
		  str_mode[param->mode], str_resol[param->resol], str_fps[param->fps]);
...
static int is_hw_dvfs_get_front_single_remosaic_scenario(
	struct is_device_ischain *device, struct is_dvfs_scenario_param *param)
{
	mdbg_dvfs(1, "[DVFS] %s() mode: %s, resol: %s, fps: %s\n", device, __func__,
		str_mode[param->mode], str_resol[param->resol], str_fps[param->fps]);
...
int is_hw_dvfs_get_scenario(struct is_device_ischain *device, int flag_capture)
{
	...
	case IS_DVFS_NUM_SINGLE:
		...
		case IS_DVFS_FACE_REAR:
			...
			case IS_DVFS_SENSOR_SINGLETAKE:
				return IS_DVFS_SN_REAR_SINGLE_SINGLETAKE;
			default:
				goto dvfs_err;
			}
		case IS_DVFS_FACE_FRONT:
			...
			case IS_DVFS_SENSOR_SINGLETAKE:
				return IS_DVFS_SN_FRONT_SINGLE_SINGLETAKE;
			default:
				goto dvfs_err;
			}
		default:
	...
	case IS_DVFS_NUM_DUAL:
		...
		case IS_DVFS_FACE_PIP:
			if (param.sensor == IS_DVFS_SENSOR_FASTAE)
				return IS_DVFS_SN_PIP_DUAL_FASTAE;
			else
...
static inline u32 get_otf_clock(struct is_sensor_cfg *cfg, u32 mbps, bool potf)
{
	u32 vc, outbpp, bpc, min_bpc = IS_DVFS_POTF_BIT;

	/* IS_SENSOR_ONLY */
	if (potf)
		return is_hw_dvfs_get_csis_clk(mbps, IS_DVFS_POTF_BIT);

	/* OTF output to ISP */
	for (vc = 0; vc < cfg->max_vc; vc++) {
		if (cfg->input[vc].hwformat == HW_FORMAT_UNKNOWN)
			continue;

		outbpp = csi_hw_g_bpp(cfg->input[vc].hwformat);
		bpc = CHECK_POTF_EN(cfg->input[vc].extformat) ? IS_DVFS_POTF_BIT :
								outbpp * IS_DVFS_OTF_PPC;
		min_bpc = MIN(min_bpc, bpc);
	}

	return is_hw_dvfs_get_csis_clk(mbps, min_bpc);
}

static inline u32 get_potf_unpack_clock(struct sensor_dma_output *dma_output,
	struct pablo_internal_subdev (*arr_sd)[DMA_VIRTUAL_CH_MAX], struct is_sensor_cfg *cfg,
	u32 mbps)
{
	...
	u32 potf_clk, potf_unpack_clk;

	potf_clk = is_hw_dvfs_get_csis_clk(mbps, IS_DVFS_POTF_BIT);
	for (idx_wdma = 0; idx_wdma < cfg->dma_num; idx_wdma++) {
		for (wdma_vc = DMA_VIRTUAL_CH_0; wdma_vc < DMA_VIRTUAL_CH_MAX; wdma_vc++) {
			...
			if (!CHECK_POTF_EN(cfg->output[idx_wdma][wdma_vc].extformat))
				continue;

			link_vc = cfg->link_vc[idx_wdma][wdma_vc];
			format = &dma_output[link_vc].fmt;
			if (format->hw_format) {
				/* for LVN */
				...
				outbpp = csi_hw_g_bpp(cfg->output[idx_wdma][wdma_vc].hwformat);
				out_bitwidth = i_subdev->memory_bitwidth;
				hw_format = DMA_INOUT_FORMAT_MAX;
			}

			if (is_hw_dvfs_get_packed(hw_format, outbpp, out_bitwidth))
				continue;

			/* check pOTF unpack throughput */
			potf_unpack_clk = potf_clk * out_bitwidth / outbpp;
			max_clk_csis = MAX(max_clk_csis, potf_unpack_clk);

			/* check pOTF unpack limitation(70%) */
			potf_unpack_clk = potf_clk * 100 / 70;
			max_clk_csis = MAX(max_clk_csis, potf_unpack_clk);
		}
	}
...
static u32 is_hw_dvfs_get_csis(struct is_sensor_cfg *cfg, struct is_device_sensor *ids)
{
	...
	/* When DMA out is disabled. */
	otf_clk = get_otf_clock(cfg, mbps, csi->potf);

	/* When DMA out is enabled. */
	potf_unpack_clk =
		get_potf_unpack_clock(ids->dma_output, csi->i_subdev[csi->nfi_toggle], cfg, mbps);

	return MAX(otf_clk, potf_unpack_clk);
}
