...
static const u32 s2mpu_address_table[SYSMMU_DMAX_S2] = {
	...
	[SYSMMU_CSIS_S0_S2] = 0x175A0054,
	[SYSMMU_D_CSTAT_S2] = 0x178F0054,
	[SYSMMU_RGBP_S0_S2] = 0x17C50054,
	[SYSMMU_YUVP_S0_S2] = 0x16C80054,
	[SYSMMU_M2M_S0_S2] = 0x160F0054,
	[SYSMMU_ICPU_S0_S2] = 0x15870054,
};

static const struct is_reg sysreg_regs[SYSREG_REG_NUM] = {
	/*SYSREG_CSTAT*/
	{0x4FC, "AXAC_CSTAT0"},
	{0x500, "AXAC_CSTAT1"},
	{0x504, "AXAC_CSTAT2"},
	...
	{0x54C, "AXAC_MCSC1"},
	{0x550, "AXAC_MCSC2"},
	{0x55C, "AXIC_BYRP_MCSC"},
	/*SYSREG_M2M*/
	{0x528, "AXIC_1"}
};

static const struct is_field sysreg_fields[SYSREG_REG_FIELD_NUM] = {
	/*SYSREG_CSTAT*/
	{"AW_AC_CSTAT0", 6, 4, RW, 0x8},
	{"AW_AC_CSTAT1", 6, 2, RW, 0x2},
	{"AW_AC_CSTAT2", 6, 2, RW, 0x2},
	{"AW_AC_CSTAT3", 2, 2, RW, 0x2},
	{"AW_AC_CSTAT4", 6, 2, RW, 0x2},
	{"AW_AC_CSTAT5", 6, 2, RW, 0x2},
	{"AW_IC_TARGET_0", 8, 7, RW, 0x2C},
	{"AW_AC_MCSC0", 6, 7, RW, 0x30},
	{"AR_AC_MCSC0", 0, 6, RW, 0x20},
	{"AW_AC_MCSC1", 6, 7, RW, 0x30},
	{"AW_AC_MCSC2", 2, 7, RW, 0x30},
	{"AW_IC_TARGET_2", 9, 9, RW, 0xBD},
	/*SYSREG_M2M*/
	{"AW_IC_1", 10, 9, RW, 0xA4}
};

struct sysreg_reset_reg {
	u32 reg;
	u32 field;
	u32 blk_type;
};

static const struct sysreg_reset_reg sysreg_reset_list[SYSREG_REG_FIELD_NUM] = {
	{AXAC_CSTAT0, AW_AC_CSTAT0, SYSREG_CSTAT},
	{AXAC_CSTAT1, AW_AC_CSTAT1, SYSREG_CSTAT},
	{AXAC_CSTAT2, AW_AC_CSTAT2, SYSREG_CSTAT},
	{AXAC_CSTAT3, AW_AC_CSTAT3, SYSREG_CSTAT},
	{AXAC_CSTAT4, AW_AC_CSTAT4, SYSREG_CSTAT},
	{AXAC_CSTAT5, AW_AC_CSTAT5, SYSREG_CSTAT},
	{AXIC_CSTAT, AW_IC_TARGET_0, SYSREG_CSTAT},
	{AXAC_MCSC0, AW_AC_MCSC0, SYSREG_CSTAT},
	{AXAC_MCSC0, AR_AC_MCSC0, SYSREG_CSTAT},
	{AXAC_MCSC1, AW_AC_MCSC1, SYSREG_CSTAT},
	{AXAC_MCSC2, AW_AC_MCSC2, SYSREG_CSTAT},
	{AXIC_BYRP_MCSC, AW_IC_TARGET_2, SYSREG_CSTAT},
	{AXIC_1, AW_IC_1, SYSREG_M2M}
};
...
{
	void __iomem *reg;
	u32 mux_addr[] = {0x174E0254, 0x174E0258, 0x174E025C, 0x174E0260};
...
static void __is_hw_ischain_sysreg_reset(void)
{
	void __iomem *cstat_sys_regs, *m2m_sys_regs, *sys_regs;
	u32 elem_num;
	int idx;
	u32 val;

	cstat_sys_regs = ioremap(SYSREG_CSTAT_BASE_ADDR, 0x1000);
	m2m_sys_regs = ioremap(SYSREG_M2M_BASE_ADDR, 0x1000);

	elem_num = sizeof(sysreg_reset_list)/sizeof(struct sysreg_reset_reg);
	for (idx = 0; idx < elem_num; idx++) {
		if (sysreg_reset_list[idx].blk_type == SYSREG_CSTAT)
			sys_regs = cstat_sys_regs;
		else
			sys_regs = m2m_sys_regs;

		val = is_hw_get_field(sys_regs,
				&sysreg_regs[sysreg_reset_list[idx].reg],
				&sysreg_fields[sysreg_reset_list[idx].field]);
		info("[%d] reset %s(0x%x) -> 0x%x\n", idx,
				sysreg_fields[sysreg_reset_list[idx].field].field_name,
				val, sysreg_fields[sysreg_reset_list[idx].field].reset);
		is_hw_set_field(sys_regs,
				&sysreg_regs[sysreg_reset_list[idx].reg],
				&sysreg_fields[sysreg_reset_list[idx].field],
				sysreg_fields[sysreg_reset_list[idx].field].reset);
	}

	iounmap(cstat_sys_regs);
	iounmap(m2m_sys_regs);
}
...
int is_hw_ischain_enable(struct is_core *core)
{
	...
	/* EVT0 BLK_CSTAT sysreg needs SW reset */
	__is_hw_ischain_sysreg_reset();
...
void is_hw_csi_qchannel_enable_all(bool enable)
{
	phys_addr_t csis_cmn_ctrls[NUM_OF_CSIS] = {
		0x17480004, 0x17490004, 0x174A0004, 0x174B0004,
		0x174C0004,
	};
	phys_addr_t csis_pdp_top = 0x174E0000;
	void __iomem *base;
	u32 val;
	int i;
...
size_t is_hw_get_param_dump(char *buf, size_t buf_size, struct is_param_region *param, u32 group_id)
{
	...
	case GROUP_ID_LME0:
		if (hw_ip)
			wr_size = CALL_HWIP_OPS(hw_ip, dump_params, 0, p, rem);
		break;
	default:
