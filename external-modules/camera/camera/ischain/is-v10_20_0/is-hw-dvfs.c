...
static u32 _check_remosaic_size(struct is_device_ischain *device)
{
	...
	child = device->group[GROUP_SLOT_PAF];
	if (child) {
		ldr = &child->leader;
	...
	mdbg_dvfs(1, "[DVFS] check remosaic size (%d * %d vs %d)\n", device, w, h,
		  MIN_REMOSAIC_SIZE);
	...
	struct is_device_ischain *device, struct is_dvfs_scenario_param *param)
{
	...
	mdbg_dvfs(1, "[DVFS] %s() width: %d, height: %d\n", device, __func__, width, height);
	resol = width * height;
...
static void is_hw_dvfs_init_face_mask(
	struct is_device_ischain *device, struct is_dvfs_scenario_param *param)
{
	int pos;
	...
	struct is_device_ischain *device, int flag_capture, struct is_dvfs_scenario_param *param)
{
	...
	mdbg_dvfs(1, "[DVFS] %s() param_face(%d), num(%d), sensor(%d)\n", device, __func__,
		  param->face, param->num, param->sensor);
	...
	mdbg_dvfs(1, "[DVFS] %s() param_mode(%d), resol(%d), fps(%d)\n", device, __func__,
		  param->mode, param->resol, param->fps);
...
static int is_hw_dvfs_get_rear_dual_tele_ultrawide_scenario(
	struct is_device_ischain *device, struct is_dvfs_scenario_param *param)
{
	mdbg_dvfs(1, "[DVFS] %s() mode: %s, resol: %s, fps: %s\n", device, __func__,
		  str_mode[param->mode], str_resol[param->resol], str_fps[param->fps]);
...
static int is_hw_dvfs_get_rear_dual_tele_ultratele_scenario(
	struct is_device_ischain *device, struct is_dvfs_scenario_param *param)
{
	mdbg_dvfs(1, "[DVFS] %s() mode: %s, resol: %s, fps: %s\n", device, __func__,
		  str_mode[param->mode], str_resol[param->resol], str_fps[param->fps]);
...
static int is_hw_dvfs_get_rear_dual_ultrawide_ultratele_scenario(
	struct is_device_ischain *device, struct is_dvfs_scenario_param *param)
{
	mdbg_dvfs(1, "[DVFS] %s() mode: %s, resol: %s, fps: %s\n", device, __func__,
		  str_mode[param->mode], str_resol[param->resol], str_fps[param->fps]);
...
static int is_hw_dvfs_get_rear_dual_wide_ultratele_scenario(
	struct is_device_ischain *device, struct is_dvfs_scenario_param *param)
{
	mdbg_dvfs(1, "[DVFS] %s() mode: %s, resol: %s, fps: %s\n", device, __func__,
		  str_mode[param->mode], str_resol[param->resol], str_fps[param->fps]);
...
static u32 _get_cam_lv(struct is_dvfs_ctrl *dvfs_ctrl)
{
	...
	for (device_id = 0; device_id < IS_SENSOR_COUNT; device_id++) {
		...
		dbg_dvfs(2, "mipi_speed(%d) lanes(%d)\n", cfg->mipi_speed, lanes);
	}
	...
	if (lv_csis > IS_DVFS_CAM_MIN_LV) {
		...
		dbg_dvfs(1, "change CSIS clk(%d) lv(%d)\n", clk_csis, lv_csis);
	}
...
static u32 _get_isp_lv(struct is_dvfs_ctrl *dvfs_ctrl)
{
	...
	for (device_id = 0; device_id < IS_SENSOR_COUNT; device_id++) {
		...
		child = idi->group[GROUP_SLOT_ISP];
		while (child) {
			ldr = &child->leader;
			w = MAX(w, ldr->input.width);
			h = MAX(h, ldr->input.height);
		...
		dbg_dvfs(2, "w(%d) h(%d) max_fps(%d) stripe_num(%d)\n", w, h, max_fps, stripe_num);
	}
	...
	if (lv_isp > IS_DVFS_ISP_MIN_LV) {
		...
		dbg_dvfs(1, "change clk(isp:%d MHz) lv(%d)\n", clk_isp, lv_isp);
	}
