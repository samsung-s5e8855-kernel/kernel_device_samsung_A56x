...
inline u32 __get_otf_cmd(u32 pidx, u32 req)
{
	u32 cmd = OTF_OUTPUT_COMMAND_DISABLE;

	if ((pidx - PARAM_MCS_OUTPUT0) == MCSC_INPUT_HF && req)
		cmd = OTF_OUTPUT_COMMAND_ENABLE;

	return cmd;
}

inline u32 __get_dma_cmd(u32 pidx)
{
	u32 cmd = DMA_OUTPUT_COMMAND_ENABLE;

	if ((pidx - PARAM_MCS_OUTPUT0) == MCSC_INPUT_HF)
	...
	int index)
{
	...
	mcs_output->otf_cmd = __get_otf_cmd(pindex, node->request);
	...
	mcs_output->dma_cmd = __get_dma_cmd(pindex);
	mcs_output->dma_bitwidth = fmt->hw_bitwidth;
	mcs_output->dma_format = fmt->hw_format;
	mcs_output->sbwc_type = hw_sbwc;
