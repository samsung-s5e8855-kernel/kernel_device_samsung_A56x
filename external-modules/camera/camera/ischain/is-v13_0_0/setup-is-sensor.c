...
static int exynos_is_csi_gate(struct device *dev, u32 instance, bool mask)
{
	int ret;
	char *clk_name;
...
int exynos_is_sensor_iclk_on(struct device *dev, u32 scenario, u32 channel)
{
	...
	for (i = 0; i < 7; i++)
		exynos_is_csi_gate(dev, i, true);

	if (channel > 6) {
		pr_err("channel is invalid(%d)\n", channel);
		return -EINVAL;
	} else {
		exynos_is_csi_gate(dev, channel, false);
	}
...
{
	exynos_is_csi_gate(dev, channel, true);
...
int exynos_is_sensor_mclk_on(struct device *dev, u32 scenario, u32 channel, u32 freq)
{
	char clk_name[30];
...
int exynos_is_sensor_mclk_off(struct device *dev, u32 scenario, u32 channel)
{
	char clk_name[30];
