...
static void *get_rta_info_addr(u32 hw_id, struct is_frame *frame, u32 index)
{
	...
	switch (hw_id) {
	case DEV_HW_MTNR0:
		addr = (void *)frame->kva_mtnr0_rta_info[index];
		break;
	case DEV_HW_MTNR1:
		addr = (void *)frame->kva_mtnr1_rta_info[index];
		break;
	case DEV_HW_MSNR:
		addr = (void *)frame->kva_msnr_rta_info[index];
		break;
	case DEV_HW_DLFE:
		addr = (void *)frame->kva_mtnr1_rta_info[index];
	...
	case DEV_HW_LME0:
		...
		break;
	case DEV_HW_MCSC0:
		addr = (void *)frame->kva_mcsc_rta_info[index];
		break;
	case DEV_HW_MLSC0:
	case DEV_HW_MLSC1:
	case DEV_HW_MLSC2:
	case DEV_HW_MLSC3:
		addr = (void *)frame->kva_mlsc_rta_info[index];
		break;
	default:
...
static bool pablo_hw_check_mlsc_group(struct is_hardware *hw, int group_id)
{
	switch (group_id) {
	case GROUP_ID_MLSC0:
	case GROUP_ID_MLSC1:
...
static const struct pablo_hw_chain_info_ops phci_ops = {
	...
	.check_mlsc_group = pablo_hw_check_mlsc_group,
};
