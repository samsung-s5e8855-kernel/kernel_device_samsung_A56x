	...
	struct is_device_ischain *device, struct is_dvfs_scenario_param *param)
{
	...
	case IS_DVFS_MODE_PHOTO:
		if (param->resol == IS_DVFS_RESOL_FULL) {
			...
			switch (param->fps) {
			case IS_DVFS_FPS_60:
				return IS_DVFS_SN_REAR_SINGLE_PHOTO_60;
			default:
	...
	case IS_DVFS_MODE_VIDEO:
		...
		case IS_DVFS_RESOL_8K:
			...
			case IS_DVFS_FPS_30:
				if (param->dlnr)
					return IS_DVFS_SN_REAR_SINGLE_VIDEO_8K30_DLNR;
				switch (param->hf) {
				case 0:
		...
		case IS_DVFS_RESOL_QHD:
			switch (param->fps) {
			case IS_DVFS_FPS_30:
				return IS_DVFS_SN_REAR_SINGLE_VIDEO_QHD30_SUPERSTEADY;
			case IS_DVFS_FPS_60:
				return IS_DVFS_SN_REAR_SINGLE_VIDEO_QHD60_SUPERSTEADY;
			default:
				goto dvfs_err;
			}
		default:
	...
	struct is_device_ischain *device, struct is_dvfs_scenario_param *param)
{
	...
	case IS_DVFS_MODE_VIDEO:
		...
		case IS_DVFS_RESOL_UHD:
			...
			case IS_DVFS_FPS_30:
				if (param->hf)
					return IS_DVFS_SN_FRONT_SINGLE_VIDEO_UHD30_HF;
				if (param->recursive)
					return IS_DVFS_SN_FRONT_SINGLE_VIDEO_UHD30_RECURSIVE;
				return IS_DVFS_SN_FRONT_SINGLE_VIDEO_UHD30;
			case IS_DVFS_FPS_60:
				if (param->hf)
					return IS_DVFS_SN_FRONT_SINGLE_VIDEO_UHD60_HF;
				return IS_DVFS_SN_FRONT_SINGLE_VIDEO_UHD60;
			case IS_DVFS_FPS_120:
				if (param->hf)
					return IS_DVFS_SN_FRONT_SINGLE_VIDEO_UHD120_HF;
	...
	struct is_device_ischain *device, struct is_dvfs_scenario_param *param)
{
	...
	case IS_DVFS_MODE_VIDEO:
		...
		case IS_DVFS_RESOL_UHD:
			...
			case IS_DVFS_FPS_30:
				if (param->hf)
					return IS_DVFS_SN_TRIPLE_VIDEO_UHD30_HF;
				return IS_DVFS_SN_TRIPLE_VIDEO_UHD30;
			case IS_DVFS_FPS_60:
				if (param->hf)
					return IS_DVFS_SN_TRIPLE_VIDEO_UHD60_HF;
