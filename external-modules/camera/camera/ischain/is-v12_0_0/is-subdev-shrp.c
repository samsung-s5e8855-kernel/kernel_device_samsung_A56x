	...
	struct camera2_node *node)
{
	...
	ret = __shrp_otf_cfg(device, group, frame, out_node, &frame->stripe_info, pmap);
	if (ret) {
		mlverr("[F%d] otf_in_cfg fail. ret %d", device, node->vid, frame->fcount, ret);
		return ret;
	}

	ret = __shrp_stripe_in_cfg(device, subdev, frame, out_node, PARAM_SHRP_STRIPE_INPUT, pmap);
	if (ret) {
	...
	for (n = 0; n < CAPTURE_NODE_MAX; n++) {
		...
		if (dma_type == 1)
			ret = __shrp_dma_in_cfg(device, subdev, frame, cap_node, pindex, pmap);
		else if (dma_type == 2)
			ret = __shrp_dma_out_cfg(device, subdev, frame, cap_node, pindex, pmap);
		if (ret) {
	...
	for (n = 0; n < CAPTURE_NODE_MAX; n++) {
		...
		if ((sframe->id >= IS_LVN_MCSC_P0) && (sframe->id <= IS_LVN_MCSC_P5))
			continue;
