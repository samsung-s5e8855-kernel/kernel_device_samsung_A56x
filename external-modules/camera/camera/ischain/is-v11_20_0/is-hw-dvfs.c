...
KUNIT_EXPORT_SYMBOL(is_hw_dvfs_get_face);
...
KUNIT_EXPORT_SYMBOL(is_hw_dvfs_get_num);
...
KUNIT_EXPORT_SYMBOL(is_hw_dvfs_get_sensor);
...
KUNIT_EXPORT_SYMBOL(is_hw_dvfs_get_mode);
...
KUNIT_EXPORT_SYMBOL(is_hw_dvfs_get_resol);
...
KUNIT_EXPORT_SYMBOL(is_hw_dvfs_get_fps);
...
KUNIT_EXPORT_SYMBOL(is_hw_dvfs_get_rear_single_scenario);
...
KUNIT_EXPORT_SYMBOL(is_hw_dvfs_get_rear_single_remosaic_scenario);
...
KUNIT_EXPORT_SYMBOL(is_hw_dvfs_get_rear_dual_wide_tele_scenario);
...
KUNIT_EXPORT_SYMBOL(is_hw_dvfs_get_rear_dual_wide_ultrawide_scenario);
...
KUNIT_EXPORT_SYMBOL(is_hw_dvfs_get_rear_dual_wide_macro_scenario);
...
KUNIT_EXPORT_SYMBOL(is_hw_dvfs_get_front_single_front_scenario);
...
KUNIT_EXPORT_SYMBOL(is_hw_dvfs_get_front_single_remosaic_scenario);
...
KUNIT_EXPORT_SYMBOL(is_hw_dvfs_get_pip_dual_scenario);
...
KUNIT_EXPORT_SYMBOL(is_hw_dvfs_get_triple_scenario);
...
static u32 _get_isp_lv(struct is_dvfs_ctrl *dvfs_ctrl)
{
	...
	for (device_id = 0; device_id < IS_SENSOR_COUNT; device_id++) {
		...
		child = idi->group[GROUP_SLOT_YUVP];
		while (child) {
			ldr = &child->leader;
			w = MAX(w, ldr->input.width);
			h = MAX(h, ldr->input.height);
...
/* LME, GDC, VRA */
static u32 _get_m2m_lv(struct is_dvfs_ctrl *dvfs_ctrl)
{
	struct is_device_sensor *ids;
	struct is_device_ischain *idi;
	struct is_sensor_cfg *cfg;
	...
	for (lv = IS_DVFS_LV_END - 1; lv >= 0; lv--) {
		clk = dvfs_ctrl->dvfs_info.qos_tb[IS_DVFS_M2M][lv][IS_DVFS_VAL_FRQ];
