	...
	int index)
{
	...
	if (IS_ENABLED(CHAIN_STRIPE_PROCESSING) && stripe_info->region_num) {
		margin = stripe_info->out.left_margin + stripe_info->out.right_margin;
		if (stripe_info->out.crop_width < margin) {
			mlverr("[F%d] crop_width(%d), left_margin(%d), right_margin(%d)\n", device,
				node->vid, frame->fcount, stripe_info->out.crop_width,
				stripe_info->out.left_margin, stripe_info->out.right_margin);

			return -EINVAL;
		}
		width = stripe_info->out.crop_width - margin;
		start_x = stripe_info->out.left_margin;
	} else {
		width = otcrop->w;
		start_x = 0;
	}
	...
	if (test_bit(IS_GROUP_OTF_OUTPUT, &device->group[GROUP_SLOT_RGBP]->state)) {
		if (test_bit(IS_GROUP_VOTF_OUTPUT, &device->group[GROUP_SLOT_RGBP]->state))
	...
	dma_output->height = otcrop->h;
	dma_output->dma_crop_offset_x = start_x;
	...
	struct camera2_node *node)
{
	...
	ret = __rgbp_dma_in_cfg(device, subdev, frame, out_node, PARAM_RGBP_DMA_INPUT, pmap);
	if (ret) {
		mlverr("[F%d] dma_in_cfg fail. ret %d", device, node->vid, frame->fcount, ret);
		return ret;
	}

	ret = __rgbp_otf_in_cfg(device, subdev, frame, out_node, PARAM_RGBP_OTF_INPUT, pmap);
	if (ret) {
	...
	ret = __rgbp_otf_out_cfg(device, subdev, frame, out_node, PARAM_RGBP_OTF_OUTPUT, pmap);
	if (ret) {
	...
	for (i = 0; i < CAPTURE_NODE_MAX; i++) {
		...
		if (dma_type == 1) {
			ret = __rgbp_dma_in_cfg(device, subdev, frame, cap_node, pindex, pmap);
		} else if (dma_type == 2) {
			ret = __rgbp_dma_out_cfg(device, subdev, frame, cap_node,
				&frame->stripe_info, pindex, pmap, i);
		} else {
			if (IS_ENABLED(IRTA_CALL) && cap_node->vid == IS_LVN_RGBP_RTA_INFO &&
				!cap_node->request) {
