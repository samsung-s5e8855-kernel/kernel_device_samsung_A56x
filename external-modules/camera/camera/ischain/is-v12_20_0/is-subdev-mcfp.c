	...
	struct camera2_node *node)
{
	...
	ret = __mcfp_dma_in_cfg(device, subdev, frame, out_node, PARAM_MCFP_DMA_INPUT, pmap);
	if (ret) {
		mlverr("[F%d] dma_in_cfg fail. ret %d", device, node->vid, frame->fcount, ret);
		return ret;
	}

	ret = __mcfp_otf_in_cfg(device, subdev, frame, out_node, PARAM_MCFP_OTF_INPUT, pmap);
	if (ret) {
	...
	ret = __mcfp_otf_out_cfg(device, subdev, frame, out_node, PARAM_MCFP_OTF_OUTPUT, pmap);
	if (ret) {
		mlverr("[F%d] otf_out_cfg fail. ret %d", device, node->vid, frame->fcount, ret);
		return ret;
	}

	ret = __mcfp_stripe_in_cfg(device, subdev, frame, out_node, PARAM_MCFP_STRIPE_INPUT, pmap);
	if (ret) {
	...
	for (i = 0; i < CAPTURE_NODE_MAX; i++) {
		...
		if (dma_type == 1) {
			ret = __mcfp_dma_in_cfg(device, subdev, frame, cap_node, pindex, pmap);
		} else if (dma_type == 2) {
			ret = __mcfp_dma_out_cfg(device, subdev, frame, cap_node, pindex, pmap, i);
		} else {
			if (IS_ENABLED(IRTA_CALL) && cap_node->vid == IS_LVN_MCFP_RTA_INFO &&
				!cap_node->request) {
