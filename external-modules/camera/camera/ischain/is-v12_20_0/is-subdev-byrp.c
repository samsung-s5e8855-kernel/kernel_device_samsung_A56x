	...
	int index)
{
	...
	if (test_bit(IS_GROUP_OTF_OUTPUT, &device->group[GROUP_SLOT_BYRP]->state)) {
		if (test_bit(IS_GROUP_VOTF_OUTPUT, &device->group[GROUP_SLOT_BYRP]->state))
	...
	struct camera2_node *node)
{
	...
	for (i = 0; i < CAPTURE_NODE_MAX; i++) {
		...
		if (dma_type == 1) {
			ret = __byrp_dma_in_cfg(device, subdev, frame, cap_node, pindex, pmap);
		} else if (dma_type == 2) {
			ret = __byrp_dma_out_cfg(device, subdev, frame, cap_node,
				&frame->stripe_info, pindex, pmap, i);
		} else {
			if (IS_ENABLED(IRTA_CALL) && cap_node->vid == IS_LVN_BYRP_RTA_INFO &&
				!cap_node->request) {
