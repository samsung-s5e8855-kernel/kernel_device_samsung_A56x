...
static const u32 s2mpu_address_table[SYSMMU_DMAX_S2] = {
	...
	[SYSMMU_S0_BYRP_S2] = 0x24160054,
	[SYSMMU_S0_CSIS_S2] = 0x271A0054,
...
{
	const u32 ibuf_mux_base = 0x27154000;
...
void is_hw_csi_qchannel_enable_all(bool enable)
{
	...
	phys_addr_t csis_pdp_top = 0x27150000;
	void __iomem *base;
	u32 val;
	int i;
...
static struct is_llc_way_num is_llc_way[IS_LLC_SN_END] = {
	...
	/* UHD scenario : VOTF 0MB, MTNR 1MB, ICPU 2MB */
	[IS_LLC_SN_UHD].votf = 0,
...
void is_hw_update_frame_info(struct is_group *group, struct is_frame *frame)
{
	...
	struct is_dvfs_ctrl *dvfs_ctrl = &sensor->resourcemgr->dvfs_ctrl;
	u32 vvalid, vblank, frame_rate, frame_duration;
	u32 req_vvalid, post_frame_gap, line_time, line_ratio;
	u32 freq[IS_DVFS_END];
	const u32 min_vvalid = REQ_START_2_SHOT + REQ_SHOT_2_END;
	const u32 min_frame_duration = min_vvalid + (REQ_END_2_START * DVFS_LEVEL_RATIO);

	sensor_itf->cis_itf_ops.get_sensor_frame_timing(sensor_itf, &vvalid, &vblank);
	is_dvfs_get_freq(dvfs_ctrl, freq);
	...
	/**
	 * Case 1. There is not enough vvalid time or frame duration
	 *   - post_frame_gap: Don't add more gap after frame_end
	 *   - line_time: Set the fastest line time to get shot processing time.
	 */
	if (!vvalid || frame_duration < min_frame_duration) {
		post_frame_gap = 0;
	...
	/**
	 * Case 2. vvalid is shorter than the minimum required vvalid.
	 *   - post_frame_gap: Add more gap to get shot processing time.
	 *   - line_time: Calculate it as required.
	 */
	if (vvalid < min_vvalid) {
		post_frame_gap = ZERO_IF_NEG(min_vvalid - vvalid);
		if (post_frame_gap > vblank / DVFS_LEVEL_RATIO)
			post_frame_gap = vblank / DVFS_LEVEL_RATIO;

		req_vvalid = vvalid + post_frame_gap;
		line_time = (req_vvalid > min_vvalid) ? ZERO_IF_NEG(req_vvalid - REQ_SHOT_2_END) :
							REQ_START_2_SHOT;
	} else {
		post_frame_gap = 0;
		line_time = ZERO_IF_NEG(vvalid - REQ_SHOT_2_END);
	}

	line_ratio = line_time * 100 / vvalid;
