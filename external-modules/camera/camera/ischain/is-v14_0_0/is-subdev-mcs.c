	...
	IS_DECLARE_PMAP(pmap))
{
	...
	if (test_bit(IS_GROUP_OTF_INPUT, &group->state)) {
		if (mscs_input->otf_cmd  != OTF_INPUT_COMMAND_ENABLE)
			mlvinfo("[MCS][F%d] OTF enable: %d -> %d\n", device, IS_VIDEO_MCSC,
				ldr_frame->fcount, mscs_input->otf_cmd, OTF_INPUT_COMMAND_ENABLE);
	...
	struct camera2_node *node)
{
	...
	ret = __mcsc_in_cfg(device, subdev, frame, out_node, PARAM_MCS_INPUT, pmap);
	if (ret) {
		mlverr("[F%d] mcsc_in_cfg fail. ret %d", device, node->vid, frame->fcount, ret);
		return ret;
	}
	...
	for (i = 0; i < CAPTURE_NODE_MAX; i++) {
		...
		if (dma_type == 1)
			ret = __mcsc_in_cfg(device, subdev, frame, cap_node, pindex, pmap);
		else if (dma_type == 2)
			ret = __mcsc_dma_out_cfg(device, frame, cap_node, pindex, pmap, i);
