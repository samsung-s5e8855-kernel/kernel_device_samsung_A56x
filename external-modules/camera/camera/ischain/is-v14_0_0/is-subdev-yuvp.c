...
static int __ischain_yuvp_slot(struct camera2_node *node, u32 *pindex)
{
	...
	case IS_LVN_YUVP_PCCHIST_R:
		*pindex = PARAM_YUVP_PCCHIST_R;
		ret = 1;
		break;
	case IS_LVN_YUVP_PCCHIST_W:
		*pindex = PARAM_YUVP_PCCHIST_W;
		ret = 2;
		break;
	default:
		ret = 0;
...
static int __yuvp_dma_out_cfg(struct is_device_ischain *device, struct is_subdev *leader,
	struct is_frame *frame, struct camera2_node *node, u32 pindex, IS_DECLARE_PMAP(pmap))
{
	...
	if (dma_output->sbwc_type != hw_sbwc)
		mlvinfo("[F%d][%d] WDMA sbwc_type: %d -> %d\n", device, node->vid, frame->fcount,
			pindex, dma_output->sbwc_type, hw_sbwc);
...
static int __yuvp_otf_cfg(struct is_device_ischain *device, struct is_group *group,
	struct is_frame *ldr_frame, struct camera2_node *node, struct is_stripe_info *stripe_info,
	IS_DECLARE_PMAP(pmap))
{
	...
	mdbgs_ischain(4, "%s : otcrop (w x h) = (%d x %d)\n", device, __func__, otf_in->width,
		otf_in->height);

	return 0;
}

static int __yuvp_dma_in_cfg(struct is_device_ischain *device, struct is_subdev *leader,
	struct is_frame *frame, struct camera2_node *node, u32 pindex, IS_DECLARE_PMAP(pmap))
{
	struct is_fmt *fmt;
	...
	if (dma_input->cmd) {
		...
		if (!frame->stripe_info.region_num &&
			((dma_input->width != width) || (dma_input->height != height)))
			mlvinfo("[F%d]RDMA incrop[%d, %d, %d, %d]\n", device, node->vid,
				frame->fcount, incrop->x, incrop->y, width, height);
	}
...
static int __yuvp_stripe_in_cfg(struct is_device_ischain *device, struct is_subdev *leader,
	struct is_frame *frame, struct camera2_node *node, u32 pindex, IS_DECLARE_PMAP(pmap))
{
	int ret;
...
static void __yuvp_control_cfg(struct is_device_ischain *device, struct is_group *group,
...
static int is_ischain_yuvp_tag(struct is_subdev *subdev, void *device_data, struct is_frame *frame,
	struct camera2_node *node)
{
	...
	ret = __yuvp_otf_cfg(device, group, frame, out_node, &frame->stripe_info, pmap);
	if (ret) {
		mlverr("[F%d] otf_in_cfg fail. ret %d", device, node->vid, frame->fcount, ret);
		return ret;
	}

	ret = __yuvp_dma_in_cfg(device, subdev, frame, out_node, PARAM_YUVP_DMA_INPUT, pmap);
	if (ret) {
		mlverr("[F%d] dma_in_cfg fail. ret %d", device, node->vid, frame->fcount, ret);
		return ret;
	}

	ret = __yuvp_stripe_in_cfg(device, subdev, frame, out_node, PARAM_YUVP_STRIPE_INPUT, pmap);
	if (ret) {
	...
	for (n = 0; n < CAPTURE_NODE_MAX; n++) {
		...
		if (dma_type == 1) {
			ret = __yuvp_dma_in_cfg(device, subdev, frame, cap_node, pindex, pmap);
		} else if (dma_type == 2) {
			ret = __yuvp_dma_out_cfg(device, subdev, frame, cap_node, pindex, pmap);
		} else {
			if (IS_ENABLED(IRTA_CALL) && cap_node->vid == IS_LVN_YUVP_CR &&
				!cap_node->request) {
...
static int is_ischain_yuvp_get(struct is_subdev *subdev, struct is_frame *frame,
