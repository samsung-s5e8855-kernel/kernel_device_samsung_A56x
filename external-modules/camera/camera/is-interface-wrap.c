	...
	for_each_set_bit(i, &idi->ginstance_map, IS_STREAM_COUNT) {
		...
		if (!test_bit(IS_GROUP_RNR_2ND, &group->state)) {
			ret = CALL_ADT_MSG_OPS(
				icpu_adt, send_msg_open, i, ch, rep_flag, ids->position, f_type);
			if (ret) {
...
static int is_hw_icpu_stop(struct is_device_ischain *idi)
{
	...
	for_each_set_bit(i, &idi->ginstance_map, IS_STREAM_COUNT) {
		...
		if (!test_bit(IS_GROUP_RNR_2ND, &group->state)) {
			ret_adt =
				CALL_ADT_MSG_OPS(icpu_adt, send_msg_stop, i, PABLO_STOP_IMMEDIATE);
			if (ret_adt)
				merr("failed to call send_msg_stop of icpu_adt(%lu/%d)", idi, i,
					ret_adt);
			ret |= ret_adt;
		}
	}
