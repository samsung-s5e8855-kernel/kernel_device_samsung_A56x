...
static u32 csi_hw_s_config_sbwc(u32 width, u32 __iomem *ctl_reg, u32 sbwc_type)
{
	u32 sbwc_en, h_stride;
	u32 comp_64b_align;
	...
	switch (sbwc_en) {
	case COMP:
		h_stride = is_hw_dma_get_header_stride(width, CSIS_COMP_BLOCK_WIDTH, 32);
		is_hw_set_reg(ctl_reg, &csi_dmax_regs[CSIS_DMAX_R_SBWC_HEADER_STRIDE], h_stride);
		break;
	case COMP_LOSS:
		/*
		...
		is_hw_set_field(ctl_reg, &csi_dmax_regs[CSIS_DMAX_R_LOSSY_QUALITY_CONTROL],
			&csi_dmax_fields[CSIS_DMAX_F_LOSSY_DMA_COMP_LOSSY_QUALITY_CONTROL], 0);
		break;
	default:
		sbwc_en = NONE;
...
static void csi_hw_get_dim_hwfmt(u32 pixelformat, u32 *dim, u32 *hw_format)
{
	switch (pixelformat) {
...
static int csi_hw_get_dma_fmt(
	u32 hw_bitwidth, u32 hwformat, u32 hw_format, u32 *dma_format, u32 *bitwidth)
{
	int ret = 0;
...
static u32 csi_hw_s_fmt_dma(
	u32 __iomem *vc_reg, struct is_frame_cfg *cfg, u32 vc, u32 hwformat, u32 *bitwidth)
{
	u32 dim, hw_format, dma_format;
	u32 val;
	u32 ret;

	csi_hw_get_dim_hwfmt(cfg->format->pixelformat, &dim, &hw_format);

	ret = csi_hw_get_dma_fmt(
		cfg->format->hw_bitwidth, hwformat, hw_format, &dma_format, bitwidth);
	if (ret) {
...
int csi_hw_s_config_dma(u32 __iomem *ctl_reg, u32 __iomem *vc_reg, u32 vc, struct is_frame_cfg *cfg,
	u32 hwformat, u32 *stride)
{
	int ret = 0;
	...
	ret = csi_hw_s_fmt_dma(vc_reg, cfg, vc, hwformat, &bitwidth);
	if (ret)
		goto p_err;

	/* VC0 only has SBWC configuration */
	if (vc == CSI_VIRTUAL_CH_0)
		sbwc_type = csi_hw_s_config_sbwc(cfg->width, ctl_reg, sbwc_type);
...
void csi_hw_s_dma_addr_setb(u32 __iomem *ctl_reg, u32 __iomem *vc_reg, dma_addr_t addr)
{
	...
	/* LSB(4bits) is automatically padded to 0 */
	is_hw_set_reg(vc_reg, &csi_dmax_chx_regs[CSIS_DMAX_CHX_R_ADDR_SET_B], val);
}
...
void csi_hw_s_dma_header_addr_setb(u32 __iomem *ctl_reg, dma_addr_t dva)
{
	...
	/* LSB(4bits) is automatically padded to 0 */
	is_hw_set_reg(ctl_reg, &csi_dmax_regs[CSIS_DMAX_R_SBWC_HEADER_SET_B], val);
}
...
int csi_hw_enable(u32 __iomem *base_reg, u32 use_cphy)
{
	...
	/* HW Trigger for NFI */
	is_hw_set_field(base_reg, &csi_regs[CSIS_R_CSIS_CMN_CTRL],
		&csi_fields[CSIS_F_UPDATE_SHADOW_SEL], 1);
...
#define CSIS_DMA_CMN_DEBUG_DMA_OFFSET                                                              \
	(CSIS_DMA_CMN_R_CSIS_CMN_DEBUG_INFO_CSIS1_CH0_VCNT -                                       \
	 CSIS_DMA_CMN_R_CSIS_CMN_DEBUG_INFO_CSIS0_CH0_VCNT)
#define CSIS_DMA_AXI_MASTER_NUM 12
void csi_hw_dma_cmn_dump_dbg_state(u32 __iomem *regs)
{
	bool debug_en;
	u32 dma_id, vc, offset, val;
	u32 hcnt, vcnt, fcnt;
	u32 wvalid_cnt, nwready_cnt, wdata_cnt, bresp_cnt, awvalid_cnt, nawready_cnt;

	debug_en = is_hw_get_field(regs, &csi_dma_cmn_regs[CSIS_DMA_CMN_R_CSIS_CMN_DEBUG_EN],
				   &csi_dma_cmn_fields[CSIS_DMA_CMN_F_DMA_DEBUG_ENABLE]);
	if (!debug_en)
		return;

	info("[DUMP] CSIS_DMA_CMN DBG ==============\n");

	for (dma_id = 0; dma_id < CSIS_MAX_NUM_DMA_HW; dma_id++) {
		offset = CSIS_DMA_CMN_DEBUG_DMA_OFFSET * dma_id;

		/* Horizontal, Vertical counter & Frame counter */
		for (vc = 0; vc < DMA_VIRTUAL_CH_MAX; vc++) {
			fcnt = is_hw_get_reg(
				regs,
				&csi_dma_cmn_regs[CSIS_DMA_CMN_R_CSIS_CMN_DEBUG_INFO_CSIS0_CH0_VCNT +
						  offset + vc]);
			if (!fcnt)
				continue;
			...
			val = is_hw_get_reg(
				regs,
				&csi_dma_cmn_regs[CSIS_DMA_CMN_R_CSIS_CMN_DEBUG_INFO_CSIS0_CH0_HDCNT +
						  offset + vc]);
			hcnt = (val & GENMASK(15, 0)) * 8; /* 8ppc */
			vcnt = (val & GENMASK(31, 16)) >> 16;

			info("[DUMP][DMA%d][VC%d] %dx%d(%d)\n", dma_id, vc, hcnt, vcnt, fcnt);
		}
	}

	val = is_hw_get_reg(regs,
			    &csi_dma_cmn_regs[CSIS_DMA_CMN_R_CSIS_CMN_DEBUG_INFO_AXI_DBG_CNT]);
	info("[DUMP] DBG_CNT 0x%08x\n", val);

	for (dma_id = 0; dma_id < CSIS_DMA_AXI_MASTER_NUM; dma_id++) {
		val = is_hw_get_reg(
			regs, &csi_dma_cmn_regs[CSIS_DMA_CMN_R_CSIS_CMN_DEBUG_INFO_AXI_BLK_CNT_M0 +
						dma_id]);
		nwready_cnt = (val & GENMASK(15, 0));
		wvalid_cnt = (val & GENMASK(31, 16)) >> 16;
		val = is_hw_get_reg(
			regs, &csi_dma_cmn_regs[CSIS_DMA_CMN_R_CSIS_CMN_DEBUG_INFO_AXI_MO_CNT_M0 +
						dma_id]);
		bresp_cnt = (val & GENMASK(15, 0));
		wdata_cnt = (val & GENMASK(31, 16)) >> 16;

		val = is_hw_get_reg(
			regs,
			&csi_dma_cmn_regs[CSIS_DMA_CMN_R_CSIS_CMN_DEBUG_INFO_AXI_AWBLK_CNT_M0 +
					  dma_id]);
		nawready_cnt = (val & GENMASK(15, 0));
		awvalid_cnt = (val & GENMASK(31, 16)) >> 16;

		info("[DUMP][M%d] WVALID 0x%x/0x%x WADDR 0x%x/0x%x AWVALID 0x%x/0x%x\n", dma_id,
		     wvalid_cnt, nwready_cnt, wdata_cnt, bresp_cnt, awvalid_cnt, nawready_cnt);
	}
}
		...
		u32 *width, u32 *height)
{
	...
	factor_x = factor_y = sensor_cfg->hw_bns ? sensor_cfg->hw_bns : HW_BNS_1_0;
	out_width = in_width * HW_BNS_1_0 / factor_x;
	out_height = in_height * HW_BNS_1_0 / factor_y;
...
static void csi_hw_set_emb_lc(u32 __iomem *reg, u32 lc)
{
	is_hw_set_reg(reg, &emb_lc_regs[EMB_LC_R_EMB_LC_CTRL], lc);
}

static struct csi_hw_ops csi_hw_60_ops = {
	.set_emb_lc = csi_hw_set_emb_lc,
};

struct csi_hw_ops *csi_hw_get_ops(void)
{
	return &csi_hw_60_ops;
}
...
static struct pablo_kunit_hw_csi_func pablo_kunit_hw_csi = {
	...
	.csi_hw_s_config_sbwc = csi_hw_s_config_sbwc,
	.csi_hw_get_dim_hwfmt = csi_hw_get_dim_hwfmt,
	.csi_hw_get_dma_fmt = csi_hw_get_dma_fmt,
	.csi_hw_s_fmt_dma = csi_hw_s_fmt_dma,
	.csi_hw_s_config_dma = csi_hw_s_config_dma,
};
