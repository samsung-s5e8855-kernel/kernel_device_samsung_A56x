...
static void is_resourcemgr_free_mem(struct is_resourcemgr *resourcemgr)
{
	struct is_minfo *minfo = resourcemgr->minfo;
	int i;

	if (DUMMY_DMA_SIZE) {
		CALL_VOID_BUFOP(minfo->pb_dummy, free, minfo->pb_dummy);
		minfo->pb_dummy = NULL;
		info("[RSC]memory_free(DUMMY_DMA_SIZE");
	}
	...
	info("[RSC]memory_free(ORBMCH_DMA_SIZE");
#endif
#if defined(ENABLE_TNR)
	CALL_VOID_BUFOP(minfo->pb_tnr, free, minfo->pb_tnr);
	minfo->pb_tnr = NULL;
	info("[RSC]memory_free(TNR DMA");
#endif
	CALL_VOID_BUFOP(minfo->pb_taaisp, free, minfo->pb_taaisp);
	minfo->pb_taaisp = NULL;
	info("[RSC]memory_free(TAAISP_DMA");
#endif

	if (IS_ENABLED(CONFIG_PABLO_HW_HELPER_V1)) {
		CALL_VOID_BUFOP(minfo->pb_sfr_dump_value, free, minfo->pb_sfr_dump_value);
		minfo->pb_sfr_dump_value = NULL;
		info("[RSC]memory_free(SFR_DUMP_VALUE_SIZE");

		CALL_VOID_BUFOP(minfo->pb_sfr_dump_addr, free, minfo->pb_sfr_dump_addr);
		minfo->pb_sfr_dump_addr = NULL;
		info("[RSC]memory_free(SFR_DUMP_ADDR_SIZE");
	}

	CALL_VOID_BUFOP(minfo->pb_pregion, free, minfo->pb_pregion);
	minfo->pb_pregion = NULL;
	info("[RSC]memory_free(PARAM_REGION_SIZE");

	if (!IS_ENABLED(CLOG_RESERVED_MEM) && IS_ENABLED(CONFIG_PABLO_HW_HELPER_V1)) {
		CALL_VOID_BUFOP(minfo->pb_debug, free, minfo->pb_debug);
		minfo->pb_debug = NULL;
		info("[RSC]memory_free(DEBUG_REGION_SIZE");
	}

	if (IS_ENABLED(CONFIG_PABLO_HW_HELPER_V1)) {
		for (i = 0; i < SENSOR_POSITION_MAX; i++) {
			CALL_VOID_BUFOP(minfo->pb_cal[i], free, minfo->pb_cal[i]);
			minfo->pb_cal[i] = NULL;
			info("[RSC]memory_free(TOTAL_CAL_DATA_SIZE");
		}
	}
...
static void pablo_vunmap(unsigned long addr)
{
	rscmgr_sys_ops.vunmap((const void *)addr);
}
...
void pablo_rscmgr_deinit_log_rmem(struct is_resourcemgr *resourcemgr)
{
	pablo_vunmap(resourcemgr->minfo->kvaddr_debug);
}
EXPORT_SYMBOL(pablo_rscmgr_deinit_log_rmem);
...
void pablo_resourcemgr_deinit_mem(struct is_resourcemgr *resourcemgr)
{
	is_resourcemgr_free_mem(resourcemgr);
}
EXPORT_SYMBOL(pablo_resourcemgr_deinit_mem);
