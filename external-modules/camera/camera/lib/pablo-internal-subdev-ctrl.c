...
static int pis_alloc(struct pablo_internal_subdev *sd, size_t buf_size)
{
	...
	ret = frame_manager_open(&sd->internal_framemgr, sd->num_buffers, false);
	if (ret) {
	...
	for (i = 0; i < sd->num_buffers; i++) {
		...
		frame->size[0] = sd->size[0];
		set_bit(FRAME_MEM_INIT, &frame->mem_state);

		frame->dvaddr_buffer[0] = CALL_BUFOP(sd->pb[i], dvaddr, sd->pb[i]);

		if (icpu_dev && test_bit(PABLO_SUBDEV_ICPU_ATTACH, &sd->state)) {
			ret = CALL_BUFOP(sd->pb[i], attach_ext, sd->pb[i], icpu_dev);
			if (ret) {
				merr("failed to attach_ext(%d)", sd, ret);
				goto err_attach_ext_pb_subdev;
			}
		}
...
err_allocate_pb_subdev:
	while (i-- > 0) {
		if (sd->pb[i]) {
			if (icpu_dev && test_bit(PABLO_SUBDEV_ICPU_ATTACH, &sd->state))
				CALL_VOID_BUFOP(sd->pb[i], detach_ext, sd->pb[i]);
...
static int pablo_internal_subdev_free(struct pablo_internal_subdev *sd)
{
	...
	msinfo(" %s (size: %dx%d, bpp: %d, num_batch: %d, num_buffers: %d)\n",
		sd, sd, __func__, sd->width, sd->height, sd->bits_per_pixel,
		sd->num_batch, sd->num_buffers);
...
int pablo_internal_subdev_probe(struct pablo_internal_subdev *sd, u32 instance,
				struct is_mem *mem, const char *name)
{
	...
	sd->secure = 0;
	sd->sbwc_type = 0;
	sd->lossy_byte32num = 0;
	memset(sd->size, 0, sizeof(sd->size));

	frame_manager_probe(&sd->internal_framemgr, name);

	return 0;
}
EXPORT_SYMBOL_GPL(pablo_internal_subdev_probe);

void pablo_i_subdev_set_icpu_dev(struct device *dev)
{
	icpu_dev = dev;
}
EXPORT_SYMBOL_GPL(pablo_i_subdev_set_icpu_dev);
