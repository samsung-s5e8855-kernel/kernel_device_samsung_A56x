...
void is_fpsimd_remove(void)
{
	free_percpu(is_percpu_fpsimd_state);
#if !IS_ENABLED(CONFIG_ARCH_VELOCE_HYCON)
	unregister_trace_android_vh_is_fpsimd_save(is_fpsimd_save, NULL);
#endif
}
EXPORT_SYMBOL_GPL(is_fpsimd_remove);
