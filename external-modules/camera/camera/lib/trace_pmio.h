...
DECLARE_EVENT_CLASS(pmio_reg,
	...
	TP_printk("%s reg=0x%x val=0x%x", __get_str(name),
...
DECLARE_EVENT_CLASS(pmio_block,
	...
	TP_printk("%s reg=0x%x count=%d", __get_str(name),
		  (unsigned int)__entry->reg,
		  (int)__entry->count)
);
...
TRACE_EVENT(pmio_cache_fsync_region,
	...
	TP_STRUCT__entry(
		__string(	name,		pmio_name(pmio)	)
		__string(	type,		type		)
		__string(fmt,		pmio_formatter_name(fmt))
		__field(	unsigned int,	from		)
		__field(	unsigned int,	to		)
		__string(	status,		status		)
	),
	TP_fast_assign(
		__assign_str(name, pmio_name(pmio));
		__assign_str(type, type);
		__assign_str(fmt, pmio_formatter_name(fmt));
		__entry->from = from;
		__entry->to = to;
		__assign_str(status, status);
	),
	TP_printk("%s %s %s %u-%u %s", __get_str(name), __get_str(type),
		__get_str(fmt), (unsigned int)__entry->from,
...
#define TRACE_INCLUDE_PATH PABLO_TRACE_INCLUDE_PATH
