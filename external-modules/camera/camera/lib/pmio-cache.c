...
#ifdef PKV_KVMEMDUP_UNSUPPORTED
...
static int pmio_cache_hw_init(struct pablo_mmio *pmio)
{
	...
	if (!pmio->reg_defaults_raw && (!pmio->rd_table || !pmio->rd_table->n_no_ranges)) {
		bool bypass = pmio->cache_bypass;
		pr_debug("(%s) no cache defaults, reading back from HW\n", pmio->name);
...
int pmio_cache_init(struct pablo_mmio *pmio, const struct pmio_config *config)
{
	...
	if (config->volatile_table && config->volatile_table->yes_ranges &&
			config->volatile_table->n_yes_ranges == 0) {
		pr_err("volatile table has no range!\n");
		return -EINVAL;
	}
	...
	if (!pmio->max_register) {
		if (!pmio->num_reg_defaults_raw) {
			pr_err("no max_register hint for cache!\n");
			ret = -EINVAL;
			goto err_no_hint;
		}
	...
	if (pmio->cache_ops->init) {
		pr_debug("(%s) initializing %s cache\n", pmio->name, pmio->cache_ops->name);
			   ...
			   unsigned int max)
{
	...
	if (min == 0 && max == pmio->max_register)
		pr_debug("(%s) syncing %s cache\n", pmio->name, pmio->cache_ops->name);
	else
		pr_debug("(%s) syncing %s cache from 0x%x-0x%x\n",
				pmio->name, pmio->cache_ops->name, min, max);
			    ...
			    unsigned int min, unsigned int max)
{
	...
	bypass = pmio->cache_bypass;
	if (min == 0 && max == pmio->max_register)
		pr_debug("(%s) syncing %s cache to formatter: %d\n",
				pmio->name, pmio->cache_ops->name, fmt);
	else
		pr_debug("(%s) syncing %s cache from 0x%x-0x%x to formatter: %d\n",
				pmio->name, pmio->cache_ops->name, min, max, fmt);
...
{
	return pmio->cache_only;
}
EXPORT_SYMBOL(pmio_cache_get_only);
					   ...
					   unsigned int base, unsigned int cur)
{
	...
	pr_debug("(%s) writing %zu bytes for %zu registers from 0x%x-0x%x\n",
				pmio->name, len, count, base, cur - PMIO_REG_STRIDE);
...
static void fill_c_loader_header(struct pablo_mmio *pmio, struct c_loader_buffer *clb,
				 u8 format, unsigned int base, u32 type_map)
{
	struct c_loader_header *clh = &clb->clh[clb->num_of_headers];
	const unsigned int payload_offset = clb->num_of_headers * SIZE_OF_CLD_PAYLOAD;

	clh->mode.format = format;
	clh->mode.enable = 1;
	clh->frame_id = clb->frame_id;
	clh->payload_dva = (clb->payload_dva + payload_offset) >> pmio->dma_addr_shift;
	/**
	 * In the case of pair mode, this field(cr_addr) is not used.
	 * Therefore, it can be handled in the same way as other modes.
	 */
	clh->cr_addr = pmio->ignore_phys_base ? base : pmio->phys_base + base;
	clh->type_map = type_map;
}

static inline void test_and_inc_c_loader_header(struct c_loader_buffer *clb,
...
static int inc_addr_formatter_flush(struct pablo_mmio *pmio, const void **data,
				    void *buf, unsigned int base, unsigned int cur)
{
	size_t count;
	...
	while (count >= NUM_OF_CLD_VALUES) {
		...
		fill_c_loader_header(pmio, clb, C_LOADER_FMT_INC_ADDR, base,
				     type_map_lookup_tbl[NUM_OF_CLD_VALUES]);
...
/* (0x1 << 2 | 0x1):  2-bits for address and its value each */
#define TYPE_MAP_VAL_FOR_PAIR	(0x5)
				...
				void *buf, unsigned int base, unsigned int cur)
{
	...
	while (count > 0) {
		if (clb->num_of_pairs == 0)
			fill_c_loader_header(pmio, clb, C_LOADER_FMT_PAIR,
					     base, TYPE_MAP_VAL_FOR_PAIR);
		else
			clh->type_map = (clh->type_map << 4)
						| TYPE_MAP_VAL_FOR_PAIR;

		clp->pairs[clb->num_of_pairs].addr =
			pmio->ignore_phys_base ? base : pmio->phys_base + base;
				      ...
				      unsigned int start, unsigned int end)
{
	...
	for (i = start; i < end; i++) {
		...
		if (!pmio_writeable(pmio, regtmp)) {
			ret = formatter_flush(pmio, &data, buf, base, regtmp);
			if (ret)
				return ret;
			continue;
		}
	...
	return formatter_flush(pmio, &data, buf, base, regtmp + PMIO_REG_STRIDE);
}
...
static int rpt_addr_formatter_flush(struct pablo_mmio *pmio, const void **data,
				    void *buf, unsigned int base, unsigned int cur)
{
	size_t count = (cur - base) / PMIO_REG_STRIDE;
	const size_t payload_bytes = NUM_OF_CLD_VALUES * PMIO_REG_STRIDE;
	struct c_loader_buffer *clb = (struct c_loader_buffer *)buf;
	struct c_loader_header *clh = &clb->clh[clb->num_of_headers];
	struct c_loader_payload *clp = &clb->clp[clb->num_of_headers];
	...
	pr_debug("(%s) writing %zu bytes for noinc register: 0x%x\n",
			pmio->name, count * PMIO_REG_STRIDE, base);

	test_and_inc_c_loader_header(clb, &clh, &clp, (clb->num_of_pairs > 0));

	while (count >= NUM_OF_CLD_VALUES) {
		memcpy(clp, val, payload_bytes);

		fill_c_loader_header(pmio, clb, C_LOADER_FMT_RPT_ADDR, base,
				     type_map_lookup_tbl[NUM_OF_CLD_VALUES]);
...
static int rpt_addr_formatter_flush_ext_hdr(struct pablo_mmio *pmio,
					    const void **data,
					    void *buf,  unsigned int base,
					    unsigned int cur)
{
	size_t count = (cur - base) / PMIO_REG_STRIDE;
	const size_t payload_bytes = NUM_OF_CLD_VALUES * PMIO_REG_STRIDE;
	...
	/* fill as a normal header */
	fill_c_loader_header(pmio, clb, C_LOADER_FMT_RPT_ADDR, base,
			type_map_lookup_tbl[NUM_OF_CLD_VALUES]);

	/* chagne it to an extended header */
	if ((count / NUM_OF_CLD_VALUES) > 1) {
		clh->mode.hdr_type = C_LOADER_HDR_EXTEDED;
		clh->mode.num_of_ext_hdr = (count / NUM_OF_CLD_VALUES) - 2;
	}
...
{
	if (pmio->use_ext_hdr)
		return rpt_addr_formatter_flush_ext_hdr(pmio, (const void **)&range->cache,
							buf, range->min,
							range->min + range->cache_size);

	return rpt_addr_formatter_flush(pmio, (const void **)&range->cache, buf, range->min,
					range->min + range->cache_size);
}

static int pair_formatter_flush_ext(struct pablo_mmio *pmio, const void **data,
				    void *buf, unsigned int base, unsigned int cur)
{
	size_t count = (cur - base) / PMIO_REG_STRIDE;
	...
	struct c_loader_header *clh = &clb->clh[clb->num_of_headers];
	struct c_loader_payload *clp = &clb->clp[clb->num_of_headers];
	/* each item consists of {address, value} */
	const void *items = *data;
	int i;

	pr_debug("(%s) writing %zu bytes for external register\n",
			pmio->name, count * PMIO_REG_STRIDE);

	test_and_inc_c_loader_header(clb, &clh, &clp, (clb->num_of_pairs > 0));

	while (count >= NUM_OF_CLD_PAIRS) {
		memcpy(clp, items, payload_bytes);
		if (!pmio->ignore_phys_base)
			for (i = 0; i < NUM_OF_CLD_PAIRS; i++)
				clp->pairs[i].addr += pmio->phys_base;

		fill_c_loader_header(pmio, clb, C_LOADER_FMT_PAIR, base,
				     type_map_lookup_tbl[NUM_OF_CLD_PAIRS * 2]);

		items += payload_bytes;
		count -= NUM_OF_CLD_PAIRS;
	...
	if (count) {
		memcpy(clp, items, count * PMIO_REG_STRIDE * 2);
		if (!pmio->ignore_phys_base)
			for (i = 0; i < count; i++)
				clp->pairs[i].addr += pmio->phys_base;

		fill_c_loader_header(pmio, clb, C_LOADER_FMT_PAIR, base,
				     type_map_lookup_tbl[count * 2]);
...
int pmio_cache_fsync_ext(struct pablo_mmio *pmio, void *buf, const void *items,
			 size_t block_count)
{
	return pair_formatter_flush_ext(pmio, &items, buf, 0, block_count * PMIO_REG_STRIDE);
}
EXPORT_SYMBOL(pmio_cache_fsync_ext);
