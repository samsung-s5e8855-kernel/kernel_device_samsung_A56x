...
static void is_hw_dvfs_deinit(struct is_dvfs_ctrl *dvfs_ctrl)
{
	kvfree(dvfs_ctrl->scenario_idx);
	dvfs_ctrl->scenario_idx = NULL;
}
...
void pablo_dvfs_deinit(struct is_dvfs_ctrl *dvfs_ctrl)
{
	is_hw_dvfs_deinit(dvfs_ctrl);

	kfree(dvfs_ctrl->iter_mode);
	dvfs_ctrl->iter_mode = NULL;

	kvfree(dvfs_ctrl->dynamic_ctrl);
	dvfs_ctrl->dynamic_ctrl = NULL;

	kvfree(dvfs_ctrl->static_ctrl);
	dvfs_ctrl->static_ctrl = NULL;
}
EXPORT_SYMBOL_GPL(pablo_dvfs_deinit);
...
static int is_set_dvfs_by_path(struct is_dvfs_ctrl *dvfs_ctrl, int scenario_id, u32 otf_path)
{
	...
	/* General PM QOS */
	for (dvfs_t = 0; dvfs_t < IS_DVFS_END; dvfs_t++) {
		if (!test_bit(otf_path, &dvfs_ctrl->dvfs_info.qos_otf[dvfs_t]))
			continue;
		...
		if (!qos_tb[dvfs_t])
			...
			lv = qos_tb[dvfs_t][IS_DVFS_VAL_LEV];

		if (test_bit(IS_DVFS_PATH_OTF, &dvfs_ctrl->dvfs_info.qos_otf[dvfs_t]) &&
		...
		if (dvfs_ctrl->iter_mode->iter_svhist && otf_path == IS_DVFS_PATH_M2M)
...
{
	return is_set_dvfs_by_path(dvfs_ctrl, scenario_id, IS_DVFS_PATH_OTF);
}
...
int is_set_dvfs_m2m(struct is_dvfs_ctrl *dvfs_ctrl, int scenario_id, bool reprocessing_mode)
{
	...
	is_set_dvfs_by_path(dvfs_ctrl, scenario_id, IS_DVFS_PATH_M2M);
...
void is_set_dvfs_fast_launch(struct is_dvfs_ctrl *dvfs_ctrl)
{
	int scn = dvfs_ctrl->static_ctrl->cur_scenario_id;

	if (dvfs_ctrl->dynamic_ctrl->cur_frame_tick > 0)
		return;
...
EXPORT_SYMBOL_GPL(is_set_dvfs_fast_launch);

int is_set_static_dvfs(struct is_dvfs_ctrl *dvfs_ctrl, int scenario_id, bool stream_on)
{
	int scn;
	...
	if (stream_on)
		scn = pablo_set_static_dvfs(dvfs_ctrl, "", scenario_id, IS_DVFS_PATH_M2M, false);
	else
