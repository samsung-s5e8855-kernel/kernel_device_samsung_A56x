...
KUNIT_EXPORT_SYMBOL(pmio_cached);
...
static int pmio_range_init(struct pablo_mmio *pmio, const struct pmio_config *config)
{
	const struct pmio_range_cfg *range;
	...
	for (i = 0; i < config->num_ranges; i++) {
		...
		if (config->use_ext_hdr &&
		    (new->cache_size > (PMIO_MAX_NUM_EXT_HDR * SIZE_OF_CLD_PAYLOAD))) {
			pr_err("exceed the maximum number of extended header %d\n", i);
			ret = -EINVAL;
			goto err_range_size;
		}
...
static int pmio_corex_init(struct pablo_mmio *pmio, const struct pmio_config *config)
{
	int i;
...
{
	struct pablo_mmio *pmio __free(kfree) = kzalloc(sizeof(*pmio), GFP_KERNEL);
	int ret;
	...
	ret = pmio_corex_init(pmio, config);
	if (ret)
		goto err_init_corex;
	...
	pmio->ignore_phys_base = config->ignore_phys_base;
	pmio->use_ext_hdr = config->use_ext_hdr;
	...
	ret = pmio_range_init(pmio, config);
	if (ret)
		goto err_init_range;
 ...
 * @num_fields:	[in]	Number of register fields.
 *
 * Context: Process context. May sleep due to 'GFP_KERNEL' flag.
 *
...
/**
 * pmio_clb_hex_dump - print a text hex dump for a C_loader buffer
 *
 * @title:	[in]	String to identify each dump
 * @buf:	[in]	C_loader buffer to dump
 * @limit:	[in]	Number to limit the number of header of dump
 *
 * Context: Any context.
 */
void pmio_clb_hex_dump(const char *title, const void *buf, unsigned int limit)
{
	...
	limit = (limit > 0) ? min(clb->num_of_headers, limit) : clb->num_of_headers;

	pr_info("-------C-loader-buffer-dump-------\n");
	pr_info("%s [frame_id: %d]\n", title, clb->frame_id);

	if (limit > 0 && limit != clb->num_of_headers)
		pr_info("number of headers: %d (limit: %d)\n", clb->num_of_headers, limit);
	else
		pr_info("number of headers: %d\n", clb->num_of_headers);
	pr_info("number of values : %d\n", clb->num_of_values);
	pr_info("number of pairs  : %d\n", clb->num_of_pairs);

	ellipsis = (limit > 2) && (clb->num_of_headers > limit);
	if (ellipsis)
		limit--;

	pr_info("header  dva: %pad\n", &clb->header_dva);
	print_hex_dump(KERN_INFO, "header  ", DUMP_PREFIX_OFFSET, 16, 4,
		       clb->clh, limit * sizeof(struct c_loader_header), false);
	if (ellipsis) {
		pr_info(" . . . . . . . . . . . . . . . . .\n");
		print_hex_dump(KERN_INFO, "header  ", DUMP_PREFIX_OFFSET, 16, 4,
			       clb->clh + (clb->num_of_headers - 1),
			       sizeof(struct c_loader_header), false);
	}

	pr_info("payload dva: %pad\n", &clb->payload_dva);
	print_hex_dump(KERN_INFO, "payload ", DUMP_PREFIX_OFFSET, 16, 4,
		       clb->clp, limit * sizeof(struct c_loader_payload), false);
	if (ellipsis) {
		pr_info(" . . . . . . . . . . . . . . . . .\n");
		print_hex_dump(KERN_INFO, "payload ", DUMP_PREFIX_OFFSET, 16, 4,
			       clb->clp + (clb->num_of_headers - 1),
			       sizeof(struct c_loader_payload), false);
	}
	pr_info("----------------------------------\n");
}
EXPORT_SYMBOL(pmio_clb_hex_dump);
