...
#define CALL_DMABUF_CON_OPS(buf, op, args...)                                                      \
	((buf)->dbuf_con_ops->op ? (buf)->dbuf_con_ops->op(args) : 0)
...
static inline ulong is_vb2_dma_sg_plane_kvaddr(struct is_vb2_buf *vbuf, u32 plane)

{
	return (ulong)vb2_plane_vaddr(&vbuf->vb.vb2_buf, plane);
}

static inline dma_addr_t is_vb2_dma_sg_plane_dvaddr(struct is_vb2_buf *vbuf, u32 plane)
...
static inline ulong is_vb2_dma_sg_plane_kmap(struct is_vb2_buf *vbuf, u32 plane, u32 buf_idx)
{
	...
	if (IS_ENABLED(DMABUF_CONTAINER) && vbuf->num_merged_dbufs) {
		...
		if (plane >= num_i_planes)
			adj_plane =
				(vbuf->num_merged_dbufs * num_i_planes) + (plane - num_i_planes);
		else
...
static inline void is_vb2_dma_sg_plane_kunmap(struct is_vb2_buf *vbuf, u32 plane, u32 buf_idx)
{
	struct vb2_buffer *vb = &vbuf->vb.vb2_buf;
	unsigned int num_i_planes = vb->num_planes - NUM_OF_META_PLANE;
	struct dma_buf *dbuf;
...
static inline dma_addr_t is_vb2_dma_sg_plane_dmap(struct is_vb2_buf *vbuf, u32 plane, u32 buf_idx,
						  struct device *dev_ext)
{
	struct vb2_buffer *vb = &vbuf->vb.vb2_buf;
	unsigned int num_i_planes = vb->num_planes - NUM_OF_META_PLANE;
	struct dma_buf *dbuf;
	...
	vbuf->sgt_ext[adj_plane] =
		CALL_DMABUF_OPS(vbuf, map_attachment, vbuf->atch_ext[adj_plane], DMA_BIDIRECTIONAL);
...
err_ion_map_io:
	CALL_DMABUF_OPS(vbuf, unmap_attachment, vbuf->atch_ext[adj_plane], vbuf->sgt_ext[adj_plane],
			DMA_BIDIRECTIONAL);
...
static inline void is_vb2_dma_sg_plane_dunmap(struct is_vb2_buf *vbuf, u32 plane, u32 buf_idx)
{
	struct vb2_buffer *vb = &vbuf->vb.vb2_buf;
	unsigned int num_i_planes = vb->num_planes - NUM_OF_META_PLANE;
	struct dma_buf *dbuf;
	...
	if (vbuf->dva_ext[adj_plane]) {
		...
		CALL_DMABUF_OPS(vbuf, unmap_attachment, vbuf->atch_ext[adj_plane],
				vbuf->sgt_ext[adj_plane], DMA_BIDIRECTIONAL);
		vbuf->sgt_ext[adj_plane] = NULL;
		CALL_DMABUF_OPS(vbuf, detach, dbuf, vbuf->atch_ext[adj_plane]);
		vbuf->atch_ext[adj_plane] = NULL;
...
static long is_vb2_dma_sg_remap_attr(struct is_vb2_buf *vbuf, int attr)
{
	...
	for (plane = 0; plane < vb->num_planes; ++plane) {
		...
		vbuf->sgt[plane] =
			CALL_DMABUF_OPS(vbuf, map_attachment, buf->db_attach, buf->dma_dir);
...
err_get_sgt:
	while (plane-- > 0) {
		...
		CALL_DMABUF_OPS(vbuf, unmap_attachment, buf->db_attach, vbuf->sgt[plane],
				buf->dma_dir);
		vbuf->dva[plane] = 0;
	}
...
static long is_dbufcon_prepare(struct is_vb2_buf *vbuf, struct device *dev)
{
	...
	for (p = 0; p < num_i_planes; p++) {
		...
		if (IS_ERR_OR_NULL(dbufcon)) {
			err("failed to get dmabuf of fd: %d, plane: %d", vb->planes[p].m.fd, p);
			ret = -EINVAL;
		...
		if (((count * num_i_planes) + NUM_OF_META_PLANE) > IS_MAX_PLANES) {
			err("out of range plane. require: %d > supported: %d",
			    (count * num_i_planes) + NUM_OF_META_PLANE, IS_MAX_PLANES);
			ret = -EINVAL;
		...
		for (b = 0, i = p; b < count; b++, i = (b * num_i_planes) + p) {
			...
			vbuf->sgt[i] = CALL_DMABUF_OPS(vbuf, map_attachment, vbuf->atch[i],
						       DMA_BIDIRECTIONAL);
...
err_get_dbuf:
	while (b-- > 0) {
		i = (b * num_i_planes) + p;
		CALL_DMABUF_OPS(vbuf, unmap_attachment, vbuf->atch[i], vbuf->sgt[i],
				DMA_BIDIRECTIONAL);
		CALL_DMABUF_OPS(vbuf, detach, vbuf->dbuf[i], vbuf->atch[i]);
		CALL_DMABUF_OPS(vbuf, put, vbuf->dbuf[i]);
		vbuf->sgt[i] = NULL;
...
static void is_dbufcon_finish(struct is_vb2_buf *vbuf)
{
	int i;
	struct vb2_buffer *vb = &vbuf->vb.vb2_buf;
	unsigned int num_i_planes = vbuf->num_merged_dbufs * (vb->num_planes - NUM_OF_META_PLANE);
...
static long is_dbufcon_map(struct is_vb2_buf *vbuf)
{
	...
	for (i = 0; i < num_i_planes; i++) {
		vbuf->sgt[i] =
			CALL_DMABUF_OPS(vbuf, map_attachment, vbuf->atch[i], DMA_BIDIRECTIONAL);
		if (IS_ERR(vbuf->sgt[i])) {
			ret = PTR_ERR(vbuf->atch[i]);
			pr_err("Error getting dmabuf scatterlist\n");
			goto err_map_dbuf;
...
static int is_dbufcon_kmap(struct is_vb2_buf *vbuf, u32 plane)
{
	...
	if (is_dbuf) {
		...
		/* Shot buffers */
		adj_plane = (vbuf->num_merged_dbufs * num_i_planes) + (plane - num_i_planes);
		num_planes = 1;
	...
	if (IS_ERR_OR_NULL(dbufcon)) {
		err("failed to get dmabuf of fd: %d, plane: %d", vb->planes[plane].m.fd, plane);
		ret = -EINVAL;
		goto err_get_dbufcon;
	}
	...
	if (count < 0) {
		...
		err("empty dmabuf-container of fd: %d", vb->planes[plane].m.fd);
		ret = -EINVAL;
		goto err_empty_dbufcon;
	}
	...
	if (!is_dbuf) {
		...
		if (total_planes > IS_MAX_PLANES) {
			err("out of range plane. require: %d > supported: %d", total_planes,
			    IS_MAX_PLANES);
			ret = -EINVAL;
			goto err_too_many_planes;
		}
...
skip_dbufcon_release:
	for (b = 0, p = adj_plane; b < count; b++, p += num_planes) {
		...
		if (IS_ERR_OR_NULL((void *)vbuf->kva[p])) {
			err("[P%d]Failed to get kva %ld", p, vbuf->kva[p]);
			ret = -ENOMEM;
			goto err_kmap;
		}
	}
...
void is_subbuf_prepare(struct is_sub_dma_buf *buf, struct v4l2_plane *plane, struct device *dev)
{
	struct dma_buf *dbuf;
	int cnt = 0;
	...
	for (p = 0; p < num_p; p++) {
		...
		if (IS_ERR_OR_NULL(dbuf)) {
			err("[V%d][P%d]Failed to get dmabuf. fd %d ret %ld", buf->vid, p,
			    plane[p].m.fd, PTR_ERR(dbuf));
			goto err_get_dbufcon;
		}
		...
		if (cnt < 0) {
			...
			err("[V%d][P%d]Empty dma_buf_container. fd %d", buf->vid, p, plane[p].m.fd);
			goto err_get_dbuf;
		}
		...
		if ((cnt * num_p) > IS_MAX_PLANES) {
			err("[V%d][P%d]Out of range plane. required %d > supported %d", buf->vid, p,
		...
		for (b = 0, i = p; b < cnt; b++, i = (num_p * b) + p) {
			...
			if (IS_ERR_OR_NULL(buf->dbuf[i])) {
				err("[V%d][P%d]Failed to get dmabuf. fd %d ret %ld", buf->vid, i,
			...
			if (IS_ERR(buf->atch[i])) {
				err("[V%d][P%d]Failed to attach dmabuf. ret %ld", buf->vid, i,
				    PTR_ERR(buf->atch[i]));
				goto err_get_dbuf;
			}

			do {
				buf->sgt[i] = CALL_DMABUF_OPS(buf, map_attachment, buf->atch[i],
							      DMA_BIDIRECTIONAL);
				if (IS_ERR(buf->sgt[i])) {
...
err_get_dbufcon:
	...
	while (p-- > 0) {
		for (b = 0, i = p; b < cnt; b++, i = (b * num_p) + p) {
			CALL_DMABUF_OPS(buf, unmap_attachment, buf->atch[i], buf->sgt[i],
					DMA_BIDIRECTIONAL);
...
long is_subbuf_dvmap(struct is_sub_dma_buf *buf)
{
	...
	for (p = 0; p < num_planes && p < IS_MAX_PLANES; p++) {
		if (!buf->sgt[p]) {
			err("[N%d][P%d]sgt is NULL. fd %d", buf->vid, p, buf->buf_fd[p]);
		...
		if (IS_ERR_VALUE(buf->dva[p])) {
			err("[N%d][P%d]Failed to get dva", buf->vid, p);
			ret = buf->dva[p];
			goto err_map_dbuf;
		}
	}
...
err_map_dbuf:
	while (p-- > 0) {
		CALL_DMABUF_OPS(buf, unmap_attachment, buf->atch[p], buf->sgt[p],
				DMA_BIDIRECTIONAL);
		buf->dva[p] = 0;
	}
...
ulong is_subbuf_kvmap(struct is_sub_dma_buf *buf)
{
	...
	for (p = 0; p < num_planes && p < IS_MAX_PLANES; p++) {
		...
		if (IS_ERR_OR_NULL((void *)buf->kva[p])) {
			err("[N%d][P%d]Failed to get kva", buf->vid, p);
			ret = buf->kva[p];
			goto err_vmap;
		}
	}
...
const struct is_vb2_buf_ops is_vb2_buf_ops_dma_sg = { .plane_kvaddr = is_vb2_dma_sg_plane_kvaddr,
						      ...
						      .plane_sync_for_device =
							      is_vb2_dma_sg_plane_sync_for_device,
...
static int pablo_dmabuf_heap_attach_ext(struct is_priv_buf *pbuf, struct device *dev)
{
	...
	pbuf->sgt_ext =
		CALL_DMABUF_OPS(pbuf, map_attachment, pbuf->attachment_ext, DMA_BIDIRECTIONAL);
...
err_ion_map_io:
	CALL_DMABUF_OPS(pbuf, unmap_attachment, pbuf->attachment_ext, pbuf->sgt_ext,
			DMA_BIDIRECTIONAL);
	pbuf->sgt_ext = NULL;
err_map_dmabuf:
...
static void pablo_mem_sync(struct is_priv_buf *pbuf, off_t offset, size_t size, struct device *dev,
			   enum dma_data_direction dir, enum dma_sync_target target)
{
	...
	dbg_mem(1, "%s_for_%s begin (dva: %pad, offset: %ld, size: %zu, sg_dma_len:%d)", __func__,
		target == SYNC_FOR_DEVICE ? "device" : "cpu", &pbuf->iova, offset, size,
		sg_dma_len(pbuf->sgt->sgl));

	for_each_sg (pbuf->sgt->sgl, sg, pbuf->sgt->orig_nents, i) {
		dbg_mem(2, "[I:%d] size_offset: %lu, sg->length: %d", i, size_offset, sg->length);
		...
		if (size_offset + sg->length >= offset) {
			...
			if (actual_size > sg->length) {
				warn("%s, actual_size: %zu > sg->length: %d", __func__, actual_size,
				     sg->length);
				actual_offset = size_offset;
				actual_size = sg->length;
			}

			dbg_mem(2, "[I:%d] do cache ops(actual_offset: %lu, actual_size :%zu)", i,
...
static void is_ion_sync_for_device(struct is_priv_buf *pbuf, off_t offset, size_t size,
...
static void is_ion_sync_for_cpu(struct is_priv_buf *pbuf, off_t offset, size_t size,
				enum dma_data_direction dir)
{
	struct is_ion_ctx *iic;
...
static struct is_priv_buf *is_ion_alloc(void *ctx, size_t size, const char *heapname,
					unsigned int flags)
{
	struct is_ion_ctx *alloc_ctx = ctx;
	...
	heapmask =
		(flags & ION_EXYNOS_FLAG_PROTECTED) ? alloc_ctx->heapmask_s : alloc_ctx->heapmask;
...
static void pablo_dmabuf_heap_sync_for_device(struct is_priv_buf *pbuf, off_t offset, size_t size,
					      enum dma_data_direction dir)
{
...
static void pablo_dmabuf_heap_sync_for_cpu(struct is_priv_buf *pbuf, off_t offset, size_t size,
					   enum dma_data_direction dir)
{
	struct pablo_dmabuf_heap_ctx *pdhc;
...
static struct dma_heap *pablo_dmabuf_heap_find(void *ctx, const char *heapname)
{
	...
	if (!strncmp(heapname, "system", 6))
		...
		return pdhc->dh_camera_uncached ? pdhc->dh_camera_uncached :
						  pdhc->dh_system_uncached;
#endif
	else
		return NULL;
}

static struct is_priv_buf *pablo_dmabuf_heap_alloc(void *ctx, size_t size, const char *heapname,
						   unsigned int flags)
{
	struct pablo_dmabuf_heap_ctx *pdhc = (struct pablo_dmabuf_heap_ctx *)ctx;
	struct dma_heap *dh;
	...
	dh = heapname && strlen(heapname) ? pablo_dmabuf_heap_find(ctx, heapname) : pdhc->dh_system;
	if (!dh)
...
void is_mem_check_stats(struct is_mem *mem)
{
	...
	if (unbalanced) {
		dev_err(mem->dev, "counters for memory OPs unbalanced!\n");
		dev_err(mem->dev, "\tplane_kmap: %d, plane_kunmap: %d\n",
			atomic_read(&mem->stats->cnt_plane_kmap),
			atomic_read(&mem->stats->cnt_plane_kunmap));
		dev_err(mem->dev, "\tplane_dmap: %d, plane_dunmap: %d\n",
			atomic_read(&mem->stats->cnt_plane_dmap),
			atomic_read(&mem->stats->cnt_plane_dunmap));
		dev_err(mem->dev, "\tbuf_remap: %d, buf_unremap: %d\n",
			atomic_read(&mem->stats->cnt_buf_remap),
			atomic_read(&mem->stats->cnt_buf_unremap));
		dev_err(mem->dev, "\tdbuf_prepare: %d, dbuf_finish: %d\n",
			atomic_read(&mem->stats->cnt_dbuf_prepare),
			atomic_read(&mem->stats->cnt_dbuf_finish));
		dev_err(mem->dev, "\tdbuf_map: %d, dbuf_unmap: %d\n",
			atomic_read(&mem->stats->cnt_dbuf_map),
			atomic_read(&mem->stats->cnt_dbuf_unmap));
		dev_err(mem->dev, "\tdbuf_kmap: %d, dbuf_kunmap: %d\n",
			atomic_read(&mem->stats->cnt_dbuf_kmap),
			atomic_read(&mem->stats->cnt_dbuf_kunmap));
		dev_err(mem->dev, "\tsubbuf_prepare: %d, subbuf_finish: %d\n",
			atomic_read(&mem->stats->cnt_subbuf_prepare),
			atomic_read(&mem->stats->cnt_subbuf_finish));
		dev_err(mem->dev, "\tsubbuf_kvmap: %d, subbuf_kunmap: %d\n",
			atomic_read(&mem->stats->cnt_subbuf_kvmap),
			atomic_read(&mem->stats->cnt_subbuf_kunmap));
	}
...
static void __deinit_percpu_free_work(void)
{
	free_percpu(is_vfree_deferred);
	is_vfree_deferred = NULL;
}
...
void pablo_mem_deinit(struct is_mem *mem)
{
	__deinit_percpu_free_work();

	CALL_VOID_MEMOP(mem, cleanup, mem->priv);
	mem->priv = NULL;
}
EXPORT_SYMBOL_GPL(pablo_mem_deinit);
