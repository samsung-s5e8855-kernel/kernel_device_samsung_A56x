...
int is_vendor_device_info_set_sec2lsi_buff(void __user *user_data)
{
	...
	int32_t rom_awb_start_addr = 0;
	int32_t rom_awb_end_addr;
	int32_t rom_shading_start_addr = 0;
	...
	if (rom_info->sec2lsi_conv_done == false) {
		...
		if (standard_cal_data->rom_awb_sec2lsi_start_addr >= 0) {
			rom_awb_start_addr = standard_cal_data->rom_awb_sec2lsi_start_addr;
			rom_awb_end_addr = standard_cal_data->rom_awb_sec2lsi_checksum_addr
		...
		if (standard_cal_data->rom_shading_sec2lsi_start_addr >= 0) {
			rom_shading_start_addr = standard_cal_data->rom_shading_sec2lsi_start_addr;
			rom_shading_end_addr = standard_cal_data->rom_shading_sec2lsi_checksum_addr
				+ (SEC2LSI_CHECKSUM_SIZE - 1);
		...
		if (rom_awb_start_addr > 0) {
			info("%s : caminfo.awb_size is %d", __func__, romdata_sec2lsi.awb_size);
			if (copy_from_user(&cal_buf[rom_awb_start_addr], romdata_sec2lsi.lsiBuf,
				sizeof(uint8_t) * (romdata_sec2lsi.awb_size))) {
		...
		if (rom_shading_start_addr > 0) {
			info("%s : caminfo.lsc_size is %d", __func__, romdata_sec2lsi.lsc_size);
			if (copy_from_user(&cal_buf[rom_shading_start_addr], romdata_sec2lsi.lsiBuf +
...
int is_vendor_device_info_get_mipi_phy(void __user *user_data)
{
	...
	phy_num = csi->phy_sf_tbl->sf[PPS_COMM].phy_sfr_cnt + csi->phy_sf_tbl->sf[PPS_LANE].phy_sfr_cnt;
	...
	for (i = 0; i < phy_num; i++) {
		if (i < csi->phy_sf_tbl->sf[PPS_COMM].phy_sfr_cnt) {
			phy = &csi->phy_sf_tbl->sf[PPS_COMM].phy_sfr_info[i];
			sprintf(str_val, "comm %d", i);
		} else {
			phy = &csi->phy_sf_tbl->sf[PPS_LANE].phy_sfr_info[i - csi->phy_sf_tbl->sf[PPS_COMM].phy_sfr_cnt];
			sprintf(str_val, "lane %d", (int)(i - csi->phy_sf_tbl->sf[PPS_COMM].phy_sfr_cnt));
		}
...
int is_vendor_device_info_set_mipi_phy(void __user *user_data)
{
	...
	for (i = 0; i < phy_num; i++) {
		if (strstr(token, "comm ") != 0) {
			...
			token_sub = strsep(&token, " ");
			ret = kstrtoint(token_sub, 10, &phy_index);
			phy = &csi->phy_sf_tbl->sf[PPS_LANE].phy_sfr_info[phy_index];
		} else {
			break;
		}
...
int is_vendor_device_info_get_lens_distance_data(void __user *user_data)
{
	...
	int rom_cal_index = 0;
	int i;
	uint32_t tempbuf[AF_CAL_DISTANCE_MAX + 1];
	int32_t rom_af_distance_use_addr, rom_af_position_use_addr, rom_af_cal_use_addr_len;
	struct device_info_data_ctrl lens_data_addr_ctrl;

	if (copy_from_user((void *)&lens_data_addr_ctrl, user_data, sizeof(struct device_info_data_ctrl))) {
		err("%s : failed to copy data from user", __func__);
		ret = -EINVAL;
	}

	position = lens_data_addr_ctrl.param1;
	is_vendor_get_rom_info_from_position(position, &rom_id, &rom_cal_index);
	...
	rom_af_cal_use_addr_len = 0;

	if (rom_cal_index == 1 && rom_info->rom_sensor2_af_distance_cal_addr_len)
		rom_af_cal_use_addr_len = rom_info->rom_sensor2_af_distance_cal_addr_len;
	else if (rom_cal_index == 0 && rom_info->rom_af_distance_cal_addr_len)
		rom_af_cal_use_addr_len = rom_info->rom_af_distance_cal_addr_len;

	lens_data_addr_ctrl.ptr_size = rom_af_cal_use_addr_len;
	memset(tempbuf, 0, sizeof(tempbuf));

	if(rom_info->skip_cal_loading || rom_info->crc_error || !rom_af_cal_use_addr_len) {
		lens_data_addr_ctrl.ptr_size = 0;
		goto exit;
	}

	for (i = 0; i < rom_af_cal_use_addr_len; i += 2) {
		if (rom_cal_index == 1) {
			rom_af_distance_use_addr = rom_info->rom_sensor2_af_distance_cal_addr[i];
			rom_af_position_use_addr = rom_info->rom_sensor2_af_distance_cal_addr[i+1];
		} else {
			rom_af_distance_use_addr = rom_info->rom_af_distance_cal_addr[i];
			rom_af_position_use_addr = rom_info->rom_af_distance_cal_addr[i+1];
		}

		memcpy(&tempbuf[i], (uint16_t *)&cal_buf[rom_af_distance_use_addr], sizeof(uint16_t));
		memcpy(&tempbuf[i + 1], &cal_buf[rom_af_position_use_addr], sizeof(uint32_t));
	}

exit:
	if (copy_to_user(user_data, &lens_data_addr_ctrl, sizeof(struct  device_info_data_ctrl))) {
		err("%s : failed to copy data to user", __func__);
		return -EINVAL;
	}

	if (copy_to_user(lens_data_addr_ctrl.uint32_ptr, &tempbuf, sizeof(uint32_t) * AF_CAL_DISTANCE_MAX)) {
		err("%s : failed to copy data to user", __func__);
		ret = -EINVAL;
	}
