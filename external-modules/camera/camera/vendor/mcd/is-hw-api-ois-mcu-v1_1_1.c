...
{
	u32 reg_value = is_hw_get_reg(base, &ois_mcu_regs[cmd]);
...
{
	u8 reg_value = is_hw_get_reg_u8(base, &ois_mcu_regs[cmd]);
	dbg_ois("[GET_REG] reg:[%s][0x%04X], reg_value(R):[0x%08X]\n",
		ois_mcu_regs[cmd].reg_name, ois_mcu_regs[cmd].sfr_offset, reg_value);
	return reg_value;
}
...
int __is_mcu_hw_enable(void __iomem *base)
{
	...
	is_mcu_hw_set_field(base, OIS_CM0P_REMAP_I2C_ADDR, F_OIS_CM0P_I2C_ADDR, 0x15620000); /* USI10 */
	is_mcu_hw_set_field(base, OIS_CM0P_REMAP_SPI_ADDR, F_OIS_CM0P_SPI_ADDR, 0x15610000); /* USI09 */
