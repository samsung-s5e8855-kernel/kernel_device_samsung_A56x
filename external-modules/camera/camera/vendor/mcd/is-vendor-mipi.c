				...
				unsigned long size, void *buf)
{
	...
	struct cam_cp_ril_bridge_msg *msg;
	int cp_adaptive_mipi_id;
	int data_size;
	int msg_data_size;
	int i;
	...
	msg = (struct cam_cp_ril_bridge_msg *)buf;
	cp_adaptive_mipi_id = CALL_VENDOR_INTERFACE_OPS(core->vendor, get_cp_adaptive_mipi);

	if (size == sizeof(struct cam_cp_ril_bridge_msg)
				&& msg->dev_id == cp_adaptive_mipi_id) {
...
void is_vendor_register_ril_notifier(void)
{
	...
	if (!g_init_notifier) {
		...
		CALL_VENDOR_INTERFACE_OPS(core->vendor,
...
int is_vendor_update_mipi_info(struct is_device_sensor *device)
{
	...
	if (device->cfg[device->nfi_toggle]->mode >= cis->mipi_sensor_mode_size) {
		err("sensor mode is out of bound");
		return -1;
	}

	cur_mipi_sensor_mode = &cis->mipi_sensor_mode[device->cfg[device->nfi_toggle]->mode];
	...
	if (found != -1) {
		if (found < cur_mipi_sensor_mode->sensor_setting_size) {
			device->cfg[device->nfi_toggle]->mipi_speed = 
				cur_mipi_sensor_mode->sensor_setting[found].mipi_rate;
