...
bool check_ois_init_factory_done;
...
bool ois_is_camera_running(void)
{
	bool is_camera_running = false;

	is_camera_running = is_vendor_check_camera_running(SENSOR_POSITION_REAR);
#if defined(CAMERA_2ND_OIS)
	is_camera_running = (is_camera_running || is_vendor_check_camera_running(SENSOR_POSITION_REAR2));
#endif
#if defined(CAMERA_3RD_OIS)
	is_camera_running = (is_camera_running || is_vendor_check_camera_running(SENSOR_POSITION_REAR4));
#endif
...
void ois_power_control(int onoff)
{
	struct is_core *core = is_get_is_core();
	struct is_vendor *vendor;
	bool is_camera_running = ois_is_camera_running();
	...
	switch (onoff) {
	case 0:
		if (!is_camera_running && check_ois_power) {
			check_ois_power = false;
			is_ois_gpio_off(core);
	...
	case 1:
		if (!is_camera_running && !check_ois_power) {
			...
			check_ois_init_factory_done = false;
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (is_camera_running || check_ois_power) {
		if (check_ois_power && !check_ois_init_factory_done) {
			is_ois_init_factory(core);
			check_ois_init_factory_done = true;
		}
