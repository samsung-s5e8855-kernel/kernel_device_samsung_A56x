...
static int flash_s2mpb02_probe(struct device *dev, struct i2c_client *client)
{
	...
	flash = devm_kzalloc(dev, sizeof(struct is_flash) * sensor_id_len, GFP_KERNEL);
	if (!flash) {
		err("flash is NULL");
		ret = -ENOMEM;
		goto p_err;
	...
	subdev_flash = devm_kzalloc(dev, sizeof(struct v4l2_subdev) * sensor_id_len, GFP_KERNEL);
	if (!subdev_flash) {
...
static void flash_s2mpb02_remove(struct device *dev, struct i2c_client *client)
{
	struct class *camera_class;
...
static int flash_s2mpb02_platform_remove_i2c(struct platform_device *pdev)
{
	struct device *dev = &pdev->dev;

	flash_s2mpb02_remove(dev, NULL);
...
static struct platform_driver sensor_flash_s2mpb02_platform_driver = {
	...
	.remove	= flash_s2mpb02_platform_remove_i2c,
	.driver = {
...
#ifdef MODULE
module_driver(
	sensor_flash_s2mpb02_platform_driver, platform_driver_register, platform_driver_unregister)
#else
