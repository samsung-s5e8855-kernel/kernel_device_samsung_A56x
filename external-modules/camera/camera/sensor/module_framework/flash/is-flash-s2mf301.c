...
static int flash_s2mf301_init(struct v4l2_subdev *subdev, u32 val)
{
	...
	for (i = 0; i < FLASH_LED_CH_MAX; i++) {
		if (flash->led_ch[i] >= 0) {
			CALL_VENDOR_INTERFACE_OPS(core->vendor, s2mf301_fled_set_mode_ctrl, flash->led_ch[i], CAM_FLASH_MODE_OFF);
		}
	}
#endif
...
#if IS_ENABLED(CONFIG_LEDS_S2MF301_FLASH)
			ret = CALL_VENDOR_INTERFACE_OPS(core->vendor, s2mf301_fled_set_mode_ctrl, flash->led_ch[i], CAM_FLASH_MODE_OFF);
			if (ret < 0) {
...
#if IS_ENABLED(CONFIG_LEDS_S2MF301_FLASH)
			ret = CALL_VENDOR_INTERFACE_OPS(core->vendor, s2mf301_fled_set_curr, flash->led_ch[i], CAM_FLASH_MODE_SINGLE);
			if (ret < 0) {
				err("capture flash set current fail(led_ch:%d)", flash->led_ch[i]);
				ret = -EINVAL;
			}
			ret = CALL_VENDOR_INTERFACE_OPS(core->vendor, s2mf301_fled_set_mode_ctrl, flash->led_ch[i], CAM_FLASH_MODE_SINGLE);
			if (ret < 0) {
...
#if IS_ENABLED(CONFIG_LEDS_S2MF301_FLASH)
			ret = CALL_VENDOR_INTERFACE_OPS(core->vendor, s2mf301_fled_set_curr, flash->led_ch[i], CAM_FLASH_MODE_TORCH);
			if (ret < 0) {
				err("torch flash set current fail(led_ch:%d)", flash->led_ch[i]);
				ret = -EINVAL;
			}

			ret = CALL_VENDOR_INTERFACE_OPS(core->vendor, s2mf301_fled_set_mode_ctrl, flash->led_ch[i], CAM_FLASH_MODE_TORCH);
			if (ret < 0) {
...
int flash_s2mf301_g_ctrl(struct v4l2_subdev *subdev, struct v4l2_control *ctrl)
{
	...
	case V4L2_CID_FLASH_GET_DELAYED_PREFLASH_TIME:
		voltage = CALL_VENDOR_INTERFACE_OPS(core->vendor, muic_afc_get_voltage);
		if (voltage == 9) {
			ctrl->value = 200; /* ms */
		} else {
			ctrl->value = 0;
