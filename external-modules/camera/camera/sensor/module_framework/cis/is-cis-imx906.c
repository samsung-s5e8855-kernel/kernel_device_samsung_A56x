...
int sensor_imx906_cis_get_updated_binning_ratio(struct v4l2_subdev *subdev, u32 fcount, u32 *binning_ratio)
{
	struct is_cis *cis = sensor_cis_get_cis(subdev);
	cis_shared_data *cis_data = cis->cis_data;
	u32 zoom_ratio;
	u32 rms_mode = sensor_imx906_mode_groups[SENSOR_IMX906_MODE_RMS_CROP];
	u32 vsync_count = (fcount > cis_data->sen_vsync_count) ? fcount : cis_data->sen_vsync_count;
	u32 ref_history_index = (NUM_FRAMES + vsync_count - 1) % NUM_FRAMES;

	if (rms_mode == MODE_GROUP_NONE)
		return 0;

	zoom_ratio = cis_data->seamless_history[ref_history_index].remosaic_crop_zoom_ratio;
	...
	if (cis_data->remosaic_crop_update_vsync_cnt == vsync_count)
		info("[%s][%d] pre_zoom_ratio(%d), binning_ratio(%d)\n",
			__func__, vsync_count, zoom_ratio, *binning_ratio);
	else
...
DEFINE_I2C_DRIVER_PROBE(cis_imx906_probe_i2c)
{
	int ret;
	struct is_cis *cis;
...
PKV_DEFINE_I2C_DRIVER_REMOVE(cis_imx906_remove_i2c)
{
...
#ifdef MODULE
module_driver(sensor_cis_imx906_driver, i2c_add_driver,
	i2c_del_driver);
#else
