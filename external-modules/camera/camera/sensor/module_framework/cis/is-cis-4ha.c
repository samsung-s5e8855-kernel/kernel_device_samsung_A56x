...
DEFINE_I2C_DRIVER_PROBE(cis_4ha_probe_i2c)
{
	int ret;
	u32 mclk_freq_khz;
	struct is_cis *cis;
	...
	if (mclk_freq_khz == 19200 && !strcmp(setfile, "setC")) {
		probe_info("[%s] setfile_C mclk: %u\n", __func__, mclk_freq_khz);
		cis->sensor_info = &sensor_4ha_info_C;
	} else {
		if (!strcmp(setfile, "default") || !strcmp(setfile, "setA"))
			probe_info("[%s] setfile_A mclk: %u\n", __func__, mclk_freq_khz);
		else
