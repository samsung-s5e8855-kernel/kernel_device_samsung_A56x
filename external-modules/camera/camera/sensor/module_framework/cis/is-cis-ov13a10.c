...
bool sensor_ov13a10_cis_check_lte(struct v4l2_subdev *subdev, u32 input_duration)
{
	...
	if (input_duration > max_duration_16bit) {
		dbg_sensor(1, "[%d][%s][%d] enable LTE : input_duration(%d), max_duration(%llu)\n",
			cis->id, __func__, cis_data->sen_vsync_count, input_duration, max_duration_16bit);
		return true;
	}
...
int sensor_ov13a10_cis_lte_on(struct v4l2_subdev *subdev, u32 input_duration)
{
	int ret = 0;
	struct is_cis *cis = sensor_cis_get_cis(subdev);
	cis_shared_data *cis_data = cis->cis_data;
...
int sensor_ov13a10_cis_lte_off(struct v4l2_subdev *subdev)
{
	...
	if (lte_mode) {
		dbg_sensor(1, "[%d][%s][%d] lte_mode(%d) disable LTE\n", cis->id, __func__, cis->cis_data->sen_vsync_count, lte_mode);
		/* group 0 hold start */
		ret |= cis->ixc_ops->write8(cis->client, cis->reg_addr->group_param_hold, 0x00);
...
int sensor_ov13a10_cis_set_exposure_time(struct v4l2_subdev *subdev, struct ae_param *target_exposure)
{
	...
	if (sensor_ov13a10_cis_check_lte(subdev, input_duration) == true) {
		ret = sensor_ov13a10_cis_lte_on(subdev, input_duration);
	} else {
		ret = sensor_ov13a10_cis_lte_off(subdev);
...
#ifdef MODULE
builtin_i2c_driver(sensor_cis_ov13a10_driver);
#else
