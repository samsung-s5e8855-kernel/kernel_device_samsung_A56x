...
void sensor_hp2_cis_set_mode_group(u32 mode)
{
	...
	case SENSOR_HP2_4080x3060_60FPS_12BIT_VPD:
		sensor_hp2_mode_groups[SENSOR_HP2_MODE_LN4] =
			SENSOR_HP2_4080x3060_15FPS_12BIT_VPD_LN4;
		break;
	...
	case SENSOR_HP2_4000x3000_60FPS_VPD:
		...
		sensor_hp2_rms_zoom_mode[SENSOR_HP2_RMS_ZOOM_MODE_X_17] =
			SENSOR_HP2_4000x3000_30FPS_4SUM_BDS_VPD;
		sensor_hp2_rms_zoom_mode[SENSOR_HP2_RMS_ZOOM_MODE_X_20] =
			SENSOR_HP2_4000x3000_60FPS_4SUM_VPD;
		break;

	case SENSOR_HP2_4000x3000_60FPS_12BIT:
		sensor_hp2_mode_groups[SENSOR_HP2_MODE_IDCG] =
			SENSOR_HP2_4000x3000_60FPS_12BIT_IDCG;
		sensor_hp2_mode_groups[SENSOR_HP2_MODE_RMS_CROP] = MODE_GROUP_RMS;
		sensor_hp2_rms_zoom_mode[SENSOR_HP2_RMS_ZOOM_MODE_X_17] =
			SENSOR_HP2_4000x3000_30FPS_4SUM_BDS_12BIT;
		sensor_hp2_rms_zoom_mode[SENSOR_HP2_RMS_ZOOM_MODE_X_20] =
			SENSOR_HP2_4000x3000_60FPS_4SUM_12BIT;
		break;
	...
	case SENSOR_HP2_4000x2252_60FPS_12BIT:
		...
		sensor_hp2_mode_groups[SENSOR_HP2_MODE_IDCG] =
			SENSOR_HP2_4000x2252_60FPS_12BIT_IDCG;
		sensor_hp2_mode_groups[SENSOR_HP2_MODE_RMS_CROP] = MODE_GROUP_RMS;
		sensor_hp2_rms_zoom_mode[SENSOR_HP2_RMS_ZOOM_MODE_X_17] =
			SENSOR_HP2_4000x2252_60FPS_4SUM_BDS_12BIT;
		sensor_hp2_rms_zoom_mode[SENSOR_HP2_RMS_ZOOM_MODE_X_20] =
			SENSOR_HP2_4000x2252_60FPS_4SUM_12BIT;
		break;
	}
...
void sensor_hp2_cis_seamless_state_init(struct v4l2_subdev *subdev, int mode)
{
	struct is_cis *cis = sensor_cis_get_cis(subdev);
...
int sensor_hp2_cis_init(struct v4l2_subdev *subdev)
{
	...
	sensor_hp2_cis_seamless_state_init(subdev, 0);
...
int sensor_hp2_cis_write_calibration(struct v4l2_subdev *subdev, u16 start_addr, u16 end_addr,
				     u16 *endian)
{
	int ret = 0;
	int i = 0;
...
int sensor_hp2_cis_set_cal(struct v4l2_subdev *subdev)
{
	...
	for (i = 0; i < size; i += HP2_IXC_NEXT) {
		...
		case HP2_IXC_MODE_EEPROM:
			...
			sensor_hp2_cis_write_calibration(subdev, cal_info[index].start,
							 cal_info[index].end, &cur_endian);

			break;

		default:
			ret = cis->ixc_ops->write16(cis->client, regs[i + HP2_IXC_ARGV0],
						    regs[i + HP2_IXC_ARGV1]);
			if (ret < 0) {
				err("is_sensor_write16 fail, ret(%d), addr(%#x), data(%#x)", ret,
...
int sensor_hp2_cis_set_seamless_mode(struct v4l2_subdev *subdev, u32 next_mode)
{
	...
	cis_data->is_writing_sensor_mode = true;
	IXC_MUTEX_LOCK(cis->ixc_lock);
	...
	IXC_MUTEX_UNLOCK(cis->ixc_lock);
	cis_data->is_writing_sensor_mode = false;
	...
	info("[%s] pre(%d)->cur(%d), 12bit[%d] LN[%d] RMS[%d] AEB[%d] => fast_change_idx[0x%x]\n",
	     __func__, cur_mode, next_mode, cis_data->cur_12bit_mode, cis_data->cur_lownoise_mode,
...
int sensor_hp2_cis_mode_change(struct v4l2_subdev *subdev, u32 mode)
{
	...
	sensor_hp2_cis_seamless_state_init(subdev, mode);
	...
	if (actuator && mode == SENSOR_HP2_16320x12240_11FPS) {
		...
		ret |= cis->ixc_ops->write16(cis->client, REG_HP2_IMB_SETTING, actuator->position);
	} else {
		ret |= cis->ixc_ops->write16(cis->client, REG_HP2_IMB_SETTING,
					     DATA_HP2_IMB_SETTING_OFF);
	}
	...
	info("[%s] mode[%d] 12bit[%d] LN[%d] ZOOM[%d] AEB[%d] => fast_change_idx[0x%x]\n", __func__,
	     ...
	     cis->cis_data->cur_remosaic_zoom_ratio, cis->cis_data->cur_hdr_mode,
...
int sensor_hp2_cis_stream_on(struct v4l2_subdev *subdev)
{
	...
	if (cis->vendor_use_adaptive_mipi) {
		...
		cfg = device->cfg[device->nfi_toggle];
		if (cfg) {
			for (i = 0; i < priv->default_mipi_list_count; i++) {
				if (priv->default_mipi_list[i].mipi_rate == cfg->mipi_speed) {
					info("[%s] default mipi setting is found (mipirate = %d)\n",
						__func__, cfg->mipi_speed);
					sensor_cis_write_registers_locked(
						subdev, priv->default_mipi_list[i].mipi_setting);
					break;
				}
			}
		} else {
			info("[%s] default mipi setting (mipirate = 3712)\n", __func__);
			sensor_cis_write_registers_locked(subdev,
							  priv->default_mipi_list[0].mipi_setting);
		}
	}
	...
	if (seamless_fe_done_cnt > cis->cis_data->sen_vblank_count) {
		...
		do {
			info("[%s] seamless_fe_done_cnt(%d) cur_fe_count(%d)\n", __func__,
			     seamless_fe_done_cnt, cis->cis_data->sen_vblank_count);
			if (wait_cnt >= max_cnt) {
...
int sensor_hp2_sensor_hp2_cis_verify_bpc_crc(char *bpc_data)
{
	...
	calc_bpc_crc = bpc_data[0] << 24 | bpc_data[1] << 16 | bpc_data[2] << 8 | bpc_data[3];
...
int sensor_hp2_sensor_hp2_cis_read_bpc_otp(struct v4l2_subdev *subdev, char *bpc_data,
					   u32 *bpc_size)
{
	int ret = 0, read_size;
	struct is_cis *cis = sensor_cis_get_cis(subdev);
...
DEFINE_I2C_DRIVER_PROBE(cis_hp2_probe_i2c)
{
	int ret;
	u32 mclk_freq_khz;
	struct is_cis *cis;
	...
	cis->priv_runtime =
		devm_kzalloc(dev, sizeof(struct sensor_hp2_private_runtime), GFP_KERNEL);
	if (!cis->priv_runtime) {
		kfree(cis->cis_data);
		kfree(cis->subdev);
...
PKV_DEFINE_I2C_DRIVER_REMOVE(cis_hp2_remove_i2c)
{
...
#ifdef MODULE
module_driver(sensor_cis_hp2_driver, i2c_add_driver,
	i2c_del_driver)
#else
static int __init sensor_cis_hp2_init(void)
{
	...
	if (ret)
		err("failed to add %s driver: %d", sensor_cis_hp2_driver.driver.name, ret);
