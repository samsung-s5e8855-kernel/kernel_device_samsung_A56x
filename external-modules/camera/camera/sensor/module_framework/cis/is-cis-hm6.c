...
#include "is-cis-hm6-setB.h"
#include "is-cis-hm6-setC.h"
...
int sensor_hm6_cis_uXTC_write(struct v4l2_subdev *subdev, bool isRemosaic)
{
	...
	FIMC_BUG(!sensor_peri->eeprom);

	ret = CALL_EEPROMOPS(sensor_peri->eeprom, eeprom_get_cal_buf, sensor_peri->subdev_eeprom, &cal_buf);
	if (ret < 0 || !cal_buf)
		goto p_err;
...
DEFINE_I2C_DRIVER_PROBE(cis_hm6_probe_i2c)
{
	int ret;
	u32 mclk_freq_khz;
	struct is_cis *cis;
	...
	if (strcmp(setfile, "default") == 0 || strcmp(setfile, "setA") == 0){
		...
		probe_info("[%s] setfile_B mclk: %d \n", __func__, mclk_freq_khz);
		cis->sensor_info = &sensor_hm6_info_B;
	}else if (strcmp(setfile, "setC") == 0){
		probe_info("[%s] setfile_B mclk: %d \n", __func__, mclk_freq_khz);
		cis->sensor_info = &sensor_hm6_info_C;
	}else{
		cis->sensor_info = &sensor_hm6_info_A;
