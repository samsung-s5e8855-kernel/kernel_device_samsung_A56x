...
int sensor_imx754_cis_update_crop_region(struct v4l2_subdev *subdev)
{
	...
	if (mode == SENSOR_IMX754_1920X1080_120FPS || mode == SENSOR_IMX754_1920X1080_240FPS ||
		mode == SENSOR_IMX754_912X684_120FPS) {
	...
	struct device *cdev, enum ixc_type type)
{
	...
	cis->priv_runtime =
		devm_kzalloc(cdev, sizeof(struct sensor_imx754_private_runtime), GFP_KERNEL);
	if (!cis->priv_runtime) {
		kfree(cis->cis_data);
		kfree(cis->subdev);
...
DEFINE_I2C_DRIVER_PROBE(cis_imx754_probe_i2c)
{
...
PKV_DEFINE_I2C_DRIVER_REMOVE(cis_imx754_remove_i2c)
{
...
PKV_DEFINE_I3C_DRIVER_REMOVE(cis_imx754_remove_i3c)
{
	PKV_DEFINE_I3C_DRIVER_REMOVE_RETURN;
}
