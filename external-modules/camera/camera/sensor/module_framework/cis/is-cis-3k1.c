...
void sensor_3k1_cis_seamless_state_init(struct v4l2_subdev *subdev, int mode)
{
	struct is_cis *cis = sensor_cis_get_cis(subdev);
	struct sensor_3k1_private_runtime *priv_runtime;
	const struct sensor_cis_mode_info *mode_info;

	priv_runtime = (struct sensor_3k1_private_runtime *)cis->priv_runtime;
...
#endif
	sensor_3k1_cis_seamless_state_init(subdev, 0);
...
int sensor_3k1_cis_update_crop_region(struct v4l2_subdev *subdev)
{
	...
	if (mode == SENSOR_3K1_1824x1368_120FPS || mode == SENSOR_3K1_1824x1368_30FPS) {
	...
	start_pos_infos[mode].x_start = x_start;
	start_pos_infos[mode].y_start = y_start;
...
int sensor_3k1_cis_check_aeb(struct v4l2_subdev *subdev)
{
	int ret = 1;
	struct is_device_sensor *device;
	struct is_cis *cis = sensor_cis_get_cis(subdev);
...
int sensor_3k1_cis_update_seamless_mode(struct v4l2_subdev *subdev)
{
	...
	if (sensor_3k1_cis_check_aeb(subdev) != 1)
		goto EXIT;
...
int sensor_3k1_cis_mode_change(struct v4l2_subdev *subdev, u32 mode)
{
	...
	sensor_3k1_cis_seamless_state_init(subdev, mode);
...
int sensor_3k1_cis_stream_on(struct v4l2_subdev *subdev)
{
	...
	if (//cis->cis_data->is_data.scene_mode == AA_SCENE_MODE_BOKEH_VIDEO pamir_bringup
		...
		ret = sensor_cis_write_registers(subdev, priv->dual_slave);
		cis->cis_data->dual_slave = true;
	} else {
		info("[%s] dual sync standalone\n", __func__);
		ret = sensor_cis_write_registers(subdev, priv->dual_standalone);
...
int sensor_3k1_cis_wait_seamless_update_delay(struct v4l2_subdev *subdev)
{
	struct is_cis *cis = sensor_cis_get_cis(subdev);
	struct sensor_3k1_private_runtime *priv_runtime = (struct sensor_3k1_private_runtime *)cis->priv_runtime;
...
static struct is_cis_ops cis_ops = {
	...
	.cis_wait_seamless_update_delay = sensor_3k1_cis_wait_seamless_update_delay,
