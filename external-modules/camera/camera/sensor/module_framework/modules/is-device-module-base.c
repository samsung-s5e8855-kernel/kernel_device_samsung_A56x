...
static int sensor_module_init(struct v4l2_subdev *subdev, u32 val)
{
	...
	if (subdev_flash != NULL && !is_get_debug_param(IS_DEBUG_PARAM_PHY_TUNE)) {
...
int sensor_module_deinit(struct v4l2_subdev *subdev)
{
	...
	if (sensor_peri->actuator) {
		...
		if (CALL_ACTUATOROPS(sensor_peri->actuator, perform_soft_landing_on_exit,
			sensor_peri->subdev_actuator)) {
...
static int sensor_module_g_ctrl(struct v4l2_subdev *subdev, struct v4l2_control *ctrl)
{
	...
	case V4L2_CID_SENSOR_GET_BINNING_RATIO:
		if (!device->cfg[device->nfi_toggle]) {
			err("get sensor_cfg is fail");
			ret = -EINVAL;
			goto p_err;
		}
		ret = CALL_CISOPS(&sensor_peri->cis, cis_get_binning_ratio,
				sensor_peri->subdev_cis, device->cfg[device->nfi_toggle]->mode, &ctrl->value);
		if (ret < 0) {
			err("err!!! ret(%d)", ret);
			ret = -EINVAL;
			goto p_err;
...
static int sensor_module_s_stream(struct v4l2_subdev *sd, int enable)
{
	...
	sensor_peri = (struct is_device_sensor_peri *)module->private_data;
	cfg = sensor->cfg[sensor->nfi_toggle];
...
static inline void __sensor_module_calculate_data(struct is_cis *cis,
	struct is_device_sensor *device, struct is_sensor_cfg *sensor_cfg,
	struct v4l2_subdev_format *fmt)
{
	cis->cis_data->request_min_frame_us_time = ((1 * 1000 * 1000) / device->image.framerate);
	cis->cis_data->sens_config_index_cur = sensor_cfg->mode;
	cis->cis_data->sens_config_ex_mode_cur = sensor_cfg->ex_mode;
...
static inline bool __sensor_module_is_new_mode(
	struct is_cis *cis, struct is_sensor_cfg *cfg, struct is_device_sensor_peri *sensor_peri)
{
	return (cis->cis_data->sens_config_index_cur != cfg->mode) ||
	       (cis->cis_data->sens_config_ex_mode_cur != cfg->ex_mode) ||
	       (sensor_peri->mode_change_first == true);
}
	...
	struct v4l2_subdev *subdev, v4l2_subdev_config_t *cfg, struct v4l2_subdev_format *fmt)
{
	...
	sensor_cfg = device->cfg[device->nfi_toggle];
	FIMC_BUG(!sensor_cfg);

	new_mode = __sensor_module_is_new_mode(cis, sensor_cfg, sensor_peri);

	is_sensor_flush_prev_mode_change(cis);

	CALL_CISOPS(cis, cis_check_wdr_mode, sensor_peri->subdev_cis, sensor_cfg->mode);

	__sensor_module_calculate_data(cis, device, sensor_cfg, fmt);

	if (new_mode) {
		dbg_sensor(1, "[%s] mode changed(%d:%d->%d:%d)\n", __func__,
			cis->cis_data->sens_config_index_cur, sensor_cfg->mode,
			cis->cis_data->sens_config_ex_mode_cur, sensor_cfg->mode);
...
static int sensor_module_probe_i2c(struct platform_device *pdev)
{
	...
	if (device->pdata->i2c_dummy_enable) {
		...
		if (pdata->cis_np) {
			...
			pdata->cis_client = i2c_new_client_device(ixc_client->adapter, cis_info);
		}
		...
		if (pdata->af_np) {
			...
			pdata->af_client = i2c_new_client_device(ixc_client->adapter, af_info);
		}
		...
		if (pdata->eeprom_np) {
			...
			pdata->eeprom_client = i2c_new_client_device(ixc_client->adapter, eeprom_info);
		}
		...
		if (pdata->ois_np) {
			...
			pdata->ois_client = i2c_new_client_device(ixc_client->adapter, ois_info);
		}
	}
...
static int sensor_module_remove_i2c(struct platform_device *pdev)
{
	struct exynos_platform_is_module *pdata;
	struct device *dev = &pdev->dev;
	...
	if (!IS_ERR_OR_NULL(pdata->cis_client))
		i2c_unregister_device(pdata->cis_client);
	if (!IS_ERR_OR_NULL(pdata->af_client))
		i2c_unregister_device(pdata->af_client);
	if (!IS_ERR_OR_NULL(pdata->eeprom_client))
		i2c_unregister_device(pdata->eeprom_client);
	if (!IS_ERR_OR_NULL(pdata->ois_client))
		i2c_unregister_device(pdata->ois_client);

	is_sensor_module_free_dt(dev);
...
static struct platform_driver sensor_module_driver = {
	...
	.remove	= sensor_module_remove_i2c,
	.driver = {
