...
#define STREAM_OFF_WAIT_CNT 3000
...
static int sensor_zebu_encode_binary(void *dst_addr, struct is_binary *bin, struct is_vci_config *vci_cfg)
{
	...
	if (!dst_addr || !bin || !vci_cfg) {
		err("%s:Invalid argument. dst_addr %p bin %p vci_cfg %p\n", __func__, dst_addr, bin,
			vci_cfg);
		return -EINVAL;
	}
	...
	info("%s:Generate CSIS packet stream. %dx%d(0x%x) %p -> %p (%d)\n", __func__,
		vci_cfg->width, vci_cfg->height, vci_cfg->hwformat, src8, dst, length);
...
int sensor_zebu_stream_on(struct v4l2_subdev *subdev)
{
	...
	is_hw_set_field(regs, &img_drv_regs[IMG_DRV_R_FRAME_BLANK_MULT_VALUE],
		&img_drv_fields[IMG_DRV_F_FRAME_BLANK_MULT_VALUE], 0x2);

	if (is_debug_support_crta())
		is_hw_set_field(regs, &img_drv_regs[IMG_DRV_R_LINE_BLANK],
			&img_drv_fields[IMG_DRV_F_LINE_BLANK], 0x6000);
	else
		is_hw_set_field(regs, &img_drv_regs[IMG_DRV_R_LINE_BLANK],
			&img_drv_fields[IMG_DRV_F_LINE_BLANK], 0x1800);
	...
	for (vc = CSI_VIRTUAL_CH_0; vc < CSI_VIRTUAL_CH_MAX; vc++) {
		...
		if (ret) {
			...
			err("%s:[VC%d]Too large bin size 0x%zX", __func__, vc, bin.size);
			release_binary(&bin);
			continue;
		}
		...
		release_binary(&bin);
		if (ret) {
			err("%s:[VC%d]failed to encode image. ret %d", __func__, vc, ret);
			ret = 0;
			continue;
		}
...
static void _sensor_zebu_wait_streamoff(void)
{
	void __iomem *regs = module_zebu.reg;
	u32 wait_cnt = 0;

	is_hw_set_field(regs, &img_drv_regs[IMG_DRV_R_FRAME_NUM],
		&img_drv_fields[IMG_DRV_F_INFINITE_RUN], 0);

	while (is_hw_get_field(
		regs, &img_drv_regs[IMG_DRV_R_BUSY], &img_drv_fields[IMG_DRV_F_LOADER_BUSY])) {
		usleep_range(STREAM_OFF_WAIT_TIME, STREAM_OFF_WAIT_TIME + 10);

		if (wait_cnt++ >= STREAM_OFF_WAIT_CNT) {
			err("%s:time_out!", __func__);
			break;
		}
	}
}
...
static int sensor_module_zebu_parse_dt(struct platform_device *pdev)
{
	...
	module_zebu.reg = devm_ioremap(dev, res->start, resource_size(res));
	if (!module_zebu.reg) {
...
static int sensor_module_zebu_probe(struct platform_device *pdev)
{
	...
	subdev_module = devm_kzalloc(&pdev->dev, sizeof(struct v4l2_subdev), GFP_KERNEL);
	if (!subdev_module) {
...
static int sensor_module_zebu_remove(struct platform_device *pdev)
{
#ifdef CONFIG_OF
	struct device *dev = &pdev->dev;
...
struct platform_driver sensor_module_zebu_driver = {
	...
	.remove = sensor_module_zebu_remove,
	.driver = {
		.name   = SENSOR_NAME,
		.owner  = THIS_MODULE,
