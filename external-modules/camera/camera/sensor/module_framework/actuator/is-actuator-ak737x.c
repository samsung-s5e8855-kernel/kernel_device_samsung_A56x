...
DEFINE_I2C_DRIVER_PROBE(sensor_ak737x_actuator_probe_i2c)
{
	int ret = 0;
	struct is_core *core;
	...
	actuator = devm_kzalloc(dev, sizeof(struct is_actuator), GFP_KERNEL);
	if (!actuator) {
	...
	subdev_actuator = devm_kzalloc(dev, sizeof(struct v4l2_subdev), GFP_KERNEL);
	if (!subdev_actuator) {
	...
	camera_focus_dev[sensor_id] =
		device_create(camera_class, NULL, MKDEV(1, sensor_id), actuator, filename);
...
PKV_DEFINE_I2C_DRIVER_REMOVE(sensor_ak737x_actuator_remove_i2c)
{
	u32 sensor_id;
	struct device *dev;
	struct device_node *dnode;
	...
	dnode = dev->of_node;
	camera_class = is_get_camera_class();
	of_property_read_u32(dnode, "id", &sensor_id);

	device_remove_file(camera_focus_dev[sensor_id], &dev_attr_focus_position);
	device_destroy(camera_class, camera_focus_dev[sensor_id]->devt);
	camera_focus_dev[sensor_id] = NULL;
...
static struct i2c_driver actuator_ak737x_driver = {
	...
	.remove	= sensor_ak737x_actuator_remove_i2c,
	.driver = {
		.name	= ACTUATOR_NAME,
		.owner	= THIS_MODULE,
...
module_driver(actuator_ak737x_driver, i2c_add_driver,
	i2c_del_driver)
