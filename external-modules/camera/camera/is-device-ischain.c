...
static int is_ischain_group_shot(struct is_device_ischain *device, struct is_group *group,
	...
	if (!im->loaded_setfile[position]) {
		...
		if (module->setfile_size) {
			...
			minfo("preloaded(%lldus) %s is ready to use, size: 0x%zx\n", device,
			      PABLO_KTIME_US_DELTA_NOW(time_to_ready), module->setfile_name,
			      module->setfile_size);
		} else {
...
static void is_ischain_close_setfile(struct is_device_ischain *device)
{
	...
	if (!pinned && test_bit(IS_ISCHAIN_INIT, &device->state) &&
	    refcount_dec_and_test(&im->refcount_setfile[position]) && loaded) {
...
int is_itf_s_param(struct is_device_ischain *device, struct is_frame *frame, IS_DECLARE_PMAP(pmap))
{
	int ret = 0;
	u32 pi;
...
void *is_itf_g_param(struct is_device_ischain *device, struct is_frame *frame, u32 index)
{
...
int is_itf_grp_shot(struct is_device_ischain *device, struct is_group *group,
		    struct is_frame *frame)
{
	int ret = 0;
	unsigned long flags;
...
p_sensor_config:
	...
	if (sensor->obte_config & BIT(OBTE_CONFIG_BIT_SENSOR_BINNING_EN)) {
		cfg->binning = (u32)OBTE_CONFIG_GET_SENSOR_BINNING_RATIO(sensor->obte_config);
...
static bool __is_repeating_same_frame(struct is_frame *frame)
{
	...
	if (frame->repeat_info.num && frame->repeat_info.idx < frame->repeat_info.num - 1)
		return true;
					 ...
					 u32 fcount, u32 size, ulong addr)
{
	...
	return is_hardware_capture_meta_request(&core->hardware, group, device->instance, fcount,
						size, addr);
}

int is_ischain_g_ddk_setfile_version(struct is_device_ischain *device, void *user_ptr)
{
	int ret = 0;
	int position;
	...
	strncpy(version_info->ddk_version, get_binary_version(IS_BIN_LIBRARY, IS_BIN_LIB_HINT_DDK),
		60);
...
int is_ischain_probe(struct is_device_ischain *device, struct is_resourcemgr *resourcemgr,
		     struct platform_device *pdev, u32 instance)
{
	int ret, slot;
...
static int is_ischain_init_wrap(struct is_device_ischain *device, u32 stream_type, u32 position)
{
	int ret = 0;
	u32 sindex;
	struct is_core *core;
	...
	mdbgd_ischain("%s(%d, %d)\n", device, __func__, atomic_read(&device->init_cnt),
		      atomic_read(&device->group_open_cnt));
	...
	if (atomic_read(&device->init_cnt) == atomic_read(&device->group_open_cnt)) {
		...
		if (stream_type) {
			...
			if (sensor->instance != device->instance) {
				merr("instance is mismatched (!= %d)[SS%d]", device,
				     sensor->instance, sensor->device_id);
				ret = -EINVAL;
				goto p_err;
			}
...
static int is_ischain_start(struct is_device_ischain *device)
{
	...
	if (device->sensor->scene_mode >= AA_SCENE_MODE_DISABLED)
		device->is_region->parameter.taa.vdma1_input.scene_mode =
			device->sensor->scene_mode;
#endif
...
p_err:
	minfo("[ISC:D] %s(%d):%d\n", device, __func__, device->setfile & IS_SETFILE_MASK, ret);

	return ret;
}

int is_ischain_start_wrap(struct is_device_ischain *device, struct is_group *group)
{
	int ret = 0;
...
int is_ischain_stop_wrap(struct is_device_ischain *device, struct is_group *group)
{
	int ret = 0;
	struct is_group *leader;
...
static int change_vctx_group(struct is_device_ischain *device, struct is_video_ctx *ivc)
{
	struct is_device_ischain *ischain;
	struct is_core *core = is_get_is_core();
	u32 stream;
	...
	minfo("use ischain(%d) group for V%02d", device, ischain->instance, ivc->video->id);

	return 0;
}

int is_ischain_group_open(struct is_device_ischain *device, struct is_video_ctx *vctx, u32 group_id)
{
	int ret = 0;
	int ret_err = 0;
...
err_group_stop:
err_group_init:
	mginfo("%s, group->scount: %d (%d)\n", idi, ig, __func__, atomic_read(&ig->scount), ret);

	return ret;
}

static int is_ischain_group_s_fmt(struct is_device_ischain *idi, struct is_queue *iq,
				  struct is_group *ig)
{
...
int is_ischain_group_s_input(struct is_device_ischain *device, struct is_group *group,
			     u32 stream_type, u32 position, u32 input_type, u32 stream_leader)
{
	int ret;
...
static struct is_frame *is_ischain_check_frame(struct is_group *group, struct is_frame *check_frame)
{
	struct is_frame *frame;
	struct is_framemgr *framemgr;
...
static void is_ischain_update_setfile(struct is_device_ischain *device, struct is_group *group,
				      struct is_frame *frame)
{
	int ret;
...
static int is_ischain_child_tag(struct is_device_ischain *device, struct is_group *group,
				struct is_frame *frame)
{
	int ret = 0;
	...
	TRANS_CROP(ldr_node.input.cropRegion, node_group->leader.input.cropRegion);
...
static void is_ischain_update_shot(struct is_device_ischain *device, struct is_frame *frame)
{
	...
	if (fastctlmgr->queued_count[state]) {
		/* get req list */
		fast_ctl =
			list_first_entry(&fastctlmgr->queued_list[state], struct is_fast_ctl, list);
...
static void is_ischain_update_otf_data(struct is_device_ischain *device, struct is_group *group,
				       struct is_frame *frame)
{
	struct is_resourcemgr *resourcemgr = device->resourcemgr;
	struct is_device_sensor *sensor = device->sensor;
	...
	if (sensor->exposure_fcount[frame->fcount % IS_EXP_BACKUP_COUNT] ==
		    (frame->fcount - SENSOR_REQUEST_DELAY) &&
	    sensor->exposure_value[frame->fcount % IS_EXP_BACKUP_COUNT] >=
		    ((SHOT_TIMEOUT * 1000) - 1000000)) {
		resourcemgr->shot_timeout =
			(sensor->exposure_value[frame->fcount % IS_EXP_BACKUP_COUNT] / 1000) +
			SHOT_TIMEOUT;
	...
	mdbgd_ischain(
		"[%s] fcount(%d), exposure_fcount(%d), exposure_value(%d), shot_timeout(%d), tick(%d)\n",
		device, __func__, frame->fcount,
...
p_err:
	...
	if (ret) {
		...
		framemgr = GET_HEAD_GROUP_FRAMEMGR(group);
		if (!framemgr) {
			mgerr("[F%d] Failed to get head framemgr", group, group,
			      check_frame->fcount);
			return -EINVAL;
		}
