...
#include "modules/is-device-module-base.h"
...
EXPORT_SYMBOL_GPL(is_get_sensor_device);
...
EXPORT_SYMBOL_GPL(is_get_ischain_device);
...
#if !IS_ENABLED(CONFIG_SENSOR_GROUP_LVN)
			/* vc */
			for (idx_wdma = 0; idx_wdma < csi->sensor_cfg[csi->nfi_toggle]->dma_num;
				idx_wdma++) {
				for (wdma_vc = DMA_VIRTUAL_CH_0; wdma_vc < DMA_VIRTUAL_CH_MAX;
				     wdma_vc++)
					is_print_frame_dva(&sensor->ssvc[idx_wdma][wdma_vc]);
			}
#endif
		}
	}
				 ...
				 const char *buf, size_t count)
{
	...
	case '0':
		...
		is_dvfs_init(&resourcemgr->dvfs_ctrl, is_dvfs_dt_arr, is_hw_dvfs_get_lv);
		for (i = 0; i < IS_STREAM_COUNT; i++) {
			if (test_bit(IS_ISCHAIN_OPEN, &((core->ischain[i]).state))) {
				is_set_dvfs_otf(&resourcemgr->dvfs_ctrl, IS_SN_MAX);
				is_set_dvfs_m2m(&resourcemgr->dvfs_ctrl, IS_SN_MAX, false);
			}
		}
...
static int is_probe(struct platform_device *pdev)
{
	...
	ret = is_vendor_probe(&core->vendor);
	if (ret) {
		probe_err("is_vendor_probe is fail(%d)", ret);
		goto p_err1;
	}

	/* group initialization */
	ret = is_groupmgr_probe();
	if (ret) {
	...
	ret = is_hardware_probe(&core->hardware, &core->interface,
				&core->interface_ischain, core->pdev);
	if (ret) {
	...
	is_register_iommu_fault_handler_cb(__is_fault_handler, is_dev);
	...
	sysfs_debug->emulate_irq = csi_emulate_irq;
	...
	if (pdata) {
		...
		if (!IS_ERR(s)) {
			...
			probe_info("pinctrl_lookup no release state\n");
		}
	}
...
static int is_driver_init(void)
{
	...
	ret = platform_driver_register(sensor_module_get_platform_driver());
	if (ret) {
	...
	ret = platform_driver_register(pablo_video_get_platform_driver());
	if (ret) {
		err("pablo_video_driver register failed(%d)", ret);
		return ret;
	}
	...
	ret = is_vendor_driver_init();
	if (ret) {
		err("is_vendor_driver_init failed(%d)", ret);
		return ret;
	}
...
#endif
	platform_driver_unregister(sensor_module_get_platform_driver());
