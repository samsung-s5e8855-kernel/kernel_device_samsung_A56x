...
load("//exynos/soc-series/s-android15:s5e9955_modules.bzl",
...
def get_erd9955_kunit_test_list(variant = None):
    modules_list = []
    if variant != "user":
        modules_list += _VENDOR_KUNIT_TEST_LIST
    return modules_list

def get_erd9955_system_dlkm_blocklist():
    modules_list = []
    modules_list += _SYSTEM_DLKM_BLOCKLIST
    return modules_list

def get_erd9955_vendor_dlkm_blocklist():
    modules_list = []
    modules_list += _VENDOR_DLKM_BLOCKLIST
    return modules_list

def get_erd9955_vendor_dlkm_modules_list(variant = None):
    modules_list = []
    modules_list += _VENDOR_DLKM_MODULE_LIST
    modules_list += _VENDOR_DLKM_BOOT_COMPLETE_MODULE_LIST
    modules_list += get_erd9955_vendor_dlkm_blocklist()
    if variant != "user":
...
def get_erd9955_vendor_boot_modules_list(variant = None):
    modules_list = []
...
def get_all_vendor_boot_modules_list(variant = None):
    modules_list = []
    modules_list += get_erd9955_vendor_boot_modules_list(variant)
...
def get_all_system_dlkm_blocklist():
    ...
    modules_list += get_erd9955_system_dlkm_blocklist()
    return modules_list
...
def get_all_kunit_test_list(variant = None):
    ...
    modules_list += get_erd9955_kunit_test_list(variant)
    return modules_list
...
def get_all_vendor_dlkm_modules_list(variant = None):
    ...
    modules_list += get_erd9955_vendor_dlkm_modules_list(variant)
    modules_list += _EXTERNAL_MODULE_IN_DLKM_LIST
    return modules_list

# All modules built from this directory
def get_erd9955_vendor_modules_list(variant = None):
    modules_list = []
    modules_list += get_erd9955_vendor_boot_modules_list(variant)
    modules_list += get_erd9955_vendor_dlkm_modules_list(variant)
    modules_list += get_erd9955_kunit_test_list(variant)
...
def define_erd9955_module(variant = None):
    MODULE_OUTS = get_erd9955_vendor_modules_list(variant)
    if MODULE_OUTS:
        kernel_module(
            name = "erd9955_module.{}".format(variant),
             srcs = [
                ":exynos.erd9955_sources",
            ],
            outs = MODULE_OUTS,
...
def register_erd9955_module():
    define_erd9955_module("eng")
    define_erd9955_module("userdebug")
    define_erd9955_module("user")
