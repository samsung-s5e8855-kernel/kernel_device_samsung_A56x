...
static bool write_bat_property = false;
				...
				int *data, int data_type)
{
	...
	if (psy) {
		...
		if (ret < 0) {
			aw_dev_err(aw_dev->dev, "get data failed, ret = %d", ret);
			return -EINVAL;
		}
		*data = prop.intval;
...
int aw_update_sys_temp(struct aw_device *aw_dev, int32_t *te)
{
	...
	ret = aw_monitor_get_data_form_system(aw_dev, te, POWER_SUPPLY_PROP_TEMP);
	if (ret) {
		aw_dev_err(aw_dev->dev, "get temperature from system failed!");
		return ret;
	}
	aw_dev_info(aw_dev->dev, "get temperature %d from system!", *te);
...
int aw_update_voltage_current(struct aw_device *aw_dev, int32_t *vol, int32_t *cur)
{
	...
	ret = aw_monitor_get_data_form_system(aw_dev, vol, POWER_SUPPLY_PROP_VOLTAGE_NOW);
	if (ret) {
		aw_dev_err(aw_dev->dev, "get voltage from system failed!");
		return ret;
	}

	ret = aw_monitor_get_data_form_system(aw_dev, cur, POWER_SUPPLY_PROP_CURRENT_NOW);
	if (ret) {
		aw_dev_err(aw_dev->dev, "get current from system failed!");
		return ret;
	}

	aw_dev_info(aw_dev->dev, "get voltage %d, current %d from system!", *vol, *cur);
...
static int aw_monitor_abox_work(struct aw_device *aw_dev)
{
	...
	ret = aw_big_data_update(aw_dev);
	if (ret < 0)
		aw_dev_err(aw_dev->dev, "get big data failed");
...
{
	struct aw_device *aw_dev  = container_of(work,
		struct aw_device, monitor_desc.abox_work.work);
	struct aw882xx *aw882xx = (struct aw882xx *)aw_dev->private_data;
	struct aw_monitor_desc *monitor = &aw_dev->monitor_desc;
	struct aw_monitor_cfg *monitor_cfg = &aw_dev->monitor_desc.monitor_cfg;
	...
	if (monitor_cfg->monitor_switch) {
		...
		if (aw_dev->channel == AW_DEV_CH_PRI_L )
			queue_delayed_work(aw882xx->work_queue,
				&monitor->abox_work, msecs_to_jiffies(g_abox_monitor_ms));
	}
}
void aw882xx_monitor_start(struct aw_monitor_desc *monitor_desc)
{
	...
	if (delayed_work_pending(&monitor_desc->abox_work))
		aw_dev_info(aw_dev->dev, "abox_work is running");
	else if (aw_dev->channel == AW_DEV_CH_PRI_L)
		queue_delayed_work(aw882xx->work_queue,
			&monitor_desc->abox_work, msecs_to_jiffies(g_abox_monitor_ms));
#endif
...
int aw882xx_monitor_stop(struct aw_monitor_desc *monitor_desc)
{
	...
	write_bat_property = false;

	if (monitor_desc->monitor_mode == AW_MON_KERNEL_MODE)
		cancel_delayed_work_sync(&monitor_desc->delay_work);

	if (delayed_work_pending(&monitor_desc->abox_work))
		cancel_delayed_work_sync(&monitor_desc->abox_work);
				...
				uint8_t *data, uint32_t data_len)
{
	...
	if (monitor_desc == NULL || data == NULL) {
		aw_pr_err("monitor_desc or data is NULL");
		return -EINVAL;
	}
...
static ssize_t abox_monitor_ms_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	struct aw882xx *aw882xx = dev_get_drvdata(dev);
	struct aw_device *aw_dev = aw882xx->aw_pa;
...
static ssize_t abox_monitor_ms_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	ssize_t len = 0;
...
static ssize_t abox_temp_check_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	struct aw882xx *aw882xx = dev_get_drvdata(dev);
	struct aw_device *aw_dev = aw882xx->aw_pa;
...
static ssize_t abox_temp_check_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	ssize_t len = 0;
	struct aw882xx *aw882xx = dev_get_drvdata(dev);
...
static DEVICE_ATTR_RW(abox_monitor_ms);
static DEVICE_ATTR_RW(abox_temp_check);
...
void aw882xx_monitor_init(struct aw_monitor_desc *monitor_desc)
{
	...
	monitor_desc->monitor_cfg.monitor_switch = false;
	aw_monitor_parse_mode(aw_dev);
	INIT_DELAYED_WORK(&monitor_desc->delay_work, aw_monitor_work_func);
	INIT_DELAYED_WORK(&monitor_desc->abox_work, aw_monitor_abox_work_func);
