...
{ \
	ssize_t len = 0; \
	struct aw_device *aw_dev = dev_get_drvdata(dev); \
	int ret = -1; \
	...
	list_for_each(pos, dev_list) { \
		...
		if (local_dev->channel == id) { \
			aw_dev = local_dev; \
			break; \
		} \
	} \
	if (local_dev == NULL) { \
		aw_pr_err("can't find dev num %d", id); \
		return -EINVAL; \
	} \
	len += snprintf(buf + len, PAGE_SIZE - len, \
		"max_temperature: [%d]\n", aw_dev->bd_record.max_temp); \
	return len; \
} \
static DEVICE_ATTR(max_temperature_##id, S_IRUGO | S_IWUSR | S_IWGRP, \
...
{ \
	...
	aw_dev_info(aw_dev->dev, "channel=%u", aw_dev->channel); \
	len += snprintf(buf + len, PAGE_SIZE - len, \
		"max_temperature_keep: [%d]\n", aw_dev->max_temperature_keep); \
	return len; \
} \
static DEVICE_ATTR(max_temperature_keep_##id, S_IRUGO | S_IWUSR | S_IWGRP, \
...
{ \
	...
	len += snprintf(buf + len, PAGE_SIZE - len, \
		"over_temperature_cnt: [%d]\n", aw_dev->bd_record.over_temp_cnt); \
	return len; \
} \
static DEVICE_ATTR(over_temperature_cnt_##id, S_IRUGO | S_IWUSR | S_IWGRP, \
...
{ \
	...
	len += snprintf(buf + len, PAGE_SIZE - len, "max_excursion: [%d] um\n", \
		aw_dev->bd_record.max_excu); \
	return len; \
} \
static DEVICE_ATTR(max_excursion_##id, S_IRUGO | S_IWUSR | S_IWGRP, \
...
{ \
	...
	len += snprintf(buf + len, PAGE_SIZE - len, \
		"over_excursion_cnt: [%d]\n", aw_dev->bd_record.over_excu_cnt); \
	return len; \
} \
static DEVICE_ATTR(over_excursion_cnt_##id, S_IRUGO | S_IWUSR | S_IWGRP, \
			audio_amp_##id##_excursion_overcount_show, NULL); \
static struct attribute *audio_amp_##id##_attr[] = { \
	&dev_attr_max_temperature_##id.attr, \
	&dev_attr_max_temperature_keep_##id.attr, \
	&dev_attr_over_temperature_cnt_##id.attr, \
	&dev_attr_max_excursion_##id.attr, \
	&dev_attr_over_excursion_cnt_##id.attr, \
	NULL, \
}


#define DECLARE_SINGLE_AMP_BIGDATA_SYSFS(id) \
static ssize_t audio_amp_temperature_max_show(struct device *dev, \
	struct device_attribute *attr, char *buf) \
{ \
	ssize_t len = 0; \
	struct aw_device *aw_dev = dev_get_drvdata(dev); \
...
static DEVICE_ATTR(max_temperature, S_IRUGO | S_IWUSR | S_IWGRP, \
			audio_amp_temperature_max_show, NULL); \
static ssize_t audio_amp_temperature_keep_max_show(struct device *dev, \
	struct device_attribute *attr, char *buf) \
{ \
	struct aw_device *aw_dev = dev_get_drvdata(dev); \
	ssize_t len = 0; \
...
static DEVICE_ATTR(max_temperature_keep, S_IRUGO | S_IWUSR | S_IWGRP, \
			audio_amp_temperature_keep_max_show, NULL); \
static ssize_t audio_amp_temperature_overcount_show(struct device *dev, \
	struct device_attribute *attr, char *buf) \
{ \
	ssize_t len = 0; \
	struct aw_device *aw_dev = dev_get_drvdata(dev); \
...
static DEVICE_ATTR(over_temperature_cnt, S_IRUGO | S_IWUSR | S_IWGRP, \
			audio_amp_temperature_overcount_show, NULL); \
static ssize_t audio_amp_excursion_max_show(struct device *dev, \
	struct device_attribute *attr, char *buf) \
{ \
	ssize_t len = 0; \
	struct aw_device *aw_dev = dev_get_drvdata(dev); \
...
static DEVICE_ATTR(max_excursion, S_IRUGO | S_IWUSR | S_IWGRP, \
			audio_amp_excursion_max_show, NULL); \
static ssize_t audio_amp_excursion_overcount_show(struct device *dev, \
	struct device_attribute *attr, char *buf) \
{ \
	ssize_t len = 0; \
	struct aw_device *aw_dev = dev_get_drvdata(dev); \
...
static DEVICE_ATTR(over_excursion_cnt, S_IRUGO | S_IWUSR | S_IWGRP, \
			audio_amp_excursion_overcount_show, NULL); \
static struct attribute *aw_bd_dev_attrs[] = { \
	&dev_attr_max_temperature.attr, \
	&dev_attr_max_temperature_keep.attr, \
	&dev_attr_over_temperature_cnt.attr, \
	&dev_attr_max_excursion.attr, \
	&dev_attr_over_excursion_cnt.attr, \
	NULL, \
}
/*bigdata*/
...
#define AW_EXCU_SHOW(excu)	((int32_t)(((int64_t)(excu) * (1000)) >> (24)))
...
#define AWINIC_CALI_TE_FILE  "/mnt/vendor/persist/factory/audio/aw_cali_t0.bin"
...
static int aw_cali_write_cali_te_to_file(int32_t cali_te, int channel)
{
	struct file *fp = NULL;
	char buf[50] = {0};
	loff_t pos = 0;
	...
	fp = filp_open(AWINIC_CALI_TE_FILE, O_RDWR | O_CREAT, 0644);
	if (IS_ERR(fp)) {
		aw_pr_err("channel:%d open %s failed, error=%ld",
		channel, AWINIC_CALI_TE_FILE, PTR_ERR(fp));
		return -EINVAL;
	}

	pos = AW_INT_DEC_DIGIT * channel;

	snprintf(buf, sizeof(buf), "%10d", cali_te);
	...
	aw_pr_info("channel:%d buf:%s cali_te:%d",
			channel, buf, cali_te);
...
static int aw_cali_get_read_cali_te(int32_t *cali_te, int channel)
{
	struct file *fp = NULL;
	/*struct inode *node;*/
	int f_size;
	char *buf = NULL;
	int32_t int_cali_te = 0;
	loff_t pos = 0;
#if !defined AW_KERNEL_VER_OVER_6_1_0
	mm_segment_t fs;
#endif
	...
	fp = filp_open(AWINIC_CALI_TE_FILE, O_RDONLY, 0);
	if (IS_ERR(fp)) {
		aw_pr_err("channel:%d open %s failed, error=%ld",
			channel, AWINIC_CALI_TE_FILE, PTR_ERR(fp));
		return -EINVAL;
	...
	if (kstrtoint(re_buf, 10, &int_cali_te) == 0)
		*cali_te = int_cali_te;
	else
		*cali_te = AW_ERRO_CALI_VALUE;

	re_buf = NULL;
	aw_pr_info("channel:%d buf:%s int_cali_te: %d",
		channel, buf, int_cali_te);
...
int aw_cali_write_te_to_nvram(int32_t cali_te, int32_t channel)
{
#ifdef AW_CALI_STORE_EXAMPLE
	...
	return aw_cali_write_cali_te_to_file(cali_te, channel);
#else
	return 0;
#endif
}

int aw882xx_cali_read_te_from_nvram(int32_t *cali_te, int32_t channel)
{
	...
	return aw_cali_get_read_cali_te(cali_te, channel);
#else
	return 0;
#endif
}
...
static int aw_cali_svc_dev_get_re(struct aw_device *aw_dev)
{
	...
	for (i = 0; i < AW_CALI_READ_RE_TIMES; i++) {
		...
		aw_dev_info(aw_dev->dev, "%d %d", i, re[i]);
		if (re[i] > max) {
			max = re[i];
		}
		sum += re[i];
	...
	if (aw_dev->cali_desc.cali_compute_type == CALI_COMPUTE_TYPE_MAX_VAL) {
		aw_dev_info(aw_dev->dev, "get re max val %d", max);
		re[0] = max;
	} else {
	...
	if (ret) {
		aw_dev_err(aw_dev->dev, "store cali_re failed %d", ret);
	}

//cali te
	ret = aw882xx_dsp_read_cali_te(aw_dev, &cali_te);
	if (ret < 0) {
		aw_dev_err(aw_dev->dev, "read cali_te failed");
		return ret;
	}
	aw_dev_info(aw_dev->dev, "read cali_te %d done", cali_te);

	aw_dev->cali_desc.cali_te = cali_te;
	ret = aw_cali_write_te_to_nvram(cali_te, aw_dev->channel);
...
static int aw_cali_svc_dev_cali_t0(struct aw_device *aw_dev)
{
	int ret = -1;

	ret = aw_dsp_write_ambient_temperature(aw_dev, &g_aw_sys_te);
	if (ret < 0) {
		aw_dev_err(aw_dev->dev, "write system temperature failed");
		return ret;
	}
	aw_dev_info(aw_dev->dev, "set system temperature %d done", g_aw_sys_te);

	ret = aw882xx_dsp_write_cali_te(aw_dev, g_aw_sys_te);
	if (ret) {
		aw_dev_err(aw_dev->dev, "write cali_te failed");
		return ret;
	}
	aw_dev_info(aw_dev->dev, "set cali_te %d done", g_aw_sys_te);

	return ret;
}

static int aw_cali_svc_devs_cali_t0(struct aw_device *aw_dev, struct list_head *dev_list)
{
	int ret = -1;
	struct list_head *pos = NULL;
	struct aw_device *local_dev = NULL;
	...
	list_for_each(pos, dev_list) {
		...
		ret = aw882xx_dsp_write_cali_te(local_dev, g_aw_sys_te);
		if (ret) {
			aw_dev_err(local_dev->dev, "write cali_te failed");
			return ret;
		}
		aw_dev_info(local_dev->dev, "set cali_te %d done", g_aw_sys_te);
	}
...
static int aw_cali_svc_dev_cali_re(struct aw_device *aw_dev, unsigned int flag)
{
	...
	aw_cali_svc_dev_cali_t0(aw_dev);
	/*wait time*/
...
static int aw_cali_svc_devs_cali_re(struct aw_device *aw_dev, unsigned int flag)
{
	...
	aw_cali_svc_devs_cali_t0(aw_dev, dev_list);
...
static int aw_cali_svc_cali_re(struct aw_device *aw_dev, bool is_single, unsigned int flag)
{
	...
	ret = aw_update_sys_temp(aw_dev, &g_aw_sys_te);
	if (ret) {
		aw_dev_err(aw_dev->dev, "get temperature from system failed!");
		return ret;
	}
	aw_dev_info(aw_dev->dev, "update sys temperature %d done", g_aw_sys_te);
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (2 == sscanf(buf, "%d %d", &databuf[0], &databuf[1])) {
		time = databuf[0];
		aw_dev->cali_desc.cali_compute_type = databuf[1];
	}
	...
	g_cali_re_time = time;
	aw_dev_dbg(aw_dev->dev, "time:%d compute type %d", time, databuf[1]);
	...
	struct device_attribute *attr, char *buf)
{
	...
	len += snprintf(buf+len, PAGE_SIZE-len,
		"time: %d %d\n", g_cali_re_time, aw_dev->cali_desc.cali_compute_type);
...
static ssize_t cali_te_store(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	int ret;
	struct aw_device *aw_dev = dev_get_drvdata(dev);
	...
	list_for_each(pos, dev_list) {
		local_dev = container_of(pos, struct aw_device, list_node);
		if (local_dev->channel == AW_DEV_CH_PRI_L) {
			aw_dev = local_dev;
			break;
		}
	}
	...
	aw_dev->cali_desc.cali_te = data;
	ret = aw_cali_write_te_to_nvram(data, aw_dev->channel);
	if (ret < 0) {
		aw_dev_err(aw_dev->dev, "set re %d failed", data);
		return -EPERM;
	}
	aw_dev_info(aw_dev->dev, "store ch[%u] cali te %d", aw_dev->channel, data);

	return count;
}

static ssize_t cali_te_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct aw_device *aw_dev = dev_get_drvdata(dev);
	ssize_t len = 0;
	...
	aw_dev_info(aw_dev->dev, "ch[%u] cali te %d", aw_dev->channel, aw_dev->cali_desc.cali_te);

	len += snprintf(buf+len, PAGE_SIZE-len, "%d",
		aw_dev->cali_desc.cali_te);
...
static ssize_t cali_time_store_1(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	uint32_t time = 0;
	struct aw_device *aw_dev = dev_get_drvdata(dev);
	...
	list_for_each(pos, dev_list) {
		local_dev = container_of(pos, struct aw_device, list_node);
		if (local_dev->channel == AW_DEV_CH_PRI_R) {
			aw_dev = local_dev;
			break;
		}
...
static ssize_t cali_time_show_1(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	ssize_t len = 0;
	struct aw_device *aw_dev = dev_get_drvdata(dev);
...
static ssize_t cali_store_1(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	int ret;
	struct aw_device *aw_dev = dev_get_drvdata(dev);
...
static ssize_t cali_te_store_1(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	int ret;
	struct aw_device *aw_dev = dev_get_drvdata(dev);
...
static ssize_t cali_te_show_1(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct aw_device *aw_dev = dev_get_drvdata(dev);
	ssize_t len = 0;
...
static ssize_t cali_re_store_1(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	int ret;
	struct aw_device *aw_dev = dev_get_drvdata(dev);
...
static ssize_t cali_re_show_1(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct aw_device *aw_dev = dev_get_drvdata(dev);
	ssize_t len = 0;
	...
	if ((aw_dev->cali_desc.cali_re >= aw_dev->re_min) &&
		...
		len += snprintf(buf+len, PAGE_SIZE-len, "re25_cal val err. val %d\n",
											aw_dev->cali_desc.cali_re);
	}
	return len;
}

static ssize_t cali_f0_store_1(struct device *dev,
	struct device_attribute *attr, const char *buf, size_t count)
{
	struct aw_device *aw_dev = dev_get_drvdata(dev);
	int ret;
...
static ssize_t cali_f0_show_1(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	int ret;
	struct aw_device *aw_dev = dev_get_drvdata(dev);
...
static ssize_t re_range_show_1(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	ssize_t len = 0;
	struct aw_device *aw_dev = dev_get_drvdata(dev);
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (1 == sscanf(buf, "%d", &databuf[0])) {
		ret = aw_cali_svc_cali_cmd(aw_dev, AW_CALI_CMD_RE, is_single_cali, CALI_OPS_HMUTE);
		if (ret < 0) {
			aw_dev_err(aw_dev->dev, "cali failed");
		}
	}

	aw_dev_err(aw_dev->dev, "done");
	return count;
}
	...
	struct device_attribute *attr, char *buf)
{
	...
	status = aw882xx_cali_svc_get_cali_status();
	len += snprintf(buf+len, PAGE_SIZE-len,
		"%s", status ? "Enable" : "Disable");

	return len;
}

static DEVICE_ATTR(status, 0644, status_show, status_store);
static DEVICE_ATTR(cali_time_0, 0644, cali_time_show, cali_time_store);
static DEVICE_ATTR(cali_0, 0200, NULL, cali_store);
static DEVICE_ATTR(re25_cal_0, 0644, cali_re_show, cali_re_store);
static DEVICE_ATTR(cali_t0_0, 0644, cali_te_show, cali_te_store);
static DEVICE_ATTR(cali_f0_0, 0644, cali_f0_show, cali_f0_store);
static DEVICE_ATTR(re_range_0, 0444, re_range_show, NULL);

static DEVICE_ATTR(cali_time_1, 0644, cali_time_show_1, cali_time_store_1);
static DEVICE_ATTR(cali_1, 0200, NULL, cali_store_1);
static DEVICE_ATTR(re25_cal_1, 0644, cali_re_show_1, cali_re_store_1);
static DEVICE_ATTR(cali_t0_1, 0644, cali_te_show_1, cali_te_store_1);
static DEVICE_ATTR(cali_f0_1, 0644, cali_f0_show_1, cali_f0_store_1);
static DEVICE_ATTR(re_range_1, 0444, re_range_show_1, NULL);

static struct attribute *aw_cal_dev_attrs[] = {
	&dev_attr_status.attr,
	NULL
};


static struct attribute *aw_cal_dev_attrs0[] = {
	&dev_attr_cali_time_0.attr,
	&dev_attr_cali_0.attr,
	&dev_attr_re25_cal_0.attr,
	&dev_attr_cali_t0_0.attr,
	&dev_attr_cali_f0_0.attr,
	&dev_attr_re_range_0.attr,
	NULL
};

static struct attribute *aw_cal_dev_attrs1[] = {
	&dev_attr_cali_time_1.attr,
	&dev_attr_cali_1.attr,
	&dev_attr_re25_cal_1.attr,
	&dev_attr_cali_t0_1.attr,
	&dev_attr_cali_f0_1.attr,
	&dev_attr_re_range_1.attr,
	NULL
};

static const struct attribute_group aw_cal_group[]= {
	{
		.attrs = aw_cal_dev_attrs0,
	},
	{
		.attrs = aw_cal_dev_attrs1,
	},
	{
		.attrs = aw_cal_dev_attrs,
	},
};

/* for samsung BigData */
#if 0
static ssize_t max_excursion_show_0(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	ssize_t len = 0;
	struct aw_device *aw_dev = dev_get_drvdata(dev);
	abox_big_data_t *big_data = aw_dev->big_data;

	aw_dev_info(aw_dev->dev, "enter");

	aw882xx_dsp_get_big_data(aw_dev, big_data, 2*sizeof(abox_big_data_t));

	len += snprintf(buf + len, PAGE_SIZE - len,
		"max_excursion: [%d]\n", (1000 * big_data->max_excursion) >> AW_DSP_BD_EXCU_SHIFT);

	return len;
}

static ssize_t max_temperature_show_0(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	ssize_t len = 0;
	int ret = 0;
	...
	aw882xx_dsp_get_big_data(aw_dev, big_data, 2*sizeof(abox_big_data_t));
	max_temp = big_data->max_temperature >> AW_DSP_BD_TEMP_SHIFT;

	len += snprintf(buf + len, PAGE_SIZE - len,
		"max_temperature: [%d]\n", max_temp);

	if (max_temp > aw_dev->max_temperature_keep)
		aw_dev->max_temperature_keep = max_temp;

	return len;
}

static ssize_t max_temperature_keep_show_0(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	ssize_t len = 0;
	int ret = 0 ;
...
static ssize_t over_excursion_cnt_show_0(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	ssize_t len = 0;
	struct aw_device *aw_dev = dev_get_drvdata(dev);
	...
	len += snprintf(buf + len, PAGE_SIZE - len,
		"over_excursion_cnt: [%d]\n", big_data->over_excursion_cnt);

	return len;
}

static ssize_t over_temperature_cnt_show_0(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	ssize_t len = 0;
	struct aw_device *aw_dev = dev_get_drvdata(dev);
	...
	len += snprintf(buf + len, PAGE_SIZE - len,
		"over_temperature_cnt: [%d]\n", big_data->over_temperature_cnt);

	return len;
}

static ssize_t max_excursion_show_1(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	ssize_t len = 0;
	struct aw_device *aw_dev = dev_get_drvdata(dev);
	...
	len += snprintf(buf + len, PAGE_SIZE - len,
		"max_excursion: [%d]\n", (1000 * big_data[1].max_excursion) >> AW_DSP_BD_EXCU_SHIFT);

	return len;
}

static ssize_t max_temperature_show_1(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	int ret;
	struct aw_device *aw_dev = dev_get_drvdata(dev);
	...
	max_temp = big_data[1].max_temperature >> AW_DSP_BD_TEMP_SHIFT;
...
static ssize_t max_temperature_keep_show_1(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	int ret;
	struct aw_device *aw_dev = dev_get_drvdata(dev);
...
static ssize_t over_excursion_cnt_show_1(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	ssize_t len = 0;
	struct aw_device *aw_dev = dev_get_drvdata(dev);
	...
	len += snprintf(buf + len, PAGE_SIZE - len,
		"over_excursion_cnt: [%d]\n", big_data[1].over_excursion_cnt);

	return len;
}

static ssize_t over_temperature_cnt_show_1(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	ssize_t len = 0;
	struct aw_device *aw_dev = dev_get_drvdata(dev);
	...
	len += snprintf(buf + len, PAGE_SIZE - len,
		"over_temperature_cnt: [%d]\n", big_data[1].over_temperature_cnt);

	return len;
}

static DEVICE_ATTR(max_excursion_0, 0444, max_excursion_show_0, NULL);
static DEVICE_ATTR(max_temperature_0, 0444, max_temperature_show_0, NULL);
static DEVICE_ATTR(max_temperature_keep_0, 0444, max_temperature_keep_show_0, NULL);
static DEVICE_ATTR(over_excursion_cnt_0, 0444, over_excursion_cnt_show_0, NULL);
static DEVICE_ATTR(over_temperature_cnt_0, 0444, over_temperature_cnt_show_0, NULL);

static DEVICE_ATTR(max_excursion_1, 0444, max_excursion_show_1, NULL);
static DEVICE_ATTR(max_temperature_1, 0444, max_temperature_show_1, NULL);
static DEVICE_ATTR(max_temperature_keep_1, 0444, max_temperature_keep_show_1, NULL);
static DEVICE_ATTR(over_excursion_cnt_1, 0444, over_excursion_cnt_show_1, NULL);
static DEVICE_ATTR(over_temperature_cnt_1, 0444, over_temperature_cnt_show_1, NULL);

static struct attribute *aw_bd_dev_attrs0[] = {
	&dev_attr_max_excursion_0.attr,
	&dev_attr_max_temperature_0.attr,
	&dev_attr_max_temperature_keep_0.attr,
	&dev_attr_over_excursion_cnt_0.attr,
	&dev_attr_over_temperature_cnt_0.attr,
	NULL
};

static struct attribute *aw_bd_dev_attrs1[] = {
	&dev_attr_max_excursion_1.attr,
	&dev_attr_max_temperature_1.attr,
	&dev_attr_max_temperature_keep_1.attr,
	&dev_attr_over_excursion_cnt_1.attr,
	&dev_attr_over_temperature_cnt_1.attr,
	NULL
};

static const struct attribute_group aw_bd_group[]= {
	{
		.attrs = aw_bd_dev_attrs0,
	},
	{
		.attrs = aw_bd_dev_attrs1,
	},	
};
#endif

static void aw_big_data_init(struct aw_device *aw_dev)
{
	aw_dev->cur_temp = 20;
	aw_dev->max_temperature_keep = 20;
	aw_dev->bd_record.max_temp = 20;
	aw_dev->bd_record.over_temp_cnt = 0;
	aw_dev->bd_record.max_excu = 0;
	aw_dev->bd_record.over_excu_cnt = 0;
	aw_dev->reset_st.first_update = false;
	aw_dev->reset_st.exec_clear = false;
	aw_dev->reset_st.temp_clear = false;

	aw_dev->bd_record.iic_abnormal_cnt = 0;
	aw_dev->bd_record.iis_abnormal_cnt = 0;
	aw_dev->bd_record.short_circuit_cnt = 0;
}

void aw_big_data_reset_st(struct aw_device *aw_dev, bool st)
{
	aw_dev_dbg(aw_dev->dev, "enter, %x", st);

	if (st) {
		aw_dev->reset_st.excu = true;
		aw_dev->reset_st.temp = true;
		aw_dev->reset_st.excu_cnt = true;
		aw_dev->reset_st.temp_cnt = true;
		aw_dev->reset_st.iic_cnt = true;
		aw_dev->reset_st.iis_cnt = true;
		aw_dev->reset_st.shorts_cnt = true;
		aw_dev->reset_st.mute_cnt = true;
	} else {
		aw_dev->reset_st.excu = false;
		aw_dev->reset_st.temp = false;
		aw_dev->reset_st.excu_cnt = false;
		aw_dev->reset_st.temp_cnt = false;
		aw_dev->reset_st.iic_cnt = false;
		aw_dev->reset_st.iis_cnt = false;
		aw_dev->reset_st.shorts_cnt = false;
		aw_dev->reset_st.mute_cnt = false;
	}
}

static void aw_big_data_record_update(struct aw_device *aw_dev)
{
	if (!aw_dev->reset_st.first_update) {
		aw_dev->max_temperature_keep = aw_dev->big_data.max_temperature;
		aw_dev->bd_record.max_temp = aw_dev->big_data.max_temperature;
		aw_dev->reset_st.first_update = true;
	}

	if (aw_dev->reset_st.exec_clear) {
		aw_dev->bd_record.max_excu = aw_dev->big_data.max_excursion;
		aw_dev->reset_st.exec_clear = false;
	}

	if (aw_dev->reset_st.temp_clear) {
		aw_dev->bd_record.max_temp = aw_dev->big_data.max_temperature;
		aw_dev->reset_st.temp_clear = false;
	}

	if (aw_dev->big_data.max_temperature > aw_dev->max_temperature_keep)
		aw_dev->max_temperature_keep = aw_dev->big_data.max_temperature;

	if (aw_dev->big_data.max_temperature > aw_dev->bd_record.max_temp) {
		aw_dev->bd_record.max_temp = aw_dev->big_data.max_temperature;
		aw_dev->bd_record.over_temp_cnt = aw_dev->big_data.over_temperature_cnt;
	}

	if (aw_dev->big_data.max_excursion > aw_dev->bd_record.max_excu) {
		aw_dev->bd_record.max_excu = aw_dev->big_data.max_excursion;
		aw_dev->bd_record.over_excu_cnt = aw_dev->big_data.over_excursion_cnt;
	}
}

static int32_t aw_data_around(int32_t data_value, int32_t data_width)
{
...
int aw_big_data_update(struct aw_device *aw_pa)
{
	int ret = -1;
	struct aw882xx *aw882xx = (struct aw882xx *)aw_pa->private_data;
	struct list_head *dev_list = NULL;
	struct list_head *pos = NULL;
	struct aw_device *local_dev = NULL;
	...
	if (!aw_pa->pstream) {
		ret = aw882xx_dsp_get_abox_big_data(aw_pa, aw882xx->big_data, 2*sizeof(struct aw_big_data));
		if (ret < 0) {
			aw_dev_err(aw_pa->dev, "get abox big data fail %d", ret);
			return ret;
		}
	} else {
		ret = aw882xx_dsp_get_big_data(aw_pa, aw882xx->big_data, 2*sizeof(struct aw_big_data));
		if (ret < 0) {
			aw_dev_err(aw_pa->dev, "get algo big data fail %d", ret);
			return ret;
		}
	}
	...
	list_for_each(pos, dev_list) {
		...
		if (local_dev->channel == AW_DEV_CH_PRI_L) {
			aw_dev = local_dev;
			aw_dev->big_data.max_excursion = AW_EXCU_SHOW(aw882xx->big_data[0].max_excursion);
			aw_dev->big_data.max_temperature = aw_data_around(aw882xx->big_data[0].max_temperature, AW_DSP_BD_TEMP_SHIFT);
			aw_dev->big_data.over_excursion_cnt = aw882xx->big_data[0].over_excursion_cnt;
			aw_dev->big_data.over_temperature_cnt = aw882xx->big_data[0].over_temperature_cnt;
			aw_big_data_record_update(aw_dev);
			if (!aw_pa->pstream) {
				aw_dev->mute_count += aw882xx->big_data[0].mute_num;
			}
		} else if (local_dev->channel == AW_DEV_CH_PRI_R) {
			aw_dev = local_dev;
			aw_dev->big_data.max_excursion = AW_EXCU_SHOW(aw882xx->big_data[1].max_excursion);
			aw_dev->big_data.max_temperature = aw_data_around(aw882xx->big_data[1].max_temperature, AW_DSP_BD_TEMP_SHIFT);
			aw_dev->big_data.over_excursion_cnt = aw882xx->big_data[1].over_excursion_cnt;
			aw_dev->big_data.over_temperature_cnt = aw882xx->big_data[1].over_temperature_cnt;
			aw_big_data_record_update(aw_dev);
			if (!aw_pa->pstream) {
				aw_dev->mute_count += aw882xx->big_data[1].mute_num;
			}
		} else {
			aw_pr_err("Not supported channel");
		}
		aw_dev_info(aw_dev->dev, "dev[%u] keep_temp[%d] max_temp[%d] over_cnt[%d] max_excu[%d] over_cnt[%d] ",
			aw_dev->channel, aw_dev->max_temperature_keep, aw_dev->bd_record.max_temp,
			aw_dev->bd_record.over_temp_cnt, aw_dev->bd_record.max_excu, aw_dev->bd_record.over_excu_cnt);
	}
...
#if 0
int aw_get_big_data(struct aw_device *aw_dev, unsigned int ch)
{
	int ret = -1;
	...
	aw_dev_info(aw_dev->dev, "enter, ch[%u]", ch);

	if (!aw_dev->pstream) {
		aw_dev_info(aw_dev->dev, "exit, no pstream");
		return 0;
	} else {
		ret = aw882xx_dsp_get_big_data(aw_dev, aw882xx->big_data, 2*sizeof(struct aw_big_data));
		if (ret < 0) {
			aw_dev_err(aw_dev->dev, "get algo big data fail %d", ret);
			return ret;
		}
	}
	...
	case AW_DEV_CH_PRI_L:
		aw_dev->big_data.max_excursion = AW_EXCU_SHOW(aw882xx->big_data[0].max_excursion);
		aw_dev->big_data.max_temperature = aw882xx->big_data[0].max_temperature >> AW_DSP_BD_TEMP_SHIFT;
		aw_dev->big_data.over_excursion_cnt = aw882xx->big_data[0].over_excursion_cnt;
		aw_dev->big_data.over_temperature_cnt = aw882xx->big_data[0].over_temperature_cnt;
		break;
	case AW_DEV_CH_PRI_R:
		aw_dev->big_data.max_excursion = AW_EXCU_SHOW(aw882xx->big_data[1].max_excursion);
		aw_dev->big_data.max_temperature = aw882xx->big_data[1].max_temperature >> AW_DSP_BD_TEMP_SHIFT;
		aw_dev->big_data.over_excursion_cnt = aw882xx->big_data[1].over_excursion_cnt;
		aw_dev->big_data.over_temperature_cnt = aw882xx->big_data[1].over_temperature_cnt;

		break;
	default:
		aw_dev_err(aw_dev->dev, "unsupport ch[%u]", ch);
		return -EINVAL;
	}

	max_temp = aw_dev->big_data.max_temperature;
	if (max_temp > aw_dev->max_temperature_keep)
		aw_dev->max_temperature_keep = max_temp;

	aw_dev_info(aw_dev->dev, "temp_keep=%d, max_temp=%d, max_excu=%d",
		aw_dev->max_temperature_keep, max_temp, max_excu);

	return ret;
}
#endif

int aw_big_data_callback(unsigned int type, unsigned int ch)
{
	int ret = -1;
	struct list_head *dev_list = NULL;
	struct list_head *pos = NULL;
	...
	if (aw_dev == NULL) {
		aw_pr_err("can't find dev id %d", ch);
		return -EINVAL;
	}

	if (aw_dev->reset_st.excu && aw_dev->reset_st.temp &&
		aw_dev->reset_st.excu_cnt && aw_dev->reset_st.temp_cnt &&
		aw_dev->reset_st.mute_cnt) {
		aw_dev_info(aw_dev->dev,"got bigdata already");
	} else {
		if (aw_dev->pstream)
			aw_big_data_update(aw_dev);
	}

	switch (type) {
	case AW_MAX_EXCU:
		aw_dev->reset_st.exec_clear = true;
		if (aw_dev->reset_st.excu) {
			aw_dev_info(aw_dev->dev,"got max_excu already, reset 0");
			return 0;
		} else {
			aw_dev_info(aw_dev->dev,"max_excu[%d]", aw_dev->bd_record.max_excu);
			aw_dev->reset_st.excu = true;
			return aw_dev->bd_record.max_excu;
		}
		break;
	case AW_MAX_TEMP:
		aw_dev->reset_st.temp_clear = true;
		if (aw_dev->reset_st.temp) {
			aw_dev_info(aw_dev->dev,"got max_temp already, reset 0");
			return 0;
		} else {
			aw_dev_info(aw_dev->dev,"max_temp[%d]", aw_dev->bd_record.max_temp);
			aw_dev->reset_st.temp = true;
			return aw_dev->bd_record.max_temp;
		}
		break;
	case AW_MAX_EXCU_CNT:
		ret = aw_dev->bd_record.over_excu_cnt;
		aw_dev->bd_record.over_excu_cnt = 0;
		if (aw_dev->reset_st.excu_cnt) {
			aw_dev_info(aw_dev->dev,"got over_excu_cnt already, reset 0");
			return 0;
		} else {
			aw_dev_info(aw_dev->dev, "over_excu_cnt[%d]", ret);
			aw_dev->reset_st.excu_cnt = true;
			return ret;
		}
		break;
	case AW_MAX_TEMP_CNT:
		ret = aw_dev->bd_record.over_temp_cnt;
		aw_dev->bd_record.over_temp_cnt = 0;
		if (aw_dev->reset_st.temp_cnt) {
			aw_dev_info(aw_dev->dev,"got over_temp_cnt already, reset 0");
			return 0;
		} else {
			aw_dev_info(aw_dev->dev, "over_temp_cnt[%d]", ret);
			aw_dev->reset_st.temp_cnt = true;
			return ret;
		}
		break;

	case AW_IIC_ABNORMAL_CNT:
		ret = aw_dev->bd_record.iic_abnormal_cnt;
		aw_dev->bd_record.iic_abnormal_cnt = 0;
		if (aw_dev->reset_st.iic_cnt) {
			aw_dev_info(aw_dev->dev,"got iic_abnormal_cnt already, reset 0");
			return 0;
		} else {
			aw_dev_info(aw_dev->dev, "iic_abnormal_cnt[%d]", ret);
			aw_dev->reset_st.iic_cnt = true;
			return ret;
		}
		break;
	case AW_IIS_ABNORMAL_CNT:
		ret = aw_dev->bd_record.iis_abnormal_cnt;
		aw_dev->bd_record.iis_abnormal_cnt = 0;
		if (aw_dev->reset_st.iis_cnt) {
			aw_dev_info(aw_dev->dev,"got iis_abnormal_cnt already, reset 0");
			return 0;
		} else {
			aw_dev_info(aw_dev->dev, "iis_abnormal_cnt[%d]", ret);
			aw_dev->reset_st.iis_cnt = true;
			return ret;
		}
		break;

	case AW_SHORT_CIRCUIT_CNT:
		ret = aw_dev->bd_record.short_circuit_cnt;
		aw_dev->bd_record.short_circuit_cnt = 0;
		if (aw_dev->reset_st.shorts_cnt) {
			aw_dev_info(aw_dev->dev,"got short_circuit_cnt already, reset 0");
			return 0;
		} else {
			aw_dev_info(aw_dev->dev, "short_circuit_cnt[%d]", ret);
			aw_dev->reset_st.shorts_cnt = true;
			return ret;
		}
		break;
	case AW_MAX_TEMP_KEEP:
		aw_dev_info(aw_dev->dev,"max_temp_keep[%d]", aw_dev->max_temperature_keep);
		return aw_dev->max_temperature_keep;
		break;
	default:
		aw_dev_err(aw_dev->dev, "unsupport ch[%u]", ch);
		return -EINVAL;
...
EXPORT_SYMBOL_GPL(aw_big_data_callback);
...
static struct attribute_group aw_bd_group[AW_AMP_ID_MAX] = {
	[AW_AMP_0] = {.attrs = audio_amp_0_attr, },
	[AW_AMP_1] = {.attrs = audio_amp_1_attr, },
	[AW_AMP_2] = {.attrs = audio_amp_2_attr, },
	[AW_AMP_3] = {.attrs = audio_amp_3_attr, },
};

DECLARE_SINGLE_AMP_BIGDATA_SYSFS(0);

static const struct attribute_group aw_bd_single_group= {
	.attrs = aw_bd_dev_attrs,
};
...
static struct device *g_aw_cal_dev = NULL;
static struct device *g_aw_bd_dev = NULL;
...
static void aw_cali_class_attr_init(struct aw_device *aw_dev)
{
	...
	if (!g_aw_cal_dev) {
		...
		g_aw_cal_dev = device_create(&aw_cali_class, NULL, 0,
					aw_dev, "%s", "awinic_cal");
		if (!g_aw_cal_dev) {
			aw_dev_info(aw_dev->dev, "error creating awinic_cal");
		}

		g_aw_bd_dev = device_create(&aw_cali_class, NULL, 0,
					aw_dev, "%s", "awinic_bd");
		if (!g_aw_bd_dev) {
			aw_dev_info(aw_dev->dev, "error creating awinic_bd");
		}
	...
	if (aw_dev->channel == AW_DEV_CH_PRI_L) {
		ret = sysfs_create_group(&g_aw_cal_dev->kobj, &aw_cal_group[0]);
		if (ret < 0) {
			aw_dev_err(aw_dev->dev, "error creating sysfs cal attr files");
		}
		ret = sysfs_create_group(&g_aw_cal_dev->kobj, &aw_cal_group[2]);
		if (ret < 0) {
			aw_dev_err(aw_dev->dev, "error creating sysfs cal attr files");
		}
	} else if (aw_dev->channel == AW_DEV_CH_PRI_R) {
		ret = sysfs_create_group(&g_aw_cal_dev->kobj, &aw_cal_group[1]);
		if (ret < 0) {
			aw_dev_err(aw_dev->dev, "error creating sysfs cal attr files");
		}
	}

	/*create dynamic Big data nodes*/
	if (aw882xx->num_amp == 1) {
		ret = sysfs_create_group(&g_aw_bd_dev->kobj, &aw_bd_single_group);
		if (ret)
			aw_dev_err(aw_dev->dev, "error creating sysfs bd attr files");
		else
			aw_dev_info(aw_dev->dev, "create class node done");
		return;
	}

	switch (aw_dev->channel) {
	case AW_DEV_CH_PRI_L:
		ret = sysfs_create_group(&g_aw_bd_dev->kobj, &aw_bd_group[AW_DEV_CH_PRI_L]);
		if (ret)
			aw_dev_err(aw_dev->dev, "error creating sysfs bd attr files");
		break;
	case AW_DEV_CH_PRI_R:
		ret = sysfs_create_group(&g_aw_bd_dev->kobj, &aw_bd_group[AW_DEV_CH_PRI_R]);
		if (ret)
			aw_dev_err(aw_dev->dev, "error creating sysfs bd attr files");
		break;
	case AW_DEV_CH_SEC_L:
		ret = sysfs_create_group(&g_aw_bd_dev->kobj, &aw_bd_group[AW_DEV_CH_SEC_L]);
		if (ret)
			aw_dev_err(aw_dev->dev, "error creating sysfs bd attr files");
		break;
	case AW_DEV_CH_SEC_R:
		ret = sysfs_create_group(&g_aw_bd_dev->kobj, &aw_bd_group[AW_DEV_CH_SEC_R]);
		if (ret)
			aw_dev_err(aw_dev->dev, "error creating sysfs bd attr files");
		break;
	default:
		aw_dev_err(aw_dev->dev, "not support ch[%u] sysfs bd attr", aw_dev->channel);
	}
...
static void aw_cali_class_attr_deinit(struct aw_device *aw_dev)
{
	...
	/*rm attr node*/
	if (aw_dev->channel == AW_DEV_CH_PRI_L) {
		sysfs_remove_group(&g_aw_cal_dev->kobj, &aw_cal_group[0]);
		sysfs_remove_group(&g_aw_cal_dev->kobj, &aw_cal_group[2]);
	} else if (aw_dev->channel == AW_DEV_CH_PRI_R) {
		sysfs_remove_group(&g_aw_cal_dev->kobj, &aw_cal_group[1]);
	}

	/*remove dynamic Big data nodes*/
	if (aw882xx->num_amp == 1) {
		sysfs_remove_group(&g_aw_bd_dev->kobj, &aw_bd_single_group);
	} else {
		if (aw_dev->channel == 0) {
			for (i = 0; i < aw882xx->num_amp; i++) {
				aw_dev_info(aw_dev->dev, "sysfs_remove_group i=[%d]", i);
				sysfs_remove_group(&g_aw_bd_dev->kobj, &aw_bd_group[i]);
			}
		}
	}

	device_destroy(&aw_cali_class, 0);
...
static int aw_cali_misc_params_ptr(struct aw_device *aw_dev, struct ptr_params_data *p_params)
{
	...
	if (p_params->len < 0 || p_params->len > PAGE_SIZE) {
		aw_dev_err(aw_dev->dev, "p_params len[%d] out of range", p_params->len);
		ret = -EFAULT;
		return ret;
	}
...
static int aw_get_dev_addr_ch(struct aw_device *aw_dev, aw_ioctl_msg_t *msg)
{
	int ret = 0;
	struct aw882xx *aw882xx = (struct aw882xx *)aw_dev->private_data;
	...
	data_ptr = kzalloc(data_len, GFP_KERNEL);
	if (!data_ptr) {
		aw_dev_err(aw_dev->dev, "data_ptr is NULL");
		return -ENOMEM;
	}

	info[0] = i2caddr;
	info[1] = aw_dev->channel;
	aw_dev_info(aw_dev->dev, "id[0x%x], i2caddr[0x%x], ch[%d], info[0x%x][%d]",
			dsp_msg_id, i2caddr, aw_dev->channel,info[0],info[1]);

	if (copy_to_user((void __user *)user_data,
		(char *)info, data_len)) {
		aw_dev_err(aw_dev->dev, "copy_to_user failed");
		ret = -EFAULT;
	}
...
static int aw_cali_misc_read_dsp(struct aw_device *aw_dev, aw_ioctl_msg_t *msg)
{
	...
	if (dsp_msg_id == 0x0000000F) {
		ret = aw_get_dev_addr_ch(aw_dev, msg);
		return ret;
	}

	if (data_len < 0 || data_len > PAGE_SIZE) {
		aw_dev_err(aw_dev->dev, "msg data_len[%d] out of range", data_len);
		return -EFAULT;
	}
...
int aw882xx_cali_init(struct aw_cali_desc *cali_desc)
{
	...
	cali_desc->cali_q = 0;
	cali_desc->status = 0;
	cali_desc->cali_te = 200;	/*means 20.0 Celsius*/

	aw_big_data_init(aw_dev);
	...
	cali_desc->cali_compute_type = CALI_COMPUTE_TYPE_MAX_VAL;
