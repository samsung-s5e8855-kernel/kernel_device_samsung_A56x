...
#define MIPI_DSI_OPMODE_FLAGS	(MIPI_DSI_MODE_VIDEO|MIPI_DSI_CLOCK_NON_CONTINUOUS)
...
enum exynos_panel_cmd {
	...
	PANEL_CLOSE_CMDS,
	PANEL_OPEN_CMDS,
	PANEL_MODE_SWITCH_CMDS,
	PANEL_FREQ_STEP_CHANGE_CMDS,
	PANEL_UPDATE_CMDS,
	NUM_OF_CMDS,
};
...
struct exynos_panel_funcs {
	...
	void (*change_freq_step)(struct exynos_panel *exynos_panel,
				 const struct exynos_freq_step *freq_step);
	void (*panel_update)(struct exynos_panel *exynos_panel);
#if IS_ENABLED(CONFIG_DRM_PANEL_MCD_COMMON)
	/*
	 *@ set_clcok_info:
	 *
...
#define BRIGHTNESS_MAX_QUEUE_SZ 256
struct brightness_queue {
	u32 queue[BRIGHTNESS_MAX_QUEUE_SZ];
	int front;
	int rear;
	int fsz;
};

struct brightness_desc {
	u32 max;
	u32 cur;
	u32 min;
	bool endian_swap;
	bool commit_supported;
	bool enable_dimming;
	u32 dimming_step;
	struct brightness_queue dimq;
};
...
struct exynos_panel_desc {
	...
	struct brightness_desc brightness;
	...
	size_t num_freq_steps;
	const struct exynos_freq_step *freq_steps;
...
#if IS_ENABLED(CONFIG_DRM_PANEL_MCD_COMMON)
	const char *dqe_xml_suffix;
	const char *aiqe_xml_suffix;
#endif
};
...
struct exynos_panel {
	...
	/* @exynos_panel_off_begin: An indicator of whether panel-disable is triggerd. */
	bool panel_off_trigger;
	/**
	...
	u32 cur_br;
	bool enable_dimming_brightness;
#if IS_ENABLED(CONFIG_DRM_PANEL_MCD_COMMON)
	bool enabled;
...
static inline bool is_panel_enabled(struct exynos_panel *ctx)
{
	return (is_panel_active(ctx) || is_panel_idle(ctx));
}
