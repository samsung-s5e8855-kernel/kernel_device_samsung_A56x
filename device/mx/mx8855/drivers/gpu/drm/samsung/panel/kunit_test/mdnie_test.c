...
extern struct class *mdnie_class;
...
extern int mdnie_create_class();
extern int mdnie_destroy_class();
...
{
	KUNIT_EXPECT_EQ(test, mdnie_destroy_class(), -EINVAL);
}
...
static void mdnie_test_mdnie_create_and_destroy_class_success(struct kunit *test)
{
	...
	KUNIT_EXPECT_NOT_ERR_OR_NULL(test, mdnie_class);

	KUNIT_EXPECT_EQ(test, mdnie_destroy_class(), 0);
	KUNIT_EXPECT_TRUE(test, !mdnie_class);

	/* create 'mdnie-1' class */
	KUNIT_ASSERT_EQ(test, mdnie_set_name(mdnie, 1), 0);
	KUNIT_ASSERT_STREQ(test, mdnie_get_name(mdnie), "mdnie-1");

	KUNIT_EXPECT_EQ(test, mdnie_create_class(), 0);
	KUNIT_EXPECT_NOT_ERR_OR_NULL(test, mdnie_class);
...
static void mdnie_test_mdnie_destroy_device_fail_if_mdnie_device_is_not_exist(struct kunit *test)
{
	...
	KUNIT_EXPECT_EQ(test, mdnie_create_class(), 0);
	KUNIT_ASSERT_NOT_ERR_OR_NULL(test, mdnie_class);
...
static void mdnie_test_mdnie_create_and_destroy_device_success(struct kunit *test)
{
	...
	KUNIT_ASSERT_EQ(test, mdnie_create_class(), 0);
	KUNIT_ASSERT_NOT_ERR_OR_NULL(test, mdnie_class);
...
static void mdnie_test_mdnie_create_and_remove_device_files_success(struct kunit *test)
{
	...
	KUNIT_EXPECT_FALSE(test, attr_exist_for_each(mdnie_class, "mode"));
	KUNIT_EXPECT_FALSE(test, attr_exist_for_each(mdnie_class, "scenario"));
	KUNIT_EXPECT_FALSE(test, attr_exist_for_each(mdnie_class, "accessibility"));
	KUNIT_EXPECT_FALSE(test, attr_exist_for_each(mdnie_class, "bypass"));
	KUNIT_EXPECT_FALSE(test, attr_exist_for_each(mdnie_class, "lux"));
	KUNIT_EXPECT_FALSE(test, attr_exist_for_each(mdnie_class, "mdnie"));
	KUNIT_EXPECT_FALSE(test, attr_exist_for_each(mdnie_class, "sensorRGB"));
	KUNIT_EXPECT_FALSE(test, attr_exist_for_each(mdnie_class, "whiteRGB"));
	KUNIT_EXPECT_FALSE(test, attr_exist_for_each(mdnie_class, "night_mode"));
	KUNIT_EXPECT_FALSE(test, attr_exist_for_each(mdnie_class, "color_lens"));
	KUNIT_EXPECT_FALSE(test, attr_exist_for_each(mdnie_class, "hdr"));
	KUNIT_EXPECT_FALSE(test, attr_exist_for_each(mdnie_class, "light_notification"));
	KUNIT_EXPECT_FALSE(test, attr_exist_for_each(mdnie_class, "mdnie_ldu"));
#ifdef CONFIG_USDM_PANEL_HMD
	KUNIT_EXPECT_FALSE(test, attr_exist_for_each(mdnie_class, "hmt_color_temperature"));
#endif
#ifdef CONFIG_USDM_MDNIE_AFC
	KUNIT_EXPECT_FALSE(test, attr_exist_for_each(mdnie_class, "afc"));
#endif

	KUNIT_EXPECT_EQ(test, mdnie_create_device_files(mdnie), 0);

	/* check sysfs existence */
	KUNIT_EXPECT_TRUE(test, attr_exist_for_each(mdnie_class, "mode"));
	KUNIT_EXPECT_TRUE(test, attr_exist_for_each(mdnie_class, "scenario"));
	KUNIT_EXPECT_TRUE(test, attr_exist_for_each(mdnie_class, "accessibility"));
	KUNIT_EXPECT_TRUE(test, attr_exist_for_each(mdnie_class, "bypass"));
	KUNIT_EXPECT_TRUE(test, attr_exist_for_each(mdnie_class, "lux"));
	KUNIT_EXPECT_TRUE(test, attr_exist_for_each(mdnie_class, "mdnie"));
	KUNIT_EXPECT_TRUE(test, attr_exist_for_each(mdnie_class, "sensorRGB"));
	KUNIT_EXPECT_TRUE(test, attr_exist_for_each(mdnie_class, "whiteRGB"));
	KUNIT_EXPECT_TRUE(test, attr_exist_for_each(mdnie_class, "night_mode"));
	KUNIT_EXPECT_TRUE(test, attr_exist_for_each(mdnie_class, "color_lens"));
	KUNIT_EXPECT_TRUE(test, attr_exist_for_each(mdnie_class, "hdr"));
	KUNIT_EXPECT_TRUE(test, attr_exist_for_each(mdnie_class, "light_notification"));
	KUNIT_EXPECT_TRUE(test, attr_exist_for_each(mdnie_class, "mdnie_ldu"));
#ifdef CONFIG_USDM_PANEL_HMD
	KUNIT_EXPECT_TRUE(test, attr_exist_for_each(mdnie_class, "hmt_color_temperature"));
#endif
#ifdef CONFIG_USDM_MDNIE_AFC
	KUNIT_EXPECT_TRUE(test, attr_exist_for_each(mdnie_class, "afc"));
#endif
