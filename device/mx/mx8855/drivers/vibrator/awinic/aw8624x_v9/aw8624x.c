 ...
 *
 ******************************************************/
#define AW8624X_DRIVER_VERSION "v0.0.0.4"
...
static int aw8624x_i2c_reads(struct aw8624x *aw8624x, uint8_t reg_addr, uint8_t *buf, uint32_t len)
{
	...
	while (cnt < AW8624X_I2C_RETRIES) {
		...
		if (ret < 0) {
			aw_err("transfer failed, cnt=%u", cnt);
			//return ret;
		} else if (ret != AW8624X_I2C_READ_MSG_NUM) {
			aw_err("transfer failed(size error), cnt=%u", cnt);
			//return -ENXIO;
		}
		if (ret == AW8624X_I2C_READ_MSG_NUM) {
			break;
		} else {
			cnt++;
			usleep_range(2000, 2500);
...
static int aw8624x_i2c_writes(struct aw8624x *aw8624x, uint8_t reg_addr, uint8_t *buf, uint32_t len)
{
	...
	while (cnt < AW8624X_I2C_RETRIES) {
		...
		if (ret < 0) {
			aw_err("i2c master send err, cnt=%u", cnt);
			cnt++;
			usleep_range(2000, 2500);
			continue;
		}
 ...
 */
static void aw8624x_reg_unlock(struct aw8624x *aw8624x, bool flag)
{
	uint8_t reg_val = 0;

	if (flag) {
		/* Unlock register */
		reg_val = AW8624X_BIT_TMCFG_TM_UNLOCK;
		aw8624x_i2c_write(aw8624x, AW8624X_REG_TMCFG, reg_val);
	} else {
		/* Lock register */
		reg_val = AW8624X_BIT_TMCFG_TM_LOCK;
		aw8624x_i2c_write(aw8624x, AW8624X_REG_TMCFG, reg_val);
	}
}
...
static uint8_t aw8624x_calculate_cali_data_range(struct aw8624x *aw8624x, uint32_t f0_temp)
{
	char f0_cali_lra = 0;
	int f0_cali_step = 0;
	uint8_t f0_cali_data = 0;

	f0_cali_step = 100000 * ((int)f0_temp - (int)aw8624x->dts_info.f0_pre) /
				((int)aw8624x->dts_info.f0_pre * 24);
	aw_info("f0_cali_step=%d", f0_cali_step);
	if (f0_cali_step >= 0) {	/*f0_cali_step >= 0 */
		if (f0_cali_step % 10 >= 5)
	...
	if (f0_cali_step >= 64)
		...
		f0_cali_lra = (char)f0_cali_step + 64;
	/* update cali step */
	f0_cali_data = f0_cali_lra;

	aw_info("f0_cali_data = 0x%02X", f0_cali_data);
...
static int aw8624x_parse_dt(struct device *dev, struct aw8624x *aw8624x, struct device_node *np)
{
	...
	val = of_property_read_u32(np, "vib_adjust_f0_min", &aw8624x->dts_info.f0_ajust_min);
	if (val != 0)
		aw_err("aw8624x_vib_adjust_f0_min not found");

	val = of_property_read_u32(np, "vib_adjust_f0_max", &aw8624x->dts_info.f0_ajust_max);
	if (val != 0)
		aw_err("aw8624x_vib_vib_adjust_f0_max not found");
...
static void aw8624x_vibrator_work_routine(struct work_struct *work)
{
	...
	if (aw8624x->state) {
		if (aw8624x->activate_mode == AW8624X_RAM_LOOP_MODE) {
			if (aw8624x->f0_cali_data == aw8624x->f0_cali_adjust_min)
				aw8624x->f0_cali_data = 0x1;
			else if (aw8624x->f0_cali_data >= 0x0 && aw8624x->f0_cali_data <= aw8624x->f0_cali_adjust_max)
				aw8624x->f0_cali_data += 2;
			aw_info("RAM LOOP 5, change f0_cali_data:0x%02x", aw8624x->f0_cali_data);
		}
		aw8624x_upload_lra(aw8624x, AW8624X_F0_CALI_LRA);
		if (aw8624x->activate_mode == AW8624X_RAM_LOOP_MODE) {
			aw8624x->f0_cali_data = aw8624x->sample_param[8];
			aw_info("RAM LOOP 5, restore f0_cali_data:0x%02x", aw8624x->f0_cali_data);
		}
		if (aw8624x->activate_mode == AW8624X_RAM_MODE) {
...
static void aw8624x_rtp_work_routine(struct work_struct *work)
{
	...
	if (aw8624x->state) {
		...
		vfree(aw8624x->aw_rtp);
		aw8624x->aw_rtp = NULL;
		if (aw8624x->rtp_file_num == 8) {
			if (aw8624x->f0 >= aw8624x->dts_info.f0_ajust_min && aw8624x->f0 <= aw8624x->dts_info.f0_ajust_max)
				aw8624x->f0 += 8;
			aw_info("RTP 8, change f0:%d", aw8624x->f0);
		}
		...
		if (!aw8624x->aw_rtp) {
			...
			if (aw8624x->rtp_file_num == 8) {
				aw8624x->f0 = aw8624x->sample_param[5];
				aw_info("RTP 8, restore f0:%d", aw8624x->f0);
			}
			mutex_unlock(&aw8624x->lock);
			return;
		}
...
static void aw8624x_misc_para_init(struct aw8624x *aw8624x)
{
	...
	aw_info("enter");
	aw8624x_reg_unlock(aw8624x, true);
	...
	aw8624x_reg_unlock(aw8624x, false);
	/* GAIN_BYPASS config */
	aw8624x_i2c_write_bits(aw8624x, AW8624X_REG_SYSCTRL5,
...
/* init CONTCFG1 */
	...
	aw8624x->f0_cali_adjust_min = aw8624x_calculate_cali_data_range(aw8624x, aw8624x->dts_info.f0_ajust_min);
	aw_info("aw8624x->f0_cali_adjust_min 0x%02x", aw8624x->f0_cali_adjust_min);
	aw8624x->f0_cali_adjust_max = aw8624x_calculate_cali_data_range(aw8624x, aw8624x->dts_info.f0_ajust_max);
	aw_info("aw8624x->f0_cali_adjust_max 0x%02x", aw8624x->f0_cali_adjust_max);
}
...
static int aw8624x_i2c_probe(struct i2c_client *i2c)
{
	int ret = -1;
...
#if defined(CONFIG_AW8624X_SAMSUNG_FEATURE) && defined(CONFIG_AW8624X_USE_BOOST_EN)
