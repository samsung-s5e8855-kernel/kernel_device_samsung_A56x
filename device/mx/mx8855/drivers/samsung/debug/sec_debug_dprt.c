...
static int init_sdg_bdev(void)
{
	...
	bdev = blkdev_get_by_path(bdev_path, mode, NULL, NULL);
	if (IS_ERR(bdev)) {
		dev_t devt = 0;

		pr_err("%s: plan b\n", __func__);
		/* TODO : please check if name_to_dev_t is needed */
...
static int secdbg_part_buf_alloc(struct dprt_buf_data *buf_data, size_t size)
{
	char *buf = vzalloc(size);
...
static void secdbg_part_buf_free(struct dprt_buf_data *buf_data)
{
	char *buf = buf_data->buf;
...
static void secdbg_part_buf_putnul(struct dprt_buf_data *buf_data)
{
	buf_data->buf[buf_data->size - 1] = '\0';
}

static struct dprt_buf_data dhst_buf;
...
static int secdbg_part_dhst_open(struct inode *inode, struct file *file)
{
	...
	ret = secdbg_part_buf_alloc(&dhst_buf, bufsize);
	if (ret) {
	...
	if (!sdg_part_read(dhst_buf.buf, p_dh->size, p_dh->offset_in_part)) {
		pr_err("%s: fail to read (DHST/%x/%x)\n", __func__,
			p_dh->size, p_dh->offset_in_part);
	}

	offset = strnlen(dhst_buf.buf, dhst_buf.size);

	if (!sdg_part_read((dhst_buf.buf + offset), p_br->size, p_br->offset_in_part)) {
		pr_err("%s: fail to read (BORE/%x/%x)\n", __func__,
			p_br->size, p_br->offset_in_part);
	}

	secdbg_part_buf_putnul(&dhst_buf);

	if (!strnlen(dhst_buf.buf, dhst_buf.size)) {
		pr_err("%s: nothing to write\n", __func__);
		secdbg_part_buf_free(&dhst_buf);
		mutex_unlock(&dhst_buf_mutex);
		return -EINVAL;
	}
...
static ssize_t secdbg_part_proc_read(struct dprt_buf_data *buf_data, struct file *file, char __user *buf,
					size_t count, loff_t *ppos)
{
	size_t size;
	char *src = buf_data->buf;

	if (!src)
		return 0;

	size = strnlen(src, buf_data->size);
	if (!size)
		return 0;

	return simple_read_from_buffer(buf, count, ppos, src, size);
}
...
static struct dprt_buf_data bore_buf;
...
static int secdbg_part_bore_open(struct inode *inode, struct file *file)
{
	...
	ret = secdbg_part_buf_alloc(&bore_buf, bufsize);
	if (ret) {
		pr_err("%s: fail to alloc, %x\n", __func__, bufsize);
	...
	if (!sdg_part_read(bore_buf.buf, p->size, p->offset_in_part)) {
		pr_err("%s: fail to read (BORE/%x/%x)\n", __func__,
			p->size, p->offset_in_part);
	}

	secdbg_part_buf_putnul(&bore_buf);

	if (!strnlen(bore_buf.buf, bore_buf.size)) {
		pr_err("%s: nothing to write\n", __func__);
		secdbg_part_buf_free(&bore_buf);
		mutex_unlock(&bore_buf_mutex);
		return -EINVAL;
	}
...
static struct dprt_buf_data summ_buf;
...
static int secdbg_part_summ_open(struct inode *inode, struct file *file)
{
	...
	ret = secdbg_part_buf_alloc(&summ_buf, bufsize);
	if (ret) {
		pr_err("%s: fail to alloc, %x\n", __func__, bufsize);
	...
	if (!sdg_part_read(summ_buf.buf, p->size, p->offset_in_part)) {
		pr_err("%s: fail to read (SUMM/%x/%x)\n", __func__,
			p->size, p->offset_in_part);
	}

	secdbg_part_buf_putnul(&summ_buf);

	if (!strnlen(summ_buf.buf, summ_buf.size)) {
		pr_err("%s: nothing to write\n", __func__);
		secdbg_part_buf_free(&summ_buf);
		mutex_unlock(&summ_buf_mutex);
		return -EINVAL;
	}
