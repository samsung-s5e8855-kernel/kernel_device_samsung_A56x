...
static struct device_attribute dio8018_attrs[] = {
	__ATTR(dio8018_write, 0644, dio8018_write_show, dio8018_write_store),
	__ATTR(dio8018_read, 0644, dio8018_read_show, dio8018_read_store),
};
...
static int dio8018_create_sysfs(struct dio8018_data *dio8018)
{
...
#if IS_ENABLED(CONFIG_DRV_SAMSUNG)
	dio8018_pmic = sec_device_create(dio8018, device_name);
	...
	for (i = 0; i < ATTR_REGULATOR; i++) {
		err = device_create_file(dio8018_pmic, &dio8018_attrs[i]);
		if (err)
...
remove_pmic_device:
	for (i--; i >= 0; i--)
		device_remove_file(dio8018_pmic, &dio8018_attrs[i]);
#if IS_ENABLED(CONFIG_DRV_SAMSUNG)
	sec_device_destroy(dio8018_pmic->devt);
#else
