...
static int get_display_info(struct GetDisplayInfo_cmd *cmd, struct GetDisplayInfo_rsp *rsp)
{
	(void)cmd;
...
static int open_driver(struct OpenPeripheral_cmd *cmd, struct OpenPeripheral_rsp *rsp)
{
	...
	pr_debug(TUIHW_LOG_TAG " %s >>  cmd->flags=%X, cmd->num=%d size=%zu\n",
		__func__, cmd->flags, cmd->num, sizeof(struct OpenPeripheral_cmd));
	for (i = 0; i < cmd->num; i++)
...
static int close_driver(struct ClosePeripheral_cmd *cmd)
{
	int i = 0;
...
static int connecting_thread(void *data)
{
	...
	while (atomic_cmpxchg(&thread_flag, 1, 1) == 1
		...
		cmd.task_id    = -1;
		cmd.SetDrvInf_cmd.drv_tui_mode = stui_check_mode(STUI_MODE_ALL);
		cmd.SetDrvInf_cmd.index        = TUILL_TUIHW;
		...
		while (ret >= 0) {
			...
			case TUILL_ICMD_GET_DISPLAY_INFO:
				...
				rsp.ret_code = get_display_info(&cmd.GetDisplayInf_cmd, &rsp.GetDisplayInf_rsp);
				break;
			case TUILL_ICMD_OPEN_DRIVER:
				pr_debug(TUIHW_LOG_TAG " received TUILL_ICMD_OPEN_DRIVER\n");
				rsp.ret_code = open_driver(&cmd.OpenPeriph_cmd, &rsp.OpenPeriph_rsp);
				break;
			case TUILL_ICMD_CLOSE_DRIVER:
				pr_debug(TUIHW_LOG_TAG " received TUILL_ICMD_CLOSE_DRIVER\n");
				rsp.ret_code = close_driver(&cmd.ClosePeriph_cmd);
				break;
...
int iwd_cancel_session(void)
{
	...
	cmd.CnclTUI_cmd.event = TEE_EVENT_TUI_REE_CANCEL;
	ret = tz_iwsock_write(sd, &cmd, sizeof(cmd), 0);
